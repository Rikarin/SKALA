...
public readonly struct XmlDocOptions {
    public XmlDocOptions(in FormattingOptions options) {
        ...
        // `skala_xmldoc_max_line_length = 0` and again at `1` the oracle puts every single word
        ...
        // ⚠ The *C#* indent, not `skala_xmldoc_indent_size`, and this is measured. On
        // `/// <remarks>` holding a `/// <para>`, under `OracleProfile.DocComments`:
        // `skala_xmldoc_indent_size = 1` leaves the child at four columns, while `indent_size = 1`
        // moves it to one and `indent_size = 2` (with `tab_width = 8` in the same run, so the tab
        // width could not be answering) moves it to two. `skala_xmldoc_indent_style = tab` leaves
    ...
    /// <summary><c>skala_xmldoc_wrap_lines</c>: the master switch for width-driven wrapping.</summary>
    public bool WrapLines { get; }

    /// <summary>
    ///     <c>skala_xmldoc_max_line_length</c>: the column the line must fit in, measured from
    ...
    ///     keep their shape. Implementing <c>skala_xmldoc_wrap_text</c> without deciding this is not possible, which
    ///     is why it is in the implemented set rather than the refused one.
    /// </remarks>
    public bool KeepUserLinebreaks { get; }

    /// <summary><c>skala_xmldoc_max_blank_lines_between_tags</c>: the export sets 0.</summary>
    public int MaxBlankLinesBetweenTags { get; }

    /// <summary><c>skala_xmldoc_indent_child_elements</c>: inside an element with no text.</summary>
    /// <remarks>
    ...
    /// <summary><c>skala_xmldoc_indent_text</c>: inside an element that contains text.</summary>
    public ChildIndentStyle IndentText { get; }

    /// <summary>
    ///     <c>skala_xmldoc_linebreaks_inside_tags_for_elements_with_child_elements</c>: whether
    ...
    /// <summary>
    ///     <c>skala_xmldoc_linebreaks_inside_tags_for_multiline_elements</c>: whether an element
    ...
    /// <summary><c>skala_xmldoc_linebreak_before_multiline_elements</c>.</summary>
    public bool LinebreakBeforeMultilineElements { get; }

    /// <summary><c>skala_xmldoc_linebreak_before_singleline_elements</c>.</summary>
    public bool LinebreakBeforeSinglelineElements { get; }

    /// <summary>
    ///     <c>skala_xmldoc_spaces_inside_tags</c>: <c>&lt;summary&gt; Text &lt;/summary&gt;</c>.
    /// </summary>
    /// <remarks>
    ...
    /// <summary><c>skala_xmldoc_space_before_self_closing</c>: <c>&lt;br/&gt;</c> or <c>&lt;br /&gt;</c>.</summary>
    public bool SpaceBeforeSelfClosing { get; }

    /// <summary>
    ///     <c>skala_space_after_triple_slash</c>, live only inside the sub-formatter.
    /// </summary>
    /// <remarks>
    ...
    ///     fixture could ever pin it; <c>constructs/xmldoc/skala_space_after_triple_slash.cs</c>
    ...
    /// <summary>
    ///     <c>skala_xmldoc_space_after_last_attribute</c>: <c>&lt;param name="a" &gt;</c>.
    /// </summary>
    /// <remarks>
    ///     ⚠ Start tags only. A self-closing tag's gap is <c>skala_xmldoc_space_before_self_closing</c>'s, and the
    ...
    /// <summary>
    ///     <c>skala_xmldoc_spaces_around_eq_in_attribute</c>: <c>name = "a"</c> or <c>name="a"</c>.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    ///     <c>skala_xmldoc_blank_line_after_pi</c>: a blank <c>///</c> line after a
    ///     <c>&lt;?…?&gt;</c>.
    /// </summary>
    /// <remarks>
    ...
    ///         <c>skala_xmldoc_max_blank_lines_between_tags</c>'s blank lines do not carry one: an empty line's
    ...
    ///         probed at <c>skala_xmldoc_max_blank_lines_between_tags = 1</c>, <em>those</em> blank lines carry the
    ...
    /// <summary>
    ///     <c>skala_xmldoc_linebreaks_inside_tags_for_elements_longer_than</c>: open an element up
    ...
    /// </summary>
    /// <remarks>
    ///     ⚠ <b>Not <c>skala_xmldoc_indent_size</c>, which the oracle does not read.</b> On
    ///     `/// &lt;remarks&gt;` holding a `/// &lt;para&gt;`, under <c>OracleProfile.DocComments</c>:
    ///     <c>skala_xmldoc_indent_size = 1</c> leaves the child at four columns, while
    ...
    /// <summary>
    ///     <c>skala_xmldoc_linebreak_before_elements</c>: the export lists
    ...
    /// <summary>
    ///     The indent unit, as text. ⚠ Always spaces, and <c>skala_xmldoc_indent_style</c> does not
    ///     change that.
    /// </summary>
    /// <remarks>
    ///     ⚠ Measured on the same fixture as <see cref="IndentSize" />:
    ///     <c>skala_xmldoc_indent_style = tab</c> leaves the child element indented with four
...
public static class XmlDocIds {
    ...
    public static ImmutableArray<KeyValuePair<string, string>> Refused => [
        ...
        new(
            "skala_xmldoc_insert_final_newline",
            "A '///' comment has no file end to put a newline at — and that is now a diff rather than an argument. Measured under OracleProfile.DocComments at both values on a file carrying a type-level <summary>, a member <remarks>/<returns> pair and a trailing member comment that ends the type: one output. skala_xmldoc_wrap_text = false moves the same file in the same run."
        ),
        ...
        // run: `skala_xmldoc_blank_line_after_pi = false` removes the blank `///` line after
        ...
        new(
            "skala_xmldoc_pi_attributes_indent",
        ...
        new(
            "skala_xmldoc_space_after_last_pi_attribute",
            """⚠ Not pending — measured inert in the oracle, at both values, on an instruction written tight ('third="3"?>') and one written with the space ('epsilon="epsilonvalue" ?>'). The oracle copies the instruction's bytes, '?>' and whatever precedes it included. Control in the same run: skala_xmldoc_blank_line_after_pi = false."""
        ),
        new(
            "skala_xmldoc_spaces_around_eq_in_pi_attribute",
            """⚠ Not pending — measured inert in the oracle, asked on the shape the key would have to normalise: a '=' written with a space on each side, `<?skala-probe first = "1" …?>`, beside one written tight. The spaces survive at `false` and none is added at `true`. Control in the same run: skala_xmldoc_blank_line_after_pi = false."""
        )
    ];
}
