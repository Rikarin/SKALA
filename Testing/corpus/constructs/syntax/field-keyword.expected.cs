...
class FieldKeyword {
    ...
    public string Overflowing {
        get =>
            field
            ?? throw new InvalidOperationException("the overflowing property was read before anything assigned it");
        set =>
            field = value is { Length: > 0 } supplied ? supplied : throw new ArgumentException("empty", nameof(value));
    }
