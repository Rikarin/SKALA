...
ref struct ScopedAndRefReadonly {
    ...
    static void Mixed(scoped ReadOnlySpan<char> text, ref readonly int origin, scoped ref int cursor, out int written) =>
        written = 0;

    static int Overflowing(scoped ReadOnlySpan<char> text, scoped ref int cursor, ref readonly int origin, int alpha, int bravo) =>
