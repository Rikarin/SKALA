...
unsafe class UnsafeAndFunctionPointers {
    ...
    delegate* unmanaged[Stdcall, MemberFunction, SuppressGCTransition]<int, byte*, nint, double, long, void> Overflowing;
    ...
    static int Chained(Node* node) => node->Next->Next->Next->Value + node->Next->Value + node->Value + node->Next->Next->Value;
}
