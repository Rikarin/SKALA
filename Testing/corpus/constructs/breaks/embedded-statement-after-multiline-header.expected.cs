...
public class EmbeddedStatementAfterMultilineHeader {
    ...
    void StatementPushedOff(bool c, bool d, int n, int[] xs, int depth) {
        ...
        if (c.ToString().Length > 0 && d.ToString().Length > 0)
            throw new System.InvalidOperationException("a message long enough");

        while (c && d)
            Frobnicate(xs.Length, xs.Length, xs.Length, xs.Length, xs.Length, xs.Length, xs.Length, xs.Length);
