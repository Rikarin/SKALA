...
/// <inheritdoc />
public void Configure(GameplayModuleBuilder builder){ArgumentNullException.ThrowIfNull(builder);builder.DependsOn<GameplayKernelModule>().Definition<ExperienceCurveDefinition>().Definition<TalentTreeDefinition>().Definition<SpecialisationDefinition>().Definition<ProfessionDefinition>().Definition<ReputationDefinition>().Tag(ProfessionRoot).Tag(FactionRoot).Tag(SpecialisationRoot);}}
