...
using Vixen.Core;namespace Vixen.Gameplay.Progression;
...
///     </para>
/// </remarks>
[DataContract("ExperienceCurveDefinition" )]public sealed record ExperienceCurveDefinition:Definition{
...
[DataContract("TalentPrerequisite" )]public sealed class TalentPrerequisiteDefinition{
...
[DataContract("TalentNode" )]public sealed class TalentNodeDefinition{
...
[DataContract("TalentTreeDefinition" )]public sealed record TalentTreeDefinition:Definition{
/// <summary>What it is called in the UI.</summary>
public string DisplayName{get;set;}=string.Empty;
/// <summary>Its nodes.</summary>
public List<TalentNodeDefinition>Nodes{get;set;}=[];
/// <inheritdoc />
public override void CollectTags(ICollection<string>tags){ArgumentNullException.ThrowIfNull(tags);foreach(var node in Nodes){foreach(var tag in node.GrantsTags){tags.Add(tag);}}}}
...
/// </remarks>
[DataContract("SpecialisationDefinition" )]public sealed record SpecialisationDefinition:Definition{
/// <summary>What it is called in the UI.</summary>
public string DisplayName{get;set;}=string.Empty;
...
public List<ModifierDefinition>Modifiers{get;set;}=[];
/// <inheritdoc />
public override void CollectTags(ICollection<string>tags){ArgumentNullException.ThrowIfNull(tags);if(Tag.Length>0){tags.Add(Tag);}foreach(var requirement in Requirements){if(requirement.Kind!=RequirementKind.Value&&requirement.Subject.Length>0){tags.Add(requirement.Subject);}}}}
/// <summary>One step of a profession's skill line.</summary>
[DataContract("ProfessionTier" )]public sealed class ProfessionTierDefinition{
...
[DataContract("ProfessionDefinition" )]public sealed record ProfessionDefinition:Definition{
/// <summary>What it is called in the UI.</summary>
public string DisplayName{get;set;}=string.Empty;
...
/// <inheritdoc />
public override void CollectTags(ICollection<string>tags){ArgumentNullException.ThrowIfNull(tags);if(Tag.Length>0){tags.Add(Tag);}foreach(var tier in Tiers){if(tier.Tag.Length>0){tags.Add(tier.Tag);}}}}
/// <summary>One standing with a faction.</summary>
[DataContract("ReputationRank" )]public sealed class ReputationRankDefinition{
...
[DataContract("ReputationDefinition" )]public sealed record ReputationDefinition:Definition{
/// <summary>What it is called in the UI.</summary>
public string DisplayName{get;set;}=string.Empty;
...
/// <inheritdoc />
public override void CollectTags(ICollection<string>tags){ArgumentNullException.ThrowIfNull(tags);if(Tag.Length>0){tags.Add(Tag);}foreach(var rank in Ranks){if(rank.Tag.Length>0){tags.Add(rank.Tag);}}}}
