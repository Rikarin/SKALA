...
using Vixen.Core;namespace Vixen.Gameplay.Economy;
/// <summary>One row of a vendor's stock list.</summary>
[DataContract("VendorStock" )]public sealed class VendorStockDefinition{
...
[DataContract("VendorDefinition" )]public sealed record VendorDefinition:Definition{
...
/// <inheritdoc />
public override void CollectTags(ICollection<string>tags){ArgumentNullException.ThrowIfNull(tags);if(Tag.Length>0){tags.Add(Tag);}foreach(var row in Stock){foreach(var requirement in row.Requires){if(requirement.Kind!=RequirementKind.Value&&requirement.Subject.Length>0){tags.Add(requirement.Subject);}}}}}
/// <summary>One stock row, compiled.</summary>
public sealed class VendorStock{internal VendorStock(VendorStockDefinition definition,int index,RequirementSet requirements){Definition=definition;Index=index;Requirements=requirements;Item=DefId.From(definition.Item);Currency=DefId.From(definition.Currency);}
...
public sealed class Vendor{readonly VendorStock[]stock;internal Vendor(VendorDefinition definition,GameplayTag tag,VendorStock[]stock){Definition=definition;Tag=tag;this.stock=stock;}
/// <summary>What it was compiled from.</summary>
...
///     </para>
/// </remarks>
public sealed class VendorState{readonly int[]remaining;readonly float[]restockAt;readonly Dictionary<PlayerId,List<BuybackEntry>>buyback=[];
/// <summary>Makes a fresh one with everything in stock.</summary>
    /// <param name="vendor">Which vendor.</param>
public VendorState(Vendor vendor){ArgumentNullException.ThrowIfNull(vendor);Vendor=vendor;remaining=new int[vendor.Stock.Length];restockAt=new float[vendor.Stock.Length];for(var row=0;row<remaining.Length;row++){remaining[row]=vendor.Stock[row].Quantity;}}
...
public int Remaining(int row)=>(uint)row>=(uint)remaining.Length?0:Vendor.Stock[row].IsUnlimited?-1:remaining[row];
/// <summary>What a vendor is holding for somebody.</summary>
    /// <param name="player">Who.</param>
    /// <returns>Their buyback list, newest first.</returns>
public IReadOnlyList<BuybackEntry>BuybackFor(PlayerId player)=>buyback.TryGetValue(player,out var entries)?entries:[];
/// <summary>Puts back whatever has restocked.</summary>
    /// <param name="now">The clock.</param>
    /// <returns>How many rows came back.</returns>
public int Restock(float now){var restocked=0;for(var row=0;row<remaining.Length;row++){var entry=Vendor.Stock[row];if(entry.IsUnlimited||entry.RestockSeconds<=0f||remaining[row]>=entry.Quantity){continue;}if(now<restockAt[row]){continue;}remaining[row]=entry.Quantity;restocked++;}return restocked;}
...
public VendorRefusal CanBuy(int row,int count,IRequirementContext?context){if((uint)row>=(uint)remaining.Length){return VendorRefusal.UnknownStock;}if(count<=0){return VendorRefusal.BadCount;}var entry=Vendor.Stock[row];if(!entry.IsUnlimited&&remaining[row]<count){return VendorRefusal.OutOfStock;}if(context is not null&&!entry.Requirements.IsMetBy(context)){return VendorRefusal.Requirements;}return VendorRefusal.None;}
/// <summary>Buys something.</summary>
    ...
    /// </remarks>
public VendorRefusal Buy(PlayerId buyer,int row,int count,IEconomyLedger ledger,IRequirementContext?context,float now,string operation){ArgumentNullException.ThrowIfNull(ledger);var refusal=CanBuy(row,count,context);if(refusal!=VendorRefusal.None){return refusal;}var entry=Vendor.Stock[row];var vendor=EconomyAccount.Of(EconomyAccount.Vendor);var account=EconomyAccount.Of(buyer);var cost=entry.Price*count;var result=ledger.Post(new($"vendor/{Vendor.Id.Value}/{row}/{operation}" ,[new(account,entry.Currency,-cost),new(vendor,entry.Currency,cost),new(vendor,entry.Item,-count),new(account,entry.Item,count)],$"{buyer} buys {count} of {entry.Definition.Item}" ));if(!result.Ok){return result.Verdict==EconomyVerdict.Insufficient?VendorRefusal.Insufficient:VendorRefusal.Refused;}if(result.Verdict==EconomyVerdict.Applied&&!entry.IsUnlimited){remaining[row]-=count;if(remaining[row]<entry.Quantity&&entry.RestockSeconds>0f){restockAt[row]=now+entry.RestockSeconds;}}return VendorRefusal.None;}
/// <summary>Sells something to the vendor, which records it for buyback.</summary>
    ...
    /// <returns>The refusal, or <see cref="VendorRefusal.None" />.</returns>
public VendorRefusal Sell(PlayerId seller,DefId item,int count,DefId currency,long price,IEconomyLedger ledger,string operation){ArgumentNullException.ThrowIfNull(ledger);if(count<=0||price<0){return VendorRefusal.BadCount;}var vendor=EconomyAccount.Of(EconomyAccount.Vendor);var account=EconomyAccount.Of(seller);var result=ledger.Post(new($"vendor/{Vendor.Id.Value}/sell/{operation}" ,[new(account,item,-count),new(vendor,item,count),new(vendor,currency,-price),new(account,currency,price)],$"{seller} sells {count} of {item}" ));if(!result.Ok){return result.Verdict==EconomyVerdict.Insufficient?VendorRefusal.Insufficient:VendorRefusal.Refused;}if(result.Verdict!=EconomyVerdict.Applied||Vendor.BuybackSlots==0){return VendorRefusal.None;}if(!buyback.TryGetValue(seller,out var entries)){entries=[];buyback.Add(seller,entries);}entries.Insert(0,new(item,count,currency,price)); // The oldest falls off the end, which is what makes it a window rather than a second bank.
while(entries.Count>Vendor.BuybackSlots){entries.RemoveAt(entries.Count-1);}return VendorRefusal.None;}
/// <summary>Buys something back at exactly what it was sold for.</summary>
    ...
    /// </remarks>
public VendorRefusal BuyBack(PlayerId player,int slot,IEconomyLedger ledger,string operation){ArgumentNullException.ThrowIfNull(ledger);if(!buyback.TryGetValue(player,out var entries)||(uint)slot>=(uint)entries.Count){return VendorRefusal.NothingToBuyBack;}var entry=entries[slot];var vendor=EconomyAccount.Of(EconomyAccount.Vendor);var account=EconomyAccount.Of(player);var result=ledger.Post(new($"vendor/{Vendor.Id.Value}/buyback/{operation}" ,[new(account,entry.Currency,-entry.Paid),new(vendor,entry.Currency,entry.Paid),new(vendor,entry.Item,-entry.Count),new(account,entry.Item,entry.Count)],$"{player} buys back {entry.Count} of {entry.Item}" ));if(!result.Ok){return result.Verdict==EconomyVerdict.Insufficient?VendorRefusal.Insufficient:VendorRefusal.Refused;}if(result.Verdict==EconomyVerdict.Applied){entries.RemoveAt(slot);}return VendorRefusal.None;}}
