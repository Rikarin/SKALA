...
using System.Diagnostics.CodeAnalysis;using Vixen.Core;using Vixen.Editor.Core;using Vixen.Ui;namespace Vixen.Editor.AssetEditors;
...
///     </para>
/// </remarks>
public sealed class AssetEditorRegistry{readonly Dictionary<string,IAssetEditorFactory>byExtension=new(StringComparer.OrdinalIgnoreCase);readonly Dictionary<string,IAssetEditorFactory>byName=new(StringComparer.Ordinal);
...
public AssetEditorRegistry Add(IAssetEditorFactory editor){ArgumentNullException.ThrowIfNull(editor);if(!byName.TryAdd(editor.Name,editor)){throw new InvalidOperationException($"Both {byName[editor.Name].GetType().Name} and {editor.GetType().Name} are called " +$"'{editor.Name}'. Two editors under one name make a menu that cannot say which is which." );}foreach(var extension in editor.Extensions){if(!byExtension.TryAdd(extension,editor)){throw new InvalidOperationException($"Both '{byExtension[extension].Name}' and '{editor.Name}' claim '{extension}'. One of them " +"has to give it up; whichever registered first is not a rule anyone can rely on." );}}return this;}
...
public bool TryGetForFile(string path,[NotNullWhen(true)]out IAssetEditorFactory?editor){ArgumentException.ThrowIfNullOrEmpty(path);var extension=Path.GetExtension(path);if(extension.Length==0){editor=null;return false;}return byExtension.TryGetValue(extension,out editor);}
/// <summary>Finds an editor by name.</summary>
    /// <param name="name">The name.</param>
    /// <param name="editor">The editor.</param>
    /// <returns>Whether this build has it.</returns>
public bool TryGetByName(string name,[NotNullWhen(true)]out IAssetEditorFactory?editor){ArgumentException.ThrowIfNullOrEmpty(name);return byName.TryGetValue(name,out editor);}
/// <summary>Opens an asset, or brings the document already editing it forward.</summary>
    ...
    ///     </para>
    /// </remarks>
public event Action<EditorDocument>?Opened;public bool TryOpen(EditorProject project,AssetId asset,[NotNullWhen(true)]out EditorDocument?opened){ArgumentNullException.ThrowIfNull(project);if(project.TryGetDocument(asset,out var existing)){project.Activate(existing);opened=existing;return true;}if(!project.Assets.TryGetByGuid(asset,out var entry)||entry.IsFolder){opened=null;return false;}if(!TryGetForFile(entry.Path,out var editor)){opened=null;return false;}opened=editor.Open(new(project,asset,project.Paths.Absolute(entry.Path)));Opened?.Invoke(opened);return true;}}
