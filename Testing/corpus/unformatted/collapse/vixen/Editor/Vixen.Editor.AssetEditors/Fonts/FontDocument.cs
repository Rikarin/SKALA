...
using Vixen.Core;using Vixen.Core.Yaml;using Vixen.Editor.Core;using Vixen.Ui.Text;namespace Vixen.Editor.AssetEditors.Fonts;
...
///     </para>
/// </remarks>
[DataContract("FontAsset" )]public sealed class FontAsset{
...
public static FontAsset FromYaml(string yaml){ArgumentNullException.ThrowIfNull(yaml);if(yaml.Trim().Length==0){return new();}var font=YamlSerializer.Parse<FontAsset>(yaml);return font.Version<=Current?font:throw new NotSupportedException($"This font is version {font.Version} and this build reads {Current}." );}
...
[DataContract("FontRange" )]public sealed class FontRangeData{
...
/// <summary>What a font asset is written as.</summary>
public const string Extension=FontAsset.Extension;readonly List<FontFace>loaded=[];
...
public IReadOnlyList<FontFace>Fallbacks=>fallbacks;readonly List<FontFace>fallbacks=[];
...
public FontDocument(EditorProject project,AssetId asset,string path):base(project,asset,Path.GetFileName(path)){ArgumentException.ThrowIfNullOrEmpty(path);AssetPath=path;try{Font=FontAsset.FromYaml(AssetFile.Read(path));}catch(Exception exception)when(exception is YamlBindingException or YamlParseException or NotSupportedException){Font=new();LoadError=exception.Message;}if(Font.Name.Length==0){Font.Name=Path.GetFileNameWithoutExtension(path);}Resolve();}
/// <summary>Loads the faces the asset names, replacing whatever was loaded before.</summary>
    ...
    /// </remarks>
public void Resolve(){foreach(var face in loaded){face.Dispose();}loaded.Clear();fallbacks.Clear();List<string>problems=[];Face=Load(Font.Face,Font.FaceIndex,problems);foreach(var fallback in Font.Fallbacks){if(Load(fallback,0,problems)is{}face){fallbacks.Add(face);}}Problems=problems;}FontFace?Load(AssetId asset,int index,List<string>problems){if(asset.IsEmpty){return null;}if(!Project.Assets.TryGetByGuid(asset,out var entry)){problems.Add($"{asset} is not in this project." );return null;}try{var face=FontFace.Load(File.ReadAllBytes(Project.Paths.Absolute(entry.Path)),index,entry.Path);loaded.Add(face);return face;}catch(Exception exception)when(exception is IOException or UnauthorizedAccessException or InvalidDataException or NotSupportedException){problems.Add($"{entry.Path}: {exception.Message}" );return null;}}
...
public void Edit(string name,Action<FontAsset>change,bool reloads=false){ArgumentException.ThrowIfNullOrEmpty(name);ArgumentNullException.ThrowIfNull(change);var before=Font.ToYaml();change(Font);var after=Font.ToYaml();if(string.Equals(before,after,StringComparison.Ordinal)){return;} // ⚠ Undo by reparsing the text rather than by remembering which member moved. The asset is a
...
Stack.Execute(new DelegateCommand(name,_=>Apply(after,reloads),_=>Apply(before,reloads)));}void Apply(string yaml,bool reloads){Font=FontAsset.FromYaml(yaml);if(reloads){Resolve();}Changed?.Invoke(this);}
...
public IReadOnlyList<FontCoverage>Coverage(bool includeFallbacks=true){List<FontCoverage>rows=[];foreach(var(name,first,last)in Blocks){var covered=0;var assigned=0;for(var code=first;code<=last;code++){if(char.GetUnicodeCategory((char)code)==System.Globalization.UnicodeCategory.OtherNotAssigned){continue;}assigned++;if(Supports(code,includeFallbacks)){covered++;}}rows.Add(new(name,first,last,covered,assigned));}return rows;}
...
public bool Supports(int codePoint,bool includeFallbacks=true){if(Face?.Supports(codePoint)==true){return true;}return includeFallbacks&&fallbacks.Any(face=>face.Supports(codePoint));}
/// <summary>Which face in the chain draws a code point, or −1 for none.</summary>
    ...
    /// </remarks>
public int Resolves(int codePoint){if(Face?.Supports(codePoint)==true){return 0;}for(var index=0;index<fallbacks.Count;index++){if(fallbacks[index].Supports(codePoint)){return index+1;}}return-1;}
/// <summary>The blocks this editor reports on.</summary>
    ...
    /// </remarks>
static readonly(string Name,int First,int Last)[]Blocks=[("Basic Latin" ,0x0020,0x007E),("Latin-1 Supplement" ,0x00A0,0x00FF),("Latin Extended-A" ,0x0100,0x017F),("Latin Extended-B" ,0x0180,0x024F),("Greek and Coptic" ,0x0370,0x03FF),("Cyrillic" ,0x0400,0x04FF),("General Punctuation" ,0x2000,0x206F),("Currency Symbols" ,0x20A0,0x20BF),("Arrows" ,0x2190,0x21FF),("Mathematical Operators" ,0x2200,0x22FF),("Box Drawing" ,0x2500,0x257F),("Geometric Shapes" ,0x25A0,0x25FF),("Miscellaneous Symbols" ,0x2600,0x26FF)];
/// <inheritdoc />
protected override void SaveCore()=>AssetFile.Write(AssetPath,Font.ToYaml());
/// <inheritdoc />
protected override void OnClosed(){base.OnClosed();foreach(var face in loaded){face.Dispose();}loaded.Clear();fallbacks.Clear();Face=null;}}
