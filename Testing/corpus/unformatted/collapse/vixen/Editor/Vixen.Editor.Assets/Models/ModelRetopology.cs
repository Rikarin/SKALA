...
using Vixen.Core.Mathematics;using Vixen.Geometry;using Vixen.Geometry.Remeshing;using Vixen.Geometry.Uv;using Vixen.Rendering;namespace Vixen.Editor.Assets.Models;
...
public readonly record struct MeshResult(MeshData Mesh,bool Remeshed,bool Unwrapped,IReadOnlyList<string>Messages);
...
public static MeshResult Run(MeshData mesh,ModelImportSettings settings,IReadOnlyList<RemeshGuide>?guides=null){ArgumentNullException.ThrowIfNull(mesh);ArgumentNullException.ThrowIfNull(settings);var messages=new List<string>();var wants=settings.Unwrap switch{UnwrapMode.Always=>true,UnwrapMode.WhenMissing=>mesh.TexCoords.Length!=mesh.Positions.Length||mesh.TexCoords.Length==0,_=>false};if(!settings.Retopologize&&!wants){return new(mesh,false,false,messages);}if(mesh.Indices.Length==0){return new(mesh,false,false,messages);}var kernel=ModelGeometry.ToEditMesh(mesh);var remeshed=false;if(settings.Retopologize){var quads=Remesher.Remesh(kernel,settings.ToRemeshSettings(guides),out var report);messages.AddRange(report.Warnings);if(quads.IsEmpty){messages.Add($"'{mesh.Name}' was not retopologised and was kept as it arrived." );}else{kernel=quads;remeshed=true;messages.Add($"'{mesh.Name}' retopologised to {report.QuadCount} quads, " +$"max deviation {report.MaxDeviation:0.####} of the diagonal." );}}if(!wants){return new(remeshed?ModelGeometry.ToMeshData(kernel,mesh.Name):mesh,remeshed,false,messages);}var(coordinates,unwrapped)=Unwrap(kernel,settings,mesh.Name,messages);if(!remeshed&&!unwrapped){return new(mesh,false,false,messages);}return new(ModelGeometry.ToMeshData(kernel,mesh.Name,coordinates),remeshed,unwrapped,messages);}
/// <summary>docs/plan/42 § D1's three stages, in order, over one mesh.</summary>
    ...
    /// </remarks>
static(Vector2[]Coordinates,bool Unwrapped)Unwrap(EditMesh mesh,ModelImportSettings settings,string name,List<string>messages){try{var charts=UvUnwrap.Charts(mesh,settings.ToUvSettings());var islands=UvUnwrap.Flatten(mesh,charts,settings.ToUvSettings());var placements=UvUnwrap.Pack(islands,settings.ToPackSettings(),out var report);messages.AddRange(report.Warnings);messages.Add($"'{name}' unwrapped into {report.ChartCount} charts, " +$"{report.EffectiveEfficiency:P0} of the atlas used after margin." );return(ModelGeometry.Atlas(mesh,islands,placements),true);}catch(Exception failure)when(failure is InvalidOperationException or ArgumentException){messages.Add($"'{name}' was not unwrapped: {failure.Message}" );return([],false);}}
...
public static RemeshGuide ToGuide(Spline spline,float strength=1f,int samples=DefaultGuideSamples){ArgumentNullException.ThrowIfNull(spline);ArgumentOutOfRangeException.ThrowIfLessThan(samples,2);var points=new Vector3[samples];var length=spline.Length;for(var index=0;index<samples;index++){points[index]=spline.EvaluateAtDistance(length*index/(samples-1));}return new(points,Math.Clamp(strength,0f,1f));}
