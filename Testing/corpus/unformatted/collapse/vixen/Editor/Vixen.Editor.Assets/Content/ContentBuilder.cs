...
public sealed class ContentBuilder {
    ...
    public ContentBuildResult Build(
        ...
        var diagnostics = ImmutableArray.CreateBuilder<ImportDiagnostic>(); // Sorted here rather than trusted from the caller: the whole determinism story rests on this
        ...
        var entries = ordered.Where(asset => bundleOf.ContainsKey(asset.Address))
    ...
    static string BundleNameFor(
        ...
        switch (group.Packing) { case BundlePacking.PackSeparately: // Hashed rather than the address itself: an address contains slashes and may contain
                ...
                return $"{group.Name}_{ContentHash.Compute(System.Text.Encoding.UTF8.GetBytes(asset.Address))
                    .ToString()[..12]}"; case BundlePacking.PackTogetherByLabel: {
    ...
    static string Join(string prefix, string fileName) =>
        prefix.Length == 0 ? fileName : prefix.EndsWith('/') ? prefix + fileName : $"{prefix}/{fileName}";
}
