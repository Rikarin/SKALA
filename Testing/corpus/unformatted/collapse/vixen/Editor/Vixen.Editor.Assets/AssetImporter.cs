...
using Vixen.Core.Reflection;using Vixen.Core.Yaml.Meta;namespace Vixen.Editor.Assets;
...
/// </remarks>
[AttributeUsage(AttributeTargets.Class)]public sealed class ImporterAttribute(params string[]extensions):Attribute{
...
///     </para>
/// </remarks>
public abstract class AssetImporter<TSettings>:IAssetImporter where TSettings:class,IImportSettings,new(){
/// <inheritdoc />
public abstract int Version{get;}
/// <inheritdoc />
public Type SettingsType=>typeof(TSettings);
/// <inheritdoc />
public string Name=>TypeRegistry.TryGet<TSettings>(out var descriptor)?descriptor.Alias:throw new InvalidOperationException($"{typeof(TSettings).Name} has no descriptor, so this importer has no name. Give it " +"[DataContract], which is also what makes it the tag in a .meta file." );
/// <inheritdoc />
public IReadOnlyList<string>Extensions=>(Attribute.GetCustomAttribute(GetType(),typeof(ImporterAttribute))as ImporterAttribute)?.Extensions??throw new InvalidOperationException($"{GetType().Name} has no [Importer] attribute, so nothing knows which files it claims." );
/// <inheritdoc />
public IImportSettings CreateSettings()=>new TSettings();
/// <inheritdoc />
public ValueTask<ImportResult>ImportAsync(ImportContext context,CancellationToken cancellationToken=default){ArgumentNullException.ThrowIfNull(context);return context.Settings is TSettings settings?ImportAsync(context,settings,cancellationToken):throw new ArgumentException($"{Name} was handed {context.Settings.GetType().Name} rather than {typeof(TSettings).Name}." ,nameof(context));}
/// <summary>Imports one asset.</summary>
    /// <param name="context">Everything it is allowed to read, and everything it must declare.</param>
    /// <param name="settings">How this asset is configured, with per-target overrides already applied.</param>
    /// <param name="cancellationToken">Cancels the import.</param>
    /// <returns>What it produced.</returns>
protected abstract ValueTask<ImportResult>ImportAsync(ImportContext context,TSettings settings,CancellationToken cancellationToken);}
