...
using System.Globalization;using Vixen.Core;using Vixen.Core.Mathematics;using Vixen.Core.Yaml;using Vixen.Ecs;using Vixen.Editor.Assets.Scenes;using Vixen.Editor.Core.Scenes;using Vixen.Engine.Transforms;using Vixen.Physics;using Vixen.Physics.Ecs;using Vixen.Rendering;using Vixen.Rendering.Water;using Vixen.Water;using Vixen.Water.Physics;using Xunit;namespace Vixen.Editor.Assets.Tests;
...
///     </para>
/// </remarks>
public sealed class WaterSceneRunsTests{const float Step=1f/60f;
                        ...
                        roots:
                          ...
                          - id: 0000000000000000000000000000c3c3
                            ...
                            components:
                              ...
                              - !BuoyancyState
                                wet: 0
                        """ ;public WaterSceneRunsTests(){ // The module initializers, on `SceneWaterComponentTests`' terms: a type named only inside a
// YAML string is a type whose declaring assembly may never have been loaded.
_=WaterZoneComponent.Default;_=BuoyancyBody.Default;}
/// <summary>
    ...
    /// </remarks>
[Fact]public void ACrateAuthoredInASceneFloatsOnTheWaterAuthoredBesideIt(){using var world=new World();var created=Instantiate(world);var dinghy=created[2];using var physics=new PhysicsScene(world); // The rigid body, which is what a scene cannot yet carry on its own — see the class remarks.
world.Add(dinghy,Collider.Of(physics.Shapes.Sphere(0.75f)));world.Add(dinghy,RigidBody.Dynamic()with{Mass=400f,AllowSleeping=false});var zones=Fold(world);var buoyancy=new BuoyancySystem(physics,zones);for(var index=0;index<900;index++){ // ⚠ Where `WaterClockSystem` would be, in `EarlyUpdate` — before anything reads it.
zones.WaterTime=index*Step;physics.Synchronize(Step);buoyancy.Step(world);physics.Step(Step);physics.Writeback();}var state=world.Read<BuoyancyState>(dinghy);var settled=world.Read<LocalTransform>(dinghy).Position;Assert.True(state.IsFloating,"the crate authored in the scene never touched the water" );var query=zones.QueryAt(new(settled.X,settled.Z));Assert.NotNull(query); // Within a hull radius of the surface the fold reports: the crate is *at* the waterline
// rather than sunk to the bed twelve metres down or hovering above the swell.
var surface=query.Height(new(settled.X,settled.Z),zones.WaterTime);Assert.InRange(settled.Y,surface-0.75f,surface+0.75f);}
/// <summary>The zone's <c>waveAsset</c> resolves to the committed file, and the sea is not flat.</summary>
    ...
    /// </remarks>
[Fact]public void TheZonesSeaStateComesOffDiskAndTheSurfaceMoves(){using var world=new World();Instantiate(world);var zones=Fold(world);Assert.Equal(1,zones.ZoneCount);Assert.Equal(1,zones.BodyCount);Assert.Equal(0,zones.ZonelessBodies);Assert.Equal(0,zones.UnresolvedBodies);Assert.Equal(0,zones.UnresolvedWaves);var query=zones.QueryAt(Vector2.Zero);Assert.NotNull(query);Assert.True(query.MaximumAmplitude>0f,"the sea state off disk sums to a dead flat sea" ); // Two times, one surface: the swell has to actually move, which is the half a still clock
...
static WaterZoneSystem Fold(World world){ // The transforms first: the fold rasterises a body where `WorldTransform` says it is, and a
// scene that has just been instantiated has only written the local ones.
new TransformSystem().Resolve(world);var zones=new WaterZoneSystem(new RenderView("test")){Splines=new PondSpline(),Waves=new FileWaves(), // A basin: the bed is well below the surface everywhere, so the shoreline is the body's
// own falloff rather than the ground rising to meet it.
Ground=new FlatWaterGround(-12f)};zones.Fold(world);return zones;}static Entity[]Instantiate(World world){var problems=new List<string>();var content=SceneCompiler.Compile(SceneFile.FromYaml(Pond),(severity,message)=>{if(severity==ImportSeverity.Error){problems.Add(message);}});Assert.Empty(problems);Assert.NotNull(content);var created=new Entity[3];content.Instantiate(world,created);return created;}
/// <summary>The one curve the scene names, which a project would keep as a <c>.vxspline</c>.</summary>
sealed class PondSpline:IWaterSplineSource{public Spline?SplineFor(string name,in Matrix4x4 placement){if(!string.Equals(name,"water/pond" ,StringComparison.Ordinal)){return null;}var transform=placement;Vector3[]ring=[new(-24f,0f,-24f),new(24f,0f,-24f),new(24f,0f,24f),new(-24f,0f,24f)];for(var index=0;index<ring.Length;index++){ring[index]=Matrix4x4.TransformPosition(ring[index],transform);}return new Spline(Spline.SmoothTangents(ring,closed:true,tension:1f),closed:true);}}
/// <summary>The sea state, read off the committed <c>.vxwaves</c> rather than written here.</summary>
sealed class FileWaves:IWaterWaveSource{public WaterWaveSpectrum?SpectrumFor(string name){if(!string.Equals(name,"water/pond" ,StringComparison.Ordinal)){return null;}var path=Path.Combine(AppContext.BaseDirectory,WavesFile);Assert.True(File.Exists(path),FormattableString.Invariant($"{WavesFile} was not copied beside the test" ));var asset=YamlSerializer.Parse<WaterWavesAsset>(File.ReadAllText(path));Assert.NotNull(asset);Assert.Null(asset.Validate());return asset.Spectrum;}}}
