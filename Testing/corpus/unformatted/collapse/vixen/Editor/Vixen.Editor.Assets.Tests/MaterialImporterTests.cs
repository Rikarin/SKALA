...
using Vixen.Core;using Vixen.Core.IO;using Vixen.Core.Reflection;using Vixen.Core.Serialization;using Vixen.Editor.Assets;using Vixen.Editor.Assets.Materials;using Vixen.Rendering.Materials;using Xunit;namespace Tests;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class MaterialImporterTests{static readonly AssetId Albedo=new(Guid.Parse("9e8a44c9930c64e388ca034c5fe4c426" ));[Fact]public void ItClaimsTheMaterialExtensionAndNothingElse(){var importer=new MaterialImporter();Assert.Equal("MaterialImporter" ,importer.Name);Assert.Equal([".vxmat" ],importer.Extensions);}
/// <summary>
    ...
    /// </remarks>
[Fact]public void TheArtifactTypeIsTheContractOfWhatIsWritten(){Assert.True(TypeRegistry.TryGetByAlias(MaterialImporter.MaterialType,out var descriptor));Assert.Equal(typeof(MaterialContent),descriptor.Type);}
/// <summary>
    ...
    /// </remarks>
[Fact]public async Task AMaterialWithFeaturesCompilesToAChunkThatReadsBack(){var(_,result)=await Import("""
            shader: ForwardPlus
            ...
            features:
              ...
              - !NormalMap
                strength: 0.5
            """ );var artifact=Assert.Single(result.Artifacts);Assert.Equal(MaterialImporter.MaterialType,artifact.Type);Assert.Equal(SubAssetId.Main,artifact.SubAsset);var content=Serializer.Read<MaterialContent>(artifact.Content.ToArray());Assert.Equal("ForwardPlus" ,content.Shader);Assert.Equal(2,content.Features.Length);var workflow=Assert.IsType<MetalRoughnessFeature>(content.Features[0]);Assert.Equal(1f,workflow.Metalness);Assert.Equal(0.25f,workflow.Roughness);Assert.Equal(0.8f,workflow.BaseColor.X,1e-6f);Assert.IsType<NormalMapFeature>(content.Features[1]);}
/// <summary>
    ...
    /// </remarks>
[Fact]public async Task ATextureIsBothCarriedAndDeclared(){var(context,result)=await Import("""
            features:
            ...
            """ );Assert.Contains(Albedo,context.AssetDependencies);var content=Serializer.Read<MaterialContent>(Assert.Single(result.Artifacts).Content.ToArray());var texture=Assert.Single(content.Textures);Assert.Equal("baseColorMap" ,texture.Parameter);Assert.Equal(Albedo,texture.Texture.Asset);}
/// <summary>
    ...
    /// </remarks>
[Fact]public async Task AShadingModelThisBuildDoesNotHaveIsAnError(){var(_,result)=await Import("shading: ToonShading\n" );Assert.False(result.Succeeded);Assert.Empty(result.Artifacts);Assert.Contains("ToonShading" ,Assert.Single(result.Diagnostics).Message,StringComparison.Ordinal);}
/// <summary>
    ...
    /// </remarks>
[Fact]public async Task AMaterialThatWouldCompileWrongIsRefusedWithTheCompilersOwnWords(){var(_,result)=await Import("""
            features:
              - !NormalMap
                strength: 1
              - !NormalMap
                strength: 0.5
            """ );Assert.False(result.Succeeded);Assert.Empty(result.Artifacts);Assert.Contains("NormalMapSurface" ,Assert.Single(result.Diagnostics).Message,StringComparison.Ordinal);}
/// <summary>
    ...
    /// </remarks>
[Fact]public async Task AMaterialWithNoFeaturesIsBuiltAndQuestioned(){var(_,result)=await Import("shader: ForwardPlus\n" );Assert.True(result.Succeeded);Assert.Single(result.Artifacts);Assert.Equal(ImportSeverity.Warning,Assert.Single(result.Diagnostics).Severity);}
/// <summary>A material from a newer editor is refused by number rather than half-read.</summary>
[Fact]public async Task AMaterialFromANewerBuildIsRefused(){var(_,result)=await Import($"version: {MaterialContent.Current + 1}\n" );Assert.False(result.Succeeded);Assert.Empty(result.Artifacts);}
/// <summary>A feature tag this build has never heard of is reported, not thrown.</summary>
[Fact]public async Task AFeatureThisBuildDoesNotHaveIsReported(){var(_,result)=await Import("features:\n  - !Iridescence\n    strength: 1\n" );Assert.False(result.Succeeded);Assert.Empty(result.Artifacts);}static async Task<(ImportContext Context,ImportResult Result)>Import(string text){var path=new VirtualPath("/Assets/hero.vxmat" );var files=new MemoryFileProvider();files.Seed(path,text);var importer=new MaterialImporter();var context=new ImportContext(AssetId.New(),path,importer.CreateSettings(),files,importer.Name,"Windows" );return(context,await importer.ImportAsync(context,TestContext.Current.CancellationToken));}}
