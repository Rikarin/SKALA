...
using Vixen.Core; using Vixen.Core.Mathematics; using Vixen.Ecs; using Vixen.Editor.Core; using Vixen.Editor.Core.Scenes; using Vixen.Engine.Transforms; using Vixen.Rendering; using Xunit; namespace Vixen.Editor.SceneView.Tests;
...
public class ViewportStateTests : IDisposable {
    ...
    public void Turning_the_grid_off_removes_the_grid_and_nothing_else() {
        ...
        lines.Build(scene, pane.Viewport, Height);
        Assert.True(lines.World.Count < all); // The marker cross is still there: six vertices for its three arms.
