...
using System.Collections.Concurrent;using System.Diagnostics;using System.Diagnostics.CodeAnalysis;using System.Globalization;using System.IO.Hashing;using Vixen.Core;using Vixen.Core.Yaml;using Vixen.Core.Yaml.Meta;namespace Vixen.Editor.Core;
...
///     </para>
/// </remarks>
public sealed class AssetDatabase{const string IndexHeader="vixen-guid-index 2" ;const string ScannedPrefix="scanned\t" ;const string TerminatorPrefix="end\t" ;Dictionary<AssetId,AssetEntry>byGuid=[];Dictionary<string,AssetEntry>byPath=new(StringComparer.Ordinal); // The sidecar stamp each entry was read from, by the entry's project-relative path. Held beside
...
Dictionary<string,MetaStamp>stamps=new(StringComparer.Ordinal); // A second, weaker filter over the same question: nothing whose sidecar was written at or after
...
DateTime trustedBeforeUtc=DateTime.SpecifyKind(DateTime.MinValue,DateTimeKind.Utc); // Injectable only so that a test can run a clock that leads the filesystem, which is what NTFS
...
internal AssetDatabase(ProjectPaths paths,TimeProvider clock){ArgumentNullException.ThrowIfNull(paths);ArgumentNullException.ThrowIfNull(clock);Paths=paths;time=clock;}
...
/// <summary>Finds an asset by where it is.</summary>
    ...
    /// <param name="entry">What is there.</param>
    /// <returns>Whether anything is.</returns>
public bool TryGetByPath(string relativePath,[MaybeNullWhen(false)]out AssetEntry entry)=>byPath.TryGetValue(relativePath,out entry);
/// <summary>Brings the index up to date with the sidecars on disk.</summary>
    ...
    ///     </para>
    /// </remarks>
public ScanReport Scan(ScanOptions?options=null){options??=ScanOptions.Default;var clock=Stopwatch.StartNew();var issues=new ConcurrentBag<AssetIssue>(); // Taken before the walk, and it is what the *next* scan will refuse to trust from. It catches
...
var startedUtc=time.GetUtcNow().UtcDateTime;var previousEntries=byPath;var previousStamps=stamps;var previousCutoff=trustedBeforeUtc;if(!Directory.Exists(Paths.Assets)){byGuid=[];byPath=new(StringComparer.Ordinal);stamps=new(StringComparer.Ordinal);trustedBeforeUtc=startedUtc;return new(0,clock.Elapsed,[],0);}var survey=Walk();var candidates=survey.Candidates;var found=new Scanned?[candidates.Count];var claimed=new bool[candidates.Count]; // Parallel because this is an I/O walk over thousands of small files and the cores are idle
// during it. Each index is written by exactly one iteration, so the arrays need no locking.
Parallel.For(0,candidates.Count,index=>{claimed[index]=survey.Sidecars.ContainsKey(candidates[index].Absolute);found[index]=Read(candidates[index],survey,previousEntries,previousStamps,previousCutoff,options,issues);}); // After the read rather than before it, because "an orphan" is exactly "a sidecar no asset
...
Quarantine(survey,claimed,options,issues); // Built into fresh dictionaries and swapped in at the end, so the previous index stays whole
...
byGuid=new(candidates.Count);byPath=new(candidates.Count,StringComparer.Ordinal);stamps=new(candidates.Count,StringComparer.Ordinal);var reused=0; // Insertion is sequential and in path order, so two machines scanning one checkout resolve a
// duplicate the same way. A parallel insert would make the winner depend on thread timing.
foreach(var scanned in found){if(scanned is not{}present){continue;}if(present.Reused){reused++;}Insert(present.Entry,present.Stamp,options,issues);}trustedBeforeUtc=startedUtc;return new(Count,clock.Elapsed,[ .. issues.OrderBy(issue=>issue.Path,StringComparer.Ordinal)],reused);}
/// <summary>Writes the index to <c>Library/GuidIndex</c>.</summary>
    ...
    ///     </para>
    /// </remarks>
public void Save(){Directory.CreateDirectory(Paths.Library);var temporary=Paths.GuidIndexFile+".writing" ;using(var writer=new StreamWriter(temporary)){writer.NewLine="\n";writer.WriteLine(IndexHeader);writer.WriteLine(ScannedPrefix+trustedBeforeUtc.ToString("O",CultureInfo.InvariantCulture));foreach(var entry in byPath.Values.OrderBy(entry=>entry.Path,StringComparer.Ordinal)){var stamp=stamps.GetValueOrDefault(entry.Path,MetaStamp.Unknown);writer.WriteLine(string.Create(CultureInfo.InvariantCulture,$"{entry.Guid}\t{entry.MetaVersion}\t{(entry.IsFolder ? 1 : 0)}\t{entry.ImporterTag}\t{stamp.Length}\t{stamp.WrittenUtc.ToString("O", CultureInfo.InvariantCulture)}\t{entry.Path}" ));}writer.WriteLine(string.Create(CultureInfo.InvariantCulture,$"{TerminatorPrefix}{byPath.Count}" ));}File.Move(temporary,Paths.GuidIndexFile,overwrite:true);}
...
public bool TryLoad(){if(!File.Exists(Paths.GuidIndexFile)){return false;}using var reader=new StreamReader(Paths.GuidIndexFile);if(reader.ReadLine()!=IndexHeader){return false;}var scanned=reader.ReadLine();if(scanned is null||!scanned.StartsWith(ScannedPrefix,StringComparison.Ordinal)||!DateTime.TryParse(scanned[ScannedPrefix.Length .. ],CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind,out var cutoff)){return false;}var entries=new Dictionary<string,AssetEntry>(StringComparer.Ordinal);var read=new Dictionary<string,MetaStamp>(StringComparer.Ordinal);var terminated=false;while(reader.ReadLine()is{}line){if(line.StartsWith(TerminatorPrefix,StringComparison.Ordinal)){terminated=int.TryParse(line[TerminatorPrefix.Length .. ],CultureInfo.InvariantCulture,out var declared)&&declared==entries.Count;break;}var parts=line.Split('\t');if(parts.Length!=7||!AssetId.TryParse(parts[0],out var guid)||!int.TryParse(parts[1],CultureInfo.InvariantCulture,out var version)||!long.TryParse(parts[4],CultureInfo.InvariantCulture,out var length)||!DateTime.TryParse(parts[5],CultureInfo.InvariantCulture,DateTimeStyles.RoundtripKind,out var written)){ // Skipped, which the terminator then catches: one line fewer than the count the file
...
continue;}var entry=new AssetEntry(guid,parts[6],parts[3].Length==0?null:parts[3],version,parts[2]=="1");entries[entry.Path]=entry;read[entry.Path]=new(length,written);}if(!terminated){return false;}byGuid=[];byPath=entries;stamps=read;trustedBeforeUtc=cutoff.ToUniversalTime();foreach(var entry in entries.Values){byGuid[entry.Guid]=entry;}return true;}
...
public bool IsStale(){if(!Directory.Exists(Paths.Assets)){return byPath.Count!=0;}var survey=Walk(); // Every sidecar must belong to something indexed, or a scan would have an orphan to move.
if(survey.Candidates.Count!=byPath.Count||survey.Sidecars.Count!=byPath.Count){return true;}foreach(var candidate in survey.Candidates){var relative=RelativeUnderRoot(candidate.Absolute);if(!byPath.TryGetValue(relative,out var entry)||entry.IsFolder!=candidate.IsFolder||!survey.Sidecars.TryGetValue(candidate.Absolute,out var stamp)||!IsFresh(stamp,relative,stamps,trustedBeforeUtc)){return true;}}return false;}Survey Walk(){var candidates=new List<(string Absolute,bool IsFolder)>();var sidecars=new Dictionary<string,MetaStamp>(StringComparer.Ordinal); // DirectoryInfo rather than the string overloads: the enumerator already has each entry's
...
foreach(var info in new DirectoryInfo(Paths.Assets).EnumerateFileSystemInfos("*",SearchOption.AllDirectories)){if(info is FileInfo file&&file.Name.EndsWith(AssetMetaFile.Extension,StringComparison.Ordinal)){ // Keyed by the asset it belongs to rather than by its own name, so that asking "what
...
sidecars[file.FullName[ .. ^AssetMetaFile.Extension.Length]]=new(file.Length,file.LastWriteTimeUtc);continue;}candidates.Add((info.FullName,info is DirectoryInfo));} // Sorted so that the scan, the duplicate resolution and the report are the same on every
// machine. Directory enumeration order is not a promise any filesystem makes.
candidates.Sort(static(left,right)=>string.CompareOrdinal(left.Absolute,right.Absolute));return new(candidates,sidecars);}
/// <summary>Whether a stamp seen on disk is the one an index entry may be kept on.</summary>
    ...
    /// </remarks>
static bool IsFresh(MetaStamp current,string relative,Dictionary<string,MetaStamp>previousStamps,DateTime cutoff)=>previousStamps.TryGetValue(relative,out var recorded)&&recorded==current&&current.WrittenUtc<cutoff;
/// <summary>The project-relative form of a path the walk produced, which is known to be under the root.</summary>
    ...
    /// </remarks>
string RelativeUnderRoot(string absolute){var root=Paths.Root;if(absolute.Length>root.Length&&absolute.StartsWith(root,StringComparison.Ordinal)&&(absolute[root.Length]==Path.DirectorySeparatorChar||absolute[root.Length]=='/')){return absolute[(root.Length+1) .. ].Replace('\\','/');}return Paths.Relative(absolute);}Scanned?Read((string Absolute,bool IsFolder)candidate,Survey survey,Dictionary<string,AssetEntry>previousEntries,Dictionary<string,MetaStamp>previousStamps,DateTime cutoff,ScanOptions options,ConcurrentBag<AssetIssue>issues){var relative=RelativeUnderRoot(candidate.Absolute);var hasMeta=survey.Sidecars.TryGetValue(candidate.Absolute,out var stamp); // The whole point: an asset whose sidecar is the size and age the index left it does not get
// opened, parsed, or thought about again.
if(hasMeta&&IsFresh(stamp,relative,previousStamps,cutoff)&&previousEntries.TryGetValue(relative,out var kept)&&kept.IsFolder==candidate.IsFolder){return new(kept,stamp,true);}var metaPath=AssetMetaFile.PathFor(candidate.Absolute);if(!hasMeta){if(!options.CreateMissingMeta){issues.Add(new(AssetIssueKind.MetaCreated,relative,"Has no .meta, and none was created." ));return null;}var minted=CreateMeta(metaPath);issues.Add(new(AssetIssueKind.MetaCreated,relative,$"Had no .meta; created one with GUID {minted}." )); // No stamp, rather than the one the write just left behind. This scan cannot tell an edit
...
return new(new(minted,relative,null,MetaMigrationChain.CurrentVersion,candidate.IsFolder),MetaStamp.Unknown,false);}if(!MetaScanner.TryScanFile(metaPath,out var envelope)){issues.Add(new(AssetIssueKind.MetaUnreadable,relative,"Its .meta has no readable GUID. It is being ignored rather than re-created, because " +"minting a new GUID would break every reference to this asset." ));return null;}return new(new(envelope.Guid,relative,envelope.ImporterTag,envelope.MetaVersion,candidate.IsFolder),stamp,false);}void Insert(AssetEntry entry,MetaStamp stamp,ScanOptions options,ConcurrentBag<AssetIssue>issues){byPath[entry.Path]=entry;stamps[entry.Path]=stamp;if(!byGuid.TryGetValue(entry.Guid,out var existing)){byGuid[entry.Guid]=entry;return;} // Two assets claiming one GUID: someone copied a folder, or merged two branches that each
// added a file. The one whose recorded source hash still matches its file is the original.
var incomingMatches=SourceHashMatches(entry);var existingMatches=SourceHashMatches(existing); // If the hashes do not settle it, the one already in the index keeps the GUID — and because
...
var incomingWins=incomingMatches&&!existingMatches;var winner=incomingWins?entry:existing;var loser=incomingWins?existing:entry;var message=$"'{winner.Path}' and '{loser.Path}' both claim GUID {entry.Guid}. " +(incomingMatches!=existingMatches?"The one whose recorded sourceHash still matches its file kept it." :"Neither sourceHash settled it, so the first path in order kept it." );if(!options.ResolveDuplicateGuids){byGuid[winner.Guid]=winner;issues.Add(new(AssetIssueKind.DuplicateGuid,loser.Path,message+" Nothing was changed." ));return;}var minted=ReGuid(loser);var repaired=loser with{Guid=minted};byGuid[winner.Guid]=winner;byGuid[minted]=repaired;byPath[repaired.Path]=repaired; // Its sidecar was just rewritten, so the pre-rewrite stamp is a claim about a file that no
...
stamps[repaired.Path]=MetaStamp.Unknown;issues.Add(new(AssetIssueKind.DuplicateGuid,loser.Path,$"{message} '{loser.Path}' was re-GUIDed to {minted}." ));}void Quarantine(Survey survey,bool[]claimed,ScanOptions options,ConcurrentBag<AssetIssue>issues){var spokenFor=0;foreach(var claim in claimed){if(claim){spokenFor++;}} // The overwhelmingly common case, and the one worth not paying for: every sidecar belongs to
// an asset, so there is nothing to search for and no set of paths to build in order to search.
if(spokenFor==survey.Sidecars.Count){return;}var owners=new HashSet<string>(StringComparer.Ordinal);foreach(var candidate in survey.Candidates){owners.Add(candidate.Absolute);} // Ordered so that two machines quarantining the same wreckage write the same files in the
// same order.
foreach(var owner in survey.Sidecars.Keys.Order(StringComparer.Ordinal).ToList()){ // A sidecar for a sidecar — `foo.meta.meta` beside a real `foo.meta` — is not an orphan,
...
if(owners.Contains(owner)||(owner.EndsWith(AssetMetaFile.Extension,StringComparison.Ordinal)&&survey.Sidecars.ContainsKey(owner[ .. ^AssetMetaFile.Extension.Length]))){continue;}var meta=owner+AssetMetaFile.Extension;var relative=Paths.Relative(meta);if(!options.QuarantineOrphanMeta){issues.Add(new(AssetIssueKind.MetaOrphaned,relative,"Its asset is gone. It was left where it is." ));continue;} // Moved, never deleted. A mis-ordered git operation — the asset removed before its
...
var destination=Path.Combine(Paths.OrphanMeta,Paths.Relative(meta));Directory.CreateDirectory(Path.GetDirectoryName(destination)!);File.Move(meta,destination,overwrite:true);survey.Sidecars.Remove(owner);issues.Add(new(AssetIssueKind.MetaOrphaned,relative,$"Its asset is gone. Moved to '{Paths.Relative(destination)}'." ));}}
/// <summary>Whether a sidecar's recorded source hash still describes the file next to it.</summary>
    ...
    /// </remarks>
bool SourceHashMatches(AssetEntry entry){if(entry.IsFolder){return false;}var absolute=Paths.Absolute(entry.Path);var metaPath=AssetMetaFile.PathFor(absolute);try{if(YamlReader.Read(File.ReadAllText(metaPath))is not YamlMapping root||root["importer" ]is not YamlMapping importer||importer["sourceHash" ]is not YamlScalar recorded||recorded.Value.Length==0){return false;}return string.Equals(recorded.Value,HashOf(absolute),StringComparison.OrdinalIgnoreCase);}catch(Exception failure)when(failure is IOException or YamlParseException){return false;}}AssetId ReGuid(AssetEntry entry){var metaPath=AssetMetaFile.PathFor(Paths.Absolute(entry.Path));var minted=AssetId.New(); // Read and rewrite as nodes, so everything the file said other than its GUID — the importer
// settings, the addressable block, the comments — comes back out byte for byte.
var root=YamlReader.Read(File.ReadAllText(metaPath))as YamlMapping??new YamlMapping();root.Set("guid",new YamlScalar(minted.ToString()));File.WriteAllText(metaPath,YamlWriter.Write(root));return minted;}static AssetId CreateMeta(string metaPath){var minted=AssetId.New(); // No importer key: which importer claims a file is decided at import time, and writing a
// guess here would be a fact the file asserts and nothing checks.
var root=new YamlMapping().Set("guid",new YamlScalar(minted.ToString())).Set("metaVersion" ,new YamlScalar(MetaMigrationChain.CurrentVersion.ToString(CultureInfo.InvariantCulture),YamlScalarStyle.Plain));File.WriteAllText(metaPath,YamlWriter.Write(root));return minted;}static string HashOf(string path){using var stream=File.OpenRead(path);var hash=new XxHash128();hash.Append(stream);return Convert.ToHexStringLower(hash.GetCurrentHash());}
