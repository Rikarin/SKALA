...
public sealed class AssetDatabase {
    ...
    Dictionary<string, AssetEntry> byPath = new(
    ...
    Dictionary<string, MetaStamp> stamps = new(
        StringComparer.Ordinal
    ...
    DateTime trustedBeforeUtc = DateTime.SpecifyKind(
        DateTime.MinValue,
        DateTimeKind.Utc
    ...
    public ScanReport Scan(ScanOptions? options = null) {
        ...
        var issues = new ConcurrentBag<AssetIssue>(); // Taken before the walk, and it is what the *next* scan will refuse to trust from. It catches
        ...
        var claimed = new bool[candidates.Count]; // Parallel because this is an I/O walk over thousands of small files and the cores are idle
        // during it. Each index is written by exactly one iteration, so the arrays need no locking.
        Parallel.For(
            0,
            candidates.Count,
            index => { claimed[index] = survey.Sidecars.ContainsKey(candidates[index].Absolute); found[index] = Read(
                    candidates[index],
                    survey,
                    previousEntries,
                    previousStamps,
    ...
    public bool IsStale() {
        ...
        var survey = Walk(); // Every sidecar must belong to something indexed, or a scan would have an orphan to move.
    ...
    Survey Walk() {
        ...
        var sidecars = new Dictionary<string, MetaStamp>(
            StringComparer.Ordinal
    ...
    Scanned? Read(
        ...
        var relative = RelativeUnderRoot(candidate.Absolute);
        var hasMeta = survey.Sidecars.TryGetValue(
            candidate.Absolute,
            out var stamp
    ...
    void Insert(AssetEntry entry, MetaStamp stamp, ScanOptions options, ConcurrentBag<AssetIssue> issues) {
        ...
        var existingMatches = SourceHashMatches(
            existing
        ...
        byPath[repaired.Path] = repaired; // Its sidecar was just rewritten, so the pre-rewrite stamp is a claim about a file that no
    ...
    AssetId ReGuid(AssetEntry entry) {
        ...
        var minted = AssetId.New(); // Read and rewrite as nodes, so everything the file said other than its GUID — the importer
    ...
    static AssetId CreateMeta(string metaPath) {
        var minted = AssetId.New(); // No importer key: which importer claims a file is decided at import time, and writing a
