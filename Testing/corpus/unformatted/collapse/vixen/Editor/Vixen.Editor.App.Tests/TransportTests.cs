...
public class TransportTests {
    ...
    public void The_play_button_fills_while_the_editor_is_playing() {
        using var fixture = EditorSession.Start();
        var button = Button(fixture, "transport-play"); // Off: a green triangle on the surface.
        Assert.False(button.State.HasFlag(ElementState.Checked));
        Assert.True(fixture.Shell.Commands.Execute("play.play"));
        fixture.Frames(2); // On: `:checked` is what the theme fills, so the button is green and the glyph is white.
    ...
    public void Stop_and_step_are_disabled_until_there_is_something_to_stop() {
        ...
        Assert.True(fixture.Shell.Commands.Execute("play.play"));
        fixture.Frames(2);
        Assert.False(stop.Disabled); // Step is for a paused editor: stepping a running one is a frame nobody sees.
        Assert.True(step.Disabled);
    ...
    static ButtonBase Button(EditorSession fixture, string className) =>
        Descendants(fixture.Shell.Toolbar.Strip).OfType<ButtonBase>()
