...
///     </para>
/// </remarks>
public sealed class PluginRegistrations:IDisposable{readonly List<Action>undo=[];readonly List<Exception>failures=[];
...
public void Add(Action action){ArgumentNullException.ThrowIfNull(action);undo.Add(action);}
...
Updates.Clear();for(var index=undo.Count-1;index>=0;index--){try{undo[index]();}catch(Exception exception){failures.Add(exception);}}undo.Clear();}}
