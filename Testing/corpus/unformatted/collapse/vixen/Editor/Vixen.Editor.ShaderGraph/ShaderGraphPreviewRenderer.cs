...
public sealed class ShaderGraphPreviewRenderer : INodePreviewSource, IDisposable {
    ...
    public bool TryGet(NodeGraphModel graph, GraphNode node, NodeTypeDefinition definition, out NodePreview preview) {
        ...
        if (entry is not null) {
            entry.Revision = revision; // The second gate: the graph changed and this node's expression did not. Moving a node,
    ...
    static readonly Corner[] Quad = [
        new(new Vector2(-1f, 1f), new Vector2(0f, 0f)), new(new Vector2(-1f, -1f), new Vector2(0f, 1f)),
        new(new Vector2(1f, 1f), new Vector2(1f, 0f)), new(new Vector2(1f, 1f), new Vector2(1f, 0f)),
        new(new Vector2(-1f, -1f), new Vector2(0f, 1f)), new(new Vector2(1f, -1f), new Vector2(1f, 1f))
    ];
