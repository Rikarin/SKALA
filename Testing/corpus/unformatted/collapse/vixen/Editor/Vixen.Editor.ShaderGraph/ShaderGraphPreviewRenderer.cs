...
using System.Runtime.InteropServices;using Vixen.Core.Mathematics;using Vixen.Editor.NodeGraph;using Vixen.Graphics;using Vixen.ShaderCompiler;using Vixen.Shaders;namespace Vixen.Editor.ShaderGraph;
...
public const PixelFormat Format=PixelFormat.Rgba8UNorm ;readonly IGraphicsDevice device;readonly NodeTypeRegistry registry;readonly IPreviewImages?images;readonly EffectLoader loader;
/// <summary>
    ...
    /// </remarks>
readonly Dictionary<(NodeGraphModel Graph,NodeId Node),Entry>entries=[];readonly Dictionary<NodeGraphModel,Watched>watched=[];readonly List<(NodeGraphModel Graph,NodeId Node)>recent=[];readonly List<(NodeGraphModel Graph,NodeId Node)>dirty=[];bool disposed;
...
public ShaderGraphPreviewRenderer(IGraphicsDevice device,NodeTypeRegistry registry,IPreviewImages?images=null){ArgumentNullException.ThrowIfNull(device);ArgumentNullException.ThrowIfNull(registry);this.device=device;this.registry=registry;this.images=images;loader=new EffectLoader(device);}
...
/// <inheritdoc />
    ...
    ///     </para>
    /// </remarks>
public bool TryGet(NodeGraphModel graph,GraphNode node,NodeTypeDefinition definition,out NodePreview preview){ArgumentNullException.ThrowIfNull(graph);ArgumentNullException.ThrowIfNull(node);preview=default;if(disposed){return false;}var revision=Watch(graph);var key=(graph,node.Id);if(entries.TryGetValue(key,out var entry)&&entry.Revision==revision){ // ⚠ The first of three gates, and the only one that runs sixty times a second. Nothing
...
Touch(key);return Answer(entry,out preview);}var compilation=ShaderGraphPreview.Compile(graph,node.Id,registry);Emissions++;if(compilation.Artefact is not{}source){return false;}Touch(key);if(entry is not null){entry.Revision=revision; // The second gate: the graph changed and this node's expression did not. Moving a node,
...
if(!string.Equals(entry.Source,source.Source,StringComparison.Ordinal)){entry.Source=source.Source;if(!dirty.Contains(key)){dirty.Add(key);}}}else{entries[key]=new Entry{Source=source.Source,Revision=revision};dirty.Add(key);Evict();return false;}return Answer(entry,out preview);}
/// <summary>What a node's preview is, once it is known there is nothing to recompute.</summary>
static bool Answer(Entry entry,out NodePreview preview){preview=default;if(entry.Image==0){return false;}preview=new NodePreview(new Color4(1f,1f,1f,1f),Image:entry.Image);return true;}
/// <summary>Rebuilds what has been invalidated, up to <see cref="RebuildsPerUpdate" />.</summary>
    ...
    /// </remarks>
public int Update(){ObjectDisposedException.ThrowIf(disposed,this);if(dirty.Count==0){return 0;}var taken=Math.Min(RebuildsPerUpdate,dirty.Count);List<(NodeGraphModel Graph,NodeId Node)>built=[];for(var index=0;index<taken;index++){var key=dirty[index];if(entries.TryGetValue(key,out var entry)&&Build(key.Node,entry)){built.Add(key);}}dirty.RemoveRange(0,taken);if(built.Count==0){return 0;}using(var commands=device.BeginCommandList(QueueKind.Graphics,"shader graph previews" )){foreach(var key in built){Draw(commands,entries[key]);}commands.Finish();device.GraphicsQueue.Submit([commands]);}return built.Count;}
/// <inheritdoc />
    /// <remarks>
    ...
    /// </remarks>
public void Dispose(){if(disposed){return;}disposed=true;device.WaitIdle();foreach(var entry in entries.Values){Destroy(entry);}foreach(var(graph,state)in watched){graph.Changed-=state.Handler;}watched.Clear();entries.Clear();recent.Clear();dirty.Clear();}
...
public string?RefusalFor(NodeGraphModel graph,NodeId node)=>entries.TryGetValue((graph,node),out var entry)?entry.Refusal:null;
/// <summary>The target one node's picture is in, for a caller that reads it back.</summary>
    ...
    /// </remarks>
internal TextureHandle TextureOf(NodeGraphModel graph,NodeId node)=>entries.TryGetValue((graph,node),out var entry)?entry.Texture:default;
/// <summary>Compiles one preview's Raven and makes everything the draw needs.</summary>
bool Build(NodeId id,Entry entry){Release(entry);Compilations++;Effect effect;try{var data=Compile(entry.Source);if(data is null){Refusals++;return false;}effect=loader.Load(data);}catch(Exception failure)when(failure is ShaderCompilationException or ArgumentException or IOException){ // ⚠ A refusal rather than a throw. A half-wired graph emits Raven that does not type
...
entry.Refusal=failure.Message;Refusals++;return false;} // The one resource this binds is the uniform block holding the two transforms. Anything else
// — a texture, a sampler — is a material's and a preview has no material.
foreach(var binding in effect.Bindings){if(binding.Kind is not(DescriptorKind.UniformBuffer or DescriptorKind.DynamicUniformBuffer)){entry.Refusal=$"'{binding.Name}' is a {binding.Kind} and a preview binds no resources, so there is " +"nothing this node could be shown against." ;Refusals++;return false;}}entry.Refusal=null;Ensure(id,entry);var block=effect.BlockOf(DescriptorSetSlot.PerMaterial);if(block.Exists){entry.Constants=device.CreateBuffer(new(block.Size,BufferUsage.Uniform,MemoryAccess.HostUpload,"shader graph preview constants" ));device.Write(entry.Constants,0,Transforms(block));entry.Descriptors=device.CreateDescriptorSet(effect.SetLayouts[(int)DescriptorSetSlot.PerMaterial],"shader graph preview" );device.UpdateDescriptorSet(entry.Descriptors,[DescriptorWrite.Uniform(block.Binding,entry.Constants)]);}var(vertices,layout)=Geometry(effect);if(vertices.Length>0){entry.Vertices=device.CreateBuffer(new(vertices.Length*sizeof(float),BufferUsage.Vertex,MemoryAccess.HostUpload,"shader graph preview quad" ));device.Write(entry.Vertices,0,MemoryMarshal.AsBytes<float>(vertices));}entry.Vertex=device.CreateShader(ShaderStage.Vertex,Bytecode(effect,ShaderStage.Vertex),"preview vertex" );entry.Fragment=device.CreateShader(ShaderStage.Fragment,Bytecode(effect,ShaderStage.Fragment),"preview fragment" );entry.Pipeline=device.CreateGraphicsPipeline(new(entry.Vertex,entry.Fragment,effect.Layout,[new ColourTargetState(Format,BlendState.Opaque)],layout.Length>0?[new VertexBufferLayout(layout.Stride,layout.Elements)]:[], // Two-sided: a quad whose winding disagrees with the rasterizer draws nothing at all,
// and that failure is indistinguishable from a preview nobody implemented.
Rasterizer:RasterizerState.TwoSided,DepthStencil:DepthStencilState.Disabled,Name:"shader graph preview" ));return true;}
/// <summary>Records one preview's pass.</summary>
void Draw(ICommandList commands,Entry entry){commands.Barrier(new BarrierGroup([],[new TextureBarrier(entry.Texture,entry.State,ResourceState.ColourTarget)]));commands.BeginRenderPass(new RenderPassDescription([new ColourAttachment(entry.View,LoadAction.Clear,StoreAction.Store,new Color4(0f,0f,0f,1f))],name:"shader graph preview" ));commands.SetViewport(new Viewport(0f,0f,Size,Size));commands.SetScissor(new ScissorRect(0,0,Size,Size));commands.BindPipeline(entry.Pipeline);if(entry.Descriptors.IsValid){commands.BindDescriptorSet(DescriptorSetSlot.PerMaterial,entry.Descriptors);}if(entry.Vertices.IsValid){commands.BindVertexBuffer(0,entry.Vertices);}commands.Draw(Quad.Length);commands.EndRenderPass();commands.Barrier(new BarrierGroup([],[new TextureBarrier(entry.Texture,ResourceState.ColourTarget,ResourceState.ShaderRead)]));entry.State=ResourceState.ShaderRead;Draws++;}
/// <summary>The target for a node, made once and drawn into for ever after.</summary>
void Ensure(NodeId id,Entry entry){if(entry.Texture.IsValid){return;}entry.Texture=device.CreateTexture(new(Format,Size,Size,TextureUsage.ColourTarget|TextureUsage.Sampled|TextureUsage.CopySource,Name:$"shader graph preview {id}" ));entry.View=device.CreateTextureView(entry.Texture);entry.State=ResourceState.Undefined;entry.Image=images?.Register(entry.View)??0;Created++;}
/// <summary>Compiles one preview's Raven, in memory.</summary>
    ...
    /// </remarks>
static EffectData?Compile(string source)=>RavenEffectCompiler.FromSources([(ShaderGraphPreview.Name+".rvn",source)]).TryGet(EffectKey.Of(ShaderGraphPreview.Name));static ReadOnlySpan<byte>Bytecode(Effect effect,ShaderStage stage){foreach(var compiled in effect.Stages){if(compiled.Stage==stage){return compiled.Bytecode.AsSpan();}}return[];}
/// <summary>Identity for the two transforms every graph declares, and zero for anything else.</summary>
    ...
    /// </remarks>
static byte[]Transforms(EffectBlock block){var bytes=new byte[block.Size];foreach(var parameter in block.Members){if(parameter.Size<64||!IsTransform(parameter.Key.Name)){continue;}var identity=Matrix4x4.Identity;MemoryMarshal.Write(bytes.AsSpan(parameter.Offset,64),in identity);}return bytes;}
/// <summary>Whether a reflected parameter is one of the two transforms every graph declares.</summary>
    ...
    /// </remarks>
static bool IsTransform(string name){var bare=name.AsSpan()[(name.LastIndexOf('.')+1) .. ];return bare.SequenceEqual("worldViewProjection" )||bare.SequenceEqual("world" );}
/// <summary>The quad, interleaved for exactly the attributes this variant's vertex stage reads.</summary>
    ...
    /// </remarks>
static(float[]Vertices,Layout Layout)Geometry(Effect effect){List<VertexElement>elements=[];List<(int Lanes,Func<Corner,Vector4>Of)>readers=[];var offset=0;foreach(var input in effect.VertexInputs.OrderBy(input=>input.Location)){var lanes=LanesOf(input.Kind);if(lanes==0){continue;}elements.Add(new((uint)input.Location,FormatOf(lanes),offset));readers.Add((lanes,ReaderOf(input.Name)));offset+=lanes*sizeof(float);}var stride=offset;var floats=new List<float>(Quad.Length*Math.Max(1,stride/sizeof(float)));foreach(var corner in Quad){foreach(var(lanes,of)in readers){var value=of(corner);for(var lane=0;lane<lanes;lane++){floats.Add(lane switch{0=>value.X,1=>value.Y,2=>value.Z,_=>value.W});}}}return([ .. floats],new Layout(stride,[ .. elements]));}static int LanesOf(ShaderValueKind kind)=>kind switch{ShaderValueKind.Float=>1,ShaderValueKind.Float2=>2,ShaderValueKind.Float3=>3,ShaderValueKind.Float4=>4,_=>0};static VertexFormat FormatOf(int lanes)=>lanes switch{1=>VertexFormat.Float32,2=>VertexFormat.Float32X2,3=>VertexFormat.Float32X3,_=>VertexFormat.Float32X4};
/// <summary>What one named vertex attribute holds, corner by corner.</summary>
static Func<Corner,Vector4>ReaderOf(string name)=>name switch{"texcoord" =>corner=>new Vector4(corner.Uv.X,corner.Uv.Y,0f,0f), // Facing the viewer, so a graph reading a world normal previews a flat surface rather than
// one whose normal is zero — which normalises to a NaN and shades as nothing.
"normal" =>_=>new Vector4(0f,0f,1f,0f),"colour" =>_=>new Vector4(1f,1f,1f,1f),_=>corner=>new Vector4(corner.Position.X,corner.Position.Y,0f,1f)};
/// <summary>This graph's revision, subscribing to it the first time it is seen.</summary>
    ...
    ///     </para>
    /// </remarks>
int Watch(NodeGraphModel graph){if(watched.TryGetValue(graph,out var watching)){return watching.Revision;}var state=new Watched();state.Handler=_=>state.Revision++;graph.Changed+=state.Handler;watched[graph]=state;return state.Revision;}void Touch((NodeGraphModel Graph,NodeId Node)key){recent.Remove(key);recent.Add(key);}void Evict(){while(recent.Count>Math.Max(1,Capacity)){var oldest=recent[0];recent.RemoveAt(0);dirty.Remove(oldest);if(entries.Remove(oldest,out var entry)){Destroy(entry);}}}
/// <summary>Destroys what a rebuild replaces, keeping the target and its number.</summary>
void Release(Entry entry){if(entry.Pipeline.IsValid){device.Destroy(entry.Pipeline);entry.Pipeline=default;}if(entry.Vertex.IsValid){device.Destroy(entry.Vertex);entry.Vertex=default;}if(entry.Fragment.IsValid){device.Destroy(entry.Fragment);entry.Fragment=default;}if(entry.Descriptors.IsValid){device.Destroy(entry.Descriptors);entry.Descriptors=default;}if(entry.Constants.IsValid){device.Destroy(entry.Constants);entry.Constants=default;}if(entry.Vertices.IsValid){device.Destroy(entry.Vertices);entry.Vertices=default;}}void Destroy(Entry entry){Release(entry);if(!entry.Texture.IsValid){return;} // ⚠ The number goes before the texture does, or the interface holds a descriptor naming freed
// memory and the next frame that draws it is undefined rather than wrong.
if(entry.Image!=0){images?.Release(entry.Image);entry.Image=0;}device.Destroy(entry.View);device.Destroy(entry.Texture);entry.Texture=default;entry.View=default;Destroyed++;}
/// <summary>One corner of the quad: where it is in clip space, and what it samples.</summary>
    ...
    /// </remarks>
readonly record struct Corner(Vector2 Position,Vector2 Uv);static readonly Corner[]Quad=[new(new Vector2(-1f,1f),new Vector2(0f,0f)),new(new Vector2(-1f,-1f),new Vector2(0f,1f)),new(new Vector2(1f,1f),new Vector2(1f,0f)),new(new Vector2(1f,1f),new Vector2(1f,0f)),new(new Vector2(-1f,-1f),new Vector2(0f,1f)),new(new Vector2(1f,-1f),new Vector2(1f,1f))];readonly record struct Layout(int Stride,VertexElement[]Elements){public int Length=>Elements.Length;}
/// <summary>One graph's revision, and the handler that moves it.</summary>
sealed class Watched{public int Revision=1;public Action<NodeGraphModel>Handler=_=>{};}sealed class Entry{public required string Source{get;set;}
...
public string?Refusal{get;set;}public TextureHandle Texture;public TextureViewHandle View;public ResourceState State=ResourceState.Undefined;public ulong Image;public PipelineHandle Pipeline;public ShaderHandle Vertex;public ShaderHandle Fragment;public BufferHandle Constants;public BufferHandle Vertices;public DescriptorSetHandle Descriptors;}}
