...
using System.Diagnostics.CodeAnalysis;using Vixen.Editor.Core;namespace Vixen.Editor.Inspector;
...
/// <inheritdoc />
    ...
    ///     allocates nothing.
    /// </remarks>
public IReadOnlyList<IEditMember>MembersOf(Type type){ArgumentNullException.ThrowIfNull(type);return InspectorRegistry.Find(type)?.Members??[];}
/// <inheritdoc />
public bool TryResolve(Type type,string path,[NotNullWhen(true)]out IEditMember?member){ArgumentNullException.ThrowIfNull(type);ArgumentException.ThrowIfNullOrEmpty(path);if(InspectorRegistry.TryGet(type,out var descriptor)&&descriptor.TryGetMember(path,out var found)){member=found;return true;}member=null;return false;}}
