...
using Vixen.Core;using Vixen.Ecs;using Vixen.Editor.Core;using Vixen.Rendering.Water;using Vixen.Water;namespace Vixen.Editor.Water; // ⚠ There is no CreateWaterBodyCommand here, and its absence is the decision rather than an omission.
...
public sealed class WaterProfileCommand(World world,Entity entity,WaterBodyComponent before,WaterBodyComponent after):IEditorCommand{
/// <inheritdoc />
public string Name=>"Edit Water Profile" ;
/// <inheritdoc />
public void Do(EditorContext context)=>Apply(after);
/// <inheritdoc />
public void Undo(EditorContext context)=>Apply(before);void Apply(in WaterBodyComponent value){if(world.IsAlive(entity)){world.Set(entity,value);}}}
...
public sealed class WaterCarveCommand(Vixen.Terrain.Terrain terrain,IReadOnlyList<(WaterBody Body,WaterCarveProfile Profile)>before,IReadOnlyList<(WaterBody Body,WaterCarveProfile Profile)>after):IEditorCommand{
/// <inheritdoc />
...
/// <remarks>
    ...
    /// </remarks>
void Regenerate(IReadOnlyList<(WaterBody Body,WaterCarveProfile Profile)>bodies){WaterCarve.Regenerate(terrain,WaterCarve.LayerOf(terrain),bodies);terrain.Resolve();applied=bodies;}}
