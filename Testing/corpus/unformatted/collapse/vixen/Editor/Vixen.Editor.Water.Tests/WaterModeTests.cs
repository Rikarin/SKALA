...
public sealed class WaterModeTests {
    ...
    public void Drawing_a_lake_makes_a_closed_curve() {
        ...
        mode.Drawn += (spline, drawnKind) => { laid = spline; kind = drawnKind; };
    ...
    public void A_lake_with_two_points_cannot_be_finished() {
        ...
        Assert.Null(mode.Finish()); // And it is still in flight, so the author can carry on clicking.
        Assert.Equal(2, mode.Editing.Points.Count);
    }
    ...
    public void Escape_cancels_a_draw_in_flight() {
        ...
        Assert.Empty(mode.Editing.Points); // With nothing in flight it is somebody else's key.
