...
using CsCheck; using Xunit; namespace Vixen.Ui.Layout.Tests;
...
public class PixelRoundingTests {
    ...
    public void An_incrementally_rounded_tree_matches_one_rounded_from_cold() {
        ...
        Gen.Select(Gen.Int[2, 5], Gen.Int[1, 4], Gen.Int[0, 999].Array[1, 12])
            .Sample(shape => { var (rows, cells, mutations) = shape; var spec =
                        new PanelSpec(rows, cells); using var incremental =
                        new LayoutTree { PointScaleFactor = 2f }; var incrementalRoot =
                        spec.Build(incremental); incremental.CalculateLayout(
                        incrementalRoot,
                        320.5f,
                        240.25f,
                        Direction.Ltr
