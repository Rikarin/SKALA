...
using Vixen.Net.Messaging;namespace Vixen.Net.Engine;
...
///     </para>
/// </remarks>
public sealed class SyncVar<T>:ISyncField{readonly ISyncCodec<T>codec;readonly IEqualityComparer<T>comparer;T value;
/// <inheritdoc />
...
public T Value{get=>value;set{if(comparer.Equals(this.value,value)){return;}this.value=value;IsDirty=true;}}
...
public SyncVar(T initial=default!,IEqualityComparer<T>?comparer=null){codec=SyncCodecs.For<T>();this.comparer=comparer??EqualityComparer<T>.Default;value=initial;}
/// <inheritdoc />
public void Write(ref BitWriter writer)=>codec.Write(ref writer,in value);
/// <inheritdoc />
public bool Apply(ref BitReader reader){if(!codec.Read(ref reader,out var arrived)){return false;}if(comparer.Equals(value,arrived)){return true;}var was=value;value=arrived;Changed?.Invoke(was,arrived);return true;}
/// <inheritdoc />
...
public static implicit operator T(SyncVar<T>variable){ArgumentNullException.ThrowIfNull(variable);return variable.value;}}
