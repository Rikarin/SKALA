...
public sealed class RenderGraphLintTests : IDisposable {
    ...
    public void ADiscardedWriteIsWarnedAboutOnce() {
        ...
        graph.AddPass(
            "producer",
            pass => { pass.ColourAttachment(shared); pass.SideEffect(); pass.Execute(_ => { }); }
        );
        graph.AddPass(
            "overwriter",
            pass => { pass.ColourAttachment(shared, LoadAction.Clear); pass.SideEffect(); pass.Execute(_ => { }); }
        );
        graph.Compile();
    ...
    public void ALoadedOverwriteIsNotAWarning() {
        ...
        graph.AddPass(
            "accumulator",
            pass => { pass.ColourAttachment(shared, LoadAction.Load); pass.SideEffect(); pass.Execute(_ => { }); }
        );
        graph.Compile();
    ...
    public void AConsumedWriteMayBeCleared() {
        ...
        graph.AddPass("producer", pass => { pass.ColourAttachment(shared); pass.Execute(_ => { }); });
        graph.AddPass(
            "reader",
            pass => { pass.Reads(shared); pass.ColourAttachment(derived); pass.SideEffect(); pass.Execute(_ => { }); }
        );
        graph.AddPass(
            "recycler",
            pass => { pass.ColourAttachment(shared, LoadAction.Clear); pass.SideEffect(); pass.Execute(_ => { }); }
