...
using Vixen.Core.Mathematics;using Vixen.Core.Threading;using Xunit;namespace Vixen.Geometry.Remeshing.Tests;
...
/// <summary>Every face has four sides, on every fixture that produces one.</summary>
    ...
    /// </remarks>
[Theory][InlineData("box")][InlineData("sphere" )][InlineData("cylinder" )][InlineData("stairs" )][InlineData("plate" )][InlineData("union" )][InlineData("difference" )]public void Every_face_that_comes_out_has_four_sides(string name){var quads=Remesher.Remesh(Fixture(name),new(){TargetQuads=400},out var report);Assert.True(report.QuadCount>0,$"{name} produced nothing: {string.Join(" · ", report.Warnings)}" );Assert.Equal(0,report.NonQuadCount);Assert.True(report.IsAllQuad);Assert.Equal(report.QuadCount,quads.FaceCount);for(var face=0;face<quads.FaceCount;face++){Assert.Equal(4,quads.Faces[face].Count);}}
/// <summary>A boolean of two boxes reproduces its feature lines to the order the criterion names.</summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][InlineData("box",5e-4f)][InlineData("plate" ,1e-3f)][InlineData("cylinder" ,1e-4f)][InlineData("union" ,5e-4f)][InlineData("difference" ,1e-3f)]public void A_hard_edge_is_a_chain_of_output_edges(string name,float tolerance){Remesher.Remesh(Fixture(name),new(){TargetQuads=400},out var report);Assert.True(report.QuadCount>0,string.Join(" · ",report.Warnings));Assert.True(report.FeatureReproductionError<=tolerance,$"{name}: the feature reproduction error is {report.FeatureReproductionError:E3} of the diagonal." );}
/// <summary>Every closed fixture but one comes back closed, consistent and with nothing left over.</summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][InlineData("sphere" )][InlineData("box")][InlineData("plate" )][InlineData("stairs" )][InlineData("union" )][InlineData("difference" )]public void A_closed_surface_comes_back_solid(string name){var quads=Remesher.Remesh(Fixture(name),new(){TargetQuads=400},out var report);Assert.True(report.IsAllQuad);Assert.True(report.Mesh.IsSolid,$"{name}: {report.Mesh.Describe()}" );Assert.True(report.MaxDeviation<0.2f,$"The deviation is {report.MaxDeviation:E3} of the diagonal." );Assert.NotEmpty(quads.Positions.ToArray());}
/// <summary>No fixture loses a patch to the extractor, which is what a hole in the result is.</summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][InlineData("box")][InlineData("sphere" )][InlineData("cylinder" )][InlineData("stairs" )][InlineData("plate" )][InlineData("union" )][InlineData("difference" )]public void No_patch_is_skipped_by_the_extractor(string name){Remesher.Remesh(Fixture(name),new(){TargetQuads=400},out var report);Assert.True(report.QuadCount>0,string.Join(" · ",report.Warnings));Assert.DoesNotContain(report.Warnings,warning=>warning.Contains("patches were skipped" ,StringComparison.Ordinal));}
/// <summary>No patch's boundary walks the same arc twice, on any fixture.</summary>
    ...
    /// </remarks>
[Theory][InlineData("box")][InlineData("sphere" )][InlineData("cylinder" )][InlineData("stairs" )][InlineData("plate" )][InlineData("union" )][InlineData("difference" )]public void No_patch_walks_the_same_arc_twice(string name){var layout=Layout(name,out _,out _);Assert.True(layout.IsUsable,string.Join(" · ",layout.Warnings));for(var patch=0;patch<layout.Patches.Count;patch++){var seen=new HashSet<int>();foreach(var side in layout.Patches[patch].Sides){foreach(var use in side){Assert.True(seen.Add(use.Arc),$"{name}: patch {patch} walks arc {use.Arc} twice, which is a slit in the partition." );}}}}
/// <summary>Two patches sharing a side hold the same position indices, in reverse.</summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][InlineData("sphere" )][InlineData("box")][InlineData("union" )]public void A_shared_side_is_one_set_of_vertices(string name){var layout=Layout(name,out var mesh,out var features);var quantization=Quantizer.Solve(layout);var projector=SurfaceProjector.Build(mesh.Positions.ToArray(),mesh.Triangles.ToArray());var extraction=PatchExtractor.Extract(mesh,features,layout,quantization,projector);var uses=new Dictionary<int,List<(int Patch,bool Reversed)>>();for(var patch=0;patch<layout.Patches.Count;patch++){foreach(var side in layout.Patches[patch].Sides){foreach(var use in side){if(!uses.TryGetValue(use.Arc,out var list)){uses[use.Arc]=list=[];}list.Add((patch,use.Reversed));}}}var shared=0;foreach(var(arc,list)in uses.OrderBy(entry=>entry.Key)){if(list.Count!=2||list[0].Patch==list[1].Patch){continue;}shared++; // One array, read forwards by one patch and backwards by the other. Index equality, so
// there is no size of model at which the seam opens.
Assert.NotEqual(list[0].Reversed,list[1].Reversed);Assert.Equal(quantization.Counts[arc]+1,extraction.Samples[arc].Length);foreach(var position in extraction.Samples[arc]){Assert.InRange(position,0,extraction.Mesh.PositionCount-1);}}Assert.True(shared>0,$"{name}: no arc is shared between two patches, so nothing was tested." );}
/// <summary>Ten runs are ten identical meshes, and so are runs at one, four and sixteen workers.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void A_remesh_is_the_same_bits_at_any_worker_count(){var settings=new RemeshSettings{TargetQuads=400};var first=Remesher.Remesh(Fixture("sphere" ),settings,out var report);Assert.True(report.QuadCount>0);for(var run=0;run<3;run++){Same(first,Remesher.Remesh(Fixture("sphere" ),settings,out _),$"run {run}" );}foreach(var workers in new[]{1,4,16}){using var scheduler=new JobScheduler(workers);Same(first,Remesher.Remesh(Fixture("sphere" ),settings,out _,scheduler),$"{workers} workers" );}}
/// <summary>The exact router beats the approximate one on the energy, on every fixture.</summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][InlineData("box")][InlineData("sphere" )][InlineData("cylinder" )][InlineData("stairs" )]public void The_exact_flow_is_never_worse_than_the_approximate_one(string name){var layout=Layout(name,out _,out _);Assert.True(layout.IsUsable,string.Join(" · ",layout.Warnings));var exact=Quantizer.Solve(layout);var rough=Quantizer.Solve(layout,QuantizeMode.Approximate);Assert.True(exact.IsFeasible,string.Join(" · ",exact.Warnings));if(!rough.IsFeasible){return;}Assert.True(exact.Deviation<=rough.Deviation,$"{name}: the exact router scored {exact.Deviation:F1} against the approximate one's {rough.Deviation:F1}." );}
/// <summary>Every quantized count is a whole number of quads and no patch collapsed to nothing.</summary>
    ...
    /// </remarks>
[Theory][InlineData("box")][InlineData("sphere" )][InlineData("cylinder" )][InlineData("plate" )]public void A_side_may_collapse_and_a_patch_may_not(string name){var layout=Layout(name,out _,out _);var quantization=Quantizer.Solve(layout);Assert.True(quantization.IsFeasible,string.Join(" · ",quantization.Warnings));foreach(var count in quantization.Counts){Assert.True(count>=0,"A side quantized to a negative number of quads." );}for(var index=0;index<layout.Patches.Count;index++){var patch=layout.Patches[index];if(patch.IsFan){var(a,b,c)=quantization.Spokes[index];Assert.True(a>0&&b>0&&c>0,$"{name}: a fan quantized to spokes {a}, {b}, {c}." );int[]wanted=[a+c,a+b,b+c];for(var at=0;at<3;at++){Assert.Equal(wanted[at],patch.Sides[at].Sum(use=>quantization.Counts[use.Arc]));}continue;}var wide=patch.Sides[0].Sum(use=>quantization.Counts[use.Arc]);var tall=patch.Sides[1].Sum(use=>quantization.Counts[use.Arc]);Assert.True(wide>0&&tall>0,$"{name}: a patch quantized to {wide} by {tall}." ); // The constraint the whole flow exists to satisfy: opposite side groups agree exactly.
Assert.Equal(wide,patch.Sides[2].Sum(use=>quantization.Counts[use.Arc]));Assert.Equal(tall,patch.Sides[3].Sum(use=>quantization.Counts[use.Arc]));}}
/// <summary>The same shape at a thousandth and a thousand times gives the same relative answer.</summary>
    ...
    /// </remarks>
[Theory][InlineData(9e-4f)][InlineData(9.9e-4f)][InlineData(1e-3f)][InlineData(1.1e-3f)][InlineData(1e+3f)]public void The_same_shape_at_another_size_gives_the_same_answer(float factor){var settings=new RemeshSettings{TargetQuads=400};Remesher.Remesh(Fixture("sphere" ),settings,out var unit);Remesher.Remesh(Scaled(Fixture("sphere" ),factor),settings,out var scaled);Assert.True(scaled.QuadCount>0,$"×{factor}: {string.Join(" · ", scaled.Warnings)}" );Assert.Equal(0,scaled.NonQuadCount);Assert.Equal(unit.SingularitiesOnFeatures,scaled.SingularitiesOnFeatures);Assert.Equal(unit.Mesh.IsSolid,scaled.Mesh.IsSolid);Assert.Equal(unit.Mesh.Boundary.Count,scaled.Mesh.Boundary.Count); // Within a factor of two of the same count, which is a statement that the density field and
// the quantization are reading a scale-free number rather than a length.
Assert.InRange(scaled.QuadCount,unit.QuadCount/2,unit.QuadCount*2); // Relative measures, so they compare directly rather than after a division.
Assert.True(MathF.Abs(unit.MaxDeviation-scaled.MaxDeviation)<1e-2f,$"×{factor}: the deviation moved from {unit.MaxDeviation:E3} to {scaled.MaxDeviation:E3}." );}
/// <summary>Every stage is timed and named, so a slow or a lossy one can be attributed.</summary>
    ...
    /// </remarks>
[Fact]public void The_report_names_every_stage(){Remesher.Remesh(Fixture("sphere" ),new(){TargetQuads=400},out var report);foreach(var stage in Enum.GetValues<RemeshStage>()){Assert.Contains(report.Stages,timing=>timing.Stage==stage);} // ⚠ This asserted TimeSpan.Zero while stage seven was a recorded seam and nothing else. R5
...
var transfer=report.Stages.Single(timing=>timing.Stage==RemeshStage.Transfer);Assert.True(transfer.Elapsed>TimeSpan.Zero,"Stage seven did no work at all." );Assert.True(transfer.Elements>0,"Stage seven carried nothing." );Assert.True(report.Conditioning.Triangles>0);}
/// <summary>Nothing throws and nothing hangs, on eighteen deliberately broken meshes.</summary>
    ...
    /// </remarks>
[Fact]public void A_broken_mesh_produces_a_result_or_a_reason_and_never_an_exception(){foreach(var(name,source)in BrokenMeshes.Corpus()){var quads=Remesher.Remesh(source,new(){TargetQuads=200},out var report);Assert.Equal(0,report.NonQuadCount);Assert.Equal(report.QuadCount,quads.FaceCount);if(report.QuadCount==0){Assert.NotEmpty(report.Warnings);}Assert.NotEmpty(report.Stages);_=name;}}
/// <summary>A symmetry plane produces a symmetric mesh rather than a warning.</summary>
    ...
    /// </remarks>
[Fact]public void Symmetry_is_applied_rather_than_apologised_for(){var quads=Remesher.Remesh(Fixture("sphere" ),new(){TargetQuads=200,Symmetry=new Plane(Vector3.UnitX,0f)},out var report);Assert.True(quads.FaceCount>0,string.Join(" · ",report.Warnings));Assert.DoesNotContain(report.Warnings,warning=>warning.Contains("not applied yet" ,StringComparison.Ordinal));Assert.Equal(0,report.NonQuadCount);}static void Same(EditMesh first,EditMesh again,string what){Assert.Equal(first.PositionCount,again.PositionCount);Assert.Equal(first.FaceCount,again.FaceCount);Assert.Equal(first.CornerCount,again.CornerCount);for(var position=0;position<first.PositionCount;position++){var one=first.Positions[position];var two=again.Positions[position];Assert.Equal(BitConverter.SingleToInt32Bits (one.X),BitConverter.SingleToInt32Bits (two.X));Assert.Equal(BitConverter.SingleToInt32Bits (one.Y),BitConverter.SingleToInt32Bits (two.Y));Assert.Equal(BitConverter.SingleToInt32Bits (one.Z),BitConverter.SingleToInt32Bits (two.Z));}for(var corner=0;corner<first.CornerCount;corner++){Assert.Equal(first.Corners[corner],again.Corners[corner]);}_=what;}
/// <summary>The layout and the fields behind it, for the tests that look inside a stage.</summary>
internal static PatchLayout Layout(string name,out ManifoldMesh mesh,out FeatureGraph features){var source=Fixture(name);var settings=new RemeshSettings{TargetQuads=400};var soup=TriangleSoup.From(source);var area=0f;for(var triangle=0;triangle<soup.TriangleCount;triangle++){area+=soup.Area(triangle);}mesh=MeshConditioner.Condition(source,settings.Conditioning,out _,MathF.Sqrt(area/400f));features=FeatureDetector.Detect(mesh,settings,FeatureCurves.All(source,settings));var curvature=CurvatureField.Build(mesh);var solved=CrossFieldSolver.Solve(mesh,settings,features,curvature);var field=SingularityPass.Place(mesh,settings,features,curvature,solved,out _);return PatchLayout.Build(mesh,field,features,DensityField.Build(mesh,settings,features,curvature),SingularityPass.Extract(mesh,field));}static EditMesh Scaled(EditMesh source,float factor){var scaled=new EditMesh(source);for(var position=0;position<scaled.PositionCount;position++){scaled.MovePosition(position,scaled.Positions[position]*factor);}return scaled;}internal static EditMesh Fixture(string name)=>name switch{"box"=>MeshShapes.Create(ShapeKind.Box),"sphere" =>MeshShapes.Create(ShapeParameters.Default(ShapeKind.Sphere)with{Sides=24,Steps=12}),"cylinder" =>MeshShapes.Create(ShapeParameters.Default(ShapeKind.Cylinder)with{Sides=16}),"stairs" =>MeshShapes.Create(ShapeKind.Stairs),"plate" =>FieldFixtures.Plate(5,5,[(2,2)]),"union" =>Boolean(BooleanOperation.Union),"difference" =>Boolean(BooleanOperation.Difference),_=>throw new ArgumentOutOfRangeException(nameof(name),name,"No such fixture." )};
/// <summary>Doc 24's blockout case: two boxes through <see cref="MeshBoolean" />.</summary>
static EditMesh Boolean(BooleanOperation operation)=>MeshBoolean.Apply(MeshShapes.Create(ShapeKind.Box),MeshShapes.Create(ShapeKind.Box),operation,Matrix4x4.FromTranslation(new(0.6f,0.4f,0.3f)))??throw new InvalidOperationException("The boolean fixture came back empty." );}
