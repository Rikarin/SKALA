...
public class RemesherTests {
    ...
    public void A_shared_side_is_one_set_of_vertices(string name) {
        ...
        foreach (var (arc, list) in uses.OrderBy(entry => entry.Key)) {
            ...
            shared++; // One array, read forwards by one patch and backwards by the other. Index equality, so
    ...
    public void A_side_may_collapse_and_a_patch_may_not(string name) {
        ...
        for (var index = 0; index < layout.Patches.Count; index++) {
            ...
            Assert.True(
                wide > 0 && tall > 0,
                $"{name}: a patch quantized to {wide} by {tall}."
    ...
    public void The_same_shape_at_another_size_gives_the_same_answer(float factor) {
        ...
        Assert.Equal(
            unit.Mesh.Boundary.Count,
            scaled.Mesh.Boundary.Count
    ...
    internal static EditMesh Fixture(string name) =>
        name switch {
