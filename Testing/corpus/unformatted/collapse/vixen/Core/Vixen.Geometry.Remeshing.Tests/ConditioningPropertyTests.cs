...
using CsCheck;using Vixen.Core.Mathematics;using Vixen.Geometry.Testing;using Xunit;namespace Vixen.Geometry.Remeshing.Tests;
...
///     </para>
/// </remarks>
[Collection(TimedCases.Name)]public class ConditioningPropertyTests{
...
/// <summary>docs/plan/41's robustness criterion, over the space and under the clock.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void Every_broken_mesh_conditions_to_a_manifold_or_a_report_and_never_to_an_exception(){Gen.Select(BrokenMeshSpace.Sized,Gen.Int[0,3],Gen.Bool,Gen.Bool,(recipe,rounds,fill,shrinkwrap)=>(recipe,rounds,fill,shrinkwrap)).Sample(entry=>{var(recipe,rounds,fill,shrinkwrap)=entry;var settings=new ConditioningSettings{PreRemeshIterations=rounds,FillHoles=fill,Shrinkwrap=shrinkwrap};var what=$"conditioning {recipe} with {rounds} rounds, fill {fill}, shrinkwrap {shrinkwrap}" ;var mesh=RunawayGuard.Run($"building {recipe}" ,()=>BrokenMeshSpace.Build(recipe));var view=RunawayGuard.Run(what,()=>MeshConditioner.Condition(mesh,settings,out var report)is var conditioned?(Mesh:conditioned,Report:report):default); // The stage's own claim about what it returns: a manifold view, every count in the
// report a count of something that happened, and every triangle index in range.
Assert.True(view.Report.Mesh.IsManifold,$"{what}: {view.Report.Mesh.Describe()}." );Assert.Equal(view.Mesh.TriangleCount,view.Report.Triangles);Assert.True(view.Report.Welded>=0,what);Assert.True(view.Report.Unorientable>=0,what);Assert.True(view.Report.Despecked>=0,what);Assert.True(view.Report.Cut>=0,what);Assert.True(view.Report.Filled>=0,what);foreach(var corner in view.Mesh.Triangles){Assert.InRange(corner,0,Math.Max(view.Mesh.VertexCount-1,0));}foreach(var position in view.Mesh.Positions){Assert.True(float.IsFinite(position.X)&&float.IsFinite(position.Y)&&float.IsFinite(position.Z),$"{what}: a vertex came out at {position}." );}},iter:PropertyBudget.Iterations(200),threads:1);}
/// <summary>Steps one to four, over the space, at a thousandth and a thousand times.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void The_first_four_steps_agree_on_their_topology_at_both_scales(){BrokenMeshSpace.Recipe.Sample(recipe=>{var mesh=RunawayGuard.Run($"building {recipe}" ,()=>BrokenMeshSpace.Build(recipe));RunawayGuard.Run($"conditioning {recipe} at both scales" ,()=>ScaleInvarianceTests.Compare(recipe.ToString(),mesh,new(){PreRemeshIterations=0,FillHoles=false},ScaleInvarianceTests.Agreement.Rate,0.1f));},iter:PropertyBudget.Iterations(150),threads:1);}
/// <summary>Step five, whose threshold is relative and whose comparison is still a knife edge.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void Filling_holes_disagrees_by_at_most_one_hole_across_six_orders_of_magnitude(){BrokenMeshSpace.Recipe.Sample(recipe=>{var mesh=RunawayGuard.Run($"building {recipe}" ,()=>BrokenMeshSpace.Build(recipe));var settings=new ConditioningSettings{PreRemeshIterations=0,FillHoles=true,HoleSize=0.3f};var small=RunawayGuard.Run($"filling holes in {recipe} at a thousandth" ,()=>Condition(mesh,settings,1e-3f));var large=RunawayGuard.Run($"filling holes in {recipe} at a thousand times" ,()=>Condition(mesh,settings,1e+3f));Assert.True(small.Report.Mesh.IsManifold&&large.Report.Mesh.IsManifold,$"{recipe}: {small.Report.Mesh.Describe()} against {large.Report.Mesh.Describe()}." );Assert.True(small.Report.Unorientable==large.Report.Unorientable,$"{recipe}: unorientable {small.Report.Unorientable} against {large.Report.Unorientable}." );Assert.True(Math.Abs(small.Report.Filled-large.Report.Filled)<=1,$"{recipe}: filled {small.Report.Filled} against {large.Report.Filled}." );Near(recipe.ToString(),"triangles" ,small.Report.Triangles,large.Report.Triangles);Near(recipe.ToString(),"vertices" ,small.Vertices,large.Vertices);},iter:PropertyBudget.Iterations(150),threads:1);}
/// <summary>Step six, one round: the four operators are all comparisons against a length.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void One_round_of_the_pre_remesh_stays_within_a_fifth_at_both_scales(){var measured=0;BrokenMeshSpace.Recipe.Sample(recipe=>{var mesh=RunawayGuard.Run($"building {recipe}" ,()=>BrokenMeshSpace.Build(recipe));var settings=new ConditioningSettings{PreRemeshIterations=1};var size=RunawayGuard.Run($"sizing {recipe}" ,()=>{MeshConditioner.Condition(mesh,settings,out var report);return report.Triangles;});if(size<Floor){return;}measured++;RunawayGuard.Run($"one pre-remesh round on {recipe} at both scales" ,()=>ScaleInvarianceTests.Compare(recipe.ToString(),mesh,settings,ScaleInvarianceTests.Agreement.Rate,0.2f));},iter:PropertyBudget.Iterations(100),threads:1); // 36 per cent of the space reaches the floor, so a hundred draws expect 36 with a standard
...
/// <summary>Five rounds, where exact agreement is not achievable and the reason is not a constant.</summary>
    ...
    ///     </para>
    /// </remarks>
[Fact]public void Five_rounds_of_the_pre_remesh_stay_within_a_rate_of_each_other(){var measured=0;BrokenMeshSpace.Recipe.Sample(recipe=>{var mesh=RunawayGuard.Run($"building {recipe}" ,()=>BrokenMeshSpace.Build(recipe));var settings=new ConditioningSettings{PreRemeshIterations=5};var size=RunawayGuard.Run($"sizing {recipe}" ,()=>{MeshConditioner.Condition(mesh,settings,out var report);return report.Triangles;});if(size<Floor){return;}measured++;RunawayGuard.Run($"five pre-remesh rounds on {recipe} at both scales" ,()=>ScaleInvarianceTests.Compare(recipe.ToString(),mesh,settings,ScaleInvarianceTests.Agreement.Rate, // A third, and the remarks name the capsule that moved it off a quarter.
1f/3f));},iter:PropertyBudget.Iterations(150),threads:1);Assert.True(measured>20,$"Only {measured} of the sampled recipes reached {Floor} triangles." );}
/// <summary>⚠ And the guard on the guard: a scale property proves nothing about a mesh that has no size.</summary>
[Fact]public void Every_generated_mesh_really_is_six_orders_of_magnitude_from_its_own_small_copy(){BrokenMeshSpace.Recipe.Sample(recipe=>{var mesh=BrokenMeshSpace.Build(recipe);if(mesh.PositionCount==0){return;}var small=Diagonal(BrokenMeshes.Scaled(mesh,1e-3f));var large=Diagonal(BrokenMeshes.Scaled(mesh,1e+3f));Assert.True(small>0f,$"{recipe} has no extent at all." );Assert.True(large/small>1e5f,$"{recipe} only grew by {large / small:E2}." );},iter:PropertyBudget.Iterations(100),threads:1);}static(ConditioningReport Report,int Vertices)Condition(EditMesh mesh,ConditioningSettings settings,float scale){var view=MeshConditioner.Condition(BrokenMeshes.Scaled(mesh,scale),settings,out var report);return(report,view.VertexCount);}
/// <summary>Two counts within a tenth of each other. Measured: the worst seen is 3.85 per cent.</summary>
static void Near(string name,string what,int one,int two){var span=Math.Abs(one-two);var bound=Math.Max(one,two)*0.1f;Assert.True(span<=bound,$"{name}: {one} {what} against {two}." );}static float Diagonal(EditMesh mesh){var box=mesh.Bounds;return(box.Maximum-box.Minimum).Length();}}
