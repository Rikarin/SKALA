...
using Vixen.Core.Mathematics;namespace Vixen.Foliage;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class FoliageCollision{readonly HashSet<FoliageAddress>active=[];readonly List<FoliageAddress>activated=[];readonly List<FoliageAddress>deactivated=[];readonly HashSet<FoliageAddress>wanted=[];
...
/// <summary>Works out which instances should have a body, and says what changed.</summary>
    ...
    /// </remarks>
public int Update(FoliageVolume volume,IReadOnlyList<Vector3>sources){ArgumentNullException.ThrowIfNull(volume);ArgumentNullException.ThrowIfNull(sources);wanted.Clear();activated.Clear();deactivated.Clear();for(var type=0;type<volume.Palette.Count;type++){var settings=volume.Palette[type];if(!settings.Collides||settings.Storage==FoliageStorage.Derived){continue;}var only=new HashSet<int>{type};foreach(var source in sources){foreach(var address in volume.Within(new(source.X,source.Z),settings.ActivationRadius,only)){wanted.Add(address);}}}foreach(var address in wanted){if(active.Add(address)){activated.Add(address);}}foreach(var address in active){if(!wanted.Contains(address)){deactivated.Add(address);}}foreach(var address in deactivated){active.Remove(address);}return active.Count;}
/// <summary>Drops every body, which is what leaving a level does.</summary>
    ...
    /// </remarks>
public void Clear(){activated.Clear();deactivated.Clear();deactivated.AddRange(active);active.Clear();wanted.Clear();}
...
public int ForgetCell(int type,FoliageCellKey cell){var stale=active.Where(address=>address.Type==type&&address.Cell==cell).ToArray();foreach(var address in stale){active.Remove(address);}return stale.Length;}}
