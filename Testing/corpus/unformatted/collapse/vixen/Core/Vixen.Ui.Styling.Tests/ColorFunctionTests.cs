...
public class ColorFunctionTests {
    ...
    public void A_malformed_colour_function_is_unknown_rather_than_a_guess(string text) =>
        AssertUnknown(text); // ---------------------------------------------------------------- out of gamut
    ...
    public void An_out_of_gamut_colour_is_carried_through_rather_than_clamped() {
        ...
        Assert.False(
            float.IsNaN(wide.R) || float.IsNaN(wide.G) || float.IsNaN(wide.B),
            "no channel is NaN"
    ...
    public void An_endpoint_may_be_written_any_way_the_parser_understands() {
        ...
        AssertLinear(Color("color-mix(in oklab, oklab(0.627955 0.224863 0.125846), transparent)"), 1f, 0f, 0f, 0.5f);
    }
    ...
    public void Excss_leaves_a_colour_function_it_does_not_know_exactly_as_written() {
        ...
        Assert.Equal("oklch(62.3% 0.214 259.815)", fixture.Value(fixture.Tree.CreateElement("div", classNames: ["a"])));
