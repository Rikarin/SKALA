...
using System.Numerics;namespace Vixen.Ui.Styling;
...
///     </para>
/// </remarks>
struct AncestorBloom{ulong low;ulong high;
/// <summary>Records that an ancestor is called this.</summary>
    /// <param name="nameId">An interned tag, id or class name.</param>
public void Add(int nameId){var(first,second)=Bits(nameId);Set(first);Set(second);}
/// <summary>Whether an ancestor could be called this.</summary>
    ...
    /// </returns>
public readonly bool MightContain(int nameId){var(first,second)=Bits(nameId);return Test(first)&&Test(second);}
/// <summary>How many of the 128 bits are set. For diagnostics and tests.</summary>
public readonly int PopulationCount=>BitOperations.PopCount(low)+BitOperations.PopCount(high);void Set(int bit){if(bit<64){low|=1UL<<bit;}else{high|=1UL<<(bit-64);}}readonly bool Test(int bit)=>bit<64?(low&(1UL<<bit))!=0:(high&(1UL<<(bit-64)))!=0;static(int First,int Second)Bits(int nameId){ // Interned ids are dense small integers assigned in first-seen order, which is close to the
...
var mixed=Mix((uint)nameId);return((int)(mixed&127),(int)((mixed >> 7)&127));}static uint Mix(uint value){ // MurmurHash3's finaliser. Cheap, and it spreads the low bits of a counter across the word,
// which is the only property needed here.
value^=value >> 16;value*=0x85EBCA6B;value^=value >> 13;value*=0xC2B2AE35;value^=value >> 16;return value;}}
