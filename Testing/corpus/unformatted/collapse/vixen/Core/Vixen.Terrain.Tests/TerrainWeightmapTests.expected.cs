...
public sealed class TerrainWeightmapTests {
    ...
    public void An_import_restores_the_invariant_rather_than_trusting_the_file() {
        var terrain = Built();
        var mask = new byte[terrain.Description
            .SampleCount]; // A mask that means nothing about the other layers: half of it fully covered.
        for (var z = 0; z < terrain.Description.SamplesZ; z++) {
    ...
    public void A_mask_of_another_size_is_resampled_edge_to_edge() {
        var terrain = Built();
        var mask = new byte[64 * 64]; // A ramp along X, so the corners are checkable.
        for (var z = 0; z < 64; z++) {
            for (var x = 0; x < 64; x++) {
    ...
    public void A_layer_that_does_not_exist_is_refused() {
        var terrain = Built();
        Assert.Throws<ArgumentOutOfRangeException>(() => TerrainWeightmap.Import(
                terrain,
                5,
                new byte[64 * 64],
                64,
    ...
    public void Coverage_is_what_the_panels_bar_draws() {
        var terrain = Built(); // The first weight-blended layer starts covering everything.
    ...
    public void A_quad_takes_the_layer_with_the_most_weight_over_its_four_corners() {
        var terrain = Built(); // Three corners of the quad at (10, 10) painted, one not.
    ...
    public void A_terrain_with_no_layers_says_so_rather_than_claiming_the_first_one() {
        ...
        var quads = terrain.Description.TileQuads;
        var materials = new sbyte[quads * quads];
        terrain.Weights.FillCollisionMaterials(
            0,
            0,
            materials
