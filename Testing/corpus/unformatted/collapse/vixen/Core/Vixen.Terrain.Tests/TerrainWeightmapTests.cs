...
using Xunit; namespace Vixen.Terrain.Tests;
...
public sealed class TerrainWeightmapTests {
    ...
    public void An_import_restores_the_invariant_rather_than_trusting_the_file() {
        var terrain = Built();
        var mask = new byte[terrain.Description.SampleCount]; // A mask that means nothing about the other layers: half of it fully covered.
        for (var z = 0; z < terrain.Description.SamplesZ; z++) {
