...
using CsCheck; using Vixen.Core.Mathematics; using Vixen.Terrain; using Xunit; namespace Vixen.Terrain.Tests;
...
public sealed class TerrainBrushTests {
    ...
    public void APatternStampReadsItsMaskInWorldSpaceSoAStrokeRevealsOneTexture() {
        ...
        var mask = new RecordingMask(); // The same world point read by two stamps in different places must land on the same texel.
    ...
    public void AnyBrushIsMonotonicFromItsCentreToItsEdge() {
        Gen.Select(Gen.Float[0.01f, 200f], Gen.Float[0f, 1f], Gen.Float[0f, 1f], Gen.Int[0, 3])
            .Sample(
                input => { var (radius, strength, falloff, curve) = input; var brush = TerrainBrush.Default with {
                        Radius = radius, Strength = strength, Falloff = falloff, Curve = (BrushFalloffKind)curve
                    }; var stamp = new BrushStamp(Vector2.Zero); Assert.Equal(
                        strength,
                        brush.WeightAt(Vector2.Zero, stamp),
                        4
                    ); Assert.Equal(0f, brush.WeightAt(new(radius, 0f), stamp), 5); var previous =
                        float.PositiveInfinity; for (var step = 0; step <= 64; step++) {
