...
public const char InvalidCharacter=char.MaxValue;readonly string text=text;public int Position{get;private set;}public bool AtEnd=>Position>=text.Length;
/// <summary>The character at the current position, or <see cref="InvalidCharacter" /> at the end.</summary>
public char Current=>Peek();public char Peek(int delta=0){var index=Position+delta;return index>=0&&index<text.Length?text[index]:InvalidCharacter;}
...
/// <summary>Moves back to a position already passed, undoing a scan that turned out not to be one.</summary>
    ...
    /// </remarks>
public void Rewind(int position){if(position>Position){throw new ArgumentOutOfRangeException(nameof(position),position,$"A rewind cannot move forward: the window is already at {Position}." );}Position=position;}
/// <summary>Consumes <paramref name="expected" /> when it is the current character.</summary>
public bool TryAdvance(char expected){if(Current!=expected){return false;}Position++;return true;}
