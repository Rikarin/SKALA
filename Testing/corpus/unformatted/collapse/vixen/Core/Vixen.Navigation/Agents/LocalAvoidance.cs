...
using Vixen.Core.Mathematics;namespace Vixen.Navigation.Agents;
...
public LocalAvoidance(LocalAvoidanceSettings settings=default)=>Settings=settings==default?new():settings;
...
public Vector3 Sample(Vector3 position,float radius,Vector3 velocity,Vector3 desired,float maxSpeed,ReadOnlySpan<AvoidanceNeighbour>neighbours){if(neighbours.IsEmpty||maxSpeed<=0){return desired;}var best=desired;var bestPenalty=Penalty(position,radius,velocity,desired,desired,maxSpeed,neighbours); // Standing still is deliberately not a candidate. It is very often the lowest-penalty
...
var heading=desired.LengthSquared()>1e-6f?desired:velocity;var baseAngle=heading.LengthSquared()>1e-6f?MathF.Atan2(heading.Z,heading.X):0f;var step=MathF.Tau/Settings.Samples;for(var ring=1;ring<=Settings.Rings;ring++){var speed=maxSpeed*ring/Settings.Rings;for(var sample=0;sample<Settings.Samples;sample++){ // Every other ring is offset by half a step, so the samples do not all lie on the
// same few directions — the same reason a dither pattern is not a grid.
var angle=baseAngle+(sample*step)+(ring%2==0?step*0.5f:0f);var candidate=new Vector3(MathF.Cos(angle)*speed,0f,MathF.Sin(angle)*speed);var penalty=Penalty(position,radius,velocity,desired,candidate,maxSpeed,neighbours);if(penalty<bestPenalty){bestPenalty=penalty;best=candidate;}}}return best;}float Penalty(Vector3 position,float radius,Vector3 velocity,Vector3 desired,Vector3 candidate,float maxSpeed,ReadOnlySpan<AvoidanceNeighbour>neighbours){var desiredPenalty=Settings.DesiredWeight*(NavGeometry.Distance2D(candidate,desired)/maxSpeed);var currentPenalty=Settings.CurrentWeight*(NavGeometry.Distance2D(candidate,velocity)/maxSpeed);var soonest=Settings.TimeHorizon;var side=0f;var counted=0;foreach(var neighbour in neighbours){ // The reciprocal assumption: both agents are expected to move, so the relative velocity
// this candidate implies is twice it, less both agents' current velocities.
var relative=(candidate*2f)-velocity-neighbour.Velocity;var towards=neighbour.Position-position;var distance=towards.Length();var direction=distance>1e-6f?towards/distance:Vector3.UnitX; // Which way this encounter is already leaning. Passing that way is cheaper than passing
// the other, and the two agents work it out from the same geometry, so they agree.
var perpendicular=NavGeometry.Cross2D(direction,neighbour.Velocity-desired)<0.01f?new Vector3(-direction.Z,0f,direction.X):new Vector3(direction.Z,0f,-direction.X);side+=Math.Clamp(MathF.Min((Dot2D(direction,relative)*0.5f)+0.5f,Dot2D(perpendicular,relative)*2f),0f,1f);counted++;if(!Sweep(position,radius,relative,neighbour,out var enter,out var exit)){continue;} // Already overlapping. Half the exit time, so that getting out is urgent but not
// infinitely so — an agent standing inside another one still has to choose a direction.
if(enter<0f&&exit>0f){enter=-enter*0.5f;}if(enter>=0f&&enter<soonest){soonest=enter;}}var sidePenalty=counted>0?Settings.SideWeight*(side/counted):0f;var timePenalty=Settings.TimeWeight/(0.1f+(soonest/Settings.TimeHorizon));return desiredPenalty+currentPenalty+sidePenalty+timePenalty;}
/// <summary>When two circles moving at a constant relative velocity would touch, and stop touching.</summary>
static bool Sweep(Vector3 position,float radius,Vector3 relative,in AvoidanceNeighbour neighbour,out float enter,out float exit){enter=0f;exit=0f;var offset=neighbour.Position-position;var combined=radius+neighbour.Radius;var a=Dot2D(relative,relative);if(a<1e-6f){return false;}var b=Dot2D(relative,offset);var c=Dot2D(offset,offset)-(combined*combined);var discriminant=(b*b)-(a*c);if(discriminant<0f){return false;}var root=MathF.Sqrt(discriminant);enter=(b-root)/a;exit=(b+root)/a;return true;}static float Dot2D(Vector3 left,Vector3 right)=>(left.X*right.X)+(left.Z*right.Z);}
