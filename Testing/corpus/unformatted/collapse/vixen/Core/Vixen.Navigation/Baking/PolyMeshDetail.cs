...
using Vixen.Core.Mathematics;namespace Vixen.Navigation.Baking;
...
public static PolyMeshDetail Build(PolyMesh mesh,CompactHeightfield field,float sampleDistance,float maxError,int walkableHeight){var detail=new PolyMeshDetail();if(sampleDistance<=0){return detail;}var hull=new List<Vector3>();var vertices=new List<Vector3>();var triangles=new List<int>();var samples=new List<Vector3>();for(var poly=0;poly<mesh.PolyCount;poly++){var offset=poly*mesh.MaxVerticesPerPoly;var count=PolyMesh.CountVertices(mesh.Polys,offset,mesh.MaxVerticesPerPoly);hull.Clear();for(var slot=0;slot<count;slot++){var vertex=mesh.Polys[offset+slot];hull.Add(new(mesh.Vertices[vertex*3],mesh.Vertices[(vertex*3)+1],mesh.Vertices[(vertex*3)+2]));}vertices.Clear();vertices.AddRange(hull);triangles.Clear(); // A fan. Every polygon out of the merge is convex, so every diagonal from the first
// corner is inside it and there is nothing to decide.
for(var slot=2;slot<count;slot++){triangles.Add(0);triangles.Add(slot-1);triangles.Add(slot);}Legalise(vertices,triangles);RefineEdges(field,vertices,triangles,count,sampleDistance,maxError,walkableHeight);RefineInterior(field,vertices,triangles,hull,samples,sampleDistance,maxError,walkableHeight); // Counted in triangles, not in indices — the reader multiplies by three, and doing it
// twice is a read three polygons further down the array.
detail.Polys.Add(new(detail.Vertices.Count,vertices.Count-count,detail.Triangles.Count/3,triangles.Count/3));for(var index=count;index<vertices.Count;index++){detail.Vertices.Add(vertices[index]);}detail.Triangles.AddRange(triangles);}return detail;}
/// <summary>The height of the ground at a column, for a caller that already knows roughly where.</summary>
    ...
    ///     </para>
    /// </remarks>
static bool TryGroundHeight(CompactHeightfield field,int x,int z,float expected,int walkableHeight,out float height){height=expected;if(x<0||z<0||x>=field.Width||z>=field.Depth){return false;}ref var cell=ref field.Cells[x+(z*field.Width)];var best=(float)walkableHeight*HeightfieldSpan.DropScale;var found=false;for(var index=cell.Index;index<cell.Index+cell.Count;index++){if(field.Areas[index]==NavArea.Null){continue;} // Both in sixteenths of a voxel: the polygon's plane arrives that way because its corners
// did, and the span reports its surface the same way.
var surface=field.Spans[index].Surface;var difference=MathF.Abs(surface-expected);if(difference<best){best=difference;height=surface;found=true;}}return found;}
/// <summary>Adds vertices along the polygon's edges where the straight edge misses the ground.</summary>
    ...
    ///     </para>
    /// </remarks>
static void RefineEdges(CompactHeightfield field,List<Vector3>vertices,List<int>triangles,int hullCount,float sampleDistance,float maxError,int walkableHeight){for(var edge=0;edge<hullCount;edge++){var first=vertices[edge];var second=vertices[(edge+1)%hullCount];var(from,to)=first.X<second.X||(first.X==second.X&&first.Z<second.Z)?(first,second):(second,first);var length=MathF.Sqrt(((to.X-from.X)*(to.X-from.X))+((to.Z-from.Z)*(to.Z-from.Z)));var steps=(int)MathF.Ceiling(length/sampleDistance);if(steps<2){continue;}for(var step=1;step<steps;step++){if(vertices.Count-hullCount>=MaxEdgeVertices){return;}var fraction=step/(float)steps;var point=new Vector3(from.X+((to.X-from.X)*fraction),from.Y+((to.Y-from.Y)*fraction),from.Z+((to.Z-from.Z)*fraction));if(!TryGroundHeight(field,(int)MathF.Floor(point.X),(int)MathF.Floor(point.Z),point.Y,walkableHeight,out var ground)){continue;}if(MathF.Abs(ground-point.Y)<=maxError){continue;}SplitEdge(vertices,triangles,new(point.X,ground,point.Z));Legalise(vertices,triangles);}}}
/// <summary>Adds vertices inside the polygon, worst first, until nothing is out by more than the tolerance.</summary>
static void RefineInterior(CompactHeightfield field,List<Vector3>vertices,List<int>triangles,List<Vector3>hull,List<Vector3>samples,float sampleDistance,float maxError,int walkableHeight){samples.Clear();var minimumX=float.MaxValue;var minimumZ=float.MaxValue;var maximumX=float.MinValue;var maximumZ=float.MinValue;foreach(var corner in hull){minimumX=MathF.Min(minimumX,corner.X);minimumZ=MathF.Min(minimumZ,corner.Z);maximumX=MathF.Max(maximumX,corner.X);maximumZ=MathF.Max(maximumZ,corner.Z);} // Anchored to a multiple of the spacing rather than to the polygon's own corner, so that two
// polygons over the same ground sample the same places and describe it the same way.
for(var z=MathF.Ceiling(minimumZ/sampleDistance)*sampleDistance;z<=maximumZ;z+=sampleDistance){for(var x=MathF.Ceiling(minimumX/sampleDistance)*sampleDistance;x<=maximumX;x+=sampleDistance){var point=new Vector3(x,0,z);if(!Contains(hull,point)){continue;}if(TryGroundHeight(field,(int)MathF.Floor(x),(int)MathF.Floor(z),Height(vertices,triangles,point),walkableHeight,out var ground)){samples.Add(new(x,ground,z));}}}var budget=MaxInteriorVertices;while(budget-->0){var worst=-1;var worstError=maxError;for(var index=0;index<samples.Count;index++){var current=Height(vertices,triangles,samples[index]);if(float.IsNaN(current)){continue;}var error=MathF.Abs(samples[index].Y-current);if(error>worstError){worstError=error;worst=index;}}if(worst<0){return;}var sample=samples[worst];samples.RemoveAt(worst);SplitInterior(vertices,triangles,sample);Legalise(vertices,triangles);}}
/// <summary>Replaces the triangle a point falls in with three that meet at it.</summary>
static void SplitInterior(List<Vector3>vertices,List<int>triangles,Vector3 point){for(var triangle=0;triangle<triangles.Count;triangle+=3){var a=vertices[triangles[triangle]];var b=vertices[triangles[triangle+1]];var c=vertices[triangles[triangle+2]];if(Side(a,b,point)<0||Side(b,c,point)<0||Side(c,a,point)<0){continue;}var(first,second,third)=(triangles[triangle],triangles[triangle+1],triangles[triangle+2]);var added=vertices.Count;vertices.Add(point);triangles[triangle+2]=added;triangles.AddRange([second,third,added,third,first,added]);return;}}
/// <summary>Replaces the one triangle owning the boundary edge a point sits on with two.</summary>
    ...
    /// </remarks>
static void SplitEdge(List<Vector3>vertices,List<int>triangles,Vector3 point){for(var triangle=0;triangle<triangles.Count;triangle+=3){for(var edge=0;edge<3;edge++){var from=triangles[triangle+edge];var to=triangles[triangle+((edge+1)%3)];if(!IsBoundary(triangles,from,to)||!OnSegment(vertices[from],vertices[to],point)){continue;}var opposite=triangles[triangle+((edge+2)%3)];var added=vertices.Count;vertices.Add(point); // The triangle keeps its winding: the slot that held the far end of the split edge
// now holds the new vertex, and the other half is appended.
triangles[triangle+((edge+1)%3)]=added;triangles.AddRange([added,to,opposite]);return;}}}
/// <summary>Whether an edge is on the polygon's outline: nothing traverses it the other way.</summary>
static bool IsBoundary(List<int>triangles,int from,int to){for(var triangle=0;triangle<triangles.Count;triangle+=3){for(var edge=0;edge<3;edge++){if(triangles[triangle+edge]==to&&triangles[triangle+((edge+1)%3)]==from){return false;}}}return true;}
/// <summary>The height of the detail surface over a point, or NaN if it is not over it.</summary>
static float Height(List<Vector3>vertices,List<int>triangles,Vector3 point){for(var triangle=0;triangle<triangles.Count;triangle+=3){var a=vertices[triangles[triangle]];var b=vertices[triangles[triangle+1]];var c=vertices[triangles[triangle+2]];if(Side(a,b,point)<0||Side(b,c,point)<0||Side(c,a,point)<0){continue;}var area=Side(a,b,c);if(MathF.Abs(area)<1e-6f){continue;}var alpha=Side(b,c,point)/area;var beta=Side(c,a,point)/area;var gamma=1f-alpha-beta;return(a.Y*alpha)+(b.Y*beta)+(c.Y*gamma);}return float.NaN;}
/// <summary>Flips interior edges until every triangle's circumcircle is empty.</summary>
    ...
    ///     </para>
    /// </remarks>
static void Legalise(List<Vector3>vertices,List<int>triangles){for(var sweep=0;sweep<MaxFlipSweeps;sweep++){var flipped=false;for(var triangle=0;triangle<triangles.Count;triangle+=3){for(var slot=0;slot<3;slot++){var apex=triangles[triangle+slot];var from=triangles[triangle+((slot+1)%3)];var to=triangles[triangle+((slot+2)%3)];var other=FindEdge(triangles,to,from,out var otherSlot);if(other<0){continue;}var far=triangles[other+((otherSlot+2)%3)];if(!InCircumcircle(vertices[apex],vertices[from],vertices[to],vertices[far])){continue;} // The quadrilateral has to be convex, or the flip produces two triangles that
// overlap instead of two that tile it.
if(Side(vertices[apex],vertices[from],vertices[far])<=0||Side(vertices[apex],vertices[far],vertices[to])<=0){continue;}triangles[triangle]=apex;triangles[triangle+1]=from;triangles[triangle+2]=far;triangles[other]=apex;triangles[other+1]=far;triangles[other+2]=to;flipped=true;break;}}if(!flipped){return;}}}
/// <summary>The triangle traversing an edge in the given direction, or -1.</summary>
static int FindEdge(List<int>triangles,int from,int to,out int slot){for(var triangle=0;triangle<triangles.Count;triangle+=3){for(slot=0;slot<3;slot++){if(triangles[triangle+slot]==from&&triangles[triangle+((slot+1)%3)]==to){return triangle;}}}slot=-1;return-1;}
/// <summary>Whether a point is inside the circle through three counter-clockwise ones.</summary>
    ...
    /// </remarks>
static bool InCircumcircle(Vector3 a,Vector3 b,Vector3 c,Vector3 point){double ax=a.X-point.X;double az=a.Z-point.Z;double bx=b.X-point.X;double bz=b.Z-point.Z;double cx=c.X-point.X;double cz=c.Z-point.Z;var determinant=(((ax*ax)+(az*az))*((bx*cz)-(cx*bz)))-(((bx*bx)+(bz*bz))*((ax*cz)-(cx*az)))+(((cx*cx)+(cz*cz))*((ax*bz)-(bx*az)));return determinant>1e-9;}
/// <summary>Twice the signed area of a triangle in XZ. Positive when it turns counter-clockwise.</summary>
static float Side(Vector3 a,Vector3 b,Vector3 c)=>((b.X-a.X)*(c.Z-a.Z))-((c.X-a.X)*(b.Z-a.Z));static bool Contains(List<Vector3>hull,Vector3 point){for(var index=0;index<hull.Count;index++){if(Side(hull[index],hull[(index+1)%hull.Count],point)<=0){return false;}}return true;}static bool OnSegment(Vector3 from,Vector3 to,Vector3 point){if(MathF.Abs(Side(from,to,point))>1e-3f){return false;}var along=((point.X-from.X)*(to.X-from.X))+((point.Z-from.Z)*(to.Z-from.Z));var length=((to.X-from.X)*(to.X-from.X))+((to.Z-from.Z)*(to.Z-from.Z));return along>1e-4f&&along<length-1e-4f;}}
