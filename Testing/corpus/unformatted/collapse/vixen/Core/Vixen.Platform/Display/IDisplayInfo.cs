...
using System.Diagnostics.CodeAnalysis; using Vixen.Core.Mathematics; namespace Vixen.Platform;
...
public readonly record struct DisplayMode(Int2 Size, float RefreshRate, bool IsHdr) {
    /// <inheritdoc />
    public override string ToString() => $"{Size.X}×{Size.Y} @ {RefreshRate:0.##} Hz{(IsHdr ? " HDR" : string.Empty)}";
}
