...
public sealed class BuoyancySystem(PhysicsScene scene, IWaterSurface surface) : SystemBase, IDeclaredAccess {
    ...
    public void Step(World world) {
        ...
        WetPontoons = 0;
        LastCount = 0;
        var gravity = scene.World.Gravity.Y; // ⚠ One entity at a time, and not a span. BuoyancyBody holds an array of pontoons, which
    ...
    void Apply(World world, Entity entity, float gravity) {
        ...
        var body = world.Read<PhysicsBody>(
            entity
        ...
        Pontoons += pontoons.Length; // ⚠ The centre of the body, not of a pontoon — QueryAt picks a *zone*, and a hull is smaller
        ...
        var evaluator = query.Evaluator(); // ⚠ No ripples. See the class remarks: the closed form is what a rollback can re-ask.
