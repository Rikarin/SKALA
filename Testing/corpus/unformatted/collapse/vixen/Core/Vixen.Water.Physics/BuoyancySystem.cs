...
using Vixen.Core;using Vixen.Core.Mathematics;using Vixen.Core.Threading;using Vixen.Ecs;using Vixen.Ecs.Systems;using Vixen.Physics.Ecs;namespace Vixen.Water.Physics;
...
[UpdateInGroup(SystemPhase.FixedUpdate)][UpdateAfter(typeof(PhysicsSyncSystem))][UpdateBefore(typeof(PhysicsStepSystem))]public sealed class BuoyancySystem(PhysicsScene scene,IWaterSurface surface):SystemBase,IDeclaredAccess{readonly PhysicsScene scene=scene??throw new ArgumentNullException(nameof(scene));readonly QueryDescription bodies=new QueryDescription().WithAll<BuoyancyBody,PhysicsBody>();readonly List<Entity>asked=[];BuoyancyForce[]forces=new BuoyancyForce[8];
...
/// <inheritdoc />
    ...
    ///     where it is.
    /// </remarks>
public SystemAccess Access{get;}=SystemAccess.Declare().Read<PhysicsBody>().Write<BuoyancyState>().Build();
...
/// <inheritdoc />
public override JobHandle Update(in SystemContext context,JobHandle dependency){ // The sync has just created bodies and pushed authored velocities in, and applying a force
// is a native call the ECS cannot see into. Nothing scheduled may still be reading what it
// writes.
dependency.Complete();Step(context.World);return dependency;}
...
public void Step(World world){ArgumentNullException.ThrowIfNull(world);Floating=0;Pontoons=0;WetPontoons=0;LastCount=0;var gravity=scene.World.Gravity.Y; // ⚠ One entity at a time, and not a span. BuoyancyBody holds an array of pontoons, which
...
asked.Clear();foreach(var chunk in world.Chunks(bodies)){asked.AddRange(chunk.Entities[ .. chunk.Count]);}foreach(var entity in asked){Apply(world,entity,gravity);}}void Apply(World world,Entity entity,float gravity){var authored=world.Read<BuoyancyBody>(entity);if(authored.Pontoons is not{Length:>0}pontoons){return;}var body=world.Read<PhysicsBody>(entity).Handle; // ⚠ Out of the simulation and not out of a component. `WorldTransform` is written by
...
scene.World.GetTransform(body,out var position,out var rotation); // ⚠ Unit scale, and it is not an oversight: Jolt has no notion of a scaled body — a scaled
...
var placement=Matrix4x4.Compose(Vector3.One,rotation,position);var velocity=scene.World.GetLinearVelocity(body);Pontoons+=pontoons.Length; // ⚠ The centre of the body, not of a pontoon — QueryAt picks a *zone*, and a hull is smaller
...
var origin=position;if(Surface.QueryAt(new(origin.X,origin.Z))is not{}query){Dry(world,entity,pontoons.Length);return;}if(forces.Length<pontoons.Length){forces=new BuoyancyForce[pontoons.Length];}var evaluator=query.Evaluator(); // ⚠ No ripples. See the class remarks: the closed form is what a rollback can re-ask.
var wet=Buoyancy.Solve(in evaluator,pontoons,in placement,velocity,gravity,authored.Settings,Surface.WaterTime,forces.AsSpan(0,pontoons.Length));LastCount=pontoons.Length;WetPontoons+=wet;var lift=0f;var submerged=0f;var was=world.Has<BuoyancyState>(entity)?world.Read<BuoyancyState>(entity).Wet:0;for(var index=0;index<pontoons.Length;index++){var force=forces[index];submerged+=force.Submerged;if(force.Submerged<=0f){continue;}lift+=force.Force.Y;Disturb(in force,velocity,entered:was==0); // At the pontoon's own world position, which is what makes the hull pitch. A force at
// the centre of mass would be a boat that bobs and never rolls.
scene.World.ApplyForce(body,force.Force,force.Position);}if(wet>0){Floating++;}world.Set(entity,new BuoyancyState{Wet=wet,Total=pontoons.Length,Submerged=submerged/pontoons.Length,Lift=lift,SurfaceHeight=query.Height(new(origin.X,origin.Z),Surface.WaterTime)});}
/// <summary>Queues whatever this pontoon is doing to the surface, if anything.</summary>
    ...
    ///     </para>
    /// </remarks>
void Disturb(in BuoyancyForce force,Vector3 velocity,bool entered){if(Disturbances is not{}queue){return;}var lateral=new Vector2(velocity.X,velocity.Z).Length();var falling=-velocity.Y;if(entered&&falling>=SplashSpeed){queue.Add(new(new(force.Position.X,force.Position.Z),1f,-falling*force.Submerged,WaterDisturbanceKind.Splash,force.SurfaceHeight));return;}if(lateral>=WakeSpeed){queue.Add(new(new(force.Position.X,force.Position.Z),0.75f,-lateral*0.2f*force.Submerged,WaterDisturbanceKind.Wake,force.SurfaceHeight));}}
/// <summary>Records a body no zone reaches, so the readout says "dry" rather than going stale.</summary>
    ...
    /// </remarks>
static void Dry(World world,Entity entity,int total)=>world.Set(entity,new BuoyancyState{Wet=0,Total=total,Submerged=0f,Lift=0f});}
