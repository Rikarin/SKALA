...
using Vixen.Core;using Vixen.Core.Mathematics;namespace Vixen.Rendering.DistanceFields;
/// <summary>
...
///     </para>
/// </remarks>
[DataContract("MeshDistanceField" )]public sealed class MeshDistanceField:IDistanceField{
...
public MeshDistanceField(BoundingBox bounds,Int3 resolution,float[]distances){Bounds=bounds;Resolution=resolution;Distances=distances;}
...
/// <summary>Where a point sits in a volume texture holding <see cref="Distances" />, as 0..1.</summary>
    ...
    ///     </para>
    /// </remarks>
public Vector3 TextureCoordinate(Vector3 position){var grid=ToGrid(position);var count=new Vector3(Resolution.X,Resolution.Y,Resolution.Z);return(grid+new Vector3(0.5f))/count;}
/// <summary>The signed distance at any point, trilinearly interpolated.</summary>
    ...
    /// </remarks>
public float Sample(Vector3 position){var grid=ToGrid(position);var x0=(int)MathF.Floor(grid.X);var y0=(int)MathF.Floor(grid.Y);var z0=(int)MathF.Floor(grid.Z);x0=Math.Clamp(x0,0,Resolution.X-2);y0=Math.Clamp(y0,0,Resolution.Y-2);z0=Math.Clamp(z0,0,Resolution.Z-2);var tx=Math.Clamp(grid.X-x0,0f,1f);var ty=Math.Clamp(grid.Y-y0,0f,1f);var tz=Math.Clamp(grid.Z-z0,0f,1f);var c00=Lerp(this[x0,y0,z0],this[x0+1,y0,z0],tx);var c10=Lerp(this[x0,y0+1,z0],this[x0+1,y0+1,z0],tx);var c01=Lerp(this[x0,y0,z0+1],this[x0+1,y0,z0+1],tx);var c11=Lerp(this[x0,y0+1,z0+1],this[x0+1,y0+1,z0+1],tx);return Lerp(Lerp(c00,c10,ty),Lerp(c01,c11,ty),tz);}
/// <summary>Which way the distance grows fastest at a point — the surface normal, near one.</summary>
    ...
    /// </remarks>
public Vector3 SampleGradient(Vector3 position){var cell=CellSize;var gradient=new Vector3(Sample(position+new Vector3(cell.X,0,0))-Sample(position-new Vector3(cell.X,0,0)),Sample(position+new Vector3(0,cell.Y,0))-Sample(position-new Vector3(0,cell.Y,0)),Sample(position+new Vector3(0,0,cell.Z))-Sample(position-new Vector3(0,0,cell.Z)));var length=gradient.Length();return length>MathUtil.ZeroTolerance?gradient/length:Vector3.Zero;}
...
public void Validate(){if(Resolution.X<2||Resolution.Y<2||Resolution.Z<2){throw new InvalidOperationException($"A field needs at least two samples along every axis, not {Resolution}." );}if(Distances.Length!=Resolution.Volume){throw new InvalidOperationException($"{Resolution} is {Resolution.Volume} samples, but {Distances.Length} were given." );}if(Bounds.IsEmpty){throw new InvalidOperationException("A field over an empty box describes nothing." );}}
...
Vector3 ToGrid(Vector3 position){var cell=CellSize;var local=position-Bounds.Minimum;return new(cell.X>0?local.X/cell.X:0,cell.Y>0?local.Y/cell.Y:0,cell.Z>0?local.Z/cell.Z:0);}static float Lerp(float from,float to,float amount)=>from+((to-from)*amount);}
