...
using Vixen.Core;using Vixen.Ecs;using Vixen.Net.Sessions;namespace Vixen.Net.Replication;
...
public sealed class AllNetworkedSource:IInterestSource{static readonly QueryDescription Networked=new QueryDescription().RequireAll([ComponentType<NetworkId>.Id]);
/// <inheritdoc />
public void Candidates(World world,PlayerId player,List<Entity>into){ArgumentNullException.ThrowIfNull(world);ArgumentNullException.ThrowIfNull(into);foreach(var chunk in world.Chunks(Networked)){into.AddRange(chunk.Entities);}}}
...
///     </para>
/// </remarks>
public sealed class InterestChain:IInterestResolver{readonly List<Entity>candidates=[];
...
/// <inheritdoc />
public void Resolve(World world,PlayerId player,List<Entity>observed){ArgumentNullException.ThrowIfNull(world);ArgumentNullException.ThrowIfNull(observed);candidates.Clear();Source.Candidates(world,player,candidates);ConsideredCount=candidates.Count;HiddenCount=0;foreach(var entity in candidates){if(Decide(world,player,entity)==Interest.Observed){observed.Add(entity);}else{HiddenCount++;}}}
/// <summary>What the chain says about one object, for a test or a diagnostic.</summary>
    ...
    /// </returns>
public Interest Decide(World world,PlayerId player,Entity entity){foreach(var rule in Rules){var verdict=rule.Decide(world,player,entity);if(verdict!=Interest.Undecided){return verdict;}}return Fallback;}}
...
///     </para>
/// </remarks>
public sealed class ExplicitInterestRule:IInterestRule{readonly Dictionary<uint,Dictionary<uint,Interest>>byPlayer=[];
...
public bool Clear(PlayerId player,NetworkId id)=>byPlayer.TryGetValue(player.Value,out var overrides)&&overrides.Remove(id.Value);
...
/// <summary>Forgets an object that has been destroyed, for every player.</summary>
    ...
    /// </remarks>
public void Forget(NetworkId id){foreach(var overrides in byPlayer.Values){overrides.Remove(id.Value);}}
/// <inheritdoc />
public Interest Decide(World world,PlayerId player,Entity entity){ArgumentNullException.ThrowIfNull(world);if(byPlayer.Count==0||!world.TryGet<NetworkId>(entity,out var id)){return Interest.Undecided;}return byPlayer.TryGetValue(player.Value,out var overrides)&&overrides.TryGetValue(id.Value,out var verdict)?verdict:Interest.Undecided;}void Set(PlayerId player,NetworkId id,Interest verdict){if(!byPlayer.TryGetValue(player.Value,out var overrides)){overrides=[];byPlayer[player.Value]=overrides;}overrides[id.Value]=verdict;}}
