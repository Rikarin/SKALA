...
const uint WholeValue=3;static ReadOnlySpan<int>OffsetWidths=>[4,8,16];
/// <summary>How many bits an encoding described by these lanes occupies.</summary>
    ...
    /// </remarks>
public static int TotalBits(ReadOnlySpan<WireLane>lanes){var total=0;foreach(var lane in lanes){total+=lane.Bits;}return total;}
/// <summary>How many bits of offset a selector code stands for, on a lane of a given width.</summary>
    ...
    /// </remarks>
public static int OffsetWidth(uint selector,int bits)=>selector>=WholeValue?bits:OffsetWidths[(int)selector];
/// <summary>The most a delta over these lanes could ever cost.</summary>
    /// <param name="lanes">The layout.</param>
    /// <returns>The worst case, in bits.</returns>
public static int MaxBits(ReadOnlySpan<WireLane>lanes){var total=0;foreach(var lane in lanes){total+=1+SelectorBits+lane.Bits;}return total;}
...
public static bool TryEncode(ReadOnlySpan<WireLane>lanes,ref BitReader previous,ref BitReader current,ref BitWriter delta,Span<int>costs=default){for(var i=0;i<lanes.Length;i++){var lane=lanes[i];var before=delta.BitsWritten;if(!previous.TryRead(lane.Bits,out var was)||!current.TryRead(lane.Bits,out var now)){return false;}if(was==now){delta.WriteBool(false);}else{delta.WriteBool(true);WriteChange(lane,was,now,ref delta);}if(i<costs.Length){costs[i]=delta.BitsWritten-before;}}return!delta.Overflowed;}static void WriteChange(in WireLane lane,uint was,uint now,ref BitWriter delta){if(!lane.Offset||lane.Bits<MinimumOffsetBits){delta.Write(now,lane.Bits);return;}var zigzag=ZigZag((long)now-was);var selector=Selector(zigzag,lane.Bits);delta.Write(selector,SelectorBits);if(selector==WholeValue){delta.Write(now,lane.Bits);}else{delta.Write((uint)zigzag,OffsetWidths[(int)selector]);}}
...
public static bool TryDecode(ReadOnlySpan<WireLane>lanes,ref BitReader previous,ref BitReader delta,ref BitWriter current){foreach(var lane in lanes){if(!previous.TryRead(lane.Bits,out var was)||!delta.TryReadBool(out var changed)){return false;}if(!changed){current.Write(was,lane.Bits);continue;}if(!lane.Offset||lane.Bits<MinimumOffsetBits){if(!delta.TryRead(lane.Bits,out var whole)){return false;}current.Write(whole,lane.Bits);continue;}if(!delta.TryRead(SelectorBits,out var selector)){return false;}if(selector==WholeValue){if(!delta.TryRead(lane.Bits,out var whole)){return false;}current.Write(whole,lane.Bits);continue;}if(!delta.TryRead(OffsetWidths[(int)selector],out var zigzag)){return false;} // Masked rather than checked: the sender's arithmetic was done on values of this width,
...
current.Write((uint)unchecked(was+UnZigZag(zigzag))&Mask(lane.Bits),lane.Bits);}return!current.Overflowed;}static uint Selector(ulong zigzag,int bits){for(var i=0;i<OffsetWidths.Length;i++){if(OffsetWidths[i]<bits&&zigzag<1ul<<OffsetWidths[i]){return(uint)i;}}return WholeValue;}static uint Mask(int bits)=>bits>=32?uint.MaxValue:(1u<<bits)-1;static ulong ZigZag(long value)=>(ulong)((value<<1)^(value >> 63));static long UnZigZag(uint value)=>(long)(value >> 1)^-(long)(value&1);}
