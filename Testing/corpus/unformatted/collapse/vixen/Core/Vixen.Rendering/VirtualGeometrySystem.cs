...
using Vixen.Graphics;using Vixen.Rendering.Compositor;using Vixen.Rendering.Features;using Vixen.Shaders;namespace Vixen.Rendering;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class VirtualGeometrySystem:IDisposable{bool disposed;
/// <summary>Builds the stack on a device.</summary>
    ...
    /// </remarks>
public VirtualGeometrySystem(IGraphicsDevice device,int slots=512,int pageSize=128*1024){ArgumentNullException.ThrowIfNull(device);Pages=new(device,Source,slots,pageSize);Residency=new(Pages,(long)slots*pageSize); // The traversal reads the slot table the residency keeps, and asks it for the pages a cut wanted
...
Visibility=new(device){Residency=Residency};Raster=new(device){Visibility=Visibility,Pages=Pages};SoftwareRaster=new(device){Visibility=Visibility,Pages=Pages};Tiles=new(device){Visibility=Visibility};Resolve=new(device){Visibility=Visibility,Tiles=Tiles,Pages=Pages};Feature=new(){Visibility=Visibility,Pages=Pages};}
...
/// <summary>Which material each bin holds, as the resolve dispatches them.</summary>
    ...
    /// </remarks>
public IReadOnlyList<ResolveMaterial>Materials{get=>Resolve.Materials;set=>Resolve.Materials=value;}
...
/// <summary>
    ...
    /// </remarks>
public EffectSystem?Effects{get=>Visibility.Effects;set{Visibility.Effects=value;Raster.Effects=value;SoftwareRaster.Effects=value;Tiles.Effects=value;Resolve.Effects=value;}}
/// <summary>Where the compute pipelines come from, on the same terms.</summary>
public ComputePipelineCache?Pipelines{get=>Visibility.Pipelines;set{Visibility.Pipelines=value;SoftwareRaster.Pipelines=value;Tiles.Pipelines=value;Resolve.Pipelines=value;}}
/// <summary>Where the raster's shader modules come from.</summary>
    ...
    /// </remarks>
public EffectPipelineDescriber?Modules{get=>Raster.Modules;set=>Raster.Modules=value;}
/// <summary>The frame's per-frame constants, which the resolve's lighting reads.</summary>
public SceneConstants?Scene{get=>Resolve.Scene;set=>Resolve.Scene=value;}
/// <summary>The frame's per-view constants.</summary>
public ViewConstants?ViewConstants{get=>Resolve.ViewConstants;set=>Resolve.ViewConstants=value;}
/// <summary>
...
public int Content(int id,ReadOnlySpan<byte>hierarchy,ReadOnlySpan<byte>pages,Stream data){ObjectDisposedException.ThrowIf(disposed,this);return VirtualGeometryContent.Load(Feature,Source,id,hierarchy,pages,data);}
/// <summary>Loads a mesh out of the one chunk a build writes the records as.</summary>
    ...
    /// <returns>The index to put in <see cref="VirtualGeometryDraw.Mesh" />.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="data" /> is null.</exception>
public int Content(int id,VirtualGeometryAsset asset,Stream data){ObjectDisposedException.ThrowIf(disposed,this);return VirtualGeometryContent.Load(Feature,Source,id,asset,data);}
/// <summary>
    ...
    ///     </para>
    /// </remarks>
public bool Release(int mesh){ObjectDisposedException.ThrowIf(disposed,this);var id=Feature.Unregister(mesh);if(id<0){return false;}Source.Remove(id);return true;}
...
public void Register(RenderSystem system){ArgumentNullException.ThrowIfNull(system);ObjectDisposedException.ThrowIf(disposed,this);system.AddFeature(Feature);}
/// <summary>Hands a compositor builder everything its two nodes need.</summary>
    ...
    /// </remarks>
public void Supply(CompositorBuilder builder){ArgumentNullException.ThrowIfNull(builder);ObjectDisposedException.ThrowIf(disposed,this);builder.Clusters=Visibility;builder.Pages=Pages;builder.Raster=Raster;builder.SoftwareRaster=SoftwareRaster;builder.Tiles=Tiles;builder.Resolve=Resolve;}
/// <inheritdoc />
public void Dispose(){if(disposed){return;}disposed=true;Resolve.Dispose();Tiles.Dispose();SoftwareRaster.Dispose();Raster.Dispose();Visibility.Dispose();Pages.Dispose();Source.Dispose();}}
