...
public sealed class ComputeRenderer : SceneRenderer, IDisposable {
    ...
    string? Declare(CompositorFrame frame) {
        ...
        frame.Graph.AddPass(
            ToString(),
            pass => { pass.Kind = PassKind.Compute; foreach (var read in bufferReads) {
                    pass.Reads(read);
                ...
                } pass.Execute(context => { context.CommandList.BindPipeline(pipeline); bound?.Bind(
                            context,
                            extra
                        ); OnBind?.Invoke(new(context, this) { Effect = effect }); context.CommandList.Dispatch(
                            groups.X,
                            groups.Y,
                            groups.Z
                        ); }
