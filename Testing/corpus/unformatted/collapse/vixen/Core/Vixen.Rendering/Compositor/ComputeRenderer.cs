...
using Vixen.Core.Mathematics;using Vixen.Graphics;using Vixen.Graphics.RenderGraph;using Vixen.Shaders;namespace Vixen.Rendering.Compositor;
...
/// </remarks>
public sealed class ComputeDispatch{internal ComputeDispatch(RenderGraphContext context,ComputeRenderer node){Context=context;Node=node;}
...
public BufferHandle Buffer(string name){ArgumentException.ThrowIfNullOrEmpty(name);return Context.Buffer(Node.Resolved(name));}RenderGraphContext Context{get;}}
/// <summary>
...
///     </para>
/// </remarks>
public sealed class ComputeRenderer:SceneRenderer,IDisposable{readonly Dictionary<string,GraphBuffer>buffers=new(StringComparer.Ordinal);readonly Dictionary<string,GraphTexture>textures=new(StringComparer.Ordinal);EffectConstants?constants;
...
internal GraphBuffer Resolved(string name)=>buffers.TryGetValue(name,out var buffer)?buffer:throw new CompositorBindingException(ToString(),"buffer" ,name);
/// <inheritdoc />
    /// <remarks>
    ...
    /// </remarks>
protected internal override void Build(GraphicsCompositor compositor,CompositorFrame frame){ArgumentNullException.ThrowIfNull(compositor);ArgumentNullException.ThrowIfNull(frame);Degrade(Declare(frame));}string?Declare(CompositorFrame frame){if(Pipelines is null){return"no Pipelines, so this dispatch was never declared and whatever it writes still " +"holds the last value written to it" ;}if(Groups.X<=0||Groups.Y<=0||Groups.Z<=0){return$"Groups is {Groups.X}×{Groups.Y}×{Groups.Z}, so there is no work to dispatch and " +"whatever this pass writes still holds the last value written to it" ;}var key=EffectKey.From(ShaderName,Parameters,PermutationKeys,Composition);if(frame.Effects.Resolve(key)is not{}effect){ // Also reported by EffectSystem.Misses, which is what makes "no runtime compilation in a
...
return$"the compute effect '{ShaderName}' did not resolve, so this dispatch was never " +"declared and whatever it writes still holds the last value written to it" ;}var pipeline=Pipelines.GetOrCreate(effect);if(!pipeline.IsValid){return$"the compute pipeline for '{ShaderName}' is not valid — the module was missing or " +"the device refused it — so this dispatch was never declared" ;}buffers.Clear();textures.Clear();foreach(var name in BufferReads.Concat(BufferWrites)){buffers[name]=frame.Buffer(ToString(),name);}foreach(var name in Reads.Concat(Writes).Concat(Bound)){textures[name]=frame.Texture(ToString(),name);} // The layout the effect was compiled with, unless the host insisted on one of its own. A set
...
if(!Descriptors.Layout.IsValid&&(int)Descriptors.Slot<effect.SetLayouts.Length){Descriptors.Layout=effect.SetLayouts[(int)Descriptors.Slot];}var bound=Descriptors.Resolve(ToString(),textures,buffers,effect,Samplers); // Filled here rather than in the pass body: the values are the host's, and writing a
// host-visible buffer inside a command list is a map and a copy between two dispatches.
constants??=frame.Device is{}device?new(device,$"{this}.Constants" ):null;var hasConstants=ConstantBinding is not null&&constants?.Update(effect,Parameters)==true; // At the block's own offset, not at zero: the buffer holds one region per frame in flight, so
// changing a value does not overwrite what an unfinished frame is reading.
var extra=hasConstants?new[]{DescriptorWrite.Uniform(ConstantBinding!.Value,constants!.Buffer,constants.Offset,constants.Size)}:[];var bufferReads=BufferReads.Select(name=>buffers[name]).ToArray();var bufferWrites=BufferWrites.Select(name=>buffers[name]).ToArray();var textureReads=Reads.Select(name=>textures[name]).ToArray();var textureWrites=Writes.Select(name=>textures[name]).ToArray();var textureBound=Bound.Select(name=>textures[name]).ToArray();var groups=Groups;frame.Graph.AddPass(ToString(),pass=>{pass.Kind=PassKind.Compute;foreach(var read in bufferReads){pass.Reads(read);}foreach(var write in bufferWrites){pass.Writes(write);}foreach(var read in textureReads){pass.Reads(read);}foreach(var write in textureWrites){pass.Writes(write);} // A use and not a production — see Bound. The state is the one a storage descriptor
...
foreach(var bound in textureBound){pass.Reads(bound,ResourceState.ShaderWrite);}pass.Execute(context=>{context.CommandList.BindPipeline(pipeline);bound?.Bind(context,extra);OnBind?.Invoke(new(context,this){Effect=effect});context.CommandList.Dispatch(groups.X,groups.Y,groups.Z);});});return null;}
/// <inheritdoc />
public void Dispose(){constants?.Dispose();constants=null;}}
