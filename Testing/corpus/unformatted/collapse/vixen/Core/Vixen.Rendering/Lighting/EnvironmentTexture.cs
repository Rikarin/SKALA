...
using System.Runtime.InteropServices;using Vixen.Core.Mathematics;using Vixen.Graphics;namespace Vixen.Rendering.Lighting;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class EnvironmentTexture:IDisposable{readonly CubeImage[]chain;readonly IGraphicsDevice device;TextureHandle texture;TextureViewHandle view;SamplerHandle sampler;BufferHandle staging;bool uploaded;bool disposed;
/// <summary>Mirrors one prefiltered chain.</summary>
    ...
    ///     the handles do not.
    /// </remarks>
public EnvironmentTexture(IGraphicsDevice graphics,IReadOnlyList<CubeImage>prefiltered,PixelFormat format=PixelFormat.Rgba16Float ){ArgumentNullException.ThrowIfNull(graphics);ArgumentNullException.ThrowIfNull(prefiltered);if(prefiltered.Count==0){throw new ArgumentException("A prefiltered chain needs at least one level. An environment with no levels is one a " +"shader would sample at a mip that does not exist." ,nameof(prefiltered));}chain=[ .. prefiltered]; // ⚠ Checked rather than assumed, because the failure is silent: a mip chain whose levels are
...
for(var level=1;level<chain.Length;level++){var expected=Math.Max(1,chain[0].Size >> level);if(chain[level].Size!=expected){throw new ArgumentException($"Level {level} of the chain is {chain[level].Size}² where the level above makes it " +$"{expected}². A mip chain halves, and the hardware assumes so." ,nameof(prefiltered));}}device=graphics;Format=format;Create();}
/// <summary>Bakes a chain from one environment and mirrors it.</summary>
    ...
    /// </remarks>
public static EnvironmentTexture Bake(IGraphicsDevice graphics,CubeImage source,int mipCount=5,int samples=64){ArgumentNullException.ThrowIfNull(graphics);ArgumentNullException.ThrowIfNull(source);return new(graphics,EnvironmentBaker.Prefilter(source,mipCount,samples));}
...
/// <summary>Copies the chain up, once, into the cube the constructor allocated.</summary>
    ...
    /// </remarks>
public bool Upload(ICommandList commands){ArgumentNullException.ThrowIfNull(commands);ObjectDisposedException.ThrowIf(disposed,this);if(uploaded){return false;} // ⚠ Undefined only on the first upload. A cube that was copied into and left in ShaderRead is
// in ShaderRead, and telling the barrier otherwise discards its contents on a tiler.
Transition(commands,Uploads==0?ResourceState.Undefined:ResourceState.ShaderRead,ResourceState.CopyDestination);var offset=0L;for(var level=0;level<chain.Length;level++){var image=chain[level];var faceTexels=image.Size*image.Size;for(var face=0;face<6;face++){device.Write(staging,offset,Staged(image.Face((CubeFace)face)));commands.CopyBufferToTexture(staging,offset,new TextureRegion(texture,level,face),new Int3(image.Size,image.Size,1));offset+=faceTexels*BytesPerTexel;}}Transition(commands,ResourceState.CopyDestination,ResourceState.ShaderRead);uploaded=true;Uploads++;return true;}
/// <summary>Points an environment light at this cube.</summary>
    ...
    ///     </para>
    /// </remarks>
public void Apply(EnvironmentLight light){ArgumentNullException.ThrowIfNull(light);ObjectDisposedException.ThrowIf(disposed,this);light.Prefiltered=view;light.Sampler=sampler;light.MipCount=MipCount;}
/// <inheritdoc />
public void Dispose(){if(disposed){return;}disposed=true;if(view.IsValid){device.Destroy(view);}if(texture.IsValid){device.Destroy(texture);}if(staging.IsValid){device.Destroy(staging);}if(sampler.IsValid){device.Destroy(sampler);}}
...
long StagingSize{get{var total=0L;foreach(var level in chain){total+=(long)level.Size*level.Size*6*BytesPerTexel;}return total;}}
/// <summary>Moves the whole cube from one state to another, in one barrier.</summary>
void Transition(ICommandList commands,ResourceState before,ResourceState after)=>commands.Barrier(new([],[new TextureBarrier(texture,before,after)]));
/// <summary>One face's texels in whatever <see cref="Format" /> says, with an opaque alpha.</summary>
    ...
    /// </remarks>
ReadOnlySpan<byte>Staged(ReadOnlySpan<Vector3>texels){if(Format==PixelFormat.Rgba32Float ){var wide=new float[texels.Length*4];for(var index=0;index<texels.Length;index++){wide[(index*4)+0]=texels[index].X;wide[(index*4)+1]=texels[index].Y;wide[(index*4)+2]=texels[index].Z;wide[(index*4)+3]=1f;}return MemoryMarshal.AsBytes(wide.AsSpan());}var narrow=new Half[texels.Length*4];for(var index=0;index<texels.Length;index++){narrow[(index*4)+0]=(Half)texels[index].X;narrow[(index*4)+1]=(Half)texels[index].Y;narrow[(index*4)+2]=(Half)texels[index].Z;narrow[(index*4)+3]=(Half)1f;}return MemoryMarshal.AsBytes(narrow.AsSpan());}
/// <summary>Allocates the cube, the staging buffer, the view and the sampler.</summary>
void Create(){texture=device.CreateTexture(new TextureDescription(Format,Size,Size,TextureUsage.Sampled|TextureUsage.CopyDestination,MipLevels:MipCount, // Six, and the dimension says what they mean. A cube bound as an array is a shader
// sampling by layer index where it meant to sample by direction.
ArrayLayers:6,Dimension:TextureDimension.TextureCube,Name:"Environment" ));view=device.CreateTextureView(texture);staging=device.CreateBuffer(new BufferDescription(StagingSize,BufferUsage.CopySource,MemoryAccess.HostUpload,"Environment.Staging" )); // MaxLod is the chain's last level rather than the default thousand: a roughness that selects
// past the levels that exist is clamped by the sampler, not by the shader's arithmetic.
sampler=device.CreateSampler(SamplerDescription.LinearClamp with{MaxLod=MipCount-1,Name="Environment" });}}
