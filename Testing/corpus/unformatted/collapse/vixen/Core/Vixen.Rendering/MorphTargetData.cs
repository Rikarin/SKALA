...
using Vixen.Core;using Vixen.Core.Mathematics;namespace Vixen.Rendering;
...
///     </para>
/// </remarks>
[DataContract("MorphTarget" )]public sealed record MorphTargetData{
...
/// <summary>What this target costs, resident, not counting its name.</summary>
    ...
    /// </remarks>
public long SizeInBytes=>(Indices.Length*(long)sizeof(int))+(Positions.Length*(long)sizeof(short))+(Normals.Length*(long)sizeof(short));
...
public Vector3 PositionDelta(int entry){ArgumentOutOfRangeException.ThrowIfNegative(entry);ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(entry,Count);return Dequantize(Positions,entry,PositionScale);}
/// <summary>The normal delta of one entry, dequantised. Zero where there are none.</summary>
    /// <param name="entry">Which entry, in <c>[0, <see cref="Count" />)</c>.</param>
    /// <returns>The delta.</returns>
    /// <exception cref="ArgumentOutOfRangeException">The entry is outside the target.</exception>
public Vector3 NormalDelta(int entry){ArgumentOutOfRangeException.ThrowIfNegative(entry);ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(entry,Count);return HasNormals?Dequantize(Normals,entry,NormalScale):Vector3.Zero;}
...
public static MorphTargetData Encode(string name,ReadOnlySpan<int>indices,ReadOnlySpan<Vector3>positions,ReadOnlySpan<Vector3>normals){ArgumentNullException.ThrowIfNull(name);if(positions.Length!=indices.Length){throw new ArgumentException($"'{name}' has {indices.Length} indices and {positions.Length} position deltas." ,nameof(positions));}if(normals.Length!=0&&normals.Length!=indices.Length){throw new ArgumentException($"'{name}' has {indices.Length} indices and {normals.Length} normal deltas. A target " +"carries a normal delta for every vertex it moves, or none at all." ,nameof(normals));}var positionScale=Range(positions);var normalScale=Range(normals);return new(){Name=name,Indices=indices.ToArray(),PositionScale=positionScale,Positions=Quantize(positions,positionScale),NormalScale=normalScale,Normals=normals.Length==0?[]:Quantize(normals,normalScale)};}
/// <summary>Builds a target from one delta per vertex, dropping the ones that do not move.</summary>
    ...
    ///     </para>
    /// </remarks>
public static MorphTargetData Sparsify(string name,ReadOnlySpan<Vector3>positions,ReadOnlySpan<Vector3>normals,float threshold){ArgumentNullException.ThrowIfNull(name);if(normals.Length!=0&&normals.Length!=positions.Length){throw new ArgumentException($"'{name}' has {positions.Length} position deltas and {normals.Length} normal deltas." ,nameof(normals));}var squared=threshold*threshold;List<int>kept=[];for(var index=0;index<positions.Length;index++){var moves=positions[index].LengthSquared()>squared;var reshades=normals.Length!=0&&normals[index].LengthSquared()>squared;if(moves||reshades){kept.Add(index);}}var keptPositions=new Vector3[kept.Count];var keptNormals=normals.Length==0?[]:new Vector3[kept.Count];for(var entry=0;entry<kept.Count;entry++){keptPositions[entry]=positions[kept[entry]];if(normals.Length!=0){keptNormals[entry]=normals[kept[entry]];}}return Encode(name,[ .. kept],keptPositions,keptNormals);}
/// <summary>The largest absolute component over a run of deltas, which is the target's range.</summary>
static float Range(ReadOnlySpan<Vector3>deltas){var range=0f;foreach(var delta in deltas){range=MathF.Max(range,MathF.Abs(delta.X));range=MathF.Max(range,MathF.Abs(delta.Y));range=MathF.Max(range,MathF.Abs(delta.Z));}return range;}
/// <summary>Three signed shorts per delta, against a range.</summary>
    ...
    /// </remarks>
static short[]Quantize(ReadOnlySpan<Vector3>deltas,float range){var packed=new short[deltas.Length*3];if(range<=0f){return packed;}var scale=Quantum/range;for(var index=0;index<deltas.Length;index++){packed[(index*3)+0]=Component(deltas[index].X,scale);packed[(index*3)+1]=Component(deltas[index].Y,scale);packed[(index*3)+2]=Component(deltas[index].Z,scale);}return packed;}static short Component(float value,float scale)=>(short)Math.Clamp((int)MathF.Round(value*scale),-Quantum,Quantum);
/// <summary>
    ...
    ///     same reason.
    /// </remarks>
static Vector3 Dequantize(short[]packed,int entry,float scale){var step=scale/Quantum;return new(packed[(entry*3)+0]*step,packed[(entry*3)+1]*step,packed[(entry*3)+2]*step);}}
