...
public sealed class VfxGpuSimulation : IDisposable {
    ...
    readonly VfxShaderBinding[] particles; // Two full sets of the particle attributes, and which one is the live set flips on every reap.
    ...
    readonly byte[] scratch; // Rebuilt in place rather than allocated per call: a dispatch pair is a per-frame path, and this
    // is the only array in it.
    readonly BufferBarrier[] barriers;
    readonly BufferHandle staging;
    readonly BufferHandle readback; // Only when the shader reaps. An invalid handle is what "this effect has no lifetime" looks like
    ...
    readonly PipelineLayoutHandle layout; // What the storage buffers were last left in, so a barrier can name a truthful `before`. One
    ...
    public VfxGpuSimulation(IGraphicsDevice device, VfxShader shader, int capacity) {
        ...
        Bytes = total;
        scratch = new byte[total]; // ⚠ **A second full set of the attribute buffers when the shader reaps, and it is the whole
        ...
        if (shader.HasReap) {
            ...
            template = device.CreateBuffer(
                new(DrawArgumentsSize, BufferUsage.CopySource, MemoryAccess.HostUpload, $"{shader.Name}.DrawTemplate")
        ...
        barriers = new BufferBarrier[(particles.Length * copies)
            + (shader.HasReap
                    ? 1
                    : 0)]; // Set 0, because the emitted bindings are [PerFrame] — see VfxShaderEmitter, which explains
        ...
        layout = device.CreatePipelineLayout(
            new([setLayout], [new(ShaderStage.Compute, 0, VfxShaderUniforms.Size)], $"{shader.Name}.Layout")
    ...
    /// </remarks>
    public int IndicesPerParticle { get => indicesPerParticle; set { ArgumentOutOfRangeException.ThrowIfNegative(
                value
            ); indicesPerParticle = value; if (!template.IsValid) {
                return;
            ...
            // decides.
            Span<uint> command = [(uint)value, 0u, 0u, 0u, 0u]; device.Write(
                template,
                0,
                MemoryMarshal.AsBytes(command)
            ); } }
    ...
    void Dispatch(ICommandList list, PipelineHandle pipeline, VfxShaderUniforms constants) {
        ...
        Dispatches++; // Left in ShaderWrite, so a second dispatch — an initialize followed by an update, the usual
