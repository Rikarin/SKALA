...
using System.Runtime.InteropServices;using Vixen.Core.Mathematics;using Vixen.Graphics;using Vixen.Vfx;namespace Vixen.Rendering;
/// <summary>
...
/// <summary>How many bytes a <c>DrawIndexedIndirect</c> command occupies.</summary>
    ...
    ///     stride to.
    /// </remarks>
public const int DrawArgumentsSize=20;readonly IGraphicsDevice device;readonly VfxShaderBinding[]bindings;readonly VfxShaderBinding[]particles; // Two full sets of the particle attributes, and which one is the live set flips on every reap.
...
readonly BufferHandle[][]sets;readonly DescriptorSetHandle[]descriptorSets;int current;readonly long[]offsets;readonly byte[]scratch; // Rebuilt in place rather than allocated per call: a dispatch pair is a per-frame path, and this
// is the only array in it.
readonly BufferBarrier[]barriers;readonly BufferHandle staging;readonly BufferHandle readback; // Only when the shader reaps. An invalid handle is what "this effect has no lifetime" looks like
// everywhere below, which is one test rather than a parallel `hasReap` flag to keep in step.
readonly BufferHandle counter;readonly BufferHandle zero;readonly BufferHandle counterReadback;readonly BufferHandle template;readonly BufferHandle arguments;readonly DescriptorSetLayoutHandle setLayout;readonly PipelineLayoutHandle layout; // What the storage buffers were last left in, so a barrier can name a truthful `before`. One
// state for all of them because every path here touches all of them together.
ResourceState state=ResourceState.Undefined;ResourceState argumentState=ResourceState.Undefined;bool disposed;
...
public VfxGpuSimulation(IGraphicsDevice device,VfxShader shader,int capacity){ArgumentNullException.ThrowIfNull(device);ArgumentNullException.ThrowIfNull(shader);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(capacity);this.device=device;Capacity=capacity;HasReap=shader.HasReap;bindings=[ .. shader.Bindings];particles=[ .. bindings.Where(binding=>binding.Role==VfxBindingRole.Particle)];offsets=new long[particles.Length];var total=0L;for(var index=0;index<particles.Length;index++){offsets[index]=total;total+=VfxShaderPacking.Size(particles[index],capacity);}Bytes=total;scratch=new byte[total]; // ⚠ **A second full set of the attribute buffers when the shader reaps, and it is the whole
...
var copies=shader.HasReap?2:1;sets=new BufferHandle[copies][];for(var copy=0;copy<copies;copy++){sets[copy]=new BufferHandle[particles.Length];for(var index=0;index<particles.Length;index++){sets[copy][index]=device.CreateBuffer(new(VfxShaderPacking.Size(particles[index],capacity),BufferUsage.Storage|BufferUsage.CopySource|BufferUsage.CopyDestination,MemoryAccess.DeviceLocal,$"{shader.Name}.{particles[index].Name}{(copy == 0 ? "" : "B")}" ));}} // One staging buffer and one readback buffer covering every attribute, rather than a pair
...
staging=device.CreateBuffer(new(total,BufferUsage.CopySource,MemoryAccess.HostUpload,$"{shader.Name}.Staging" ));readback=device.CreateBuffer(new(total,BufferUsage.CopyDestination,MemoryAccess.HostReadback,$"{shader.Name}.Readback" ));if(shader.HasReap){counter=device.CreateBuffer(new(sizeof(uint),BufferUsage.Storage|BufferUsage.CopySource|BufferUsage.CopyDestination,MemoryAccess.DeviceLocal,$"{shader.Name}.Survivors" )); // ⚠ A four-byte upload buffer holding zero, written once here and copied over the counter
...
zero=device.CreateBuffer(new(sizeof(uint),BufferUsage.CopySource,MemoryAccess.HostUpload,$"{shader.Name}.Zero" ));device.Write(zero,0,new byte[sizeof(uint)]);counterReadback=device.CreateBuffer(new(sizeof(uint),BufferUsage.CopyDestination,MemoryAccess.HostReadback,$"{shader.Name}.SurvivorsReadback" ));template=device.CreateBuffer(new(DrawArgumentsSize,BufferUsage.CopySource,MemoryAccess.HostUpload,$"{shader.Name}.DrawTemplate" )); // ⚠ `CopySource` as well, which a draw does not need. A command a device assembles and
...
arguments=device.CreateBuffer(new(DrawArgumentsSize,BufferUsage.Indirect|BufferUsage.CopyDestination|BufferUsage.CopySource,MemoryAccess.DeviceLocal,$"{shader.Name}.DrawArguments" ));IndicesPerParticle=6;}barriers=new BufferBarrier[(particles.Length*copies)+(shader.HasReap?1:0)]; // Set 0, because the emitted bindings are [PerFrame] — see VfxShaderEmitter, which explains
// why a compute pipeline with nothing else in it does not want the material set.
var entries=new DescriptorBinding[bindings.Length];for(var index=0;index<bindings.Length;index++){entries[index]=new((uint)index,DescriptorKind.StorageBuffer,ShaderStage.Compute);}setLayout=device.CreateDescriptorSetLayout(new(DescriptorSetSlot.PerFrame,entries,$"{shader.Name}.Particles" ));layout=device.CreatePipelineLayout(new([setLayout],[new(ShaderStage.Compute,0,VfxShaderUniforms.Size)],$"{shader.Name}.Layout" )); // ⚠ **One descriptor set per direction, rather than one set rewritten between frames.**
...
descriptorSets=new DescriptorSetHandle[copies];for(var copy=0;copy<copies;copy++){descriptorSets[copy]=device.CreateDescriptorSet(setLayout,$"{shader.Name}.Particles{copy}" );var writes=new DescriptorWrite[bindings.Length];for(var index=0;index<bindings.Length;index++){writes[index]=DescriptorWrite.Storage((uint)index,bindings[index].Role switch{VfxBindingRole.Particle=>sets[copy][index],VfxBindingRole.Compacted=>sets[1-copy][index-particles.Length],_=>counter});}device.UpdateDescriptorSet(descriptorSets[copy],writes);}}
...
/// <summary>How many indices one particle's geometry uses.</summary>
    ...
    /// </remarks>
public int IndicesPerParticle{get=>indicesPerParticle;set{ArgumentOutOfRangeException.ThrowIfNegative(value);indicesPerParticle=value;if(!template.IsValid){return;} // indexCount, instanceCount, firstIndex, vertexOffset, firstInstance. The instance count
// is left at zero and overwritten from the counter, which is the one field a device
// decides.
Span<uint>command=[(uint)value,0u,0u,0u,0u];device.Write(template,0,MemoryMarshal.AsBytes(command));}}int indicesPerParticle;
/// <summary>The buffer holding one attribute, for a renderer that wants to draw from it.</summary>
    ...
    ///     an exception.
    /// </remarks>
public BufferHandle Storage(VfxAttribute attribute){for(var index=0;index<particles.Length;index++){if(particles[index].Slot<0&&particles[index].Attribute==attribute){return sets[current][index];}}return default;}
...
public BufferHandle Custom(int slot){for(var index=0;index<particles.Length;index++){if(particles[index].Slot==slot){return sets[current][index];}}return default;}
/// <summary>How many workgroups cover a run of particles.</summary>
public static int Groups(int count)=>count<=0?0:((count+VfxShader.WorkgroupSize-1)/VfxShader.WorkgroupSize);
/// <summary>Records the copies that put a CPU buffer's particles on the device.</summary>
    ...
    /// </remarks>
public void Upload(ICommandList list,ParticleBuffer particles,int count){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(list);ArgumentNullException.ThrowIfNull(particles);ArgumentOutOfRangeException.ThrowIfNegative(count);ArgumentOutOfRangeException.ThrowIfGreaterThan(count,Capacity);if(count==0){return;}for(var index=0;index<this.particles.Length;index++){var bytes=VfxShaderPacking.Size(this.particles[index],count);VfxShaderPacking.Pack(particles,this.particles[index],count,scratch.AsSpan((int)offsets[index],bytes));}device.Write(staging,0,scratch);Transition(list,ResourceState.CopyDestination);for(var index=0;index<this.particles.Length;index++){list.CopyBuffer(staging,offsets[index],sets[current][index],0,VfxShaderPacking.Size(this.particles[index],count));}state=ResourceState.CopyDestination;}
/// <summary>Records the initializer dispatch over one run of newly spawned particles.</summary>
    ...
    /// </remarks>
public void Initialize(ICommandList list,PipelineHandle pipeline,int first,int count,uint seed,float time,Vector3 origin=default)=>Dispatch(list,pipeline,new(){DeltaTime=0f,Seed=seed,First=first,ParticleCount=count,Time=time,Origin=origin});
/// <summary>Records the update dispatch over every live particle.</summary>
    ...
    /// <exception cref="ArgumentNullException"><paramref name="list" /> is null.</exception>
public void Update(ICommandList list,PipelineHandle pipeline,int count,float deltaTime,uint seed,float time)=>Dispatch(list,pipeline,new(){DeltaTime=deltaTime,Seed=seed,First=0,ParticleCount=count,Time=time});
/// <summary>Records the copies that bring the device's particles back.</summary>
    ...
    /// </remarks>
public void Download(ICommandList list,int count){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(list);ArgumentOutOfRangeException.ThrowIfNegative(count);ArgumentOutOfRangeException.ThrowIfGreaterThan(count,Capacity);if(count==0){return;}Transition(list,ResourceState.CopySource);for(var index=0;index<particles.Length;index++){list.CopyBuffer(sets[current][index],0,readback,offsets[index],VfxShaderPacking.Size(particles[index],count));}state=ResourceState.CopySource;}
/// <summary>Puts a completed <see cref="Download" /> into a CPU buffer.</summary>
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="count" /> is negative or above the capacity.</exception>
public void Read(ParticleBuffer particles,int count){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(particles);ArgumentOutOfRangeException.ThrowIfNegative(count);ArgumentOutOfRangeException.ThrowIfGreaterThan(count,Capacity);if(count==0){return;}device.Read(readback,0,scratch);for(var index=0;index<this.particles.Length;index++){var bytes=VfxShaderPacking.Size(this.particles[index],count);VfxShaderPacking.Unpack(scratch.AsSpan((int)offsets[index],bytes),this.particles[index],count,particles);}}
/// <summary>Records the compaction: every survivor claims a slot, and the dead leave no hole.</summary>
    ...
    ///     </para>
    /// </remarks>
public void Reap(ICommandList list,PipelineHandle pipeline,int count){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(list);ArgumentOutOfRangeException.ThrowIfNegative(count);ArgumentOutOfRangeException.ThrowIfGreaterThan(count,Capacity);if(!HasReap){throw new InvalidOperationException("This effect's shader has no reap kernel, so there is nothing to compact and no buffers to " +"compact into. A graph needs both an age and a lifetime before a particle can be finished — " +"see VfxShader.HasReap." );}if(count==0){return;}Transition(list,ResourceState.CopyDestination);list.CopyBuffer(zero,0,counter,0,sizeof(uint));Transition(list,ResourceState.ShaderWrite);list.BindPipeline(pipeline);list.BindDescriptorSet(DescriptorSetSlot.PerFrame,descriptorSets[current]);list.PushConstants(ShaderStage.Compute,0,Raw(new(){DeltaTime=0f,Seed=0u,First=0,ParticleCount=count,Time=0f}));list.Dispatch(Groups(count));Dispatches++;state=ResourceState.ShaderWrite;Transition(list,ResourceState.CopySource);list.CopyBuffer(counter,0,counterReadback,0,sizeof(uint));state=ResourceState.CopySource;current=1-current;}
/// <summary>How many particles the last <see cref="Reap" /> kept.</summary>
    ...
    /// </remarks>
public int ReadSurvivors(){ObjectDisposedException.ThrowIf(disposed,this);if(!HasReap){throw new InvalidOperationException("This effect's shader has no reap kernel, so nothing counted." );}Span<byte>bytes=stackalloc byte[sizeof(uint)];device.Read(counterReadback,0,bytes);return(int)BitConverter.ToUInt32(bytes);}
/// <summary>Records the copies that put the survivor count into the indirect draw command.</summary>
    ...
    ///     </para>
    /// </remarks>
public void WriteDrawArguments(ICommandList list){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(list);if(!HasReap){throw new InvalidOperationException("This effect's shader has no reap kernel, so there is no device-side count to draw from. An " +"effect whose particles never die knows how many it has without asking." );}list.Barrier(new([new(arguments,argumentState,ResourceState.CopyDestination)],[]));list.CopyBuffer(template,0,arguments,0,DrawArgumentsSize); // The counter is in CopySource from the end of `Reap`, which is where a copy out of it wants
// it — so the only barrier this needs is the argument buffer's own.
list.CopyBuffer(counter,0,arguments,sizeof(uint),sizeof(uint));list.Barrier(new([new(arguments,ResourceState.CopyDestination,ResourceState.IndirectArgument)],[]));argumentState=ResourceState.IndirectArgument;}
/// <summary>Frees every device object it made.</summary>
public void Dispose(){if(disposed){return;}disposed=true;foreach(var set in descriptorSets){device.Destroy(set);}device.Destroy(layout);device.Destroy(setLayout);device.Destroy(readback);device.Destroy(staging);foreach(var set in sets){foreach(var buffer in set){device.Destroy(buffer);}}if(!HasReap){return;}device.Destroy(arguments);device.Destroy(template);device.Destroy(counterReadback);device.Destroy(zero);device.Destroy(counter);}void Dispatch(ICommandList list,PipelineHandle pipeline,VfxShaderUniforms constants){ObjectDisposedException.ThrowIf(disposed,this);ArgumentNullException.ThrowIfNull(list);if(constants.ParticleCount<=0){return;}Transition(list,ResourceState.ShaderWrite);list.BindPipeline(pipeline);list.BindDescriptorSet(DescriptorSetSlot.PerFrame,descriptorSets[current]);list.PushConstants(ShaderStage.Compute,0,Raw(constants));list.Dispatch(Groups(constants.ParticleCount));Dispatches++; // Left in ShaderWrite, so a second dispatch — an initialize followed by an update, the usual
...
/// <summary>Barriers every attribute buffer into a state, from whatever it was left in.</summary>
    ...
    /// </remarks>
void Transition(ICommandList list,ResourceState next){var at=0;foreach(var set in sets){foreach(var buffer in set){barriers[at++]=new(buffer,state,next);}}if(HasReap){barriers[at]=new(counter,state,next);}list.Barrier(new(barriers,[]));}static ReadOnlySpan<byte>Raw(in VfxShaderUniforms constants)=>MemoryMarshal.AsBytes(new ReadOnlySpan<VfxShaderUniforms>(in constants))[ .. VfxShaderUniforms.Size];}
