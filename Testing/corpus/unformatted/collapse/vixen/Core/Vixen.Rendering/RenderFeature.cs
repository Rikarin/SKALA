...
///     </para>
/// </remarks>
public abstract class RootRenderFeature{readonly List<SubRenderFeature>subFeatures=[];
...
public void Add(SubRenderFeature subFeature){ArgumentNullException.ThrowIfNull(subFeature);if(subFeature.Parent is not null){throw new InvalidOperationException($"Sub-feature '{subFeature.Name}' already belongs to '{subFeature.Parent.Name}'." );}subFeature.Parent=this;subFeatures.Add(subFeature);if(System is not null){subFeature.Initialize(System);}}
...
/// <summary>The sort group for one object, which the key puts above depth.</summary>
    ...
    ///     one.
    /// </remarks>
protected internal virtual uint SortGroupOf(RenderSystem system,RenderObjectId id,RenderStage stage)=>system.Objects[id].SortGroup;internal void InitializeAll(RenderSystem system){Initialize(system);foreach(var subFeature in subFeatures){subFeature.Initialize(system);}}internal void ExtractAll(RenderSystem system){Extract(system);foreach(var subFeature in subFeatures){subFeature.Extract(system);}}internal void PrepareAll(RenderSystem system){Prepare(system);foreach(var subFeature in subFeatures){subFeature.Prepare(system);}}
/// <inheritdoc />
