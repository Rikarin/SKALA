...
using Vixen.Core.Mathematics; using Vixen.Rendering; using Vixen.Rendering.Features; using Xunit; namespace Vixen.Rendering.Tests;
...
public sealed class InstanceCullingTests {
    ...
    public void InstancesBehindTheViewAreDropped() {
        ...
        InstanceBounds[] instances =
            [new(new(0f, 0f, 10f), 0.5f), new(new(0f, 0f, -10f), 0.5f), new(new(0f, 0f, 20f), 0.5f)];
    ...
    public void SurvivorsAreBinnedByLevelAndEachRunIsContiguousAndAscending() {
        var culler = new InstanceCuller(); // Instances at 1…20 m; level 0 out to 5, level 1 out to 12, level 2 beyond.
        var instances = Line(20);
    ...
    public void ALevelBoundaryIsInclusiveOfTheFartherLevel() {
        var culler = new InstanceCuller(); // Exactly on the boundary belongs to the coarser level, and the rule is stated so the two
    ...
    public void WithNoAuthoredParametersEverySurvivorGetsNeutralOnes() {
        ...
        Assert.All(
            ...
            parameter => { Assert.Equal(1f, parameter.Scale); Assert.Equal(1f, parameter.Fade); }
        );
    }
