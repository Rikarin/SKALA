...
using Vixen.Ai.Ecs; using Vixen.Ai.Perception.Ecs; using Vixen.Core; using Vixen.Core.Mathematics; using Vixen.Ecs; using Xunit; namespace Vixen.Ai.Perception.Tests; public class SightTests {
    [Fact]
    ...
    public void TheConeIsWhatTheAgentIsFacing() {
        ...
        fleet.Step();
        Assert.True(
            fleet.Perceived(listener).IsPerceiving(SenseMask.Sight)
    ...
    public void DistanceLodStretchesTheIntervalInBands() {
        ...
        Assert.Equal(
            0.15f,
            governor.IntervalFor(config, 30f),
            4
    ...
    public void NoFocusMeansFullRateRatherThanTheSlowestBand() {
        ...
        fleet.Step(4); // Five hundred metres out, so a pass every 0.25 s against a 0.1 s frame.
