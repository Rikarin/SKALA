...
using Vixen.Audio.Devices; using Vixen.Audio.Effects; using Vixen.Audio.Mixing; using Xunit; namespace Vixen.Audio.Tests;
...
public sealed class PerVoiceSendTests {
    ...
    public void ASoundWithASendReachesBoth() {
        ...
        using (engine) {
            ...
            Assert.True(
                direct > 0.5f,
                $"the dry path was only {direct:F3}"
    ...
    public void TwoSoundsOnOneBusCanBeWetByDifferentAmounts() {
        ...
        using (engine) {
            ...
            var loud = PeakOf(device, far); // Eight times as wet, on the same bus, from the same block.
