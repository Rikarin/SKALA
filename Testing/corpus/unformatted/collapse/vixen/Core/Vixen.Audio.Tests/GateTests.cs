...
public sealed class GateTests {
    ...
    public void QuietSignalIsClosedDownAndLoudSignalIsNot() {
        var gate = Gate(); // A tenth of a second of speech-level signal, which opens it.
        var loud = Level(0.5f, 4_800);
        gate.Process(loud, 4_800, 1);
        Assert.True(
            AudioTestData.Peak(loud.AsSpan(2_400)) > 0.4f,
    ...
    public void ItOpensQuicklyEnoughNotToClipTheStartOfAWord() {
        ...
        gate.Process(word, 480, 1); // Within ten milliseconds it is essentially all the way open.
    ...
    public void TheRangeIsHowFarDownAClosedGateGoesRatherThanSilence() {
        var gate = Gate();
        gate.RangeDb = -12f;
        gate.Reset(); // −60 dB of room tone, which is well under the threshold.
        var quiet = Level(0.001f, 9_600);
        gate.Process(quiet, 9_600, 1); // −12 dB is a quarter of it, not nothing.
