...
using System.Numerics;namespace Vixen.Ui.Layout;
...
///     </para>
/// </remarks>
sealed class FragmentArena{const int MinimumBlock=2;const int MinimumBlockLog2=1;const int BucketCount=24;readonly List<int>?[]free=new List<int>?[BucketCount];LayoutFragment[]storage=new LayoutFragment[16];int used;
/// <summary>The fragments in a block.</summary>
    ...
    /// </remarks>
public Span<LayoutFragment>Slice(int offset,int length)=>offset<0||length<=0?default:storage.AsSpan(offset,length);
...
public(int Offset,int Capacity)Write(int offset,int capacity,ReadOnlySpan<LayoutFragment>fragments){if(fragments.IsEmpty){Free(offset,capacity);return(-1,0);}if(offset>=0&&capacity>=fragments.Length){fragments.CopyTo(storage.AsSpan(offset,fragments.Length));return(offset,capacity);}Free(offset,capacity);var wanted=int.Max(MinimumBlock,(int)BitOperations.RoundUpToPowerOf2((uint)fragments.Length));var next=Allocate(wanted);fragments.CopyTo(storage.AsSpan(next,fragments.Length));return(next,wanted);}
/// <summary>Hands a block back.</summary>
    /// <param name="offset">Where it starts, or -1.</param>
    /// <param name="capacity">How big it is.</param>
public void Free(int offset,int capacity){if(offset<0||capacity<MinimumBlock){return;}var bucket=BucketOf(capacity);if(bucket>=BucketCount){return;}(free[bucket]??=[]).Add(offset);}int Allocate(int size){var bucket=BucketOf(size);if(bucket<BucketCount&&free[bucket]is{Count:>0}available){var offset=available[^1];available.RemoveAt(available.Count-1);return offset;}if(used+size>storage.Length){Array.Resize(ref storage,int.Max(storage.Length*2,used+size));}var allocated=used;used+=size;return allocated;}static int BucketOf(int size)=>BitOperations.TrailingZeroCount((uint)size)-MinimumBlockLog2;}
