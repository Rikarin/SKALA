...
using System.Runtime.InteropServices;namespace Vixen.Ui.Layout;
/// <summary>
...
/// <summary>Sets which ordinal group an item is laid out and painted in.</summary>
    ...
    /// </remarks>
public void SetOrder(LayoutNodeId node,int order){var index=Validate(node);if(styles[index].Order==order){return;}styles[index].Order=order;InvalidateChildOrder(links[index].Parent,order);MarkDirtyAndPropagate(index);}
/// <summary>Where a node's ordered child block lives, or -1 if it has none.</summary>
    ...
    /// </remarks>
[StructLayout(LayoutKind.Sequential)]struct OrderedChildren{public int Offset;public int Capacity;}
...
/// <summary>Records that a parent's order-modified child list has to be rebuilt.</summary>
    ...
    /// </param>
void InvalidateChildOrder(int parent,int order){if(parent<0||(order==0&&!HasOrderedBlock(parent))){return;}if((flags[parent]&LayoutNodeState.ChildOrderStale)!=0){return;}flags[parent]|=LayoutNodeState.ChildOrderStale;(reorderQueue??=[]).Add(parent);}
/// <summary>Rebuilds every stale order-modified child list, before the pass descends.</summary>
    ...
    /// </remarks>
void FlushChildOrder(){if(reorderQueue is not{Count:>0}queue){return;}foreach(var index in queue){ // The slot may have been destroyed, or destroyed and handed to a new node, since it was
// queued. Both are caught here: `CreateNode` clears the flag it would have inherited.
if((flags[index]&(LayoutNodeState.Live|LayoutNodeState.ChildOrderStale))==(LayoutNodeState.Live|LayoutNodeState.ChildOrderStale)){RebuildChildOrder(index);}}queue.Clear();}
/// <summary>Sorts one node's children into order-modified document order.</summary>
void RebuildChildOrder(int index){flags[index]&=~LayoutNodeState.ChildOrderStale;var count=links[index].ChildCount;if(count<=1||!AnyChildIsOrdered(index,count)){ // Every item defaulted, so order-modified document order *is* document order and the
...
ReleaseOrderedBlock(index);return;} // Allocated before either span is taken: this is the call that can move the arena.
EnsureOrderedCapacity(index,count);if(orderKeys.Length<count){Array.Resize(ref orderKeys,int.Max(count,orderKeys.Length*2));}var target=children.Slice(orderedChildren![index].Offset,count);children.Slice(links[index].ChildOffset,count).CopyTo(target);var keys=orderKeys.AsSpan(0,count);for(var i=0;i<count;i++){ // ⚠ <b>The document position is packed into the low half of the key, which is what makes
...
keys[i]=((long)styles[target[i]].Order<<32)|(uint)i;}keys.Sort(target);}bool AnyChildIsOrdered(int index,int count){foreach(var child in children.Slice(links[index].ChildOffset,count)){if(styles[child].Order!=0){return true;}}return false;}void EnsureOrderedCapacity(int index,int count){if(orderedChildren is null){orderedChildren=new OrderedChildren[capacity];ClearOrderedRange(orderedChildren,0);}ref var ordered=ref orderedChildren[index];while(ordered.Capacity<count){ // A live count of zero: nothing in the old block is worth copying, because every id is
// about to be written over from the document block.
var grown=children.Grow(ordered.Offset,0,ordered.Capacity);ordered.Offset=grown.Offset;ordered.Capacity=grown.Capacity;}}void ReleaseOrderedBlock(int index){if(orderedChildren is null){return;}ref var ordered=ref orderedChildren[index];children.Free(ordered.Offset,ordered.Capacity);ordered=new OrderedChildren{Offset=-1};}
...
static void ClearOrderedRange(OrderedChildren[]array,int from){for(var i=from;i<array.Length;i++){array[i]=new OrderedChildren{Offset=-1};}}}
