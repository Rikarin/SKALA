...
using System.Runtime.CompilerServices;using System.Runtime.InteropServices;namespace Vixen.Ui.Layout;
/// <summary>Nine lengths, one per <see cref="Edge" />.</summary>
[InlineArray(9)]public struct EdgeLengths{StyleLength element;}
/// <summary>Three lengths, one per <see cref="Gutter" />.</summary>
[InlineArray(3)]public struct GutterLengths{StyleLength element;}
/// <summary>Two lengths, one per <see cref="Dimension" />.</summary>
[InlineArray(2)]public struct DimensionLengths{StyleLength element;}
...
///     </para>
/// </remarks>
[StructLayout(LayoutKind.Sequential)]public struct LayoutStyle{
...
/// <inheritdoc cref="GridTemplateColumns" />
internal GridTemplate GridAutoRows;static LayoutStyle CreateDefault(){var style=default(LayoutStyle);style.Direction=Direction.Inherit;style.FlexDirection=FlexDirection.Column;style.JustifyContent=Justify.FlexStart;style.AlignContent=Align.FlexStart;style.AlignItems=Align.Stretch;style.AlignSelf=Align.Auto; // An unprefixed alignment keyword is the `unsafe` one, so all six start there. Stated rather
// than left to the zero, like every other initial value in this method.
style.JustifyContentOverflow=OverflowAlignment.Unsafe;style.AlignContentOverflow=OverflowAlignment.Unsafe;style.AlignItemsOverflow=OverflowAlignment.Unsafe;style.AlignSelfOverflow=OverflowAlignment.Unsafe;style.JustifyItemsOverflow=OverflowAlignment.Unsafe;style.JustifySelfOverflow=OverflowAlignment.Unsafe;style.LegacyTextAlign=LegacyTextAlign.None;style.Float=FloatSide.None;style.Clear=Clear.None;style.PositionType=PositionType.Relative;style.FlexWrap=Wrap.NoWrap;style.OverflowX=Overflow.Visible;style.OverflowY=Overflow.Visible;style.Display=Display.Flex;style.BoxSizing=BoxSizing.BorderBox;style.Flex=float.NaN;style.FlexGrow=float.NaN;style.FlexShrink=float.NaN;style.FlexBasis=StyleLength.Auto;style.AspectRatio=float.NaN;style.GridAutoFlow=GridAutoFlow.Row; // ⚠ `normal` behaves as `stretch` for a grid item, per CSS Box Alignment §6.2 — a grid area
...
style.JustifyItems=Align.Stretch;style.JustifySelf=Align.Auto;style.GridTemplateColumns=GridTemplate.Empty;style.GridTemplateRows=GridTemplate.Empty;style.GridAutoColumns=GridTemplate.Empty;style.GridAutoRows=GridTemplate.Empty;for(var i=0;i<2;i++){style.Dimensions[i]=StyleLength.Auto;}return style;}}
