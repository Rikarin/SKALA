...
public struct LayoutStyle {
    ...
    static LayoutStyle CreateDefault() {
        ...
        style.AlignSelf = Align.Auto; // An unprefixed alignment keyword is the `unsafe` one, so all six start there. Stated rather
        ...
        style.GridAutoFlow = GridAutoFlow.Row; // ⚠ `normal` behaves as `stretch` for a grid item, per CSS Box Alignment §6.2 — a grid area
