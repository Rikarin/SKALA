...
public sealed class WheelEvent : UiEvent {
    ...
    void Track(UiSurface surface, PointerEvent args) {
        ...
        switch (args.Action) { case PointerAction.Pressed:
                Restate(pressed, chain, ElementState.Active, null, null, args);
                break; case PointerAction.Released:
                chain.Clear();
                Restate(pressed, chain, ElementState.Active, null, null, args);
                break; default: // A move neither presses nor releases. `:active` stays on whatever the press put it
