...
using Vixen.Ui.Styling;namespace Vixen.Ui;
...
public TimeSpan Timestamp{get;init;}}public sealed partial class UiDocument{readonly List<UiElement>hovered=[];readonly List<UiElement>pressed=[];readonly List<UiElement>chain=[];
...
/// <summary>Sends a wheel event to whatever is under it in one surface.</summary>
    ...
    /// <returns>The element it went to, or <c>null</c> if nothing was under it.</returns>
public UiElement?Dispatch(UiSurface surface,WheelEvent args){ArgumentNullException.ThrowIfNull(surface);ArgumentNullException.ThrowIfNull(args);var target=Captured??HitTest(surface,args.X,args.Y);target?.Raise(args);return target;}
/// <summary>Brings the hover and press states up to date with where the pointer is.</summary>
    ...
    ///     </para>
    /// </remarks>
void Track(UiSurface surface,PointerEvent args){if(args.Action==PointerAction.Pressed){ // A press is the moment the pointer takes over the interaction. Moving a mouse across
// the screen while somebody tabs through a form is not.
LeaveKeyboardMode();}var under=HitTest(surface,args.X,args.Y);chain.Clear();for(var element=under;element is not null;element=element.Parent){chain.Add(element);}Restate(hovered,chain,ElementState.Hover,PointerAction.Exited,PointerAction.Entered,args);switch(args.Action){case PointerAction.Pressed:Restate(pressed,chain,ElementState.Active,null,null,args);break;case PointerAction.Released:chain.Clear();Restate(pressed,chain,ElementState.Active,null,null,args);break;default: // A move neither presses nor releases. `:active` stays on whatever the press put it
...
/// <summary>Moves a state flag from one chain of elements to another, telling the ones that changed.</summary>
    ...
    /// </remarks>
static void Restate(List<UiElement>previous,List<UiElement>next,ElementState state,PointerAction?left,PointerAction?entered,PointerEvent args){foreach(var element in previous){if(next.Contains(element)){continue;} // A removed element is not restyled and not told. It has no style slot to clear and no
// business hearing that the pointer left something it is no longer part of.
if(element.IsRemoved){continue;}element.State&=~state;if(left is{}action){Crossed(element,args,action);}}foreach(var element in next){if(previous.Contains(element)){continue;}element.State|=state;if(entered is{}action){Crossed(element,args,action);}}previous.Clear();previous.AddRange(next);}
/// <summary>Tells one element that the pointer crossed its edge.</summary>
    ...
    ///     already is.
    /// </remarks>
static void Crossed(UiElement element,PointerEvent args,PointerAction action)=>EventRouter.Direct(element,new PointerEvent{PointerId=args.PointerId,X=args.X,Y=args.Y,Button=PointerButton.None,Action=action,Timestamp=args.Timestamp});
/// <summary>Drops the hover and press states of a subtree about to go.</summary>
    ...
    /// </remarks>
void ForgetHover(UiElement element){ForgetHover(hovered,element);ForgetHover(pressed,element);}static void ForgetHover(List<UiElement>tracked,UiElement removed){for(var i=tracked.Count-1;i>=0;i--){for(var walk=tracked[i];walk is not null;walk=walk.Parent){if(!ReferenceEquals(walk,removed)){continue;}tracked.RemoveAt(i);break;}}}}
