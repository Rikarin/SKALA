...
public class UiShapeLayoutTests {
    ...
    static readonly (string Property, string Field)[] Lanes = [
        ("Size", "size"), ("RadiiX", "radiiX"), ("RadiiY", "radiiY"), ("Axis", "axis"), ("End", "endColour"),
        ("Mid", "midColour"), ("Stops", "stops")
    ];
    ...
    public void The_bytes_carry_what_the_constructor_was_given() {
        ...
        var members = Reflected().Members; // The four radii are stored across two lanes rather than in the pair order they were given —
    ...
    public void A_gradient_built_the_old_way_still_serialises_the_old_way() {
        var shape = new UiShape(new Vector2(4f, 4f), 0f, default, new Color4(1f, 1f, 1f, 1f), new Vector2(0f, 1f));
    ...
    static int OffsetOf(string property) {
        var field = typeof(UiShape).GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
            .FirstOrDefault(candidate => candidate.Name == property || candidate.Name == $"<{property}>k__BackingField"
            );
