...
using Vixen.Ui.Layout; using Vixen.Ui.Styling; using Xunit; namespace Vixen.Ui.Tests;
...
public class LayoutStyleBridgeTests {
    ...
    public void An_em_on_font_size_is_the_parent_s_and_everywhere_else_is_the_element_s() {
        var fixture = new BridgeFixture(); // 1.5em of a 20px parent is 30px.
        Assert.Equal(
            30f,
            fixture.FontSize("font-size: 1.5em", 20f),
            Tolerance
    ...
    public void A_font_size_chain_compounds() {
        ...
        var size = LengthContext.InitialFontSize; // Three nested 1.2em are 1.728x, not 1.2x. Resolving `em` against the element's own size
    ...
    public void A_keyword_the_bridge_does_not_know_leaves_the_initial_value_alone() {
        var style = new BridgeFixture().Build(
            "justify-content: legacy-nonsense; align-items: center"
