...
public class IncrementalDocumentTests {
    ...
    public void An_incremental_pass_produces_exactly_what_a_cold_one_produces() {
        ...
        Gen.Select(Gen.Int[0, 100_000], Gen.Int[2, 4], Gen.Int[2, 3], Gen.Int[4, 12])
            .Sample(
                shape => { var (seed, depth, breadth, mutations) = shape; var css = Stylesheet(seed); var random =
                        new Random(seed ^ 0x51D2); using var live = new UiDocument(400f, 300f); live.Load(
                        css
                    ); var elements = Build(
                        live,
                        seed,
                    ...
                    live.Update(); for (var m = 0; m < mutations; m++) {
                        stateMutations += Mutate(elements, random) ? 1 : 0;
                    } live.Update(); if (!live.LastPassWasCold) {
                        incrementalPasses++;
                    ...
                    using var cold = new UiDocument(400f, 300f); cold.Load(css); var reference =
                        Build(cold, seed, depth, breadth); Copy(elements, reference); cold.Update(); Assert.True(
                        cold.LastPassWasCold
                    ); for (var i = 0; i < elements.Count; i++) {
                        styledElements += elements[i].Style.Count > 0 ? 1 : 0;
                        Assert.Equal(Describe(cold, reference[i].Style), Describe(live, elements[i].Style));
                    } },
                iter: 300
    ...
    public void A_change_the_record_cannot_express_falls_back_to_a_cold_pass() {
        ...
        document.Update(); // A class change is narrow.
        box.AddClass("a");
        document.Update();
        Assert.False(
            document.LastPassWasCold
        ...
        document.Update();
        Assert.True(document.LastPassWasCold);
        Assert.NotSame(ComputedStyle.Empty, fresh.Style); // Nor is an inline style, nor a move, nor a stylesheet.
