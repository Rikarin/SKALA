...
using Xunit;namespace Vixen.Ui.Tests;
/// <summary>
...
///     </para>
/// </remarks>
public class VisibilityTests{const float Tolerance=0.001f;static UiDocument Drawn(string css,Action<UiDocument>build){var document=new UiDocument(400f,300f);document.Load(css);build(document);document.Update();document.Draw();return document;}
/// <summary>The baseline the three keyword tests are a difference against.</summary>
[Fact]public void A_visible_box_paints_and_occupies_its_space(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            .box { width: 60px; height: 40px; background-color: #ff0000; }
            """ ,document=>document.Root.Add("div",classNames:"box"));var command=Assert.Single(document.Drawing.Commands);Assert.Equal(DrawCommandKind.Rectangle,command.Kind);var box=document.Root.Children[0];Assert.Equal(60f,box.Width,Tolerance);Assert.Equal(40f,box.Height,Tolerance);}
/// <summary>
    ///     The difference from <c>display: none</c>, and the whole reason CSS has both.
    /// </summary>
[Fact]public void Hidden_keeps_the_rectangle_in_layout_and_takes_it_out_of_the_draw_list(){using var document=Drawn("""
            ...
            """ ,document=>{document.Root.Add("div",classNames:"box");document.Root.Add("div",classNames:"after" );}); // The hidden box paints nothing; its sibling still paints.
var command=Assert.Single(document.Drawing.Commands);Assert.Equal(10f,command.Width,Tolerance); // ⚠ The load-bearing half, and the reason the root is a block rather than the default flex
...
var box=document.Root.Children[0];Assert.Equal(60f,box.Width,Tolerance);Assert.Equal(40f,box.Height,Tolerance);Assert.Equal(40f,document.Root.Children[1].AbsoluteTop,Tolerance);}
/// <summary>
    ...
    /// </summary>
[Fact]public void Collapse_reads_as_hidden_because_there_are_no_table_rows_to_mean_anything_else(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            .box { width: 60px; height: 40px; background-color: #ff0000; visibility: collapse; }
            """ ,document=>document.Root.Add("div",classNames:"box"));Assert.Empty(document.Drawing.Commands);Assert.Equal(40f,document.Root.Children[0].Height,Tolerance);}
/// <summary>
    ...
    /// </remarks>
[Fact]public void A_visible_child_paints_inside_a_hidden_parent(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            ...
            """ ,document=>document.Root.Add("div",classNames:"parent" ).Add("div",classNames:"child" ));var command=Assert.Single(document.Drawing.Commands);Assert.Equal(20f,command.Width,Tolerance);Assert.Equal(15f,command.Height,Tolerance);}
/// <summary>Hiding is inherited, so a subtree goes with its root.</summary>
[Fact]public void A_hidden_parent_hides_a_child_that_says_nothing(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            .parent { width: 100px; height: 80px; background-color: #ff0000; visibility: hidden; }
            .child { width: 20px; height: 15px; background-color: #0000ff; }
            """ ,document=>document.Root.Add("div",classNames:"parent" ).Add("div",classNames:"child" ));Assert.Empty(document.Drawing.Commands);}
/// <summary>
    ...
    /// </summary>
[Fact]public void A_hidden_box_does_not_catch_the_pointer_and_what_is_behind_it_does(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            ...
            """ ,document=>{document.Root.Add("div",classNames:"under" );document.Root.Add("div",classNames:"over");});var under=document.Root.Children[0];var over=document.Root.Children[1];Assert.False(over.IsHitTestVisible);Assert.True(under.IsHitTestVisible); // The pointer falls through the hidden overlay to the box underneath it.
...
/// <summary>
    ...
    /// </summary>
[Fact]public void A_visible_child_of_a_hidden_parent_is_still_a_pointer_target(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            .parent { width: 100px; height: 80px; visibility: hidden; }
            .child { width: 40px; height: 30px; visibility: visible; }
            """ ,document=>document.Root.Add("div",classNames:"parent" ).Add("div",classNames:"child" ));var parent=document.Root.Children[0];var child=parent.Children[0];Assert.False(parent.IsHitTestVisible);Assert.True(child.IsHitTestVisible);Assert.Same(child,document.HitTest(10f,10f));}
/// <summary>Collapse is hidden here too, rather than only in the paint walk.</summary>
[Fact]public void Collapse_is_not_a_pointer_target_either(){using var document=Drawn("""
            root { width: 400px; height: 300px; }
            .box { width: 100px; height: 80px; visibility: collapse; }
            """ ,document=>document.Root.Add("div",classNames:"box"));Assert.False(document.Root.Children[0].IsHitTestVisible);}}
