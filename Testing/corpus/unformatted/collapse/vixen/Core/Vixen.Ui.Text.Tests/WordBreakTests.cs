...
using Xunit;namespace Vixen.Ui.Text.Tests;
...
///     </para>
/// </remarks>
public class WordBreakTests{static List<int>Opportunities(string text,WordBreakMode mode){var found=new List<int>();LineBreaker.Collect(text,found,mode);return found;}
/// <summary>Wraps a paragraph whose every character is one unit wide.</summary>
    ...
    /// </remarks>
static List<string>Wrap(string text,float width,TextWrapMode mode,WordBreakMode wordBreak){var advances=new float[text.Length+1];Array.Fill(advances,1f,0,text.Length);var lines=new List<WrappedLine>();LineWrapper.Wrap(text,advances,width,lines,mode,wordBreak);return lines.Select(line=>text[line.Start .. line.End]).ToList();}[Fact]public void Normal_offers_nothing_between_two_letters()=>Assert.Equal([5],Opportunities("hello" ,WordBreakMode.Normal));[Fact]public void Break_all_offers_a_break_between_every_pair_of_letters()=>Assert.Equal([1,2,3,4,5],Opportunities("hello" ,WordBreakMode.BreakAll));
/// <summary>
    ...
    /// </remarks>
[Fact]public void Break_all_still_refuses_to_begin_a_line_with_a_comma(){var found=Opportunities("ab,cd" ,WordBreakMode.BreakAll);Assert.Contains(1,found);Assert.DoesNotContain(2,found);}
/// <summary>And the same for a bracket, on both sides of it.</summary>
[Fact]public void Break_all_keeps_a_bracket_with_what_it_encloses(){var found=Opportunities("a(bc)d" ,WordBreakMode.BreakAll); // No break after the opening bracket (LB14) and none before the closing one (LB13).
Assert.DoesNotContain(2,found);Assert.DoesNotContain(4,found); // But the letters inside it are still broken between, or the mode did nothing at all.
Assert.Contains(3,found);}
/// <summary>
    ///     ⚠ A break offered inside a character is a broken character, not a narrow line.
    /// </summary>
[Fact]public void Break_all_does_not_offer_a_break_inside_a_grapheme_cluster(){ // A base letter, its combining acute, then another letter.
var found=Opportunities("áb",WordBreakMode.BreakAll);Assert.DoesNotContain(1,found);Assert.Contains(2,found);}
/// <summary>
    ...
    /// </summary>
[Theory][InlineData("中文")] // Two Han ideographs.
[InlineData("한글")] // Two precomposed Hangul syllables.
public void Keep_all_suppresses_the_break_between_two_letter_units(string text){Assert.Equal([1,2],Opportunities(text,WordBreakMode.Normal));Assert.Equal([2],Opportunities(text,WordBreakMode.KeepAll));}
/// <summary>A space is not a letter, so <c>keep-all</c> leaves the break after one alone.</summary>
    ...
    /// </remarks>
[Fact]public void Keep_all_still_breaks_at_a_space()=>Assert.Contains(3,Opportunities("中文 中文" ,WordBreakMode.KeepAll));
/// <summary>And a hard newline is not something a <c>word-break</c> gets to decline.</summary>
[Fact]public void Keep_all_still_breaks_at_a_newline()=>Assert.Contains(2,Opportunities("中\n文",WordBreakMode.KeepAll));
/// <summary>
    ...
    /// </summary>
[Fact]public void Keep_all_leaves_latin_exactly_as_it_was(){const string Prose="the quick brown fox, jumps over-the lazy dog" ;Assert.Equal(Opportunities(Prose,WordBreakMode.Normal),Opportunities(Prose,WordBreakMode.KeepAll));}
/// <summary>
    ...
    /// </remarks>
[Fact]public void Break_all_fills_the_line_where_anywhere_only_rescues_the_word(){Assert.Equal(["ab ","cdefg" ,"hij"],Wrap("ab cdefghij" ,5f,TextWrapMode.Anywhere,WordBreakMode.Normal));Assert.Equal(["ab cd" ,"efghi" ,"j"],Wrap("ab cdefghij" ,5f,TextWrapMode.Word,WordBreakMode.BreakAll));}
/// <summary><c>break-all</c> needs no help from <c>overflow-wrap</c> to keep a word in its box.</summary>
[Fact]public void Break_all_keeps_every_line_inside_the_width()=>Assert.Equal(["antid" ,"isest" ,"ablis" ,"hment" ],Wrap("antidisestablishment" ,5f,TextWrapMode.Word,WordBreakMode.BreakAll));
/// <summary><c>keep-all</c> makes a CJK run overflow rather than break inside itself.</summary>
[Fact]public void Keep_all_lets_an_ideographic_run_overflow(){Assert.Equal(["中文","中文"],Wrap("中文中文",2f,TextWrapMode.Word,WordBreakMode.Normal));Assert.Equal(["中文中文"],Wrap("中文中文",2f,TextWrapMode.Word,WordBreakMode.KeepAll));}
/// <summary>
    ...
    /// </remarks>
[Fact]public void Keep_all_and_anywhere_are_both_honoured(){ // A space to break at, so `keep-all` has the choice it is supposed to prefer …
Assert.Equal(["中文 ","中文"],Wrap("中文 中文" ,2f,TextWrapMode.Anywhere,WordBreakMode.KeepAll)); // … and none, so the last resort still fires and nothing overflows.
Assert.Equal(["中文","中文"],Wrap("中文中文",2f,TextWrapMode.Anywhere,WordBreakMode.KeepAll));}
/// <summary>The lines still partition the text under either mode.</summary>
    ...
    /// </remarks>
[Theory][InlineData(WordBreakMode.Normal)][InlineData(WordBreakMode.BreakAll)][InlineData(WordBreakMode.KeepAll)]public void The_lines_partition_the_text(WordBreakMode mode){const string Text="ab 中文cd-ef\n한글 ghij" ;for(var width=0f;width<=12f;width+=1f){var pieces=Wrap(Text,width,TextWrapMode.Word,mode);Assert.Equal(Text,string.Concat(pieces));Assert.All(pieces,piece=>Assert.NotEqual(string.Empty,piece));}}}
