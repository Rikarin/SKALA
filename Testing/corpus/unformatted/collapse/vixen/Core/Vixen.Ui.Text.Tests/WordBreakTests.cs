...
public class WordBreakTests {
    ...
    public void Keep_all_leaves_latin_exactly_as_it_was() {
        ...
        Assert.Equal(Opportunities(Prose, WordBreakMode.Normal), Opportunities(Prose, WordBreakMode.KeepAll));
    }
    ...
    public void Break_all_fills_the_line_where_anywhere_only_rescues_the_word() {
        Assert.Equal(["ab ", "cdefg", "hij"], Wrap("ab cdefghij", 5f, TextWrapMode.Anywhere, WordBreakMode.Normal));
        Assert.Equal(["ab cd", "efghi", "j"], Wrap("ab cdefghij", 5f, TextWrapMode.Word, WordBreakMode.BreakAll));
    }
