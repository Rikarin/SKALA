...
public class SolverDeterminismTests {
    ...
    public static TheoryData<int, int> Configurations { get { var data =
                new TheoryData<int, int>(); foreach (var workers in new[] { 1, 4, 16 }) {
    ...
    public void Every_worker_count_agrees_with_every_other() {
        var matrix = PoissonGrid.Matrix(Extent);
        var right = RightHandSide(matrix.RowCount);
        var answers = new List<double[]>(); // ⚠ Sequential, and disposed each time round. Sixteen schedulers alive at once would be
