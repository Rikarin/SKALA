...
public class UvChartTests {
    public static TheoryData<string> Corpus => [
        "sphere-cut-open", "cylinder-slit", "cylinder-closed", "torus-slit", "torus-closed", "hemisphere", "saddle",
        "strip", "obtuse-grid", "sphere-nearly-closed", "dumbbell", "grouped-plate", "bowtie", "two-islands",
        "one-triangle", "two-triangles", "degenerate-triangle"
    ];
    ...
    public void AClosedOrAnnularSurfaceIsCutOpen(string shape) {
        ...
        Assert.True(
            report.SeamLength > 0f,
            $"{shape} was charted into {report.ChartCount} with no seam."
    ...
    public void ADecompositionProposesAndNeverDecides() {
        ...
        Assert.True(
            declining.Asked > 0,
            "the hook was never reached, so declining proved nothing"
