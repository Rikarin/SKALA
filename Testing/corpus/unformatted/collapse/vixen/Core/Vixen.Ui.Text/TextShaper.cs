...
public static class TextShaper {
    ...
    static Feature[] Convert(FontFeatureSet? features) {
        ...
        for (var i = 0; i < applied.Length; i++) {
            var feature = features.Features[i]; // ⚠ The four characters rather than the packed integer, because HarfBuzzSharp's `Tag`
        ...
        Tags.GetOrAdd(
            ...
            static value => HbScript.Parse(
                string.Create(
                    4,
                    (uint)value,
                    static (span, tag) => { span[0] = (char)(byte)(tag >> 24); span[1] =
                            (char)(byte)(tag >> 16); span[2] = (char)(byte)(tag >> 8); span[3] = (char)(byte)tag; }
                )
            )
        );
}
