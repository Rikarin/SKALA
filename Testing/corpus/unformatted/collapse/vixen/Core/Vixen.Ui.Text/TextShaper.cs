...
using System.Collections.Concurrent;using HarfBuzzSharp;using HbBuffer=HarfBuzzSharp.Buffer;using HbScript=HarfBuzzSharp.Script;namespace Vixen.Ui.Text;
...
public static ShapedText Shape(FontFace font,string text,ParagraphDirection direction=ParagraphDirection.Auto,FontVariation?variation=null,FontFeatureSet?features=null){ArgumentNullException.ThrowIfNull(font);ArgumentNullException.ThrowIfNull(text); // ⚠ Set on every call, including when nobody asked for an instance. The face's shaper is
...
font.SetInstance(variation);var items=TextItemizer.Itemize(text,direction);if(items.Count==0){return new ShapedText(text,[],0);} // ⚠ Built once for the paragraph rather than once per run. HarfBuzz copies the array on
...
// that names one.
var applied=Convert(features);var shaped=new ShapedRun[items.Count];for(var i=0;i<items.Count;i++){shaped[i]=ShapeRun(font,text,items[i],applied);}var order=TextItemizer.VisualOrder(items);var runs=new ShapedRun[order.Length];var advance=0f;for(var i=0;i<order.Length;i++){runs[i]=shaped[order[i]];advance+=runs[i].Advance;}return new ShapedText(text,runs,advance);}
/// <summary>Shapes one run.</summary>
    ...
    ///     </para>
    /// </remarks>
internal static ShapedRun ShapeRun(FontFace font,string text,TextItem item)=>ShapeRun(font,text,item,[]);internal static ShapedRun ShapeRun(FontFace font,string text,TextItem item,Feature[]features){using var buffer=new HbBuffer();buffer.AddUtf16(text,item.Start,item.Length);buffer.Direction=item.IsRightToLeft?Direction.RightToLeft:Direction.LeftToRight;buffer.Script=TagFor(item.Script); // ⚠ Default-ignorables are deleted rather than hidden. Left alone, HarfBuzz keeps a zero
...
buffer.Flags=BufferFlags.RemoveDefaultIgnorables;font.Shaper.Shape(buffer,features);var infos=buffer.GetGlyphInfoSpan();var positions=buffer.GetGlyphPositionSpan();var glyphs=new ShapedGlyph[infos.Length];var advance=0f;for(var i=0;i<infos.Length;i++){glyphs[i]=new ShapedGlyph((ushort)infos[i].Codepoint,(int)infos[i].Cluster,positions[i].XAdvance,positions[i].YAdvance,positions[i].XOffset,positions[i].YOffset);advance+=positions[i].XAdvance;}return new ShapedRun(item,font,glyphs,advance);}
/// <summary>Turns the engine's feature set into the array HarfBuzz wants.</summary>
    ...
    /// </remarks>
static Feature[]Convert(FontFeatureSet?features){if(features is null||features.IsEmpty){return[];}var applied=new Feature[features.Features.Length];for(var i=0;i<applied.Length;i++){var feature=features.Features[i]; // ⚠ The four characters rather than the packed integer, because HarfBuzzSharp's `Tag`
...
applied[i]=new Feature(new Tag((char)(tag >> 24),(char)((tag >> 16)&0xFF),(char)((tag >> 8)&0xFF),(char)(tag&0xFF)),feature.Value,0,uint.MaxValue);}return applied;}static HbScript TagFor(Script script)=>Tags.GetOrAdd(script,static value=>HbScript.Parse(string.Create(4,(uint)value,static(span,tag)=>{span[0]=(char)(byte)(tag >> 24);span[1]=(char)(byte)(tag >> 16);span[2]=(char)(byte)(tag >> 8);span[3]=(char)(byte)tag;})));}
