...
public readonly record struct DistanceFieldBitmap(int Width, int Height, float Range, float[] Channels) {
    ...
    public (float R, float G, float B) this[int x, int y] { get { var at =
                ((y * Width) + x) * 3; return (Channels[at], Channels[at + 1], Channels[at + 2]); } }
...
public static class DistanceField {
    ...
    static float Nearest(Prepared[] edges, Vector2 point, float winding) {
        ...
        var bestSquared = float.MaxValue; // ⚠ Larger is worse, so <c>MaxValue</c> is "nothing has answered yet" for this as well —
        ...
        foreach (var edge in edges) {
            ...
            var t = Math.Clamp(
                Vector2.Dot(point - edge.From, edge.Direction) * edge.InverseLengthSquared,
                0f,
                1f
            ...
            var distanceSquared = offset.LengthSquared(); // Clearly further than the best so far, which is the ordinary case and settles it before
            ...
            var alongside = t > 0f && t < 1f
                ...
                * edge.InverseLength; // Tied on distance, so the corner rule decides.
    ...
    static Prepared[] Prepare(List<ColouredEdge> edges, EdgeChannels channel) {
        ...
        foreach (var edge in edges) {
            ...
            var lengthSquared = direction.LengthSquared(); // A zero-length edge has no direction to measure against, and dropping it here is what
