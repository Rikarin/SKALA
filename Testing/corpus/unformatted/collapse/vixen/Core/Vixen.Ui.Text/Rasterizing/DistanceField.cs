...
using System.Numerics;using Vixen.Ui.Text.Outlines;namespace Vixen.Ui.Text.Rasterizing;
...
public(float R,float G,float B)this[int x,int y]{get{var at=((y*Width)+x)*3;return(Channels[at],Channels[at+1],Channels[at+2]);}}
...
public float Median(int x,int y){var(r,g,b)=this[x,y];return Math.Max(Math.Min(r,g),Math.Min(Math.Max(r,g),b));}}
/// <summary>
...
public static DistanceFieldBitmap Generate(GlyphOutline outline,int width,int height,float scale,Vector2 origin,float range=4f){ArgumentNullException.ThrowIfNull(outline);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(width);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(height);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(scale);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(range);var channels=new float[width*height*3];if(outline.IsEmpty){Array.Fill(channels,0f);return new DistanceFieldBitmap(width,height,range,channels);} // Flattened finely relative to a pixel, because the field's own resolution is what decides
// how much of a curve is visible — the same argument the rasteriser makes.
var edges=EdgeColoring.Colour(outline,OutlineFlattener.Flatten(outline,0.05f/scale));var inside=GlyphRasterizer.Rasterize(outline,width,height,scale,origin); // Which way the outline is wound, so "left of the edge" and "inside the shape" agree. Fonts
...
var winding=Winding(edges); // ⚠ <b>Split by channel and precomputed once, because the inner loop runs a few million
...
var red=Prepare(edges,EdgeChannels.Red);var green=Prepare(edges,EdgeChannels.Green);var blue=Prepare(edges,EdgeChannels.Blue);for(var y=0;y<height;y++){for(var x=0;x<width;x++){ // The pixel's centre, back in the outline's own units.
var point=new Vector2(((x+0.5f)/scale)+origin.X,((height-1-y+0.5f)/scale)+origin.Y); // ⚠ <b>Each channel carries its own sign, and that is the mechanism rather than a
...
var redDistance=Nearest(red,point,winding);var greenDistance=Nearest(green,point,winding);var blueDistance=Nearest(blue,point,winding); // ⚠ The fill still settles the *overall* answer. A sign taken from an edge's
...
if(inside[x,y]>=0.5f!=Median(redDistance,greenDistance,blueDistance)>=0){redDistance=-redDistance;greenDistance=-greenDistance;blueDistance=-blueDistance;}var at=((y*width)+x)*3;channels[at]=Encode(redDistance,scale,range);channels[at+1]=Encode(greenDistance,scale,range);channels[at+2]=Encode(blueDistance,scale,range);}}return new DistanceFieldBitmap(width,height,range,channels);}
/// <summary>Maps a signed distance in outline units into the texture's <c>[0, 1]</c>.</summary>
static float Encode(float distance,float scale,float range)=>Math.Clamp((distance*scale/range)+0.5f,0f,1f);static float Median(float a,float b,float c)=>Math.Max(Math.Min(a,b),Math.Min(Math.Max(a,b),c));
/// <summary>Which way round the outline runs, as +1 or −1.</summary>
static float Winding(List<ColouredEdge>edges){var area=0f;foreach(var edge in edges){area+=Cross(edge.From,edge.To);}return area>=0?1f:-1f;}
/// <summary>
...
/// <summary>
    ...
    ///     </para>
    /// </remarks>
static float Nearest(Prepared[]edges,Vector2 point,float winding){var best=float.MaxValue;var bestSquared=float.MaxValue; // ⚠ Larger is worse, so <c>MaxValue</c> is "nothing has answered yet" for this as well —
// and a first edge that is exactly alongside its point still beats it.
var bestAlongside=float.MaxValue;var signed=0f; // The tie's two bounds, kept rather than recomputed: they change only when the best does, and
// the comparison against them is the hottest line in the encoder.
var further=float.MaxValue;var nearer=float.MaxValue;foreach(var edge in edges){ // ⚠ <b>An exact rejection, not an approximate one.</b> Nothing on a segment can be nearer
...
var toMiddle=point-edge.Middle;var reach=best+edge.HalfLength;if(toMiddle.LengthSquared()>reach*reach){continue;}var t=Math.Clamp(Vector2.Dot(point-edge.From,edge.Direction)*edge.InverseLengthSquared,0f,1f); // ⚠ The far endpoint is the stored one rather than <c>From + Direction</c>, so that two
...
var closest=t>=1f?edge.To:edge.From+(t*edge.Direction);var offset=point-closest;var distanceSquared=offset.LengthSquared(); // Clearly further than the best so far, which is the ordinary case and settles it before
// anything else is worked out.
if(distanceSquared>further){continue;} // How far the point lies <i>along</i> this edge past the nearest point on it: nought
...
var alongside=t>0f&&t<1f?0f:MathF.Abs(Vector2.Dot(edge.Direction,offset))*edge.InverseLength; // Tied on distance, so the corner rule decides.
if(distanceSquared>=nearer&&alongside>=bestAlongside){continue;}bestSquared=distanceSquared;best=MathF.Sqrt(distanceSquared);bestAlongside=alongside;further=distanceSquared*(1f+Tie);nearer=distanceSquared*(1f-Tie);signed=winding*Cross(edge.Direction*edge.InverseLength,point-edge.From);}return best==float.MaxValue?0f:signed;}
/// <summary>The edges carrying one channel, with everything pixel-independent worked out.</summary>
static Prepared[]Prepare(List<ColouredEdge>edges,EdgeChannels channel){var prepared=new List<Prepared>(edges.Count);foreach(var edge in edges){if((edge.Channels&channel)==0){continue;}var direction=edge.To-edge.From;var lengthSquared=direction.LengthSquared(); // A zero-length edge has no direction to measure against, and dropping it here is what
// the per-pixel loop used to do on every pixel.
if(lengthSquared<=0f){continue;}var length=MathF.Sqrt(lengthSquared);prepared.Add(new Prepared(edge.From,edge.To,direction,1f/lengthSquared,1f/length,edge.From+(direction*0.5f),length*0.5f));}return[ .. prepared];}
/// <summary>One edge, with the parts that do not depend on the pixel already computed.</summary>
readonly record struct Prepared(Vector2 From,Vector2 To,Vector2 Direction,float InverseLengthSquared,float InverseLength,Vector2 Middle,float HalfLength);static float Cross(Vector2 a,Vector2 b)=>(a.X*b.Y)-(a.Y*b.X);}
