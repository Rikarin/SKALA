...
public sealed partial class CurveEditor : Control {
    ...
    public AnimationCurve Curve { get => curve; set { ArgumentNullException.ThrowIfNull(value); if (ReferenceEquals(
                                                                                                        curve,
                                                                                                        value
                                                                                                    )) {
                return;
            } curve.Changed -= OnCurveChanged; curve = value; curve.Changed +=
                OnCurveChanged; selection.Clear(); active = null; Document.Invalidate(); } }
    ...
    void Pointed(PointerEvent args) {
        switch (args.Action) { case PointerAction.Pressed: Begin(args); break; case PointerAction.Moved when drag
                != CurveDrag.None:
                Track(args);
                break; case PointerAction.Released when drag != CurveDrag.None:
                drag = CurveDrag.None;
    ...
    void Track(PointerEvent args) {
        ...
        switch (drag) { case CurveDrag.Pan:
                ...
                Document.Invalidate();
                break; case CurveDrag.Key when active is { } key:
                ...
                break; case CurveDrag.InHandle or CurveDrag.OutHandle when active is { } handled:
    ...
    void Keyed(KeyEvent args) {
        ...
        switch (args.Key) { case InputKey.Delete or InputKey.Backspace when selection.Count > 0:
                ...
                active = null;
                SelectionChanged?.Invoke(this);
                break; case InputKey.F: Frame(); break; case InputKey.A when args.Modifiers.HasFlag(
