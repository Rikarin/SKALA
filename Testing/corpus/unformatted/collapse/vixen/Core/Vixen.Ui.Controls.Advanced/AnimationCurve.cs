...
using System.Buffers;using Vixen.Core.Curves;namespace Vixen.Ui.Controls.Advanced;
...
public CurveKey(float time,float value,TangentMode mode=TangentMode.Auto){Time=time;Value=value;Mode=mode;}
...
/// <summary>Above this many keys the projection of <see cref="Fill" /> goes to the pool.</summary>
    ...
    /// </remarks>
const int StackKeys=64;readonly List<CurveKey>keys=[];
...
public AnimationCurve(params ReadOnlySpan<CurveKey>source){foreach(var key in source){keys.Add(key);}Sort();}
...
public static AnimationCurve Linear()=>new(new CurveKey(0f,0f,TangentMode.Linear),new CurveKey(1f,1f,TangentMode.Linear));
/// <summary>A curve that starts slowly.</summary>
public static AnimationCurve EaseIn()=>new(new CurveKey(0f,0f,TangentMode.Free){OutTangent=0f},new CurveKey(1f,1f,TangentMode.Free){InTangent=2f});
/// <summary>A curve that ends slowly.</summary>
public static AnimationCurve EaseOut()=>new(new CurveKey(0f,0f,TangentMode.Free){OutTangent=2f},new CurveKey(1f,1f,TangentMode.Free){InTangent=0f});
/// <summary>A curve that starts and ends slowly. The default for almost everything.</summary>
public static AnimationCurve EaseInOut()=>new(new CurveKey(0f,0f,TangentMode.Free){OutTangent=0f},new CurveKey(1f,1f,TangentMode.Free){InTangent=0f});
/// <summary>A single step in the middle.</summary>
public static AnimationCurve Step()=>new(new CurveKey(0f,0f,TangentMode.Constant),new CurveKey(1f,1f,TangentMode.Constant));
/// <summary>Adds a key.</summary>
    /// <param name="key">The key.</param>
    /// <returns>Where it landed in time order.</returns>
public int Add(CurveKey key){ArgumentNullException.ThrowIfNull(key);keys.Add(key);Sort();return keys.IndexOf(key);}
/// <summary>Adds a key at a time and value.</summary>
    /// <param name="time">When.</param>
    /// <param name="value">What.</param>
    /// <returns>The key.</returns>
public CurveKey Add(float time,float value){var key=new CurveKey(time,value);Add(key);return key;}
/// <summary>Removes a key.</summary>
    /// <param name="key">The key.</param>
    /// <returns>Whether it was there.</returns>
public bool Remove(CurveKey key){if(!keys.Remove(key)){return false;}Sort();return true;}
/// <summary>Moves a key and puts it back in time order.</summary>
    ...
    ///     over another.
    /// </remarks>
public void Move(CurveKey key,float time,float value){ArgumentNullException.ThrowIfNull(key);key.Time=time;key.Value=value;Sort();}
...
/// <summary>The value at a time.</summary>
    ...
    /// </remarks>
public float Evaluate(float time){if(keys.Count==0){return 0f;}if(keys.Count<=StackKeys){Span<CurveSample>stack=stackalloc CurveSample[StackKeys];Fill(stack);return CurveEvaluation.Evaluate(stack[ .. keys.Count],time);}var rented=ArrayPool<CurveSample>.Shared.Rent(keys.Count);try{Fill(rented);return CurveEvaluation.Evaluate(rented.AsSpan(0,keys.Count),time);}finally{ArrayPool<CurveSample>.Shared.Return(rented);}}
...
public(float Outgoing,float Incoming)Slopes(int index){if(keys.Count<=StackKeys){Span<CurveSample>stack=stackalloc CurveSample[StackKeys];Fill(stack);return CurveEvaluation.Slopes(stack[ .. keys.Count],index);}var rented=ArrayPool<CurveSample>.Shared.Rent(keys.Count);try{Fill(rented);return CurveEvaluation.Slopes(rented.AsSpan(0,keys.Count),index);}finally{ArrayPool<CurveSample>.Shared.Return(rented);}}
/// <summary>Projects the keys into the evaluator's form.</summary>
    ...
    /// </remarks>
void Fill(Span<CurveSample>buffer){for(var index=0;index<keys.Count;index++){var key=keys[index];buffer[index]=new(key.Time,key.Value,key.InTangent,key.OutTangent,key.Mode);}}void Sort(){keys.Sort(static(left,right)=>left.Time.CompareTo(right.Time));Changed?.Invoke(this);}}
