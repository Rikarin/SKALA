...
using System.Globalization; using Vixen.Input; using Xunit; namespace Vixen.Ui.Controls.Advanced.Tests;
...
public class DataGridTests {
    ...
    public void A_frozen_column_stays_put_while_the_rest_scrolls() {
        ...
        Assert.Equal(
            "500px",
            grid.Rows[0].Cells[0].GetStyle("left")
    ...
    public void Grouping_puts_a_header_above_each_run_and_collapsing_hides_it() {
        using var fixture = new AdvancedFixture();
        var grid = Grid(fixture, Sample());
        grid.GroupBy(grid.Columns[2]); // Two groups of two, each with a header above it.
    ...
    public void Setting_items_again_drops_the_selection() {
        ...
        grid.SetItems(Sample()); // Otherwise index 2 means a different object and the highlight is on a stranger.
    ...
    public void A_mixed_column_sorts_as_text_rather_than_throwing() {
        ...
        grid.SetItems(Sample());
        fixture.Update();
        grid.Refresh(); // A bug in the caller, and it must not become an exception at a mouse click.
        grid.SortBy(column);
        Assert.Equal(4, grid.RowCount);
    }
    ...
    public void A_number_column_sorts_numerically_rather_than_as_text() {
        using var fixture = new AdvancedFixture();
        var grid = Grid(fixture, [new Unit("a", 2, "x"), new Unit("b", 10, "x"), new Unit("c", 1, "x")]);
        grid.SortBy(
            grid.Columns[1]
