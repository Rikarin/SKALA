...
using Vixen.Input; using Xunit; namespace Vixen.Ui.Controls.Advanced.Tests;
...
public class TimelineSpanTests {
    ...
    public void A_double_click_on_an_empty_stretch_asks_for_a_span() {
        ...
        var asked = 0f;
        var times = 0;
        timeline.SpanRequested += (_, track, time) => { asked = time; times++; track.AddSpan(time, time + 0.5f); };
