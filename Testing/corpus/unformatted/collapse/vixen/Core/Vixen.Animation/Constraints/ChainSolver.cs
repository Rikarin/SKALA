...
using Vixen.Animation.Ik;using Vixen.Core.Mathematics;namespace Vixen.Animation.Constraints;
...
/// </param>
public readonly record struct ChainSolveRequest(ChainSpec Chain,Vector3 Position,float PositionWeight,Quaternion Rotation,float RotationWeight,Vector3 Pole,Vector3 EffectorOffset);
...
bool Solve(Skeleton skeleton,in ChainSolveRequest request,Span<BoneTransform>local,Span<BoneTransform>model);}
...
/// <inheritdoc />
public bool Solve(Skeleton skeleton,in ChainSolveRequest request,Span<BoneTransform>local,Span<BoneTransform>model){ArgumentNullException.ThrowIfNull(skeleton);var effector=request.Chain.Effector;if((uint)effector>=(uint)local.Length){return false;} // The last two bones, which for a two-bone chain is the whole of it and for a longer one is
...
var single=request.Chain.First==effector;var mid=single?-1:skeleton.ParentOf(effector);var root=mid<0?-1:skeleton.ParentOf(mid);if(root>=0&&request.PositionWeight>0f){return TwoBoneIk.Solve(skeleton,local,model,new(root,mid,effector,Target(skeleton,local,model,request),request.Pole==Vector3.Zero?Pole(skeleton,local,model,root,mid,effector,request.Position):request.Pole,request.Rotation,request.PositionWeight,request.RotationWeight));}if(request.RotationWeight<=0f){return false;} // Nothing above the effector to bend, or nothing asked of its position: all that is left is
...
SkeletonPose.ComputeModelSpace(skeleton,local,model);var parent=skeleton.ParentOf(effector);var parentRotation=parent<0?Quaternion.Identity:model[parent].Rotation;var desired=Quaternion.Concatenate(request.Rotation,Quaternion.Conjugate(parentRotation));local[effector].Rotation=Quaternion.Nlerp(local[effector].Rotation,desired,request.RotationWeight);return true;}
/// <summary>
    ...
    /// </remarks>
static Vector3 Target(Skeleton skeleton,Span<BoneTransform>local,Span<BoneTransform>model,in ChainSolveRequest request){if(request.EffectorOffset==Vector3.Zero){return request.Position;}SkeletonPose.ComputeModelSpace(skeleton,local,model);var rotation=request.RotationWeight>0f?request.Rotation:model[request.Chain.Effector].Rotation;return request.Position-Quaternion.Transform(request.EffectorOffset,rotation);}
/// <summary>Which way the middle joint bends when nobody said.</summary>
    ...
    ///     </para>
    /// </remarks>
static Vector3 Pole(Skeleton skeleton,Span<BoneTransform>local,Span<BoneTransform>model,int root,int mid,int tip,Vector3 target){SkeletonPose.ComputeModelSpace(skeleton,local,model);var from=model[root].Translation;var bend=model[mid].Translation;var axis=model[tip].Translation-from;if(Vector3.Cross(axis,bend-from).LengthSquared()>1e-8f){return bend;}var length=axis.LengthSquared();if(length<=1e-8f){return bend+Vector3.Up;}var toTarget=target-from;var sideways=toTarget-(axis*(Vector3.Dot(toTarget,axis)/length));if(sideways.LengthSquared()>1e-8f){return bend+Vector3.Normalize(sideways);} // The target is straight down the chain, so no direction is better than another. Any
...
var straight=Vector3.Normalize(axis);var reference=MathF.Abs(Vector3.Dot(straight,Vector3.Up))>0.99f?Vector3.Forward:Vector3.Up;return bend+Vector3.Normalize(Vector3.Cross(straight,reference));}}
