...
/// <summary>The <c>@using</c> headers, in source order.</summary>
    ...
    /// </remarks>
public IEnumerable<UsingDirectiveSyntax>Usings{get{foreach(var directive in Directives){if(directive is UsingDirectiveSyntax@using){yield return@using;}}}}T?First<T>()where T:DirectiveSyntax{foreach(var directive in Directives){if(directive is T found){return found;}}return null;}}
