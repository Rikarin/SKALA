...
using System.Globalization;using Vixen.Core.Mathematics;using Vixen.Input;namespace Vixen.Ui.Controls;
...
///     </para>
/// </remarks>
public sealed partial class RadialMenu:Overlay{readonly List<RadialItem>items=[];Vector2 centre;int highlighted=-1;bool held;
/// <inheritdoc />
...
/// <inheritdoc />
protected override void OnCreated(){base.OnCreated();IsFocusScope=true;LightDismiss=true;CloseOnEscape=true; // ⚠ On the root and capturing, like the light dismiss above it. A pie menu is aimed by moving
...
aimed=(_,args)=>Aimed(args);Document.Root.AddHandler(aimed,RoutingStrategy.Capture,handledEventsToo:true); // ⚠ And the key going up, on the root for the same reason. The gesture this menu is for is
...
lifted=(_,args)=>Lifted(args);Document.Root.AddHandler(lifted,RoutingStrategy.Capture,handledEventsToo:true);AddHandler<ClickEvent>(static(element,args)=>((RadialMenu)element).Chosen(args));AddHandler<KeyEvent>(static(element,args)=>((RadialMenu)element).Keyed(args));}Action<UiElement,PointerEvent>?aimed;Action<UiElement,KeyEvent>?lifted;
/// <inheritdoc />
protected override void OnRemoved(){if(aimed is not null){Document.Root.RemoveHandler(aimed);aimed=null;}if(lifted is not null){Document.Root.RemoveHandler(lifted);lifted=null;}base.OnRemoved();}
/// <summary>Empties it.</summary>
public void Clear(){while(Children.Count>0){Children[^1].Remove();}items.Clear();highlighted=-1;}
...
public RadialItem AddItem(string?label=null){var item=Add<RadialItem>();item.Label=label;item.Focusable=false;item.Index=items.Count;items.Add(item);return item;}
/// <summary>Opens it centred on a point in document space.</summary>
    ...
    /// </remarks>
public void OpenAt(float x,float y,bool hold=false){Hold=hold;held=hold;highlighted=-1;Open(); // The pass that gives the wedges a size, so that placing them has something to measure.
Document.Update();centre=new Vector2(x,y);MoveTo(x-(Bounds.Width*0.5f),y-(Bounds.Height*0.5f)); // ⚠ Placed, laid out, and placed again — which is one pass more than it looks like it needs.
...
public bool Accept(){if(highlighted<0||highlighted>=items.Count){return false;}var item=items[highlighted]; // Closed before the command runs, for `CommandPalette.Accept`'s reason: a command that opens
// a dialog would otherwise be covered by the menu that started it.
Close(CloseReason.Committed);Chose?.Invoke(this,item);return true;}
/// <summary>Aims at a wedge by index, as the keyboard does.</summary>
    /// <param name="index">Which one, or <c>-1</c> for none.</param>
public void Aim(int index){highlighted=index>=0&&index<items.Count?index:-1;Restyle();}
/// <summary>Which wedge a direction from the centre points at.</summary>
    ...
    /// </remarks>
public int WedgeAt(Vector2 offset){if(items.Count==0||offset.Length()<DeadZone){return-1;} // Zero points up and grows clockwise, which is what `Place` lays the wedges out by. Screen y
// grows downwards, so this is `Atan2(x, -y)` rather than the usual argument order.
var angle=MathF.Atan2(offset.X,-offset.Y);if(angle<0f){angle+=MathF.Tau;}var step=MathF.Tau/items.Count;return(int)MathF.Round(angle/step)%items.Count;}
/// <summary>Puts each wedge on the ring.</summary>
    ...
    ///     button.
    /// </remarks>
void Place(){var step=MathF.Tau/Math.Max(1,items.Count);var middle=new Vector2(Bounds.Width*0.5f,Bounds.Height*0.5f);for(var index=0;index<items.Count;index++){var angle=step*index;var item=items[index];item.Angle=angle;var point=middle+new Vector2(MathF.Sin(angle)*Radius,-MathF.Cos(angle)*Radius);item.SetStyle("left",Px(point.X-(item.Bounds.Width*0.5f)));item.SetStyle("top",Px(point.Y-(item.Bounds.Height*0.5f)));}}static string Px(float value)=>value.ToString("0.##",CultureInfo.InvariantCulture)+"px";void Restyle(){for(var index=0;index<items.Count;index++){if(index==highlighted){items[index].State|=Styling.ElementState.Checked;}else{items[index].State&=~Styling.ElementState.Checked;}}}void Aimed(PointerEvent args){if(!IsOpen){return;}if(args.Action is PointerAction.Moved or PointerAction.Pressed){Aim(WedgeAt(new Vector2(args.X-centre.X,args.Y-centre.Y)));return;} // ⚠ Only a release that belongs to the gesture that opened it. A menu opened by a click is
...
if(args.Action==PointerAction.Released){Commit();}}
/// <summary>The key that opened it going up, which is the other half of the same gesture.</summary>
    ...
    /// </remarks>
void Lifted(KeyEvent args){if(IsOpen&&args.Action==KeyAction.Released){Commit();}}
/// <summary>Ends the held gesture: runs what is aimed at, or falls back to click-to-choose.</summary>
void Commit(){if(!held){return;}held=false;if(!Accept()){ // A release with nothing aimed at is somebody who opened the menu to look at it. It stays
// up and becomes the click-to-choose kind, which is the first of the two gestures.
Hold=false;}}void Chosen(ClickEvent args){for(var element=args.Source;element is not null;element=element.Parent){if(element is not RadialItem{Index:>=0}item){continue;}Aim(item.Index);Accept();args.Handled=true;return;}}
/// <summary>Arrow keys aim and Enter commits, for a menu opened without a pointer at all.</summary>
    ...
    /// </remarks>
void Keyed(KeyEvent args){if(args.Action!=KeyAction.Pressed||items.Count==0){return;}switch(args.Key){case InputKey.Up:Aim(WedgeAt(new Vector2(0f,-Radius)));break;case InputKey.Down:Aim(WedgeAt(new Vector2(0f,Radius)));break;case InputKey.Left:Aim(WedgeAt(new Vector2(-Radius,0f)));break;case InputKey.Right:Aim(WedgeAt(new Vector2(Radius,0f)));break;case InputKey.Tab:Aim(highlighted<0?0:(highlighted+1)%items.Count);break;case InputKey.Enter or InputKey.KeypadEnter or InputKey.Space:Accept();break;default:return;}args.Handled=true;}}
