...
public sealed partial class RadialMenu : Overlay {
    ...
    protected override void OnCreated() {
        ...
        IsFocusScope = true;
        LightDismiss = true;
        CloseOnEscape = true; // ⚠ On the root and capturing, like the light dismiss above it. A pie menu is aimed by moving
    ...
    public void OpenAt(float x, float y, bool hold = false) {
        ...
        held = hold;
        highlighted = -1;
        Open(); // The pass that gives the wedges a size, so that placing them has something to measure.
        Document.Update();
    ...
    public bool Accept() {
        ...
        var item = items[highlighted]; // Closed before the command runs, for `CommandPalette.Accept`'s reason: a command that opens
    ...
    void Keyed(KeyEvent args) {
        ...
        switch (args.Key) { case InputKey.Up: Aim(WedgeAt(new Vector2(0f, -Radius))); break; case InputKey.Down:
                Aim(WedgeAt(new Vector2(0f, Radius)));
                break; case InputKey.Left: Aim(WedgeAt(new Vector2(-Radius, 0f))); break; case InputKey.Right:
                Aim(WedgeAt(new Vector2(Radius, 0f)));
                break; case InputKey.Tab:
                Aim(highlighted < 0 ? 0 : (highlighted + 1) % items.Count);
                break; case InputKey.Enter or InputKey.KeypadEnter or InputKey.Space: Accept(); break; default:
                return; }
