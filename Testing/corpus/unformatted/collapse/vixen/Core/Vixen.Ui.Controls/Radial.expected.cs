...
// up and becomes the click-to-choose kind, which is the first of the two gestures.
    ...
    void Keyed(KeyEvent args) {
        ...
        switch (args.Key) {
            case InputKey.Up: Aim(WedgeAt(new Vector2(0f, -Radius))); break;
            case InputKey.Down: Aim(WedgeAt(new Vector2(0f, Radius))); break;
            case InputKey.Left: Aim(WedgeAt(new Vector2(-Radius, 0f))); break;
            case InputKey.Right: Aim(WedgeAt(new Vector2(Radius, 0f))); break;
            case InputKey.Tab: Aim(highlighted < 0 ? 0 : (highlighted + 1) % items.Count); break;
            case InputKey.Enter or InputKey.KeypadEnter or InputKey.Space: Accept(); break;
            default: return;
        }
