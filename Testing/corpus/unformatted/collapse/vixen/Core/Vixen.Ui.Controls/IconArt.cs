...
public static IconPaint Named(string token){ArgumentException.ThrowIfNullOrEmpty(token);return new(){Kind=IconPaintKind.Token,Token=token};}
/// <summary>A colour written into the icon, which does not follow the theme.</summary>
    ...
    /// </remarks>
public static IconPaint Of(Color4 color)=>new(){Kind=IconPaintKind.Literal,Token=string.Empty,Color=color};}
...
/// </remarks>
public readonly record struct IconPath(PathBuilder Geometry,IconPaint Fill,IconPaint Stroke=default,float Width=1.5f,PathFillRule FillRule=PathFillRule.NonZero);
...
public IconArt(Rectangle viewBox,IReadOnlyList<IconPath>paths){ArgumentNullException.ThrowIfNull(paths);if(viewBox.Width<=0f||viewBox.Height<=0f){throw new ArgumentException("An icon's view box is what its geometry is scaled against, so a zero one would divide " +"by zero on the first draw. Twenty-four square is what Material, Lucide, Feather and " +"Fluent all author against." ,nameof(viewBox));}ViewBox=viewBox;Paths=paths;}
...
/// <summary>One path in the inherited colour — what every icon written before this type was.</summary>
    ...
    ///     redrawn.
    /// </remarks>
public static IconArt Of(PathBuilder geometry,PathFillRule rule=PathFillRule.NonZero){ArgumentNullException.ThrowIfNull(geometry);return new(new IconPath(geometry,IconPaint.Foreground,FillRule:rule));}
/// <summary>One path in a colour of its own.</summary>
    /// <param name="geometry">The path.</param>
    /// <param name="tint">What to fill it with.</param>
    /// <param name="rule">How its inside is decided.</param>
    /// <returns>The art.</returns>
public static IconArt Of(PathBuilder geometry,Color4 tint,PathFillRule rule=PathFillRule.NonZero){ArgumentNullException.ThrowIfNull(geometry);return new(new IconPath(geometry,IconPaint.Of(tint),FillRule:rule));}}
