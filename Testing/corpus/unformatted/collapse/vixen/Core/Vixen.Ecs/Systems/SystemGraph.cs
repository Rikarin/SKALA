...
using System.Reflection;using System.Text;namespace Vixen.Ecs.Systems;
...
public IEnumerable<SystemPhase>Phases=>Enum.GetValues<SystemPhase>().Where(phase=>Placements.Any(placement=>placement.Phase==phase));
/// <summary>The systems in a phase, in execution order.</summary>
    /// <param name="phase">The phase.</param>
    /// <returns>Its systems, or an empty list.</returns>
public IReadOnlyList<SystemPlacement>InPhase(SystemPhase phase)=>[ .. Placements.Where(placement=>placement.Phase==phase)];}
/// <summary>
...
///     </para>
/// </remarks>
public sealed class SystemGraph{readonly Dictionary<SystemPhase,List<SystemNode>>byPhase=[];
/// <summary>The phases that have systems in them, in execution order.</summary>
public IEnumerable<SystemPhase>Phases=>Enum.GetValues<SystemPhase>().Where(phase=>byPhase.ContainsKey(phase));
/// <summary>The systems in a phase, in execution order.</summary>
    /// <param name="phase">The phase.</param>
    /// <returns>Its systems, or an empty list.</returns>
public IReadOnlyList<SystemNode>InPhase(SystemPhase phase)=>byPhase.TryGetValue(phase,out var nodes)?nodes:[];
...
/// <summary>Builds the graph from systems in registration order.</summary>
    ...
    /// </exception>
public static SystemGraph Build(IReadOnlyList<ISystem>systems){ArgumentNullException.ThrowIfNull(systems);var graph=new SystemGraph();var grouped=new Dictionary<SystemPhase,List<ISystem>>();foreach(var system in systems){var phase=PhaseOf(system.GetType());if(!grouped.TryGetValue(phase,out var members)){grouped[phase]=members=[];}members.Add(system);}foreach(var(phase,members)in grouped){var types=new Type[members.Count];for(var position=0;position<members.Count;position++){types[position]=members[position].GetType();}var ordered=new List<SystemNode>(members.Count);foreach(var position in Sort(phase,types,everywhere:null,unsatisfied:null)){var system=members[position];var access=system is IDeclaredAccess declared?declared.Access:SystemAccess.FromAttributes(system.GetType());ordered.Add(new(system,phase,access,ordered.Count));}Connect(ordered);graph.byPhase[phase]=ordered;}return graph;}
/// <summary>Works out the order a set of system types would run in, building none of them.</summary>
    ...
    ///     </para>
    /// </remarks>
public static SystemPlan Plan(IReadOnlyList<Type>systemTypes){ArgumentNullException.ThrowIfNull(systemTypes);var everywhere=new Dictionary<Type,SystemPhase>();var grouped=new Dictionary<SystemPhase,List<Type>>();foreach(var type in systemTypes){var phase=PhaseOf(type);everywhere[type]=phase;if(!grouped.TryGetValue(phase,out var members)){grouped[phase]=members=[];}members.Add(type);}var placements=new List<SystemPlacement>(systemTypes.Count);var unsatisfied=new List<string>(); // Phase order rather than dictionary order, so the plan reads down the frame.
foreach(var phase in Enum.GetValues<SystemPhase>()){if(!grouped.TryGetValue(phase,out var members)){continue;}var order=0;foreach(var position in Sort(phase,members,everywhere,unsatisfied)){placements.Add(new(members[position],phase,order++));}}return new(placements,unsatisfied);}
/// <summary>Which phase a system type belongs to. Without an attribute, <see cref="SystemPhase.Update" />.</summary>
static SystemPhase PhaseOf(Type systemType)=>systemType.GetCustomAttribute<UpdateInGroupAttribute>(inherit:true)?.Phase??SystemPhase.Update;
...
static List<int>Sort(SystemPhase phase,IReadOnlyList<Type>members,IReadOnlyDictionary<Type,SystemPhase>?everywhere,List<string>?unsatisfied){var index=new Dictionary<Type,int>();for(var position=0;position<members.Count;position++){index[members[position]]=position;}var successors=new List<int>[members.Count];var incoming=new int[members.Count];for(var position=0;position<members.Count;position++){successors[position]=[];}for(var position=0;position<members.Count;position++){var type=members[position];foreach(var attribute in type.GetCustomAttributes<UpdateBeforeAttribute>(inherit:true)){if(index.TryGetValue(attribute.SystemType,out var after)){Link(position,after,successors,incoming);}else if(unsatisfied is not null){unsatisfied.Add(Explain(type,"UpdateBefore" ,attribute.SystemType,phase,everywhere));}}foreach(var attribute in type.GetCustomAttributes<UpdateAfterAttribute>(inherit:true)){if(index.TryGetValue(attribute.SystemType,out var before)){Link(before,position,successors,incoming);}else if(unsatisfied is not null){unsatisfied.Add(Explain(type,"UpdateAfter" ,attribute.SystemType,phase,everywhere));}}} // Kahn's algorithm with a ready set ordered by registration position, so a graph with no
...
var ready=new PriorityQueue<int,int>();for(var position=0;position<members.Count;position++){if(incoming[position]==0){ready.Enqueue(position,position);}}var ordered=new List<int>(members.Count);while(ready.TryDequeue(out var position,out _)){ordered.Add(position);foreach(var successor in successors[position]){if(--incoming[successor]==0){ready.Enqueue(successor,successor);}}}if(ordered.Count!=members.Count){var stuck=Enumerable.Range(0,members.Count).Where(position=>incoming[position]>0).Select(position=>members[position].Name);throw new InvalidOperationException($"The {phase} systems contain an ordering cycle. These could never run: " +$"{string.Join(", ", stuck)}. One of the [UpdateBefore] or [UpdateAfter] " +"attributes on them has to go." );}return ordered;}
/// <summary>Says, in a sentence somebody can act on, why one ordering attribute did nothing.</summary>
    ...
    /// </remarks>
static string Explain(Type declaring,string attribute,Type named,SystemPhase phase,IReadOnlyDictionary<Type,SystemPhase>?everywhere)=>everywhere is not null&&everywhere.TryGetValue(named,out var other)?$"{declaring.Name}'s [{attribute}(typeof({named.Name}))] does nothing: {named.Name} is in " +$"the {other} phase and {declaring.Name} is in {phase}, and ordering only ever applies " +"within a phase. Phases already run in their declared order." :$"{declaring.Name}'s [{attribute}(typeof({named.Name}))] does nothing: no {named.Name} " +$"is in this set, so the constraint is dropped and {declaring.Name} runs wherever " +"registration order puts it." ;
/// <summary>
    ...
    ///     attributes.
    /// </summary>
static void Link(int before,int after,List<int>[]successors,int[]incoming){if(before==after){return;}successors[before].Add(after);incoming[after]++;}static void Connect(List<SystemNode>ordered){for(var position=0;position<ordered.Count;position++){var dependencies=new List<int>();for(var earlier=0;earlier<position;earlier++){if(ordered[position].Access.ConflictsWith(ordered[earlier].Access)){dependencies.Add(earlier);}}ordered[position].DependsOn=dependencies;}}
/// <summary>Renders the graph as Graphviz DOT.</summary>
    /// <returns>The DOT source.</returns>
public string ToDot(){var text=new StringBuilder("digraph systems {\n  rankdir=LR;\n  node [shape=box];\n" );foreach(var phase in Phases){text.Append("  subgraph cluster_" ).Append(phase).Append(" {\n    label=\"" ).Append(phase).Append("\";\n" );foreach(var node in InPhase(phase)){text.Append("    \"" ).Append(phase).Append('.').Append(node.Name).Append("\" [label=\"" ).Append(node.Name).Append("\\n").Append(node.Access).Append("\"];\n");}text.Append("  }\n" );}foreach(var phase in Phases){var nodes=InPhase(phase);foreach(var node in nodes){foreach(var dependency in node.DependsOn){text.Append("  \"").Append(phase).Append('.').Append(nodes[dependency].Name).Append("\" -> \"" ).Append(phase).Append('.').Append(node.Name).Append("\";\n" );}}}text.Append("}\n");return text.ToString();}
/// <summary>Renders the graph as Mermaid, which pastes straight into a pull request.</summary>
    /// <returns>The Mermaid source.</returns>
public string ToMermaid(){var text=new StringBuilder("flowchart LR\n" );foreach(var phase in Phases){var nodes=InPhase(phase);text.Append("  subgraph " ).Append(phase).Append('\n');foreach(var node in nodes){text.Append("    ").Append(phase).Append('_').Append(node.Name).Append('[').Append(node.Name).Append("]\n");}text.Append("  end\n" );foreach(var node in nodes){foreach(var dependency in node.DependsOn){text.Append("  ").Append(phase).Append('_').Append(nodes[dependency].Name).Append(" --> " ).Append(phase).Append('_').Append(node.Name).Append('\n');}}}return text.ToString();}}
