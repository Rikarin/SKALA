...
// refuses each of them in the other's property for exactly that asymmetry.
    ...
    static readonly Dictionary<string, string> Initials = new(StringComparer.Ordinal) {
        ...
        [GradientToPosition] =
            "100%", // See the mask fragments' own remark for why the near end is opaque where the gradient's is
// not: a mask that defaulted to `transparent` at both ends would erase whatever set it.
        ...
        [MaskConicAngle] =
            "0deg", // See the mask layers' own remark: an opaque layer is the identity under `intersect`, which
...
// twenty of them and a loop is one place to get it wrong instead of twenty.
        ...
        [MaskConic] =
            MaskOpaque, // ⚠ <b><c>0px</c> rather than <c>0</c>, and the unit is <i>not</i> doing the work it looks
...
// load-bearing has the answer here instead of the argument.
        [TranslateX] = "0px",
        [TranslateY] =
            "0px", // ⚠ <b>One, and this is the pair where the identity is not zero — which is the whole reason
...
// fill in.
        [DropShadow] =
            "0 0 transparent", // ⚠ <c>0deg</c> and not <c>0</c>, and here the unit is load-bearing rather than legibility.
...
// which is every element that writes a `blur-*`.
        [HueRotate] =
            "0deg", // ⚠ <b>The backdrop's nine, and the values are the same identities for the same reason</b> —
