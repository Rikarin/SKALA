...
/// <summary>The prefix every fragment name carries.</summary>
    ...
    /// </remarks>
public const string Prefix="--tw-" ; // ── The gradient stops ──────────────────────────────────────────────────────────────────
//
...
/// <summary>The assembled stop list every gradient assembler interpolates.</summary>
    ...
    /// </remarks>
public const string GradientStops=Prefix+"gradient-stops" ; // ── The mask stops ──────────────────────────────────────────────────────────────────────
//
...
public const string MaskConicAngle=Prefix+"mask-conic-angle" ; // ── The mask layers ─────────────────────────────────────────────────────────────────────
//
...
public static string MaskEdgeToPosition(string edge)=>MaskEdge(edge)+"-to-position" ; // ── The translation ─────────────────────────────────────────────────────────────────────
//
...
public const string ScaleY=Prefix+"scale-y" ; // ── The ring ────────────────────────────────────────────────────────────────────────────
//
...
/// <summary>What colour it is.</summary>
    ...
    /// </remarks>
public const string RingColor=Prefix+"ring-color" ; // ── The filter ──────────────────────────────────────────────────────────────────────────
//
...
/// <summary>How far a <c>filter: blur()</c> spreads, as a Gaussian standard deviation.</summary>
    ...
    ///     on.
    /// </remarks>
public const string Blur=Prefix+"blur"; // ── The seven colour functions ──────────────────────────────────────────────────────────
//
...
/// <summary>A <c>filter: drop-shadow()</c>'s arguments: two or three lengths and a colour.</summary>
    ...
    /// </remarks>
public const string DropShadow=Prefix+"drop-shadow" ; // ── The backdrop's nine ─────────────────────────────────────────────────────────────────
//
...
public const string BackdropSepia=Prefix+"backdrop-sepia" ;static readonly Dictionary<string,string>Initials=new(StringComparer.Ordinal){[GradientFrom]="transparent" ,[GradientVia]="transparent" ,[GradientTo]="transparent" ,[GradientFromPosition]="0%",[GradientViaPosition]="50%",[GradientToPosition]="100%", // See the mask fragments' own remark for why the near end is opaque where the gradient's is
// not: a mask that defaulted to `transparent` at both ends would erase whatever set it.
[MaskFrom]="black" ,[MaskTo]="transparent" ,[MaskFromPosition]="0%",[MaskToPosition]="100%",[MaskLinearAngle]="180deg" ,[MaskConicAngle]="0deg", // See the mask layers' own remark: an opaque layer is the identity under `intersect`, which
...
[MaskLinear]=MaskOpaque,[MaskRadial]=MaskOpaque,[MaskConic]=MaskOpaque, // ⚠ <b><c>0px</c> rather than <c>0</c>, and the unit is <i>not</i> doing the work it looks
...
[TranslateX]="0px",[TranslateY]="0px", // ⚠ <b>One, and this is the pair where the identity is not zero — which is the whole reason
...
[ScaleX]="1",[ScaleY]="1", // ⚠ <b>Zero, so that a colour on its own paints nothing — which is what v4 does too.</b>
...
[RingWidth]="0px",[RingColor]="currentcolor" ,[Blur]="0px", // ⚠ <b>Each initial is the identity of <i>its own</i> function, which is one for four of
...
[Brightness]="1",[Contrast]="1",[Grayscale]="0",[Invert]="0",[Saturate]="1",[Sepia]="0", // ⚠ <b>A <i>transparent</i> shadow, because <c>drop-shadow</c> is the one function with no
...
// fill in.
[DropShadow]="0 0 transparent" , // ⚠ <c>0deg</c> and not <c>0</c>, and here the unit is load-bearing rather than legibility.
...
[HueRotate]="0deg", // ⚠ <b>The backdrop's nine, and the values are the same identities for the same reason</b> —
...
[BackdropBlur]="0px",[BackdropBrightness]="1",[BackdropContrast]="1",[BackdropGrayscale]="0",[BackdropHueRotate]="0deg",[BackdropInvert]="0",[BackdropOpacity]="1",[BackdropSaturate]="1",[BackdropSepia]="0"};static readonly List<string>Names;static UtilityComposition(){ // ⚠ Before the snapshot below, which is the whole of why this loop is here rather than beside
...
foreach(var edge in MaskEdges){Initials[MaskEdge(edge)]=MaskOpaque;Initials[MaskEdgeFrom(edge)]="black" ;Initials[MaskEdgeTo(edge)]="transparent" ;Initials[MaskEdgeFromPosition(edge)]="0%";Initials[MaskEdgeToPosition(edge)]="100%";} // Two passes, because one fragment's initial value is written in terms of the others. The
...
/// <summary>Whether a property is a fragment rather than something a consumer reads.</summary>
    ...
    /// </remarks>
public static bool IsFragment(string property){ArgumentNullException.ThrowIfNull(property);return Initials.ContainsKey(property);}
...
public static string InitialValueOf(string fragment){ArgumentNullException.ThrowIfNull(fragment);return Initials.TryGetValue(fragment,out var initial)?initial:throw new ArgumentException($"'{fragment}' is not a composition fragment" ,nameof(fragment));}
...
/// <summary>One edge's ramp, as a gradient running towards that edge.</summary>
    ...
    /// </remarks>
public static string MaskEdgeImage(string edge){var from=$"{Reference(MaskEdgeFrom(edge))} {Reference(MaskEdgeFromPosition(edge))}" ;var to=$"{Reference(MaskEdgeTo(edge))} {Reference(MaskEdgeToPosition(edge))}" ;return$"linear-gradient(to {edge}, {from}, {to})" ;}
/// <summary>One mask assembler: the shape, its geometry, and the two-stop ramp.</summary>
    ...
    ///     </para>
    /// </remarks>
public static string MaskImage(string shape,string geometry){var stops=$"{Reference(MaskFrom)} {Reference(MaskFromPosition)}, {Reference(MaskTo)} {Reference(MaskToPosition)}" ;return geometry.Length==0?$"{shape}-gradient({stops})" :$"{shape}-gradient({geometry}, {stops})" ;}
/// <summary>The comma-separated stop list a gradient function takes.</summary>
    ...
    /// </remarks>
public static string StopList(bool via){var stops=new List<string>{$"{Reference(GradientFrom)} {Reference(GradientFromPosition)}" };if(via){stops.Add($"{Reference(GradientVia)} {Reference(GradientViaPosition)}" );}stops.Add($"{Reference(GradientTo)} {Reference(GradientToPosition)}" );return string.Join(", ",stops);}
...
/// <summary>The <c>filter</c> declaration the eight filter families assemble into.</summary>
    ...
    ///     </para>
    /// </remarks>
public static string Filter()=>$"blur({Reference(Blur)}) brightness({Reference(Brightness)}) contrast({Reference(Contrast)}) " +$"grayscale({Reference(Grayscale)}) hue-rotate({Reference(HueRotate)}) invert({Reference(Invert)}) " +$"saturate({Reference(Saturate)}) sepia({Reference(Sepia)}) drop-shadow({Reference(DropShadow)})" ;
/// <summary>The <c>backdrop-filter</c> declaration the ten backdrop families assemble into.</summary>
    ...
    ///     </para>
    /// </remarks>
public static string BackdropFilter()=>$"blur({Reference(BackdropBlur)}) brightness({Reference(BackdropBrightness)}) " +$"contrast({Reference(BackdropContrast)}) grayscale({Reference(BackdropGrayscale)}) " +$"hue-rotate({Reference(BackdropHueRotate)}) invert({Reference(BackdropInvert)}) " +$"opacity({Reference(BackdropOpacity)}) saturate({Reference(BackdropSaturate)}) " +$"sepia({Reference(BackdropSepia)})" ;}
