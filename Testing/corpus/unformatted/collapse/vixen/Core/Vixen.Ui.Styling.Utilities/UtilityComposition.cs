...
public static class UtilityComposition {
    ...
    /// </remarks>
    public const string GradientStops = Prefix
        + "gradient-stops"; // ── The mask stops ──────────────────────────────────────────────────────────────────────
    ...
    public const string MaskConicAngle = Prefix
        + "mask-conic-angle"; // ── The mask layers ─────────────────────────────────────────────────────────────────────
    ...
    /// </remarks>
    public const string RingColor = Prefix
        + "ring-color"; // ── The filter ──────────────────────────────────────────────────────────────────────────
    ...
    ///     on.
    /// </remarks>
    public const string Blur = Prefix
        + "blur"; // ── The seven colour functions ──────────────────────────────────────────────────────────
    ...
    /// </remarks>
    public const string DropShadow = Prefix
        + "drop-shadow"; // ── The backdrop's nine ─────────────────────────────────────────────────────────────────
    ...
    static readonly Dictionary<string, string> Initials = new(StringComparer.Ordinal) {
        ...
        [GradientToPosition] = "100%", // See the mask fragments' own remark for why the near end is opaque where the gradient's is
        ...
        [MaskConicAngle] = "0deg", // See the mask layers' own remark: an opaque layer is the identity under `intersect`, which
        ...
        [MaskConic] = MaskOpaque, // ⚠ <b><c>0px</c> rather than <c>0</c>, and the unit is <i>not</i> doing the work it looks
        ...
        [TranslateX] = "0px",
        [TranslateY] = "0px", // ⚠ <b>One, and this is the pair where the identity is not zero — which is the whole reason
        ...
        [ScaleX] = "1",
        [ScaleY] = "1", // ⚠ <b>Zero, so that a colour on its own paints nothing — which is what v4 does too.</b>
        ...
        [Blur] = "0px", // ⚠ <b>Each initial is the identity of <i>its own</i> function, which is one for four of
        ...
        [Invert] = "0",
        [Saturate] = "1",
        [Sepia] = "0", // ⚠ <b>A <i>transparent</i> shadow, because <c>drop-shadow</c> is the one function with no
    ...
    public static string MaskImage(string shape, string geometry) {
        ...
        return geometry.Length == 0 ? $"{shape}-gradient({stops})" : $"{shape}-gradient({geometry}, {stops})";
    }
