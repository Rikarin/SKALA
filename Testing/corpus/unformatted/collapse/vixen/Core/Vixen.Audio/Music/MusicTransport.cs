...
public void Start(long frame,MusicTempoMap map){ArgumentNullException.ThrowIfNull(map);Origin=frame;Map=map;}
...
/// <summary>The first frame at or after a given one that a change is allowed to land on.</summary>
    ...
    ///     </para>
    /// </remarks>
public long NextBoundary(long frame,MusicQuantize quantize,long segmentFrames=0){switch(quantize){case MusicQuantize.Beat:return Origin+Map.NextBeat(PositionAt(frame));case MusicQuantize.Bar:return Origin+Map.NextBar(PositionAt(frame));case MusicQuantize.Segment:{if(segmentFrames<=0){goto case MusicQuantize.Bar;}var end=Origin+segmentFrames;if(end>=frame){return end;} // Past its first pass, so the next whole one — a looping segment has as many ends as
// it has times round.
var offset=frame-Origin;var steps=MusicTempoMap.FloorDivide(offset,segmentFrames);if(steps*segmentFrames<offset){steps++;}return Origin+(steps*segmentFrames);}default:return frame;}}}
