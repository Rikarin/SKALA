...
public sealed class MusicTransport(int sampleRate) {
    ...
    public long NextBoundary(long frame, MusicQuantize quantize, long segmentFrames = 0) {
        switch (quantize) { case MusicQuantize.Beat:
                return Origin + Map.NextBeat(PositionAt(frame)); case MusicQuantize.Bar:
                return Origin + Map.NextBar(PositionAt(frame)); case MusicQuantize.Segment: {
                if (segmentFrames <= 0) {
