...
using Vixen.Core.Mathematics;using Vixen.Foliage;using Vixen.Rendering.Features;namespace Vixen.Rendering.Terrain;
...
public readonly record struct FoliageBatch(int Type,FoliageCellKey Cell,int First,int Count,DrawCommand[]Commands);
/// <summary>
...
///     </para>
/// </remarks>
public sealed class FoliageRenderer{readonly InstanceCuller culler=new();readonly List<FoliageBatch>batches=[];readonly List<Matrix4x4>transforms=[];readonly List<InstanceParameters>parameters=[];readonly List<InstanceBounds>bounds=[];
...
public ReadOnlySpan<InstanceParameters>Parameters=>System.Runtime.InteropServices.CollectionsMarshal.AsSpan(parameters);
...
public int Cull(FoliageVolume volume,IReadOnlyList<FoliageDraw>draws,in BoundingFrustum frustum,Vector3 viewPosition,float densityScale=1f){ArgumentNullException.ThrowIfNull(volume);ArgumentNullException.ThrowIfNull(draws);batches.Clear();transforms.Clear();parameters.Clear();CellsConsidered=0;CellsDrawn=0;InstancesConsidered=0;InstancesDrawn=0;Draws=0;var byType=new Dictionary<int,FoliageDraw>(draws.Count);foreach(var draw in draws){byType[draw.Type]=draw;}var view=frustum;foreach(var chunk in volume.Chunks){if(chunk.IsEmpty||!byType.TryGetValue(chunk.Type,out var draw)){continue;}CellsConsidered++; // Stage one: the cell as one object. A forest is a few thousand of these and most of them
// are behind the camera.
if(!view.Intersects(chunk.Bounds)){continue;}var settings=volume.Palette[chunk.Type];CellsDrawn++;InstancesConsidered+=chunk.Count;Fill(chunk,settings);var culled=culler.Cull(System.Runtime.InteropServices.CollectionsMarshal.AsSpan(bounds),[],new InstanceCullSettings{Frustum=view,ViewPosition=viewPosition,StartCullDistance=settings.StartCullDistance,EndCullDistance=settings.EndCullDistance,DensityScale=densityScale,Fade=true},draw.Distances);if(culled==0){continue;}var first=transforms.Count;var survivors=culler.Survivors;var fades=culler.Parameters;for(var index=0;index<culled;index++){transforms.Add(chunk.Instances[(int)survivors[index]].Transform);parameters.Add(fades[index]);}var commands=new DrawCommand[culler.LevelCount];culler.FillCommands(draw.Levels,(uint)first,commands);batches.Add(new(chunk.Type,chunk.Cell,first,culled,commands));InstancesDrawn+=culled;Draws+=commands.Length;}return InstancesDrawn;}
/// <summary>What a cell's instances are, as the culler wants them.</summary>
    ...
    /// </remarks>
void Fill(FoliageChunk chunk,in FoliageType settings){bounds.Clear();var reach=MathF.Max(settings.Radius,0.5f);foreach(var instance in chunk.Instances){bounds.Add(new(instance.Position,reach*instance.Scale));}}}
