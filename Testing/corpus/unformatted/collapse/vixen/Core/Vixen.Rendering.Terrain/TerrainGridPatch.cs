...
using System.Runtime.InteropServices;using Vixen.Core.Mathematics;using Vixen.Terrain;namespace Vixen.Rendering.Terrain;
/// <summary>
...
/// </param>
[StructLayout(LayoutKind.Sequential)]public readonly record struct TerrainNodeRecord(Vector2 Origin,float Step,float Morph,float Level,float Padding0=0f){
...
/// <summary>The record for a node the quadtree selected.</summary>
    ...
    /// </remarks>
public static TerrainNodeRecord Of(in TerrainLodNode node,int gridQuads,int maxLevel=0){var step=node.Quads/(float)gridQuads;var level=Math.Clamp((float)Math.Log2(Math.Max(step,1f)),0f,Math.Max(maxLevel,0));return new(new(node.X,node.Z),step,node.Morph,level,0f);}}
/// <summary>
...
/// <summary>Fills the index buffer for a patch.</summary>
    ...
    /// </remarks>
public static int FillIndices(int gridQuads,Span<uint>indices){if(gridQuads<2||(gridQuads&(gridQuads-1))!=0){throw new ArgumentException($"A grid patch must span a power of two quads of at least two; it was {gridQuads}." ,nameof(gridQuads));}var required=IndexCount(gridQuads);if(indices.Length<required){throw new ArgumentException($"A {gridQuads}-quad patch needs {required} indices, not {indices.Length}." ,nameof(indices));}var side=gridQuads+1;var at=0;for(var z=0;z<gridQuads;z++){for(var x=0;x<gridQuads;x++){var topLeft=(uint)((z*side)+x);var topRight=topLeft+1;var bottomLeft=(uint)(((z+1)*side)+x);var bottomRight=bottomLeft+1;if(((x+z)&1)==0){indices[at++]=topLeft;indices[at++]=bottomLeft;indices[at++]=topRight;indices[at++]=topRight;indices[at++]=bottomLeft;indices[at++]=bottomRight;}else{indices[at++]=topLeft;indices[at++]=bottomLeft;indices[at++]=bottomRight;indices[at++]=topLeft;indices[at++]=bottomRight;indices[at++]=topRight;}}}return at;}
/// <summary>Where a vertex of the patch sits, before the morph.</summary>
    ...
    ///     device in the room.
    /// </remarks>
public static(int X,int Z)VertexOf(int vertex,int gridQuads){var side=gridQuads+1;return(vertex%side,vertex/side);}}
