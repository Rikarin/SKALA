...
public sealed class GrassRenderer {
    ...
    readonly List<InstanceParameters> source = []; // [slot][field]. Rectangular and allocated once, because the ring is fixed and the field list is
    ...
    void Fill(List<GrassBlade> held, in GrassType type) {
        ...
        foreach (var blade in held) {
            ...
            source.Add(new() { Tint = blade.Tint, WindPhase = blade.WindPhase, Scale = 1f, Fade = 1f });
        }
    }
