...
using Vixen.Core.Mathematics;namespace Vixen.Core.Imaging;
...
public const int Count=9;readonly Vector3[]coefficients;
...
public SphericalHarmonicsL2(ReadOnlySpan<Vector3>coefficients){if(coefficients.Length!=Count){throw new ArgumentException($"Three bands is {Count} coefficients, not {coefficients.Length}." );}this.coefficients=coefficients.ToArray();}
...
/// <summary>Projects a cube map onto the first three bands.</summary>
    ...
    /// </remarks>
public static SphericalHarmonicsL2 Project(TextureData cube,int level=0){CubeMap.Require(cube,nameof(cube));var size=cube.Levels[level].Width;var coefficients=new Vector3[Count];Span<float>basis=stackalloc float[Count];for(var face=0;face<CubeMap.Faces;face++){for(var y=0;y<size;y++){for(var x=0;x<size;x++){var radiance=CubeMap.ReadTexel(cube,level,face,x,y);var solidAngle=CubeMap.SolidAngleOfTexel(x,y,size);Evaluate(CubeMap.DirectionOfTexel(face,x,y,size),basis);for(var index=0;index<Count;index++){coefficients[index]+=radiance*(basis[index]*solidAngle);}}}}return new(coefficients);}
...
public static void Evaluate(Vector3 direction,Span<float>basis){var(x,y,z)=(direction.X,direction.Y,direction.Z);basis[0]=0.282095f;basis[1]=0.488603f*y;basis[2]=0.488603f*z;basis[3]=0.488603f*x;basis[4]=1.092548f*x*y;basis[5]=1.092548f*y*z;basis[6]=0.315392f*((3f*z*z)-1f);basis[7]=1.092548f*x*z;basis[8]=0.546274f*((x*x)-(y*y));}
/// <summary>
    ...
    /// </remarks>
public Vector3 Irradiance(Vector3 normal){Span<float>basis=stackalloc float[Count];Evaluate(normal,basis);var total=Vector3.Zero;for(var index=0;index<Count;index++){total+=coefficients[index]*(basis[index]*BandFactors[index]);}return total;}
/// <summary>
    ///     The cosine lobe's own coefficients, already divided by π. Band zero is π/π, band one is
    ///     (2π/3)/π and band two is (π/4)/π.
    /// </summary>
static readonly float[]BandFactors=[1f,2f/3f,2f/3f,2f/3f,0.25f,0.25f,0.25f,0.25f,0.25f];}
