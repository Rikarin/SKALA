...
using Vixen.Core;using Vixen.Core.Curves;namespace Vixen.Ai;
...
/// </remarks>
[DataContract("UtilityConsideration" )]public sealed class UtilityConsiderationContent{
...
public ResponseCurve BuildCurve()=>new(){Kind=Curve,Slope=Slope,Exponent=Exponent,Shift=Shift,Centre=Centre,Keys=Curve==ResponseCurveKind.Sampled&&Keys.Count>0?[ .. Keys.OrderBy(key=>key.Time).Select(key=>key.ToSample())]:null};}
/// <summary>One key of a sampled curve, as a file holds it.</summary>
[DataContract("UtilityCurveKey" )]public sealed class UtilityCurveKeyContent{
...
/// </remarks>
[DataContract("UtilityAction" )]public sealed class UtilityActionContent{
...
///     </para>
/// </remarks>
[DataContract("UtilitySet" )]public sealed class UtilitySetContent{
...
public BlackboardLayout BuildLayout(ICollection<BehaviorTreeDiagnostic>?diagnostics=null){var builder=new BlackboardLayoutBuilder();foreach(var key in Keys){try{builder.Add(key.Name,key.Type);}catch(Exception error)when(error is InvalidOperationException or ArgumentException){diagnostics?.Add(new(Symbol.Intern(key.Name),error.Message));}}return builder.Build();}
/// <summary>The selector this describes.</summary>
    /// <returns>The selector.</returns>
public IUtilitySelector BuildSelector()=>Selector switch{UtilitySelectorKind.WeightedRandom=>UtilitySelectors.WeightedRandom,UtilitySelectorKind.TopWeightedRandom=>UtilitySelectors.TopWeightedRandom(Math.Max(1,SelectorCount)),UtilitySelectorKind.Bucketed=>UtilitySelectors.Bucketed,_=>UtilitySelectors.Highest};}
