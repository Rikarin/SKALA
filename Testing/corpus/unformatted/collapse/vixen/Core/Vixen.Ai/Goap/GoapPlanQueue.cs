...
using Vixen.Core.Threading;namespace Vixen.Ai;
...
///     </para>
/// </remarks>
public sealed class GoapPlanQueue{readonly GoapPlanner[]planners;readonly Slot[]slots;readonly Queue<int>waiting=new();uint nextGeneration=1;
...
public GoapPlanQueue(GoapDomain domain,GoapSettings?settings=null,int capacity=64,int parallelSearches=4){ArgumentNullException.ThrowIfNull(domain);ArgumentOutOfRangeException.ThrowIfLessThan(capacity,1);ArgumentOutOfRangeException.ThrowIfLessThan(parallelSearches,1);Domain=domain;Settings=settings??GoapSettings.Default;planners=new GoapPlanner[parallelSearches];slots=new Slot[capacity];for(var index=0;index<parallelSearches;index++){planners[index]=new(domain,Settings);}for(var index=0;index<capacity;index++){slots[index]=new(){Snapshot=new(domain),Plan=new()};}}
...
public GoapPlanRequest Submit(in AgentContext context,int goal=-1,IActionCostModel?costs=null,GoapTargetSensors?sensors=null,GoapCapabilities capabilities=default){var index=Free();if(index<0){return GoapPlanRequest.Null;}var slot=slots[index];slot.Generation=nextGeneration++;slot.State=GoapRequestState.Waiting;slot.Plan.Clear();if(!slot.Snapshot.Take(in context,goal,costs,sensors,capabilities,Settings.DistanceCost)){ // Nothing to plan for. Answered at once rather than queued, because an agent whose goals
// are all met should not wait a frame to be told so.
slot.Plan.Failure=PlanFailure.AlreadyMet;slot.State=GoapRequestState.Ready;PendingCount++;return new(index,slot.Generation);}waiting.Enqueue(index);PendingCount++;return new(index,slot.Generation);}
/// <summary>Gives a slot back.</summary>
    /// <param name="request">The ticket.</param>
    /// <returns>Whether it named a live request.</returns>
public bool Cancel(GoapPlanRequest request){if(!TryResolve(request,out var index)){return false;}Release(index);return true;}
/// <summary>How a request is getting on.</summary>
    /// <param name="request">The ticket.</param>
    /// <returns>Its state.</returns>
public GoapRequestState GetState(GoapPlanRequest request)=>TryResolve(request,out var index)?slots[index].State:GoapRequestState.Unknown;
/// <summary>Runs some of the waiting searches.</summary>
    ...
    /// </remarks>
public void Update(int resolves=4){LastResolves=0;LastExpanded=0;if(resolves<=0||waiting.Count==0){return;}var batch=Math.Min(resolves,waiting.Count);var taken=new int[batch];for(var index=0;index<batch;index++){taken[index]=waiting.Dequeue();}if(Scheduler is{}scheduler&&batch>1){var job=new SearchJob{Queue=this,Taken=taken};scheduler.ScheduleParallel(job,batch).Complete();}else{for(var index=0;index<batch;index++){Run(taken,index);}}for(var index=0;index<batch;index++){slots[taken[index]].State=GoapRequestState.Ready;LastExpanded+=slots[taken[index]].Plan.Expanded;}LastResolves=batch;}
...
public bool TryTakeResult(GoapPlanRequest request,GoapPlan plan){ArgumentNullException.ThrowIfNull(plan);if(!TryResolve(request,out var index)||slots[index].State!=GoapRequestState.Ready){return false;}plan.Copy(slots[index].Plan);Release(index);return true;}
/// <summary>Runs one search. ⚠ Touches only its own slot and its own planner.</summary>
void Run(int[]taken,int index){ // One planner per parallel slot, round-robin over the batch: a planner is a node pool and an
// open list, and two searches in one would be the race this arrangement exists to avoid.
var planner=planners[index%planners.Length];var slot=slots[taken[index]];planner.Search(slot.Snapshot,slot.Plan);}int Free(){for(var index=0;index<slots.Length;index++){if(slots[index].State==GoapRequestState.Unknown){return index;}}return-1;}void Release(int index){slots[index].State=GoapRequestState.Unknown;slots[index].Generation=0;PendingCount=Math.Max(0,PendingCount-1);}bool TryResolve(GoapPlanRequest request,out int index){index=request.Index;return!request.IsNull&&(uint)request.Index<(uint)slots.Length&&slots[request.Index].Generation==request.Generation&&slots[request.Index].State!=GoapRequestState.Unknown;}sealed class Slot{public uint Generation;public GoapRequestState State;public GoapSnapshot Snapshot=null!;public GoapPlan Plan=null!;}
/// <summary>
    ...
    /// </summary>
readonly struct SearchJob:IJobParallelFor{public GoapPlanQueue Queue{get;init;}public int[]Taken{get;init;}public void Execute(int index)=>Queue.Run(Taken,index);}}
