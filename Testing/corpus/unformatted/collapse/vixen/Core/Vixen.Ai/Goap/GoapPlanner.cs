...
using Vixen.Core;using Vixen.Core.Mathematics;using Vixen.Ecs;namespace Vixen.Ai;
...
public static IActionCostModel StraightLine(float perMetre=0.1f)=>new StraightLineCostModel(perMetre);}sealed class FlatCostModel:IActionCostModel{public float Cost(in AgentContext context,GoapAction action,Vector3 from,in GoapTarget target){ArgumentNullException.ThrowIfNull(action);return MathF.Max(1f,action.BaseCost);}}sealed class StraightLineCostModel(float perMetre):IActionCostModel{public float Cost(in AgentContext context,GoapAction action,Vector3 from,in GoapTarget target){ArgumentNullException.ThrowIfNull(action);var distance=target.Found?(target.Position-from).Length():0f; // ⚠ Floored at one, not at zero. The heuristic counts unmet conditions, and it is admissible
...
/// </remarks>
public sealed class GoapPlan{readonly List<int>steps=[];
...
public void Clear(){steps.Clear();Goal=Symbol.None;GoalIndex=-1;Failure=PlanFailure.None;Cost=0f;Expanded=0;}
/// <summary>Drops the head, which is what finishing a step does.</summary>
    /// <returns>Whether there was one.</returns>
public bool Advance(){if(steps.Count==0){return false;}steps.RemoveAt(0);return true;}
/// <summary>Copies another plan over this one.</summary>
    /// <param name="other">The plan to copy.</param>
    /// <exception cref="ArgumentNullException"><paramref name="other" /> is null.</exception>
public void Copy(GoapPlan other){ArgumentNullException.ThrowIfNull(other);steps.Clear();steps.AddRange(other.steps);Goal=other.Goal;GoalIndex=other.GoalIndex;Failure=other.Failure;Cost=other.Cost;Expanded=other.Expanded;}internal void Add(int action)=>steps.Add(action);}
...
///     </para>
/// </remarks>
public sealed class GoapPlanner{readonly List<Node>nodes=[];readonly List<int>open=[];readonly int[]candidates;
...
public GoapPlanner(GoapDomain domain,GoapSettings?settings=null){ArgumentNullException.ThrowIfNull(domain);Domain=domain;Settings=settings??GoapSettings.Default;candidates=new int[Math.Max(1,domain.Count)];}
...
public PlanFailure Resolve(in AgentContext context,GoapPlan plan,int goal=-1,IActionCostModel?costs=null,GoapTargetSensors?sensors=null,GoapCapabilities capabilities=default){ArgumentNullException.ThrowIfNull(plan);var snapshot=new GoapSnapshot(Domain);if(!snapshot.Take(in context,goal,costs,sensors,capabilities,Settings.DistanceCost)){plan.Clear();plan.Failure=PlanFailure.AlreadyMet;return PlanFailure.AlreadyMet;}return Search(snapshot,plan);}
...
public PlanFailure Search(GoapSnapshot snapshot,GoapPlan plan){ArgumentNullException.ThrowIfNull(snapshot);ArgumentNullException.ThrowIfNull(plan);plan.Clear();Traced?.Clear();LastExpanded=0;if((uint)snapshot.Goal>=(uint)Domain.Goals.Length){plan.Failure=PlanFailure.NoGoal;return PlanFailure.NoGoal;}var wanted=Domain.Goals[snapshot.Goal];var world=(ReadOnlySpan<int>)snapshot.World;plan.Goal=wanted.Name;plan.GoalIndex=snapshot.Goal;if(wanted.Met(world)){plan.Failure=PlanFailure.AlreadyMet;return PlanFailure.AlreadyMet;}nodes.Clear();open.Clear(); // The goal's own unmet conditions are the roots: one node per action that can serve one of
// them, which is where a backwards search starts.
foreach(var condition in wanted.Conditions){if(condition.Holds(world)){continue;}var found=Domain.Servers(in condition,candidates);for(var index=0;index<found;index++){Push(snapshot,candidates[index],parent:-1);}}var deepest=false;while(open.Count>0){if(LastExpanded>=Settings.NodeBudget){return Give(plan,PlanFailure.BudgetExhausted);}var index=Take();var node=nodes[index];LastExpanded++;if(Runnable(world,node.Action)){Unwind(plan,index);plan.Cost=node.Cost;plan.Expanded=LastExpanded;return PlanFailure.None;}if(node.Depth>=Settings.DepthLimit){deepest=true;Traced?.Add(new(node.Action,GoapRejection.TooDeep));continue;}Traced?.Add(new(node.Action,GoapRejection.ConditionsUnmet));Expand(snapshot,index);}return Give(plan,deepest?PlanFailure.DepthExceeded:PlanFailure.Unreachable);}PlanFailure Give(GoapPlan plan,PlanFailure failure){plan.Failure=failure;plan.Expanded=LastExpanded;return failure;}
/// <summary>Whether every one of an action's conditions holds right now.</summary>
bool Runnable(ReadOnlySpan<int>world,int action){foreach(var condition in Domain[action].Conditions){if(!condition.Holds(world)){return false;}}return true;}void Expand(GoapSnapshot snapshot,int index){var node=nodes[index];var action=Domain[node.Action];var world=(ReadOnlySpan<int>)snapshot.World;for(var slot=0;slot<action.Conditions.Length;slot++){if(action.Conditions[slot].Holds(world)){continue;}foreach(var server in Domain.Servers(node.Action,slot)){Push(snapshot,server,index);}}}void Push(GoapSnapshot snapshot,int action,int parent){if(!snapshot.Capabilities.Allows(action)){Traced?.Add(new(action,GoapRejection.NotCapable));return;}if(Repeats(parent,action)){Traced?.Add(new(action,GoapRejection.AlreadyInTheChain));return;}var cost=(parent>=0?nodes[parent].Cost:0f)+snapshot.Costs[action];var node=new Node{Action=action,Parent=parent,Depth=(parent>=0?nodes[parent].Depth:0)+1,Cost=cost,Estimate=cost+Unmet(snapshot.World,action)};nodes.Add(node);open.Add(nodes.Count-1);}
/// <summary>⚠ An action may not appear twice in one chain, or two that serve each other never end.</summary>
bool Repeats(int parent,int action){for(var index=parent;index>=0;index=nodes[index].Parent){if(nodes[index].Action==action){return true;}}return false;}
/// <summary>The heuristic: how many of an action's conditions are not true yet.</summary>
    ...
    /// </remarks>
int Unmet(ReadOnlySpan<int>world,int action){var count=0;foreach(var condition in Domain[action].Conditions){if(!condition.Holds(world)){count++;}}return count;}
/// <summary>The cheapest open node, taken out.</summary>
    ...
    ///     state to get wrong.
    /// </remarks>
int Take(){var best=0;for(var index=1;index<open.Count;index++){if(nodes[open[index]].Estimate<nodes[open[best]].Estimate){best=index;}}var chosen=open[best];open.RemoveAt(best);return chosen;}
/// <summary>Walks a finished node back to the goal, which is the plan in execution order.</summary>
void Unwind(GoapPlan plan,int index){for(var node=index;node>=0;node=nodes[node].Parent){plan.Add(nodes[node].Action);}}struct Node{public int Action;public int Parent;public int Depth;public float Cost;public float Estimate;}}
