...
public static class ActionCostModels {
    ...
    public float Cost(in AgentContext context, GoapAction action, Vector3 from, in GoapTarget target) {
        ArgumentNullException.ThrowIfNull(action);
        var distance = target.Found
            ? (target.Position - from).Length()
            : 0f; // ⚠ Floored at one, not at zero. The heuristic counts unmet conditions, and it is admissible
...
public sealed class GoapPlanner {
    ...
    public PlanFailure Search(GoapSnapshot snapshot, GoapPlan plan) {
        ...
        nodes.Clear();
        open.Clear(); // The goal's own unmet conditions are the roots: one node per action that can serve one of
