...
using System.Globalization;using Vixen.Core;using Vixen.Ecs;namespace Vixen.Ai.Diagnostics;
...
/// <inheritdoc />
public override string ToString()=>string.Create(CultureInfo.InvariantCulture,$"[{Tick}] {Entity} stopped at {Node} on {Breakpoint}" );}
/// <summary>
...
///     </para>
/// </remarks>
public sealed class AiBreakpoints{readonly HashSet<AiBreakpoint>breakpoints=[];
...
public void Clear(){breakpoints.Clear();LastHit=default;Hits=0;}
/// <summary>
...
public bool Halts(BehaviorTreeTemplate template,int node,Entity entity,long tick){ArgumentNullException.ThrowIfNull(template);if(!Enabled||breakpoints.Count==0){return false;} // Up the parents rather than over the set, because a path is a handful of nodes and the set
...
for(var walk=node;walk>=0;walk=template[walk].Parent){var candidate=new AiBreakpoint(template.Name,walk);if(!breakpoints.Contains(candidate)){continue;}LastHit=new(candidate,entity,node,tick);Hits++;return true;}return false;}
/// <summary>Lets a stopped agent go.</summary>
    ...
    /// </remarks>
public static void Resume(BehaviorTreeInstance instance){ArgumentNullException.ThrowIfNull(instance);instance.Resume();}}
