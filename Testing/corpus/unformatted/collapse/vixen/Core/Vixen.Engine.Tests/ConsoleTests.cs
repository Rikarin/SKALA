...
using Vixen.Core; using Vixen.Core.Mathematics; using Vixen.Engine.Diagnostics; using Vixen.Engine.Diagnostics.Overlays; using Xunit; namespace Vixen.Engine.Tests;
...
public sealed class ConsoleTests {
    ...
    public void FlagsAcceptTheWordsPeopleActuallyType() {
        ...
        commands.Register(
            "set",
            "",
            entry => { if (entry.TryFlag(0, out var value)) {
                    results.Add(value);
                } }
        );
