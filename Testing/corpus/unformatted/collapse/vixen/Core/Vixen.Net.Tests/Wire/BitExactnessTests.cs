...
public sealed class BitExactnessTests {
    ...
    static readonly (string Name, float Value)[] Interesting = [
        ("zero", 0f), ("negative-zero", BitConverter.UInt32BitsToSingle(0x8000_0000)), ("one", 1f), ("minus-one", -1f),
        ("smallest-denormal", BitConverter.UInt32BitsToSingle(0x0000_0001)),
        ("largest-denormal", BitConverter.UInt32BitsToSingle(0x007F_FFFF)), ("epsilon", float.Epsilon),
    ...
    public void AWholeSnapshotIsTheSameBytesEverywhere() {
        ...
        var entities = new List<Entity>(); // Positions and rotations stated rather than generated. A loop with a trigonometric function
