...
using Vixen.Core.Imaging; using Vixen.Core.Mathematics; using Vixen.Ui.Testing.Visual; using Xunit; namespace Vixen.Ui.Testing.Tests;
...
public sealed class ScreenshotTests : IDisposable {
    readonly string root = Path.Combine(Path.GetTempPath(), "vixen-ui-testing-" + Guid.NewGuid().ToString("n"));
