...
public sealed class GamutMapTests {
    ...
    public void Two_of_three_Tailwind_colours_need_sRGB_and_none_need_P3() {
        ...
        Assert.True(
            GamutMap.InGamut(red, ColorGamut.Srgb)
    ...
    public void An_in_gamut_colour_is_untouched() {
        ...
        Assert.Equal(
            colour,
            GamutMap.Map(colour, ColorGamut.Srgb)
    ...
    public void A_mapped_colour_is_inside_the_gamut(ColorGamut gamut) {
        for (var hue = 0f; hue < 360f; hue += 15f) {
            ...
            Assert.True(GamutMap.InGamut(mapped, gamut), $"hue {hue} mapped to {mapped}, still outside {gamut}");
        }
    }
    ...
    public void The_difference_metric_is_a_plain_distance() {
        ...
        Assert.Equal(5f, GamutMap.DeltaEOk(new Oklab(0f, 0f, 0f), new Oklab(3f, 4f, 0f)), 4);
    }
    ...
    public void Map_agrees_with_the_specification_ordering() {
        ...
        Gen.Select(channel, channel, channel, Gen.Int[0, 2])
            .Sample(sample => { var (r, g, b, which) = sample; var colour = new Vector3(r, g, b); var gamut =
                        (ColorGamut)which; var actual = GamutMap.Map(colour, gamut); var expected =
                        SpecificationOrder(colour, gamut); Assert.Equal(expected.X, actual.X, 4); Assert.Equal(
                        expected.Y,
                        actual.Y,
                        4
