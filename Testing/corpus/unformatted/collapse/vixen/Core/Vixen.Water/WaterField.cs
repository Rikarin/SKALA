...
using Vixen.Core.Mathematics;namespace Vixen.Water;
...
public string?Validate(){if(Resolution<2){return$"A field of {Resolution} texels has no interior to interpolate across." ;}if(!(Extent>0f)){return"A field of no extent covers nothing." ;}return null;}
/// <summary>The window this one becomes when the view moves, snapped so it does not shimmer.</summary>
    ...
    ///     </para>
    /// </remarks>
public WaterFieldDescription Snap(Vector2 centre,float coarsestTexel=0f){var step=coarsestTexel>0f?coarsestTexel:MetresPerTexel;if(!(step>0f)){return this with{Origin=centre-new Vector2(Extent*0.5f)};}var low=centre-new Vector2(Extent*0.5f);return this with{Origin=new(MathF.Floor(low.X/step)*step,MathF.Floor(low.Y/step)*step)};}}
...
/// </remarks>
public readonly record struct WaterFieldSample(float Coverage,float SurfaceHeight,Vector2 Flow,float GroundHeight){
...
///     </para>
/// </remarks>
public sealed class WaterField{readonly float[]surface;readonly float[]flowX;readonly float[]flowZ;readonly float[]ground;readonly float[]coverage;
...
public WaterField(in WaterFieldDescription description){if(description.Validate()is{}why){throw new ArgumentException(why,nameof(description));}Description=description;var texels=description.Resolution*description.Resolution;surface=new float[texels];flowX=new float[texels];flowZ=new float[texels];ground=new float[texels];coverage=new float[texels];}
...
/// <summary>Moves the window without reallocating, and forgets what was in it.</summary>
    ...
    /// </remarks>
public void Move(in WaterFieldDescription description){if(description.Resolution!=Description.Resolution){throw new ArgumentException($"The field holds {Description.Resolution}² texels and cannot become " +$"{description.Resolution}². Moving a window keeps its memory; changing its " +"resolution is a new field." ,nameof(description));}if(description.Validate()is{}why){throw new ArgumentException(why,nameof(description));}Description=description;Clear();}
/// <summary>Forgets everything the field holds.</summary>
public void Clear(){Array.Clear(surface);Array.Clear(flowX);Array.Clear(flowZ);Array.Clear(ground);Array.Clear(coverage);CoveredTexels=0;}
/// <summary>Rasterises every body and the ground beneath them.</summary>
    ...
    ///     </para>
    /// </remarks>
public void Rasterize(IReadOnlyList<WaterBody>bodies,IWaterGround beneath){ArgumentNullException.ThrowIfNull(bodies);ArgumentNullException.ThrowIfNull(beneath);Clear(); // Highest priority first, because the composite is an *over*. The sort's own stability does
// not matter and must not: what makes two orderings agree is that everything inside one
// priority is combined commutatively.
var ordered=new List<WaterBody>(bodies);ordered.Sort(static(left,right)=>right.Priority.CompareTo(left.Priority));var resolution=Description.Resolution;var step=Description.Extent/(resolution-1);for(var z=0;z<resolution;z++){for(var x=0;x<resolution;x++){var index=(z*resolution)+x;var position=Description.Origin+new Vector2(x*step,z*step);var bed=beneath.HeightAt(position); // ⚠ Starts below everything rather than at the ground, so that a place no island
...
var raised=float.NegativeInfinity;float claimed=0f,height=0f,vx=0f,vz=0f;var remaining=1f;for(var first=0;first<ordered.Count&&remaining>0f;){var priority=ordered[first].Priority;var last=first;while(last<ordered.Count&&ordered[last].Priority==priority){last++;} // One priority group, averaged by coverage — the commutative half.
float weight=0f,groupHeight=0f,groupX=0f,groupZ=0f,groupCoverage=0f;var subtracted=0f;for(var at=first;at<last;at++){var body=ordered[at];var contribution=body.Sample(position);if(contribution.Coverage<=0f){continue;}if(body.IsSubtractive){subtracted=MathF.Max(subtracted,contribution.Coverage);raised=MathF.Max(raised,contribution.SurfaceHeight+(contribution.BedDepth*contribution.Coverage));continue;}weight+=contribution.Coverage;groupHeight+=contribution.SurfaceHeight*contribution.Coverage;groupX+=contribution.Flow.X*contribution.Coverage;groupZ+=contribution.Flow.Y*contribution.Coverage;groupCoverage=MathF.Max(groupCoverage,contribution.Coverage); // The bed a body wants, only where it is deeper than what is already there —
...
bed=MathF.Min(bed,contribution.SurfaceHeight-(contribution.BedDepth*contribution.Coverage));}if(weight>0f){var take=groupCoverage*remaining;height+=groupHeight/weight*take;vx+=groupX/weight*take;vz+=groupZ/weight*take;claimed+=take;remaining-=take;}if(subtracted>0f){ // An island takes its share and gives no water back, which is what makes it
// hide the bodies underneath rather than blend with them.
remaining-=subtracted*remaining;}first=last;}ground[index]=MathF.Max(bed,raised);if(claimed>0f){ // Normalised, so a place a single body half-covers still reads that body's own
// surface height rather than half of it plus half of nothing.
surface[index]=height/claimed;flowX[index]=vx/claimed;flowZ[index]=vz/claimed;coverage[index]=WaterMath.Saturate(claimed);CoveredTexels++;}else{surface[index]=ground[index];}}}}
/// <summary>What the field says about a place, interpolated between its texels.</summary>
    ...
    ///     </para>
    /// </remarks>
public WaterFieldSample Sample(Vector2 position){var resolution=Description.Resolution;var step=Description.Extent/(resolution-1);var local=(position-Description.Origin)/step;if(local.X<-1f||local.Y<-1f||local.X>resolution||local.Y>resolution){return WaterFieldSample.None;}var x0=Math.Clamp((int)MathF.Floor(local.X),0,resolution-1);var z0=Math.Clamp((int)MathF.Floor(local.Y),0,resolution-1);var x1=Math.Min(x0+1,resolution-1);var z1=Math.Min(z0+1,resolution-1);var fx=WaterMath.Saturate(local.X-x0);var fz=WaterMath.Saturate(local.Y-z0);var a=(z0*resolution)+x0;var b=(z0*resolution)+x1;var c=(z1*resolution)+x0;var d=(z1*resolution)+x1;return new(Blend(coverage,a,b,c,d,fx,fz),Blend(surface,a,b,c,d,fx,fz),new(Blend(flowX,a,b,c,d,fx,fz),Blend(flowZ,a,b,c,d,fx,fz)),Blend(ground,a,b,c,d,fx,fz));}
...
public WaterFieldSample At(int x,int z){if((uint)x>=(uint)Description.Resolution||(uint)z>=(uint)Description.Resolution){return WaterFieldSample.None;}var index=(z*Description.Resolution)+x;return new(coverage[index],surface[index],new(flowX[index],flowZ[index]),ground[index]);}static float Blend(float[]channel,int a,int b,int c,int d,float fx,float fz){var top=channel[a]+((channel[b]-channel[a])*fx);var bottom=channel[c]+((channel[d]-channel[c])*fx);return top+((bottom-top)*fz);}}
