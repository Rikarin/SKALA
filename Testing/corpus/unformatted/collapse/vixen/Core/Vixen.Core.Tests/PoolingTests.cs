...
using Vixen.Core.Pooling; using Xunit; namespace Vixen.Core.Tests;
...
public class PoolingTests {
    ...
    public void An_object_pool_builds_an_instance_when_it_is_empty() {
        var built = 0;
        var pool = new ObjectPool<Box>(() => { built++; return new(); });
        pool.Rent();
        pool.Rent();
    ...
    public void Clearing_a_pooled_list_keeps_the_buffer_and_disposing_gives_it_back() {
        ...
        list.Dispose();
        Assert.Equal(0, list.Count);
        Assert.Equal(0, list.Capacity); // Disposing twice must not return the same buffer to the pool twice.
        list.Dispose();
    }
    ...
    public void Disposing_a_pooled_dictionary_clears_it_before_the_pool_takes_it_back() {
        var map = new PooledDictionary<string, int>();
        map.Add("stale", 1); // The instance the pool will hand to the next renter.
