...
// what to expect, and tagging every node would make a .meta file unreadable.
    ...
    static YamlNode EmitValue(object value, Type runtime, YamlSerializerOptions options, string path) {
        ...
        switch (value) {
            case bool flag: return new YamlScalar(flag ? "true" : "false", YamlScalarStyle.Plain);
            ...
            case IDictionary dictionary: return EmitDictionary(dictionary, runtime, options, path);
            case IEnumerable items: return EmitSequence(items, runtime, options, path);
            default: return EmitContract(value, runtime, options, path);
        }
    }
