...
public sealed partial class PhysicsScene : IDisposable {
    static readonly QueryDescription NeedBody =
        new QueryDescription().WithAll<Collider, LocalTransform>().WithNone<PhysicsBody>();

    static readonly QueryDescription HaveBody = new QueryDescription().WithAll<PhysicsBody, LocalTransform>();
    static readonly QueryDescription OrphanedBody = new QueryDescription().WithAll<PhysicsBody>().WithNone<Collider>();
    ...
    void CollectStructuralChanges() {
        ...
        foreach (var chunk in Entities.Chunks(HaveBody)) {
            ...
            for (var index = 0; index < chunk.Count; index++) {
                ...
                var motion = Entities.TryGet<RigidBody>(entity, out var rigid) ? rigid.Motion : BodyMotion.Static;
                var body = bodies[index];
    ...
    void Arrive(Entity entity, in LocalTransform transform) {
        ...
        interpolation.CurrentRotation = transform.Rotation; // The receipt, kept honest: nothing on a rigid body reads it, but a character bridged through
