...
using Vixen.Core.Threading;using Vixen.Ecs.Systems;using Vixen.Engine.Players;using Vixen.Engine.Transforms;using Vixen.Physics.Characters;namespace Vixen.Physics.Ecs;
...
///     </para>
/// </remarks>
[UpdateInGroup(SystemPhase.FixedUpdate)][UpdateAfter(typeof(PhysicsStepSystem))]public sealed class CharacterMovementSystem(PhysicsScene scene):SystemBase,IDeclaredAccess{readonly PhysicsScene scene=scene??throw new ArgumentNullException(nameof(scene));
/// <inheritdoc />
    /// <remarks>
    ...
    /// </remarks>
public SystemAccess Access{get;}=SystemAccess.Declare().Read<CharacterMovement>().Read<MoveIntent>().Write<CharacterState>().Write<CharacterBody>().Write<LocalTransform>().Write<PhysicsInterpolation>().Build();
/// <inheritdoc />
...
dependency.Complete();scene.StepCharacters((float)context.Time.Elapsed.TotalSeconds);return default;}}
