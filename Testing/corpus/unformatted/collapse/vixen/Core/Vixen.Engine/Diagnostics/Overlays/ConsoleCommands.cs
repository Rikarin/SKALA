...
using System.Diagnostics.CodeAnalysis;using System.Reflection;namespace Vixen.Engine.Diagnostics.Overlays;
...
///     </para>
/// </remarks>
[AttributeUsage(AttributeTargets.Method)]public sealed class ConsoleCommandAttribute:Attribute{
/// <summary>Declares a command.</summary>
    /// <param name="name">What is typed to run it.</param>
public ConsoleCommandAttribute(string name){ArgumentException.ThrowIfNullOrWhiteSpace(name);Name=name;}
...
/// </remarks>
public sealed class ConsoleContext{readonly List<string>arguments=[];internal ConsoleContext(ConsoleCommands commands)=>Commands=commands;
...
public bool TryNumber(int index,out float value)=>float.TryParse(this[index],System.Globalization.CultureInfo.InvariantCulture,out value);
/// <summary>Reads an argument as on or off.</summary>
    ...
    /// </remarks>
public bool TryFlag(int index,out bool value){value=false;switch(this[index]?.ToLowerInvariant()){case"on"or"1"or"true"or"yes":value=true;return true;case"off"or"0"or"false" or"no":return true;default:return false;}}internal void Reset(string line,ReadOnlySpan<char>rest){Line=line;arguments.Clear();while(!rest.IsEmpty){rest=rest.TrimStart();if(rest.IsEmpty){break;} // Quoted runs are one argument, so `echo "hello world"` says what it looks like it says.
if(rest[0]=='"'){var close=rest[1 .. ].IndexOf('"');var end=close<0?rest.Length:close+1;arguments.Add(rest[1 .. end].ToString());rest=close<0?default:rest[(end+1) .. ];continue;}var space=rest.IndexOf(' ');if(space<0){arguments.Add(rest.ToString());break;}arguments.Add(rest[ .. space].ToString());rest=rest[space .. ];}}}
...
public const int MaxHistory=64;static readonly Lock ContributionLock=new();static readonly List<Action<ConsoleCommands>>Contributions=[];static readonly List<WeakReference<ConsoleCommands>>Live=[];readonly Dictionary<string,ConsoleCommand>commands=new(StringComparer.OrdinalIgnoreCase);readonly List<string>output=[];readonly List<string>history=[];readonly ConsoleContext context;
/// <summary>Builds a registry with the built-in commands and every contributed one in it.</summary>
public ConsoleCommands(){context=new(this);Register("help","Lists the commands, or explains one: help [command]" ,Help);Register("clear" ,"Empties the console output" ,_=>Clear());Register("echo","Writes its arguments back" ,static entry=>entry.Write(string.Join(' ',entry.Arguments)));Action<ConsoleCommands>[]contributed;lock(ContributionLock){contributed=[ .. Contributions];Live.RemoveAll(reference=>!reference.TryGetTarget(out _));Live.Add(new(this));}foreach(var contribution in contributed){contribution(this);}}
/// <summary>
    ...
    ///     </para>
    /// </remarks>
public static void Contribute(Action<ConsoleCommands>register){ArgumentNullException.ThrowIfNull(register);ConsoleCommands[]existing;lock(ContributionLock){Contributions.Add(register);existing=[ .. Live.Select(reference=>reference.TryGetTarget(out var live)?live:null).OfType<ConsoleCommands>()];}foreach(var registry in existing){register(registry);}}
...
public void Register(string name,string help,Action<ConsoleContext>run){ArgumentException.ThrowIfNullOrWhiteSpace(name);ArgumentNullException.ThrowIfNull(run);commands[name]=new(name,help??string.Empty,run);}
...
public int Complete(ReadOnlySpan<char>prefix,List<string>matches){ArgumentNullException.ThrowIfNull(matches);matches.Clear();foreach(var name in commands.Keys){if(name.AsSpan().StartsWith(prefix,StringComparison.OrdinalIgnoreCase)){matches.Add(name);}}matches.Sort(StringComparer.OrdinalIgnoreCase);return matches.Count;}
/// <summary>Runs a line.</summary>
    ...
    /// </remarks>
public bool Execute(string line){if(string.IsNullOrWhiteSpace(line)){return false;}Remember(line);Write($"> {line}" );var trimmed=line.AsSpan().Trim();var space=trimmed.IndexOf(' ');var name=space<0?trimmed:trimmed[ .. space];if(!commands.TryGetValue(name.ToString(),out var command)){Write($"There is no command called '{name}'. Try 'help'." );return false;}context.Reset(line,space<0?default:trimmed[space .. ]);try{command.Run(context);return true;}catch(Exception exception){Write($"{command.Name} failed: {exception.GetType().Name}: {exception.Message}" );return false;}}
/// <summary>Writes a line of output.</summary>
    /// <param name="text">What to say.</param>
public void Write(string text){output.Add(text??string.Empty);if(output.Count>MaxOutput){output.RemoveRange(0,output.Count-MaxOutput);}}
...
/// <summary>
    ...
    /// </remarks>
[RequiresUnreferencedCode("Scans every type in the assembly for [ConsoleCommand]; trimming may remove them." )]public int RegisterFrom(Assembly assembly){ArgumentNullException.ThrowIfNull(assembly);var found=0;foreach(var type in assembly.GetTypes()){found+=RegisterFrom(type);}return found;}
/// <summary>Registers the marked static methods of one type.</summary>
    /// <param name="type">Where to look.</param>
    /// <returns>How many were registered.</returns>
[RequiresUnreferencedCode("Reads the type's methods reflectively; trimming may remove them." )]public int RegisterFrom(Type type){ArgumentNullException.ThrowIfNull(type);var found=0;foreach(var method in type.GetMethods(BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.Static)){if(method.GetCustomAttribute<ConsoleCommandAttribute>()is not{}attribute){continue;}var parameters=method.GetParameters(); // Refused rather than skipped: a method somebody marked and got the shape wrong on is a
// command they expect to be able to type, and silence is the worst possible answer.
if(parameters.Length!=1||parameters[0].ParameterType!=typeof(ConsoleContext)){throw new InvalidOperationException($"[ConsoleCommand] {type.Name}.{method.Name} must be static and take one ConsoleContext." );}Register(attribute.Name,attribute.Help,method.CreateDelegate<Action<ConsoleContext>>());found++;}return found;}void Remember(string line){ // A repeat of the last line is not worth a history entry — holding Up to find something is
// the thing history is for, and a run of identical entries defeats it.
if(history.Count>0&&string.Equals(history[^1],line,StringComparison.Ordinal)){return;}history.Add(line);if(history.Count>MaxHistory){history.RemoveRange(0,history.Count-MaxHistory);}}void Help(ConsoleContext entry){if(entry.Count>0){if(commands.TryGetValue(entry[0]!,out var one)){entry.Write($"{one.Name} — {one.Help}" );}else{entry.Write($"There is no command called '{entry[0]}'." );}return;}var names=new List<string>(commands.Keys);names.Sort(StringComparer.OrdinalIgnoreCase);foreach(var name in names){entry.Write($"{name,-16} {commands[name].Help}" );}}}
