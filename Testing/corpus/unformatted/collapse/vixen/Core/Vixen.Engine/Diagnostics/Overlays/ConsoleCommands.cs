...
public sealed class ConsoleContext {
    ...
    public bool TryFlag(int index, out bool value) {
        value = false;
        switch (this[index]?.ToLowerInvariant()) { case "on" or "1" or "true" or "yes":
                value = true;
                return true; case "off" or "0" or "false" or "no": return true; default: return false; }
    }
...
public sealed class ConsoleCommands {
    ...
    public static void Contribute(Action<ConsoleCommands> register) {
        ...
        lock (ContributionLock) {
            ...
            existing = [
                .. Live.Select(reference => reference.TryGetTarget(out var live) ? live : null)
    ...
    public int RegisterFrom(Type type) {
        ...
        foreach (var method in type.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static)) {
            ...
            var parameters = method.GetParameters(); // Refused rather than skipped: a method somebody marked and got the shape wrong on is a
            ...
            Register(attribute.Name, attribute.Help, method.CreateDelegate<Action<ConsoleContext>>());
            found++;
        }
