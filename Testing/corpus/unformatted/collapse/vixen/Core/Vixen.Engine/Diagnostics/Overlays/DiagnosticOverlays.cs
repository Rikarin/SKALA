...
using Vixen.Core;using Vixen.Core.Mathematics;namespace Vixen.Engine.Diagnostics.Overlays;
...
///     </para>
/// </remarks>
public sealed class DiagnosticOverlays{readonly List<IDiagnosticOverlay>overlays=[];readonly OverlaySurface surface=new();readonly HashSet<string>requested=new(StringComparer.OrdinalIgnoreCase);
...
public void Add(IDiagnosticOverlay overlay){ArgumentNullException.ThrowIfNull(overlay); // Names are what the console types and what a settings file writes down, so two overlays
...
if(Find(overlay.Name)is not null){throw new ArgumentException($"An overlay called '{overlay.Name}' is already registered." ,nameof(overlay));}overlays.Add(overlay); // ⚠ Applied on registration and not once at start-up, because a subsystem's panel is
...
if(requested.Contains(overlay.Name)){overlay.Enabled=true;}}
/// <summary>Asks for panels by name, whether or not they have been registered yet.</summary>
    ...
    /// </remarks>
public void Request(IEnumerable<string>?names){if(names is null){return;}foreach(var name in names){if(string.IsNullOrWhiteSpace(name)){continue;}requested.Add(name.Trim());if(Find(name)is{}existing){existing.Enabled=true;}}}
/// <summary>Removes an overlay.</summary>
    /// <param name="name">What it is called.</param>
    /// <returns><see langword="true" /> if one was removed.</returns>
public bool Remove(string name){var overlay=Find(name);return overlay is not null&&overlays.Remove(overlay);}
/// <summary>Finds a registered overlay by name, case-insensitively.</summary>
    /// <param name="name">What it is called.</param>
    /// <returns>The overlay, or <see langword="null" />.</returns>
public IDiagnosticOverlay?Find(string?name){if(string.IsNullOrEmpty(name)){return null;}foreach(var overlay in overlays){if(string.Equals(overlay.Name,name,StringComparison.OrdinalIgnoreCase)){return overlay;}}return null;}
...
public bool?Set(string name,bool?enabled=null){var overlay=Find(name);if(overlay is null){return null;}overlay.Enabled=enabled??!overlay.Enabled;return overlay.Enabled;}
/// <summary>Adds the <c>overlay</c> and <c>overlays</c> commands to a console.</summary>
    ...
    ///     without the other.
    /// </remarks>
public void RegisterCommands(ConsoleCommands commands){ArgumentNullException.ThrowIfNull(commands);commands.Register("overlays" ,"Lists the diagnostic overlays and whether each is on" ,entry=>{foreach(var overlay in overlays){entry.Write($"{overlay.Name,-14} {(overlay.Enabled ? "on" : "off")}  ({overlay.Anchor})" );}});commands.Register("overlay" ,"Turns one on or off: overlay <name> [on|off]" ,entry=>{if(entry.Count==0){entry.Write("overlay <name> [on|off]" );return;}var name=entry[0]!;var state=entry.TryFlag(1,out var wanted)?wanted:(bool?)null;var result=Set(name,state);entry.Write(result is null?$"There is no overlay called '{name}'. Try 'overlays'." :$"{name} is {(result.Value ? "on" : "off")}." );});}
/// <summary>Turns every overlay off.</summary>
public void DisableAll(){foreach(var overlay in overlays){overlay.Enabled=false;}}
/// <summary>Draws every enabled overlay into a frame's accumulator.</summary>
    ...
    /// </remarks>
public void Draw(DebugDraw draw,Vector2 viewport,in GameTime time){ArgumentNullException.ThrowIfNull(draw);DrawnCount=0;if(!Enabled||!draw.Enabled||viewport.X<=0f||viewport.Y<=0f){return;}surface.Begin(draw,viewport,Theme);foreach(var overlay in overlays){if(!overlay.Enabled){continue;}overlay.Draw(surface,time);DrawnCount++;}}}
