...
///     </para>
/// </remarks>
sealed class SparseMatrixBuilder{readonly List<int>rows=[];readonly List<int>columns=[];readonly List<double>values=[];
...
public SparseMatrixBuilder(int rowCount,int columnCount){ArgumentOutOfRangeException.ThrowIfNegative(rowCount);ArgumentOutOfRangeException.ThrowIfNegative(columnCount);RowCount=rowCount;ColumnCount=columnCount;}
/// <summary>Records one contribution.</summary>
    ...
    /// </remarks>
public void Add(int row,int column,double value){ArgumentOutOfRangeException.ThrowIfNegative(row);ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(row,RowCount);ArgumentOutOfRangeException.ThrowIfNegative(column);ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual(column,ColumnCount);rows.Add(row);columns.Add(column);values.Add(value);}
/// <summary>Sorts, merges and compacts everything added so far.</summary>
    ...
    /// </remarks>
public SparseMatrix Build(){var count=rows.Count;var starts=new int[RowCount+1];for(var triplet=0;triplet<count;triplet++){starts[rows[triplet]+1]++;}for(var row=0;row<RowCount;row++){starts[row+1]+=starts[row];}var cursor=new int[RowCount];starts.AsSpan(0,RowCount).CopyTo(cursor);var scatteredColumns=new int[count];var scatteredValues=new double[count];for(var triplet=0;triplet<count;triplet++){var slot=cursor[rows[triplet]]++;scatteredColumns[slot]=columns[triplet];scatteredValues[slot]=values[triplet];}for(var row=0;row<RowCount;row++){SortRow(scatteredColumns,scatteredValues,starts[row],starts[row+1]);}var mergedStarts=new int[RowCount+1];var mergedColumns=new int[count];var mergedValues=new double[count];var written=0;for(var row=0;row<RowCount;row++){var index=starts[row];var end=starts[row+1];while(index<end){var column=scatteredColumns[index];var sum=scatteredValues[index];index++;while(index<end&&scatteredColumns[index]==column){sum+=scatteredValues[index];index++;}mergedColumns[written]=column;mergedValues[written]=sum;written++;}mergedStarts[row+1]=written;}Array.Resize(ref mergedColumns,written);Array.Resize(ref mergedValues,written);return new(RowCount,ColumnCount,mergedStarts,mergedColumns,mergedValues);}
/// <summary>A stable insertion sort by column, over one row's scattered entries.</summary>
static void SortRow(int[]columns,double[]values,int first,int end){for(var index=first+1;index<end;index++){var column=columns[index];var value=values[index];var slot=index-1; // Strictly greater, so entries that tie keep the order they were added in. An `>=` here
// reverses duplicates and changes the sum in the last bits.
while(slot>=first&&columns[slot]>column){columns[slot+1]=columns[slot];values[slot+1]=values[slot];slot--;}columns[slot+1]=column;values[slot+1]=value;}}}
