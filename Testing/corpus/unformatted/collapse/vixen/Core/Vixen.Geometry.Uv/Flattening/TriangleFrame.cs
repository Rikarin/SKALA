...
public static TriangleFrame Build(Vector3 a,Vector3 b,Vector3 c){double ux=b.X-(double)a.X,uy=b.Y-(double)a.Y,uz=b.Z-(double)a.Z;double vx=c.X-(double)a.X,vy=c.Y-(double)a.Y,vz=c.Z-(double)a.Z;var length=Math.Sqrt((ux*ux)+(uy*uy)+(uz*uz));if(!(length>0d)){return default;}var along=((ux*vx)+(uy*vy)+(uz*vz))/length;double nx=(uy*vz)-(uz*vy),ny=(uz*vx)-(ux*vz),nz=(ux*vy)-(uy*vx);var across=Math.Sqrt((nx*nx)+(ny*ny)+(nz*nz))/length;return new(length,along,across);}}
