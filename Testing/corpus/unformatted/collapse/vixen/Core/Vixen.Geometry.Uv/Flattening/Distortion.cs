...
static class Distortion {
    ...
    ///     </para>
    /// </remarks>
    public static Measured Measure(ChartMesh chart, TriangleFrame[] frames, double[] coordinates, Vector2[] rounded) {
        var surface = 0d;
        var parameter = 0d;
        var angular = 0d;
        var area = 0d;
        ...
        var worst = 0d;
        var measurable = 0d;
        var flipped = 0; // Two passes: the area ratio each triangle is judged against is the chart's own, which is not
    ...
    static (double Larger, double Smaller) Singular(
        ...
        var vx = coordinates[(c * 2) + 0] - coordinates[(a * 2) + 0];
        var vy = coordinates[(c * 2) + 1] - coordinates[(a * 2) + 1];
        var determinant = frame.DoubleArea; // J = U·X⁻¹ with X = [[X1, X2], [0, Y2]].
