...
sealed class WorkStealingDeque {
    ...
    ///     right.
    /// </summary>
    internal int ApproximateCount { get { var count =
                Volatile.Read(ref bottom.Value) - Volatile.Read(ref top.Value); return count <= 0 ? 0 : (int)count; } }
    ...
    internal bool TryPush(long item) {
        ...
        items[b & mask] = item; // Release: the item write above must be visible to a thief before the index that exposes it.
