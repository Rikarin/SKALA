...
using System.Numerics;using System.Runtime.CompilerServices;using System.Runtime.InteropServices;namespace Vixen.Core.Threading;
/// <summary>
...
///     </para>
/// </remarks>
sealed class WorkStealingDeque{readonly long[]items;readonly int mask; // Only the owner writes `bottom`; anyone may CAS `top`. Padded apart so the owner's ordinary
// writes to one do not invalidate the other's cache line for every thief in the system.
PaddedLong bottom;PaddedLong top;
...
/// <summary>
    ...
    ///     right.
    /// </summary>
internal int ApproximateCount{get{var count=Volatile.Read(ref bottom.Value)-Volatile.Read(ref top.Value);return count<=0?0:(int)count;}}
...
internal WorkStealingDeque(int capacity){ArgumentOutOfRangeException.ThrowIfLessThan(capacity,1);var rounded=(int)BitOperations.RoundUpToPowerOf2((uint)capacity);items=new long[rounded];mask=rounded-1;}
...
[MethodImpl(MethodImplOptions.AggressiveInlining)]internal bool TryPush(long item){var b=Volatile.Read(ref bottom.Value);var t=Volatile.Read(ref top.Value);if(b-t>=items.Length){return false;}items[b&mask]=item; // Release: the item write above must be visible to a thief before the index that exposes it.
Volatile.Write(ref bottom.Value,b+1);return true;}
...
internal bool TryPop(out long item){var b=Volatile.Read(ref bottom.Value)-1;Volatile.Write(ref bottom.Value,b); // The claim above and the read below must not be reordered, or the owner and a thief can
// both conclude they have the last item.
Interlocked.MemoryBarrier();var t=Volatile.Read(ref top.Value);if(t>b){Volatile.Write(ref bottom.Value,b+1);item=0;return false;}item=items[b&mask];if(t!=b){return true;} // Exactly one item was left, so a thief may be taking the same one. Whoever wins the CAS on
// `top` has it; either way the deque is now empty.
var won=Interlocked.CompareExchange(ref top.Value,t+1,t)==t;Volatile.Write(ref bottom.Value,b+1);if(won){return true;}item=0;return false;}
/// <summary>Takes the least recently pushed item. Any thread.</summary>
    ...
    /// </returns>
internal bool TrySteal(out long item){var t=Volatile.Read(ref top.Value);Interlocked.MemoryBarrier();var b=Volatile.Read(ref bottom.Value);if(t>=b){item=0;return false;} // Read before the CAS. The read can only be stale if the owner has already overwritten the
// slot, which needs a full lap of the buffer — and TryPush refuses to lap a live item.
var candidate=items[t&mask];if(Interlocked.CompareExchange(ref top.Value,t+1,t)!=t){item=0;return false;}item=candidate;return true;}[StructLayout(LayoutKind.Explicit,Size=128)]struct PaddedLong{[FieldOffset(64)]internal long Value;}}
