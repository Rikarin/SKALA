...
using Vixen.Core.Mathematics;using Vixen.Graphics;using Vixen.Rendering.Compositor;using Vixen.Rendering.Lighting;using Vixen.Shaders;using Vixen.Shaders.Generated;namespace Vixen.Rendering.Water;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class WaterRenderer:SceneRenderer,IDisposable{readonly FullScreenRenderer pass;readonly ComputeRenderer classify;readonly List<ResourceBinding>bindings=[];BufferHandle placeholder;bool disposed;
/// <summary>Creates the node.</summary>
public WaterRenderer(){pass=new(){Name=WaterKeys.ShaderName,ShaderName=WaterKeys.ShaderName,PermutationKeys=WaterKeys.UsedPermutationKeys,ConstantBinding=WaterKeys.ConstantBufferBinding};classify=new(){Name=WaterTilesKeys.ShaderName,ShaderName=WaterTilesKeys.ShaderName,ConstantBinding=WaterTilesKeys.ConstantBufferBinding};}
...
public ComputePipelineCache?Pipelines{get=>classify.Pipelines;set=>classify.Pipelines=value;}
...
public EffectPipelineDescriber?Modules{get=>pass.Modules;set=>pass.Modules=value;}
...
public DescriptorAllocator?Allocator{get=>pass.Descriptors.Allocator;set=>pass.Descriptors.Allocator=value;}
/// <summary>The device its pipelines and its uniform block are created on.</summary>
public IGraphicsDevice?Device{get=>pass.Device;set=>pass.Device=value;}
...
/// <summary>
    ...
    ///     </para>
    /// </remarks>
public bool LightFrom(SceneLighting lighting){ArgumentNullException.ThrowIfNull(lighting);if(lighting.Sun?.Sun is not{}sun){return false;}SunDirection=sun.Direction;SunColour=sun.Radiance;if(lighting.Environment is{}sky){ // Y₀ · 4π = 3.5449, so the mean radiance is the coefficient over that — which is the same
...
SkyColour=sky.Irradiance.L00*(0.282095f*sky.Intensity);}return true;}
/// <inheritdoc />
protected override void Build(GraphicsCompositor compositor,CompositorFrame frame){ArgumentNullException.ThrowIfNull(frame);if(string.Equals(Behind,Output,StringComparison.Ordinal)){throw new CompositorBindingException(ToString(),"target" ,Behind,"is both what the water reads and what it writes. Sampling a target a pass is also " +"writing is undefined — not slow, not approximate — so the read has to come from a " +"copy. Put a !Copy node ahead of this one and name its destination here. See " +"docs/plan/35 § B1, which calls the copy the blocker rather than the pass" );}Degrade(Declare(compositor,frame));}
/// <summary>Declares the surface pass, or says why it did not.</summary>
    ...
    /// </remarks>
string?Declare(GraphicsCompositor compositor,CompositorFrame frame){if(Samplers is null||pass.Device is null){ // Nothing to run against. A document naming !Water in a host that has not wired the
...
return Samplers is null?"no Samplers, so the surface pass was never declared and the frame has no water at " +"all — a lake that is not there rather than one that looks wrong" :"no Device on the surface pass, so it was never declared and the frame has no " +"water at all — a lake that is not there rather than one that looks wrong" ;}pass.Samplers=Samplers;pass.ColourTargets.Clear();pass.Reads.Clear();pass.BufferReads.Clear();pass.Descriptors.Bindings.Clear();bindings.Clear();pass.ColourTargets.Add(Output);var tiles=Tile(compositor,frame);if(View is{}view){CameraPosition=view.Position;if(Matrix4x4.Invert(view.ViewProjection,out var inverse)){InverseViewProjection=inverse;}}pass.Parameters.Set(WaterKeys.Reflections,Reflections.Length>0);pass.Parameters.Set(WaterKeys.Foam,Foam);pass.Parameters.Set(WaterKeys.Tiled,tiles.Tiled);pass.Parameters.Set(WaterKeys.TileCount,tiles.Count);pass.Parameters.Set(WaterKeys.TargetSize,tiles.Target);pass.Parameters.Set(WaterKeys.InverseViewProjection,InverseViewProjection);pass.Parameters.Set(WaterKeys.CameraPosition,CameraPosition);pass.Parameters.Set(WaterKeys.Scattering,Scattering);pass.Parameters.Set(WaterKeys.Absorption,Absorption);pass.Parameters.Set(WaterKeys.PhaseG,PhaseG);pass.Parameters.Set(WaterKeys.BehindScale,BehindScale); // ⚠ The frame's own sun and sky where there are any, because all three are photometric
// quantities of the scene rather than settings of the document. See Lighting.
var(direction,sunlight,sky)=Lighting();pass.Parameters.Set(WaterKeys.SunColour,sunlight);pass.Parameters.Set(WaterKeys.SunDirection,direction);pass.Parameters.Set(WaterKeys.SkyColour,sky);pass.Parameters.Set(WaterKeys.SurfaceF0,SurfaceF0);pass.Parameters.Set(WaterKeys.FoamColour,FoamColour);Read(WaterKeys.SceneColourCopyBinding,Behind);Read(WaterKeys.SceneDepthBinding,SceneDepth);Read(WaterKeys.WaterSurfaceBinding,Surface);Read(WaterKeys.WaterNormalBinding,Normal); // ⚠ Bound even when the permutation is off, because a set is written wholly or not at all —
...
Read(WaterKeys.ReflectionPlaneBinding,Reflections.Length>0?Reflections:Behind); // The tile flags, on exactly those terms: declared and bound whether or not anything filled
// them, because the untiled variant's set has the slot in it too.
pass.BufferReads.Add(tiles.Buffer);bindings.Add(new(){Binding=WaterKeys.WaterTilesBinding,Kind=DescriptorKind.StorageBuffer,Resource=tiles.Buffer});bindings.Add(new(){Binding=WaterKeys.PointSamplerBinding,Kind=DescriptorKind.Sampler,Sampler=Samplers.PointClamp});bindings.Add(new(){Binding=WaterKeys.LinearSamplerBinding,Kind=DescriptorKind.Sampler,Sampler=Samplers.LinearClamp});foreach(var binding in bindings){pass.Descriptors.Bindings.Add(binding);} // ⚠ Two triangles per tile and one instance per tile, against a target that is *loaded*. The
...
pass.Vertices=tiles.Tiled?WaterTiles.VerticesPerTile:3;pass.Instances=tiles.Tiled?WaterTiles.Total(tiles.Count):1;pass.Load=tiles.Tiled?LoadAction.Load:LoadAction.DontCare;BuildCount++; // The classification first, because the draw reads what it wrote. The graph would order them
...
string?reason=null;if(tiles.Tiled){BuildChild(classify,compositor,frame);reason=classify.Degraded;}BuildChild(pass,compositor,frame);return reason??pass.Degraded;}
/// <summary>The three numbers the medium is lit by, from the frame's lighting where it has any.</summary>
    ...
    ///     </para>
    /// </remarks>
(Vector3 Direction,Vector3 Sunlight,Vector3 Sky)Lighting(){var star=Sun?.Sun; // ⚠ Y₀ = 0.282095, and Y₀·4π = 3.5449 is what separates a mean radiance from the coefficient
...
var sky=Frame?.Lighting?.Environment is{}environment?environment.Irradiance.L00*(0.282095f*environment.Intensity):SkyColour;return(star?.Direction??SunDirection,star?.Radiance??SunColour,sky);}
/// <summary>Sizes the tiling, declares its buffer, and points the classifier at both.</summary>
    ...
    ///     </para>
    /// </remarks>
Tiling Tile(GraphicsCompositor compositor,CompositorFrame frame){var described=frame.DescriptionOf(ToString(),Output);var target=described is{}size?new Int2(size.Width,size.Height):compositor.FrameSize;var tiled=Tiled&&Pipelines is not null&&target is{X:>0,Y:>0};var count=tiled?WaterTiles.CountFor(target):default;var name=$"{this}.Tiles" ;TileCount=count;if(!frame.HasBuffer(name)){var description=new BufferDescription(WaterTiles.Bytes(count),BufferUsage.Storage,tiled?MemoryAccess.DeviceLocal:MemoryAccess.HostUpload,name); // ⚠ Created when something fills it and *imported* when nothing does, and the difference is
...
// claim being made.
frame.Add(name,tiled?frame.Graph.CreateBuffer(description):frame.Graph.ImportBuffer(Placeholder(description),description,ResourceState.ShaderRead,ResourceState.ShaderRead),description);}if(!tiled){return new(false,default,target,name);}classify.Groups=new(count.X,count.Y,1);classify.Samplers=Samplers;classify.Descriptors.Allocator=pass.Descriptors.Allocator;classify.Parameters.Set(WaterTilesKeys.TileCount,count);classify.Parameters.Set(WaterTilesKeys.TargetSize,target);classify.Reads.Clear();classify.BufferWrites.Clear();classify.Descriptors.Bindings.Clear();classify.Reads.Add(Surface);classify.BufferWrites.Add(name);classify.Descriptors.Bindings.Add(new(){Binding=WaterTilesKeys.WaterSurfaceBinding,Kind=DescriptorKind.SampledTexture,Resource=Surface});classify.Descriptors.Bindings.Add(new(){Binding=WaterTilesKeys.PointSamplerBinding,Kind=DescriptorKind.Sampler,Sampler=Samplers!.PointClamp});classify.Descriptors.Bindings.Add(new(){Binding=WaterTilesKeys.TilesBinding,Kind=DescriptorKind.StorageBuffer,Resource=name});return new(true,count,target,name);}
/// <summary>The one word the untiled variant binds and never reads, made once.</summary>
    ...
    /// </remarks>
BufferHandle Placeholder(in BufferDescription description){if(placeholder.IsValid){return placeholder;}placeholder=pass.Device!.CreateBuffer(description);pass.Device.Write(placeholder,0,new byte[description.Size]);return placeholder;}
...
/// <summary>Adds a sampled texture to the pass, and records that it is read.</summary>
    ...
    /// </remarks>
void Read(uint binding,string resource){if(string.IsNullOrEmpty(resource)){return;}pass.Reads.Add(resource);bindings.Add(new(){Binding=binding,Kind=DescriptorKind.SampledTexture,Resource=resource});}
/// <inheritdoc />
public void Dispose(){if(disposed){return;}disposed=true;if(placeholder.IsValid){pass.Device?.Destroy(placeholder);placeholder=default;}pass.Dispose();classify.Dispose();}}
