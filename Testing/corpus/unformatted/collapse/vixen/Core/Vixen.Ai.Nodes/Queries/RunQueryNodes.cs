...
using System.Runtime.CompilerServices;using System.Runtime.InteropServices;using Vixen.Core;using Vixen.Core.Mathematics;using Vixen.Engine.Transforms;namespace Vixen.Ai.Nodes;
...
///     </para>
/// </remarks>
public readonly record struct QueryBinding(EnvironmentQuery Query,BlackboardKey?Context=null,BlackboardKey?Result=null,BlackboardKey?ResultEntity=null);
...
public sealed class RunQueryTask(QueryBinding binding):IAgentAction{readonly QueryResults results=new();
...
/// <inheritdoc />
public ActionStatus Tick(in AgentContext context,Span<byte>state,float delta)=>QueryRunner.Run(in context,in binding,results)?ActionStatus.Succeeded:ActionStatus.Failed;
/// <inheritdoc />
...
public QueryResults Results=>results;
/// <inheritdoc />
public override void Tick(in BehaviorContext context,Span<byte>state,float delta){var agent=context.Agent;if(QueryRunner.Run(in agent,in binding,results)){return;}if(binding.Result is{IsValid:true}result){agent.Blackboard.Clear(result);}if(binding.ResultEntity is{IsValid:true}named){agent.Blackboard.Clear(named);}}}
/// <summary>The half a task and a service share: build the origin, run, write the keys.</summary>
static class QueryRunner{public static bool Run(in AgentContext context,in QueryBinding binding,QueryResults results){if(binding.Query is null||!context.World.Has<LocalTransform>(context.Entity)){return false;}var here=context.World.Read<LocalTransform>(context.Entity).Position;var origin=new QueryOrigin(here,Vector3.Zero);if(binding.Context is{IsValid:true}about&&AgentTarget.TryResolve(in context,about,out var at,out var entity)){origin=new(here,at,true,entity);}if(!binding.Query.Run(in context,in origin,results)||!results.TryBest(out var best)){return false;}if(binding.Result is{IsValid:true}result){context.Blackboard.SetVector3(result,best.Position);}if(binding.ResultEntity is not{IsValid:true}named){return true;}if(best.Entity.IsNull){context.Blackboard.Clear(named);}else{context.Blackboard.SetEntity(named,best.Entity);}return true;}}
