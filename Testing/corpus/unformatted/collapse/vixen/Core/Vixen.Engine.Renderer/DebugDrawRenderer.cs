...
using Vixen.Core.Mathematics;using Vixen.Engine.Diagnostics;using Vixen.Graphics;using Vixen.Rendering;namespace Vixen.Engine.Renderer;
/// <summary>
...
///     </para>
/// </remarks>
public sealed class DebugDrawRenderer:IDisposable{readonly LineRenderer world;readonly LineRenderer screen;readonly DebugGeometry geometry=new();Matrix4x4 screenProjection=Matrix4x4.Identity;bool hasScreen;bool disposed;
...
public DebugDrawRenderer(IGraphicsDevice device,LineShaders shaders,RenderOutput output,int worldCapacity=1<<16,int screenCapacity=1<<16){ArgumentNullException.ThrowIfNull(device);world=new(device,shaders,output,worldCapacity);screen=new(device,shaders,output,screenCapacity);}
...
public void Upload(DebugDraw draw,in Matrix4x4 view,Vector2 viewport){ArgumentNullException.ThrowIfNull(draw);ObjectDisposedException.ThrowIf(disposed,this);geometry.Build(draw,DebugView.FromView(view));world.Upload(geometry.World);screen.Upload(geometry.Screen);hasScreen=viewport.X>0f&&viewport.Y>0f;if(hasScreen){screenProjection=ScreenProjection(viewport);}}
/// <summary>Draws what the last upload wrote.</summary>
    ...
    /// </remarks>
public void Record(ICommandList commands,in Matrix4x4 viewProjection){ArgumentNullException.ThrowIfNull(commands);ObjectDisposedException.ThrowIf(disposed,this);Draws=0;world.Record(commands,viewProjection,DepthTested);Draws+=world.Draws;if(hasScreen){screen.Record(commands,screenProjection,depthTested:false);Draws+=screen.Draws;}}
/// <summary>Draws only the screen-space half — the overlays, with no camera involved.</summary>
    ...
    /// </remarks>
public void RecordScreen(ICommandList commands){ArgumentNullException.ThrowIfNull(commands);ObjectDisposedException.ThrowIf(disposed,this);Draws=0;if(hasScreen){screen.Record(commands,screenProjection,depthTested:false);Draws=screen.Draws;}}
/// <inheritdoc />
public void Dispose(){if(disposed){return;}disposed=true;world.Dispose();screen.Dispose();}
/// <summary>Pixels with y running down, onto clip space with y running up.</summary>
    ...
    ///     </para>
    /// </remarks>
static Matrix4x4 ScreenProjection(Vector2 viewport)=>new(2f/viewport.X,0f,0f,0f,0f,-2f/viewport.Y,0f,0f,0f,0f,0f,0f,-1f,1f,0.5f,1f);}
