...
using System.Runtime.InteropServices;namespace Vixen.Platform.Native;
...
/// <summary>The chain for a runtime identifier other than this process's.</summary>
    ...
    /// </remarks>
public static IReadOnlyList<string>ChainFor(string rid){ArgumentException.ThrowIfNullOrEmpty(rid);var separator=rid.LastIndexOf('-');return separator<=0?[rid]:[rid,rid[ .. separator]];}static string OperatingSystemPart()=>OperatingSystem.IsWindows()?"win":OperatingSystem.IsMacOS()||OperatingSystem.IsMacCatalyst()?"osx":OperatingSystem.IsIOS()?"ios":OperatingSystem.IsAndroid()?"android" :OperatingSystem.IsBrowser()?"browser" :"linux" ;static string ArchitecturePart()=>RuntimeInformation.ProcessArchitecture switch{Architecture.X64=>"x64",Architecture.X86=>"x86",Architecture.Arm64=>"arm64" ,Architecture.Arm=>"arm",Architecture.Wasm=>"wasm",var other=>other.ToString().ToLowerInvariant()};}
