...
///     </para>
/// </remarks>
sealed class GlBindingPlan{readonly Dictionary<(DescriptorSetSlot Slot,uint Binding),GlBindingSlot>slots=[];GlBindingPlan(int pushConstantVectors)=>PushConstantVectors=pushConstantVectors;
...
/// <summary>Builds the plan for a pipeline layout.</summary>
    ...
    ///     caller-dependent.
    /// </remarks>
public static GlBindingPlan Build(IReadOnlyList<(DescriptorSetSlot Slot,DescriptorBinding[]Bindings,string Name)>sets,int pushConstantBytes){var plan=new GlBindingPlan((pushConstantBytes+15)/16);uint uniforms=0,storages=0,textures=0,images=0,samplers=0;foreach(var set in sets.OrderBy(entry=>entry.Slot)){foreach(var binding in(set.Bindings??[]).OrderBy(entry=>entry.Binding)){ // An array binding takes a contiguous run, because that is what GLSL's
// `uniform sampler2D atlas[4]` occupies: units n..n+3, consecutively.
var count=(uint)Math.Max(1,binding.Count);var index=binding.Kind switch{DescriptorKind.UniformBuffer or DescriptorKind.DynamicUniformBuffer=>Take(ref uniforms,count),DescriptorKind.StorageBuffer or DescriptorKind.DynamicStorageBuffer=>Take(ref storages,count),DescriptorKind.SampledTexture=>Take(ref textures,count),DescriptorKind.StorageTexture=>Take(ref images,count),DescriptorKind.Sampler=>Take(ref samplers,count), // Named rather than left to the catch-all: a layout carrying one is a caller
...
DescriptorKind.AccelerationStructure=>throw new NotSupportedException("A descriptor set layout declares an acceleration structure on the OpenGL " +"backend, which has no ray tracing. Ask Features.HasRayTracing and take " +"the distance-field tracer." ),_=>throw new ArgumentOutOfRangeException(nameof(sets),binding.Kind,"This descriptor kind has no OpenGL binding class." )};plan.slots[(set.Slot,binding.Binding)]=new(binding.Kind,index);}}return plan;static uint Take(ref uint counter,uint count){var first=counter;counter+=count;return first;}}
/// <summary>Where a binding lives, or <see langword="null" /> if the layout does not declare it.</summary>
    ...
    /// </remarks>
public GlBindingSlot?Resolve(DescriptorSetSlot slot,uint binding)=>slots.TryGetValue((slot,binding),out var found)?found:null;}
