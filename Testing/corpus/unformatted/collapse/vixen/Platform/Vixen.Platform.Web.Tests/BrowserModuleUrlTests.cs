...
using System;using System.IO;using Vixen.Audio.Backend.WebAudio;using Vixen.Graphics.WebGPU.Browser;using Xunit;namespace Vixen.Platform.Web.Tests;
/// <summary>
...
public static TheoryData<string,string,string>Bindings=>new(){{"Vixen.Platform.Web" ,WebInterop.ModuleName,WebInterop.DefaultModuleUrl},{"Vixen.Audio.Backend.WebAudio" ,WebAudioInterop.ModuleName,WebAudioInterop.DefaultModuleUrl},{"Vixen.Graphics.WebGPU.Browser" ,WebGpuInterop.ModuleName,WebGpuInterop.DefaultModuleUrl}};
/// <summary>
    ...
    /// </remarks>
[Fact]public void ASingleDotResolvesIntoTheRuntimeDirectory(){var resolved=new Uri(RuntimeModule,"./vixen-platform.js" );Assert.Equal("/_framework/vixen-platform.js" ,resolved.AbsolutePath);}
/// <summary>
    ///     Each default module URL, resolved the way the browser will resolve it, names a file at the
    ///     site root.
    /// </summary>
[Theory][MemberData(nameof(Bindings))]public void TheDefaultModuleUrlResolvesToTheSiteRoot(string project,string moduleName,string moduleUrl){var resolved=new Uri(RuntimeModule,moduleUrl); // First, and separately, because this is the failure that has actually happened and it
// deserves the sentence rather than a string diff.
Assert.False(resolved.AbsolutePath.StartsWith("/_framework/" ,StringComparison.Ordinal),$"{project}'s default module URL '{moduleUrl}' resolves to '{resolved.AbsolutePath}'. " +"JSHost.ImportAsync resolves against the runtime's module in _framework/, and " +$"{moduleName}.js is a content file at the site root, so this fetches nothing and " +"surfaces as 'TypeError: Failed to fetch dynamically imported module' from inside " +"CreateAsync. It wants ../ and not ./ — see docs/plan/spikes/web-head/RESULT.md." ); // And then the whole of it: the right directory and the file the module name implies.
...
/// <summary>
    ...
    /// </remarks>
[Theory][MemberData(nameof(Bindings))]public void TheFileTheDefaultModuleUrlNamesIsShipped(string project,string moduleName,string moduleUrl){var fileName=Path.GetFileName(new Uri(RuntimeModule,moduleUrl).AbsolutePath);var shipped=Path.Combine(RepositoryRoot(),"Platform" ,project,"wwwroot" ,fileName);Assert.True(File.Exists(shipped),$"{project}'s default module URL '{moduleUrl}' names {fileName}, and there is no such " +$"file at '{shipped}'. The module name is '{moduleName}'; one of the two moved." );}
/// <summary>
    ...
    ///     </para>
    /// </remarks>
[Theory][MemberData(nameof(Bindings))]public void TheProjectPacksThatFileForTheSiteRoot(string project,string moduleName,string moduleUrl){var fileName=Path.GetFileName(new Uri(RuntimeModule,moduleUrl).AbsolutePath);var projectFile=Path.Combine(RepositoryRoot(),"Platform" ,project,$"{project}.csproj" );var text=File.ReadAllText(projectFile);var item=text.IndexOf($"wwwroot\\{fileName}" ,StringComparison.Ordinal);Assert.True(item>=0,$"{project}.csproj does not ship wwwroot\\{fileName}, which its module '{moduleName}' " +"is imported from. Nothing would copy it, and the import would 404." );var declaration=text[item .. ];var end=declaration.IndexOf("/>",StringComparison.Ordinal);Assert.True(end>=0,$"the <None> item for {fileName} in {project}.csproj is not closed." );declaration=declaration[ .. end];Assert.True(declaration.Contains("contentFiles/any/any/" ,StringComparison.Ordinal),$"{project}.csproj ships wwwroot\\{fileName} but does not pack it to " +"contentFiles/any/any/, which is what puts it at the root of a consuming head's " +$"content. Its module URL '{moduleUrl}' is written for that layout, so wherever this " +$"lands instead, the import will 404. The declaration reads:{Environment.NewLine}" +declaration); // The value and not merely the attribute: CopyToOutputDirectory="Never" is spelled the same
...
Assert.True(declaration.Contains("CopyToOutputDirectory" ,StringComparison.Ordinal)&&!declaration.Contains("CopyToOutputDirectory=\"Never\"" ,StringComparison.Ordinal),$"{project}.csproj does not copy wwwroot\\{fileName} to its output, so a project " +$"reference — which is how the samples and the spike head consume it — gets no " +$"{fileName} at all. The declaration reads:{Environment.NewLine}{declaration}" );}
/// <summary>The repository root, found by walking up rather than by counting directories.</summary>
static string RepositoryRoot(){for(var directory=new DirectoryInfo(AppContext.BaseDirectory);directory is not null;directory=directory.Parent){if(File.Exists(Path.Combine(directory.FullName,"Vixen.slnx" ))){return directory.FullName;}}throw new DirectoryNotFoundException($"the repository root was not found above '{AppContext.BaseDirectory}'." );}}
