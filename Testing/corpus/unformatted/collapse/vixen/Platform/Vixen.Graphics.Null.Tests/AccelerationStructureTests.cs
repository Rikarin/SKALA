...
using Xunit; namespace Vixen.Graphics.Null.Tests; public sealed class AccelerationStructureTests : IDisposable {
    ...
    public void AnInventedZeroSizeIsRefused() {
        Assert.Throws<ArgumentOutOfRangeException>(() => device.CreateAccelerationStructure(
                new(AccelerationStructureKind.BottomLevel, 0, "Empty")
            )
        );
    }
    ...
    static AccelerationStructureBuildInput BottomLevelInput(NullDevice on, int indexCount) {
        ...
        return new(AccelerationStructureKind.BottomLevel, new(vertices, 0, 24, 12, indices, 0, indexCount));
    }
