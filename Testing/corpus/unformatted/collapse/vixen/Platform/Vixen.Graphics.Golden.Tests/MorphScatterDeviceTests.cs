...
public class MorphScatterDeviceTests {
    ...
    static MorphTargetData[] Targets() {
        ...
        return [
            ...
            ), MorphTargetData.Encode("browRaise", brow, [.. brow.Select(v => Exact(-8192, 32767, v * -5))], []),
            MorphTargetData.Encode(
                "crease",
                crease,
    ...
    static SurfaceVertex[] Scatter(
        ...
        var readback = device.CreateBuffer(
            new(meshBytes.Length, BufferUsage.CopyDestination, MemoryAccess.HostReadback, "morph readback")
        );
        ...
        for (var index = 0; index < targets.Length; index++) {
            ...
            var entries = device.CreateBuffer(
                new(entryBytes.Length, BufferUsage.Storage, MemoryAccess.HostUpload, $"morph entries {target.Name}")
            );
            ...
            var uniforms = device.CreateBuffer(
                new(constants.Length, BufferUsage.Uniform, MemoryAccess.HostUpload, $"morph constants {target.Name}")
            );
