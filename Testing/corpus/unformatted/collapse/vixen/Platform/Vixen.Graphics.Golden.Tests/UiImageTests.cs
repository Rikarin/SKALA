...
public sealed class UiImageTests {
    ...
    public void Interface() {
        ...
        owned.Graph.AddPass(
            "ui",
            pass => { pass.ColourAttachment(
                    colour,
                    LoadAction.Clear,
                    new(0.08f, 0.09f, 0.11f, 1f)
                ); pass.SideEffect(); pass.Execute(context => renderer.Record(
                        context.CommandList,
                        geometry,
                        new(Side, Side)
                    )
    ...
    public void Shadowed() {
        ...
        list.BeginFrame(); // A shadow the size of the card, pushed down and right, then the card on top of it.
    ...
    public void Regrown() {
        ...
        var image = owned.Render(
            colour,
            commands => { renderer.Upload(commands, small, cache.Atlas); renderer.Upload(
                    commands,
                    large,
                    cache.Atlas
                ); }
    ...
    static UiGeometry Paint(GlyphFieldCache cache) {
        ...
        list.Add(
            new(DrawCommandKind.Border, 70, 8, 50, 40, new Color4(0.95f, 0.45f, 0.2f, 1f), 10, 4)
    ...
    static void AssertTheShadowFadesOutwards(in Bitmap image) {
        ...
        for (var y = 74; y < 108; y++) {
            ...
            if (previous >= 0) {
                Assert.True(value >= previous - 2, $"the shadow darkens again at y = {y}: {previous} then {value}");
            }
    ...
    static FontFace Font() {
        ...
        using var stream = typeof(UiImageTests).Assembly.GetManifestResourceStream(
            "Vixen.Graphics.Golden.Tests.TestShapeLana.ttf"
        )
    ...
    public void Images() {
        ...
        var list = new DrawList();
        list.BeginFrame(); // A box first, so the image's set has something before it to disturb.
        list.Add(
            new(DrawCommandKind.Rectangle, 8, 8, 40, 40, new Color4(0.2f, 0.2f, 0.25f, 1f), 6, 0)
        ...
        var image = owned.Render(
            colour,
            commands => { renderer.Upload(commands, geometry, cache.Atlas); commands.Barrier(
    ...
    static void AssertTheImageIsUpright(in Bitmap image) {
        Assert.True(Red(image, 64, 16) > Blue(image, 64, 16), "the top-left of the upright image is not reddish");
        Assert.True(Blue(image, 64, 48) > Red(image, 64, 48), "the bottom-left of the upright image is not blueish");
    }
    ...
    static void AssertTheFlippedImageIsFlipped(in Bitmap image) {
        ...
        Assert.True(Red(image, 64, 104) > Blue(image, 64, 104), "the bottom-left of the flipped image is not reddish");
    }
