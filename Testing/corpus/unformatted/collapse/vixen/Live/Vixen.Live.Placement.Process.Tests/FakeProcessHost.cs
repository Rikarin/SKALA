...
using System.Collections.Concurrent;using System.Globalization;using Xunit;namespace Vixen.Live.Placement.Tests;
...
/// </remarks>
sealed class FakeProcessHost:IRealmProcessHost{readonly ConcurrentQueue<FakeProcess>started=new();int nextId=1000;
...
public Exception?Refuse{get;set;}public IRealmProcessHandle Start(RealmProcessRequest request){if(Refuse is{}failure){throw failure;}var process=new FakeProcess(Interlocked.Increment(ref nextId).ToString(CultureInfo.InvariantCulture),request);started.Enqueue(process);return process;}}
/// <summary>A realm process that is a few fields and a <see cref="TaskCompletionSource" />.</summary>
sealed class FakeProcess(string id,RealmProcessRequest request):IRealmProcessHandle{readonly TaskCompletionSource exited=new(TaskCreationOptions.RunContinuationsAsynchronously);readonly ConcurrentQueue<string>input=new();public string Id{get;}=id;
...
public IReadOnlyCollection<string>Input=>input;public bool HasExited{get;private set;}public int ExitCode{get;private set;}
/// <summary>Whether the launcher ran out of patience.</summary>
public bool WasKilled{get;private set;}public event Action<string>?OutputLine;
/// <summary>The spec it was told to be, decoded off its own command line.</summary>
public RealmSpec Spec{get{Assert.True(RealmSpec.TryRead(Request.Arguments,_=>null,out var spec,out var error),error);return spec!;}}
...
public void Exit(int code=0){if(HasExited){return;}ExitCode=code;HasExited=true;exited.TrySetResult();}public bool WriteLine(string line){if(HasExited){return false;}input.Enqueue(line);return true;}public void Kill(){WasKilled=true;Exit(code:137);}public Task WaitForExitAsync(CancellationToken cancellation)=>exited.Task.WaitAsync(cancellation);public void Dispose(){}}
