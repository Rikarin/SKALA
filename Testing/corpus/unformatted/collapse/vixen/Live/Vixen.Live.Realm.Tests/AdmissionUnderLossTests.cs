...
public class AdmissionUnderLossTests {
    ...
    static NetworkSimulationProfile Profile(string name) =>
        name switch {
            ...
            "Awful" => NetworkSimulationProfile.Awful, // ⚠ Not a shipped profile, and it is here because duplication is the failure mode
    ...
    public void ATicketedClientIsStillAdmitted(string wire) {
        ...
        realm.Connect(ticket); // Generous, and deliberately not tuned: the point is that it gets there, not how fast.
        realm.Pump(400);
