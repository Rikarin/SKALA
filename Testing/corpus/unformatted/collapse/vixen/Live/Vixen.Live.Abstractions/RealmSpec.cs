...
public sealed record RealmSpec {
    ...
    public bool IsValid =>
        Shard.IsValid && Key.IsValid && Capacity.IsValid && TickRate > 0 && (Endpoint.IsValid || Endpoint.IsUnbound);
    ...
    static bool SameOptions(IReadOnlyDictionary<string, string> left, IReadOnlyDictionary<string, string> right) {
