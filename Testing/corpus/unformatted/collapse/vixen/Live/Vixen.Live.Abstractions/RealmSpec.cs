...
using System.Globalization;using System.Text;namespace Vixen.Live;
...
public bool IsValid=>Shard.IsValid&&Key.IsValid&&Capacity.IsValid&&TickRate>0&&(Endpoint.IsValid||Endpoint.IsUnbound);
/// <summary>The spec as the one string that crosses a process boundary.</summary>
    /// <returns>What <see cref="TryDecode" /> reads.</returns>
public string Encode(){var text=new StringBuilder(160);KeyValueText.Write(text,"shard" ,Shard.Value.ToString("D",CultureInfo.InvariantCulture));KeyValueText.Write(text,"map",Key.Map);KeyValueText.Write(text,"region" ,Key.Region);KeyValueText.Write(text,"build" ,Key.Version.Build);KeyValueText.Write(text,"content" ,Key.Version.Content.ToString("x16",CultureInfo.InvariantCulture));KeyValueText.Write(text,"kind",Kind.ToString());KeyValueText.Write(text,"host",Endpoint.Host);KeyValueText.Write(text,"port",Endpoint.Port.ToString(CultureInfo.InvariantCulture));KeyValueText.Write(text,"soft",Capacity.SoftCap.ToString(CultureInfo.InvariantCulture));KeyValueText.Write(text,"hard",Capacity.HardCap.ToString(CultureInfo.InvariantCulture));KeyValueText.Write(text,"tick",TickRate.ToString(CultureInfo.InvariantCulture));KeyValueText.Write(text,"seed",Seed.ToString(CultureInfo.InvariantCulture));if(ClusterEndpoint.Length>0){KeyValueText.Write(text,"cluster" ,ClusterEndpoint);}foreach(var option in Options){ // Prefixed, so a game's option can never collide with a field this record grows later —
// and so a reader can tell at a glance which half of the string is the engine's.
KeyValueText.Write(text,"x-"+option.Key,option.Value);}return text.ToString();}
...
public static bool TryDecode(string?text,out RealmSpec?spec,out string error){spec=null;if(!KeyValueText.TryRead(text,out var fields,out error)){return false;}if(!ShardId.TryParse(Field(fields,"shard" ),out var shard)||!shard.IsValid){error="`shard` is missing or is not a guid" ;return false;}var map=Field(fields,"map");if(map.Length==0){error="`map` is missing" ;return false;}if(!ulong.TryParse(Field(fields,"content" ),NumberStyles.HexNumber,CultureInfo.InvariantCulture,out var content)){error="`content` is missing or is not a 64-bit hexadecimal hash" ;return false;}if(!Enum.TryParse<ShardKind>(Field(fields,"kind"),ignoreCase:false,out var kind)){error="`kind` is missing or is not one of " +string.Join(", ",Enum.GetNames<ShardKind>());return false;}if(!int.TryParse(Field(fields,"port"),CultureInfo.InvariantCulture,out var port)){error="`port` is missing or is not a number" ;return false;}var endpoint=new RealmEndpoint(Field(fields,"host"),port);if(!endpoint.IsValid&&!endpoint.IsUnbound){error=$"`{endpoint}` is not an endpoint a client could be sent to" ;return false;}if(!int.TryParse(Field(fields,"soft"),CultureInfo.InvariantCulture,out var soft)||!int.TryParse(Field(fields,"hard"),CultureInfo.InvariantCulture,out var hard)){error="`soft` and `hard` are missing or are not numbers" ;return false;}var capacity=new ShardCapacity(soft,hard);if(!capacity.IsValid){error=$"a capacity of {capacity} is not one a shard could honour" ;return false;}if(!int.TryParse(Field(fields,"tick"),CultureInfo.InvariantCulture,out var tick)||tick<=0){error="`tick` is missing or is not a positive number" ;return false;}if(!ulong.TryParse(Field(fields,"seed"),CultureInfo.InvariantCulture,out var seed)){error="`seed` is missing or is not a number" ;return false;}var options=new Dictionary<string,string>(StringComparer.Ordinal);foreach(var field in fields){if(field.Key.StartsWith("x-",StringComparison.Ordinal)){options[field.Key[2 .. ]]=field.Value;}}spec=new(){Shard=shard,Key=new(map,Field(fields,"region" ),new(Field(fields,"build" ),content)),Kind=kind,Endpoint=endpoint,Capacity=capacity,TickRate=tick,Seed=seed,ClusterEndpoint=Field(fields,"cluster" ),Options=options};return true;}
...
/// <summary>Finds the spec a realm process was started with.</summary>
    ...
    /// </remarks>
public static bool TryRead(IReadOnlyList<string>?arguments,Func<string,string?>?environment,out RealmSpec?spec,out string error){spec=null;if(arguments is not null){for(var index=0;index<arguments.Count-1;index++){if(string.Equals(arguments[index],ArgumentName,StringComparison.Ordinal)){return TryDecode(arguments[index+1],out spec,out error);}}}var read=environment??Environment.GetEnvironmentVariable;var fromEnvironment=read(EnvironmentVariable);if(!string.IsNullOrEmpty(fromEnvironment)){return TryDecode(fromEnvironment,out spec,out error);}error=$"no {ArgumentName} argument and no {EnvironmentVariable} environment variable" ;return false;}
/// <summary>Whether two specs say the same thing, options included.</summary>
    ...
    /// </remarks>
public bool Equals(RealmSpec?other)=>other is not null&&Shard==other.Shard&&Key==other.Key&&Kind==other.Kind&&Endpoint==other.Endpoint&&Capacity==other.Capacity&&TickRate==other.TickRate&&Seed==other.Seed&&string.Equals(ClusterEndpoint,other.ClusterEndpoint,StringComparison.Ordinal)&&SameOptions(Options,other.Options);
/// <inheritdoc />
public override int GetHashCode()=>HashCode.Combine(Shard,Key,Kind,Endpoint,Capacity,TickRate,Seed,Options.Count);
/// <inheritdoc />
public override string ToString()=>string.Create(CultureInfo.InvariantCulture,$"{Kind} shard {Shard} of {Key} at {Endpoint}" );static bool SameOptions(IReadOnlyDictionary<string,string>left,IReadOnlyDictionary<string,string>right){if(left.Count!=right.Count){return false;}foreach(var option in left){if(!right.TryGetValue(option.Key,out var value)||!string.Equals(option.Value,value,StringComparison.Ordinal)){return false;}}return true;}static string Field(Dictionary<string,string>fields,string key)=>fields.GetValueOrDefault(key,"");}
