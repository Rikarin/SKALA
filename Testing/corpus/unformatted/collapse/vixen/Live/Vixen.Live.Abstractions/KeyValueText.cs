...
static class KeyValueText {
    ...
    public static bool TryRead(string? text, out Dictionary<string, string> fields, out string error) {
    ...
    static void Escape(StringBuilder text, string value) {
        foreach (var character in value) {
            switch (character) { case '%': text.Append("%25"); break; case ';': text.Append("%3B"); break; case '=':
                    text.Append("%3D");
                    break; default: text.Append(character); break; }
        }
    }
