...
public sealed partial class Lowerer {
    ...
    IrType LowerTypeCore(TypeSymbol type, SyntaxNode? syntax) {
        ...
        switch (type) { case PrimitiveTypeSymbol primitive:
                return LowerPrimitive(primitive, syntax); case BuiltInNamedTypeSymbol builtIn:
            ...
            case StorageImageTypeSymbol: return NotRepresentable(type, syntax); case NamedTypeSymbol {
                TypeKind: TypeKind.Enum
            }: // An enum is its underlying integer once the constants are folded.
                return IrScalarType.Int; case NamedTypeSymbol named when structs.TryGetValue(named, out var structType):
                return structType; case TupleTypeSymbol tuple: return LowerTuple(tuple, syntax); default:
                return NotRepresentable(type, syntax); }
    }

    IrType LowerPrimitive(PrimitiveTypeSymbol type, SyntaxNode? syntax) {
        switch (type.TypeKind) { case TypeKind.Void: return IrScalarType.Void; case TypeKind.Scalar:
                return LowerScalar(type.SpecialType) ?? NotRepresentable(type, syntax); case TypeKind.Vector: {
