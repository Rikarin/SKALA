...
using Vixen.StyleGen; // The whole of the entry point. Everything worth testing is in `Arguments` and `StyleGenRunner`,
...
var request=Arguments.Parse(args,out var problem);if(request is null){ // ⚠ MSBuild's canonical diagnostic shape — `subcategory code: text` on standard error — so that
...
// msbuild`.
Console.Error.WriteLine($"Vixen.StyleGen : error VXSTYLE001: {problem}" );return 2;}var result=StyleGenRunner.Run(request);foreach(var error in result.Errors){Console.Error.WriteLine($"Vixen.StyleGen : error VXSTYLE002: {error}" );}if(result.Errors.Count>0){ // ⚠ Nothing is written on failure, and it matters which way round that is. A half-written
...
return 1;}StyleGenRunner.Write(request,result); // ⚠ Two counts and not two lists, and the second count is the one that was missing. A build line
...
// a broken build.
Console.WriteLine($"Vixen.StyleGen: {result.RuleCount} utility rules from {request.Scan.Count} scanned files" +$" ({result.Unresolved.Count} named a registered family and emitted nothing," +$" {result.Unrecognised.Count} matched no family at all)." );return 0;
