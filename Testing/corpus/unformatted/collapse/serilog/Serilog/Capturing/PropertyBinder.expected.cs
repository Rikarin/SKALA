...
class PropertyBinder {
...
#endif
    {
        ...
        foreach (var property in positionalProperties) {
            if (property.TryGetPositionalValue(out var position)) {
                ...
                else result[position] = ConstructProperty(property, messageTemplateParameters[position]);
            }
        }
    ...
    EventProperty ConstructProperty(PropertyToken propertyToken, object? value) {
        return new(propertyToken.PropertyName, _valueConverter.CreatePropertyValue(value, propertyToken.Destructuring));
    }
}
