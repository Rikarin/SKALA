...
public class LoggerEnrichmentConfiguration {
    ...
    public static LoggerConfiguration Wrap(
        ...
        var capturingLoggerEnrichmentConfiguration =
            new LoggerEnrichmentConfiguration(
...
// to the capturing enrichment configuration, enabling `Enrich.WithX().Enrich.WithY()`.
        ...
        if (enrichersToWrap.Count == 0) return loggerEnrichmentConfiguration._loggerConfiguration;
        ...
        var wrappedEnricher = wrapEnricher(enclosed); // ReSharper disable once SuspiciousTypeConversion.Global
