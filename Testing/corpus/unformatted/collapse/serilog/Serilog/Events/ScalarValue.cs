...
public override void Render(TextWriter output,string?format=null,IFormatProvider?formatProvider=null){Render(Value,output,format,formatProvider);}
/// <exception cref="ArgumentNullException">When <paramref name="output"/> is <code>null</code></exception>
internal static void Render(object?value,TextWriter output,string?format=null,IFormatProvider?formatProvider=null){Guard.AgainstNull(output);if(value==null){output.Write("null");return;}if(value is string s){if(format!="l"){output.Write('"');output.Write(s.Replace("\"","\\\""));output.Write('"');}else{output.Write(s);}return;}var custom=(ICustomFormatter?)formatProvider?.GetFormat(typeof(ICustomFormatter));if(custom!=null){output.Write(custom.Format(format,value,formatProvider));return;}if(value is IFormattable f){output.Write(f.ToString(format,formatProvider??CultureInfo.InvariantCulture));}else{output.Write(value.ToString());}}
#if FEATURE_SPAN
...
public override bool Equals(object?obj){return obj is ScalarValue sv&&Equals(Value,sv.Value);}
/// <summary>
    /// Get a hash code representing the value.
    /// </summary>
    /// <returns>The instance's hash code.</returns>
public override int GetHashCode(){if(Value==null)return 0;return Value.GetHashCode();}}
