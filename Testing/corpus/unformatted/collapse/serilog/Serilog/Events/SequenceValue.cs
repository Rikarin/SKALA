...
/// </summary>
public class SequenceValue:LogEventPropertyValue{readonly LogEventPropertyValue[]_elements;
/// <summary>
...
public SequenceValue(IEnumerable<LogEventPropertyValue>elements){Guard.AgainstNull(elements);_elements=elements.ToArray();}internal SequenceValue(LogEventPropertyValue[]elements){Guard.AgainstNull(elements);_elements=elements;}
/// <summary>
...
public override void Render(TextWriter output,string?format=null,IFormatProvider?formatProvider=null){Guard.AgainstNull(output);output.Write('[');var allButLast=_elements.Length-1;for(var i=0;i<allButLast;++i){_elements[i].Render(output,format,formatProvider);output.Write(", ");}if(_elements.Length>0)_elements[_elements.Length-1].Render(output,format,formatProvider);output.Write(']');}}
