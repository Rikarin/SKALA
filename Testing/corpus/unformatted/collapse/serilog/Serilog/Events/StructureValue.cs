...
/// </summary>
public class StructureValue:LogEventPropertyValue{readonly LogEventProperty[]_properties;
/// <summary>
...
public StructureValue(IEnumerable<LogEventProperty>properties,string?typeTag=null){Guard.AgainstNull(properties);TypeTag=typeTag;_properties=properties.ToArray();}internal StructureValue(LogEventProperty[]properties,string?typeTag){TypeTag=typeTag;_properties=properties;}
/// <summary>
...
public override void Render(TextWriter output,string?format=null,IFormatProvider?formatProvider=null){Guard.AgainstNull(output);if(TypeTag!=null){output.Write(TypeTag);output.Write(' ');}output.Write("{ ");var allButLast=_properties.Length-1;for(var i=0;i<allButLast;i++){var property=_properties[i];Render(output,property,formatProvider);output.Write(", ");}if(_properties.Length>0){var last=_properties[_properties.Length-1];Render(output,last,formatProvider);}output.Write(" }");}static void Render(TextWriter output,LogEventProperty property,IFormatProvider?formatProvider=null){output.Write(property.Name);output.Write(": ");property.Value.Render(output,null,formatProvider);}}
