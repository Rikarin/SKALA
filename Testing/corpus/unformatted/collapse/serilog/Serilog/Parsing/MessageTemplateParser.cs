...
public class MessageTemplateParser : IMessageTemplateParser {
    ...
    IEnumerable<MessageTemplateToken> Tokenize(string messageTemplate) {
        ...
        while (true) {
            ...
            if (nextIndex > beforeText) yield return tt;
            ...
            if (beforeProp < nextIndex) yield return pt;
            if (nextIndex == messageTemplate.Length) yield break;
        }
    }
    ...
    MessageTemplateToken ParsePropertyToken(int startAt, string messageTemplate, out int next) {
        ...
        if (tagContent.Length == 0) return new TextToken(rawText);
        ...
        if (char.IsDigit(propertyName[0])) {
            ...
            for (var i = 0; i < propertyName.Length; ++i) {
                var c = propertyName[i];
                if (!TryContinuePropertyName(c, ref beginIdent)) return new TextToken(rawText);
            }
        ...
        return new PropertyToken(propertyName, rawText, format, alignmentValue, destructuring);
    }

    static bool TrySplitTagContent(
        string tagContent,
        [NotNullWhen(true)] out string? propertyNameAndDestructuring,
        out string? format,
    ...
    static bool TryGetDestructuringHint(char c, out Destructuring destructuring) {
        switch (c) { case '@': destructuring = Destructuring.Destructure; return true; case '$':
                destructuring = Destructuring.Stringify;
                return true; default: destructuring = Destructuring.Default; return false; }
    }
    ...
    static TextToken ParseTextToken(int startAt, string messageTemplate, out int next) {
        ...
        ++i; // The character must be either `{` or `}`, since we found its index.
        if (ch == '{') {
