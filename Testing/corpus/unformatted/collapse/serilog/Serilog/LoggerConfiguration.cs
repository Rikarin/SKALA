...
/// </summary>
public class LoggerConfiguration{readonly List<ILogEventSink>_logEventSinks=[];readonly List<ILogEventSink>_auditSinks=[];readonly List<ILogEventEnricher>_enrichers=[];readonly List<ILogEventFilter>_filters=[];readonly List<Type>_additionalScalarTypes=[];readonly HashSet<Type>_additionalDictionaryTypes=[];readonly List<IDestructuringPolicy>_additionalDestructuringPolicies=[];readonly Dictionary<string,LoggingLevelSwitch>_overrides=new();LogEventLevel _minimumLevel=LogEventLevel.Information;LoggingLevelSwitch?_levelSwitch;int _maximumDestructuringDepth=10;int _maximumStringLength=int.MaxValue;int _maximumCollectionCount=int.MaxValue;bool _loggerCreated;
/// <summary>
    /// Construct a <see cref="LoggerConfiguration"/>.
    /// </summary>
public LoggerConfiguration(){WriteTo=new(this,s=>_logEventSinks.Add(s));Enrich=new(this,e=>_enrichers.Add(e));}
/// <summary>
...
public LoggerMinimumLevelConfiguration MinimumLevel{get{return new(this,l=>{_minimumLevel=l;_levelSwitch=null;},sw=>_levelSwitch=sw,(s,lls)=>_overrides[s]=lls);}}
/// <summary>
...
/// <summary>
    ...
    /// </summary>
public LoggerDestructuringConfiguration Destructure{get{return new(this,_additionalScalarTypes.Add,type=>_additionalDictionaryTypes.Add(type),_additionalDestructuringPolicies.Add,depth=>_maximumDestructuringDepth=depth,length=>_maximumStringLength=length,count=>_maximumCollectionCount=count);}}
/// <summary>
...
public Logger CreateLogger(){if(_loggerCreated)throw new InvalidOperationException("CreateLogger() was previously called and can only be called once." );_loggerCreated=true;ILogEventSink?sink=null;if(_logEventSinks.Count>0)sink=new SafeAggregateSink(_logEventSinks);var auditing=_auditSinks.Any();if(auditing){sink=new AggregateSink(sink==null?_auditSinks:new[]{sink}.Concat(_auditSinks));}sink??=new SafeAggregateSink(Array.Empty<ILogEventSink>());if(_filters.Any()){ // A throwing filter could drop an auditable event, so exceptions in filters must be propagated
// if auditing is used.
sink=new FilteringSink(sink,_filters,auditing);}var converter=new PropertyValueConverter(_maximumDestructuringDepth,_maximumStringLength,_maximumCollectionCount,_additionalScalarTypes,_additionalDictionaryTypes,_additionalDestructuringPolicies,auditing);var processor=new MessageTemplateProcessor(converter);var enricher=_enrichers.Count switch{ // Should be a rare case, so no problem making that extra interface dispatch.
0=>new EmptyEnricher(),1=>_enrichers[0], // Enrichment failures are not considered blocking for auditing purposes.
_=>new SafeAggregateEnricher(_enrichers)};LevelOverrideMap?overrideMap=null;if(_overrides.Count!=0){overrideMap=new(_overrides,_minimumLevel,_levelSwitch);}var disposableSinks=_logEventSinks.Concat(_auditSinks).Where(s=>s is IDisposable
#if FEATURE_ASYNCDISPOSABLE
                or IAsyncDisposable
#endif
).ToArray();void Dispose(){foreach(var disposable in disposableSinks){(disposable as IDisposable)?.Dispose();}}
#if FEATURE_ASYNCDISPOSABLE
...
#endif
SelfMetrics.PipelineCreated.Add(1);return new(processor,_levelSwitch!=null?LevelAlias.Minimum:_minimumLevel,_levelSwitch,sink,enricher,Dispose,
#if FEATURE_ASYNCDISPOSABLE
            DisposeAsync,
#endif
overrideMap);}}
