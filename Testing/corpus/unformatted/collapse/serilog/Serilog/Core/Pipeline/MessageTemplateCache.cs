...
namespace Serilog.Core.Pipeline; class MessageTemplateCache : IMessageTemplateParser {
    readonly IMessageTemplateParser _innerParser;
    readonly object _templatesLock = new(); // Hashtable is used, instead of Dictionary<,> for thread safety reasons.
    //
    ...
    public MessageTemplate Parse(string messageTemplate) {
        ...
        if (messageTemplate.Length > MaxCachedTemplateLength)
            return _innerParser.Parse(messageTemplate); // ReSharper disable once InconsistentlySynchronizedField
