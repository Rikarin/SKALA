...
#endif
{
    ...
    readonly CancellationTokenSource _shutdownSignal =
        new(); // The write side can wait on this to ensure shutdown has completed.
    ...
    public BatchingSink(IBatchedLogEventSink batchedSink, BatchingOptions options) {
        ...
        if (options.BufferingTimeLimit <= TimeSpan.Zero)
            throw new ArgumentOutOfRangeException(
                nameof(options),
                "The buffering time limit must be greater than zero."
            );
    ...
    public void Emit(LogEvent logEvent) {
        ...
        if (_shutdownSignal.IsCancellationRequested) return;
            ...
            do {
                while (_currentBatch.Count < _batchSizeLimit
                    ...
                    _currentBatch.Enqueue(next);
                     ...
                     && !_shutdownSignal.IsCancellationRequested
                     && await TryWaitToReadAsync(_queue.Reader, fillBatch, _shutdownSignal.Token).ConfigureAwait(
                         false
                     ));
            try {
    ...
    void DrainOnFailure(
        ...
        var buffer = new List<LogEvent>(); // Not ideal, uses some CPU capacity unnecessarily and doesn't complete in bounded time. The goal is
    ...
    async Task<bool> TryWaitToReadAsync(
        ...
        var completed = await Task.WhenAny(timeout, waitToRead)
            .ConfigureAwait(
                false
