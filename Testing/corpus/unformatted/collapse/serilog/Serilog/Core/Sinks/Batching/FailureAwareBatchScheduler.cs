...
class FailureAwareBatchScheduler {
    ...
    public FailureAwareBatchScheduler(TimeSpan bufferingTimeLimit, TimeSpan retryTimeLimit) : this(
        bufferingTimeLimit,
        retryTimeLimit,
        TimeProvider.System
    ) { }
    ...
    public void MarkFailure(out bool shouldDropBatch, out bool shouldDropQueue) {
        ...
        _firstFailureTimestamp ??= _timeProvider.GetTimestamp(); // Once we're up against the time limit, we'll try each subsequent batch once and then drop it.
