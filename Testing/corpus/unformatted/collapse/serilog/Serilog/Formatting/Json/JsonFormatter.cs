    ...
    public void Format(LogEvent logEvent, TextWriter output) {
        ...
        if (logEvent.Properties.Count != 0) {
            ...
            foreach (var property in logEvent.Properties) {
                if (propertyDelimiter != null) output.Write(propertyDelimiter.Value); else propertyDelimiter = ',';
    ...
    void WriteRenderingsValues(
        ...
        foreach (var propertyFormats in tokensWithFormat) {
            ...
            foreach (var format in propertyFormats) {
                if (formatDelimiter != null) output.Write(formatDelimiter.Value);
                formatDelimiter = ',';
                output.Write('{');
                char? elementDelimiter = null; // Caller ensures that `tokensWithFormat` contains only property tokens that have non-null `Format`s.
