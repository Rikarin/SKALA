...
namespace Serilog.Settings.KeyValuePairs; class SettingValueConversions {
    ...
    static Dictionary<Type, Func<string, object>> ExtendedTypeConversions = new() {
        ...
        {
            typeof(Type), // Suppress this trimming warning. We'll annotate all the users of this dictionary instead
...
#pragma warning restore IL2057
    ...
    public static object? ConvertToType(string value, Type toType) {
        if (toType.IsGenericType && toType.GetGenericTypeDefinition() == typeof(Nullable<>)) {
            if (value == string.Empty) return null; // unwrap Nullable<> type since we're not handling null situations
            toType = toType.GenericTypeArguments[0];
        }

        if (toType.IsEnum) return Enum.Parse(toType, value);
        ...
        if ((toType.IsInterface || toType.IsAbstract) && !string.IsNullOrWhiteSpace(value)) {
            ...
            if (type != null) {
                var ctor = type.GetConstructors(BindingFlags.Public | BindingFlags.Instance)
                    .FirstOrDefault(ci => { var parameters = ci.GetParameters(); return parameters.Length == 0
    ...
    internal static bool TryParseStaticMemberAccessor(
        string input,
        [NotNullWhen(true)] out string? accessorTypeName,
