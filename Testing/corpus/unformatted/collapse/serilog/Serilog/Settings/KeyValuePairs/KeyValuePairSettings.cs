...
namespace Serilog.Settings.KeyValuePairs; [RequiresUnreferencedCode(
...
class KeyValuePairSettings : ILoggerSettings {
    ...
    static readonly string[] _supportedDirectives = [
        UsingDirective, LevelSwitchDirective, AuditToDirective, WriteToDirective, MinimumLevelDirective,
        MinimumLevelControlledByDirective, EnrichWithPropertyDirective, EnrichWithDirective, FilterDirective,
    ...
    readonly IReadOnlyDictionary<string, string> _settings; // This is only needed on net5 because RequiresUnreferencedCode couldn't be applied to types
    ...
    public void Configure(LoggerConfiguration loggerConfiguration) {
        ...
        var directives = _settings.Where(kvp => _supportedDirectives.Any(kvp.Key.StartsWith))
        ...
        foreach (var enrichPropertyDirective in directives.Where(dir => dir.Key.StartsWith(
                         EnrichWithPropertyDirectivePrefix
                     )
        ...
        foreach (var minimumLevelOverrideDirective in directives.Where(dir => dir.Key.StartsWith(
                         MinimumLevelOverrideDirectivePrefix
                     )
        ...
        var callableDirectives = (from wt in directives where matchCallables.IsMatch(
                wt.Key
            ) let match = matchCallables.Match(wt.Key) select new {
        ...
        foreach (var receiverGroup in callableDirectives.GroupBy(d => d.ReceiverType)) {
            ...
            var calls = receiverGroup.Select(d => d.Call).GroupBy(call => call.MethodName).ToList();
            ApplyDirectives(
                calls,
                methods,
    ...
    static IReadOnlyDictionary<string, LoggingLevelSwitch> ParseNamedLevelSwitchDeclarationDirectives(
        ...
        var switchDeclarationDirectives = (from wt in directives where matchLevelSwitchDeclarations.IsMatch(
                wt.Key
            ) let match = matchLevelSwitchDeclarations.Match(wt.Key) select new {
                SwitchName = match.Groups["switchName"].Value, InitialSwitchLevel = wt.Value
            }).ToList();
        ...
        foreach (var switchDeclarationDirective in switchDeclarationDirectives) {
            ...
            var switchInitialLevel = switchDeclarationDirective.InitialSwitchLevel; // switchName must be something like $switch to avoid ambiguities
    ...
    static void ApplyDirectives(
        ...
        foreach (var directiveInfo in directives) {
            ...
            if (target == null) {
                ...
                var call = (from p in target.GetParameters()
                        .Skip(1) let directive = directiveInfo.FirstOrDefault(s => s.ArgumentName == p.Name
                    ) select SuppressConvertCall(
                        directive,
                        p
    ...
    internal static MethodInfo? SelectConfigurationMethod(
        ...
        return candidateMethods.Where(m => m.Name == name
            && m.GetParameters()
                .Skip(1)
