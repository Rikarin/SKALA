...
Assert   .    AreEqual(   typeof  (   KeyValuePairConverter), contract. InternalConverter.GetType( ));
           ...
           IList    <KeyValuePair
	<string, int> >   values =    new   List<KeyValuePair <string,   int>>
		{  
...
(@"[
  ...
  },
  {
    ...
    ""Value"": 456
            ...
            json);  
   IList
             <    KeyValuePair   <    string ,  int>>   v2 = JsonConvert.DeserializeObject<IList<KeyValuePair < string    , int    >   >>(  json);
               
          Assert   .AreEqual  (2    , v2 .Count) ; Assert.  AreEqual(  "123", v2[0].Key);
            Assert.
          AreEqual(123  , v2[0  ]  .  Value    );
                Assert.  AreEqual (    "456", v2[    1].Key    );
               Assert.AreEqual ( 456,
           v2[1   ].  Value   );
           }
     
        [  Test] public void DeserializeUnexpectedEnd(  )
