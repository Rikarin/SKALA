...
get;  set; } =  string.Empty;
  ...
  public UtilityInputKind Input    { get
	;   set; }
...
;    set; }
   ...
   =    []   ;
    ...
    /// <returns>The curve.</returns>
      public ResponseCurve BuildCurve(    ) => new(  )    {  Kind    = Curve,
               Slope =  Slope, Exponent =    Exponent,
         Shift =
Shift  ,
 Centre =  Centre, Keys  = Curve == ResponseCurveKind.Sampled && Keys.    Count > 0   
               ?
...
Mode    {    get; set;   }
    ...
    /// <returns>The sample.</returns>
             public CurveSample  ToSample ()    => new(Time,  Value  , InTangent,
OutTangent, Mode    )  ;
	}
...
DecisionInterval { get; set; } =  0.2f;
  ...
  foreach (var key    in Keys   ) { 
            try {   builder.Add(    key  .Name, key   .    Type   )  ;
    ...
    public IUtilitySelector BuildSelector() => Selector switch {
       ...
       UtilitySelectorKind.
               TopWeightedRandom => UtilitySelectors.TopWeightedRandom( Math.Max(1,    SelectorCount)   ) ,
