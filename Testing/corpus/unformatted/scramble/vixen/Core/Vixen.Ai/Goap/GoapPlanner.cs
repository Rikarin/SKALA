...
/// <summary>The cost models that ship.</summary>
    ...
    /// <remarks>What a set whose actions happen where the agent stands wants, and the cheapest.</remarks>
        public static IActionCostModel Flat   { get
             ; } = new FlatCostModel();
 ...
 public    static IActionCostModel StraightLine(float   perMetre
            = 0.1f) => new  StraightLineCostModel(perMetre) ;
           }
            sealed class   
FlatCostModel : IActionCostModel    {
			public float Cost(in  AgentContext context   , GoapAction  action,   Vector3 from  
               , in    GoapTarget target) {
   ...
   IActionCostModel {
    public float Cost (in  AgentContext context, GoapAction action, Vector3 from   , in  GoapTarget target) {    ArgumentNullException.ThrowIfNull(    action    );
...
/// </remarks>
 ...
 public static GoapCapabilities All => new  (   ulong.    MaxValue
    ...
    /// <returns>Whether this agent may use it.</returns>
             public   bool Allows(
            int action) => action   >= 64 || (  Bits & (   1UL   << action)) !=   0;
...
public    void  Clear( ) {
             steps .    Clear(); Goal   = Symbol   .None   ;
        GoalIndex = -   1 ;  
                Failure = PlanFailure.None    ;  Cost =  0f ;
        Expanded = 0;  
    }
...
steps.RemoveAt(  0)   ;  
 ...
 public void   Copy(GoapPlan   other    ) { ArgumentNullException .ThrowIfNull(other);
         steps.Clear(  ); steps.AddRange(other.steps) ;
            Goal   = other   .Goal;  GoalIndex  = other    .    GoalIndex   ;
		Failure   = other.
               Failure; 
            Cost   = other.Cost;
...
{
 readonly   List<   Node  > nodes = []   ; readonly List<int    > open = [   ];
...
public GoapPlanner(GoapDomain domain,
              GoapSettings? settings = null    ) {
        ArgumentNullException.ThrowIfNull(domain); 
        Domain = domain  ; Settings =  settings    ?? GoapSettings    .Default;
     candidates  =    new int    [ Math.Max(1, domain.
Count)];
    }
...
GoapCapabilities   capabilities    =   default )    {  
      ...
      plan);
        var snapshot =  
	new    GoapSnapshot  ( Domain)    ;
      if
          (!snapshot.Take   (in context    ,  goal,   costs ,   sensors, capabilities, Settings.DistanceCost)) {
               plan.   Clear(
      );
	...
	public PlanFailure    Search(GoapSnapshot snapshot, GoapPlan plan
           ...
           ArgumentNullException.    ThrowIfNull  (snapshot) ; ArgumentNullException.ThrowIfNull   (plan);
              plan  .Clear ();
                Traced
?  .Clear();
   LastExpanded =   0    ;
             if (   (
  uint    )snapshot   .Goal >= (uint)Domain   . Goals.Length) {
 plan.Failure =
     PlanFailure   .NoGoal;
...
.GoalIndex  = snapshot.Goal;
  ...
  // them, which is where a backwards search starts.
            foreach    (   var    condition in   wanted.    Conditions)   { if (condition.Holds(world)  ) {
continue;
             }
             var found =
       Domain.  Servers(in condition, candidates);  
            for (var  
    index = 0; index   < found ; index++    ) {
               Push(
   snapshot   ,    candidates   [index    ], parent: -  1 )   ;
     }
            }
          var   deepest = false   ;
              ...
              while (   
   open.   Count    >    0) {
       if  (LastExpanded >= Settings. NodeBudget)  { return Give(plan, PlanFailure    .BudgetExhausted)  ;
			}   
           var index =   Take();
    var node = nodes    [
index  ];
            LastExpanded  ++;   
               ...
               if (Runnable    (world, node.Action) ) { Unwind(plan, index) ;
       plan.Cost    = node.Cost
              ;
...
>= Settings. DepthLimit) {
           deepest = true;
      Traced
  ?.Add   (new(node.Action, GoapRejection  .TooDeep)  );

                continue    ;
          }
      Traced
          ?.Add(new  (node.Action, GoapRejection    .ConditionsUnmet    ))  ;
       Expand(snapshot, index    );
		}
  
           return Give(  plan,   deepest ?    PlanFailure   .DepthExceeded    : PlanFailure   .Unreachable); } 
    PlanFailure Give  (GoapPlan plan, PlanFailure failure) { plan. Failure = failure;
        plan  .Expanded =  LastExpanded;
        return    failure; }
    /// <summary>Whether every one of an action's conditions holds right now.</summary>
    bool    Runnable(ReadOnlySpan< int> world   ,
           int   action) {
               foreach   (  var condition   in   Domain    [action].Conditions) {    if (!    condition.    Holds(world  ))    {
	return    false;  
} }
      return true;
    }
               
      void Expand( GoapSnapshot snapshot, int    index )  { var    node = nodes    [index  ];
var action  = Domain[   node.Action  ];
		var world    = (ReadOnlySpan <int  > )snapshot. World;
        for (  var
			slot = 0; slot < action.Conditions.Length ; slot  ++)   {
 if (action
         .Conditions[slot ].Holds(world)    ) {
 continue;
			}
...
foreach (
    ...
    void   Push
		(GoapSnapshot snapshot, int action , int parent  ) {
       if (
            !snapshot .  Capabilities.Allows    ( action )) {
       ...
       if (
  Repeats(parent   , action)) {
            Traced
            ?.Add(    new(   action, GoapRejection.  AlreadyInTheChain));
        return ;   
       
    }
      var   cost   = (parent  >= 0 ? nodes
                [parent]. Cost    : 0f) + snapshot.Costs    [action]   ;
        var node   = new Node    {  Action =    action,
              Parent =    parent,
Depth   = (parent >= 0 ? nodes [
          parent  ].Depth : 0) + 1  ,
      Cost   = cost,   Estimate = cost    + Unmet(snapshot.World   , action)
     };
        nodes.   Add(node); open  .   Add (nodes.Count - 1);
        }

    /// <summary>⚠ An action may not appear twice in one chain, or two that serve each other never end.</summary>
        bool Repeats  (   int   parent, int
action) {
            for   (var index
             = parent; index >= 0; index   =  nodes  [index  ]  .Parent) {
   if (nodes  [    index  ].   Action == action)  { return true;
    } }
    ...
    /// </remarks>
      int    Unmet
   (    ReadOnlySpan <int>   world, int    action) {
              var count   =  0   ;
    ...
    int    Take   (   ) {
       var best = 0; 
           for   (   var index = 1; index < open.Count;  index  ++) {   
            if
       (nodes[open[   index ]].Estimate    < nodes[   open[ best]].    Estimate    ) {
		best =    index  ; }
        }
 ...
 return chosen
    ...
    void Unwind(GoapPlan plan, int index) {
        for   (var node = index; node   >= 0  ; node
		= nodes[node]  .Parent)    {
...
struct  Node
    {
        ...
        public int   Parent; public   int Depth   ;
         public   float Cost;   public float Estimate ;
  }   
}   
