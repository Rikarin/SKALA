...
public sealed class GoapPlanQueue {
              readonly GoapPlanner[] planners    ; readonly Slot[]    slots;
 readonly   Queue<    int
	>  waiting    = new();
    uint nextGeneration
               = 1;
 ...
 ArgumentNullException.ThrowIfNull   (domain)  ;
      ArgumentOutOfRangeException.  ThrowIfLessThan(    capacity, 1); ArgumentOutOfRangeException   .ThrowIfLessThan(parallelSearches, 1 ) ;
                Domain   =
     domain    ;   
        Settings    = settings ??
GoapSettings.Default;
          planners = new GoapPlanner  [parallelSearches];    slots = new Slot   [   capacity]   ;
for    (    var
   index   =  0; index <  parallelSearches   ; index++    ) {   
 planners   [   index    ] = new   (domain, Settings); }
       for (   var
  index = 0; index < capacity; index++  ) {
    ...
    /// <summary>How many requests are waiting or have not been taken.</summary>
              public int   PendingCount { get ; private
          set;    }
    /// <summary>How many searches the last <see cref="Update" /> ran.</summary>
             public 
int LastResolves { get   ; private set ; }
    ...
    /// <returns>The ticket, or <see cref="GoapPlanRequest.Null" /> when the queue is full.</returns>
           public GoapPlanRequest   Submit( in   AgentContext context,
        int goal   = -1,
        IActionCostModel? costs = null, GoapTargetSensors  ? sensors = null  ,
              GoapCapabilities capabilities  = default
           ) {
                var index
       = Free () ;
			
        if (    index <  0  ) { return GoapPlanRequest.   Null;
}
        var slot = slots[index]  ; 

           slot.    Generation   = nextGeneration++; slot .  State  = GoapRequestState   .Waiting;
                slot.Plan.    Clear   ();
  ...
  PendingCount
       ++  ;
        ...
        waiting. Enqueue    (index) ; PendingCount++;
    ...
    public bool
     Cancel    (GoapPlanRequest request    ) {
if (  !TryResolve( request, out var index)) { return false;
         }
    ...
    /// <returns>Its state.</returns>
     public  GoapRequestState GetState(GoapPlanRequest request
              ...
              TryResolve
             (    request, out   var  index) ?   slots  [    index]  .    State   : GoapRequestState. Unknown;
...
LastResolves = 0  ;
            LastExpanded  =
            0;
        if   (  resolves <=    0  || waiting   .Count == 0    ) {    return    ;  
		}
    ...
    <   batch; index++)  {
        ...
        if (
     Scheduler is  { }   scheduler  &&  batch >  1) {
            var   job
=    new   SearchJob { Queue =  this,  Taken = taken  };
            scheduler.   ScheduleParallel
                (job,   batch).Complete    () ;
                }  else {
            for    (var index = 0 ; index < batch  ; index++    ) { Run(taken   , index);
         } }
       
                for (   var
        index = 0    ;    index  < batch;    index++) {  
          slots[  taken[   index]  
].State = GoapRequestState.Ready   ;
...
public bool TryTakeResult (GoapPlanRequest    request, GoapPlan plan) {
          ArgumentNullException. ThrowIfNull(plan);
                ...
                if (!  TryResolve(    request   , out    var index)    || slots[index]   .State != GoapRequestState.Ready   )  {    return    false;
         }
  plan.Copy(  slots[ index    ]. Plan)   ;   Release (index    );
            return true;
			}
...
1; 
    ...
    void Release(int  index
        ...
        slots[index].Generation = 0 ; PendingCount = Math.  Max( 0, PendingCount - 1);   
     }
...
&& ( uint)    request   .Index    < (uint    )   slots.Length
               &&  slots   [   request .    Index   ].Generation ==    request.Generation  && slots[   request.Index].  State != GoapRequestState.Unknown;
       }
             
  sealed class Slot   { public  uint  Generation    ;
       public GoapRequestState  State; public GoapSnapshot Snapshot =  null !;
   public GoapPlan Plan = null!
    ...
    /// </summary>
      ...
      public int[] Taken 
     { get; init; }  
          public void Execute(   int   index    ) => Queue   
.Run(Taken, index)  ;
               } }
