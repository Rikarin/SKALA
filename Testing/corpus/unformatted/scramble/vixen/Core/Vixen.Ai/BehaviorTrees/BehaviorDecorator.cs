...
/// </remarks>
    public
         readonly record struct BehaviorContext(AgentContext Agent,   BehaviorTreeInstance   Tree   , int Node) {
...
virtual bool Continuous    => false  ; 
    ...
    public abstract bool
     Evaluate ( in BehaviorContext context, ReadOnlySpan < byte> state )    ;
    /// <summary>The node is being entered.</summary>
    /// <param name="context">The agent and the node.</param>
    /// <param name="state">Its own bytes.</param>
    public virtual void Enter
               (in  BehaviorContext context,   Span<byte> state) { }
    ...
    public virtual ActionStatus  Finish
             (   in  BehaviorContext context, Span<byte> state,   ActionStatus result    ) => result    ;
    ...
    public virtual    bool ShouldRepeat
             (in BehaviorContext context,  Span<byte> state,  ActionStatus result  ) =>    false;
         }
...
public virtual int StateSize =>  0    ;
    ...
    /// <param name="delta">Seconds since this service last ran, which is not the frame's step.</param>
            public
        abstract    void Tick( in BehaviorContext    context, Span<byte    > state, float delta   );
