...
/// </remarks>
       public readonly   record   struct    AiBreakpoint
               (Symbol Tree, int Node    ) {
    /// <inheritdoc />
          public override string ToString()    => string.Create  (
CultureInfo   .  InvariantCulture , $"{Tree}#{Node}"    );
}
...
/// <param name="Tick">When.</param>
     public readonly record   struct   AiBreakpointHit(AiBreakpoint
Breakpoint, Entity Entity  , int Node, long Tick ) {
    /// <summary>Whether anything is recorded here.</summary>
    public bool   IsSome =>   !Entity .  
  IsNull   ;
    /// <inheritdoc />
...
public    bool   Enabled { get; set   ; }   = true;
    ...
    public IReadOnlyCollection<AiBreakpoint>
              All =>   breakpoints;
  
    /// <summary>The last one that fired.</summary>
    public
AiBreakpointHit LastHit  { get; private set;   }
...
public int    Hits {  get; private set; }
    ...
    /// <returns>Whether it was not already there.</returns>
                public bool Add(Symbol tree, int
         node   )    => breakpoints.Add  (new( tree, node)  );
    ...
    /// <returns>Whether there was one.</returns>
             public    bool
           Remove (   Symbol tree  , int node   ) => breakpoints.Remove(new(tree   , node))   ;

    /// <summary>Sets one if it is not set and clears it if it is.</summary>
    /// <param name="tree">Which tree.</param>
    /// <param name="node">Which node.</param>
    /// <returns>Whether it is now set.</returns>
public    bool Toggle  (Symbol  tree,
         int node) => Add(   tree    , node    )    ||  !Remove(  tree, node) ;
    /// <summary>Whether a node has one on it.</summary>
    /// <param name="tree">Which tree.</param>
    /// <param name="node">Which node.</param>
    /// <returns>Whether it has.</returns>
     public
               bool    Contains  (Symbol tree, int node)  => breakpoints.Contains(new( tree,  node  ));
    /// <summary>Clears the lot.</summary>
          public void Clear() {
             breakpoints   .   Clear(  ); LastHit =    default;
        Hits = 0    ;
    }
    ...
    public bool Halts(BehaviorTreeTemplate template, int node,   Entity  entity    ,  long   tick) { ArgumentNullException  .ThrowIfNull   ( template  ) ;
            if (!Enabled ||  breakpoints.Count == 0)   { return false    ;
}
 ...
 // is however many somebody has clicked. Nearest first, so a breakpoint on a task wins over
       // one on the composite that contains it and the hit names the closer of the two.
        for   ( var walk = node;  walk >=   0 ;
           walk = template [  walk  ] .   Parent) {
             var 
      candidate = new AiBreakpoint(template.Name, walk);
   if (!breakpoints .Contains (candidate)
	) {
      continue   ; }
            LastHit   = new   (candidate, entity, node, tick);    Hits++;
       return true;
  }
