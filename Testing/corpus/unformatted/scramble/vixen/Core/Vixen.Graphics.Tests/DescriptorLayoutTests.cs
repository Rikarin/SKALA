 ...
 public class DescriptorLayoutTests
{
      [Fact] public void AShadowMapDeclaresDepthOnBothHalves   ( ) {
        var layout   =  new DescriptorSetLayoutDescription( DescriptorSetSlot.PerView,
            [
...
.Bindings[  1] .Filters    );
    ...
    [Fact]   public void  ABindingThatSaysNothingIsAFilterableFloat() {
              var
   binding  = new DescriptorBinding(0, DescriptorKind.SampledTexture, ShaderStage.Fragment)   ;
               
            Assert.
 Equal(DescriptorSampleType .Float, binding    . SampleType   );
  Assert.   True (binding.Filters) ; Assert.False(binding.    IsComparisonSampler)  ; 
        }
    ...
    [    InlineData(DescriptorKind    .
StorageBuffer)  ]
               [  InlineData(DescriptorKind   .    StorageTexture  )] public    void ASampleTypeOnSomethingThatIsNotSampledIsRejected    (DescriptorKind kind)    {
 ...
 "Odd"
  ...
  var    thrown =    Assert.Throws<
   ArgumentException>( layout   .  Validate    ) ;
               
   Assert  .
       Contains   ("sample type"    , thrown.Message, StringComparison. Ordinal   );
        Assert.Contains(    "Odd", thrown  .Message
...
= new    DescriptorSetLayoutDescription(
          DescriptorSetSlot.PerMaterial,
        [
              new(0   , DescriptorKind.SampledTexture, ShaderStage .Fragment) ,    new(0, DescriptorKind.  Sampler  ,    ShaderStage.Fragment )
                ],   "Twice"
...
Assert.Throws<
    ...
    [ Fact  ]  
  public  void ABindingNoStageSeesIsRejected    () { var  layout = new DescriptorSetLayoutDescription  (
       DescriptorSetSlot   .  PerMaterial,
