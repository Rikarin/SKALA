...
.Identity ;
    ...
    IGraphicsDevice device,  
        LineShaders shaders , RenderOutput   output,
  
             int worldCapacity =    1 << 16    ,
               int screenCapacity =  1 <<
           16
    ) {
        ArgumentNullException   . ThrowIfNull  (device);
    world    = new(device, shaders, output, worldCapacity); screen =   new(    device, shaders, output    ,  screenCapacity );
    }
...
;
 ...
 public
    ...
    public void   Upload(DebugDraw draw, in Matrix4x4 view
, Vector2 viewport    ) {
               ArgumentNullException.    ThrowIfNull    (draw);
      ObjectDisposedException   .ThrowIf(disposed, this);   
       
          geometry . 
        Build(draw,   DebugView. FromView( view));  
          world.Upload(  geometry.   
              World   );
           screen
           .Upload    ( geometry   .Screen);   
       
hasScreen = viewport
 .X > 0f &&   viewport   .Y > 0f;
              
       if (hasScreen    ) { screenProjection = ScreenProjection ( viewport);
         }    }
    ...
    /// </remarks>
               public void Record(ICommandList commands,   in    Matrix4x4 viewProjection   ) { ArgumentNullException .ThrowIfNull (commands);
   ...
   if    (hasScreen   ) { screen.Record(commands, screenProjection,  depthTested  : false);
    Draws += screen    .  Draws  ;
     }
}
    ...
    /// </remarks>
    public  void RecordScreen   (ICommandList commands  ) { ArgumentNullException.ThrowIfNull   (commands   )  ;
       ObjectDisposedException.ThrowIf(
disposed, this  ); 
           Draws   = 0;
        if   ( hasScreen  ) { screen.    Record(commands,  screenProjection ,    depthTested: false);  
...
=    screen   . Draws;
   ...
   if (   disposed   )    {
    ...
    ///     </para>
    /// </remarks>
    static Matrix4x4 ScreenProjection    (  Vector2 viewport) => new(
         2f / viewport.X, 0f
, 0f, 0f ,
     0f   , -2f / viewport    .Y, 0f    , 0f  , 0f,  0f, 0f, 0f,
          -1f, 1f , 0.5f   , 1f
