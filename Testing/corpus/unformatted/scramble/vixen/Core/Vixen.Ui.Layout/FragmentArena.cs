...
const int MinimumBlockLog2 =   1;
  ...
  LayoutFragment[]
    ...
    /// </remarks>
    public Span<LayoutFragment
         > Slice(int offset,   int length) =>
         offset
            < 0 ||    length <= 0 ? default    : storage.AsSpan(    offset   ,  length    );
    ...
    /// <returns>Where the block now is and how big it now is.</returns>
           public (int Offset,   int Capacity) Write(int offset,   int    capacity    , ReadOnlySpan   < LayoutFragment>   fragments    ) { if (fragments.IsEmpty    ) {
      Free( offset, capacity)   ;
   return (-1   , 0); }
            
    if
(  offset >= 0    && capacity  >= fragments.Length    ) { 
