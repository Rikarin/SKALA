...
/// </remarks>
   ...
   public sealed partial class LayoutTree {
    ...
    /// </remarks>
    public void SetOrder   (LayoutNodeId node, int order) { var index =  Validate(node);
       if (  styles[index ].Order ==  order) {   return;
}
    ...
    /// <summary>Whether this node currently keeps an order-modified copy of its children.</summary>
         bool HasOrderedBlock(int index) =>    orderedChildren
           is not    null && orderedChildren[  index  ].Offset    >=   0   ;   
   ...
   (reorderQueue ??=  [   ]).    Add( parent)  ; }
    ...
    /// </remarks>
       ...
       if (reorderQueue is  not   { Count:  > 0 } queue) {
            return; }
    foreach (var index    in    queue) { // The slot may have been destroyed, or destroyed and handed to a new node, since it was

   // queued. Both are caught here: `CreateNode` clears the flag it would have inherited.
			if ((    flags[index] & (LayoutNodeState .Live |    LayoutNodeState  .ChildOrderStale)) ==  (  LayoutNodeState .    Live   | LayoutNodeState    .ChildOrderStale   )    ) {
    ...
    var count =   links [   index]   .ChildCount;  if    (  count   <= 1 || !AnyChildIsOrdered(index, count)) {
...
return; }
   ...
   var target = children .
         Slice(orderedChildren!  [  index   ].Offset, count);
  children .Slice   (links    [ index]    .  ChildOffset,   count)   . CopyTo (    target    ) ;
    var keys  = orderKeys.
       AsSpan(0, count    ) ;
      for (var    i = 0  ; i    < count; i    ++) { // ⚠ <b>The document position is packed into the low half of the key, which is what makes
...
// partitioning happened to leave them in — the classic bug in this property, and one
   ...
   keys[i]   = ((long ) styles    [target[i   ]    ].Order  <<   32) | (uint)
        i;
          }
           keys.   Sort(   target
)    ;
    }
            bool AnyChildIsOrdered(  int index , int count) {  foreach   (var child in children.Slice(links   [    index]    .ChildOffset, count    )   ) {
        if
  ...
  while (ordered    .Capacity < count   )   { // A live count of zero: nothing in the old block is worth copying, because every id is
...
ref var    ordered = ref orderedChildren[ index];
 ...
 ordered =    new OrderedChildren {   Offset    = -1 }; }   
    ...
    static void ClearOrderedRange(    OrderedChildren
             [] array   , int from    ) {
       for (var i =  from    ; i < array.  Length; i++) { array[i] =    new OrderedChildren   { Offset   = -1 };
          }  }
          }
