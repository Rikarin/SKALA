...
public static class YamlSerializer {
    ...
    static object? Bind(
        ...
        if (typeof(YamlNode)
            ...
            && underlying.IsInstanceOfType(node)) {
            return node;
        }
    ...
    static Type ResolveTag(YamlNode node, Type expected, string path) {
        if (node.Tag is not { } tag) {
            return expected.IsAbstract
                && !expected.IsSealed
                && !IsCollection(expected) ? throw new YamlBindingException(
                    path,
    ...
    static YamlSequence EmitSequence(
        IEnumerable items,
        Type runtime,
        YamlSerializerOptions
        options,
