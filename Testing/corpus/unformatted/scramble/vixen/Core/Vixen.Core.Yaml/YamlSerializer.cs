...
/// </remarks>
    ...
    /// <returns>The value.</returns>
      ...
      <  T>  (   string yaml, YamlSerializerOptions? options = null) =>
	Deserialize <
T>    (YamlReader.  Read(yaml), options    )  ;
...
public
   ...
   ArgumentNullException
             .ThrowIfNull  (    node)  ;
            ArgumentNullException  .ThrowIfNull    (expected);    return   Bind(node  , expected, options ??  YamlSerializerOptions.Default, string.Empty);
        }
    ...
    /// <exception cref="YamlBindingException">The value cannot be described.</exception>
           public static YamlNode Serialize(object? value,   Type declared, YamlSerializerOptions?   options    = null  ) { ArgumentNullException.ThrowIfNull(declared) ;
         return  Emit    (   value
    ,  declared  , options ?? YamlSerializerOptions.Default , string    .Empty);
  }
...
,
   YamlSerializerOptions
        ...
        bool declaredNullable =    true
         ...
         var   nullable    = (underlying != expected ||
            !expected.IsValueType) && declaredNullable;
        if    (IsNull  (   node    )) {
...
if (
        !nullable && YamlScalarConverters.TryGet(underlying, out var nullConverter) && nullConverter.AcceptsNull) {
 ...
 // Two different refusals wearing one word. A value type has no null at all, and naming it
    ...
    // where naming the type would read as nonsense, because of course an array can be null,
       ...
       throw new YamlBindingException(
                path , underlying  .    IsValueType
                    ?  $"null cannot be read as {underlying.Name}."
: "This member is declared without the '?' that would let it be null. Write the empty " + "value — '[]', '{}', or a quoted '' — or drop the key to keep the default."
            ); }
...
// alone is true for `object` as well — and a member declared `object` is how a scene's
      // components arrive, so the loose test handed every one of them back as a raw mapping and
         // the compiler reported that nothing had declared them.
             if ( typeof (YamlNode  )
       .IsAssignableFrom(underlying) && underlying .IsInstanceOfType    ( node   ))  {
          return    node; }
var target = ResolveTag   (node  ,   underlying   , path)  ;
        return node switch { YamlScalar    scalar => BindScalar(scalar.Value , target   , path)  ,
              YamlSequence sequence => BindSequence   (sequence, target    , options  ,   path) ,
           YamlMapping mapping when IsDictionary(target, out var
      valueType   )    =>
                BindDictionary (mapping, target    , valueType  , options    , path), YamlMapping mapping => BindContract  (mapping, target  , options, path),
    ...
    /// </remarks>
      static Type    ResolveTag(YamlNode   node , Type expected , string path)    {   if    (node    .Tag is not { } tag  )    {
            return expected.IsAbstract && !expected  .IsSealed &&   !IsCollection   (  expected)   ? throw new YamlBindingException(
              path   ,
                    $"{expected.Name} is abstract, so the node needs a type tag saying which one it is — "
                    + "'!TextureImporter', for instance."  )
                : expected ;
 }
           if (!   TypeRegistry.   TryGetByAlias
(tag,   out var descriptor)) {  
             throw new YamlBindingException(  
  path  , 
               ...
               return   expected.IsAssignableFrom(descriptor. Type ) ?    descriptor.Type
            : throw new YamlBindingException( path , $"'{tag}' is {descriptor.Type.Name}, which is not a {expected.Name}.");
  }
		
   static object BindScalar(string text,   Type type    , string
path  ) {
try {
      ...
      return text;
        ...
        TypeCode . Boolean
          => ParseBoolean(text, path) ,
             TypeCode
        .    SByte    => sbyte.Parse   (text, CultureInfo  . InvariantCulture   ),
                TypeCode. Byte => byte   .Parse (text,    CultureInfo.InvariantCulture),    TypeCode.  Int16 =>  short.  Parse(text,  CultureInfo.InvariantCulture   ) ,  
                TypeCode. UInt16 =>
     ushort    .    Parse(   text, CultureInfo. InvariantCulture),   
	TypeCode    .Int32  =>    int.Parse   (   text,
              CultureInfo.InvariantCulture  ),
            TypeCode.   UInt32 => uint   .  Parse (text  , CultureInfo.InvariantCulture  ), TypeCode.Int64   => long.Parse  (text,  CultureInfo.   InvariantCulture),
   TypeCode
           .UInt64 => ulong    . Parse(    text   , CultureInfo    .   InvariantCulture)   ,
		TypeCode. Single    => float.Parse   (text,    CultureInfo.   
            InvariantCulture    )    ,
          TypeCode
...
0]
  ...
  _ => BindOtherScalar  (text  , type   , path)
            ...
            throw  new   YamlBindingException(    path,
$"'{text}' is not a {type.Name}.", failure)   ;
          }   
    }
   static object BindOtherScalar(string text, Type  type   , string path) =>
                YamlScalarConverters.    TryGet(type   ,  out    var
			converter )
  ?    converter.Parse (text) : throw   new YamlBindingException(
     path,
                $"There is no way to read a scalar as {type.Name}. Register one with "
                + "YamlScalarConverters.Register if it should be one." );
      static bool ParseBoolean( string text  ,   string path  ) =>    text switch {
           "true"    or "True" or "TRUE" or "yes"    or "on" =>  true   , "false" or    "False"    or "FALSE" or "no" or    "off" => false    ,  
     _ =>    throw
new YamlBindingException(path, $"'{text}' is not true or false."   )
        }   ;

   static object BindSequence(YamlSequence  sequence,  Type type,   YamlSerializerOptions options, string    path) {
             var    element = ElementTypeOf(type   , path) ; var created =   Make  (type, sequence  .Count  , path); 
  ...
  // An array is already the right length and cannot grow; a List<T> is empty with the capacity
        ...
        if
            (created is Array array) {
    for
             (  var  index = 0; index  < sequence.Count; index    ++  ) {
...
(key  , value) in mapping .Entries) {
  ...
  return dictionary;
    ...
    static object  Make(Type    type  ,    int  capacity , string path) =>
         CollectionFactory    . TryCreate(type,
              capacity  ,    out   var created)
            ? created : throw   new YamlBindingException(
      path
        ,
                $"Nothing registered a way to make a {type.Name}. A collection type is registered by the "
            + "reflection generator when it appears on a described member, so either the declaring " +   "assembly does not run that generator, or this type was reached some other way."
               );

         static object  BindContract
    (YamlMapping mapping, Type   type, YamlSerializerOptions options, string path)  {
   if (   
!TypeRegistry.  TryGet   (type,   out var descriptor)) {
                throw new   YamlBindingException (
			path,
  ...
  if (!descriptor.   CanCreate) {
    ...
    foreach (  var (    key    , value) in mapping.Entries) { var member = FindMember(descriptor,   key   , options   );
   ...
   Invoke (Join(  path ,  key    )    ) ;
         continue; }
                member. SetValue  (   instance, Bind(    value , member    .MemberType  , options    , Join (
            path,   key)   , member   .   IsNullable)   );
       }  
        return instance ; }
    ...
    ///     </para>
    /// </remarks>
  static   MemberDescriptor   ?   FindMember    (TypeDescriptor
                descriptor,    string  key,   YamlSerializerOptions options) {
             foreach  (var member in descriptor.Members)  { if   (member  .IsSerialized && string.Equals   (options.KeyFor(member.Name),  key    , StringComparison.    Ordinal)) {
		return member;
            }
     }
                
             foreach
	(var member in descriptor   .Members)  {
     ...
     static YamlNode Emit(object?  value, Type declared , YamlSerializerOptions options, string path)  { if  (  value is    null) {
            return new YamlScalar
        ("null", YamlScalarStyle   .Plain);
      }
 ...
 return node; }
               static YamlNode
         EmitValue(object value, Type runtime, YamlSerializerOptions options, string path) {
...
return new YamlScalar(  text)
  ...
  if    (runtime .   IsEnum)   {
            return new YamlScalar(  value   .ToString()   !, YamlScalarStyle.Plain); }
                if (YamlScalarConverters. TryGet(runtime, out var converter)) {   return    new YamlScalar    (converter    .Format(value)    , converter.Style) ;
      }
        switch
                (    value  ) {
   case bool flag    :
   ...
   ;
    ...
    case IFormattable formattable   when   runtime   .IsPrimitive || runtime ==  typeof  (decimal):
     return new YamlScalar  (
           formattable.   ToString    (null, CultureInfo.    InvariantCulture ), YamlScalarStyle.Plain );
...
return EmitContract  (    value,  runtime,   options    , path);  }
   ...
   static YamlMapping EmitDictionary(IDictionary  dictionary, Type runtime, YamlSerializerOptions options  ,    string path) { var valueType =   runtime  .IsGenericType ?  runtime  .GetGenericArguments(  )[ ^ 1]  : typeof(object);
...
() ?? string    .Empty   ;
               mapping.Set(key
		, Emit  (   entry.Value , valueType,  options, Join(  path,  key)   ))   ;
              }
           
     return  mapping;   }
         
static YamlSequence EmitSequence(IEnumerable items, Type  runtime, YamlSerializerOptions
         options, string path) {
               var element =    runtime.  
        IsArray
...
(   object);
    ...
    static YamlMapping EmitContract( object value,    Type runtime, YamlSerializerOptions options, string   path    ) { if   (!TypeRegistry. TryGet    ( runtime, out var descriptor)) {
              throw   new
      YamlBindingException(
                path  ,
                $"{runtime.Name} has no descriptor. Give it [DataContract] so the generator describes it."
  );
        }
          var mapping = new YamlMapping(); var defaults    = options.   OmitDefaults ? DefaultOf(  descriptor) : null;
           foreach (var    member in descriptor.Members) { if (  !  member.    CanWrite) {
   ...
   .  IsSerialized   )   {
         ...
         var  member1 = member.
               GetValue( value);
                ...
                if (defaults   is not null && Equals(member1, member.GetValue(defaults ) )    ) {   continue;
           }
               
   var key   =
options    .KeyFor  (member.  Name);
               mapping.   Set(key    ,  Emit   (  member1   , member.MemberType, options, Join  (  path   , key)   )  );    }

            return mapping;  
  }
        
		static object? DefaultOf(TypeDescriptor descriptor  ) => Defaults .GetOrAdd(descriptor    . Type, _ => descriptor  .   CanCreate ? descriptor. Create (    ) : null );
...
var   definition    = type.GetGenericTypeDefinition(   )   ;
    if (definition == typeof(System.Collections.Immutable.ImmutableArray  <>)) { throw   new YamlBindingException(
              path, "ImmutableArray<T> cannot be built without MakeGenericMethod, which NativeAOT does not "
...
IList<>   )
  ...
  || definition == typeof(    IReadOnlyCollection  <>))  {
    ...
    throw  
new  YamlBindingException(path,  $"There is no way to read a sequence as {type.Name}.")    ;
      }
            
          static    bool  IsDictionary(Type type    , out Type valueType   )  { if  (type.IsGenericType) {
var definition  = type
...
typeof(IDictionary<,>)
   ...
   var arguments = type. GetGenericArguments()    ;  
      ...
      if   
         ...
         valueType = arguments[1];   return true;
}  }
        }
        ...
        return false    ;
              ...
              static bool  IsCollection(Type type) => type .  IsArray || (type !=  typeof  (string)  && typeof  (IEnumerable    ).IsAssignableFrom   (   type));
               static bool  IsNull(YamlNode node ) => node is YamlScalar { Style: YamlScalarStyle  .    Plain  or YamlScalarStyle  . Any }    scalar
			
        && scalar    .Value is "" or "null"  or "Null" or  "NULL"    or "~" ;
           
    static string Join   (string path,
string key) => path   .Length == 0 ? key   :   $"{path}.{key}";
   }
