  // SPDX-FileCopyrightText: Copyright (c) Rikarin
        ...
        using    System.Buffers;  using Vixen .Core .Curves    ;
...
readonly    List<CurveKey  > keys = []  ;
    ...
    /// <param name="source">The keys, in any order.</param>
             public AnimationCurve( params ReadOnlySpan<CurveKey  >
			source   ) {
          foreach (var key in
 source)    {
            keys  .Add    (   key); }
 ...
 public
IReadOnlyList<CurveKey > Keys   => keys    ;
                
    /// <summary>Raised after anything changes.</summary>
    public event Action<   AnimationCurve   >?
Changed  ;
...
new   CurveKey(0f   , 0f, TangentMode
     .   Free  ) {    OutTangent =  0f }  ,
...
new( new CurveKey(0f, 0f    ,  TangentMode.Free) { OutTangent = 2f }    ,
       new CurveKey    (1f, 1f, TangentMode.   Free) { InTangent = 
0f  }
   );
    ...
    /// <summary>A curve that starts and ends slowly. The default for almost everything.</summary>
			...
			static    AnimationCurve EaseInOut() =>
                new(  new   CurveKey(0f, 0f    ,  TangentMode   .   Free) { OutTangent = 0f },
               new CurveKey  (    1f, 1f, TangentMode. Free)   { InTangent    = 0f  } );
    /// <summary>A single step in the middle.</summary>
        public static
            AnimationCurve    Step()    =>
                new(new    CurveKey(0f,  0f, TangentMode .
         Constant   ) , new CurveKey(1f, 1f, TangentMode.Constant));  
...
if (!    keys.Remove   (key)) {
 ...
 Sort() ;
    ...
    /// </remarks>
    public void Move(
                CurveKey    key   , float time, float value) {
        ArgumentNullException .ThrowIfNull( key
        );
			key
            .  Time = time    ;
...
if ( keys.Count == 0 ) { return 0f  ;
       ...
       if (keys .   Count <= StackKeys) { Span   <CurveSample> stack = stackalloc  CurveSample[ StackKeys ];
     Fill    (stack   );
...
var    rented =    ArrayPool  <
 ...
 try {
               Fill  (rented);   
               return CurveEvaluation.  Slopes   (  rented
                .AsSpan(0, keys .Count) , index   );
} finally { ArrayPool  <  CurveSample>    .Shared.   Return   (   rented );
     } }
    ...
    /// </remarks>
               void   Fill(Span   <CurveSample
           > buffer)    {
        for (
var  index = 0; index < keys . Count   ; index++) {   
            var  key  = keys    [index   ];  buffer[  index    ] =   new(key.Time, key . Value, key.InTangent    , key.    OutTangent, key.Mode);
			}
         }
