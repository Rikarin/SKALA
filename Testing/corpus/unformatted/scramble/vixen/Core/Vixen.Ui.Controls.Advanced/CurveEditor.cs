...
public sealed partial class CurveEditor : Control {
    ...
    void DrawGrid(DrawContext context, Rectangle bounds) {
        ...
        for (var time = MathF.Ceiling(View.X / step) * step; time < View.Right; time += step) {
            ...
            grid.MoveTo(
                ...
                .LineTo(new Vector2(x, bounds.Bottom));
        }
        ...
        for (var
            ...
            var y
                = ToScreen(0f, value).Y;
            grid.MoveTo
                (new Vector2(bounds.Left, y))
                .LineTo(new Vector2(bounds.Right, y));
        }
    ...
    Vector2 Snap(Vector2 point) =>
        SnapToGrid && TimeStep > 0f && ValueStep > 0f ? new Vector2(
            MathF.Round(point.X / TimeStep) * TimeStep,
            MathF.Round(point.Y / ValueStep) * ValueStep
        )
            : point;
