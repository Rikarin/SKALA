...
/// </remarks>
    ...
    AnimationCurve curve = AnimationCurve.    EaseInOut( ); CurveDrag   drag;
    CurveKey? active;
    Vector2   panStart;
             Vector2 panOrigin;
    int gridColor  ;
    int curveColor; int keyColor   ;
 ...
 override  bool AcceptsFocus   => true ;
    ...
    /// <summary>The curve being edited.</summary>
     ...
     curve   .Changed  -= OnCurveChanged;
            curve   = value  ; curve.Changed +=   OnCurveChanged  ;
            selection. Clear   ()    ;
           active =
          null;
...
{ get;    set    ; } = new(-0.1f    , -0.2f, 1.2f, 1.4f)  ;  
  ...
  [UiProperty] public partial  bool SnapToGrid   { get;  set; }
   ...
   [UiProperty(Default   =   0.1f)] public partial float   ValueStep {   get;    set; }
    ...
    [UiProperty    (  Default = 44f) ]
		public partial    float HandleLength
           { get; set; } 
...
;  
keyColor  =   
               Document.PropertyId ("--key-color"    );
        handleColor = Document
              .PropertyId("--handle-color"    )   ;  
			curve.    Changed +=  OnCurveChanged;
AddHandler <
    PointerEvent  >(static    ( element,    args    ) => ((CurveEditor) element).Pointed(  args    ));
   AddHandler  <  WheelEvent
>(static (element, args) => ((    CurveEditor)    element)    .Wheeled   (    args));
AddHandler<KeyEvent>   (   static ( element  , args  ) => ((    CurveEditor) element)  .Keyed(args) ); AddHandler<   TapEvent>(static ( element , args) => (  ( CurveEditor) element)   .Tapped(args));
    }
    ...
    /// <returns>The point.</returns>
         public Vector2 ToScreen( float time   , float value  ) { var bounds = Bounds   ;
        return new Vector2(
         bounds .X + ((
time - View.   X) / View.    Width * bounds.Width    )  ,   
  bounds .Bottom  
    ...
    public Vector2 ToCurve   (    float x   , float y)  { var bounds =  Bounds;
        return  new Vector2    ( bounds    .Width <= 0f ? View.X : View.X +  (  (x    -  bounds.X) / bounds.Width * View .Width),
   bounds    .Height
              <=    0f ? View.Y    : View.Y + (   (bounds.  Bottom - y   ) / bounds    .Height * View  .Height)
           );
           }
...
minimumValue =   MathF  .Min  (minimumValue, key
                .Value)   ;
     ...
     // ⚠ A minimum span, because a constant curve has no height and a flat curve is exactly what
        ...
        var    width    = MathF  .Max  
 (  0.001f,   maximumTime   - minimumTime);
var
           height =   MathF.   Max (0.001f, maximumValue - minimumValue )    ;
         View
= new Rectangle(
    minimumTime - (width  *   0.1f), minimumValue    - ( height * 0.2f)   ,
    width *
               1.2f,
height * 1.4f
        )    ;
    ...
    /// </remarks>
            public  void Apply    (  AnimationCurve preset    ) {  ArgumentNullException.ThrowIfNull(preset   ) ;
    ...
    foreach
(var    key in preset   .Keys   ) {
curve   .Add(
     new CurveKey(key.Time   , key.    Value, key.  Mode) {   InTangent = key.InTangent ,
		OutTangent = key   .    OutTangent }
     );
      }
    
         selection.Clear (); active =   null;
SelectionChanged?.    Invoke (this  )   ;  }
  ...
  public void SetTangentMode( TangentMode    mode) {
        ...
        foreach (  var key in selection   ) { key   .Mode =  mode;
      } 
 ...
 * radius ;
        ...
        Keys) {
  var point =   ToScreen(key .Time, key    .   Value)   ; var distance = (  (point.X -  x)  * (point.X   - x) ) + ((  point  .Y - y)   * (point  .Y - y));
        if (distance  <= best
)  {
	best  = distance  ;
               found    = key;    }   
...
Document.ColorOf  (Style, handleColor    ) ??    new   Color4  (0.55f, 0.58f, 0.63f, 1f);
	if   (active
       ...
       foreach (var key in    curve.Keys )   { var point = ToScreen(key  .Time, key.    Value    )    ;
  var    size =    selection .
          Contains    (key)    ? 5f   : 4f;
...
size, point. Y    - size, size  * 2f, size  *    2f),
  ...
  new Rectangle(  point .X    - size ,    point.Y    - size    , size    * 2f, size * 2f), keys   ,
  1.5f    ,  BoxStyle  .   Rounded(  CornerRadii.Uniform(  size)    )
     )  ;
	}
            }
             void DrawGrid(   DrawContext   context   , Rectangle bounds ) {    if (   TimeStep <= 0f  ||    ValueStep <=   0f) {
        return    ;
...
Clear()   ;
 ...
 var step = TimeStep;
            while (step   /   View  . Width *    bounds.Width < 6f) { step *= 2f;
           if (step    > 1e6f) {
                return  ;
              } }
  for (var    time = MathF.Ceiling( View .X / step    ) * step;  time < View   .Right ;  time    += step) { var x = ToScreen(time, 0f   ).   X;
            grid   .MoveTo   (new Vector2
 (x   , bounds.Top   ))    .LineTo(new Vector2(x    , bounds.    Bottom )   );
        } 
...
= ValueStep;
 ...
 vertical
      *= 2f  ;
            if  (   vertical > 1e6f  ) { return;
            } } 
        for    (var   
      value = MathF.Ceiling   (View.    Y  /    vertical ) *  vertical; value < View.Bottom; value +=    vertical) {
  var y  
=  ToScreen(    0f  , value)   .  Y ;
     grid.   MoveTo
        (new Vector2   (bounds.Left, y))  .LineTo(new Vector2(bounds    .Right,   y) );
           }
           context.Stroke(grid, Document. ColorOf   (Style , gridColor) ??    new
    Color4(0f, 0f,   0f, 0.08f)    , 1f) ;
          }
              void   DrawCurve(DrawContext context, Rectangle bounds)  { if (curve  .  Keys  .Count ==    0) {
	return; }
 
    path.Clear
();   
        var columns =  (    int  ) MathF.   Ceiling(bounds.  
          Width   );
...
;    i <= columns;    i++    )  {
     ...
     var point =    ToScreen(time , curve    .   Evaluate
(time)   )    ;
         
 if (i    == 0) {
             path .MoveTo(point  ); } else {
	path  .
       LineTo   (point)   ;
            }   }
             ...
             context  .Stroke( path,
                Document.ColorOf
               (    Style, curveColor) ?? Document   .ForegroundOf(this  )  ,
  2f,
...
void
DrawHandles   (DrawContext context, CurveKey key, Color4 colour) {
    if    (
            key   .Mode    is TangentMode .Auto or TangentMode .Linear or TangentMode.  Constant) {
  return;
 }
        var centre = ToScreen   
           ( key.Time , key.Value);

        foreach (var    outgoing    in (ReadOnlySpan <bool>) [false, true])  {  var end = HandlePoint(  key, outgoing);
            path    .Clear()    .  MoveTo(centre ).LineTo(end);    context.Stroke(path, colour, 1f    ); 
    ...
    /// <returns>The point.</returns>
 public  Vector2 HandlePoint(   CurveKey
key, bool outgoing ) {
    
ArgumentNullException    .ThrowIfNull  (key  )   ;
    
          var  centre = ToScreen(    key    .Time, key.Value);  var    slope = outgoing ? key   .OutTangent  : key    .InTangent;
     ...
     direction = new   Vector2(
                (outgoing ? 1f : - 1f) / MathF. Max(1e-6f, View.Width)    * bounds.Width, -   (outgoing ? slope   : -slope) /    MathF.Max (  1e-6f    ,   View   .Height ) *  bounds. Height
       );
        var length    = MathF  .Sqrt
( (direction.X    * direction.  X) + (  direction.Y * direction.Y)    )    ;  
               
                return length <= 1e-6f ? centre   
    :
...
// ── Input ────────────────────────────────────────────────────────────────
    ...
    void OnCurveChanged(   AnimationCurve   changed)    {  CurveChanged? .Invoke(this);
        Document    .    Invalidate    (  )  ;
          
       }
    void  Pointed(    PointerEvent args) { switch (  args.   Action) {
            case PointerAction
.Pressed:   
                Begin   (args) ;
  break   ;
 case   PointerAction.   Moved    when drag   != CurveDrag.  None: Track    (args);
                break; 
        
   case
                PointerAction.Released when drag != CurveDrag.  None:
             drag = CurveDrag.None    ;
 ...
 break;
   ...
   ( PointerEvent  args    ) {
     Document .  Focus (  this);
        ...
        if  (args .Button is PointerButton.Middle or PointerButton.Secondary    ) { drag = CurveDrag.Pan;
...
return ;
 ...
 if (active is {    } current && current.Mode is TangentMode.  Free or TangentMode
            . Broken) {
		foreach (var outgoing   in (ReadOnlySpan   < bool  >) [false   , true]) { var    point = HandlePoint  ( current, outgoing)    ;  

   if (Near( point, args    .X,  args.Y, 8f    )) { drag = outgoing  ?   CurveDrag.  OutHandle : CurveDrag.InHandle    ;
                Document.CapturePointer(this);
                
                    return;
      } }  
        }
        if    (KeyAt (args .X,   args    .Y    ) is  {  } key)   { Select (key, args.Modifiers )    ;
drag   =  CurveDrag .Key  ; Document.CapturePointer(this);
              return   ;
		}
         ...
         Select(null,   args. Modifiers
          ...
          void  Track(    PointerEvent args)    { var point    =    ToCurve   (  args. X    , args.Y );
 
     switch (drag)   {
               case  CurveDrag.  Pan:    var scale =   new Vector2(
...
;
               View = new   Rectangle(
                    panOrigin.X    - (   (  args.
  X - panStart.X) *  scale.X )   ,
               panOrigin.    Y
 +  ((  args.   Y - panStart    .Y    )    * scale.Y  ),
                    View   .    Width,
            View.Height   );
 Document.  Invalidate();
                break;
      
             case   CurveDrag  .Key  when active is { } key: var snapped   = Snap(    point);
        foreach   (var   selected  in selection) { 
       if (ReferenceEquals   (selected,   key)) { continue;
}
			curve.Move(  selected, selected    .Time +   ( snapped.  X    -    key  .    Time    ),    selected   .Value + (snapped  .Y - key.   Value));  }
 
                curve.Move(key, snapped
         .    X, snapped.Y);
               break;

            case CurveDrag  .InHandle or CurveDrag.OutHandle   when    active is { }   handled:    Aim (handled, args   , drag    == CurveDrag.    OutHandle);
        break;
    ...
    /// </remarks>
 void    Aim(CurveKey    key, PointerEvent args, bool outgoing)    { var centre = ToScreen(   key.Time, key  .Value)    ;
               var
              bounds = Bounds;
	
		var    dx =  (args.  X - centre. X)  / MathF
           . Max(1f   , bounds.Width   ) * View  .  Width;
           var  dy = (centre    .    Y - args.
              Y)  /    MathF.Max(1f, bounds.Height)   * View    .  Height;
	...
	if (MathF .Abs(dx ) < 1e-6f   ) {
       ...
       var slope = dy
/ MathF.Abs(dx) *   (   outgoing ? 1f :  -1f) * (dx   <  0f ? -1f   : 1f);
         if (outgoing)  
 {
key   
    . OutTangent = slope;
     ...
     key.  InTangent  = slope  ;
        ...
        curve.
              Touch ( );
               ...
               void Tapped(TapEvent args) { if (args.Count != 2  ) {  
            return;  }

  if   (    KeyAt   ( args.X, args   .  Y) is   { } key) { curve    .   Remove(key);
            selection   .Remove(key    ); active = ReferenceEquals(   active    , key) ? null : active;
	SelectionChanged
		?    .Invoke  (this);
              } else { var point =   Snap(ToCurve(    args .X, args.Y)  );
             Select(
 curve .Add(point    .X, point   .Y    ) ,    ModifierKeys.None);  
       }
        
args.Handled = true  ;   }
void   Keyed(
                KeyEvent args    ) {
           if (args   .Action != KeyAction.
Pressed) {
                return; }
              switch (args.Key) {
 case    InputKey  .Delete or InputKey. Backspace when
   selection.Count > 0: 
...
. Remove(    key   )  ;
   ...
   active =    null  ;
        ...
        (this    )   ;
             break;  
 case InputKey.F : Frame  ();
             break  ;
              
            case InputKey .A when args.    Modifiers.HasFlag(ModifierKeys  . Control):
            selection.
Clear  ( );
                foreach (  var key in curve. Keys) { selection.Add(  key   );
                }
          
          active
     = curve.Keys    .Count >    0 ? curve.   Keys   [0  ] : null;
		SelectionChanged?.  Invoke(this);
...
before    = ToCurve(    args .X , args  .   Y   ); 
        ...
        View = new Rectangle(  View.X,  View. Y, View  .    Width
    * factor   , View   .Height * factor);
      var after
=    ToCurve(   args. X   ,  args.Y);
           View = new Rectangle(View.X + before.X  - after.X, View   . Y +    before.Y - after.    Y, View   .Width, View.Height  ) ;
      Document.Invalidate();
  args .  Handled = true;
		}
             
    void   Select(CurveKey   ?  key  , ModifierKeys modifiers) { if  (key is null ) {
 ...
 selection.  Clear();
          active  = null;
         SelectionChanged?.Invoke (  this)   ; return;
 }

         if
             (modifiers.HasFlag(   ModifierKeys    .Control  ) || modifiers.HasFlag   (  ModifierKeys. Shift))    {
           selection.Add(key);
		} else if (    !selection.Contains(key   )) { selection    .Clear();
selection
  .Add   (   key) ;
                }
 active = key;
  ...
  SnapToGrid && TimeStep > 0f    &&   ValueStep > 0f ?  new   Vector2(   MathF.Round    (point    .  X / TimeStep) *  TimeStep, MathF.Round( point   .Y / ValueStep   ) * ValueStep)
            : point   ;
         static bool Near(Vector2 point ,    float x , float y, float radius) =>
      (    (  point
     .X -    x) * (point  .X - x )) +   ((    point.Y   - y) *   (point.Y -  y)) <= radius  *    radius;
			}
