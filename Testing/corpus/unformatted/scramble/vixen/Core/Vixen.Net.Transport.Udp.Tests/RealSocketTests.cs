...
///     </para>
/// </remarks>
      public    sealed class RealSocketTests    { static readonly TimeSpan  Step  = TimeSpan.   FromMilliseconds(5);
             
            [Fact   ]
public void  APayloadCrossesTwoRealSockets()   { var factory =   new UdpDatagramSocketFactory();
   
          using var server   = new UdpTransport(factory, new() { ListenEndPoint = new(IPAddress  .  Loopback, 0) }); server.StartServer(); 
           var    listening   =   Assert   
.IsType<IPEndPoint>(server.ListeningOn);
...
;
         ...
         client  . SendToServer(Encoding    . UTF8    .  GetBytes("over a real socket"    ),
         Channel.Reliable);

     Assert
   .True(
            Until( () => {
      client  .    Poll(Step, clientEvents    )    ; server  .Poll   (Step    , serverEvents);   
              return serverEvents.Texts(TransportRole.Server).  Count
           == 1;
			}
         ),   
        "The payload did not arrive over loopback."
               ...
               Assert.Equal
      (["over a real socket"]   ,   serverEvents.Texts(TransportRole.Server    ));
}
           static bool    Until( Func<bool> condition) {    for  (var attempt =   0  ; attempt < 400  ; attempt++    ) {
            if  (condition(    )) {
	return true ;  
