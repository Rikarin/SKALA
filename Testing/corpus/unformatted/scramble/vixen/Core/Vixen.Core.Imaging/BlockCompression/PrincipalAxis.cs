...
/// </remarks>
    ...
    /// <param name="axis">The direction, normalised. All ones if the block has no extent at all.</param>
              public static  void Find(ReadOnlySpan<float
> covariance, int dimensions    , Span   <float>    axis)    {
             Span  <float   >
			next = stackalloc float[dimensions];
     var bestRow    =
          -1;
               var bestNorm = 0f;
...
; row < dimensions;  row    ++)    {
            var   norm = 0f;
              ...
              for (var column    = 0; column < dimensions    ; column++) {    var value = covariance[(row * dimensions) + column];
                norm   += value  *    value    ;    }
          if (    norm > bestNorm) {
       bestNorm  =
norm;
...
axis
        ...
        covariance .    Slice(  
	bestRow   * dimensions, dimensions).  CopyTo( axis)   ;
             Normalise(axis);

	for   (  var iteration   = 0; iteration < Iterations; iteration  ++) { next.Clear();
            for (var row = 0; row < dimensions; row  ++) { for    ( var column =    0; column   <  dimensions; column++) {
                    next[row] += covariance[ (
              row * dimensions) + column  ] *   axis[   column]   ;
                }
   }
...
next.CopyTo (  axis)   ;  }
             ...
             static bool  Normalise(Span   <float  > vector) { var length = 0f;
        foreach
(var   value in vector )  {
length    += value    * value ;
        }
