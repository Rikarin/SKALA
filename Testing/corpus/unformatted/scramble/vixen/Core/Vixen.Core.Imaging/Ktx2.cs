...
for
  ...
  var layout   =
      ...
      = layout.    Allocate(0);
       for (    var
level   =    0 ; level  < layout.   LevelCount   ; level ++) {
...
public  static Ktx2Layout   ReadLayout  (ReadOnlySpan<byte    >   file) {
       if (file.
             Length  < HeaderLength ||    !file    [..Identifier   .Length]. SequenceEqual(Identifier)) {
               throw  new Ktx2Exception(  "This is not a KTX2 file: the twelve-byte identifier does not match." );
       } 
...
var
  ...
  var    expected  = format
    .   LevelSize(levelWidth, levelHeight   ,   levelDepth  )   *    layerCount    *    faceCount;
...
( offset < indexEnd    ) {
          ...
          levels    [level] = new    ( level  ,   levelWidth, levelHeight    , levelDepth ,  offset   , length)    ; }
               ...
               return new  (format    , width
    ,  height, depth,  layerCount  , faceCount   , levels    );
              }
...
return texture  ;
   ...
   length = LayoutLength(    head )   ;
    ...
    length   ];
        head.CopyTo (  full   , 0    )    ;
 await Fill   (stream
			,   full.AsMemory(HeaderLength   ), cancellation).   ConfigureAwait(false )  ;
...
described =  layout.   Levels[level];
        if ( destination.Length < described
   .Length) {
 ...
 nameof  (destination)
    ...
    /// <summary>Reads exactly as many bytes as the buffer holds, or says the file was short.</summary>
       static async    ValueTask Fill( Stream    stream,   Memory   <  
byte> buffer  , CancellationToken cancellation   ) {
       var read = 0;
    ...
    /// </remarks>
   static  int GreatestCommonDivisor(int  a
              , int  b) => b == 0    ? a :  GreatestCommonDivisor (b, a  % b) ;
    /// <summary>Rounds up to a multiple.</summary>
        static int AlignUp(  int value,    int    alignment  )   => (value +  alignment   - 1) /
         alignment *  alignment;
