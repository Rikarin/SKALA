...
/// <summary>A file that is not KTX2, or is KTX2 in a way this does not implement.</summary>
             public sealed class Ktx2Exception    (   string message) : Exception   (
             message);
...
/// </remarks>
  public   static class  
    ...
    public static ReadOnlySpan   <byte>  Identifier =>    [    0xAB
 , 0x4B,  0x54,    0x58,    0x20  , 0x32, 0x30, 0xBB,    0x0D,  0x0A,    0x1A, 0x0A];
 ...
 public const  int LevelIndexEntryLength  = 24   ;
    ...
    public static byte[] Write (TextureData   texture) {
        ...
        var vkFormat = VkFormats.From
(    texture. Format   );
          var descriptor =
DataFormatDescriptor   .    Build(texture.Format);
            var levelIndexOffset = HeaderLength   + (  Identifier.Length -   Identifier  .Length);   var indexLength = texture.LevelCount * LevelIndexEntryLength;
		var descriptorOffset =    HeaderLength +
      indexLength   ;
 ...
 // run in. The padding between them is mipPadding and it has to be there — see the remarks
        // on LevelAlignmentOf.
        var alignment = LevelAlignmentOf    ( 
     texture.Format )  ;
...
Identifier.CopyTo (span );
  ...
  Identifier.Length..]    ;
        ...
        BinaryPrimitives   . WriteUInt32LittleEndian  (header, vkFormat);  BinaryPrimitives.   WriteUInt32LittleEndian(header[ 4..], TypeSizeOf(  texture.    Format)  )   ;
             BinaryPrimitives. WriteUInt32LittleEndian  (header   [8..   ],   (uint)texture.Width   ); BinaryPrimitives.   WriteUInt32LittleEndian(header[12..], (    uint)texture.Height   );
  ...
  // Zero rather than one for a 2D texture: the specification says pixelDepth is 0 when the
        ...
        // texture is not 3D, and a reader that saw 1 would build a 3D texture one slice deep.
            BinaryPrimitives.  WriteUInt32LittleEndian (header[ 16..]    , texture.Depth > 1  ? (    uint)  texture.Depth :   0); BinaryPrimitives.WriteUInt32LittleEndian( header [20..], texture.LayerCount >    1    ? (uint)texture  .LayerCount   : 0)  ;
  BinaryPrimitives.WriteUInt32LittleEndian( header  [24..],   (uint)texture.FaceCount); BinaryPrimitives.WriteUInt32LittleEndian(   header    [  28.. ], (  uint   )  texture.LevelCount);
          BinaryPrimitives.WriteUInt32LittleEndian(   header[32..]    , 0); BinaryPrimitives    .    WriteUInt32LittleEndian   (  header   [    36..], (uint)descriptorOffset)  ;
         BinaryPrimitives    .WriteUInt32LittleEndian( header[   40..]
      ,  (  uint)descriptor.    Length);
       BinaryPrimitives.WriteUInt32LittleEndian(   header[44..   ]    ,
0);
   BinaryPrimitives   .WriteUInt32LittleEndian(header[    48..]
       , 0);
BinaryPrimitives.  WriteUInt64LittleEndian(
         header[52..],  0) ;
        BinaryPrimitives.    WriteUInt64LittleEndian(header [60..
]    , 0) ;
...
for (var level =    0 ;   level <
    ...
    var  described = texture.Levels[level];
            ...
            var entry =   span [ (levelIndexOffset + (level *    LevelIndexEntryLength) )..]; BinaryPrimitives    . WriteUInt64LittleEndian(entry  , (ulong)offsets[level   ]);
           ...
           texture.Level(  level).    CopyTo(span[
       offsets[level    ]..   ]    ) ;
             }  
      descriptor.  CopyTo(span
[   descriptorOffset..   ]) ;
       return file
             ;
               }
...
public static  int   LayoutLength    (  ReadOnlySpan<byte    > head) { if (head .Length < HeaderLength || !   head[..Identifier    .Length]    .SequenceEqual   (Identifier))   {
               throw 
...
HeaderLength  + (levelCount * LevelIndexEntryLength)    ;
    ...
    /// <exception cref="Ktx2Exception">It is not KTX2, or uses something this does not implement.</exception>
             ...
             if (file.  Length <  HeaderLength || !file[..Identifier.Length].
...
var vkFormat =   BinaryPrimitives .    ReadUInt32LittleEndian  (header); var width    = (int   )BinaryPrimitives.  ReadUInt32LittleEndian(    header[8..]);
          var height
             = (int )  BinaryPrimitives   .ReadUInt32LittleEndian (    header   [12..] );
                var depth =   Math.Max    (1, (
  int)BinaryPrimitives.   ReadUInt32LittleEndian( header    [16..]));
    var layerCount  =  Math    .Max  (1,  (int)BinaryPrimitives.   ReadUInt32LittleEndian(    header [ 20..])) ;   var faceCount  =  Math   .Max(1    , ( int   )    BinaryPrimitives .ReadUInt32LittleEndian(header[24..]    ));
     var levelCount  = Math   .  Max(  1  , (int)BinaryPrimitives.ReadUInt32LittleEndian(
             header[   28..]))  ;
  ...
  for (
         var    level = 0; level <    levelCount;   level++    ) {
            var entry = file[   (HeaderLength + (level * LevelIndexEntryLength    )  )..]   ; var offset = (   long   )  BinaryPrimitives   .    ReadUInt64LittleEndian    (    entry);
            var length =
...
(    levelWidth, levelHeight, levelDepth) =    MipChain.ExtentOf(width    , height  ,   depth, level    );
      var expected  =   format.LevelSize
      (    levelWidth, levelHeight  , levelDepth    ) *  layerCount * faceCount;
               if  (  length !=   expected)    { throw new Ktx2Exception(
         ...
         if   (offset <
           ...
           levels[level] =  new   (    level, levelWidth, levelHeight , levelDepth    , offset, length); }
			...
			return new(format    , width, height,    depth
    , layerCount,    faceCount, levels  );
    }
...
texture = layout.  Allocate(firstLevel);
        for (var   level =  firstLevel ; level   < layout.    LevelCount    ; level++)    { var    described =    layout .Levels[  level];  
               if (described.  Offset + described.Length >    file .Length    ) {
               throw
new  Ktx2Exception($"Level {level} points outside the file.");
     }
      file
.Slice((int)   described.Offset, (  int   )  described.Length    ).CopyTo  ( texture .LevelSpan(level - firstLevel)    ) ;
  }
...
ThrowIfNull(stream) ;
         var  head   = new    byte    [HeaderLength   ];   await Fill(stream,   head, cancellation).   ConfigureAwait(false )  ;
        var length    = LayoutLength  (
          head) ;
        if  (length == HeaderLength)   { return ReadLayout    (  head)   ;
			}
...
(
    Stream stream,    Ktx2Layout layout    ,
        int   firstLevel,
         CancellationToken cancellation   =  default ) {
        ArgumentNullException.ThrowIfNull   (stream); ArgumentNullException.  ThrowIfNull(layout );

         if (    !   stream .CanSeek)
          {
		throw new ArgumentException(    "A mip tail is read by seeking past the header to the level data, so the stream has to seek. "
             + "Read the whole file into memory and use ReadTail if it cannot."    , nameof   (   stream)
  );
              }
var  
run =    new byte[layout.TailLength   (   firstLevel)]  ;

   stream.Seek  (layout.DataOffset    , SeekOrigin.Begin); await Fill(    stream,    run, cancellation).ConfigureAwait   (    false  );   
    ...
    for
        ...
        var described  
 =    layout.  Levels[   level] ;
            var at   = (int)    ( described.Offset -    layout
            .   DataOffset  );
       
            run    .   AsSpan(at, (int
       )    described   . Length   ).CopyTo(texture.LevelSpan(level - firstLevel )  );
       }
   ...
   Stream
    ...
    Ktx2Layout 
    layout,
       int level, Memory <byte> destination,
               CancellationToken
...
if ( destination.  Length < described.Length) {
    throw new ArgumentException    (
               ...
               stream   .   Seek(described  .Offset,   SeekOrigin.Begin); await Fill(   stream   , destination[..(int   )described.Length  ],    cancellation)  .ConfigureAwait(   false  );
  ...
  var read = 0 ;
        while (read <   buffer.    Length) { var got = await   stream. ReadAsync   (   buffer[   read..   ]  , cancellation).  ConfigureAwait(  false);
            if (got ==    0) {  throw new Ktx2Exception(
...
;
    ...
    ///     </para>
    /// </remarks>
     static int LevelAlignmentOf(PixelFormat  format) {    var    size  =   format .BlockSize(  )   ;
 ...
 static int AlignUp(int value, int
        alignment)   =>   (value    +  alignment -    1) / alignment  * alignment;
    /// <summary>
...
TypeSizeOf(   PixelFormat format   ) =>
        format.IsCompressed()
           ? 1u
 : format switch {   PixelFormat.  Rgba16Float    or PixelFormat.  Rg16Float or PixelFormat.R16Float
             or   PixelFormat  .Rgba16UNorm
=> 2u,
                PixelFormat.Rgba32Float or PixelFormat.Rg32Float or PixelFormat .R32Float or PixelFormat.Rgba32UInt  or  PixelFormat  .    R32UInt => 4u   ,
               _ => 1u
