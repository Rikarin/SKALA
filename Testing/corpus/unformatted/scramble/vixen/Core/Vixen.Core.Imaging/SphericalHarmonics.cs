...
/// </remarks>
  public sealed  class
   ...
   <Vector3>  Coefficients => coefficients  ;
    ...
    public SphericalHarmonicsL2(   ReadOnlySpan  <Vector3> coefficients   ) { if (coefficients.Length !=  Count) {
 ...
 () => coefficients    = new Vector3[Count  ];
    ...
    /// </remarks>
			public static   SphericalHarmonicsL2    Project(TextureData cube, int   level =
    0) {
    CubeMap
         .   Require(cube    ,   nameof(cube))  ; 
        var  size   = cube.Levels[ level]
 .    Width; 
                var coefficients    = new Vector3  [Count];  Span<float> basis = stackalloc float[Count  ] ;
    for (   var face   =  0
              ; face   <   CubeMap.Faces;    face   ++) {
 for (var y  =   0; y <  size; y++)  {
         for (var x   =   0  ; x <  size; x++   ) {
  var    radiance =  CubeMap.ReadTexel(cube,  level,  face , x, y)    ;
       var solidAngle = CubeMap  .SolidAngleOfTexel(x    , y,
size    ) ;
         Evaluate(CubeMap   .DirectionOfTexel(face
   , x, y, size), basis   );
...
coefficients   [index ] +=    radiance * (   basis[index    ]  * solidAngle); }
 ...
 return  new( coefficients); }
    ...
    /// <param name="basis">Nine floats to fill.</param>
         public static void Evaluate (Vector3 direction   , Span   <float    > basis  ) {   var   (x , y, z) = ( direction.    X,  direction. Y, direction   .Z) ;
...
1f   )   ;
 basis[7] = 1.092548f * x *    z;
    ...
    /// </remarks>
    public Vector3    Irradiance(Vector3   normal ) {    Span <    float> basis =    stackalloc float[Count];  
    Evaluate(normal , basis)   ;
   var total =  Vector3.Zero;
 for (var index  = 0 ; index < Count; index++) { total   += coefficients[index ] * (    basis[index]   * BandFactors    [ index]    );
      }
			return
total;
    }
    ...
    /// </summary>
		static readonly float[  ] BandFactors    =  [ 1f ,
                2f /    3f,
    2f  / 3f   , 2f /    3f,
        0.25f  , 0.25f,
0.25f, 0.25f ,  0.25f
