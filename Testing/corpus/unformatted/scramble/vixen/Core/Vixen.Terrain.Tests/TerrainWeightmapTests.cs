...
public sealed  class TerrainWeightmapTests {
      static TerrainDescription Shape  =>   TerrainDescription.  Default with {
            TileSamples   =    32, TilesX = 2, TilesZ = 2, MetresPerQuad = 1f   ,   MinHeight =   -100f,   MaxHeight   = 100f
		}   ;
          static Terrain   Built()
{
       var
        terrain    =   new Terrain(Shape);
    terrain.Weights.AddLayer    (   "Grass") ; terrain.Weights.AddLayer("Rock");
return terrain;
           }
      ...
      [Fact]   
		...
		terrain,
    1  ,
           TerrainBrush .Default with { Radius = 10f,    Strength = 1f }, new(new   (    30f, 30f)),   
            amount: 255 )   ;
 ...
 .Equal(bytes. Length    , TerrainWeightmap.Export(    terrain ,   1   , bytes));
			var round =  Built   (    );
           TerrainWeightmap  .Import   (round, 1, bytes ,    terrain  .Description.SamplesX,   terrain.    Description
          .SamplesZ);

          for (  var z = 0    ;    z < terrain.Description.SamplesZ; z++) { for (var x = 0 ; x < terrain.Description    .SamplesX; x++   ) {
    ...
    public void
           An_import_restores_the_invariant_rather_than_trusting_the_file( ) {
               var terrain = Built () ; var mask = new   byte [terrain.Description.SampleCount];
        
         // A mask that means nothing about the other layers: half of it fully covered.
               for (    var  z = 0; z <  terrain .    Description.SamplesZ; z ++  ) {    for ( var    x = 0 ; x < terrain.Description.SamplesX / 2    ; x++)    {
           mask   [(z * terrain.Description .
               SamplesX) + x ] =  255  ;
     } }
    TerrainWeightmap.   Import(terrain, 1   , mask, terrain.Description.SamplesX , terrain.   Description   .SamplesZ)    ;
            
            Assert   .   Equal(255, terrain.Weights   .WeightAt
         (1, 5    , 5));
        Assert . Equal(    0   , terrain.  Weights  .WeightAt (0, 5 , 5  )   )    ; Assert  .Null(terrain.   Weights. Verify())  ;
    }
...
var    terrain =    Built    ();    var mask  = new byte[64 * 64];
...
for (var x = 0; x < 64 ; x++    ) {
           ...
           TerrainWeightmap.   Import    (  terrain, 1, mask   ,  64   ,    64
              ...
              Assert.    Equal(   0   , terrain.Weights .WeightAt(   1,  0   , 0 )); Assert.Equal(   255,  terrain.Weights.    WeightAt(1, terrain.Description.   SamplesX - 1, 0  ))  ; 
		Assert.Null(terrain.  Weights.  Verify   () );    }

    [Fact   ] public   void A_mask_that_does_not_match_its_size_is_refused() {
        var terrain =   Built   (  );
		
Assert.
 Throws<  ArgumentException>   ((   )    => TerrainWeightmap.Import(terrain, 0,  new   byte[10   ]  ,    64,  64)  ) ;
        Assert.Throws
 ...
 (
       ...
       // --- The layer asset ----------------------------------------------------
    [ Fact   ] public void A_layer_carries_its_ground_beside_its_channel() {
    var terrain    = new   Terrain ( Shape)   ;
          var gravel    = new TerrainLayerDescription(   "Gravel",
         Albedo:
    "Textures/gravel-albedo",
     Surface:   "Textures/gravel-orm", TilingMetres   : 2f,
	Blend:  TerrainLayerBlend .Height    ,    PhysicsMaterial: "Materials/gravel"  
);
             
              var index
= terrain .  Weights.AddLayer(gravel);
             Assert.
Equal(  gravel   ,  terrain    .Weights  .    LayerOf( index));
    ...
    /// </remarks>
             [Fact]
             public    void Reassigning_a_layers_ground_keeps_what_was_painted_with_it(  )   {  var terrain  = Built  (    );

        TerrainPaint
.  Paint (    terrain, 1, TerrainBrush   .Default with { Radius    = 8f }   ,    new( new(   30f, 30f)),  255);
var  painted  = terrain.Weights    .  WeightAt(1, 30,   30);   
        ...
        Assert.    Equal("Gravel",  terrain  .    Weights.Names [1    ]); Assert.Equal( painted, terrain.Weights.WeightAt(1  , 30   , 30    ));
    }
...
public void Coverage_is_what_the_panels_bar_draws   ()   {
	var terrain =  Built    ();
               ...
               Assert.Equal(1f  ,  
       terrain.   Weights  .CoverageOf  (0), 3   );
        Assert  .Equal(  0f,  terrain.    Weights.CoverageOf(1), 3)   
;  
         
            TerrainPaint.Flatten(   
            terrain   , 1,
               TerrainBrush   .  Default with { Radius =    1000f,    Strength = 1f, Falloff  = 0f }    , new   (new (30f   ,    30f)),
          target: 1f
            )    ;
		Assert.Equal(1f,  terrain.Weights.  CoverageOf   (    1), 2)
  ;
                Assert.Equal(
0f    ,  terrain.Weights.   CoverageOf(0), 2  )    ;
     }

    /// <summary>The ground under a foot is one layer, not a blend.</summary>
	[Fact]    public void The_dominant_layer_is_what_a_footstep_reads   (   )    {
  var
        terrain =    Built(  );

 Assert    .Equal(
              0 ,    terrain.Weights    .DominantAt   (30, 30    ));
                
            TerrainPaint    .  Paint(terrain
    , 1, TerrainBrush  .Default    with {    Radius    =   6f  }    ,   new(new  ( 30f, 30f))    ,    255);
			Assert.   Equal(1  , terrain. Weights.DominantAt    (  30 ,
30)  ) ;   
   }
    ...
    /// <summary>A tile's quads carry the layer that claims them, for the collision shape.</summary>
        [Fact] public void    A_tile_of_quads_carries_the_ground_each_one_is(    )   {
      var terrain    =  Built (  ); var quads = terrain.Description.   TileQuads;
         var  materials   = new sbyte[    quads *
        quads]  ;  
                
         Assert.  Equal(   materials.Length
,  terrain.Weights.FillCollisionMaterials  (0, 0, materials)  );
        ...
        TerrainPaint.Paint (
            terrain,
              1,
       TerrainBrush.
      Default with { Radius =   8f, Strength   = 1f,  Falloff = 0f },
            new(    new(10f, 10f)),  amount: 255
      );
            terrain.
		Weights.FillCollisionMaterials(0, 0, materials) ;
        Assert.   Equal   (1,  materials[(10   * quads    ) +  10])
           ;
        Assert.  Equal  (0
, materials[  (28 * quads) + 28])   ;
    }
    ...
    /// </remarks>
              [Fact]  public void    A_quad_takes_the_layer_with_the_most_weight_over_its_four_corners(   )    {
         var   terrain = Built ();
 // Three corners of the quad at (10, 10) painted, one not.
        terrain.Weights.SetWeight(1, 10 , 10    ,  255   ); terrain.    Weights  .SetWeight(  1, 11, 10, 255 );  
            terrain.Weights .SetWeight(1  , 10  
,  11, 255);
 var quads = terrain.   Description.   TileQuads;
		...
		Assert
             .Equal(1 , materials[(10 * quads) + 10 ] );
      }
    [Fact]
			public void A_terrain_with_no_layers_says_so_rather_than_claiming_the_first_one  (   ) {
  var terrain = new   Terrain (Shape);    var quads  = terrain.Description.TileQuads;
       var
             materials = new sbyte   [    quads * quads ] ;   
             terrain.Weights.
			FillCollisionMaterials(0, 0 , materials );
// ⚠ −1, not 0: zero is a layer index and would claim every quad is the first ground.
 ...
 . All(materials    , material => Assert. Equal   (-1, material)   )   ;
               Assert. Null(   terrain.Weights
.GroundAt    (10,  10    ));
            }
      
         [    Fact    ]
          public void The_ground_at_a_place_is_the_layer_asset_that_claims_it() { var  terrain = new Terrain(Shape);
                terrain  
     .   Weights.AddLayer(TerrainLayerDescription.Of("Grass") with    {    PhysicsMaterial  = "Materials/grass" }   ) ;   
          terrain.Weights.    AddLayer(TerrainLayerDescription.   Of(   "Gravel"
   ) with { PhysicsMaterial    = "Materials/gravel" })   ;
    
		Assert.
     Equal("Materials/grass"    , terrain .Weights.GroundAt(10, 10)!  .Value.    PhysicsMaterial)  ;
                
        TerrainPaint.   Paint  (
         terrain,
            1,
   TerrainBrush.Default with { Radius    = 6f, Strength    =   1f, Falloff  = 0f }    , new    (new(  10f, 10f)) ,
   amount:   255

              );

        Assert    .  Equal ("Materials/gravel", terrain    .   Weights.GroundAt
(10, 10)   !  .  Value. PhysicsMaterial);
}
           
            [   Fact    ] public void Too_little_room_for_a_tiles_materials_is_refused( ) {
   var terrain = Built() ;
...
Fact   ]
   ...
   new    TerrainLayerDescription( ""). Validate(  ),
	StringComparison.Ordinal
             ...
             Assert . Contains ( "not a distance",
       ...
       // ⚠ A height blend with nothing to read the height from degrades silently to a weight blend,
        // which is the class of failure reported as "the height blending does not work".
			Assert .Contains(
            "no surface texture",    (   TerrainLayerDescription.Of( "Gravel"    ) with { Blend = TerrainLayerBlend .   Height  }).Validate    (),
      StringComparison.Ordinal
          );
}
    }
