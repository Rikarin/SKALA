...
/// </remarks>
    ...
    Gen.Select(  Gen.Int
             [2, 5]  , Gen.Int[ 1   , 4]  , Gen.Int    [  0   , 999 ].  Array[1    , 12])   .Sample(shape  =>    {
      var (rows, cells, mutations  ) =   shape   ; 
           var spec = new PanelSpec(rows  , cells)    ; using var  incremental    =    new LayoutTree { PointScaleFactor  = 2f }    ;
         var   incrementalRoot = spec.Build(incremental)  ;   incremental.CalculateLayout(  incrementalRoot,  320.5f    , 240.25f  , Direction.   Ltr) ; 
		
   foreach (var    mutation in mutations) {
                    spec.Mutate(mutation);
              spec .Apply( incremental, incrementalRoot, mutation
)  ;
                    incremental    .CalculateLayout  (incrementalRoot    , 320.5f, 240.25f
, Direction.Ltr    )    ;
                    using var    cold = new    LayoutTree   { PointScaleFactor =    2f };
                var
              coldRoot = spec. Build (cold);
   cold
        .    CalculateLayout(coldRoot , 320.5f, 240.25f    ,  Direction   .  Ltr);
                    AssertSameLayout(incremental,
incrementalRoot, cold    , coldRoot, "root") ;
            }
    }
);    }
	[Fact]
public void A_subtree_that_only_moves_is_still_rounded_against_its_new_offset (    ) {
    ...
    // rounded edge depends on where it now sits.
     using var tree = new LayoutTree   { PointScaleFactor =    2f }; var   root =   tree .CreateNode(   );
tree. SetFlexDirection (root, FlexDirection    .Column  )    ;
      tree    .SetDimension    (root, Dimension    .   Width, StyleLength
                .   Points  (  200f  )    );
...
tree.    CreateNode (  )    ;
  ...
  var inner = tree. CreateNode( )  ;
        ...
        ( group, inner);
               ...
               tree   .CalculateLayout(root,   200f
 ,    400f,  Direction.Ltr)  ;
   ...
   // and therefore the distance between its two rounded edges.
     ...
     // is touched by it.
tree.SetDimension (  spacer,   Dimension.Height, StyleLength  .Points(10.25f))    ; tree    .    CalculateLayout (root,  200f   ,   400f, Direction   .Ltr)    ;

   using var cold =   new LayoutTree
              {  PointScaleFactor  = 2f } ;
        var coldRoot =    cold.CreateNode    ()    ;
    cold.SetFlexDirection   ( coldRoot  ,    FlexDirection    .Column)    ; cold.SetDimension( coldRoot, Dimension. Width   , StyleLength .  Points (200f  ));
			var  coldSpacer = cold  .CreateNode(); cold.SetDimension  ( coldSpacer, Dimension.Height , StyleLength  .  Points(10.25f)    );
                cold.  AddChild(coldRoot,   coldSpacer); var coldGroup = cold  .    CreateNode ( )  ;
        cold.SetFlexDirection  (coldGroup    ,    FlexDirection.Column)   ; cold   .AddChild(coldRoot,    coldGroup);
 var
coldInner =   cold.CreateNode()  ;   
  cold.
SetDimension  (coldInner, Dimension.Height, StyleLength.   Points (10.25f));
        cold.AddChild    (
           coldGroup   , coldInner);
       cold.CalculateLayout( coldRoot    , 200f, 400f, Direction   .
  Ltr)  ;
        Assert   .  Equal(cold.GetTop(coldInner), tree.GetTop( inner)); Assert.Equal(    cold.    GetHeight(coldInner)  , tree   .  GetHeight(   inner));
       Assert .NotEqual    (before, tree.  GetHeight  (inner)   )
     ;
       }
   
    [Fact] public void    Adjacent_boxes_do_not_round_into_a_seam  () {
 ...
 using var tree   =    new LayoutTree { PointScaleFactor =   2f };
         ...
         tree
               . SetFlexDirection(   root, FlexDirection .Column );
   tree  .SetDimension  (   root, Dimension
               .  Width, StyleLength.Points(100f  ));
        
     var boxes =  new LayoutNodeId
[3 ] ;
for    (var i   = 0
             ;   i   <   boxes.Length;    i++ ) {
            boxes[i ] =  tree   .CreateNode(  );
            tree.SetDimension(
		boxes   [i], Dimension. Height, StyleLength    .Points(33.3f   ));
    tree .AddChild    (root,    boxes
   [   i]);
	}
   tree  .CalculateLayout(root , 100f
          ,   200f  ,   Direction    .Ltr);
 for    (  var i
= 1  ; i < boxes.   Length; i++) {
        var    previousBottom = tree.GetTop
           (  boxes[i - 1]  )  +   tree. GetHeight(boxes[    i -  1  ])    ;
                Assert   .   Equal    ( previousBottom   , tree
             .GetTop(boxes[i]    ));
              }
         }
    ...
    /// <summary>A panel of rows of cells, and the current fractional sizes of each.</summary>
             ...
             readonly
      float[]   cellWidths =   CreateSizes(rows * cells, 17.3f, 4.7f);
        readonly float
      [] rowHeights =    CreateSizes(rows    , 21.4f    , 3.3f);

          public    LayoutNodeId Build(
LayoutTree tree  ) {
             var root = tree.CreateNode(); tree   .SetFlexDirection(root   ,   FlexDirection.   Column   )    ;
            tree.SetPadding    (root,
    Edge.All, StyleLength.  Points(    3.5f  ))  ;
              for (var r = 0    ;   r < rows; r ++
      ) {
          var row   =  tree.    CreateNode();  
      tree.SetFlexDirection(   row,
FlexDirection.Row);
                tree.  SetDimension(row  , Dimension
.Height , StyleLength.Points(  rowHeights[  r    ]))   ;
                tree.AddChild(root
            ,  row);
    ...
    var   cell = tree. CreateNode ()  ;
        tree.    SetDimension(cell, Dimension.Width , StyleLength.Points(  cellWidths [(r * cells   ) + c]   )) ;   tree.   SetMargin(cell,   Edge.Right , StyleLength.  Points( 1.25f)) ;
  ...
  return    root;
     ...
     (int  mutation ) {
                if (mutation   % 2 == 0) {
 cellWidths[  mutation % cellWidths.Length
        ] = 11f + (mutation  % 23 *  0.37f);
        } else  {
...
public   void Apply(LayoutTree tree    , LayoutNodeId   root, int   mutation    ) {
  if (   mutation % 2    ==  0)
         ...
         var
   cell =   tree.    GetChild(tree.   GetChild(root   , index / cells)   , index % cells);
       tree.   SetDimension   
( cell    ,   Dimension.Width   , StyleLength.   Points(   cellWidths [index]   ) );
                return;
  }
          ...
          static float[  ] CreateSizes(
                int count, float start  ,   float    step) {
  ...
  sizes[i
    ...
    static void    AssertSameLayout(LayoutTree  left, LayoutNodeId leftNode, LayoutTree right
, LayoutNodeId  rightNode, string path) {
               Assert.    Equal  (   right .GetLeft(rightNode    )   ,
              left    .GetLeft(    leftNode));
         Assert  .Equal(right.  GetTop(    rightNode    ), left  .GetTop(leftNode ) )  ;
         Assert    .Equal  (right.GetWidth(rightNode)  , left.GetWidth   (leftNode))   ;    Assert.Equal(  right . GetHeight(rightNode), left .GetHeight(leftNode));
           Assert.Equal(right   .  GetChildCount   (rightNode)   , left.    GetChildCount  (   leftNode))   ;
        for (  var i =   0    ;   i  < left   .GetChildCount   (leftNode); i    ++  )  {   
            AssertSameLayout(left    ,
left.  GetChild(leftNode,  i)  , right   , right.   GetChild(    rightNode, i),    $"{path}/{i}");
