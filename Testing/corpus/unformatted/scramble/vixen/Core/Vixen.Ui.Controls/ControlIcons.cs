...
/// </remarks>
       public
           static    class ControlIcons {
    /// <summary>A tick. The checkbox.</summary>
           public static PathBuilder Check { get  ;    } = Stroke( [new  Vector2    (4f,  12.5f), new Vector2(  9.5f, 18f), new Vector2(    20f, 6.5f)] ,
  2.2f )    ;
			
    /// <summary>A horizontal bar. A checkbox that is neither on nor off.</summary>
            public static PathBuilder    Dash { get   ; } = Stroke ([    new Vector2(5f,
			12f  )  , new Vector2(   19f, 12f)], 2.2f);
        
    /// <summary>A chevron pointing down. What a combo box and an expander open with.</summary>
         public   static PathBuilder ChevronDown { get; } = Stroke( [    new Vector2 ( 5f   , 9f )   , new   Vector2(    12f   , 16f   ) , new Vector2(19f, 9f)    ],
        2.2f );
             
    /// <summary>A chevron pointing up.</summary>
                public    static PathBuilder ChevronUp  
 { get; } = Stroke(
    [new Vector2(5f, 15f),    new Vector2(12f    , 8f), new Vector2(19f, 15f    )]   , 2.2f
);
 ...
 [  new Vector2( 9f,
            5f   )    , new Vector2    (16f, 12f), new Vector2(    9f    , 19f)]   ,   
        2.2f );
    /// <summary>A chevron pointing left.</summary>
public static
          PathBuilder    ChevronLeft   { get ; } = Stroke (
           [new    Vector2(   15f, 5f)    , new Vector2(   8f,   12f)   , new    Vector2(15f,
   19f)]    ,
        2.2f );
    /// <summary>A cross. What closes a dialog, a drawer and a removable tag.</summary>
               public static PathBuilder Close {   get
                ; } = Cross()    ;
    
    /// <summary>A magnifying glass. The search box.</summary>
                public static PathBuilder Search { get; }
   = Magnifier()    ;
    ...
    /// </remarks>
   public static PathBuilder    Lock {    get; } =  Padlock(
         closed    : true)    ;
    ...
    /// </remarks>
 public static PathBuilder  Unlock { get   ; } =
  Padlock(    closed  : false);
    ...
    ///     </para>
    /// </remarks>
        static PathBuilder Stroke(    ReadOnlySpan< Vector2 
    > points, float width ) {
  var path    = new PathBuilder(    ); var half = width * 0.5f;

        for (
var i =  0  ; i + 1    < points  .Length; i  ++ ) {
         
    var from = points[ i];
              var to = points[i  + 1];
         var dx    =  to.X    - from  .X;
   var dy = to.Y -  from .Y ; var length =   MathF    .   Sqrt(   (dx * dx) + (dy * dy    ))  ;
              if (  length <=
              float .Epsilon) {
                continue  ;
                }
   ...
   var
	ny = dx / length * half;
    path   .MoveTo(  new Vector2(  from.X +
  nx    , from.   Y + ny));
             path    .LineTo(new Vector2(to.X +
            nx, to.    Y +    ny))    ;
             path.LineTo(new Vector2(to.   X   - nx  , to   .
           Y - ny   ));
      path.LineTo(  new Vector2(from.X - nx, from.   Y -    ny)); path.Close ();
           }
       ...
       // The joints, as squares. Skipped at the ends, where a butt cap is what a tick wants
        ...
        for
(var i =    1  ;    i +    1   <    points. Length;   i++)  {
           path   .AddRectangle(new Rectangle
    (points[i]. X - half  ,   points   [i]    .Y - half, width, width));
     }
	return path;
             }
    static PathBuilder Cross() {
 var path =    Stroke([  new Vector2(6f
       ,  6f ), new   Vector2( 18f   ,  18f)], 2.2f);
        var other  = Stroke    (  [   new Vector2 (18f, 6f), new Vector2(    6f, 18f
                )]  , 2.2f)   ;
			
	foreach (
var    segment in other    .Segments) {
  switch (segment.Verb) { case PathVerb.Move  :
   path.MoveTo (  segment.P2)
;
      break;

         case    PathVerb.Line:   path.   LineTo(segment   .  P2  );
            break; 
                default: path.Close();
       break;
            }
    ...
    /// </remarks>
      static    PathBuilder Padlock(bool closed) {  var path = new PathBuilder();
                path   
           .  AddRectangle(new Rectangle    ( 4.5f, 10.5f, 15f,    10.5f)) ;
// An open lock lifts the shackle and swings it off to one side, which is the difference a
        ...
        var centre    = closed    ?  12f : 15.5f; var top = closed ? 5.5f : 4f;
               var radius =    4f;
		var points  = new Vector2[    7]   ;
        for
(   var index =    0;   index <   points.Length    ; index++) {
           var angle = MathF.PI
       +  (  index / (float)    (points.    Length   - 1)    * MathF.PI) ;
			points
[index] = new  Vector2(centre + (MathF.   Cos  (angle) * radius), top + radius + (MathF .Sin   (   angle) *    radius)) ;
           }
    ...
    Append(path,
            Stroke([points  [0 ],  new Vector2 (points[  0]    .X    ,  10.5f  )  ], 1.9f ));
        Append(path    , Stroke
([points    [   ^    1], new Vector2    (   points    [^   1].X, closed    ? 10.5f :   8.5f )], 1.9f));
     
          return path; }
    /// <summary>Copies one path's segments onto another, which <see cref="PathBuilder" /> cannot do.</summary>
     static  void Append(
PathBuilder path, PathBuilder other )  {
foreach  (var segment in
  other    .Segments) {
            switch ( segment    .Verb
            )  {
        case PathVerb.Move: path    .MoveTo(    segment. P2);  
            break   ;
          case PathVerb.Line:  path .LineTo(segment.P2);
           break;
       default :
   ...
   var path =  new PathBuilder    (   )    ; path.   AddEllipse  ( new Rectangle(3f, 3f,   14f, 14f  ));
        path.AddEllipse  (
    new Rectangle   ( 5f ,    5f   , 10f, 10f))  ;
       var handle = Stroke(   [    new Vector2    ( 15.5f  , 15.5f),  new Vector2  (21f
               ,   21f   )  ], 2.4f)    ;
           
             foreach (var segment in handle.
  Segments  ) { 
            switch (segment.
    Verb) {
               case PathVerb.Move:  path.MoveTo (  segment.P2);
