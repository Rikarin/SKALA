...
public static IconPaint None => default;
    ...
    public
static IconPaint Foreground { get;  } = new()  { Kind    =  IconPaintKind.Foreground,   Token = string.Empty };
...
.ThrowIfNullOrEmpty   (token    );
        return
new() { Kind = IconPaintKind.  Token    ,  Token = token };
           }
...
/// </remarks>
    ...
    IconPaint Fill   , IconPaint Stroke =    default    ,  
float Width = 1.5f,
...
public  sealed class  IconArt {
 ...
 public IReadOnlyList<IconPath> Paths { get  ; }
    ...
    /// <exception cref="ArgumentException"><paramref name="viewBox" /> has no area.</exception>
      public IconArt   (Rectangle
       viewBox ,  IReadOnlyList   <IconPath   >  paths ) {
...
throw new ArgumentException(
    ...
    /// <param name="paths">Its paths, back to front.</param>
public
 IconArt(    params IconPath[]  paths) : this(new Rectangle(0f, 0f    , 24f   , 24f), paths   ) { }
    ...
    ///     redrawn.
    /// </remarks>
    public static IconArt Of (  PathBuilder geometry, PathFillRule  rule =
            PathFillRule.  NonZero) {
               ArgumentNullException.ThrowIfNull   (geometry); return new(    new IconPath (  geometry  , IconPaint   .Foreground, FillRule    : rule));
    }
    ...
    /// <param name="rule">How its inside is decided.</param>
    /// <returns>The art.</returns>
               public static   IconArt Of(PathBuilder   geometry,  Color4 tint
, PathFillRule rule = PathFillRule.NonZero   ) {
			ArgumentNullException .ThrowIfNull(   geometry); return new   (new IconPath    (geometry    ,  IconPaint .Of(   tint )  , FillRule:  rule)    );
    } }  
