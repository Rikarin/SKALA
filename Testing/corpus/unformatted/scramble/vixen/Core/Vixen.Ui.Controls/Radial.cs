         ...
         // SPDX-License-Identifier: Apache-2.0
              ...
              using System.Globalization; using  Vixen.   Core   .Mathematics;
        using Vixen.Input ;
namespace Vixen
            .Ui.Controls;
...
/// </remarks>
   ...
   <RadialItem> items = [ ]  ;
    ...
    /// <remarks>ARIA <c>menu</c>: a set of choices, presented together, one of which is taken.</remarks>
               protected override   AccessibleRole NativeRole => AccessibleRole.
       Menu    ;
 ...
 items;
  ...
  public event Action< RadialMenu, RadialItem >?   Chose
    ...
    /// <inheritdoc />
        ...
        void OnCreated( ) {
         base   .OnCreated()    ;
			IsFocusScope = true    ;
        LightDismiss = true; CloseOnEscape    =   true;
  ...
  (_, args) => Aimed( args);   
            Document.Root   .AddHandler(
              aimed, RoutingStrategy  .Capture, handledEventsToo    : true);
   ...
   // then sit there waiting for a click nobody is going to make.
        ...
        Document  .  Root.AddHandler(lifted , RoutingStrategy.
          Capture, handledEventsToo: true);
...
=> ((   RadialMenu) element    ).    Keyed(  args));
 ...
 protected override   void OnRemoved   (
       ...
       if (aimed is not null   ) {
              Document  .Root.RemoveHandler   (aimed  );   aimed    = null ;
}
  ...
  Document . Root .RemoveHandler(  lifted) ;
    ...
    public void  Clear(
        ...
        while (Children .Count  > 0) { Children[^1]  .Remove  (  );
  ...
  items   . Clear  ();
   ...
   public    RadialItem AddItem (    string? label = null) { var item = Add<RadialItem>   (  );
	item    .
   Label   =    label;
          item.  Focusable
   =  false;
  ...
  // The pass that gives the wedges a size, so that placing them has something to measure.
      ...
      centre 
          = new Vector2(    x, y  );
	MoveTo
(x - (   Bounds.    Width *    0.5f    ) ,   y - (Bounds  .Height  *    0.5f));
 ...
 // it converges there because the offset no longer moves.
    ...
    /// <returns>Whether anything was aimed at.</returns>
         public bool    Accept(    )    {    if (  highlighted < 0    || highlighted >=  items  .Count) {
            return    false    ;
...
public    void  Aim    (int index) { highlighted = index >= 0   && index  < items. Count ?   index :    -1;
    Restyle    ( 
);
	}   
    ...
    /// </remarks>
			public int WedgeAt    (    Vector2
	offset    )    {
   if (items.Count == 0 || offset  .Length    (   ) < DeadZone)    {  return -1;
           }
...
if (angle < 0f)  { 
   ...
   return
    ...
    ///     button.
    /// </remarks>
                void   Place() { var step = MathF.  Tau   /  Math.Max   (1, items.Count);
        var
     middle = new Vector2(Bounds   .Width *   0.5f,   Bounds.Height * 0.5f    );
             
              for (var    index =    0 ;  index <  items.Count; index ++)    { var angle = step *    index    ;  
            var item = items[index] ;   
 
  item.Angle    = angle
 ;
      var point = middle +
			new   Vector2(MathF.Sin(  angle) *  Radius, -MathF.Cos (angle) * Radius    );
        
            item.SetStyle    ("left"   , Px(  point   .   X -   ( item   .    Bounds  .Width * 0.5f    )  ))    ;    item.SetStyle   ("top"  , Px(point.   Y    - (   item.Bounds. Height *  0.5f)    ));
           }
    }
       static
string Px(float value) => value  .   ToString   (    "0.##", CultureInfo.InvariantCulture) +  "px";
           
        
  void Restyle()   {   
         for (var    index = 0; index < items
   .Count; index    ++  ) { 
  if (index ==   highlighted)  { items[index].State |= Styling.  ElementState.Checked ;
            } else {
  items[ index ] .State
   ...
   void Aimed   (  PointerEvent
        args  )   {
           if (    !IsOpen) {
               return  ;
        }  
        if ( 
               args.Action is  PointerAction. Moved    or PointerAction.Pressed) {
   Aim( WedgeAt( new    Vector2(
args.X -  centre .X,   args  .Y - centre    .Y)  ))   ;
             return; }
...
// opened *by* a press whose release arrives a few milliseconds later, over the middle of the
    ...
    /// </remarks>
   void Lifted(KeyEvent args   )   { if (    IsOpen &&    args.Action == KeyAction.Released   ) {
            Commit(  );
            }
    }
...
(   !   held) {
 ...
 Hold = false;  
    ...
    void Chosen(ClickEvent
args) {   
             for
( var element = args  .Source; element is not null    ;    element  =    element.Parent) {
            if (element is not    RadialItem { Index: >= 0 } item ) { continue ;
 }
         Aim(item  
     .Index )    ;
            Accept();
 ...
 if (args.    Action
!= KeyAction.Pressed || items.Count == 0) {
              return
   ;
      }
      
            switch (args.Key) { case InputKey.Up :
              Aim(WedgeAt(new Vector2    (    0f,   -Radius)) )    ;
              break;
  case  InputKey   .Down: Aim (WedgeAt(new Vector2(  0f, Radius  )    ));
                break
          ;
    
      case  InputKey.Left: Aim  (WedgeAt(new Vector2(    -Radius , 0f))   );
                break;

            case    InputKey  .Right   : Aim    (  WedgeAt(  new Vector2  (Radius   , 0f) )  );
                break;
            case InputKey.Tab:   Aim    (highlighted < 0 ? 0 : (    highlighted + 1    )    % items.Count   );
         break;
       case    InputKey.Enter or InputKey .KeypadEnter   or InputKey. Space: Accept( )   ;
   break  ;
