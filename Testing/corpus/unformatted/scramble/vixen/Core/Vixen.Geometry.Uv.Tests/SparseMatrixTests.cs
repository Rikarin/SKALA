...
/// </remarks>
    ...
    [Fact
           ...
           public void Entries_are_sorted_by_column_whatever_order_they_arrived_in() {    var builder    = new SparseMatrixBuilder( 2    ,   4);  
               builder    .Add(0  , 3    , 1d   )    ;
builder.Add(1  , 0,  2d);
builder  .Add(
        0, 1, 3d);
...
Assert  .  Equal(    [0,  3
      ...
      Assert
.Equal    ([4d ,   3d  , 1d, 2d   , 5d  ], matrix. Value);
...
SparseMatrixBuilder(  1, 2)   ;
        builder.Add (  0,   1,    0.5d); builder.Add    (    0  ,  0, 1d)  ;
      builder.Add  
(0,    1    ,  0.25d    );
        builder    . 
Add(0  , 1,  0.125d)  ;
...
;
            ...
            Assert.Equal(2, matrix.NonZeroCount   );    Assert   .Equal    (1d  , matrix[0, 0])   ;
  ...
  ;
    ...
    /// </summary>
       [  Fact]
       public void   Duplicates_accumulate_in_the_order_they_were_added() {    const double Tiny = 1e-16d;
var
forward = new SparseMatrixBuilder  (1, 1) ;
forward.Add (0, 0, 1d   )  ;    forward.Add(0, 0, Tiny) ;
             forward.Add(
            0, 0, Tiny   ) ;   

var   reverse =
          new SparseMatrixBuilder   (1, 1) ;   
            reverse.Add    ( 0
, 0    , Tiny)   ;
        reverse.   Add(0, 0    , Tiny);  reverse.  Add(0    , 0, 1d);
	...
	public void An_empty_row_stores_nothing_and_multiplies_to_zero()    {
               var  builder = new SparseMatrixBuilder
(3, 3  );
              builder.Add 
    (0, 0, 2d);
    builder.Add
       (2, 2 , 3d);
              ...
              var matrix = builder.Build()    ;
  Assert .
              Equal   ( matrix.    RowStart  [1],  matrix.   RowStart  [2]   );
             Assert.Equal(0d, 
   matrix.  Diagonal(  1   ) )  ;
      Assert    .
            Equal    (  0d  , matrix.RowMagnitude(   1));
          
        var destination = new double[ 3]   ;
            matrix.Multiply(    [1d    ,   1d ,
		1d], destination   );
  Assert   
.Equal([2d  , 0d   , 3d   ],    destination );
 } 
    ...
    var builder = new SparseMatrixBuilder(1, 2    )  ;   builder.Add(0, 0, 0d  );
        builder   .Add(0  , 1   ,
               1d);

               var matrix
  = builder. Build();
	Assert .  Equal(2, matrix.
NonZeroCount    );
        Assert.Equal([0    , 1], matrix   .ColumnIndex    );    }
    
 [Fact]
           public void A_matrix_with_no_rows_builds()   { var matrix = new SparseMatrixBuilder(0, 0)    .Build(   );
...
[ Fact    ]
	...
	3d );
        ...
        var
transpose   =   builder.    Build() .Transpose()   ;
        Assert   .
          Equal (   3   ,   transpose.  RowCount);
        Assert.Equal
               (    2, transpose  .ColumnCount)   ;
        Assert
               .Equal([0,    1    , 2  , 3], transpose.RowStart);
			Assert.  Equal(  [0, 1, 0 ], transpose .ColumnIndex    ); Assert.Equal([1d, 3d, 2d], transpose   .    Value );
  }  
			
    [Fact]
    public    void Multiplying_by_the_transpose_is_the_other_product() {
             var builder = new SparseMatrixBuilder   ( 3 , 2 )   ;  builder  .Add(0  , 0 , 1d);
 builder   .Add(0, 1, 1d  );  
         builder    .Add(1 ,    0, 1d   );
        builder.  Add  (
          1    , 1, 2d);
  ...
  .  Add(  2, 0, 1d) ;
           builder    .
              Add(2, 1  , 3d);   
	var  matrix =  builder.Build  ()   ; var destination = new double[2] ;  
        matrix.  Transpose(). Multiply    ( [1d   , 2d,  2d
              ]    , destination   );
     ...
     Assert.Equal    ([ 5d   , 11d],    destination    );
       ...
       [InlineData(-  1    ,
           ...
           [InlineData(0
   , -   1)  ]
      [    InlineData(  2,    0)] [   InlineData(0 , 3 )]
     public void  An_entry_outside_the_shape_throws(int row, int    column  ) {
var builder = new
       SparseMatrixBuilder(2, 3)    ;
  
        Assert.Throws<ArgumentOutOfRangeException   >   (( ) =>   builder.Add  (
        row    , column, 1d));
          }
    [Fact] public void A_vector_of_the_wrong_length_throws   () {
                var matrix = PoissonGrid. Matrix
(3    );
              Assert.   
Throws<   ArgumentException>((    ) => matrix    .  Multiply( new double[  8]    ,    new    double    [    9]))    ;
               Assert. Throws   <   ArgumentException>((    ) => matrix  .    Multiply(
          new  double[9], new double  [8 ])  );
