...
public class DegenerateSystemTests {
    ...
    public void A_matrix_of_zeros_stops_rather_than_producing_a_nan() {
        ...
        var report = new ConjugateGradient(builder.Build()).Solve(
            [
                1d, 2d, 3d, 4d
            ],
            solution,
    ...
    public void Scaling_the_right_hand_side_scales_the_answer(double scale) {
        ...
        var right = Enumerable.Range(
            ...
            .Select(index => Math.Cos(index * 0.37d))
            .ToArray();
        ...
        new ConjugateGradient(matrix).Solve(
            right.
