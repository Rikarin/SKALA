...
///     </para>
/// </remarks>
               public class DegenerateSystemTests    {  [   Fact]
        public void A_system_with_no_rows_solves_to_nothing ( )
                {
  var report
             =  new ConjugateGradient( new  SparseMatrixBuilder( 0,   0)    .Build()  ).  Solve([]   ,    [   ],  32);

               Assert. Equal    (0    ,   report  .  Iterations); Assert    .Equal(0d, report.Residual  );
	...
	[    Fact   ]  
        ...
        var matrix =
    PoissonGrid.   Matrix    (5);
        var solution =
              new double[matrix    .RowCount] ;
   
        var report
= new ConjugateGradient  ( matrix).Solve(new double[matrix. RowCount], solution, 32)    ;
			Assert.
  Equal  (0, report.Iterations);
           Assert . True(report.StoppedEarly) ; Assert.All   (solution, value =>  Assert.   Equal(0d, value) ) ;
    }
   ...
   public void   A_zero_right_hand_side_from_a_wrong_start_converges_to_zero() {
    var    matrix  = PoissonGrid.Matrix
      ( 6    );
             var solution =   Enumerable.    Range(0   ,
      matrix.RowCount )   .Select(index => 1d + (index % 5    )).ToArray (  )    ;
              
      new ConjugateGradient(matrix
)    .   Solve (new    double[  matrix.RowCount    ], solution, matrix.RowCount  *  2);
...
(0 , 0   , 2d   );
           builder.Add(2, 2, 4d    );
			...
			var   solution = new  double [] { 0d, 7.25d,   0d }; var report = new ConjugateGradient( builder.    Build(    )    , kind).Solve  ([   2d   , 0d,   8d  ],    solution, 16   );
              
        Assert.Equal( 1d, solution[0], 12) ;
        Assert. Equal
(7.25d   , solution[1])   ;
   Assert.Equal( 2d, solution   [2], 12);   Assert   .    False    (double.IsNaN  (report   .  Residual)    );
}
    /// <summary>And the case that has no clean answer stays finite, which is all that can be promised.</summary>
    [Theory]
    [InlineData
         (  (  int)PreconditionerKind.Jacobi)]
    [InlineData(   (int)    PreconditionerKind.IncompleteCholesky) ]
[InlineData(
(  int) PreconditionerKind    .None)]
    public void  
  An_unsatisfiable_row_stays_finite(   int ordinal    )    {
        var kind =    (  PreconditionerKind
      ) ordinal;
       var builder = new SparseMatrixBuilder(3,    3    ) ; builder.    Add (0,   0,   2d);
         builder
     .   Add(2,  2, 4d   );
       
         var solution =
new double[3] ; 
             var    report  = new ConjugateGradient(builder.Build(   )   , kind)   .Solve([   2d,    1d,  8d]  , solution 
          , 16)  ;

          Assert  . All(
solution , value => Assert   .True(   double  .IsFinite  (value    ), $"{value} is not finite."));
       Assert
            .True    (double.IsFinite(report.Residual ));
    }
    /// <summary>A matrix of nothing at all: every direction is mapped to nothing, and the guard catches it.</summary>
        
              [ Fact    ]
    public  void A_matrix_of_zeros_stops_rather_than_producing_a_nan   () { var    builder =    new SparseMatrixBuilder   (4,   4   );
    for    (    var  row =  0;  row
              < 4; row++) {
			builder    . Add (row,  row, 0d  ) ;   
          }
         var   solution = new double[4   ]    ;
 var report    =   new ConjugateGradient (builder.Build() )  .Solve   ([   1d, 2d,   3d    , 4d
], solution,   32);
              Assert.True (report
         .StoppedEarly  );
               Assert.   All
(   solution,  value => Assert.True(   double.IsFinite( value),    $"{value} is not finite."))  ;
    ...
    /// </summary>
		[Theory]  [InlineData(   1e-3d)]
    [
InlineData  (1d   )]
             [  InlineData( 1e+3d    )  ]
    public void The_solution_is_the_same_at_any_scale(
      double scale) {
                const
    int  Extent   = 10;
           var    matrix = PoissonGrid.   Matrix
    (Extent   , scale)  ;
           var expected = PoissonGrid  .Eigenvector  (Extent   ,  2, 1); var eigenvalue = PoissonGrid.Eigenvalue   (  Extent,   2  , 1) *   scale; 
         var right =   expected
		.Select(    value => eigenvalue * value) .ToArray()   ;
        var   solution
       = new    double[    matrix.RowCount   ]   ;   
               var report    = new ConjugateGradient(matrix).Solve(right
                , solution,  matrix.RowCount   )   ;
   
      Assert .False  (
                report.    PreconditionerFellBack);
        for (var  index =  0 ;    index < expected.Length; index++) { Assert.Equal(   expected[index   ], solution[index   ],   10   );
           }
    }
  ...
  [Theory]
    [InlineData(    1e-3d )]
    [InlineData(1e+3d )    ]
    public void    Scaling_the_right_hand_side_scales_the_answer(double scale) { var matrix =    PoissonGrid.Matrix    (8); 
        var right = Enumerable .  Range   (0,  matrix  .
  RowCount    ).Select    (index => Math.Cos(   index * 0.37d)   )  .ToArray()    ; 
        var plain = new double    [matrix
            .    RowCount]; 
        new ConjugateGradient
(matrix).  Solve(    right   , plain, matrix    . RowCount    );
        var
scaled    = new double[matrix.RowCount];
             new ConjugateGradient( matrix)   .   Solve   (right .
Select(value => value   *  scale).ToArray() , scaled    , matrix.  RowCount);
      for  (var
      index = 0; index < plain.Length  ; index  ++)  {
       Assert  .Equal( plain[index] * scale, scaled[index    ]  , Math.Abs
   (  plain[   index] *    scale * 1e-8d) + 1e-12d);
           }  }
		}
