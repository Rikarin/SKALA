...
/// </remarks>
    ...
    /// </remarks>
           [Fact ] public void   ThePackerDoesNotStackAnythingOnItsOwn   (    )    {
         var island  =   IslandCorpus.  Square    (0.25f, 8f
     );
var placements =  UvUnwrap    .Pack    ([island, island ]  , new(
          ) { Resolution = 256   , Margin = 4  }   )    ;
        Assert
.Equal(2 , placements .Count);
        Assert.NotEqual(
              placements[0].Offset  ,    placements[ 1].Offset);
            }
              
    /// <summary>Two copies of one island are offered as a pair, exactly.</summary>
    [    Fact] public void   AnExactDuplicateIsOfferedAtZeroResidual  () {
           var island = IslandCorpus.Square(0.25f,  8f   ); var offers = UvStacking.    Detect([island, IslandCorpus.Square(0.5f, 8f) , island ]  )   ;

        var
 offer =    Assert.Single(offers); 
			
    Assert. Equal (0  , offer.Representative    )   ; Assert.Equal(  2, offer.Partner)    ;
        Assert.Equal   (    0f   , offer
          .Residual)  ;
    }
    ...
    /// </remarks>
          [Fact
             ...
             public void
             AMirrorImageIsOfferedAsOne()  {
        var
              island = Wedge(false )   ;
 var mirrored  =    Wedge(true); var offers = UvStacking   .Detect(  [ island, mirrored]    );
                var offer
     = Assert.Single (offers  );
            
        Assert. True(offer.Mirrored,
$"the pair matched straight at {offer.Residual:E3} rather than mirrored.")  ;
 ...
 // is offered *without* the mirror, so the flag tracks which comparison actually won. A wedge
  ...
  Assert.False    (Assert.    Single    (UvStacking
      .Detect (  [island, island]))    .Mirrored);   
   } 
   ...
   [Fact  ]
             ...
             var
       islands =  IslandCorpus.    Trellis(48);
        Assert.  Empty(UvStacking.Detect(   islands)  )  ; Assert .  Empty(UvStacking   .Detect(islands    , 1e-2f)  );
  }
    ...
    /// </remarks>
    [   Fact] public void AFoldedPairSharesOneRegion( ) {
          var    island = Wedge( false)    ;
	var islands = new[   ] { island    , IslandCorpus    .Square    (0.4f,  8f )
                ,   Wedge(true), IslandCorpus .Square(   0.2f, 8f)    };
              var offers = UvStacking  .Detect    (    islands);  
      
   Assert.    Single   (offers);
             
           var folded    = UvStacking
            .Fold(islands, offers,    out var source )    ;

            Assert. Equal(    3  , folded.Count);
     Assert
 .Equal(source[  0],    source[2   ])  ;
         Assert.NotEqual ( source[0]  , source[1]);
          var
settings = new   PackSettings { Resolution = 256 ,  Margin    =   4 };
     var    placements
      =   UvStacking.   Unfold(UvUnwrap.Pack  ( folded   , settings)   , source);
        Assert.Equal(islands .
              Length, placements.Count);
for (var   index =
         0; index  < placements  . Count; index++) {
            Assert .  Equal(index
        , placements   [index].Island);
        }
           Assert    .Equal(placements[0]    .   Offset,  placements[2  ] . Offset); Assert   .Equal    (placements[  0].    Scale, placements    [    2 ].    Scale);
...
pair <  12   ; pair++) {
 ...
 islands    .Add  (   Wedge (true   , 0.1f + (0.02f *  pair))); }
   ...
   var    settings   = new PackSettings {    Resolution = 512, Margin =   4 , TexelDensity = 2400f };
              var offers
           =    UvStacking.Detect(    islands)    ;
           
             Assert  . Equal   (12
   ,   offers   .Count);
          
        UvUnwrap   .Pack(islands, settings  , out var flat);
var folded = UvStacking.Fold(    islands  ,   offers    ,
out    _);

			UvUnwrap .  Pack   (
   folded, settings, out  var stacked) ;
  
             Assert.Equal(    12, folded.Count    )    ;   
        Assert    .  Empty (flat. Warnings); Assert    .    Empty (stacked  .  Warnings);
     
 Assert.  
         Equal(0.5f  ,  stacked.PackingEfficiency / flat    .   PackingEfficiency  , 2);
}
    ...
    /// </remarks>
    [    Fact] public    void   DetectionIsTheSameListEveryTime(   )   { 
              var islands =  new List<UvIsland
    >   ();
 
        for  (var   pair = 0   ; pair   <  8  ; pair  ++) {
 islands.   Add(    Wedge(false, 0.12f + ( 0.03f * pair))  )  ; islands .Add    (  IslandCorpus.Square(    0.05f + (    0.01f * pair ), 8f  ));
...
< first   .   Count  ; index++   )  {
               Assert   .True(first [  
               index].Representative   > first[   index - 1].   Representative   );
  }
            }
...
{
             var  island = IslandCorpus.Square(0.25f   , 8f)  ; var islands = new[  ] { island, island , island };  
    ...
    /// </remarks>
     ...
     var points  = new[    ] {
          new   Vector2(0f,   0f    ),   new Vector2(side, 0f),
...
mirrored) {   
   ...
   index   <    points.    Length;   index++ )    {
           points
              [   index] = new(side - points[index].X   ,   points[ index    ].    Y);
            } }
       
 var coordinates = new[   ]  { points    [ 0], points[1    ], points[2],    points[    0]    ,    points  [2  ] , points[3] }; var minimum = coordinates [   0 ];
        var maximum
             = coordinates [    0   ]  ;
        foreach
(var coordinate in coordinates   ) {
            minimum =  Vector2.Min
       (minimum  , coordinate   )    ;
		maximum = Vector2.  Max(   maximum, coordinate); }

        return new(coordinates , [ 
   0, 1, 2,   3, 4, 5]    , minimum, maximum,   8f);
            } }
