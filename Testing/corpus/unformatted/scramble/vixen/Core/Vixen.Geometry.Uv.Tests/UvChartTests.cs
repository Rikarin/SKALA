...
namespace Vixen.Geometry.Uv
    ...
    public class UvChartTests {
        ...
        [InlineData("dumbbell")]
        public void AnIsometricThresholdRecursesToTheDepthBoundAndStops(
            string
            shape
        ) {
            var mesh = ChartFixtures.Build(shape);
            ...
            Assert.InRange(
                report
                    .ChartCount,
                2,
                (1 << 4) * Math.Max(1, CountSeeds(mesh))
            );
        ...
        public void EverySeamWeightAtZeroStillCharts() {
            ...
            foreach (var shape in new[] { "dumbbell", "torus-closed", "sphere-cut-open" }) {
                ...
                Assert.Equal(
                    report.
                    ChartCount,
                    UvUnwrap.Flatten(mesh, charts, settings).Count
                );
            }
        }
