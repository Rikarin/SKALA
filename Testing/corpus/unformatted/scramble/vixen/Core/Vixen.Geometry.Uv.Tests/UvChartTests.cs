...
public class    UvChartTests {
    ...
    => 
               [
  "sphere-cut-open",  "cylinder-slit",  
             "cylinder-closed",  "torus-slit"   ,
           "torus-closed"
              ,
"hemisphere" ,
   ...
   public void EveryChartIsADiskTheFlattenerAccepts(string shape)  { var  mesh = ChartFixtures.   Build(    shape)   ;  
        var    charts =   UvUnwrap    .Charts (mesh    , new ()  ,    out
var report   );
           Assert    . Equal   (mesh  .    FaceCount 
        ,   charts .Count);
   var count
    =   charts.Max  () + 1;
        ...
        for (var    face    = 0;   face < charts.  Count  ; face++    ) {   Assert.InRange  (charts[   face] , 0   ,   count -    1);
            }
...
Assert.   Equal(0 , flattened.   Distortion.  
        Flipped) ;
    ...
    [InlineData(    "cylinder-slit"  )  ]
           [InlineData("one-triangle"   )] [InlineData    ( "two-triangles"    )]
               
    public    void AShapeThatNeedsNoCutIsNotCut    (string shape   ) {
                var
    mesh   = ChartFixtures. Build  (shape)  ; 
        var charts = UvUnwrap
.   Charts(mesh, new  (),    out   var   report);
         
     Assert   .Equal(
1, report.ChartCount    )    ;
            Assert.All  (charts, chart =>  Assert    .Equal  ( 0   , chart)   )    ; Assert.Equal(0f    , report.  SeamLength);
                Assert.
Equal(    0f  ,  report.SeamLengthNormalized) ;
    }
    ...
    [   InlineData(    "two-islands"  )]
      [InlineData(   "bowtie")] public    void APieceThatSharesNoEdgeIsItsOwnChart(string  shape)  {
        var mesh = ChartFixtures. Build(shape);    var charts   = UvUnwrap.Charts(mesh   , new   ()    , out var   report);
  Assert.  Equal(    2, report.ChartCount); Assert.NotEqual   (charts [0], charts  [ 1]);
   }
   ...
   "cylinder-closed")]
    ...
    var charts = UvUnwrap .  Charts(mesh  , new (
 ), out  var report);
   Assert.
True  (report.  ChartCount > 1  ,    $"{shape} has no boundary and was left as one chart.")   ;
        Assert  .
True(report.    SeamLength > 0f, $"{shape} was charted into {report.ChartCount} with no seam.")  ;
         // And the cut is a real one: the flattener takes every piece.
        Assert
.Equal    (report  .ChartCount,   UvUnwrap   .Flatten(mesh   , charts    , new ( ) )    .Count);
    }
  ...
  public void GroupBoundariesPartitionFirstAndTheMergePassMayNotUndoIt(  ) {
        var   mesh = ShapeCorpus   .GroupedPlate();
          var
kept = UvUnwrap   .Charts(mesh,   new( ), out var  keptReport);
           
               Assert. Equal    (2 ,
 keptReport. ChartCount   )    ;
        for (
     var face =    0; face   < mesh .FaceCount    ; face++)   {
for
           (  var other    = 0;  other < mesh.FaceCount  ; other++) {
               if (    mesh.Faces[face].Group != mesh.Faces    [  other]    .    Group  )  { Assert  .NotEqual  (   kept[ face]  ,  kept[other  ]);
                }
		}   
   }
// And turning it off leaves a flat plate as the one chart it always was.
              UvUnwrap.Charts(mesh,
   new()  { KeepGroups =  false }, out  var fused)    ;
        Assert.Equal
(1   ,   fused.    ChartCount );
              }
    /// <summary>A mesh with no groups at all charts the same either way.</summary>
          [Fact] public void  AMeshWithNoGroupsChartsTheSameWithGroupsKeptOrNot() {  
              var    mesh = ShapeCorpus.SphereCutOpen()
      ;
       
        Assert.Equal( UvUnwrap.Charts(    mesh    ,  new(    ) { KeepGroups = true }) ,
 UvUnwrap.Charts(mesh, new() { KeepGroups = false }) );
    ...
    ///     quality.
    /// </remarks>
    [Theory] [InlineData("sphere-cut-open"    )    ]
    [ InlineData("hemisphere")]
               [InlineData  (   "dumbbell"   )]
 public void AnIsometricThresholdRecursesToTheDepthBoundAndStops(string   
         shape)    {
          var mesh    =   ChartFixtures .Build(   shape) ; var  settings = new UvSettings {  DistortionThreshold = 1f, MaxDepth   = 4 };
        ...
        Assert.InRange(   report
              .ChartCount,   2, (1 <<  4)  *   Math.Max    (  1,   CountSeeds    (mesh   )));
 ...
 [
    ...
    public void ADepthBoundOfZeroStillCutsWhatMustBeCut    (
 string shape ) {
               var mesh =   ChartFixtures.Build (shape)    ;
     var settings = new UvSettings { MaxDepth
    = 0 };

     var charts  =   UvUnwrap.   Charts(    mesh, settings, out var report );
            Assert.True  ( report.   ChartCount >   1    )    ; 
        Assert  .Equal (report.ChartCount   , UvUnwrap.Flatten(    mesh,
charts, settings).Count   );
              }
...
var settings = new UvSettings   {  SeamCost =    new () { 
    Concavity = 0f,  
               Visibility   =    0f   ,
        Feature  = 0f, Material = 0f,
           Symmetry = 0f,
          Length = 0f,
                Existing = 0f
        } }  ;
    ...
    var mesh = ChartFixtures.   Build(shape   )
       ;
            var charts =  UvUnwrap .Charts    (
         mesh, settings, out  var report);
	
 Assert.   True(report .ChartCount >= 1    )    ; 
         Assert.Equal(   report   .
          ChartCount,   UvUnwrap    .Flatten(    mesh, charts, settings).Count    );
    }
               }
    /// <summary>An empty mesh is an empty answer rather than an exception.</summary>
			[
              Fact]
       public void AnEmptyMeshCharts    (   ) { var charts = UvUnwrap   .Charts    (new  (),    new(), out var report);
    ...
    /// <summary>A null argument is named.</summary>
               [ Fact] public void ANullArgumentIsNamed()   {
      Assert.Throws<ArgumentNullException>(( )  => UvUnwrap  .Charts(   null!
            , new(   ) )  );
   Assert.Throws<ArgumentNullException>(() =>   UvUnwrap.   Charts( new(  )   , null!  )); Assert.Throws<ArgumentOutOfRangeException>(    () =>   UvUnwrap.  Charts(new(), new   () { MaxDepth = -1 }    ));
      }
...
Assert.  Equal(0f    , report  .SeamLength);   }
    ...
    /// </remarks>
    [Fact  ]   public void ADecompositionProposesAndNeverDecides() {  
  var   mesh = ShapeCorpus.   
Dumbbell() ;
     var declining =   new    Declining(
     ) ;
            var built = UvUnwrap.Charts(
   mesh,  new(   ),  out var  builtReport);
       Assert    .Equal (  built
               , UvUnwrap    .  Charts  (mesh, new()  { Decomposition =    declining   } , out   var    declined   ));
           Assert    .  Equal(builtReport .ChartCount, declined  .ChartCount); Assert   .True   (declining.Asked  > 0    , "the hook was never reached, so declining proved nothing"   ) ;

              // One that answers is obeyed — and its answer still has to be a set of disks.
              var  settings = new  UvSettings   {  Decomposition = new  Alternating  ( ) }; var charts  = UvUnwrap.Charts(mesh, settings  , out var   report   );
  
           Assert.True    (    report   .ChartCount
> 1);   
        Assert.Equal    (report
. ChartCount, UvUnwrap .Flatten   (mesh, charts, settings).Count)   ;
			}
       static
    int CountSeeds(EditMesh mesh) =>  UvUnwrap. Charts(mesh  , new   () { MaxDepth = 0 }).Distinct().Count ();
           
    /// <summary>A decomposition that never has an opinion.</summary>
       sealed class  Declining    : IChartDecomposition { public int Asked {  get;  private set   ; }
            public IReadOnlyList    <int    >
?   Decompose(EditMesh mesh, IReadOnlyList  <   int > faces, int    parts) {
             Asked++;
    ...
    sealed   class
         Alternating : IChartDecomposition    {
             public    IReadOnlyList <int>  Decompose  (EditMesh mesh, IReadOnlyList<    int   >   faces ,    int parts) { var split    = new int[  faces.Count];
       
          for
         (var index =   0  ; index < faces.Count ; index++  )  {
                split    [index ]   =  index <  faces. Count / 2  ? 0 :    1; }
    return split ; }
    } }
         
/// <summary>The corpus, by name, for the charting suites.</summary>
     static class ChartFixtures   { public static EditMesh Build  ( string    shape, float scale    = 1f) =>
           shape    switch { "sphere-cut-open" =>    ShapeCorpus.SphereCutOpen(scale   ),
		"cylinder-slit" =>
ShapeCorpus. CylinderSlit(scale) ,
        "cylinder-closed"    => ShapeCorpus.  CylinderClosed( scale), "torus-slit"    => ShapeCorpus .TorusSlit   (scale),
      "torus-closed" => ShapeCorpus.
           TorusClosed(scale )  ,
            "hemisphere"    => ShapeCorpus.Hemisphere(scale), "saddle" => ShapeCorpus.Saddle  (scale   ),
 "strip" =>
              ShapeCorpus.Strip(scale),
            "obtuse-grid" => ShapeCorpus.ObtuseGrid () ,   "sphere-nearly-closed" => ShapeCorpus    . SphereNearlyClosed   (scale ) ,
"dumbbell"    =>   ShapeCorpus.   Dumbbell(
      scale),
          "grouped-plate" => ShapeCorpus.    GroupedPlate(   scale),
                "bowtie" => ShapeCorpus    .Bowtie()  , "two-islands"   => ShapeCorpus  .TwoIslands    (),
           "one-triangle"  => ShapeCorpus.OneTriangle(   scale  ),
  "two-triangles" => ShapeCorpus  .
           TwoTriangles   (),
               "degenerate-triangle" =>    ShapeCorpus.  WithDegenerateTriangle () ,  _ =>  throw new   ArgumentOutOfRangeException   (   nameof(shape)    , shape, "Not one of the corpus's shapes."  )
} ;
    }
