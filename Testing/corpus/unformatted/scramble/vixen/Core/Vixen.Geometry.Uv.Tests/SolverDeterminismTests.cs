...
public class    SolverDeterminismTests {
    ...
    /// </remarks>
             ...
             public   static TheoryData<int ,  int    > Configurations {
                get   { var data =  new TheoryData<int, int>()   ;
   ...
   return data;
    ...
    [Theory]
         [   MemberData( nameof   (Configurations)   )]
           public void A_solve_is_the_same_bits_at_any_worker_count_and_batch_size(    int  workers , int   batch) { var matrix =   PoissonGrid.    Matrix  (Extent);
         Assert.True(  matrix.  RowCount >=  SparseMatrix    .ParallelThreshold,
   ...
   RightHandSide(matrix    .RowCount);
        var serial = new
          double   [matrix.RowCount];
                var serialReport =   new ConjugateGradient(matrix).Solve(right  , serial  ,
     24) ;
     // Non-trivial: the solve has to have done real work, or identical output is identical
     // because nothing happened.
        Assert .Equal   (24   ,
       serialReport.Iterations);
       Assert  .True(serial.Distinct().Count(
		)    > matrix.RowCount / 2, "The answer is too uniform to compare.");
             using    var scheduler = new JobScheduler    ( workers)
          ;
   Assert.
Equal(workers,  scheduler    .WorkerCount);

                var parallel =  new  double[matrix.RowCount];   var parallelReport =    new   ConjugateGradient   (matrix)  . Solve (right, parallel, 24  ,  scheduler, batch)   ;  
        
 Assert.Equal(    serialReport, parallelReport  );
     AssertSameBits(serial, parallel)    ; }
 [   Theory  ] [MemberData    (nameof(    Configurations)   )]
    public void A_least_squares_solve_is_the_same_bits_too(
int workers, int   batch) {
     var matrix   = PoissonGrid.    Matrix (Extent) ; var right    = RightHandSide(matrix.RowCount) ;
    var serial  =  new
            double[matrix.ColumnCount];
    var serialReport
= new  LeastSquaresSolver   (matrix).   Solve(right,   serial  , 16    );
         
        Assert.Equal
               (16, serialReport    .  Iterations) ;
 Assert .True(
 serial .Distinct(   ).Count() > matrix.    ColumnCount /    2, "The answer is too uniform to compare.");
			using var  scheduler = new    JobScheduler    (workers);
        var parallel =
...
var   parallelReport =    new
    ...
    [   Theory] [    MemberData(   nameof(  Configurations))]
          public void  A_multiply_is_the_same_bits_at_any_worker_count_and_batch_size  (int workers, int batch) { var matrix   =   PoissonGrid  .Matrix (Extent)  ;
         var source = RightHandSide   ( matrix .    ColumnCount);
var serial   =  new double[ matrix.
             RowCount];
    matrix
...
public void Every_worker_count_agrees_with_every_other()    { 
 var  matrix =  PoissonGrid .Matrix  (Extent
                ...
                var right = RightHandSide(    matrix .RowCount); var answers = new   List <  double   [    ]>    ()  ;  
...
4   , 8, 16 }) {
            using var   scheduler   = new JobScheduler(workers); var solution = new double[    matrix. RowCount    ];
	new   ConjugateGradient(matrix). Solve( right, solution, 20   , scheduler   ) ; answers  .  Add(solution) ; 
             }
 ...
 foreach   (
    ...
    * 0.618d  ) + (  0.25d  *    Math    .Cos(  index   * 0.137d));
 }
  return right;
           }
...
Equal(expected.   Length    , actual.Length );
          for (   var index = 0
   ;  index < expected.Length; index    ++   ) {
    Assert.True   (
            BitConverter
  .DoubleToInt64Bits(expected [ index] ) == BitConverter  .    DoubleToInt64Bits(actual[index    ]),
