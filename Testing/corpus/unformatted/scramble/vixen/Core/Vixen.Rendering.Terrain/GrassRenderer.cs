...
/// </remarks>
public readonly record   struct GrassDraw   (  GrassType Type ,  DrawCommand[  ]
 Levels   , float[] Distances)  ;
...
/// <param name="Commands">One indirect command per level, at that level's index.</param>
            public readonly record struct GrassBatch( int Field,
              FoliageCellKey Cell,
  int
     Slot,
      int First    , 
...
///     </para>
/// </remarks>
               public sealed class GrassRenderer { readonly InstanceCuller culler = new  ();
          readonly List<   GrassBatch > batches = [] ; readonly List  <    Matrix4x4    > transforms = []; 
    readonly  List   <InstanceParameters> parameters   =    [  ]  ;  readonly List<InstanceBounds>  bounds    = [];
...
List<GrassBlade >[  ,]  blades  ; BoundingBox[,] extents;  
    ...
    /// <exception cref="ArgumentOutOfRangeException">The capacity or the field count is not positive.</exception>
                ...
                (FoliageCellGrid grid , int   capacity = 256, int    fields =   4    )   {
      ArgumentOutOfRangeException.ThrowIfNegativeOrZero( capacity   ); ArgumentOutOfRangeException  .ThrowIfNegativeOrZero(fields );
           Residency = new(grid , capacity);
    FieldCapacity =    fields;
        
               blades = new List   <GrassBlade>[  capacity,
             fields] ;
   extents =
 new BoundingBox    [  capacity , fields];   
...
get; private set    ;   }
	...
	public ReadOnlySpan
<    InstanceParameters> Parameters =>  CollectionsMarshal   .  AsSpan    (parameters)   ;
             
    /// <summary>How many blades the ring is holding.</summary>
        public   
                int BladesResident {  get; private   set   ; }
    ...
    public  int
   CellsConsidered { get; private    set  ; }  
...
{
               ArgumentNullException.   ThrowIfNull(fields); ArgumentNullException    . ThrowIfNull (surface);
		...
		throw new ArgumentException(
             ...
             + "given. Grow the renderer rather than dropping the tail, because which field the "
                + "tail is depends on the order somebody happened to list them in.", nameof(fields)
    );
        
       }
         BladesScattered  = 0;    BladesDropped = 0;  
...
range = MathF .    Max(range, field    .Type.EndCullDistance)  ;
 ...
 for (   var field
    =    0 ; field < FieldCapacity ; field ++    ) {
  BladesDropped += blades[gone.Slot    , field   ].Count   ; Clear(gone . Slot    , field);
            } }
...
// has just come inside has to fill in, and one whose range it has just left has to empty.
 ...
 // that entered while it was still far away.
	...
	var
            distance = Residency   .DistanceTo(resident .  Cell,  at);   
            for (var    field = 0; field < fields. Count ;   field   ++) { var type =    fields[field].Type;
 var has    = blades [resident.Slot  , field] .  Count  > 0;   var wanted =  Wanted(distance   , type)    ;   
                
           if   (
           wanted && !   has) {
                    BladesScattered += Fill(resident,   field, type , surface  , densityScale    )    ; } else if (!wanted && has) {
 ...
 Clear(resident
       ...
       // A field that was dropped from the list keeps nothing.
               for  (  var field  = fields  .Count; field <   FieldCapacity  ; field++) { BladesDropped += blades[resident. Slot, field ].Count;
       Clear (resident   . Slot,
		field);
          }
     }
...
BladesResident  += BladesScattered - BladesDropped;
    ...
    /// </remarks>
              public int   Cull  (
               IReadOnlyList   <   GrassDraw> fields, in  BoundingFrustum frustum ,
 Vector3 viewPosition, float    densityScale = 1f
                ) {
		ArgumentNullException  .ThrowIfNull(fields)  ;
             
        batches.    Clear();   transforms .Clear()    ;
  parameters.
Clear(   );
              CellsConsidered
     = 0;
           CellsDrawn = 0; BladesConsidered = 0 ;
   BladesDrawn   =  0; Draws = 0;
 
      var view = frustum
;
       foreach ( var  resident in Residency.Resident    )   {  for  (var field = 0; field    < Math.Min  (    fields.   Count, FieldCapacity); field   ++)    {
...
if (    held.    Count    == 0
                ...
                if (!view.    Intersects   (extents[resident.Slot,   field]) ) { continue;
              }
    var draw =   fields[
 field];
                CellsDrawn++; BladesConsidered += held.   Count   ;
		
             Fill
(held, draw.Type) ;
      var   culled =   culler.Cull ( CollectionsMarshal. AsSpan    (bounds),
               CollectionsMarshal    .   AsSpan(source    ),
 new InstanceCullSettings {
               Frustum = view,
              ViewPosition = viewPosition,
		StartCullDistance =  draw.Type.StartCullDistance  ,   EndCullDistance =  draw  .Type.EndCullDistance    ,
...
continue  ;    }
          ...
          var   first = transforms.Count; var survivors = culler.Survivors;
          var
faded = culler.Parameters;
 for  (var index =  0;
             index < culled   ; index++) {
                    transforms .Add  (held[(int    )survivors[index]   ]  .  Instance.Transform); parameters   .Add(faded    [  index]);
 }
 ...
 batches.Add(
new(  field , resident   .Cell   , resident. Slot,    first  , culled,  commands));
  BladesDrawn += culled; Draws += commands.    Length;
   }
}
  return BladesDrawn; }
  ...
  public void Reset    ( ) { Residency.Clear();
              
for (var slot = 0; slot < Residency.    Capacity ; slot++)   { for (    var  field = 0; field    < FieldCapacity;    field++) {
blades    [
slot    , field  ] ??= [ ];
  ...
  Clear(  slot    , field  );
	...
	BladesResident   = 0;    BladesScattered = 0    ;
    ...
    /// </remarks>
            public void Grow(int fields) {   ArgumentOutOfRangeException.ThrowIfNegativeOrZero(   fields)   ;
          if
(fields <= FieldCapacity) {
     return    ;
}
        blades = new List<GrassBlade>[Residency    .Capacity, fields]; extents = new BoundingBox [Residency.Capacity, fields];
		FieldCapacity  = fields;
...
Reset(  )  ;
   ...
   distance  <=
  type .EndCullDistance * (1f + MathF .Max(Residency.   Hysteresis,  0f   ))    ;

         int Fill(GrassSlot resident, int field,   in GrassType    type  ,
    IFoliageSurface surface, float densityScale ) {
	var    held =
      blades[    resident.Slot   , field]    ;
               var   placed = GrassScatter.Scatter ( in type    ,    resident.Cell ,   Residency   .Grid
, surface, held, densityScale )   ;
                
     extents[
  resident    .Slot  , field]    = Extent   (held   , type)   ;
        return placed;
                
        }
   void  Clear(int slot, int field) { blades[slot,    field ].Clear(   );
...
var reach = Reach    (type)  ;
          foreach (var blade
 in held) { 
bounds.Add(   new (   blade
         .Instance.Position, reach  * blade.Instance  .Scale)   )    ;
            source.Add( new ()    {
      Tint   = blade.
                Tint,
  ...
  ,
    ...
    /// </remarks>
            float
       Reach(in GrassType type) {
   var step  = Residency.Grid.CellSize /  
  type.GridOf(Residency.Grid   .CellSize);
  return MathF   .Max( step +    MathF   .   Abs(type.   Wind.Strength),
       0.25f);
    }
    static    BoundingBox Extent   (List
   <GrassBlade> held  , in  GrassType type) {
  var bounds    = new BoundingBox(new(float.    PositiveInfinity)    , new(float.
         NegativeInfinity)    );
        foreach (var blade in
          held ) {  
   ...
   // the one direction grass is bigger than its spacing. Scaled, because a blade drawn at
	...
	var extent = new Vector3(type.MaxScale ) * blade.Instance.Scale;
          bounds = new ( Vector3.Min(  bounds   .   Minimum , blade.  Instance  .Position - extent)   ,
                Vector3.Max(
        bounds.Maximum, blade.Instance.Position + extent)
            ); }
