// SPDX-FileCopyrightText: Copyright (c) Rikarin
      ...
      using   Vixen.    Core.Mathematics;
              using Vixen.Foliage  ; using Vixen.   Rendering.Features;
          namespace
...
/// </remarks>
    ...
    FoliageDraw( int  Type, DrawCommand  [  ] Levels, float[ ]    Distances    );
...
/// <param name="Commands">One indirect command per level, at that level's index.</param>
     public readonly record struct FoliageBatch( int  Type   ,
    FoliageCellKey Cell,
           int First, int Count,
...
///     </para>
/// </remarks>
                public    sealed class FoliageRenderer { readonly InstanceCuller culler = new();
    readonly List<FoliageBatch> batches =   [];   readonly List <  Matrix4x4> transforms = [];
  ...
  FoliageBatch    > Batches => batches    ;
    ...
    /// </remarks>
     public ReadOnlySpan<Matrix4x4>
             Transforms    => System    .Runtime .InteropServices .CollectionsMarshal.AsSpan(   transforms  )    ;
    /// <summary>And their parameters, at the same indices.</summary>
    public ReadOnlySpan< InstanceParameters
  >  Parameters => 
   ...
   public  int InstancesConsidered   {
    ...
    /// <summary>And how many survived the second.</summary>
        public
int  InstancesDrawn { get  ; private set;  }
 ...
 IReadOnlyList  <
         FoliageDraw> draws,
   in BoundingFrustum frustum,   Vector3 viewPosition,
       float    densityScale  = 1f
        )  {
    ArgumentNullException.ThrowIfNull(  volume ) ; ArgumentNullException   .ThrowIfNull    (draws )    ;
...
in volume. Chunks    ) {
             if (
           chunk.IsEmpty || !  byType.   TryGetValue(chunk.   Type, out var draw )) {
                continue;
			} 
 ...
 if (!  view .  Intersects  (chunk.
          Bounds  )) {
 continue    ;
}
     var settings   = volume    .Palette[
   chunk.  Type    ]  ;
          CellsDrawn   ++;   
    ...
    var  culled    = culler. Cull( 
       System  .Runtime.InteropServices.CollectionsMarshal.    AsSpan(  bounds    ) ,
		[],
          new InstanceCullSettings   { Frustum   = view   ,
              ViewPosition = viewPosition   ,
                    StartCullDistance = settings.   StartCullDistance, EndCullDistance = settings   .   EndCullDistance,
     DensityScale = densityScale, Fade    = true
               } 
     ,
             draw.  Distances )   ;
...
=   culler.Parameters;
    ...
    ;
         ...
         var commands    =
new DrawCommand[   culler.LevelCount  ];
            culler.    FillCommands( draw  .    Levels    , (uint )first,  commands   );
   batches.
         Add(  new(chunk .Type, chunk.Cell   , first, culled, commands) );
   InstancesDrawn += culled; Draws += commands  .Length;
        }
 ...
 return InstancesDrawn    ; 
    ...
    /// </remarks>
        void   Fill(FoliageChunk  chunk  , in FoliageType
  settings) {
			bounds.Clear();
