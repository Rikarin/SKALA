...
/// </param>
        [StructLayout   (LayoutKind.   Sequential)   ]
			public readonly   record struct 
             TerrainNodeRecord(
         Vector2 Origin, float Step,
float
Morph,
          float Level    ,
    float Padding0  = 0f   )  {
    /// <summary>How many bytes one of these is.</summary>
         public    static   int SizeInBytes => Marshal.
       SizeOf<TerrainNodeRecord  >    ();
    ...
    /// </remarks>
           public static TerrainNodeRecord Of(in TerrainLodNode node, int gridQuads   , int maxLevel = 0    ) { var step    = node.  Quads / (   float)  gridQuads ;
        var level  =  Math    .Clamp( (  float  )Math   .    Log2(   Math
        .    Max   (step, 1f)), 0f, Math.Max   ( maxLevel, 0))    ; 
       return   new(   new(  node  .    X
   , node.Z), step, node.  Morph, level, 0f)    ;
              }
               }
 ...
 var topLeft = (uint  )((z * side)  
   ...
   var bottomLeft = (uint)  (
      ...
      ;
                    indices   [at++  ] 
            =  topRight;
  indices   [at++] = topRight; indices[    at    ++] =   bottomLeft    ;
    indices  [at++]   =
                bottomRight;
               }    else
 { 
             indices    [at++] = topLeft; indices    [  at++]  = bottomLeft;   
             indices    [at++] = bottomRight;
                    indices[  at++]   = topLeft   ; indices[at    ++] = bottomRight;
            indices[  at++  ] =  topRight;
          }
   }  }
        return at;
    ...
    /// </remarks>
    public
static (int X, int Z) VertexOf  (int vertex   , int  gridQuads) {
        var side = gridQuads + 1    ; return (vertex % side , vertex / side)   ;
    } }
