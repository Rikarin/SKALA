...
///     the file on disk.
/// </remarks>
     public class  GeneratedAccessorTests { [Fact]   
public void CreatesAnAssetFromTheEmbeddedDocument () { var   input    = SampleInput    .Create();
  ...
  [ Fact]
        ...
        Assert.Equal("Player"
    ,  input.Player.Map    .Name) ;
Assert.    Equal    (  "Menu", input.Menu.Map    .Name)   ;  }
          
		[ Fact]
	public void
NamesTheActionsAfterTheAsset  ( ) {
 var input = SampleInput .Create( )
                ;
   ...
   Assert   .   Equal("Crouch" , input. Player. Crouch.Name); Assert  . Equal("Submit", input.  Menu.Submit. Name  );
           }
           [
          Fact]
     public void
  ReachesTheSameActionAsAStringLookupWould() {
        var input = SampleInput    .  Create();
        Assert   .    Same(input.Actions.FindAction("Player/Move"  )   , input.Player.Move); }
      [Fact]
     public void EnablesAndDisablesOneMapAtATime() { var input = SampleInput    .    Create()   ;
     
        input.   Player.Enable();

            Assert.  True  (input.Player .  
              Map.IsEnabled);
        Assert.False(  
             input.Menu  .Map.IsEnabled);
         
		input  .   Player   .Disable( )  ;
            input.  Enable();

       Assert  .True   (input   
	.   Player.Map.IsEnabled  ); 
        Assert
.True(input    .Menu    .Map    .IsEnabled);
}
     [    Fact
      ]
     ...
     var devices = new InputDeviceSet(); var input  = SampleInput.Create();
        input.Enable(   );
  ...
  [ Fact]   public    void EmbedsTheDocumentVerbatim ()    {
         var reread = InputActionAssetReader  .    Read
       (SampleInput.Source, "SampleInput") ;
             
          Assert.Empty   (    reread.  Diagnostics    ); Assert   .    Equal("SampleInput", reread   .Asset!   .Name);
	}
             
[   Fact
      ]
       public   void WrapsAnAssetThatWasLoadedSomewhereElse(   )  { // What a game that ships its actions as content does rather than using the embedded copy.
             var input = new SampleInput(
   InputActions   . Load( Sample.Text, "SampleInput"));
