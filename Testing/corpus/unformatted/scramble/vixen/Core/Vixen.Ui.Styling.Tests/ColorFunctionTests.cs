...
///     </para>
/// </remarks>
  public class ColorFunctionTests {   const float Tolerance  = 1e-3f    ;
         static StyleValueParser Parser
()    =>   new (new NameTable(   )  ,   new NameTable()    );
              static Color4 Color(string text) {
              var value    =    Parser   ().Parse (text)   ; Assert.Equal    (    StyleValueKind.Color, value. Kind)  ;
        return value.Color;
}
            
           static void   AssertUnknown    (string text)
   =>
Assert    .Equal  ( StyleValueKind.  
             Unknown,  Parser    ().    Parse(    text )   .Kind);
           static    void AssertLinear   (Color4   colour, float    r,    float   g, float   b ,  float a)   {
                Assert.Equal    (r    , colour.R , Tolerance  ); Assert    .Equal( g    , colour. G   , Tolerance);
        Assert  .Equal   (b, colour    .B, Tolerance); Assert   .Equal  (a, colour.A,    Tolerance);
    }
  // ---------------------------------------------------------------- oklab() and oklch()
            
           [Theory] [InlineData ("oklab(1 0 0)"    , 1f, 1f,    1f)  ]
			[InlineData  ("oklab(0.627955 0.224863 0.125846)", 1f,  0f, 0f
            )]
    [InlineData("oklab(0.866440 -0.233888 0.179498)" ,  0f, 1f    , 0f)] [InlineData(   "oklab(0.452014 -0.032457 -0.311528)", 0f, 0f, 1f)  ]
   [InlineData   ("oklab(0 0 0)"    , 0f, 0f  , 0f)] public void   Oklab_parses_to_the_linear_colour_its_author_published(string  text  , float r  , float g, float   b) {
// Ottosson's own table, read backwards. `OklabTests` checks linear-in/Oklab-out against these
    ...
    InlineData(    "oklch(0.627955 0.257681 29.2338)"   , 1f, 0f, 0f   )    ]
           [InlineData("oklch(0.866440 0.294827 142.4953)", 0f, 1f, 0f)    ]    [  InlineData  ("oklch(0.452014 0.313214 264.0520)",  0f, 0f, 1f    )]
			public void Oklch_is_the_same_space_in_polar_form(   string text  ,  float r, float
         g,    float   b) { 
...
[Fact] public void    The_lightness_percentage_is_against_one_and_the_chroma_percentage_is_against_zero_point_four    (   ) {
        ...
        // merely *bolder* than intended rather than obviously broken. CSS Color 4 § 9.3.
         AssertLinear(Color("oklch(50% 0 0)" ), 0.125f  , 0.125f , 0.125f  , 1f    ); Assert.Equal(Color("oklch(0.6 0.2 30)"),  Color(   "oklch(60% 50% 30)") ) ;
       Assert. Equal(
Color  ("oklab(0.6 0.2 -0.1)"), Color    ("oklab(60% 50% -25%)"   ))  ;
        }

   [ Theory  ]
    [  InlineData ("oklch(0.7 0.1 90)")]    [InlineData("oklch(0.7 0.1 90deg)"    )    ]
            [InlineData("oklch(0.7 0.1 100grad)")
     ]
    [InlineData("oklch(0.7 0.1 0.25turn)"   )] 
   public   void    A_hue_is_degrees_with_or_without_a_unit  (string    text  ) { // ⚠ The bare-number case is not a convenience: every colour Tailwind v4 ships is written
 ...
 AssertLinear(Color  (text)    , radians.R
  , radians.  G, radians.B, 1f);

         }
               
    /// <summary>A colour encoded to sRGB and rounded to bytes, clamped the way a display would.</summary>
       static   
      string Clamped (    string text    ) {
        var
          srgb    = Color(text ) .ToSrgb(    )   ;
...
var blue
      =   Color("oklch(62.3% 0.214 259.815)"    ).  ToSrgb()  ;
        var red = Color("oklch(63.7% 0.237 25.331)").
ToSrgb ();
             var
           emerald  =   Color (   "oklch(69.6% 0.17 162.48)").ToSrgb ()    ;
     
        Assert . True(blue.B > 1f,    $"blue-500 is outside sRGB, got {blue.B}")    ;
Assert.True(
emerald.   R <   0f, $"emerald-500 is outside sRGB, got {emerald.R}")   ;
Assert   .   InRange(red.R    ,  0f ,    1f ) ; Assert. InRange(  red.G,    0f, 1f)  ;
Assert  .InRange(red.B   ,  0f,
      1f    );
        ...
        Assert.Equal("#2b7fff"    , Clamped("oklch(62.3% 0.214 259.815)")); Assert   .  Equal("#fb2c36"    , Clamped("oklch(63.7% 0.237 25.331)"));
Assert.    Equal("#00bc7d"
, Clamped("oklch(69.6% 0.17 162.48)"));
		}

             [ Fact  ]
    public void Alpha_is_read_off_the_slash_in_either_notation    (    )    {   AssertLinear(   Color(  "oklch(0.5 0 0 / 0.4)")  , 0.125f, 0.125f, 0.125f, 0.4f);   
        AssertLinear
(Color("oklch(0.5 0 0 / 40%)"),  0.125f   , 0.125f  , 0.125f, 0.4f   );
              AssertLinear(Color    (
            "oklab(0.5 0 0 / 40%)"    ),    0.125f, 0.125f , 0.125f    , 0.4f); 
               }
   ...
   [   Fact  ]
    public    void  A_missing_component_is_zero() {
      ...
      // specification says a missing component behaves as in every context that has none.
          Assert  .Equal(Color
      (  "oklch(0.5 0 0)"), Color("oklch(0.5 none none)" ))   ;
             Assert .
Equal(  Color   ("oklab(0.5 0 0)"), Color("oklab(0.5 none none)"));
	}
    [Fact  ] 
              public void A_negative_chroma_is_folded_to_zero_and_a_negative_hue_is_not() { // Only one of the two has a meaningless side. -30° is 330°, and clamping it would silently
                // move the colour a third of the way round the wheel.
                Assert    .Equal  ( Color("oklch(0.5 0 0)"
),  Color  ( "oklch(0.5 -0.2 40)") );   
              Assert.Equal   (
Color    ("oklch(0.6 0.1 330)" ), Color("oklch(0.6 0.1 -30)"   ));
             }
      [ Theory]
...
InlineData("oklch(0.5 0.1 20rpm)") ]
           public void   
           A_malformed_colour_function_is_unknown_rather_than_a_guess( string text    )   =>   AssertUnknown(text   );
...
Fact]
  public void An_out_of_gamut_colour_is_carried_through_rather_than_clamped( ) {
     ...
     "oklch(0.7 0.4 30)");
         Assert.True(wide.
B <   0f, $"expected a negative blue channel outside sRGB, got {wide.B}"  );
  Assert.  False   (float.IsNaN(wide    .R ) ||  float.IsNaN(wide   . G) || float.IsNaN   (wide.B)
                ,  "no channel is NaN"   );
        ...
        // linear segment handles negatives — a `pow()` approximation there would give NaN, which is
            // how "carry it unclamped" turns into a black element three layers downstream.
               var    written = wide. ToSrgb   ( ); Assert.  False(float .IsNaN    (written.R  ) || float  .IsNaN(  written.G) ||   float.   IsNaN(written.   B)   ,   "no NaN out"  );
 ...
 Fact]
      ...
      AssertLinear(Color(  "color-mix(in srgb, white, black)"),   0.2140f,  0.2140f, 0.2140f  , 1f);   AssertLinear (Color("color-mix(in srgb-linear, white, black)"), 0.5f, 0.5f,    0.5f, 1f);
...
AssertUnknown    (   "color-mix(in lab, white, black)"    ) ;
  AssertUnknown    ("color-mix(in hsl, white, black)"  )    ; AssertUnknown("color-mix(in xyz, white, black)");  
        AssertUnknown("color-mix(oklab, white, black)"    );
  }  
    /// <summary>The midpoint hue each method produces, as the colour it lands on.</summary>
              static void AssertMidpointHue( string method, float from  , float 
to, float hue) {  
              var    mix
= $"color-mix(in oklch{method}, oklch(0.7 0.1 {from}), oklch(0.7 0.1 {to}))"   ;
        var
landed = Color($"oklch(0.7 0.1 {hue})"  );
	
        AssertLinear(   Color(mix    ),
landed.R, landed.G , landed.B,    1f  );
	}
     ...
     //
      // The pair of columns that swap under reversal is what distinguishes `increasing` and
         // `decreasing` from the two symmetric methods, and nothing else in this file does.
           ...
           AssertMidpointHue(    " longer hue"    ,
10f , 30f,  200f);
   AssertMidpointHue(" increasing hue", 10f , 30f, 20f   );
                AssertMidpointHue(" decreasing hue"   ,
   10f, 30f, 200f    ) ;
...
30f, 10f , 200f   );
  AssertMidpointHue(" increasing hue"   , 30f  ,   10f, 200f  )
        ;
       AssertMidpointHue  (" decreasing hue"    , 30f   ,    10f
,  20f);
    ...
    [Fact]   public void    A_hue_method_belongs_to_a_polar_space_and_nowhere_else   () {
	...
	AssertUnknown  (
  "color-mix(in oklab longer hue, white, black)");   
 AssertUnknown    ("color-mix(in srgb shorter hue, white, black)"
    ...
    [Fact]   public void    One_percentage_omitted_means_the_other_is_its_complement() {
 // Case one of three. CSS Values 5, step 2: what is left of 100% goes to the one that said
           // nothing.
        Assert.Equal(  Color(   "color-mix(in oklab, white 30%, black 70%)"),
  ...
  Assert  .Equal( Color ("color-mix(in oklab, white 30%, black 70%)") ,
        ...
        // between the two of them.
         Assert.   Equal(Color("color-mix(in oklab, white 50%, black 50%)")  , Color
("color-mix(in oklab, white, black)")) ;
	...
	var
under  =    Color ("color-mix(in oklab, white 20%, black 60%)"  )    ;
     var opaque   = Color (    "color-mix(in oklab, white 25%, black 75%)");   
        AssertLinear(   under,  opaque.  R
, opaque.G  , opaque.B, 0.8f);
      // Over 100% there is no shortfall, so the weights scale and the alpha does not.
       var  over = Color
   ("color-mix(in oklab, white 60%, black 60%)"   );
            AssertLinear(
 over, 0.125f, 0.125f , 0.125f ,    1f)  ;
...
Assert.Equal(Color("color-mix(in oklab, white 100%, black 0%)"), Color   ("color-mix(in oklab, white 140%, black 0%)")    ); }
 ...
 // make it do so. Step 4 does not fire because the total is not above zero, step 5 makes the
	...
	var   value =   Parser(
              ) .   Parse    (   "color-mix(in oklab, red 0%, blue 0%)");
        Assert .Equal(StyleValueKind.Color, value.Kind)  ; AssertLinear(    value.Color   , 0f  , 0f  ,  0f, 0f); 
    }
...
// `<color> && <percentage>?`, and `&&` is order-free.
 ...
 Color( "color-mix(in oklab, 30% white, 70% black)") );
         ...
         // ---------------------------------------------------------------- color-mix: premultiplied alpha
          [Fact   ]
    public void Mixing_with_transparent_keeps_the_colour_and_moves_only_the_alpha(    )  { // ⚠ The whole reason `color-mix()` is the right answer to the opacity modifier, and it works
   ...
   ;
     var opaque = Color(   "#4f7cff")    ;
                AssertLinear( half    , opaque.
         R,    opaque. G  , opaque.B, 0.5f);
      ...
      // Which makes it exactly what the old `rgba()` rewrite produced for a hex colour — the point
       ...
       // already worked.
    AssertLinear  (half,  Color( "rgba(79, 124, 255, 0.5)"
   ).R, Color  ( "rgba(79, 124, 255, 0.5)").G, Color   (   "rgba(79, 124, 255, 0.5)").B, 0.5f    )   ;  
    }
  [
            Fact    ]
   ...
   var rectangular = Color(    "color-mix(in oklab, blue 50%, transparent)" )  ;
		var    polar    = Color
(  "color-mix(in oklch, blue 50%, transparent)");
        
          AssertLinear(rectangular, Color    (  "blue") .R
     , Color("blue") .G, Color("blue")   .B, 0.5f);
  ...
  [Fact ] public void An_endpoint_may_be_written_any_way_the_parser_understands    () {
// ⚠ Not a completeness exercise. ExCSS normalises `red` to `rgb(255, 0, 0)` only where it
 ...
 Assert. Equal(expected  , Color(  "color-mix(in oklab, red 50%, blue 50%)"))  ;
   Assert.  Equal    (expected, Color("color-mix(in oklab, #ff0000 50%, #0000ff 50%)" )) ;  Assert.Equal    (expected,  Color("color-mix(in oklab, #f00 50%, #00f 50%)" ));
         AssertLinear( Color  ("color-mix(in oklab, oklab(0.627955 0.224863 0.125846), transparent)" )   ,
1f ,
    0f
               ,
  ...
  [Fact    ] public void A_mix_may_hold_a_mix() {
      ...
      Assert
		...
		Color
        (  "color-mix(in srgb-linear, color-mix(in srgb-linear, white 50%, black 50%) 50%, black 50%)" )
            );
    }
             
  [Fact]   public    void The_interpolation_method_may_be_left_out() {
  ...
  Assert.Equal(Color(    "color-mix(in oklab, white, black)") ,   Color("color-mix(white, black)"));
         ...
         // ---------------------------------------------------------------- against the real cascade
          [Fact  ]   
    public void The_mix_is_evaluated_after_var_substitution_and_never_sees_the_variable  () { // ⚠ **The order of operations, proved rather than reasoned about, because it is the one place
   ...
   // on what a `ComputedStyle` ended up holding. So `color-mix()` needs no notion of `var()`,
      ...
      var fixture = new  CascadeFixture  (   ) ;   
        fixture    .Load   (
           ...
           var written
           = fixture .Value(fixture .Tree .    CreateElement  (    "div"   , classNames  : [ "written"]));
 var held =    fixture.Value(fixture   .  Tree.
 CreateElement  ("div" , classNames:    ["held"]   ))    ;
             Assert    .    Equal("color-mix(in oklab, #4f7cff 50%, transparent)", written    )   ; Assert. Equal(  written,   held);
        // And the text both of them arrive as is a colour, at half alpha, with the hue intact.
    AssertLinear    (
  Color(  held!), Color("#4f7cff"    )  .R, Color   ("#4f7cff").    G, Color(   "#4f7cff").B, 0.5f );
             }
  [Fact  ]
	...
	fixture   .    Value(fixture.Tree.CreateElement  (    "div"
                , classNames: [    "a" ]))  
  )  ;
             
   Assert.
                Equal   ("rgb(255, 0, 0)", fixture  .   Value  (fixture  .    Tree    .CreateElement(  "div" ,  classNames: [    "b"]))) ;
  
   Assert    .    Equal
              (
        "color-mix(in srgb, red 50%, blue)",
     fixture.Value   (fixture  .  Tree   .   CreateElement("div" , classNames: ["c"
               ]), "background-color")
           );
    }
        
    [Theory] [  InlineData  (    "color-mix(in oklab, red)")   ]
      [InlineData
            ("color-mix(in oklab, red, blue, green)"  )  ]   
    [InlineData(  "color-mix(in oklab, 50%, blue)" )]
    [ InlineData("color-mix(in oklab, red 50% 20%, blue)")] [    InlineData(  "color-mix(in oklab, notacolour 50%, blue)")]
