    ...
    public class RemesherTests {
        ...
        public void A_closed_surface_comes_back_solid(string name) {
            ...
            Assert.True(
                ...
                MaxDeviation
                < 0.2f,
                $"The deviation is {report.MaxDeviation:E3} of the diagonal."
            );
            Assert.NotEmpty(
                quads.Positions
                    .ToArray()
        ...
        public void A_remesh_is_the_same_bits_at_any_worker_count() {
            ...
            for (var run = 0; run < 3; run++) {
                Same(
                    first,
                    Remesher.Remesh(Fixture("sphere"), settings, out _),
                    $"run {run}"
                );
            }
        ...
        public void Symmetry_is_applied_rather_than_apologised_for(
            ...
            Assert.DoesNotContain(
                report.Warnings,
                warning => warning.Contains("not applied yet", StringComparison.Ordinal)
            );
            Assert.Equal(0, report.NonQuadCount);
        }
        ...
        internal static EditMesh
            ...
            name switch {
                ...
                "sphere" => MeshShapes.Create(
                    ShapeParameters.Default(
                        ShapeKind.
                        Sphere
                    ) with { Sides = 24, Steps = 12 }
                ),
