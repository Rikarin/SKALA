...
using CsCheck; using Vixen.Core.Mathematics;
using Vixen.Geometry.Testing; using Xunit;

namespace
    Vixen.Geometry.Remeshing.Tests;

    /// <summary>
    ///     Stage one's contract stated over the space of broken meshes rather than over eighteen of them.
    /// </summary>
    /// <remarks>
    ///     <para>
    ...
    public
    class ConditioningPropertyTests {
            ...
            Gen.
                ...
                Sample(
                    entry => {
                        ...
                        var what =
                            $"conditioning {recipe} with {rounds} rounds, fill {fill}, shrinkwrap {shrinkwrap}";
                        var
                        mesh = RunawayGuard.Run($"building {recipe}", () => BrokenMeshSpace.Build(recipe));
                        ...
                        Assert.True(view.Report.Welded >= 0, what); Assert.True(view.Report.Unorientable >= 0, what);
                        Assert.
                            True(view.Report.Despecked >= 0, what);
                        Assert.True(view.Report.Cut
                            >= 0, what);
                        Assert
                            .True(view.Report.Filled >= 0, what);

                        foreach (var corner in view.Mesh.Triangles) {
                            Assert.InRange(corner, 0, Math.Max(view.Mesh.VertexCount - 1, 0));
                        }

                        foreach (var position in view.Mesh.
                                 Positions) {
                            Assert.True(
                                float
                                    .IsFinite(position.X)
        ...
        public void Filling_holes_disagrees_by_at_most_one_hole_across_six_orders_of_magnitude() {
            BrokenMeshSpace.Recipe.Sample(
                recipe => {
                    var mesh = RunawayGuard.Run($"building {recipe}", () => BrokenMeshSpace.Build(recipe));
                    var
                    settings = new ConditioningSettings { PreRemeshIterations = 0, FillHoles = true, HoleSize = 0.3f };

                    var small = RunawayGuard.Run(
                        $"filling holes in {recipe} at a thousandth",
                        () => Condition(mesh, settings, 1e-3f)
                    );
                    var large = RunawayGuard
                        .Run(
                    ...
                    Assert.True(
                        Math.Abs
                        (small.Report.Filled - large.Report.Filled)
                        <= 1,
        ...
        public void One_round_of_the_pre_remesh_stays_within_a_fifth_at_both_scales() {
            ...
            BrokenMeshSpace.Recipe.Sample(
                recipe
                    => {
                    var mesh = RunawayGuard.Run(
                        $"building {recipe}",
                        (
                        ) => BrokenMeshSpace.Build(recipe)
                    );
                    var settings
                        = new ConditioningSettings { PreRemeshIterations = 1 };

                    var
                    size = RunawayGuard.Run(
                        $"sizing {recipe}",
                        () => {
                            MeshConditioner
                                .Condition(mesh, settings, out var report);
        ...
        /// <summary>
        ///     ⚠ And the guard on the guard: a scale property proves nothing about a mesh that has no size.
        /// </summary>
        [Fact]
            ...
            BrokenMeshSpace.Recipe.Sample
                (
                    recipe
                        ...
                        var
                        large = Diagonal(BrokenMeshes.Scaled(mesh, 1e+3f));
        ...
        static (ConditioningReport
            Report, int Vertices) Condition(
            EditMesh mesh,
            ConditioningSettings settings,
            float scale
        ) {
            var view = MeshConditioner.
                Condition(BrokenMeshes.Scaled(mesh, scale), settings, out var report);
        ...
        static float Diagonal(EditMesh mesh) {
            var box = mesh.Bounds;
            return
                (box.Maximum - box.Minimum).Length();
        }
    }
