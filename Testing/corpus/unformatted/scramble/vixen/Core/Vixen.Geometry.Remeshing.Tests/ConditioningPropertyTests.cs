 ...
 class ConditioningPropertyTests {
  ...
  Floor = 64;
    ...
    /// </remarks>
     ...
     Gen .
             Select(
                BrokenMeshSpace.Sized, Gen.Int[0 ,  3    ],
  Gen. Bool
,
    Gen.  Bool,
...
mesh    = RunawayGuard.    Run(   $"building {recipe}", (   ) => BrokenMeshSpace.   Build  (  recipe));
    ...
    what,  () => MeshConditioner. Condition(   mesh, settings   , out var report) is  var conditioned
...
.True  (   view.    Report. Filled >= 0    , what);
  ...
  foreach    (var corner in view.Mesh.Triangles) { Assert.InRange (corner, 0, Math  . Max(view.Mesh    .VertexCount - 1 ,  0));
       }
foreach (var  position in  view.Mesh.
Positions) {
     Assert .  True(
                            float
           .  IsFinite    (   position.X)   &&    float.IsFinite    (position .Y)  && float  .IsFinite(    position.Z),
    ...
    /// </remarks>
          [Fact]
    public void    The_first_four_steps_agree_on_their_topology_at_both_scales(  ) { BrokenMeshSpace    .  Recipe.    Sample(
            recipe   => {
                var mesh = RunawayGuard   .Run    (   $"building {recipe}",   (   )   => BrokenMeshSpace.Build(
  recipe))    ;   
          RunawayGuard .
Run(
                    $"conditioning {recipe} at both scales",
		() => ScaleInvarianceTests  .Compare( recipe .  ToString (),
       mesh, new() { PreRemeshIterations   = 0, FillHoles = false },
    ...
    public void Filling_holes_disagrees_by_at_most_one_hole_across_six_orders_of_magnitude() { BrokenMeshSpace.Recipe.Sample(
             recipe  => {
...
mesh  ,   settings, 1e+3f)
	...
	Assert.True (
            small  .Report    .Mesh.IsManifold && large.Report.   
  Mesh  .   IsManifold  ,
                    $"{recipe}: {small.Report.Mesh.Describe()} against {large.Report.Mesh.Describe()}."
) ;
       
       Assert  .    True   ( small  .Report    .Unorientable == large.Report.Unorientable,
                    $"{recipe}: unorientable {small.Report.Unorientable} against {large.Report.Unorientable}."
                );
   Assert    .   True (
               Math    .Abs
 ( small.Report.Filled - large. Report.Filled) <= 1,
                    $"{recipe}: filled {small.Report.Filled} against {large.Report.Filled}."
             ) ;
              Near(    recipe.ToString()   , "triangles", small.Report.Triangles, large   . Report   .Triangles); Near(recipe. ToString (    )  , "vertices",    small.Vertices, large.Vertices);
      }, iter: PropertyBudget    .Iterations(150)   ,
            threads:
               1
        );  }
 ...
 recipe
		=> {
                var  mesh =  RunawayGuard.  Run    ($"building {recipe}"  , (
 ...
 ToString  (),
    mesh ,
     settings, ScaleInvarianceTests.    Agreement   .    Rate ,
        0.2f )
         )    ;
                }   ,
       iter: PropertyBudget  .   Iterations(100),    threads: 1
        )  ;
   ...
   // deviation of about 5. Fifteen is four deviations down: a generator would have to have
	...
	.Sample( 
        ...
        var size = RunawayGuard.    Run(
                    $"sizing {recipe}",
         () => {
                        MeshConditioner   .Condition(mesh, settings
        , out var report  );
  return  report.Triangles;
}   
   );
...
recipe.ToString()    ,    mesh,
      ...
      ScaleInvarianceTests.
       ...
       iter  : PropertyBudget. Iterations(
  150)   ,
threads  :    1
      )
;
...
, $"Only {measured} of the sampled recipes reached {Floor} triangles.") ;
    }
 ...
 recipe
         => {
            var mesh = BrokenMeshSpace .  Build(  recipe)
              ;
                ...
                if   (    mesh. PositionCount == 0) {    return;
         }
  ...
  var 
      ...
      Assert.True(
small > 0f,   $"{recipe} has no extent at all.");
	Assert .True(large / small > 1e5f
, $"{recipe} only grew by {large / small:E2}."    ) ;
...
ConditioningSettings settings
 ,
  ...
  var view =  MeshConditioner.
    ...
    static    void Near(    string name  ,   string what, int one
            , int two   )    {
            var span = Math   .Abs(  one - two   )    ;
 var bound  =    Math. Max    (    one
        , two    ) * 0.1f   ;   

        Assert.    True (    span   <=  bound
,   $"{name}: {one} {what} against {two}.");
       }
            
      static  float Diagonal  (EditMesh mesh ) { var box = mesh.    Bounds  ;
             return
