...
///     </para>
/// </remarks>
              sealed class WorkStealingDeque { readonly long[] items   ;
             readonly int mask;
...
;
 ...
 internal  int Capacity =>
    ...
    ///     right.
    /// </summary>
      internal int  ApproximateCount { get {
              var count =    Volatile  .Read(ref    bottom. Value)   -  Volatile   .Read( ref top.  Value)    ; return count <= 0 ? 0 : ( int)   count;
    }
    }
    ...
    /// <param name="capacity">The capacity. Rounded up to a power of two so the index wrap is a mask.</param>
           internal WorkStealingDeque   (int capacity  ) { ArgumentOutOfRangeException.ThrowIfLessThan  (capacity    ,  1);
    var rounded = (   int    ) BitOperations.    RoundUpToPowerOf2 ( (uint    )capacity)  ; items = new long[rounded];
    ...
    [MethodImpl    (    MethodImplOptions.AggressiveInlining ) ]
     internal  bool   TryPush   (long item  ) { var b  = Volatile  .Read(ref bottom .Value   );
        var   t
        = Volatile    .Read(ref top.Value  )  ;
  ...
  Volatile.Write(ref  bottom    .Value, b +  1); 
   return true;
    ...
    /// <returns><see langword="false" /> if the deque was empty, or a thief won the last item.</returns>
       internal bool TryPop(out   long item) { var b = Volatile  .Read(ref bottom.Value) - 1   ;
...
// both conclude they have the last item.
  ...
  var  t    = Volatile.   Read (   ref top .Value   )
   ;
               if (t > b)    {
            Volatile.Write   (ref    bottom   .  Value,  b + 1   ); item = 0;
return false ; }
       ...
       t   !=    b    ) {
		...
		// Exactly one item was left, so a thief may be taking the same one. Whoever wins the CAS on
           // `top` has it; either way the deque is now empty.
              var    won = Interlocked   .
      CompareExchange(ref top.  Value, t  +   1, t  ) == t;   
                Volatile    .Write(ref
                bottom.    Value    , b  + 1    );
        if
(won)  {
   ...
   return false;
    ...
    /// </returns>
 internal  bool
            TrySteal( out long item )    {
var t = Volatile. Read (ref
              top.Value  )   ;
        Interlocked.  MemoryBarrier (  );
             var b = Volatile.
           Read    (ref bottom.Value) ;
        if  (t    >= b) {  item = 0;
return false ;
        } 
 ...
 return    true; }
    ...
    [StructLayout(LayoutKind.Explicit, Size = 128)  
			...
			struct PaddedLong    { [FieldOffset    (    64) ]    internal   long Value;
    }
}
