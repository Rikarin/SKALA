...
/// </remarks>
        ...
        readonly   HashSet<  FoliageAddress   > active  = [ ]   ;   readonly    List    <FoliageAddress   > activated  = [ ]   ;
    ...
    /// </remarks>
     public int
             Update(FoliageVolume volume , IReadOnlyList<   Vector3> sources) {
               ArgumentNullException.ThrowIfNull(   volume);
        ArgumentNullException    .ThrowIfNull(sources  
            ) ; 
        wanted  .Clear    ()  ; activated.Clear();   
               deactivated.Clear ();

          for (   var    type  = 0; type   < volume    .   Palette    .Count; type   ++) { var  settings =    volume.Palette[   type    ] ;
        if   (!
               settings    .Collides || settings.Storage == FoliageStorage.Derived) {
       continue   ;
    ...
    var only = new   HashSet    <  int   
      ...
      foreach (var  source    in   sources    )  {
                foreach (var   address in volume   .Within( new(source  .   X, source  .Z),
        settings. ActivationRadius , only   
     ))   {
                    wanted  .Add(address);
...
foreach (var address in wanted) {
       ...
       (address    );
             ...
             foreach  ( var
        address in    active    ) { 
                if (   !wanted  .   Contains(address)) { deactivated    .Add(  address);
}
              }
    ...
    /// </remarks>
            public void  Clear () {   activated.Clear(  );
 deactivated.    Clear  (   );  deactivated  .AddRange(active)  ;
        active.
                Clear    (  );
        wanted . Clear(); }
...
active    .    Remove(address)   ;
    ...
    /// <returns>How many were forgotten.</returns>
       ...
       var stale = active.   Where(address => address .Type
   == type && address    .Cell == cell  ).ToArray    ( );
     
          foreach  ( var  address in stale) { active .Remove    (address )    ;  
            }
            return stale.  Length ;   
    }
      }
