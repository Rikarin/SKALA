...
namespace
    ...
    public readonly record struct DisplayMode(
        ...
        /// <inheritdoc />
        public override
            string ToString() => $"{Size.X}×{Size.Y} @ {RefreshRate:0.##} Hz{(IsHdr ? " HDR" : string.Empty)}";
    }

    /// <summary>One display, as the platform describes it.</summary>
    /// <param name="Index">
    ///     Its position in <see cref="IDisplayInfo.Displays" />, stable until a
    ///     <see cref="PlatformEventKind.DisplaysChanged" />.
    /// </param>
    /// <param name="Name">A human-readable name, for a settings screen.</param>
    /// <param name="Bounds">
    ///     Its position and size in the desktop's coordinate space, in logical
    ///     points.
    /// </param>
    /// <param name="WorkArea">
    ///     The part of <paramref name="Bounds" /> not covered by a taskbar, dock or
    ///     menu bar — where a window should be placed.
    /// </param>
    /// <param name="DpiScale">Physical pixels per logical point.</param>
    /// <param name="CurrentMode">The mode it is running now.</param>
    /// <param name="Modes">
    ///     Every mode it can be switched to, or empty where the platform does not allow
    ///     mode switching.
    /// </param>
    ...
    public sealed record DisplayInfo(
        ...
        public
            bool SupportsHdr => CurrentMode.IsHdr || Modes.Any(mode => mode.IsHdr);

        /// <inheritdoc />
        public override string ToString(
        ) => $"[{Index}] {Name} {CurrentMode} ×{DpiScale:0.##}";
    }
    ...
    public interface IDisplayInfo {
        ...
        /// <summary>
        ///     The display the OS considers primary, or <see langword="null" /> when there are
        ///     none.
        /// </summary>
        DisplayInfo?
            Primary { get; }
        ...
        /// <returns>
        ///     <see langword="false" /> when there are no displays, or the window is not on
        ///     one.
        /// </returns>
        bool TryGetForWindow(
            IWindow
            window,
            [NotNullWhen(true)] out DisplayInfo? display
        );
