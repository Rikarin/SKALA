...
/// <param name="Extensions">Extensions without the dot, such as <c>["vxscene"]</c>.</param>
             public readonly record struct FileFilter(    string Name    ,    params string [
              ] Extensions);
...
/// <summary>What to show in a message box.</summary>
    ...
    /// <summary>The bold first line.</summary>
     public string Title { get;   init; } = string.
       Empty  ;
...
public string Message { get  ; init; } = string.Empty    ;
    ...
    /// <summary>How serious it is.</summary>
       public
  MessageBoxSeverity Severity { get; init; } = MessageBoxSeverity.Information  ;   
}
...
public   interface INativeDialogs {
    ...
    /// <returns>A native path, or <see langword="null" /> if nothing was chosen.</returns>
        ValueTask  <  string?> OpenFileAsync 
         ...
         FileDialogOptions  options    ,
               IWindow ? owner = null,    CancellationToken cancellationToken = default
     );
    ...
    /// <returns>Native paths, empty if nothing was chosen.</returns>
         ValueTask<
               IReadOnlyList<string>> OpenFilesAsync(
  FileDialogOptions options, IWindow? owner  = null    ,
    ...
    /// <param name="cancellationToken">Dismisses the dialog, where the platform allows it.</param>
    /// <returns>A native path, or <see langword="null" /> if nothing was chosen.</returns>
  ValueTask<string?> SaveFileAsync( FileDialogOptions options,  
        IWindow?
        owner = null    , 
          CancellationToken  cancellationToken =   default
       );
    ...
    /// <param name="cancellationToken">Dismisses the dialog, where the platform allows it.</param>
    /// <returns>A native path, or <see langword="null" /> if nothing was chosen.</returns>
 ValueTask<  string?>    OpenFolderAsync  ( FileDialogOptions options ,
              IWindow?    owner  
     =   null,
      CancellationToken  cancellationToken =    default   
    ) ;
...
ValueTask<  MessageBoxResult > ShowMessageAsync    ( MessageBoxOptions options  , 
             IWindow?   owner   = null,
               CancellationToken   cancellationToken   = default
    )    ;
    }  
