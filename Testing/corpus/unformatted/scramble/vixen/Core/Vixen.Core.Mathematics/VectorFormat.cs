...
charsWritten = count ; return true;
            ...
            public static    string   ToString    (string
             ? format, IFormatProvider? provider,  ReadOnlySpan   <float > components) {
	...
	// custom format wide enough to overflow it falls back rather than truncating.
            ...
            ?
          new (    buffer[..written])
            : FormatSlow(format, provider,
         components);
   }
      
    static string FormatSlow   ( string    ? format, IFormatProvider
		? provider,    ReadOnlySpan<float> components) {
