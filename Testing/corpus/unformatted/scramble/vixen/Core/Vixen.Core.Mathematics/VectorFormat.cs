...
// Counted separately and published only on success: ISpanFormattable requires charsWritten
 ...
 provider ??= DefaultProvider  ;
       ...
       if    (    !  TryAppend(destination, ref  count,    "(")    ) {    return  false ;
        }
             
        for (    var i =   0
  ; i  <  components    .  Length; i   ++) {
 if
         (   i  >   0 && !TryAppend (destination,  ref count,    ", ")) {
      return false;
     }
            if (!  components[
i].TryFormat(destination[count.. ], out   var written,    format,   provider    )) {
return    false;
   }   
            count
             += written;
         ...
         charsWritten = count    ;   return    true;
   }
      
    public static string ToString(
		string? format  ,   IFormatProvider? provider, ReadOnlySpan< float> components   )  {
     ...
     // custom format wide enough to overflow it falls back rather than truncating.
                Span<char  >  buffer = stackalloc char[    640 ]; return TryFormat(buffer, out var  written, format ,    provider, components)
               ? new(buffer[..written])
   : FormatSlow(format   , 
   provider ,    components);
    }
    static string FormatSlow(    string? format
          , IFormatProvider? provider,    ReadOnlySpan  <  float>    components) {
           provider ??= DefaultProvider ;
               var
builder    = new System.   Text  .   StringBuilder(   "(");
    ...
    builder .Append(components[i]  .   ToString   (  format, provider)  );
        ...
        return builder .Append(')').   ToString(   );  }
          ...
          static    bool TryAppend
(Span<char > destination,   ref int  charsWritten, ReadOnlySpan<char> text)  {
if (destination.Length  - charsWritten < text.
              Length) {  
            return false ; }
text 
