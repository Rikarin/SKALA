 ...
 using Vixen.    Ecs; using    Vixen.    Engine.  Transforms ;
                using Vixen.Physics.    Bodies; using Vixen.Physics.   Events;
...
/// <param name="Other">What entered or left it.</param>
		...
		EntityTriggerEvent( ContactPhase Phase  , Entity Sensor    , Entity Other);  
...
///     </para>
/// </remarks>
                public    sealed    partial class PhysicsScene   : IDisposable {   static  readonly    QueryDescription NeedBody =   new QueryDescription   ( )
        .WithAll <Collider, LocalTransform>() .WithNone<    PhysicsBody>(  );
...
Entities { get; }
   ...
   public bool IsDisposed {    get   ; private set   ;  }
    ...
    /// <param name="settings">How the simulation is configured, or <see langword="null" /> for the defaults.</param>
     public PhysicsScene(
           EcsWorld entities, PhysicsWorldSettings? settings = null) {
...
World = new(    settings
    ...
    /// <remarks>For a host that already has a world — a replay, a server that shares one across scenes.</remarks>
      public PhysicsScene(EcsWorld entities, PhysicsWorld world) {
               ArgumentNullException.ThrowIfNull(entities   )   ; ArgumentNullException    .ThrowIfNull  (world);
...
;
    ...
    /// <returns><see langword="true" /> if the body is one of this scene's.</returns>
           ...
           entitiesByBody.
   TryGetValue    (body. Value   , out entity);
    ...
    public bool
            TryGetBody(Entity    entity  , out BodyHandle    body) {
            if (Entities.  IsAlive( entity    ) && Entities.
        TryGet    <PhysicsBody>(entity    , out  var component )    ) {
 ...
 CollectStructuralChanges(    )
  ;
            ApplyStructuralChanges    () ;   PushAuthoredState(deltaTime)  ;  
    } 
...
= World.   Step(deltaTime);
    ...
    contactEvents
       .Clear() ;
        ...
        foreach (    var    contact
	in World.Contacts   )    {  
               contactEvents.Add( new(
    contact.
         Phase,
           EntityOf(contact  . First), EntityOf  (   contact  . Second    ),
...
foreach (var chunk  in  Entities.Chunks   ( HaveBody))    { var  bodies   = chunk.ReadValues    <  PhysicsBody    >    ();
       var transforms = chunk.Values<LocalTransform>(    );
     var entities =   chunk.
  Entities;
  for ( var index =    0 ; index <    chunk.    Count    ; index ++) { var body =  bodies[index]    ;
...
World .IsAlive    (    body.    Handle)) {
      ...
      World .
GetTransform  (body.Handle,    out var position, out  var rotation  );
      transforms[index]   .Position =  position;  transforms    [index].    Rotation =  rotation   ;   
                var entity    =   entities[  index ];
            
  if (   Entities.Has<LinearVelocity>(entity  )) {   
              Entities   .Get<
LinearVelocity  >(  entity).Value    = World.GetLinearVelocity  (body    . Handle);
            }
                if (Entities.    Has
        <AngularVelocity>  (entity  ) )    {
         Entities.Get< AngularVelocity   >(
entity)  .Value = World.GetAngularVelocity    (body. Handle);
           }
                if    (Entities .Has<    PhysicsInterpolation> (entity)) { ref var interpolation =    ref Entities.Get<   PhysicsInterpolation>(entity    );
  interpolation.   PreviousPosition = interpolation.CurrentPosition; interpolation.   PreviousRotation = interpolation. CurrentRotation;  
      interpolation.CurrentPosition =    position ; interpolation   .  CurrentRotation = rotation  ;
   }  }
         }
 }
 ...
 foreach (  var  chunk in    Entities.   Chunks(NeedBody))  {   var entities =   chunk    .Entities   ;
            for (var  index = 0
; index  < chunk. Count; index++) {
pendingCreate
	.Add  (entities[    index]);
}
  }   
...
pendingDestroy.Add   (entities [index  ]    );
  ...
  // path at creation. Anything else — mass, damping, friction — is a setter, and is applied in
           ...
           foreach   (var chunk in Entities.Chunks
                (  HaveBody) )  {
   var  bodies = chunk.ReadValues    <    PhysicsBody  >( ); var entities    = chunk .Entities;
             
           for   (var    index = 0; index < chunk.Count; index  ++) { var entity = entities[index    ]  ;
             if (!Entities.TryGet   <  Collider>(   entity, out var collider)    ) {
                    continue; } 
                var motion =  Entities.
  TryGet  <  RigidBody>(entity  , out var rigid)
     ? rigid .  Motion
   : BodyMotion.   Static;
        ...
        if (body.    BuiltShape != collider. Shape  || body.BuiltMotion !=  motion
           ...
           foreach (
            ...
            Remove(entity);
              Entities .    Remove  <PhysicsBody>  (entity);
               ...
               foreach (   var entity in pendingRebuild) { Remove(entity    );
           Entities.Get   <PhysicsBody>(entity)
= Create( entity  );
        }
  ...
  var transform = Entities    .
	Read<LocalTransform>(entity  ) ;
               var hasRigid = Entities
    .TryGet  <RigidBody> (entity    ,  out var   rigid);  
    ...
    with {
       ...
       collider .   Shape   ,
             Position  = transform.Position ,
                Rotation = transform    .  Rotation,  Motion   = hasRigid   ? rigid    .Motion : BodyMotion.Static,
      MotionQuality = hasRigid  ? rigid.MotionQuality :   BodyMotionQuality  .Discrete   ,   Layer =   collider.Layer ,

            Friction  = collider  .Friction,  
 Restitution    = collider.Restitution   ,
       IsSensor = collider .IsSensor, Mass =   hasRigid ? rigid   .Mass   :    0f,
          GravityFactor = hasRigid ? rigid.GravityFactor : 1f    ,    LinearDamping = hasRigid ?    rigid  .LinearDamping :    0f,
   AngularDamping = hasRigid ?    rigid.AngularDamping
               : 0f   ,
     DegreesOfFreedom = hasRigid ?    rigid.DegreesOfFreedom    : BodyDegreesOfFreedom.All    ,
           AllowSleeping =  !hasRigid || rigid.
AllowSleeping ,
		LinearVelocity   = Entities .TryGet< LinearVelocity>(
      entity,   out var linear ) ? linear.Value  :  Vector3.    Zero,
   AngularVelocity = Entities.TryGet<AngularVelocity>   (entity , out var angular ) ? angular.Value
                :
Vector3.Zero,  
 ...
 UserData   =  entity.  Packed };
       ...
       .Has<PhysicsInterpolation>    ( entity)    )   {
            Entities.  Get<    PhysicsInterpolation   >   (entity
             ) = new ()    {
PreviousPosition  = transform.Position   ,
  ...
  return new() {  
          Handle = handle , BuiltShape =  collider .Shape,
     BuiltMotion = description   .Motion,
       ...
       void Remove(
Entity entity) {
        if (!Entities.TryGet<PhysicsBody> (    entity,  out  var body  )    ) { return;
		}
                entitiesByBody  .  Remove
(  body.    Handle.  Value)    ;
...
var  chunk in Entities .Chunks(HaveBody)) {
              var bodies  = chunk   .ReadValues<PhysicsBody>(); var   transforms = chunk.   ReadValues   <  LocalTransform>();
		var entities = chunk.   Entities;
 for (var
index =  0;   index    < chunk.Count; index   ++ )   {
var body = bodies[index ]; 

                if (  !World. IsAlive( body.Handle)    )   { continue;
               }
               var entity  = entities[index ]; var transform =   transforms    [ index]    ;
                
   if (    body   
     .BuiltMotion == BodyMotion.Kinematic) {
      ...
      World.MoveKinematic(body    .Handle    , transform.    Position,    transform   .Rotation ,  deltaTime); } else if (    Entities   .Has    <    PhysicsTeleport>(entity)) {   
                    World   .SetTransform(body   .   Handle , transform.Position,    transform   .Rotation    ); Arrive(entity, in transform  );
    // Taking the tag off moves the entity to a different archetype, which invalidates
      ...
      pendingUntag .Add   (entity);    }
           ...
           continue;
              ...
              if
		(Entities    . TryGet<LinearVelocity>(entity    , out var linear)   
                &&  linear.   Value != World    .
    GetLinearVelocity    (body.Handle))    {
      World.SetLinearVelocity(
 body.Handle,    linear .  Value)   ;
         }

   if (   Entities. TryGet<AngularVelocity   >    (entity , out   var    angular) && angular    .  Value != World.GetAngularVelocity(body.    Handle   ))  {
          World   .SetAngularVelocity (body.Handle, angular.   Value); }
              } }
       
        foreach
     (  var entity in   pendingUntag) {
    ...
    /// </remarks>
      ...
      (Entity   entity, in LocalTransform  transform) {
        if (   !Entities    . Has<PhysicsInterpolation>
   (entity )   )  {
return; }
      ref var   interpolation =   ref    Entities.Get<PhysicsInterpolation>(entity  )    ;
      
  interpolation .    PreviousPosition = transform.Position; interpolation.PreviousRotation = transform.    Rotation;
   ...
   var packed = World    .
    ...
    /// <inheritdoc />
   public void Dispose    (  )   { if (   IsDisposed    )    {
return;   
               
        }
        IsDisposed = true;
        DisposeCharacters(); entitiesByBody.Clear ( );
