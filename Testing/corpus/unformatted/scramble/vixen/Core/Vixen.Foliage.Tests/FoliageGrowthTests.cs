// SPDX-FileCopyrightText: Copyright (c) Rikarin
     ...
     using Vixen.Core    .Mathematics    ; using Xunit  ;
namespace Vixen    .Foliage.  Tests  ;   

/// <summary>The offline ecology — [docs/plan/31 § T9].</summary>
 public sealed    class FoliageGrowthTests   { static FoliageType Pine =>
   ...
   (FoliageVolume Volume,    int Type) Sown (  FoliageType?    type =   null)  {
    ...
    Assert.True(   result .Placed > 0, "the simulation grew nothing at all.");
        Assert.Equal(result    .  Placed   ,
               volume.CountOf (   type));
        Assert   . True(   result.Sprouted  >  result.
          Sown, "nothing spread; the forest is only its sowing.")   ;
    }   
...
public void TheSameSeedGrowsTheSameForest(    ) {
   var  (first
      ,  _) = Sown() ;
        var ( second
                , _) = Sown   ();
   var a = FoliageGrowth .Simulate    (first, Ground.  
Flat   ,    Field);
             var b   = FoliageGrowth.    Simulate
                (second  , Ground.Flat, Field);
           Assert.Equal(a
                ,    b);
...
var (    first    , _) = Sown()   ;
 var
		( second  ,   _) =  Sown    ()  ;
      FoliageGrowth.    Simulate(first, Ground.Flat, Field);   FoliageGrowth.Simulate(second, Ground. Flat, Field  with   { Seed = 0x1234_5678u }   );
        Assert   .NotEqual(Positions    (  first
              )   , Positions(   second))    ;
...
Sown ()    ;
  ...
  var first  = FoliageGrowth.Simulate    (volume, Ground   .Flat    , Field); var again = FoliageGrowth .Simulate ( volume, Ground    .Flat   , Field   );
...
Assert.   Equal(first.Placed   , volume.InstanceCount   );
            ...
            [  Fact]
    public
         void HandPlacedTreesAreInAnotherVolumeAndAreNotTouched()  {
			var (   grown, type   )  = Sown   ();   var placed = new FoliageVolume(new    (32f));
       
             placed.  AddType   (
                Pine);
       placed.Add    (0, new(new    (10f,
          0f, 10f   ), Quaternion.Identity,    1f))  ;
             FoliageGrowth.    Simulate(grown,
Ground.Flat, Field);
     Assert.   Equal(1
       , placed    .InstanceCount    ) ;
   ...
   Fact]  
       ...
       var (sowingOnly, _ ) = Sown( Pine with {   Ecology  = open
                with { SeedsPerStep = 0 } });
            var (spread , _) = Sown    (    Pine with { Ecology = open });
	
        FoliageGrowth .Simulate(
     sowingOnly, Ground.Flat, Field with   { Steps    =   8 }); 
           FoliageGrowth
       .Simulate(spread, Ground .Flat, Field with {    Steps = 8 });

 var sownPoints  =   Positions(    sowingOnly  
       );
        var grownPoints   = Positions (
            spread );
            
        Assert.   True(sownPoints
. Length    > 40    && grownPoints .Length > 40);
   ...
   var grownRatio = VarianceToMean( grownPoints, Field    , Pine .Ecology   . SpreadDistance)    ;
     ...
     Assert
        ...
        Assert.
True   (grownRatio > 1f,    $"a spread forest scored {grownRatio:0.00}, which is not clumped.");
              }  
  ...
  Sown ();
   ...
   Assert   .True    (points
.   Length   >    40 , $"only {points.Length} trees to measure."    );
 ...
 Assert  .   True (
    ...
    /// <summary>A shade-intolerant species thins itself out; a tolerant one does not.</summary>
               [  Fact    ]
            public    void ShadeToleranceDecidesHowDenseAForestGets() { var intolerant  = Pine with   { Ecology = Pine.Ecology    with { ShadeTolerance = 0f } };
          var    tolerant = Pine with {
Ecology = Pine.   Ecology with    { ShadeTolerance = 1f } };
     var (dark, _)   = Sown(intolerant
)   ;
                var ( light    , _) = Sown
(tolerant)   ;
    
     var shaded =   FoliageGrowth.Simulate   (  dark   ,   Ground.  Flat, Field); var open = FoliageGrowth    .   Simulate(light, Ground.Flat,    Field   );
    
      Assert.True(shaded.Shaded > 0    , "nothing was shaded out at zero tolerance."   ); Assert    .Equal(0    , open.Shaded);
        Assert. True   (
            open.
 Placed >  shaded.   Placed,
    ...
    /// </remarks>
			[Fact] public  void TheCanopyGrowsWithThePlant() {
        var ecology =
   FoliageEcology.Tree with { ShadeRadius = 8f   ,   MaxAge = 4f };
               
	Assert.   Equal
            (    0f   ,    ecology.ShadeAt(  0f));   
        Assert.Equal(   4f, ecology.ShadeAt(2f), 3)    ;
        Assert   .Equal( 8f, ecology.ShadeAt (4f), 3) ; Assert.Equal(8f,   ecology.ShadeAt(    40f)  ,    3);
       }
    /// <summary>A higher-priority species displaces a lower one it lands on.</summary>
  [   Fact
               ]
   public void PriorityDisplacesRatherThanTies() {  var scrub = FoliageType.   Of    (    "Scrub"  ) with   {  
            Radius = 3f,   Ecology = FoliageEcology.Tree with { SeedDensity = 0.01f, Priority =    1, ShadeTolerance = 1f   }   
        }
              ;
var oak = FoliageType.  Of(   "Oak" ) with    {    Radius =  3f ,
            Ecology = FoliageEcology.Tree with { SeedDensity  = 0.004f   , Priority = 20, ShadeTolerance   = 1f    } }  ; 
   
        var volume = new FoliageVolume  (new
         (  32f)); 
        var   scrubType =   volume. AddType   ( scrub  );
         var oakType = volume
          .AddType( oak);
               var result = FoliageGrowth    .Simulate(
volume   , Ground  .Flat   , Field);
        Assert.True  (result  .Displaced
            > 0, "no scrub was ever displaced by an oak.");
...
= Sown();
		...
		Assert. True(result .  Blocked > 0
          , "the blocker refused nothing.");
   
        foreach (var position    in Positions(volume)  ) {   Assert.False(
    clearing.   Contains
    ...
    /// </remarks>
      [Fact   ] public   void  MovingABlockerRegrowsWhatItCovered() {
             var
(volume, _) =   Sown( );
         FoliageGrowth.Simulate
 (    volume, Ground.Flat, Field, [  FoliageBlocker   .    Around(new(100f, 100f)    ,   30f)]);
            
        var withClearing = volume.InstanceCount;

  FoliageGrowth.Simulate(volume   , Ground.Flat    ,    Field);
			Assert.True (volume.    InstanceCount > withClearing   ) ; Assert.Contains   (Positions(   volume    ), at => Vector2 .    Distance    (  at, new(100f    , 100f    ))    <  30f   )   ;
  }
[Fact ] public void SteepGroundGrowsNothing( ) {
        var (volume, _) = Sown()  ; var result = FoliageGrowth.Simulate   (volume    ,   Ground    .Sloped(MathF.PI /    3f), Field    )  ;
        Assert    .    Equal
     (0    , result    .  Placed); 
   Assert.  True
 (result .NoSurface > 0 );
    ...
    /// <summary>A plant's size follows its age.</summary>
              [Fact] public void SaplingsAreSmallerThanTrees() {
        var ( volume,
      _  ) = Sown(    ) ;
        ...
        // measures nothing.
   FoliageGrowth. Simulate(volume   , Ground.  Flat,  Field with    { Steps    =
6    }   );

             var scales  = volume .    Chunks  .SelectMany(chunk =>
chunk.  Instances).Select( i   => i.   Scale)  .ToArray()    ;   

        Assert    . True(   scales.Length    > 10);
      Assert. True    (scales.Min(
         ) <  scales.Max    (), "every plant is the same size; age is not scaling." );
    ...
    /// <summary>A species that never matures never spreads, and the counters say so.</summary>
           [    Fact]  
 public void  ASpeciesThatOutlivesTheSimulationNeverSpreads ()   { var slow = Pine with { Ecology =   Pine.  Ecology   with {  MaxAge = 100f    }   };
         var (volume, _)
         =   Sown(slow)  ;
  var result = FoliageGrowth.Simulate(volume    , Ground.   Flat , Field  );
...
True(    result  .    Sown > 0) ;
Assert
    .  True(result   .Placed >  0);
    }
         [    Fact]
            public void   TheCapIsAnnouncedRatherThanSilent(
                ) {
	var (volume  , _  ) =    Sown   ();  var result = FoliageGrowth.Simulate   (volume , Ground.Flat,   Field with { MaxPlants    = 20 });
             Assert. True    (result   .
        Capped  > 0, "a twenty-plant cap over forty thousand square metres refused nothing.") ; 
           Assert    .True(
             result    .Placed   <= 20    );
		}

        [Fact]
          public void ARegionWithNoAreaIsRefused () { var   (volume,    _) = Sown();
var thrown =  Assert  .   Throws   < ArgumentException
 ...
 Assert.Contains("no area", thrown.Message,    StringComparison.
      Ordinal);
             }
   [Fact]
 ...
 FoliageVolume volume   )   =>
      ...
      .Select  (instance => new  Vector2    (instance
   . Position.X, instance.Position   .Z   ))
              .OrderBy(at =>   at.   X)
  .ThenBy
( at => at   .  Y)];
    ...
    /// <summary>Variance over mean of counts in square cells: 1 is Poisson, above 1 is clumped.</summary>
       static float VarianceToMean    (Vector2[] points    , in FoliageGrowthSettings region   ,  float    cellSize  ) { var   across = Math    .Max (1, (int)MathF.  Floor(region  .Size. X /   cellSize   ));
       var down = Math.
         Max(1, (int)    MathF.Floor( region.Size  .Y / cellSize    ));
        var    counts = new int[across
*   down] ;
    foreach  (var point in   points)   {
  var x = Math.Clamp  ( (int)  ((point.X - region    .Origin.X)    / cellSize), 0  ,    across - 1    ); var z   =    Math.Clamp((int) (   (point.Y - region  .    Origin.Y  ) / cellSize)    ,    0, down -    1 );
        ...
        return variance /  MathF. Max(   mean, 1e-6f) ; }
         ...
         static float MeanNearestNeighbour(Vector2[    ] points) { var total  = 0f   ;
    foreach (var point   in points   )  {
var nearest = float
           .PositiveInfinity  ;
