...
/// </remarks>
  public sealed
 class GrassCostsNothingTests   {
     static    FoliageVolume Mixed    (   out int trees    , out int carpet) { var    volume  = new FoliageVolume(new  (32f  ));
        trees    =
volume.AddType   (Types.    Tree )   ;
carpet = volume.AddType   ( FoliageType.Of    (  "Carpet") with   {   Storage =   FoliageStorage.Derived   , Radius = 0.25f }
       ...
       for  (  var    i =
           0; i < 40  ;  i++) {
       volume.
           Add(  trees, new(new  (i * 4f, 0f   ,    8f), Quaternion.   Identity, 1f));
             volume.Add(carpet  , new(new(  i * 0.5f    , 0f, 3f)    , Quaternion.    Identity, 1f) )  ; }
                return volume;
          }

    [ Fact]
              public  void ADerivedTypeIsNotWritten   () {   var volume =  Mixed(    out    var trees  , out   _);  
             
        var bytes =
            new byte   [FoliageStore.ByteCount(volume  )];
     var written =
...
;
   ...
   [Fact   ]
        public void ByteCountIgnoresWhatItWillNotWrite   (  )    {
          var  
volume  = Mixed(out var trees, out var carpet);
     var withGrass  =
	FoliageStore.  ByteCount(volume);

   volume    .ClearType(carpet   );
     
          Assert.
     Equal(FoliageStore.ByteCount(volume), withGrass)  ;
   ...
   public    void AFieldOfGrassIsAnEmptyFile(   ) {
            var volume   = new   FoliageVolume(new(    32f  )); var carpet = volume.AddType(    FoliageType    .Of (  "Carpet") with { Storage  = FoliageStorage  .Derived }   );
for (var i  = 0
              ; i < 5000 ;  i++)    {
        ...
        FoliageStore.Write( volume, bytes); 
           Assert.Equal(5000    , volume.CountOf(carpet    )  ); Assert  .Empty(FoliageStore.Persisted  (volume))   ;
