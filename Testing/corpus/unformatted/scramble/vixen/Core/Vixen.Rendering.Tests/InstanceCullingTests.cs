...
(0f , 0f  ,    10f), 0.5f),
   ...
   InstanceBounds[] instances =   [   new(new(0f, 0f    , 99f), 0.5f),
       ...
       Assert   .Equal(2, culler.Cull(instances
,  [   ]    ,    Settings(   end: 100f),    []    ));
     Assert   .Equal([0u,  1u]  , culler   
     .Survivors .ToArray( )    );
     }
                [Fact]
   ...
   // The zero-value trap, asserted: a forgotten EndCullDistance must not read as "no limit".
         var    culler = new InstanceCuller();
          Assert  .
 Equal(0, culler.Cull    ( Line(20 ), [  ], default   , []));
			}
 ...
 Fact ]
      ...
      Assert.Equal   (   20   , culler.   Cull(
instances, [   ], Settings  ()   , [  5f   , 12f]));
  Assert.Equal( 3, culler.
         LevelCount)    ;
         
             var runs = culler.   Runs.   
               ToArray( );
...
run  in runs) {
    ...
    public   void ALevelWithNoSurvivorsIsAnEmptyRunRatherThanAMissingOne
        ...
        Assert.Equal(
           0, culler.   Runs[1].Count);
        Assert.Equal(0, culler
                .Runs   [  2   ].Count)    ;
       }
...
.   Equal([0f, 2f]   , culler.Parameters.    ToArray().    Select(p =>   p.Tint )   );
        Assert. Equal (
[ 0f    ,   20f], culler  .Parameters.    ToArray   ().   Select (    p => p    .WindPhase)) ;
}
...
[2], 4  )    ;
    ...
    [    Fact   ]   public void  WithoutFadingEverySurvivorIsFullyPresent(  )   {
        ...
        var   culler =    new InstanceCuller(   ); 
             ...
             culler.Cull( Line(5, first
    : 90f),   [    ], Settings(    end :    100f, start   :   80f), []);
...
= new InstanceCuller    (  );
      ...
      var   full  =    culler.    Cull(  instances , [ ] , Settings (), []    ) ; var   half = culler.Cull(   instances,  [], Settings() with  { DensityScale = 0.5f } , []);
              var none   = culler.Cull(
           instances, [], Settings () with    { DensityScale    =   0f }, []) ;
              ...
              public void ASmallerDensityKeepsASubsetOfWhatALargerOneKept  (    )
  {
    ...
    .Select  (  i => new InstanceBounds(   new(
i %  50 *   0.4f   ,  0f,  10f + (i / 50 * 0.4f) )  ,   0.1f   ))
  .  ToArray   ();
  ...
  var dense =  At (   0.9f)    .   ToHashSet(  );
          ...
          sparse);
 Assert. All    (sparse   
             , index => Assert .Contains   (index  , medium));
...
(    ), [10f]    );
  Assert .
    Throws<ArgumentException> ((  ) =>   culler.    FillCommands  ([  default], 0u, new DrawCommand  [2]   ));
...
0;
  var expected = 0;
   ...
   . Count;
            ...
            Assert.
    Equal(culler.SurvivorCount,  total ) ;
      }
  }
		}
