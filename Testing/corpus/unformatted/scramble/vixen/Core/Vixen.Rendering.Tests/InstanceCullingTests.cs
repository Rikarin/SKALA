...
using Vixen. Core. Mathematics;   using    Vixen.    Rendering;
           using    Vixen.Rendering.   Features; using Xunit;
...
var projection   = Matrix4x4 .  PerspectiveFieldOfView(MathF .PI / 2f  ,  1f, 0.1f  ,    10_000f )    ; return   new(view * projection  )    ;
  }
           
               static    InstanceCullSettings Settings(float end = float.MaxValue  , float start = float .MaxValue) => InstanceCullSettings.  Everything(Forward(   ), Vector3.Zero)    with {
 ...
 [.. Enumerable.    Range(0, count).Select
          (   i => new InstanceBounds(new (    0f, 0f,    first + i), radius) )  ]  ;
          [    Fact]

    public void    EverythingInFrontSurvivesAWideOpenCull( ) {
  var culler    = new   InstanceCuller    () ; var instances =  Line(20    )    ;
...
Assert.Equal(2 , culler.Cull   (instances, []   , Settings  (    ) , []    ));   Assert.Equal(   [ 0u, 2u   ]  , culler.Survivors.ToArray());
          }
           [Fact ]
...
();
            ...
            InstanceBounds []   instances   = [ new(new(0f    , 0f, 99f), 0.5f),
           new  (  new(0f    , 0f, 100.4f), 1f  ),    new( new (0f,  0f,    120f),    1f   )
            ];

            Assert.Equal(2, culler.Cull
            (  instances   ,    [    ],   Settings(end: 100f    ),    [])   );
    Assert.Equal([  0u,  1u]
   , culler .Survivors. ToArray    ());
    ...
    public void   AZeroedSettingsCullsEverythingRatherThanNothing  (    ) {
     ...
     var culler = new InstanceCuller(  )
     ;
                Assert.Equal(0, culler    . Cull(Line (
20),  [],   default, [])) ;  
   }
         // --- Levels -------------------------------------------------------------
          [Fact
...
= new InstanceCuller(   ) ;
         ...
         // Instances at 1…20 m; level 0 out to 5, level 1 out to 12, level 2 beyond.
           ...
           Assert.   Equal   (   20, culler   .
Cull(instances,   [],  Settings(   ),  [   5f, 12f]));
               Assert.Equal (3,
                culler.LevelCount    );
      var runs = culler   .
Runs.ToArray ();
  Assert.Equal(    new    (
        ...
        var survivors =   culler
               .Survivors.ToArray(  );
        Assert.  Equal (20, survivors.Length ); Assert.   Equal  (survivors.   Length  , survivors.Distinct().Count()    );
      foreach
     (var  run in runs) {
            var   slice    =  survivors  [run   .First..(run.First    + run    .Count )] ; Assert.  Equal(   slice .OrderBy(index => index),  slice) ;
}  
}
             [Fact   ]
        public
...
InstanceBounds[ ]  instances = [  new(new(0f,  0f, 10f),    0f) ]  ;
        culler  .   Cull   (instances    , [] ,   Settings
			(), [10f]);
        Assert. Equal  
       (0,  culler   .Runs[   0 ].Count)    ;  
        Assert. Equal
        ( 1, culler   .Runs[1].Count);
    }
             
                [   Fact ] public void DistancesThatDoNotAscendAreRefused() {
 ...
 Assert.  Throws <   ArgumentException>(   ()   => culler.Cull(Line    (4 ),    [], Settings(), [20f,    10f])); }
  ...
  var culler =    new InstanceCuller() ;
    ...
    [   Fact
             ...
             public void AuthoredParametersFollowTheirInstanceThroughCompaction   () { var culler = new  InstanceCuller    ();
   InstanceBounds[ ] instances
= [
...
authored[ index ]  = InstanceParameters    .Neutral with    { Tint = index, WindPhase = index * 10f };
           ...
           culler.Cull(instances,    authored
         , Settings(), [    ]);   
   // The middle instance was culled, so the survivors' parameters must be 0 and 2 — not 0 and 1.
         Assert.   Equal([0f, 2f]
             ,   culler.Parameters.ToArray    ().Select    (p => p   .Tint) )   ;
    Assert.
Equal( [ 0f ,    20f   ], culler.  Parameters.ToArray    (   ).Select   ( p    => p.    WindPhase));
    }

[ Fact ] public void WithNoAuthoredParametersEverySurvivorGetsNeutralOnes   () {
                var   culler
= new InstanceCuller  ( );
        ...
        Assert.All   ( culler    .Parameters.ToArray   ()  ,
             parameter => {   Assert .Equal(1f   ,   parameter  .Scale   )  ;
                Assert   .  Equal(  1f ,
         parameter   . Fade) ;
           } );
             }   
              [
Fact    ] 
 public void FadingRampsFromOneAtTheStartDistanceToZeroAtTheEnd(   )  { var culler   = new InstanceCuller (   );
  InstanceBounds[] instances = [
     new  (
 new(    0f, 0f    , 50f), 0f) ,
       new(new(    0f ,  0f, 80f   )    , 0f),   new(  new(0f, 0f , 90f),    0f   )
        ] ;
			var   settings =    Settings    (end: 100f, start:    80f) with  {  Fade
     = true }   ;
        culler.Cull( 
             instances, [  ], settings,  [   ]);
            
        var fades = culler
   .Parameters.ToArray()   .Select(p   => p    .Fade    )    .   ToArray();
         Assert.Equal( 1f,  fades[   0], 4);  
   Assert. Equal(    1f  , fades[    1
    ], 4);
          Assert.Equal(   0.5f
            ,    fades   [2], 4);
   }
    [  Fact    ]    public   void   WithoutFadingEverySurvivorIsFullyPresent(  ) {
               var  culler   =   new InstanceCuller    ();  
    culler   .Cull(Line( 5
        , first: 90f), []   ,    Settings(end:    100f ,    start: 80f   )   , []);
...
DensityThinsTheFieldRoughlyInProportion( ) {
           var  culler
          = new InstanceCuller()    ;
       var  instances = Enumerable.   Range(0 , 4000) .Select(i => new  InstanceBounds(    new   (i %   40 * 0.5f    ,  0f, 10f + (i /    40 * 0.5f) )    ,  0.1f) )
            .
ToArray    ();
           
              var   full =   culler  .    Cull(    instances,  [ ] , Settings () , []);    var half =   culler.Cull(instances,   []  , Settings() with { DensityScale  = 0.5f }, [ ]);
        var none = culler   .
      Cull (instances, [] , Settings() with { DensityScale    = 0f    }   , []);
  ...
  var culler
      ...
      var   instances = Enumerable.Range    (  0,   2000  )   
             .Select(i  =>  new InstanceBounds(
             new(i   % 50 *  0.4f,   0f   ,  10f +  (i / 50 * 0.4f)), 0.1f  )   )  
    .   ToArray(  )   ;
uint[] At  (float density    ) { culler.   Cull   (   instances, [], Settings(   ) with {  DensityScale = density }    , []);  
  
    return culler.Survivors.ToArray()   ;
   }
        
var sparse = At  (0.25f  ); var medium =    At(0.5f)  .ToHashSet() ;
			var dense = At(  0.9f )   .ToHashSet    (   );
   
  Assert  .
NotEmpty(sparse)   ;
              Assert.All
          (sparse ,    index => Assert.Contains (index   , medium  ));
			...
			public void DensityIsHashedFromPositionSoAReorderedCellThinsIdentically   ( )
{
  ...
  .Select   (  i =>   new InstanceBounds(new(i % 25 *   0.7f, 0f    , 10f +   ( i / 25 * 0.7f    )) , 0.1f  )) .ToArray();

   var settings = Settings   () with {  DensityScale = 0.4f };
  
            culler  .    Cull(   instances   , [], settings,    []);  var forwards = culler.Survivors.ToArray(). Select(index => instances  [index ].Centre).ToHashSet(   );
             var reversed =  instances.Reverse(
            )  .ToArray( );
               culler .  Cull    (reversed   , []
           ,   settings, [  ] )  ;
		var backwards = culler    .    Survivors.    ToArray( ) .Select(index => reversed[index]    .   Centre). 
   ToHashSet( );
     ...
     // --- Draw commands ------------------------------------------------------
        ...
        [    Fact]
           ...
           var  culler  = new InstanceCuller();
             ...
             culler    .
            Cull(Line   (  20)    , [ ] , Settings ()    , [  5f,  12f   ]   );
        DrawCommand []  templates =   [ 
new   (   )    { IndexCount = 900,   FirstIndex = 0, VertexOffset =    0 }, new  ( ) { IndexCount    = 300, FirstIndex = 900, VertexOffset  = 0 }  ,
            new(  )    {   IndexCount = 60 , FirstIndex =  
 1200, VertexOffset   = 0 }
        ];  
           var commands
    = new   DrawCommand[3 ];
		culler  .FillCommands (templates, 5000u
      , commands)    ;
  Assert .   Equal(   900u, commands   [0 ] .IndexCount); Assert.   Equal(4u   ,  commands [   0]  .InstanceCount);   
      Assert.  Equal
         (5000u, commands[0]    .    FirstInstance)    ;
		Assert   .Equal(    7u, commands [1 ]. InstanceCount    ); Assert.Equal(5004u, commands[1].FirstInstance);
  
          Assert .Equal(60u,    commands    [   2 ].    IndexCount   ); Assert.Equal( 9u , commands    [2] .  InstanceCount );
   Assert.   Equal( 5011u,    commands[2]  .FirstInstance   );
             }
 
  [Fact]
    public void AnEmptyLevelGetsAZeroInstanceCommandWhichDrawsNothing(  ) { var    culler = new  InstanceCuller(  );
  
               culler.Cull(Line (3), [
]  , Settings ()  , [100f   ]);
			
       var commands =    new DrawCommand  [2];   culler.FillCommands(   [   new(    ) { IndexCount = 900 }, new  ()   {   IndexCount = 300 }], 0u, commands   );
        Assert
         .Equal(3u,    commands    [ 0].InstanceCount)   ;
 Assert.    Equal(0u, commands   [1  ].  InstanceCount);    Assert.   Equal(300u,   commands[   1   ]  .IndexCount  )  ;
      }

     [ Fact ] public void TooFewTemplatesOrTooLittleRoomIsRefused  ( ) {
           var culler = new InstanceCuller   ( );
              culler.Cull  (Line(3) ,   []
         , Settings(), [    10f]);
...
[Fact] public  void ACullerIsReusableAndDoesNotLeakTheLastFramesAnswer(   ) {
            var culler = new InstanceCuller()
    ;
            
	culler   . Cull(Line( 50)  , [], Settings(),
          [5f, 10f]);
   Assert .  Equal(50,    culler. SurvivorCount)  ;
  culler.   Cull(
Line(3    ), [   ],   Settings   () , []   )   ;
        Assert  .Equal(3 , culler.SurvivorCount  );
             Assert.  Equal(1    ,   culler.   LevelCount); Assert   .Equal(3, culler.Survivors.Length);
	Assert
  .Equal(3,   culler.Parameters   .  Length);   
        ...
        culler.Cull([   ] , [   ], Settings(), [])   ;
         Assert.Equal(  0,  culler    .SurvivorCount   ); Assert. Empty(culler.Survivors.ToArray());
   }
        [
Fact ]
     public
void  TheRunsAlwaysPartitionTheSurvivorsExactly() {
      var culler
=  new   InstanceCuller();
        foreach (var count   in new    [    ] { 0
, 1, 7, 64, 513   }    )   {
     culler .Cull (Line(count  )  ,    [],  Settings  (  end:   300f
      ) , [5f   ,    12f, 40f]);
  ...
  total   = 0;
            var expected =
0  ;
            foreach (var run in culler.   Runs) {   Assert  .Equal   (expected, run.  First    )    ;
   total
            +=  run    .  Count  ;
                expected  += run    .    Count
              ;
