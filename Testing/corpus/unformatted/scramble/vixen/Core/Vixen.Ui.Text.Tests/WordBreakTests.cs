...
/// </remarks>
                ...
                {
                static List  <    int> Opportunities (string text, WordBreakMode mode)  { var    found =    new  List<int    >();
            LineBreaker   .Collect(text, found,
mode)   ;
			return
found;
    }
    ...
    /// </remarks>
      static List  <string
             > Wrap(    string text, float width,   TextWrapMode mode  , WordBreakMode   wordBreak) {
var advances = new float[text  . Length + 1]   ; Array.Fill  (advances , 1f,    0   , text.  Length  ) ;
                var    lines = new List<WrappedLine> (); LineWrapper    .Wrap (text, advances,    width,  lines, mode   ,  wordBreak);
   return lines.   Select   (line => text  [line    .    Start..line .   End]
        ).ToList  ()    ;
    }
     [Fact  ]
                public void   Normal_offers_nothing_between_two_letters(   ) =>   Assert.    Equal  (   [5]  , Opportunities( "hello" , WordBreakMode.Normal  ));
          
    [Fact    ] public  void Break_all_offers_a_break_between_every_pair_of_letters   (   ) =>
        Assert.   Equal    ([1  , 2, 3,   4 , 5],
               Opportunities("hello"    , WordBreakMode.BreakAll)  );
    /// <summary>
    ...
    /// </remarks>
                ...
                [Fact]
    public void Break_all_still_refuses_to_begin_a_line_with_a_comma(   ) { var   found = Opportunities("ab,cd"   , WordBreakMode.BreakAll   );
       Assert
             .Contains(1,   found  )    ;
    ...
    var found    = Opportunities
   ("a(bc)d" , WordBreakMode   .   BreakAll);
...
[InlineData("한글"    )] // Two precomposed Hangul syllables.
			public void Keep_all_suppresses_the_break_between_two_letter_units (    string text)  { Assert.Equal(    [  1, 2], Opportunities    (text, WordBreakMode.Normal))   ;  
    ...
    /// </remarks>
      [   Fact ] public void Keep_all_still_breaks_at_a_space () =>
    ...
    /// </summary>
    [Fact]
         public void Keep_all_leaves_latin_exactly_as_it_was() {  const string Prose = "the quick brown fox, jumps over-the lazy dog";
       Assert.Equal
      (
            Opportunities (Prose   , WordBreakMode   . Normal),   Opportunities  (Prose, WordBreakMode .  KeepAll)
           )   ; }
    /// <summary>
...
Fact]
  public void Break_all_fills_the_line_where_anywhere_only_rescues_the_word ( ) {
             Assert.Equal( ["ab ",    "cdefg"  , "hij"    ],
               Wrap("ab cdefghij",   5f,   TextWrapMode    .Anywhere,    WordBreakMode.Normal)  );
			
      Assert.  Equal( ["ab cd",  "efghi",  "j"   ],
            Wrap ("ab cdefghij"
     , 5f,    TextWrapMode   .Word,  WordBreakMode.BreakAll)
             )
             ;
     }
    ...
    /// <summary><c>break-all</c> needs no help from <c>overflow-wrap</c> to keep a word in its box.</summary>
         [ Fact]
      public void Break_all_keeps_every_line_inside_the_width() =>    Assert.Equal   (
   ["antid"
			, "isest",   "ablis",  "hment"], 
    Wrap("antidisestablishment"    , 5f, TextWrapMode
.  Word  , WordBreakMode. BreakAll )
        );
     
    /// <summary><c>keep-all</c> makes a CJK run overflow rather than break inside itself.</summary>
			[Fact] public    void Keep_all_lets_an_ideographic_run_overflow(    ) {
        Assert.Equal
     ([    "中文",    "中文"], Wrap    ("中文中文", 2f, TextWrapMode    . Word    , WordBreakMode   .Normal ));
  ...
  Assert.
Equal (
["中文 ", "中文"]    ,  Wrap("中文 中文"   , 2f    , TextWrapMode  .Anywhere, WordBreakMode.   KeepAll)
        )    ;
  ...
  Assert.Equal    ( ["中文", "中文"    ],
    ...
    /// </remarks>
    [    Theory]
       [   InlineData( WordBreakMode.  Normal )] [InlineData(  WordBreakMode    .BreakAll)]
           [ InlineData(   WordBreakMode .KeepAll  )]  public   void The_lines_partition_the_text(WordBreakMode mode    ) {
    const string Text =    "ab 中文cd-ef\n한글 ghij";
 
        for (
var   width = 0f; width <= 12f;    width += 1f) {
             var pieces    =    Wrap(Text , width, TextWrapMode   .   Word, mode); Assert.Equal(Text   ,   string.Concat(  pieces));
