   ...
   // SPDX-License-Identifier: Apache-2.0
      ...
      ;
using Vixen.Graphics ; using    Vixen  .Rendering.Compositor;
             using Vixen.Rendering.Lighting;  using Vixen.    Shaders   ;
              using Vixen
 .Shaders.    Generated;
...
public sealed class    WaterRenderer : SceneRenderer, IDisposable {
    readonly    FullScreenRenderer pass; readonly ComputeRenderer classify;
           readonly List<ResourceBinding>
        bindings = [];
    BufferHandle placeholder   ;
 bool  disposed;
    /// <summary>Creates the node.</summary>
    public WaterRenderer( ) { pass   = new () {
		
           Name = WaterKeys .ShaderName,
            ShaderName = WaterKeys.ShaderName,  
		PermutationKeys = WaterKeys
        . UsedPermutationKeys  , 
               ConstantBinding = WaterKeys.ConstantBufferBinding
              }   ;
       classify =   new() { Name  = WaterTilesKeys.  ShaderName    ,
...
string Normal { get ; init; }
    ...
    /// </remarks>
       public
        RenderView? View { get;  set; }
    /// <summary>Clip back to world, when no <see cref="View" /> supplies it.</summary>
        public
Matrix4x4   InverseViewProjection   {  get; set ; } = Matrix4x4.Identity;
  ...
  public    Vector3    CameraPosition {    get  ; set   ;   }
    ...
    /// </remarks>
public  Vector3 Scattering { get  ; set;
       } = new  (0.03f, 0.06f, 0.09f    );
...
get  ;  set; }   = new(   0.35f, 0.06f , 0.02f);  
    ...
    ///     </para>
    /// </remarks>
   public Vector3  SunColour   { get 
; set; }  =    new(90000f,   81000f   , 63000f)   ;
...
public Vector3   SunDirection    {  get;    set; }  = new  ( 0f  , -  1f, 0f);
    ...
    /// </remarks>
    public Vector3
SkyColour   {   get; set;   }  = new   (1400f, 1680f  , 2200f);
...
public bool Foam {  get   ; set; }
    ...
    public   ComputePipelineCache    ? Pipelines { get   => classify.    Pipelines;
...
public    Int2 
    ...
    get =>    pass.   Modules;    set => pass  .Modules = value ;
}
...
return false;
    ...
    protected override void Build (
              GraphicsCompositor compositor   , CompositorFrame frame)  {
      ArgumentNullException.ThrowIfNull   (frame   );
 if (string.Equals(Behind, Output    , StringComparison.Ordinal))   { throw new CompositorBindingException    (  
               ToString( )    , 
                "target", Behind  ,
               "is both what the water reads and what it writes. Sampling a target a pass is also "
            + "writing is undefined — not slow, not approximate — so the read has to come from a " +  "copy. Put a !Copy node ahead of this one and name its destination here. See "
      + "docs/plan/35 § B1, which calls the copy the blocker rather than the pass"
             )   ;
			}
             
      Degrade(Declare (compositor
      , frame ));
      }
...
? "no Samplers, so the surface pass was never declared and the frame has no water at "
               + "all — a lake that is not there rather than one that looks wrong" 
                :  "no Device on the surface pass, so it was never declared and the frame has no " + "water at all — a lake that is not there rather than one that looks wrong";
        }
     pass.
      Samplers = Samplers;
...
Clear();
               pass.
       BufferReads.Clear ();
        pass.Descriptors.Bindings .Clear( );   bindings.Clear(   );
    pass   .ColourTargets.Add(  Output );
       ...
       { } view )    {  
           CameraPosition = view.Position   ;
            if (  Matrix4x4.Invert( view    .ViewProjection, out var inverse) )  { InverseViewProjection   =    inverse  ;
             
            }   
       }
        pass .Parameters  .Set (WaterKeys .Reflections, Reflections    .Length > 0); pass.Parameters.Set(WaterKeys.Foam  , Foam)   ;
          pass.Parameters.
     Set( WaterKeys .  Tiled    ,   tiles.Tiled  )    ;
        pass.Parameters  .Set(   WaterKeys.TileCount, tiles .Count); pass    .Parameters.Set   ( WaterKeys.TargetSize   , tiles   .    Target ) ;
   pass.Parameters.    Set    (WaterKeys.InverseViewProjection  , InverseViewProjection)    ;
        pass.   Parameters.
  Set(WaterKeys.   CameraPosition, CameraPosition  )   ;
        pass.Parameters.Set ( WaterKeys.Scattering    , Scattering); pass.Parameters.Set(WaterKeys.Absorption, Absorption);
 pass   .Parameters .    Set(WaterKeys.PhaseG    ,    PhaseG
		);   
               pass.  
   Parameters    .   Set(    WaterKeys.BehindScale,    BehindScale);
...
// quantities of the scene rather than settings of the document. See Lighting.
  ...
  pass
 .Parameters.    Set(WaterKeys.SkyColour, sky);
...
// permutation; what would happen without a binding is a descriptor the driver refuses.
  Read(WaterKeys    .  ReflectionPlaneBinding   
...
= Samplers. LinearClamp
  ...
  pass.Instances    = tiles.   Tiled   ? WaterTiles.Total(tiles.Count) : 1; pass.Load = tiles   .Tiled ? LoadAction   .Load    : LoadAction.DontCare;
  BuildCount ++    ;
...
.   Sun;
    ...
    // multiplied by, so the water reads the sky the rest of the frame is lit by.
         var sky  = Frame
		?.Lighting   ?.    Environment is {    } environment
            ? environment.Irradiance.   L00    *  (    0.282095f * environment  .Intensity)   : SkyColour;
    return (
           star ?.Direction ?? SunDirection,  star  ?.Radiance ??  SunColour, sky)  ;
      }
    ...
    ///     </para>
    /// </remarks>
       Tiling Tile(GraphicsCompositor compositor    , CompositorFrame frame) { var described = frame .DescriptionOf(ToString () , Output)   ;
             var target  = described is {    } size ?    new Int2(
              size.    Width, size.Height) : compositor.FrameSize    ;
        var tiled = Tiled && Pipelines  is not null && target is    { X: > 0, Y: > 0   }    ;   var count    = tiled ?    WaterTiles.CountFor(  target) : default;
        var name =   $"{this}.Tiles";
             TileCount    =    count  ;
         if (!
     ...
     WaterTiles .Bytes(count)   ,
        BufferUsage.Storage
     ,
   tiled ? MemoryAccess.DeviceLocal   : MemoryAccess .HostUpload   ,    name
            );
...
// imported buffer is one whose contents are the host's business, which is exactly the
  ...
  ?
          frame    .Graph .   CreateBuffer   (description)
              : frame .   
    Graph .    ImportBuffer(Placeholder(  description   )  , description, ResourceState . ShaderRead   ,  ResourceState. ShaderRead  ) ,
      description
              );
       }
                
      if (  !tiled) { return new   (false,  default , target, name) ;
        }
        classify   .Groups  = new   (count  .   X
       , count. Y, 1)  ;
           classify.Samplers    = Samplers;
               classify.Descriptors.Allocator = pass. Descriptors.Allocator ; classify .Parameters. Set    (   WaterTilesKeys.TileCount, count);
		classify.
      Parameters   .Set(WaterTilesKeys.   TargetSize  ,   target)  ;
...
Binding = WaterTilesKeys   .WaterSurfaceBinding,
  Kind = DescriptorKind
         . SampledTexture ,
            ...
            classify.    Descriptors.    Bindings.Add( new    (    )    {
 ...
 (true,  count , target, name);
    ...
    /// </remarks>
        BufferHandle Placeholder(in BufferDescription description) { if (   placeholder.IsValid) {
return placeholder; }
        placeholder = pass.  Device    !. CreateBuffer( description)
;
        pass. Device    .Write(placeholder  ,
               0, new byte [description.Size    ]    )  ;
         return   placeholder;
               }
             
    /// <summary>What one frame's tiling came to.</summary>
              readonly record struct Tiling   (bool Tiled    , Int2  Count, Int2
...
return;
  ...
  disposed =   true  ;
    if (   placeholder.IsValid) {  
                pass .Device?.   Destroy(placeholder) ; placeholder = default   ;
}
pass.Dispose   ()  ; classify.   Dispose();
       }
}
