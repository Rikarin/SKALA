...
public  UiElement  Content  { get;  private set; } =   null
    ...
    ///     </para>
    /// </remarks>
    protected internal virtual string TagName =>  GetType
  ().    Name .ToLowerInvariant ();
    ...
    /// </remarks>
             internal string? Scope => StyleIsScoped ? ScopedStyles
         .ScopeOf(GetType(  ))   : null;
 
     internal void Mount(BuildContext ctx,  UiElement root) { Root    = root  ;
              Content   =   root;
     Slots
 =   null;
...
root    .  AddClass( scope) ;
       ...
       if (  Slots?.   
         TryGetValue  (   BuildContext.DefaultSlot , out var   slot  ) == true  ) {
       Content = slot;
     }
             if (Style is { } css) {
