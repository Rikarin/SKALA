...
///         ⚠
///         <b>
///             The same three shapes <see cref="UiLayer" /> already executes, pointed at a different
///             picture — and that is the whole of why this feature needed a compositor change rather than
///             a shader.
///         </b> A blur, a colour matrix and a quad are exactly what a <c>filter</c> costs. The
...
///         ⚠
///         <b>
///             Every <see cref="UiLayer" /> is a backdrop root, which is what makes this a single
///             prefix and not a walk up the ancestors.
///         </b> Filter Effects 2 § 2 says an element forms a
...
///         ⚠
///         <b>
///             <see cref="Alpha" /> rides the backdrop quad's vertex alpha rather than the matrix,
///             for <see cref="UiDropShadow" />'s reason.
///         </b> <see cref="UiColorMatrix" /> has three rows
...
///         ⚠
///         <b>
///             The order among the three is free, and unlike <see cref="UiLayer.Shadow" /> it is free
///             for a reason rather than by convention.
///         </b> A Gaussian is a weighted sum whose weights sum
...
///         ⚠
///         <b>
///             The picture is clipped to the group's <i>border box</i> and not to
///             <see cref="UiLayer.Bounds" />, and not to its corner radius either.
///         </b> The bounds are the
...
public readonly record struct
    UiBackdrop(float Blur, float Alpha, UiColorMatrix? Matrix = null) {
    /// <summary>Whether this would change the picture, and so whether it is worth a surface.</summary>
    /// <remarks>
    ///     ⚠
    ///     <b>
    ///         A group whose backdrop came out the identity is not opened for it, which is the same
    ///         departure from CSS <c>DrawListBuilder.ElementFilter.Any</c> makes and for the same
    ///         price.
    ///     </b> <c>UtilityComposition.BackdropFilter</c> assembles all nine functions into every
    ...
    public bool
        ...
        && Alpha >= 1f
        && (
            Matrix is null || Matrix.Value.IsIdentity);
}
