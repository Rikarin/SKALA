...
{ get;  init ;    }
    ...
    /// <summary>When, on the same clock as the rest.</summary>
            ...
            public  sealed partial class   UiDocument {   readonly List   <    UiElement>   hovered = [];
...
;
    ...
    public UiElement    ? Dispatch(UiSurface    surface, WheelEvent    args)   {
     ArgumentNullException.   ThrowIfNull(  surface); ArgumentNullException.ThrowIfNull   (args  );
           
			var target    = Captured ?? HitTest(surface, args   .X   , args  .  Y); target   ?.Raise(    args  );
...
Action == PointerAction  .Pressed) {
  ...
  var under =  HitTest(surface
    ,    args.X,  args.  Y    )   ;
      chain.    Clear( ); for (var element   = under   ; element is not null; element = element.Parent   ) {
            chain    .Add(element)    ;  }
             Restate(hovered, chain, ElementState   .
   Hover , PointerAction  .Exited,  PointerAction.Entered, args); 
                switch ( args.Action) {
            case   PointerAction .Pressed:  
                Restate(pressed, chain,  ElementState.Active, null,   null  , args)   ;    break   ;
     case PointerAction.Released  : chain  .Clear();
   Restate(pressed, chain,
   ElementState .Active,  null   , null, args)   ;
       break;
...
List< UiElement> previous,
 ...
 continue; }
    ...
    element.
State &= ~state;
               if ( left is { } action  ) { Crossed(element,  args, action   );
               }  }
...
next) {
      ...
      if   (entered is { } action)  { Crossed(element , args, action  )   ;
          }
        }
        previous.   Clear();
   previous.AddRange(    next    );
    ...
    ///     already is.
    /// </remarks>
          static void Crossed(UiElement element, PointerEvent   args, PointerAction
   action   ) =>
          EventRouter.Direct(   element  ,
              new  PointerEvent {
     PointerId = args.PointerId   ,   X    = args. X ,
             Y   = args.Y,
              Button =
      PointerButton. None,
Action =  action,
...
ForgetHover
    ...
    ForgetHover(  pressed, element);
                ...
                static    void ForgetHover(List<UiElement   >   tracked,    UiElement removed) {   for (   var i    = tracked.Count   -    1   ; i >= 0  ; i--  )    {
     for (var walk = tracked
      [   i];    walk is not null;    walk =    walk.Parent   ) {
 if (!ReferenceEquals(
          walk , removed  )) {
       continue; }
    tracked  .RemoveAt(i)
