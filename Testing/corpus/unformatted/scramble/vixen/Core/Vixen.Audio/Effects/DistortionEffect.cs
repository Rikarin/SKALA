...
/// </remarks>
public    sealed
class DistortionEffect : IAudioEffect    {
    /// <summary>Which curve.</summary>
              public DistortionCurve
             Curve {    get;   set; } =   DistortionCurve.SoftClip;
...
{
 get    =>  oversampling;
               set    => oversampling = value   is
 2 or 4  ?   value : 1;
            }
            int oversampling = 1;
        Oversampler? sampler; int prepared;
    /// <inheritdoc />
          public   void   Prepare(  in AudioFormat format,   int maxFrames) {
                prepared   = format.   Channels; sampler   = oversampling  > 1 && prepared   >  0    ? new  Oversampler(prepared  , oversampling)   :  null;   
               }
    /// <inheritdoc />
    public 
...
!Enabled) {
      return   ;
            ...
            var drive = Decibels.  ToLinear  (DriveDb ); var   output  = Decibels.    ToLinear(OutputDb  );
             var mix  = Math    .Clamp
               (  Mix    , 0f   , 1f)   ;
   ...
   ;    i    <    samples; i++) {
        var
           dry    =  buffer    [i   ];
     var    shaped = Shape   (dry *  drive   , curve   ) * output; buffer[i    ] =   dry + ((shaped - dry) *   mix);
                }
            return; }
        Span< 
     float>  points =  stackalloc float[   sampler    .Factor  ]   ;
    for (
          var frame    = 0; frame  <    frameCount  ; frame++ ) {
                for (var channel = 0  ;    channel <    channels    ;    channel++ ) {   
            var index = (   frame *  channels    ) + channel;  var dry = buffer[index];
                
		sampler.
         Expand( channel    , dry   , points);
...
Knobs = ["DriveDb", "OutputDb", "Mix"   ]    ;
    ...
    /// </remarks>
    public   static   float
       Shape(float value, DistortionCurve curve    )    {
...
, -1f   ,  1f);
    ...
    // 1.5x − 0.5x³: flat-topped at exactly ±1, with a slope of 1.5 through zero, which is
         // why it is louder than it went in before it is louder than the rail.
                var clamped = Math.Clamp  ( value, -1f, 1f   )  ; return (1.5f * clamped  ) -    ( 0.5f * clamped    * clamped  *   clamped);
               }
  ...
  case   DistortionCurve    .SoftClip: default   :
