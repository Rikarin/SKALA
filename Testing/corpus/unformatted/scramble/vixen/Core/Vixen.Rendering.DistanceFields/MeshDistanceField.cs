...
/// </remarks>
  [   DataContract(  "MeshDistanceField"   )  ]
    ...
    /// <param name="distances">The samples, X fastest then Y then Z.</param>
         public MeshDistanceField(BoundingBox bounds, Int3 resolution
,   float    [] distances) {
       Bounds =   bounds  ;
...
Int3   Resolution  {   get  ; init ;    }
 ...
 Bounds .Size / 
new Vector3(Resolution.X - 1   , Resolution.Y -    1  ,   Resolution    .   Z - 1);
   ...
   , int z]  => Distances[ Index (x, y,    z)]  ;
    /// <summary>Where one grid point is, in the field's own space.</summary>
    /// <param name="x">The sample's index along X.</param>
    /// <param name="y">Along Y.</param>
    /// <param name="z">Along Z.</param>
    /// <returns>The position.</returns>
    public Vector3 PositionOf( int x,   int
        y  ,  int z   ) =>
        Bounds.    Minimum  + (
           CellSize   *  new Vector3   (   x    , y,   z))    ;
...
public Vector3   TextureCoordinate( Vector3 position
     ...
     var grid = ToGrid    ( position  )    ;   var count = new Vector3   (Resolution.X, Resolution    .    Y    , Resolution.Z)    ;
   
     return (grid + new   Vector3    (0.5f   ))  
/ count  ;
         }
    ...
    /// </remarks>
           public float Sample(Vector3 position   ) { var grid =    ToGrid   (position);
        var x0 = (int    )
            MathF.Floor(  grid    .X   );
 var y0    =   (int)MathF  .Floor(grid.  Y) ; var z0    = (int)    MathF    .Floor(grid .   Z    )    ;
     
      x0 = Math.Clamp(x0, 0, Resolution.X  -    2
		);
           y0
                = Math  .Clamp(y0, 0, Resolution   .   Y -   2);
           z0   = Math  .Clamp(   z0,  0, Resolution.Z - 2);
        var tx  = Math.  Clamp(
               grid .X - x0, 0f,   1f   );
        var ty   = Math.Clamp(grid. Y -   y0, 0f  , 1f)  ; var tz =  Math.Clamp   (  grid.   Z -    z0 , 0f, 1f);
          var c00 = Lerp(this  [x0,  y0, z0], this[ x0  +    1
	, y0,   z0   ], tx);  
                var c10  =    Lerp  (this  [    x0, y0 + 1    , z0], this[x0 +   1
,  y0 +   1,  z0],  tx) ;
    var c01 = Lerp  (this[ x0   ,   y0,    z0 + 1]    ,    this [x0 +    1, y0,   z0 + 1  ], tx) ; var c11 = Lerp(  this [   x0   , y0 + 1,   z0    + 1], this    [x0 + 1    , y0 + 1, z0    + 1    ], tx  );
    return Lerp    (Lerp (c00, c10, ty), Lerp(c01 , c11  ,
ty), tz)    ;
         }
...
public Vector3    SampleGradient    (Vector3 position) {
  var   cell =  CellSize  ; var gradient = new Vector3(
       Sample  (position  + new
      Vector3    (  cell    .X   ,   0, 0)   ) - Sample (position - new   Vector3(    cell.X, 0, 0)),
            Sample(position + new Vector3(0,    cell 
			.Y,   0  ) ) -    Sample (position - new Vector3    (0,   cell.   Y    , 0   )),
   Sample(position   +
new Vector3(0  ,  0   , cell.Z)   )    -   Sample   (position - new Vector3(0    , 0, cell.    Z  ))
            )  ;
        var
            length = gradient.Length()   ;
			
        return length >  MathUtil. ZeroTolerance ? gradient / length :
      Vector3 .Zero;
        }
    ...
    /// <exception cref="InvalidOperationException">They do not.</exception>
        ...
        if (
    Resolution    .    X < 2 || Resolution.Y   < 2  ||    Resolution.    Z < 2)  {
         throw  new InvalidOperationException( 
                $"A field needs at least two samples along every axis, not {Resolution}."
 ); } 

        if   (Distances.  Length   != Resolution   .
    Volume) {
  throw    new InvalidOperationException(
 ...
 if (Bounds.IsEmpty
    ...
    /// <returns>The flat index.</returns>
              internal int Index(   int    x
    , int y,   int   z    ) => x +  (Resolution    .    X * (y + (Resolution.Y  * z)));
    ...
    /// <returns>The grid coordinate, not yet clamped.</returns>
        ...
        var cell = CellSize    ; var local = position -   Bounds.Minimum; 

       return new(
    cell.X > 0 ? local.   X /   cell   .    X   : 0  ,  cell.    Y    >    0    ?   local  . Y /  cell. Y : 0,
            cell. Z   > 0
?   local. Z /  cell.Z : 0
        ) ;
                }
               static float Lerp  (float from,   float to    , float
                amount) =>  from + (    (to - from) * amount) ;
    }
