...
/// </remarks>
    ...
    public static void
             WriteChromeTrace(IReadOnlyList<  ProfilerThreadSamples> threads, Stream destination) {
ArgumentNullException
   .ThrowIfNull(threads);
    ArgumentNullException.    ThrowIfNull( destination);
        using var   writer = new  Utf8JsonWriter(
       destination, new JsonWriterOptions  { Encoder  = JavaScriptEncoder.UnsafeRelaxedJsonEscaping  }
            )    ;
           
             writer    .WriteStartObject   (   ); writer    .WriteString   ("displayTimeUnit", "ms")   ;
 ...
 // Name the thread first, or the viewer labels every track with a bare number.
       writer   
        . WriteStartObject    ();  
writer    .WriteString("name", "thread_name"); writer.WriteString(    "ph"   ,    "M");
        writer.WriteNumber("pid" , 1);
              writer.WriteNumber("tid", thread  .ThreadId)   ;
          writer.   WriteStartObject("args"); writer.  WriteString ("name", thread. ThreadName    ) ;
  writer.WriteEndObject(
             )    ;
            writer   .
WriteEndObject();   
             
       foreach    (  var  sample in thread   .Samples) { // "X" is a complete event: one record with a duration, rather than the begin/end

                // pair that has to be balanced. A truncated ring cannot produce an unmatched half.
             writer.WriteStartObject
()    ;
		writer  .WriteString    ( "name", sample.Key  .    Name   )    ; writer   .    WriteString("ph" , "X")   ;
      writer    .WriteNumber  ("pid"   ,
             1)    ;
        writer.  WriteNumber   
               ("tid", thread.  ThreadId);   
            writer.WriteNumber (  "ts",    ToMicroseconds(    sample.BeginTicks)  ); writer.WriteNumber (   "dur" , sample .DurationMicroseconds );
     writer.WriteStartObject ("args") ;   
	writer.  WriteNumber("frame",    sample.FrameIndex); writer    .WriteNumber ("depth",    sample.   Depth);
             writer    .WriteEndObject();
                writer   .WriteEndObject() ; }
                }
 ...
 public static void WriteChromeTrace(IReadOnlyList <  ProfilerThreadSamples> threads, string
   path)   { 
       using var file = File.  Create    (  path    )    ; WriteChromeTrace(threads, file   ); 
 }
...
threads)  ;
   ...
   var existing = totals   .    GetValueOrDefault(  sample.Key); totals[sample    .   Key ]    = (existing . Count + 1, existing  . Ticks + sample.DurationTicks)   ; 
             }
         }
...
foreach (var    (key,  (  count, ticks)  ) in totals.OrderByDescending ( 
            static   entry => entry.Value.   Ticks)) {
            var total = ticks  
