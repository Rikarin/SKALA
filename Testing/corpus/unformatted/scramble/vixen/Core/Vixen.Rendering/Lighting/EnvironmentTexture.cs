...
///     </para>
/// </remarks>
public sealed class EnvironmentTexture :  IDisposable { readonly CubeImage[  ] chain ;
   ...
   bool disposed   ;  
    ...
    ///     the handles do not.
    /// </remarks>
    public EnvironmentTexture( IGraphicsDevice graphics ,
    ...
    ArgumentNullException
        ...
        ArgumentNullException.
             ThrowIfNull    (prefiltered  )   ;
            
        if (prefiltered.Count    == 0
       ) {
       throw new ArgumentException (
                "A prefiltered chain needs at least one level. An environment with no levels is one a " +   "shader would sample at a mip that does not exist.",
                nameof   (    prefiltered )    ) ;
        }
        chain
  =    [.. prefiltered];
        ...
        for (var level  =  1   ;    level   < chain.Length;
               level++ )    {
        var expected =
...
Size => chain[   0].Size  ;
   ...
   public TextureHandle    Texture => texture;
    ...
    /// </remarks>
        public void   Invalidate(   ) => uploaded    =
               false ;
    ...
    /// </remarks>
    public bool Upload(ICommandList commands)    {    ArgumentNullException.ThrowIfNull(commands  );
...
face =   0; face < 6;    face    ++   ) {
           device    .   Write(staging,    offset    , Staged(  image.Face(    (CubeFace)   face  
              ...
              commands.CopyBufferToTexture  ( staging,
                    offset,
    new TextureRegion(texture, level,    face)    ,
     new Int3 (image.Size, image.Size 
, 1   )
                )
         ;
...
true ;
        ...
        if ( view .IsValid) { device . Destroy(view)    ;
    }
              if (texture   .IsValid ) { device   .Destroy(texture); 
       }  
        if (staging. IsValid)    {
 ...
 int BytesPerTexel
     =>   Format == PixelFormat.Rgba32Float  ? sizeof (    float) * 4 :  sizeof (ushort) * 4;
...
return  total ; }
    ...
    /// </remarks>
     ReadOnlySpan<byte    > Staged(ReadOnlySpan   <Vector3> texels) { if  (Format == PixelFormat.Rgba32Float) {
            var wide   = new float[texels.Length * 4]
;
       
            for (   var    index = 0 ; index    < texels.Length  ;  index++   )    { wide  [   (index   * 4) +  0] = texels[ index    ].X   ;   
    wide[    (index * 4    ) + 1]    = texels[   index].Y;  wide[(index * 4    ) +  2] = texels[index].Z;
                wide[   (
           index  * 4) +    3] =    1f   ;
          }

  return MemoryMarshal   .    AsBytes    (wide    .
    AsSpan())   ;   
         }
       ...
       for (var index =    0
; index < texels.    Length ; index   ++  ) {
             narrow[(index * 4) + 0 ] = (Half)texels[index]  .   X;  narrow[(index * 4) + 1]  = (  Half)texels[index  ]    .Y;  
          narrow    [(index *
            4  ) + 2] =   (  Half) texels[index  ] .Z    ;
       narrow[(    index *   4
	) +   3   ] = (Half)    1f;   
 }
  ...
  void Create    () {  
              texture =   device .CreateTexture( new TextureDescription(
                Format, Size,   
         Size   ,
TextureUsage  .   Sampled | TextureUsage  . CopyDestination  , MipLevels: MipCount,
