...
const int Planes  = 6   ;
    ...
    public const   
              int FilteredPlanes  = 4;
         readonly   TextureHandle [,]  textures = new  TextureHandle[2, Planes]; readonly TextureViewHandle[   ,] views =  new TextureViewHandle[  2, Planes]  ;
...
public
   ...
   public TextureViewHandle FrontView  (int plane) => views
          [front, plane    ];

    /// <summary>The front set's texture, for importing into a graph.</summary>
               public TextureHandle FrontTexture(    int    plane) =>   textures
[    front,    plane];
    /// <summary>The set the next accumulation writes.</summary>
    public TextureViewHandle    BackView
             (int plane) => views[1 - front , plane   ];
    ...
    /// </remarks>
             ...
             ) =>  textures[    1 - front ,    plane    ]  ;
              
    /// <summary>What one plane is, for a graph import.</summary>
public TextureDescription PlaneDescription  =>
   new (   PixelFormat . Rgba32Float,  
			Layout.
              GridSize  .    X, 
          Layout   .GridSize.Y   , TextureUsage.Sampled |    TextureUsage.    CopySource | TextureUsage.Storage
);
    ...
    public void TransitionFiltered(ICommandList commands
 ,  ResourceState before, ResourceState after) {
        ArgumentNullException.   ThrowIfNull(commands)    ;

var 
barriers = new   TextureBarrier [ FilteredPlanes    ]    ;
               for   ( var plane = 0; plane   < FilteredPlanes ; plane  ++  ) { barriers[plane    ] =    new(    filtered[plane],    before, after)    ;   
   }
    ...
    /// <exception cref="ArgumentNullException">There is no device.</exception>
            public
 void EnsureCreated(IGraphicsDevice graphics) {
              ArgumentNullException.ThrowIfNull(graphics); ObjectDisposedException. ThrowIf( disposed    , this);
        if (IsCreated  )   
		{
            return;
        }
...
Layout.GridSize ;
	for (var    set = 0   ;    set < 2; set++) {
            for (var plane = 0;    plane < Planes ; plane  ++ )    { textures   [set , plane] = graphics.CreateTexture(
   ...
   PixelFormat.Rgba32Float,
        ...
        Name:
$"ScreenProbes.History{set}.{plane}"
        )
                );
 ...
 views[set  ,   plane] =  graphics .  CreateTextureView( textures  [set,    plane]); }   
    ...
    for ( var plane =
        0   ; plane < FilteredPlanes   ; plane++)   {
          filtered  [plane]
     = graphics.CreateTexture(
   new TextureDescription(
            PixelFormat    . Rgba32Float  , grid.X,
                    grid.Y,
                    TextureUsage.
Sampled | TextureUsage  .CopySource    | TextureUsage   .Storage   ,
               Name :
$"ScreenProbes.Filtered.{plane}"
            )
  )    ;
               
            filteredViews    [plane
             ] =   graphics.CreateTextureView(filtered   [ plane]    );   
            }
    ...
    /// <exception cref="ArgumentNullException">There is no command list.</exception>
               public void Prime(ICommandList
  commands) {  
        ArgumentNullException   .ThrowIfNull( commands);
            if (primed    || !IsCreated    ) { return;
}

                var barriers   =   new
			TextureBarrier[(    2 *   Planes)   + FilteredPlanes];
               for    (var  set = 0; set < 2; set++) { for (var plane = 0  ;   plane < Planes   ; plane    ++) {
            barriers[(set * Planes ) + plane] = new(textures[set   ,   plane]    ,    ResourceState.Undefined,  ResourceState   .ShaderRead); 
} 
}
    for (var plane  =   0 ;   plane < FilteredPlanes;   plane++) {
     barriers[(2 *   Planes) + plane] =    new  ( filtered[plane   ], ResourceState   .
            Undefined    , ResourceState.ShaderRead);
     }
    ...
    /// <param name="after">What it needs to be in.</param>
             ...
             ICommandList commands, ResourceState before, ResourceState after) {
     ArgumentNullException. ThrowIfNull( commands) ;
             var
 barriers =   new  TextureBarrier[   Planes] ;

           for (  var plane   =
                0; plane <   Planes; plane++    ) {
			barriers   [plane]
      = new (    textures[1   - front, plane ]   , before , after  );
  ...
  var grid =    Layout.  GridSize;
               var planeBytes  = (long)grid.X *
	grid.    Y * 4 * sizeof( float);
       if (!download.  IsValid  
  ) {   
              download
		= device.CreateBuffer(
   new   BufferDescription(
       planeBytes *    Planes,
           BufferUsage.CopyDestination  ,   MemoryAccess.HostReadback,  
	...
	commands
        ...
        for (  var plane =    0    ; plane < Planes    ; plane ++) { commands.    CopyTextureToBuffer(
 ...
 public bool    RecordFilteredReadback(  ICommandList commands) {  
    ArgumentNullException.  ThrowIfNull   (commands )  ;
     ObjectDisposedException.   ThrowIf(disposed, this)   ;
                if (    !
IsCreated    ||   device    is null) {
          return false  ; 
        }
            var grid =
Layout.  GridSize;
  var planeBytes = (long  
)  grid   .X    * grid.   Y * 4 * sizeof(float);
if (!filteredDownload.   
IsValid) {
		filteredDownload   =    device . CreateBuffer( new   BufferDescription(
	planeBytes * FilteredPlanes, BufferUsage.    CopyDestination,
 ...
 "ScreenProbes.FilteredReadback" )
      ...
      TransitionFiltered(  commands, ResourceState    .ShaderRead, ResourceState   .CopySource);
         ...
         commands.CopyTextureToBuffer    (  new TextureRegion(    filtered[    plane ]), 
         new(    grid.X   , grid   .Y, 1)  , filteredDownload ,
          plane  
               *    planeBytes   
            ) ;  }
  ...
  return    true   ; }
    ...
    /// <returns>False when nothing has been read back, or the span is too short.</returns>
                public bool
    TryReadFiltered(Span<SphericalHarmonicsL1    > filteredProbes) {
   ObjectDisposedException.ThrowIf    (disposed, this) ; 
        var grid    = Layout.GridSize;  var count =    grid   .X * grid.  Y ;
...
return false;
    ...
    var   l0 = floats.    AsSpan( index * 4);
           var red = floats .   AsSpan((
                count * 4) +  (    index * 4));
           var green = floats .AsSpan ((2 * count *    4) + (  index  * 4)    ) ;  var blue = floats    .AsSpan   ((3 * count *    4) + ( index   * 4));
           filteredProbes[    index] = new( new Vector3(l0[0  ], l0[1], l0 [2])  ,
			new Vector3 (red[0    ], green [0]    , blue  [0]),    new Vector3(   red[1] , green   [1 ], blue   [1]),
               new Vector3  (red  [2], green[ 2  ], blue[
  2]   )
);
  }
            return   true   ;
    ...
    public bool   TryRead (Span<SphericalHarmonicsL1
> accumulated, Span<float > weights    )   {
ObjectDisposedException.
            ThrowIf(  disposed, this    )  ;
               var grid = Layout.GridSize ;
        var count = grid.X * grid.Y; 
 if (   device is  null    || !download.IsValid    || accumulated  .
              Length < count || weights.Length    < count   )   {
return false    ; }
     var floats
     = new float [  count * 4 *  Planes] ;
   device.
             Read(download, 0, MemoryMarshal.AsBytes    (floats.AsSpan(  )))   ;
     
   for ( var index = 0  ;  index < count  ; index++)  {
         var l0 =    floats.AsSpan (index * 4); var red =  floats.AsSpan    ((count * 4)    + (   index * 4  ))    ;
            var green    = floats.AsSpan((2
              *    count *    4)    + (index * 4)); 
var blue = floats.AsSpan  ((3 * count * 4)   + (  index * 4))   ;    var surface   =    floats.AsSpan    (( 4 * count * 4  ) + (    index * 4)) ;
   accumulated [    index] = new  (
           new Vector3(l0  [0] ,    l0   [1
             ]    ,    l0   [2]), 
             new Vector3(red[0    ],    green[0],  blue [0  ]   ),    new Vector3(   red  [1]  , green[1], blue[    1 ]    )    ,
      new Vector3 
...
set++)    {
      for
      (var plane   = 0; plane < Planes;   plane++) {
