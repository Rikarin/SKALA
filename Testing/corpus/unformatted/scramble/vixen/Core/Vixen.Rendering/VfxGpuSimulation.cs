    ...
    // SPDX-License-Identifier: Apache-2.0
           ...
           using Vixen.Core.   Mathematics; using Vixen.Graphics;
using
              Vixen .Vfx;
...
readonly    IGraphicsDevice  device;
      ...
      // Two full sets of the particle attributes, and which one is the live set flips on every reap.
       ...
       // copying the whole of it back every frame.
         readonly BufferHandle[]    []   sets; readonly DescriptorSetHandle   [    ] descriptorSets;
    int   current;
    ...
    // is the only array in it.
         readonly
             BufferBarrier[]   barriers;
               readonly BufferHandle staging; readonly BufferHandle readback;
  ...
  // everywhere below, which is one test rather than a parallel `hasReap` flag to keep in step.
    ...
    readonly   BufferHandle   counterReadback; readonly  BufferHandle template ;
                readonly BufferHandle
arguments;
            readonly  DescriptorSetLayoutHandle   setLayout; readonly PipelineLayoutHandle layout;
    ...
    // state for all of them because every path here touches all of them together.
       ResourceState  state = ResourceState.Undefined ;  ResourceState  argumentState = ResourceState  .  Undefined;
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="capacity" /> is not positive.</exception>
      public    VfxGpuSimulation(   IGraphicsDevice  device, VfxShader
shader, int capacity) {
...
ThrowIfNull(device);
	...
	this.  device  = device;
        Capacity =    capacity ; HasReap   = shader.  HasReap ;
      bindings  = [.. shader.Bindings]    ;
        particles  = [.. bindings.Where(binding => binding.Role  == VfxBindingRole.
         Particle)] ;
                offsets = new
long  [particles  .Length   ];
        var
total = 0L;
       for (var index =    0; index <   particles   . Length   ; index ++   ) { offsets[index  ] = total;
total    +=   VfxShaderPacking  .Size   (particles[index  ], capacity);
 ...
 new   byte[   total  ];
  ...
  // second set is copying the whole live set back after every reap, which is the same
    ...
    var   copies   = shader.HasReap ? 2 :
1 ;
    
      sets = new BufferHandle    [copies][];
    
        for  (var copy = 0;    copy < copies;  copy++  )    { sets[copy ] = new BufferHandle[  particles. Length    ];
       
for (var index =    0; index < particles.    Length;  index++ ) {
       sets[copy][  index] = device.  CreateBuffer(new( VfxShaderPacking    .Size(particles[   index]    , capacity)   ,
                    BufferUsage. 
  Storage    | BufferUsage .CopySource    |  BufferUsage   .CopyDestination    ,
 ...
 // One staging buffer and one readback buffer covering every attribute, rather than a pair
      ...
      staging = device    .CreateBuffer(
               new   (total   ,
BufferUsage    .CopySource, MemoryAccess.  HostUpload,   $"{shader.Name}.Staging" )
			)   ;
 
            readback = device
  .   CreateBuffer(
         new(  total  , BufferUsage.  CopyDestination,
MemoryAccess   .HostReadback   ,   $"{shader.Name}.Readback")
		);
        if (
               shader .    HasReap    ) {
            counter =  device.CreateBuffer(new    ( sizeof( uint)  ,
    ...
    new(
sizeof(uint), BufferUsage  .CopySource,    MemoryAccess. HostUpload  , $"{shader.Name}.Zero")
               )    ;
            device
    . Write    (zero, 0  , new byte   [    sizeof(uint)]  );
...
,
      ...
      MemoryAccess.HostReadback,  
                ...
                DrawArgumentsSize   ,    BufferUsage.CopySource,
   MemoryAccess
 .HostUpload    ,
  ...
  arguments  = device. CreateBuffer(new( DrawArgumentsSize   ,
...
=    6;
        ...
        barriers
        =    new   BufferBarrier[(particles.Length   * copies) + ( shader.HasReap   ? 1 : 0)  ]    ;
 ...
 var entries = new  DescriptorBinding[   
                bindings.Length];

		for (  var index = 0;   index < bindings .  Length   ; index  ++)  {
            entries [index] =   new((   uint    )  index, DescriptorKind
.StorageBuffer , ShaderStage.Compute)  ;
   }
   
               setLayout 
      =   device.CreateDescriptorSetLayout (
            new   
(DescriptorSetSlot   .PerFrame, entries,  $"{shader.Name}.Particles"   )
              )
			;
                layout    = device    . CreatePipelineLayout   (new( [    setLayout],
         [new    (ShaderStage.
           Compute,  0  , VfxShaderUniforms.Size)],
            $"{shader.Name}.Layout"
  )
);
...
// order `VfxShaderEmitter.Emit` assembles them in.
          descriptorSets = new DescriptorSetHandle[   copies];
           for (var
       copy = 0; copy <   copies; copy ++)    {
              descriptorSets[  copy]    = device.CreateDescriptorSet
(setLayout, $"{shader.Name}.Particles{copy}"  )    ;
                
    var writes = new DescriptorWrite[bindings.  Length    ];
   for  (var index = 0    ; index < bindings. Length; index++) { writes    [index] = DescriptorWrite.  Storage  ((uint    ) index    , bindings [index] .Role switch {
               VfxBindingRole   .Particle => sets    [copy][index],  VfxBindingRole    .    Compacted => sets[  1 - copy    ] [index - particles.Length],
              
              _ => counter
          });   }   
device
         .    UpdateDescriptorSet(descriptorSets[copy],    writes   );
           }
  }
  ...
  public  int Dispatches { get; private  set; }
    ...
    /// <summary>The buffers, in the order the shader declares them.</summary>
     public IReadOnlyList<VfxShaderBinding>
Bindings => bindings;
...
=>   layout   ;
    ...
    /// </remarks>
           public DescriptorSetHandle Descriptors => descriptorSets
[ current];
    ...
    /// </remarks>
		public
           BufferHandle   DrawArguments   => arguments    ;
...
Span<uint> command = [( uint)
 value, 0u,    0u, 0u  , 0u    ];
  ...
  for (var index
            = 0   ; index < particles.Length; index++)    {
if    (particles[ index] .
        Slot    <  0 && particles[index]  .Attribute ==  attribute ) {
         return sets[current]  [
    index   ]   ;
            } }
...
BufferHandle Custom (int slot)   {
 ...
 if (particles
    ...
    /// <summary>How many workgroups cover a run of particles.</summary>
           ...
           count <= 0 ? 0 :  (    (count + VfxShader.WorkgroupSize   - 1
                ) /   VfxShader.WorkgroupSize);
    ...
    /// </remarks>
                public void Upload(ICommandList list
,    ParticleBuffer particles, int count) {
           ObjectDisposedException.   ThrowIf(  disposed   ,    this  )  ; ArgumentNullException.ThrowIfNull(   list   )   ;
         ArgumentNullException. ThrowIfNull(particles  ); ArgumentOutOfRangeException.  ThrowIfNegative(count    );
      ...
      count,  Capacity);
        ...
        if (count    ==  0) {
           ...
           for    (var    index   =
0   ; index <  this.particles.Length    ; index ++ ) {
            var bytes =  VfxShaderPacking.    Size(this. particles   [index], count);
                VfxShaderPacking
.  Pack(particles    , this.   particles[index ],    count  , scratch.AsSpan((int)    offsets[index], bytes) );
    }
            device .Write(    staging    , 0   , scratch); Transition  (list, ResourceState. CopyDestination);  
          for (var index = 0; index <   this.particles.    Length;
index++) {
			list.CopyBuffer(
             staging,
             offsets [index  ]  ,
 ...
 ;
    ...
    /// </remarks>
    public void Initialize( ICommandList   list,
       PipelineHandle  pipeline  ,
         int
        first ,  
        int count, uint    seed,
        float time, Vector3 origin = default
           )    =>
          Dispatch( list,
             pipeline  ,
            new () {
    ...
    /// <exception cref="ArgumentNullException"><paramref name="list" /> is null.</exception>
        public void Update( ICommandList
           list,    PipelineHandle  pipeline, int count, float deltaTime ,   uint    seed    , float time) =>
	Dispatch(list    , pipeline, new   (  ) {
       DeltaTime =  deltaTime  ,   Seed = seed, First =  0,   ParticleCount  = count   , Time = time }) ;   
...
.ThrowIfNegative(   count);
 ...
 for (    var index =  0    ; index <   
          ...
          list.CopyBuffer(    sets    [current]  [index]    ,
                0    ,
    readback,
                offsets[index], VfxShaderPacking    . Size(particles[   index]    , count   )
           )   ;
        }
...
ObjectDisposedException.ThrowIf  (disposed, this) ;   ArgumentNullException.ThrowIfNull(particles );
ArgumentOutOfRangeException
              .ThrowIfNegative    (count);
               ArgumentOutOfRangeException.
            ThrowIfGreaterThan (count, Capacity )    ; 
        if   (count ==
              0) { 
return;

  }
   device. Read    (readback, 0
         , scratch   );
          
for (   var index = 0; index < this.particles .    Length;    index++) {    var bytes = VfxShaderPacking.    Size(this.particles[index],    count );
VfxShaderPacking.Unpack(
      scratch.   AsSpan((int)offsets    [index]    ,   bytes), this.  particles[index  ] ,
count ,
        particles  )    ;
        } }
    ...
    ///     </para>
    /// </remarks>
     public    void Reap( ICommandList list, PipelineHandle pipeline, int   count   ) { ObjectDisposedException   .  ThrowIf(   disposed, this);
      ArgumentNullException.ThrowIfNull(list)  ; ArgumentOutOfRangeException.  ThrowIfNegative(count);
        ArgumentOutOfRangeException    .ThrowIfGreaterThan
                (   count, Capacity    )  ;
     if (!   HasReap   ) { throw   new  InvalidOperationException    (  
                "This effect's shader has no reap kernel, so there is nothing to compact and no buffers to "
       + "compact into. A graph needs both an age and a lifetime before a particle can be finished — " + "see VfxShader.HasReap."
            )    ;
}
     if (count == 0) {  return;
               }
          Transition
             (list, ResourceState.  CopyDestination)  ;
          list   .CopyBuffer(zero, 0, counter  , 0,  sizeof (uint)    )    ;

              Transition(list, ResourceState .ShaderWrite
);
   
 list .BindPipeline(    pipeline    ); list.BindDescriptorSet(   DescriptorSetSlot.PerFrame, descriptorSets[current]);
         list    .PushConstants(
              ShaderStage.Compute,
 0, 
            Raw
     (new   ( ) { DeltaTime    = 0f, Seed = 0u    , First   = 0   , ParticleCount = count, Time  = 0f   })  
   );
        list
   ...
   state = ResourceState . ShaderWrite;
        ...
        Transition   (list  , ResourceState.   CopySource)  ;
             list.CopyBuffer(counter, 0  ,
    counterReadback   , 0, sizeof (uint)   ) ;
        state = ResourceState    .CopySource;
           current = 1 - current
;
	}
...
(disposed,   this);  
        if   (!
                HasReap ) {
            throw
          new   InvalidOperationException  (    "This effect's shader has no reap kernel, so nothing counted."    );  
              }
        Span
             <byte>  bytes   =  stackalloc byte[    sizeof(uint)  ];
    ...
    ///     </para>
    /// </remarks>
               public void WriteDrawArguments(ICommandList list) { ObjectDisposedException  .ThrowIf(disposed, this  );
            ArgumentNullException. ThrowIfNull (list);
    
if (
   !HasReap ) {
             throw new InvalidOperationException( "This effect's shader has no reap kernel, so there is no device-side count to draw from. An "
                + "effect whose particles never die knows how many it has without asking."
            )  ;  }
			list    .Barrier(
   new([   new(    arguments, argumentState  , ResourceState    .   CopyDestination)], []   )  )   ;
        
        list 
                .CopyBuffer    (template, 0    , arguments    , 0   ,   DrawArgumentsSize);
 ...
 list   .CopyBuffer(counter , 0    , arguments,
             sizeof(uint), sizeof(uint)   );
 
            list   .Barrier
        (new([    new(arguments,  ResourceState.CopyDestination , ResourceState.IndirectArgument)  ],  []    ));
    ...
    /// <summary>Frees every device object it made.</summary>
           ...
           () {   
			if (  disposed) { return  ;
               ...
               disposed = true;
        foreach   (var
set    in   descriptorSets)  { 
       device.Destroy (    set);
        } 
      
device    .   Destroy(layout);
    device.Destroy   (    setLayout);   device   . Destroy( readback);
    device.Destroy(staging  ) 
;
         foreach   (var set   in sets) {
            foreach  (var buffer  in set  )    { device   .Destroy(buffer)   ;
		}   }   
...
void Dispatch(ICommandList list, PipelineHandle  pipeline    , VfxShaderUniforms constants   ) {  ObjectDisposedException.ThrowIf  (disposed, this);
            ArgumentNullException.
        ThrowIfNull(list  )   ;
  ...
  return;
          ...
          Transition(
         list, ResourceState    .ShaderWrite); 
               
     list  .BindPipeline(pipeline    );
        list
            .BindDescriptorSet(DescriptorSetSlot. PerFrame    ,    descriptorSets[current])    ;
      list.PushConstants(ShaderStage   .Compute    , 0
               , Raw(constants))  ;
                list.
               Dispatch (Groups(   constants.ParticleCount    )) ;
               Dispatches
         ++;
    ...
    /// </remarks>
    void Transition   (  ICommandList list, ResourceState  next) { var at = 0;
      foreach    (var set in sets)
          {
...
(HasReap    ) {
              barriers
        [at] = new(    counter, state , next  );
        }
        list.Barrier    (   new(barriers, [])    )  ;
    }
 static ReadOnlySpan
<byte > Raw(in VfxShaderUniforms constants   ) =>
           MemoryMarshal.   AsBytes(    new ReadOnlySpan    <VfxShaderUniforms>(
in constants)   )[    ..VfxShaderUniforms.Size    ]    ;
          }
