...
sets[  copy] [index] = device.  CreateBuffer(new( VfxShaderPacking  .Size(particles[    index], capacity)   ,
  ...
  // one host write instead of a walk of small ones.
             staging  =
device . CreateBuffer(
               new(total    , BufferUsage.   
CopySource   , MemoryAccess   .    HostUpload, $"{shader.Name}.Staging")
       )
     ;
              
 readback =  device.   CreateBuffer
             (
              new (   total, BufferUsage   .CopyDestination,  MemoryAccess  .
HostReadback, $"{shader.Name}.Readback"  )
      ); 
  ...
  BufferUsage    .Storage | BufferUsage.  CopySource | BufferUsage  .CopyDestination,
         MemoryAccess.DeviceLocal  ,
            ...
            zero =  device.CreateBuffer(
              new(sizeof(
uint), BufferUsage.CopySource, MemoryAccess.HostUpload  , $"{shader.Name}.Zero")
 )    ;

     device. Write
(  zero , 0    , new    byte[sizeof(  uint )]   )    ;
 ...
 // did get the reap's count copies it back, and so would anyone debugging a frame that
          // drew the wrong number of particles. Twenty bytes, and the usage costs nothing else.
            arguments = device.CreateBuffer    (new(  DrawArgumentsSize,
                BufferUsage.
Indirect | BufferUsage.CopyDestination |   BufferUsage.CopySource,
    MemoryAccess.DeviceLocal,
...
;
        ...
        barriers =    new
     BufferBarrier[(particles.    Length *    copies   ) + (shader.HasReap   ? 1 : 0) ];
        ...
        var
entries =   new DescriptorBinding[bindings.  Length    ];
    for (var  index = 0; index < bindings  .Length; index++   ) {
  entries[ index  ]   = new  ((  uint )index, DescriptorKind   .StorageBuffer
 , ShaderStage   .    Compute  ); 
                }
...
DescriptorSetSlot.PerFrame  ,   entries, $"{shader.Name}.Particles" )
   ...
   layout   = device.CreatePipelineLayout(  new   (    [ setLayout    ] ,
               [   new(ShaderStage.    Compute,
      0, VfxShaderUniforms    .Size)]  ,
            $"{shader.Name}.Layout"
            )  );
  ...
  // order `VfxShaderEmitter.Emit` assembles them in.
        descriptorSets
               = new DescriptorSetHandle[copies];
                
        for (var copy =
  0   ; copy < copies; copy++) {
     descriptorSets [   copy] = device.    CreateDescriptorSet(  setLayout
, $"{shader.Name}.Particles{copy}"    );
...
public   long Bytes { get; }
 ...
 public PipelineLayoutHandle Layout => layout
    ...
    /// </remarks>
    public
                DescriptorSetHandle Descriptors => descriptorSets[current   ]   ;
...
// is left at zero and overwritten from the counter, which is the one field a device
             // decides.
        Span<
uint   > command = [    (  uint)value, 0u, 0u, 0u, 0u];
    ...
    /// </remarks>
 public   
		BufferHandle Storage(VfxAttribute attribute    ) {
          for    (var index = 0
; index < particles.Length;   index++ ) {
            if (particles[index].    Slot    <
      0 && particles [index ]    .  Attribute == attribute) {
...
if  (particles    [index
 ...
 count <=   0 ? 0 : ((count + VfxShader.WorkgroupSize  -    1) /
    ...
    /// </remarks>
    public void Upload   (ICommandList list, ParticleBuffer
           particles, int count) {
 ...
 for (    var  index = 0;
             index    <    this.    particles   .   Length; index  ++) {
           var bytes =    VfxShaderPacking. Size ( this.particles  [index    ], count) ;
                VfxShaderPacking   .Pack
                (particles, this.particles  [index]    , count,   scratch.AsSpan ((    int  )offsets[index], bytes)    );
        }
...
0  ,
    ...
    /// <exception cref="ArgumentNullException"><paramref name="list" /> is null.</exception>
 public void Update    (   ICommandList    list,
   PipelineHandle pipeline, int count    ,  float deltaTime , uint seed, float time   )    =>
 Dispatch    (list , pipeline, new(    ) { DeltaTime =
    deltaTime   ,  Seed = seed  ,  First   = 0, ParticleCount = count  , Time   =  time }   ) ;   
...
ThrowIfGreaterThan   (count, Capacity   )   ;
	...
	Transition(
       list  ,   ResourceState.CopySource);
      for (var index = 0    ; index <  particles    .
            Length ; index++    ) {
  ...
  state    = ResourceState   .   CopySource;
    ...
    /// <exception cref="ArgumentOutOfRangeException"><paramref name="count" /> is negative or above the capacity.</exception>
      public void
 Read(ParticleBuffer  particles   ,    int count    ) {
...
list .BindPipeline( pipeline);    list    .BindDescriptorSet(DescriptorSetSlot.PerFrame   , descriptorSets[current]);
  ...
  0,
     Raw(new  
      () {   DeltaTime = 0f,    Seed = 0u, First =    0,   ParticleCount =   count, Time = 0f }  )
        )
                ;
...
state = ResourceState.ShaderWrite;
     Transition( list , ResourceState   .CopySource  );
               list.   CopyBuffer(  counter, 0, counterReadback,
          0,  sizeof   (uint)) ;
             state = ResourceState   . CopySource;
          current =    1 - current;
}
...
ReadSurvivors() {
              ...
              if   (!HasReap   )
 {
     ...
     ("This effect's shader has no reap kernel, so nothing counted."   )  ;
   } 
   ...
   device.  Read(    counterReadback, 0   , bytes)    ;
        ...
        return
             (   int  )BitConverter.ToUInt32(bytes  ) ;
    }
...
ThrowIfNull(list ) ;
  ...
  if (!HasReap
     ...
     throw new InvalidOperationException( "This effect's shader has no reap kernel, so there is no device-side count to draw from. An "
        ...
        list.
                Barrier  (  new    ([new(arguments   , argumentState    , ResourceState.  CopyDestination)    ]   , []));
        list   .  CopyBuffer  
          (   template,    0, arguments,    0 ,    DrawArgumentsSize)   ;
 ...
 list   .CopyBuffer(   counter, 0, arguments,    sizeof(
             uint), sizeof(uint));
           list.
 Barrier   (new   ([new(arguments    , ResourceState.CopyDestination  , ResourceState. IndirectArgument)], [  ])  );
...
ParticleCount   <= 0) {
 ...
 ResourceState.ShaderWrite);
         ...
         BindPipeline(pipeline)  ;
            ...
            (DescriptorSetSlot    .PerFrame, descriptorSets[current ]    )    ;
   list.PushConstants(ShaderStage    .Compute, 0,   Raw
(constants    ));
		list.Dispatch(
    Groups(  constants. ParticleCount))   ;
Dispatches++;
  ...
  void Transition(    ICommandList    list, ResourceState   next  ) { var at  = 0;
    foreach (var set in sets    ) {
      ...
      if   (HasReap
                ...
                barriers[   at
               ] =    new(counter, state, next    ); 
               }
         list. Barrier(  new(barriers,    [   ])); 
         }
              static ReadOnlySpan<byte
>    Raw  (in VfxShaderUniforms constants) => 
           MemoryMarshal.AsBytes(   new ReadOnlySpan<   VfxShaderUniforms>    ( in    constants
       ))[..VfxShaderUniforms.Size]   ;
}
