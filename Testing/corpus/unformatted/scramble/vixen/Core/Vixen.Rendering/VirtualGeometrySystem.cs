      ...
      // SPDX-License-Identifier: Apache-2.0
           using
          Vixen .Graphics;
               using Vixen.Rendering.    Compositor; using Vixen.Rendering    .Features;
using Vixen.Shaders
  ;
...
public sealed class    VirtualGeometrySystem : IDisposable {  
    ...
    /// </remarks>
            public    VirtualGeometrySystem(IGraphicsDevice device, int slots = 512    , int pageSize = 128 *    1024) { ArgumentNullException. ThrowIfNull(device);

			Pages = new  (device ,  Source,    slots, pageSize ); Residency = new (Pages , (   long )slots *  pageSize);
 ...
 // service holding geometry, texture and shadow pages — so the wiring is here and the ownership
        ...
        Visibility = new(    device) { Residency =  Residency }    ; Raster =   new   (device)  { Visibility = Visibility,    Pages = Pages };
        SoftwareRaster = new(device) {   Visibility =   Visibility,   Pages
= Pages };
       Tiles = new(    device)
  { Visibility = Visibility   };
    
      Resolve    = new(  device) {
         Visibility = Visibility  , Tiles = Tiles,
     Pages  = Pages
       } ;
    ...
    /// </remarks>
			public StreamMeshletPageSource    Source  { get
;    } = new(    );
...
;
  ...
  {
                get => Visibility .  Effects; set  {
 Visibility.    Effects =
              value;
        Raster.   Effects =   value    ;  SoftwareRaster  .    Effects = value    ;
           Tiles.
                Effects = value;
Resolve.
           Effects   = value ;
...
Visibility.Pipelines;
    ...
    /// </remarks>
public EffectPipelineDescriber
      ? Modules {
               get =>    Raster. Modules   ; set => Raster. Modules =  value   ;
   }
   ...
   get => Resolve.ViewConstants; set => Resolve   .ViewConstants = value;
    ...
    /// <exception cref="ArgumentNullException"><paramref name="data" /> is null.</exception>
     public
   int  Content(int id    , ReadOnlySpan <    byte> hierarchy  ,   ReadOnlySpan<byte   > pages   , Stream data ) {
 ...
 return VirtualGeometryContent.Load (   Feature, Source, id, hierarchy , pages, data); }
    ...
    /// <returns>The index to put in <see cref="VirtualGeometryDraw.Mesh" />.</returns>
    /// <exception cref="ArgumentNullException"><paramref name="data" /> is null.</exception>
               public int   Content (   int    id    , VirtualGeometryAsset  asset, Stream
               data) {
...
disposed, this    )   ;
    ...
    builder.Clusters = Visibility; builder.    Pages  = Pages;
      builder .Raster = Raster;
         builder.
                SoftwareRaster = SoftwareRaster;
      builder.  Tiles = Tiles;
   ...
   public  void Dispose()  {
           if (disposed) { return;
               ...
               disposed
= true; 
        Resolve   .   Dispose   (    );    Tiles  .Dispose  () ;
      SoftwareRaster
        .Dispose();
			Raster   .Dispose();
         Visibility.  Dispose (    )    ;
      Pages.   Dispose();   Source   . Dispose  () ;
        }
               }
