...
/// </remarks>
		...
		object> Known  = new() {
            [typeof
        (bool)] =    new BoolCodec(),
	[typeof(    byte )  ]    = new ByteCodec  ()  , [ typeof(   int)]  = new Int32Codec   (   ),
        [ typeof(uint)] = new UInt32Codec (), [typeof   (float    )    ] = new SingleCodec()  ,
            [typeof(Vector3)]    = new  Vector3Codec   () , [typeof(  Quaternion  )] =    new RotationCodec()
    };
...
Register<T>   (    ISyncCodec<T> codec) { 
 ...
 .ThrowIfNull    (codec);
    ...
    public static ISyncCodec<    T    > For   
       <T>    () =>
  Known.   TryGetValue 
               (typeof(T), out var  codec )
 ? (ISyncCodec< T>)   codec
  : throw   new  NotSupportedException( 
                $"{typeof(T)} is not a type this wire knows. Register a codec for it with SyncCodecs.Register."
        )  ;
            sealed class BoolCodec : ISyncCodec<
bool > {
      static readonly WireLane[] Layout  = [new   ("", 1, false )];

        public ReadOnlySpan<WireLane   >
Lanes =>  Layout;
      
            public void    Write  (   ref    BitWriter writer    ,   in bool value ) =>
    ...
    public bool Read  (ref    BitReader reader   , out    bool value  ) => reader  .TryReadBool (out   value); }
        ...
        static readonly WireLane [
             ] Layout = [new (  "", 8   , false)]   ;
  
        public
    ReadOnlySpan<WireLane>    Lanes  => Layout    ;
              
        public void Write
               (  ref BitWriter writer ,  in byte value    ) => writer.    Write(value, 8   )   ;
        
public    bool Read(ref   BitReader reader   ,  out byte   value   ) {
            var read = reader.TryRead(8   , out var raw); value    =  (byte)raw    ;
              return    read
     ;
      }
    }
    ...
    sealed class Int32Codec : ISyncCodec< int> {   
        ...
        public ReadOnlySpan<WireLane > Lanes =>
            Layout;
          public void   Write(ref
              BitWriter writer,    in int value ) =>  writer    .WriteInt32(value);
            
    public bool Read(ref   BitReader reader
      , out int value )  => reader   .TryReadInt32    ( out value   );
			}
             sealed   class    UInt32Codec   :  ISyncCodec<uint> {   static readonly WireLane[] Layout    = [new("", 32    , true )];

public   ReadOnlySpan<WireLane>  Lanes
       =>    Layout;
            public void   Write(ref BitWriter writer, in uint
value    ) => writer.  WriteUInt32(  value    )   ;
                
        public bool  Read(ref BitReader    reader, out  uint  value  ) =>    reader.TryReadUInt32( out   value); }   
    sealed class SingleCodec    : 
       ISyncCodec  <  float  > {
        static readonly WireLane[
] Layout   = [new( "",    32, false)];
    public
ReadOnlySpan    <WireLane    >   Lanes =>    Layout;
        ...
        public bool   Read(ref
BitReader reader,   out float    value) => reader.TryReadSingle    (out value    )    ;
           }
    
    sealed class    Vector3Codec : ISyncCodec< Vector3  > {
     static
 readonly WireLane[] Layout  =   [new("X", 32    ,   false) , new("Y"    , 32, false), new("Z", 32,   false)  ]; 
           
                public ReadOnlySpan<WireLane> Lanes => Layout;  
            public   void Write  
(ref BitWriter writer, in  Vector3 value    ) =>  MathCodec.   WriteVector3(    ref writer , value);
    ...
    static   readonly WireLane
            [] Layout = [
			new(    "Dropped"    ,  2   , false ), new(   "A"   , MathCodec  .RotationBits,    true)    ,
 new("B" , MathCodec   .    RotationBits,   true),  new (   "C",    MathCodec  .RotationBits , true  )
      ];
       public    ReadOnlySpan
<WireLane> Lanes => Layout;
        
  public void Write(ref BitWriter writer,  in    Quaternion value    ) => MathCodec.WriteRotation   (ref   writer
                , value)  ;

	public bool Read  (  ref BitReader
        reader, out Quaternion value    ) => MathCodec.TryReadRotation(  ref reader   , out value);
    }
