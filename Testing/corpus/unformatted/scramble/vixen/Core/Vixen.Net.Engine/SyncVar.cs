...
public sealed class SyncVar<   T> : ISyncField {
              readonly ISyncCodec<  T>   codec  ; readonly IEqualityComparer  <T> comparer;
                T value;
...
public event  Action<   T, T
 ...
 set {
  ...
  Equals    (this    .value    , value))    {
    ...
    /// <exception cref="NotSupportedException">The wire does not know this type.</exception>
            public SyncVar    (  T initial =  default! , IEqualityComparer   < T  >?    comparer = null   ) { codec  =  SyncCodecs.For<T>(   );
 ...
 value = initial; 
    ...
    /// <inheritdoc />
      public void   Write(
               ref BitWriter writer) => codec.Write(ref writer, in  value);

    /// <inheritdoc />
    public bool Apply   (ref BitReader reader
     ) {
       if (    !  codec   .Read
  (    ref reader, out var arrived )) {
                return false;
        } 
                if
            (   comparer   .Equals    (value   , arrived  ))    {
         return   true;    }
...
Name = name; 
    ...
    public static implicit operator T(SyncVar<  T>   variable   ) { ArgumentNullException.  ThrowIfNull  (  variable);
