...
/// </remarks>
              sealed   class SparseMatrixBuilder  {
                readonly   List<  int    > rows = []    ; readonly List <int > columns = [];
     readonly List    < 
double    >   values =    [];
   ...
   public SparseMatrixBuilder   ( int rowCount, int columnCount ) { ArgumentOutOfRangeException    .ThrowIfNegative( rowCount  );
...
=  rowCount;
    ...
    /// </remarks>
         public void Add   (int   row, int column   , double value   ) { ArgumentOutOfRangeException.  ThrowIfNegative(row)  ;
        ArgumentOutOfRangeException.ThrowIfGreaterThanOrEqual (row , RowCount) ;  ArgumentOutOfRangeException.ThrowIfNegative(column)    ;
  ...
  values   .Add (value);
    ...
    var  count  =    rows  .Count;    var   starts = new int[RowCount +  1]; 
...
for (var triplet =    0; triplet <    count; triplet++    )   {
    starts[rows    [triplet] +  1]   ++;
       ...
       var cursor = new
   int[   RowCount ] ;
starts . AsSpan(0, RowCount)
  .   CopyTo(cursor    );
     
        var scatteredColumns = new int  [  count ]; var scatteredValues    = new double[   count ]    ;
      for ( var    triplet  =   0;  
         triplet <  count;  triplet++) {
              var    slot = cursor    [  rows   [triplet]  ]++ ; scatteredColumns[slot] =  columns[  triplet    ]   ;
...
;
          var
           mergedValues = new double[   count];
       var written = 0;
     for (var row    = 0  ; row < RowCount; row++) { var index = starts[row ];
    var
 end =   starts[row +  1];  
...
mergedStarts[   row + 1 ] = written;
    ...
    Array  .Resize(
ref    mergedColumns, written    );
          Array.    Resize(ref 
          mergedValues   , written) ;
			return new(
        RowCount, ColumnCount    , mergedStarts    , mergedColumns, mergedValues)    ;
         }

    /// <summary>A stable insertion sort by column, over one row's scattered entries.</summary>
              static void SortRow  (int    [   ]  columns, double[]    values, int first    , int  end)    { for (var index = first   + 1; index < end;    index++    ) {
      var column = columns [   index   ];    var    value = values[index]; 
