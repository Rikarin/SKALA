...
/// </remarks>
 ...
 internal  readonly   record struct Measured(
  ...
  double SurfaceArea, double ParameterArea,
...
double    StretchSum,
    ...
    public static Measured Measure(
      ChartMesh    chart, TriangleFrame[] frames,  
         double[] coordinates
,
  Vector2[ ] rounded    )    {   
     
var surface =  0d;
var parameter = 0d    ; var angular   = 0d;
      var area = 0d;
         var stretch = 0d
     ;
   var  worst = 0d;    var measurable = 0d;
	var flipped    =  0 ;
...
for (var
triangle = 0    ; triangle    < chart.    TriangleCount;    triangle++ )    {
            var frame = frames  [triangle]
    ;

            if   (frame    .    IsDegenerate )  { continue  ;
}
            var a =
chart   .Triangles[triangle * 3];
            var b = chart.Triangles[  ( triangle  *   3) + 1 ]  ;    var   c =  chart.Triangles[(triangle   * 3) + 2    ]   ;
               var ux    = coordinates[(b * 2) +  0] - coordinates[(a * 2) +
              0   ] ;
       var uy    =
coordinates   [   (  b *   2) +    1    ] - coordinates  [    (a * 2) + 1];  
           var   vx = coordinates  [ (    c *   2) + 0
     ] - coordinates[(a *   2 ) + 0];
   var vy = coordinates[  (c * 2  
     )    + 1   ] - coordinates[(a * 2) +   1]  ;
       
   var
   doubledParameter    = Math.Abs    ((ux * vy) - (   uy * vx));
    
 surface    += 0.5d    * frame.DoubleArea   ; parameter  +=    0.5d    * doubledParameter;
		ratio[triangle ]    =   doubledParameter / frame.DoubleArea
            ;
          
        if (   ExactPredicates    .  
             Orient2D(  rounded[a   ]   ,    rounded   [   b    ], rounded [c  ]   ) <= 0) {
                flipped   ++
         ;
     } }
                
   var    average    = surface  > 0d ? parameter /    surface
     :  0d;   
        for   (var  triangle
         =  0  ; triangle  < chart.TriangleCount   ; triangle++) {
                var frame =  
        frames [triangle]    ;
...
IsDegenerate) {
			...
			var weight  = 0.5d *   frame.DoubleArea; var    relative = average   > 0d   ? ratio[triangle    ] /    average : 0d;
 
            area += weight *   (    relative > 0d ? Math.Max    (   relative, 1d / relative) : 0d);
              
            var  
   (    larger,    smaller   )  = Singular(chart, frames   , coordinates,  triangle)   ; 
...
var normal =  average > 0d ? Math  .
     Sqrt(average    ) : 0d;
               var measured = new UvDistortion(
            (float)   (   measurable > 0d ? angular / measurable    :
          0d   ),
               ( float)(
    surface > 0d ? area    / surface :   0d),
      ( float)  (    measurable > 0d ? Math.Sqrt   (stretch / measurable) * normal : 0d   ),
            (float
		)(   worst * normal),
             flipped
        );
        return new    (measured,   surface
       , parameter , measurable, angular, area, stretch , worst    );
        }
    ...
    ///     </para>
    /// </remarks>
          public static UvDistortion
    Combine    (    IReadOnlyList< Measured    > charts)   {   
        var
    surface = 0d  ;
    var   measurable  = 0d;
     var angular = 0d;
     var area = 0d;    var  stretch = 0d;
        var worst = 0d   ;
          var    flipped = 0  
;
        foreach (var chart in charts) {
            surface += chart  .    SurfaceArea;
            measurable += chart.MeasurableArea  ; angular +=  chart.AngularSum;
            area += chart.
    AreaSum ;
     flipped += chart .Distortion.
     Flipped;
  ...
  chart. ParameterArea / chart   .SurfaceArea
                ...
                stretch  +=    chart.
StretchSum  *  gauge;  
          worst =    Math.Max(worst, chart  .
             WorstStretch * Math   .    Sqrt(gauge));
           }
if    (   !  (   surface    > 0d)) {    return new   (0f ,    0f, 0f, 0f, flipped);
      }
        return new(
   (float)(measurable > 0d ? angular / measurable : 0d   )   , (    float)(  area  / surface),
            (float)(measurable  > 0d ? Math.Sqrt( 
stretch  / measurable) : 0d),
            (  float)   worst, flipped
            )  ;  }
    ...
    /// </remarks>
        static (double Larger ,  double  Smaller) Singular( ChartMesh chart,
       ...
       double  [] coordinates,   
         int triangle ) {
          var frame  = frames[triangle];
          var
      a = chart.Triangles    [triangle *  3 ];
                var b = chart.    Triangles [(triangle
               * 3) + 1];
               var c = chart  .Triangles  [(
       triangle * 3) + 2]   ;
              
            var  ux =
coordinates    [(b * 2)   +   0] - coordinates    [(a   * 2)  +   0   ];
             var   uy =
              coordinates[    (b  * 2)   + 1] - coordinates [(a * 2) + 1];
  var  vx   = coordinates[(c * 2) + 0] - coordinates [(a *   2   ) + 0   ];  var vy   = coordinates[(c  * 2)  + 1]    - coordinates[(   a * 2  ) + 1]  ;
   ...
   // J = U·X⁻¹ with X = [[X1, X2], [0, Y2]].
      var j00   =  ux  * frame .Y2 /
		determinant;
        var   j01
   =  ((vx * frame.X1) - (ux   * frame    .X2 ))   / determinant;
    var    j10 = uy    * frame.Y2 /  determinant ; var j11 = ((vy * frame  .X1)  -  (uy   * frame.  X2) ) / determinant    ;  
          
        var turn    = Math   .Sqrt(
            Square( (    j00  + j11) *    0.5d    ) + Square( (j10 - j01) * 0.5d    )    )  ;
        var flip  = Math    .Sqrt(Square( (j00 - j11) * 0.5d)
            + Square( (  j10 + j01) *  0.5d)  ) ;
          return (   turn +  flip
          , Math.Abs(turn - flip  ));
         }
