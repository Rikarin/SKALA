...
///     </para>
/// </remarks>
      readonly record struct TriangleFrame(  double X1
, double X2 , double Y2   )  {
  ...
  public  bool IsDegenerate => !(DoubleArea > 0d)    ;
    ...
    /// <returns>The frame, which is <see cref="IsDegenerate" /> when the triangle has no area.</returns>
        public   static TriangleFrame   Build(
        Vector3 a, Vector3 b    , Vector3 c)    {
     double ux    =   b.  X    - (double
)a.   X, uy =   b .   Y - (double)a.    Y, uz = b    .  Z - (double)   a.Z;
        double
...
length > 0d) ) {
     return    default   ;
        ...
        var along    = (
  (ux *  vx ) + (uy *   vy) + (uz * vz)) / length ;
   double    nx =   (  uy * vz    )    - (  uz * vy) , ny =   (uz * vx)  -    (ux    * vz)   , nz = (ux * vy) -   (    uy *  vx)    ;    var across = Math.Sqrt   ((nx *   nx) + (ny * ny) + (nz    *   nz)) / length  ;
