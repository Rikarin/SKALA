...
/// </remarks>
 public readonly
            record struct   UvStackOffer(    int    Representative    , int Partner   , bool Mirrored, float Residual)  ; 
...
. ThrowIfNull(islands    );
        ...
        if   
             (tolerance < 0f ||   !   float.IsFinite   (tolerance)) {
               throw new ArgumentOutOfRangeException(    nameof(tolerance)  ,
           tolerance,   "The tolerance is a fraction of an island's extent, so it is non-negative and finite. "
   +    "Zero asks for docs/plan/41 § D11's exact mirror and nothing else."
           )  ;
            
           }
        var  taken =
new   bool[islands  .Count   ];
      var offers = new   List<UvStackOffer>();
          for (var first = 0
             ; first < islands.Count;  first++)   {
            if  ( taken [first]) { continue;
            }
    for (var   second = first +   1; second < islands.Count; second++) { if (taken[second  ]    || !Comparable(islands  [first ],   islands  [second]   )) {
               
          continue   ;
         }

var  straight = Residual(islands[first    ]   , islands[    second], false); var  mirrored =  Residual( islands [first    ],   islands    [second], true    )  ;   
              var mirror = mirrored  <   straight; var residual   =   mirror ?   mirrored : straight;
                if (!   (residual <= tolerance))  {   continue;
   }
              taken    [first    ]   = true ;  taken [ second]  = true   ;
 offers.Add( new(first
,    second,   mirror, residual));
...
public static IReadOnlyList<   UvIsland > Fold    (
  ...
  IReadOnlyList   <UvStackOffer> accepted, out int   []   source
  ) {
ArgumentNullException.  ThrowIfNull(islands  );
    ...
    ;
		...
		Array. Fill (    partnerOf,
-   1);
   foreach (var    offer in accepted) {
if (    offer   .Representative < 0 || offer
  .   Representative >=   islands   .  Count
              ||    offer.Partner <   0 || offer.Partner   >=  islands.Count) { throw new ArgumentException(
  ...
  nameof(accepted  )
           ...
           if    (   offer   .Representative == offer  .Partner ||   
       partnerOf  [  offer   .Partner  ]   >=   0) {
...
nameof   (accepted)
    ...
    // A representative that is itself somebody's partner would fold into a fold, so the two passes
     ...
     // are separated: the first records the pairing, the second rejects a chain.
        foreach  (var    offer in accepted   ) {
               if (partnerOf[offer.Representative] >= 0)    { throw    new  ArgumentException(
                    $"Island {offer.Representative} is both a representative and a partner.",
		nameof(accepted)
               );
           }
        }
            var folded = new List<  UvIsland>(islands  .
  Count  );
      var slot =
...
continue;
    ...
    folded.Add    (islands  [index]    ); }
          ...
          for   (
          var index   = 0    ;   index <    islands .  Count;   index    ++) {
...
return folded  ; }
    ...
    /// </remarks>
         public static
 IReadOnlyList  <UvPlacement   >   Unfold(IReadOnlyList<    UvPlacement> packed,  IReadOnlyList< int> source ) {
ArgumentNullException    .ThrowIfNull   (  packed   );
			ArgumentNullException    .ThrowIfNull (
source)   ;
    var byIsland   = new UvPlacement?
                [packed.    Count]  ;
            foreach (var   placement in    packed  ) { if (    placement  . Island  < 0 || placement .Island >=  packed.  Count    ) {
 throw new ArgumentException(
 ...
 for ( var
           index = 0; index < source.Count; index++)   {
   var folded
= source[ index];
    if (
	folded < 0   || folded >= byIsland.Length   || byIsland[folded] is   not { } placement)   {  
       throw new   ArgumentException( 
...
if (
  first.Coordinates is null   || second .  Coordinates is    null || first .Corners is null
                || second .Corners    is   null)  {    return  false  ;
         }
          if (first.   Coordinates.    Count != second.Coordinates.  Count    ||    first.    Coordinates.Count ==    0) { return false;
...
second. Size;   
    var
extent =    MathF .Max   (   MathF .Max(one.X, one    .Y)   , MathF.Max (two  .  X    , two.Y))    ;
   
              if
             (!(    extent > 0f ) || !    float.IsFinite(extent))   {
    ...
    /// </remarks>
 static float Residual( UvIsland first    , UvIsland
 second   ,   bool mirrored    ) {
       var  size = first   . Size
              ;  
var extent = MathF   .
     Max(size.   X, size.Y);
...
for (var corner = 0   ;    corner    < first.Coordinates.  Count;  
corner ++) {
       var  here    =
             first  .  Coordinates[    corner] -   first.Minimum   ;
    
var there = second.Coordinates[corner] - second.Minimum;
               
               if (mirrored) {
              there = new(second .Size.   X - there .X
            , there.Y);
             }
   worst = MathF.Max(    worst,    Vector2
     . Distance (   here    ,   there   )   / extent    );  
        }
          return  worst;
     }   }
