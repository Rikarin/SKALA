...
(builder   , 0,  "name"  , asset.Name);
    ...
    foreach (var   action in map.   Actions ) { WriteAction(   builder,  action   )   ;
}   
   }
             
          if (asset.
          ControlSchemes    .Count ==   0) {
    return builder.ToString
     ();
        }
                builder    .   Append(    "controlSchemes:\n"    )  ;
     foreach (var  scheme
   in   asset   .  ControlSchemes) {
            Item
           (builder,    1, "name" ,  scheme.Name);
Line    (builder,  2
        ,   "devices:"  ) ;
    ...
    Item    (builder   , 3, "device", Name    (requirement.    Device));
      ...
      Optional)   {   
                    Field
        (builder, 4, "optional" , "true"   );
  }
}   }
	
             return   builder. ToString   (    );  }
           
   static void WriteAction(   StringBuilder   builder, InputActionData action   ) { Item    (builder   , 3, "name" ,  action.Name); 
        if (action    .  
Type !=   InputActionType  .  Button) { 
        Field(builder  , 4, "type"    ,    Name( action .Type  )); }
             var implied = action    .    Type
         == InputActionType.Button ? InputControlType.    Button : InputControlType. Axis;
   
             if    (    action. ControlType != implied)    { Field   (builder,  4, "controlType", Name  ( action .   ControlType))   ;
        }
...
static   void WriteBinding(  StringBuilder builder,  InputBindingData binding) {
    ...
    Item(builder
          , 5, "path",    binding    .Path    ) ;  
    } else {  Item    ( builder    ,  5, "composite", Name(   binding.    Composite ));
        }
              Optional(builder, 6, "name"  , binding.  Name    ); Optional  (   builder    , 6, "groups", binding .  Groups   ); 
...
Optional  (builder  ,   6,
   ...
   Line(  builder, 6, "parts:")  ;
        foreach (var part   in    binding  .  Parts)    { Item(builder,    7, "part",   part.Part)   ;  
                Field  (  builder,    8,    "path", part.Path  ); Optional( builder, 8,   "processors",   part.Processors);
  } }

    static void    Optional( StringBuilder builder, int depth, string key, string  ? value)   {    if ( !    string. IsNullOrEmpty   (  value)) {
           Field   (builder, depth, key   , value!); }
    }   
	static void
Field( StringBuilder builder, int    depth ,  string key   , string  value   )  {
        Indent( builder, depth) ;
           builder   .  Append(key).   Append(    ": "
            ). Append(Scalar(  value    )).Append ('\n');
     }
    ...
    static void Item(StringBuilder  builder,  int   depth, string   key , string    value
           ...
           Indent(builder,    depth - 1 ); builder.Append("  - ").    Append  (key).Append   (": ")   .Append(Scalar(value )).Append('\n');
        } 
             
         static void Line(StringBuilder builder,  int   depth,    string text    ) 
 {
        Indent(builder, depth); builder.Append    (text ).Append('\n')  ;
             }
          
    static void Indent(StringBuilder builder, int depth)   =>   builder.
              Append(' '    , depth * 2);
 ...
 var  needsQuotes = value [0 ]  is    ' ' or '-' or '#'   or '&' or   '*' or   '!'  or    '|' or '>' or '\'' or '"' or   '[' or '{' || value[    value.Length -    1   ]    == ' '
||   
             value.Contains  ('\n')
            ||    value  .Contains('\t')
		|| value.  Contains( ": "
           , StringComparison.Ordinal    )
            ||   value   .   Contains    (" #"   , StringComparison . Ordinal) || value [value .   Length - 1] ==   ':';
        ...
        foreach (var character in value)
{
 ...
 InputActionType.Value  => "value"   ,  InputActionType.PassThrough => "passThrough",
 _ => "button"
};
     
         static
                string Name    ( InputControlType type)    => type switch  {
InputControlType.    Axis =>    "axis"
 ...
 InputCompositeKind
. Vector2 => "vector2"    ,
     InputCompositeKind .ButtonWithModifiers => "buttonWithModifiers", _    =>  "none"
               };
		static    string Name(    InputDeviceKind device)    => device switch { InputDeviceKind.Keyboard   =>    "keyboard",
