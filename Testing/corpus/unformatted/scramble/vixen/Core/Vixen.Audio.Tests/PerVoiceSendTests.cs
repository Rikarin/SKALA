...
device, aux  );
  Assert.True
(   direct >  0.5f, $"the dry path was only {direct:F3}");
...
using  (engine  ) { var dry = engine .CreateBus  ("Dry")  ;
  ...
  var handle = engine   . Play(AudioTestData  .Constant(   48_000  , 1f), new    PlaybackSettings {
                Bus =  dry.Index
           ,    SendBus  = aux.Index, SendLevel  =  0.2f
          } );  
 ...
 engine.Update( 0f  )
      ;
            ...
            Assert.True
(engine.    SetSend  (handle, aux.Index, 0.9f )  )  ;
          Assert.Equal   (
0.9f,   engine.  SendLevelOf(    handle) );
		var after = PeakOf( device,
          ...
          Assert  . True  
(before  >    0f   , "nothing reached the aux to begin with")  ;
...
aux = engine  .CreateBus("Aux"    );
  ...
  engine   .Update(  0f    )  ;
            ...
            Assert    .    True  
                (PeakOf    (  device,    dry) > 0.5f, "and the dry path still plays") ;
