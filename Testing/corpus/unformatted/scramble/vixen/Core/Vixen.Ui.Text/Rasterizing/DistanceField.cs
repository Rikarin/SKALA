...
using  System   .Numerics; using Vixen.Ui    .Text.Outlines;
...
/// <param name="Channels">Row-major, three floats per pixel, each in <c>[0, 1]</c> with 0.5 on the edge.</param>
  public readonly record struct  DistanceFieldBitmap(  int Width, int   Height , float Range
, float[]  Channels) {
    /// <summary>The three channels at a pixel.</summary>
    /// <param name="x">Column.</param>
    /// <param name="y">Row.</param>
public (float R, float G  ,
         float B) this [int   x, int   y] {
   get  {
            var at = ( (y * Width   ) +   x) * 3; return  (    Channels[    at  ], Channels    [at + 1], Channels   [at +  2    ])  ;
                } }
    ...
    public   float  Median(int x    , int y  )  { var  (r ,    g, b   ) =   this[ x  , y]    ;
...
GlyphOutline outline ,
   ...
   ArgumentNullException.
     ...
     ArgumentOutOfRangeException.ThrowIfNegativeOrZero(width) ; ArgumentOutOfRangeException.    ThrowIfNegativeOrZero   (height)    ;
        ArgumentOutOfRangeException.
               ThrowIfNegativeOrZero(scale);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero    (range   )  ;
                
                
		var channels =  new  float[width  * height * 3];
        if (outline  .  IsEmpty  ) {
            Array .Fill(channels, 0f);  return new DistanceFieldBitmap(width  , height   , range  , channels )   ;
         }
  ...
  // Flattened finely relative to a pixel, because the field's own resolution is what decides
    ...
    var    edges = EdgeColoring.Colour(   outline,
OutlineFlattener.Flatten(outline  , 0.05f / scale )    );
        var inside =
...
var winding  = Winding(edges);
   ...
   for (var x  = 0; x < width;    x++) {
        ...
        var point  = new Vector2(
			(   (x + 0.5f )  / scale) + origin.X , (  (  height   - 1 -  y   +   0.5f)  /  scale  ) + origin.    Y
                )   ; 
     ...
     // detail.</b> Taking one sign from the fill and applying it to all three makes the
       ...
       var redDistance = Nearest(red, point
, winding  );
...
;
  ...
  if (inside [x  , y] >= 0.5f != Median(redDistance
  ,   greenDistance, blueDistance) >= 0)    {
   ...
   channels [
        at + 2] =   Encode(blueDistance, scale, range); 
    }
            }
    ...
    static float
 Encode(float distance , float scale    , float   range) =>
    Math    .    Clamp(
(distance * scale  / range)    + 0.5f , 0f    ,   1f) ;
       
 static float Median
(float a    , float   b,    float c ) => Math    .Max(Math   .Min    (a, b), Math.Min(  Math   .    Max(a, b),  c)    );

    /// <summary>Which way round the outline runs, as +1 or −1.</summary>
  static float Winding (List    <ColouredEdge> edges
           ) {   
           var area =   0f;   foreach   (var   edge in edges   ) {
      area += Cross(
edge    .From,  edge    . To)   ; 
	}
...
0    ?    1f : -1f   ;
    ...
    /// </remarks>
              static float
    Nearest(   Prepared[] edges   , Vector2 point, float winding) {
...
// whole extent lies further away than the best so far cannot improve it and cannot reach
      ...
      var reach = best +  
          edge   . HalfLength;
			if (toMiddle .LengthSquared(    ) >    reach *  reach) { continue   ;

            }
               var t = Math   . Clamp  
            (  Vector2.Dot(point  -  edge  .From,    edge.Direction) *   edge  .  InverseLengthSquared, 0f, 1f);
...
var closest  =   t    >= 1f ?   edge. To :
edge.From + (t *  edge.Direction);
...
// too, and is 6% <i>slower</i> than the load it replaces — the branch predicts.
 var alongside
    = t > 0f &&    t < 1f
       ? 0f   : MathF.Abs (Vector2 .    Dot(edge.Direction, offset)    ) * edge    .InverseLength;

     // Tied on distance, so the corner rule decides.
     if (  distanceSquared >= nearer && alongside >= bestAlongside) { continue;
            }
        ...
        ;
           further   = distanceSquared *  (1f + Tie ); nearer    =  distanceSquared * (   1f - Tie);
    ...
    /// <summary>The edges carrying one channel, with everything pixel-independent worked out.</summary>
       static Prepared[]  Prepare (   List<ColouredEdge    > edges  , EdgeChannels channel) {    var   prepared   = new  List    <Prepared>(edges.Count);
       foreach    (var edge in edges) { if ( (edge.Channels &  channel) == 0) {
              continue; }
...
var length =   MathF
       ...
       prepared   .Add  (
           new Prepared(    edge.From    ,
	edge .    To, direction,
1f
         /  lengthSquared,
       1f  / length, edge  .From  + (direction * 0.5f  )   ,
            length * 0.5f
        ) );
        }
           return [.. prepared  ]  ;
    ...
    readonly record struct Prepared  ( Vector2 From,
           Vector2  To,
     Vector2 Direction, float InverseLengthSquared ,
		float InverseLength,
          Vector2   Middle,
  float HalfLength
    )  ;
