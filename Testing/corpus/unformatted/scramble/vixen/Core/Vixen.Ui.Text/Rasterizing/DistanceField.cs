...
namespace Vixen.Ui.Text.
    ...
    public static class DistanceField {
        ...
        static float
            Encode(float distance, float scale, float range) => Math.Clamp((distance * scale / range) + 0.5f, 0f, 1f);
        ...
        static float Winding(
            List<ColouredEdge> edges
        ) {
            var area = 0f;
