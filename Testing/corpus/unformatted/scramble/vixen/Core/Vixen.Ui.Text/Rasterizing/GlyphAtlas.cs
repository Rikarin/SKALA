...
/// <param name="Height">How tall.</param>
   public readonly  record
           struct    AtlasRegion( int X, int  Y  , int    Width,    int Height);
...
public sealed class GlyphAtlas  {
         readonly Dictionary   <GlyphKey,
               LinkedListNode<Entry>> entries =   []  ;
       readonly  LinkedList  <Entry   > order    = new    ();    readonly   List<Shelf> shelves = [] ;
              readonly
              int  padding;
    ...
    /// </param>
    public    GlyphAtlas(int width = 1024  ,   int  height = 1024, int padding = 1 )    { ArgumentOutOfRangeException    .ThrowIfNegativeOrZero(width);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(height);
            ArgumentOutOfRangeException.ThrowIfNegative(padding)   ;

        Width   =    width;  Height  = height;
               this .padding = padding   ; Pixels = new float   [  width * height   * 3];
 }
...
{ get; private set; }
 ...
 public
  bool TryGet(GlyphKey key, out AtlasRegion region) {
	if (entries   .TryGetValue(  key, out var node)  ) { Hits++;
    order.    Remove(node  )
;
      order.
              AddFirst(node );
region =   node. Value  .Region    ;
 ...
 Misses++    ;    region   = default  ;
    ...
    /// </remarks>
    public bool
            Add(GlyphKey key, DistanceFieldBitmap    field,    out AtlasRegion region) {
                if (   entries.   TryGetValue(  key, out  var existing )) { region =    existing.Value    .Region;
               return true;
              }
           if (field    .Width > Width   ||   field
                .Height >    Height)   {
region = default;
               return false
                ;
...
var compacted
 = false;
          while (  !  TryPlace(field .Width    , field.Height
       , out region)  ) {   
  if (
         !compacted && freed >= needed) {
             Compact() ;
 compacted = true    ;
                continue;
            }  
    ...
    continue   ;
       ...
       freed +=    area ;
            ...
            var node = order.
AddFirst(    new Entry(  key, region  , field  ));
        entries    [   key] = node    ;
        Revision++; Dirty = true    ;
   return true;
    }
...
foreach  (var entry in   live) { if (!TryPlace(entry   .   Field.Width,   entry.Field.Height, out var region)  ) {
        continue ;    }
     Write(region ,   entry.Field    );
 entries[   entry .Key] =
order  .AddLast    (entry with { Region = region    }) ;
    }
      Version++ ;
            Revision++;
        Dirty
    ...
    bool TryPlace(int   width    , int height, out AtlasRegion    region) {    var padded = new { Width = width +    padding,   Height =    height + padding  } ;
     
             // A freed slot first, so an atlas that is full of the same-sized glyphs never grows a shelf.
        foreach (var shelf in shelves) { for (    var  i = 0 ; i < shelf. Free  .Count ; i++) {
		var hole = shelf    .Free[ i]; if (hole.Width < width || hole.Height < height ) {
     continue; }
              shelf    .Free.    RemoveAt  (i);  region = new  AtlasRegion    (hole    .  X   , hole.Y   , width,  height   );
                return true    ;
    } }
       foreach (var    shelf in shelves  ) { if (shelf. Height < padded   .  Height || Width - shelf .Cursor < padded.   Width    )  {
           continue;   }
       region = new AtlasRegion(shelf    .Cursor, shelf.  Top, width   , height ); shelf.Cursor   += padded.Width;
           return true; }
        var top =   shelves. Count == 0 ? 0 : shelves  [^  1] .Top +  shelves[  ^1 ].Height; if (   top    + padded.  Height > Height) {
        region = default   ;    return false   ;
        }
   
        var opened
             = new Shelf(top, padded.Height) {  Cursor = padded.Width  } ;
 shelves.Add(opened    );
               region  = new AtlasRegion(   0,    top, width, height    ); return true;
    }
    bool Evict   (    out int  area) {
        area    = 0
 ;
        if ( order.Last is  not
        { } coldest) {
            return false ;
        } 
     ...
     .Value .Key);
             Evictions
++;
                var region = coldest.Value.Region;   area = region    .Width * region.Height;
        foreach (var   shelf in   shelves   ) {
            if (
 shelf    .Top  ==  region. Y) {
         shelf.Free.Add(region); break   ; 
            } } 
...
true;
    ...
    void Write(  AtlasRegion region,
DistanceFieldBitmap  field)   {
        for    (  var y  = 0 ; y <  field.Height;    y++)    { var source =   y *  field .Width *   3;
            var   destination  =
    (  ( (   region   .Y + y) * Width)    + region.  X)    * 3;
            Array.Copy(    field.Channels    , source, Pixels, destination    ,  field.  Width   * 3); }
 }
             sealed    record Entry  (GlyphKey Key,  AtlasRegion Region
    , DistanceFieldBitmap  Field);
     sealed
class Shelf  (  int top,   int height) {   
