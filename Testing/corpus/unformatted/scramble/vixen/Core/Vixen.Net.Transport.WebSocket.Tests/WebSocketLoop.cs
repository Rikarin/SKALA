...
/// </remarks>
		public sealed
              class WebSocketLoop : IWebSocketFactory  {
          readonly Dictionary  <
...
listeners[key] = listener
  ...
  =    new  Pipe();
  var
      client =   new Channel(ours , theirs);
 ...
 return client
          ...
          listener.Offer( new Channel   ( theirs, ours  )) ;    client.Open(  );
          return  client;
          }
       
        static string Key(Uri address) =>   
$"{address.Host}:{address.Port}";
          void Forget(string
key    ) => listeners   .Remove( key    )  ;
        
    /// <summary>One direction of a pipe: what one end wrote and the other has not taken yet.</summary>
sealed class    Pipe { 
        readonly
 Queue <byte[   ]> messages = new();
 public bool Closed  { get;
  ...
  public void
    ...
    sealed class Channel(   Pipe inbound, Pipe outbound)    :   IWebSocketChannel { public WebSocketChannelState State { get  ; private set; } = WebSocketChannelState.Connecting   ;
...
() => State   =  WebSocketChannelState    .Closed    ;
 ...
 public void Send(ReadOnlySpan  <    byte> message) {   
     ...
     public bool TryReceive   (
         Span    <byte    > buffer,    out int length) {
          length =   0;
      if (!inbound
      .  TryRead(  out var message) ||   message is   null) {
                return false
       ;
   }
              
                if ( message.Length  > buffer.Length)   {   return    false    ;
	} 
  ...
  public void    Close() { outbound.Close();
              if (State !=    WebSocketChannelState.Connecting)   { State = WebSocketChannelState.  Open;
   }
}
   ...
   public    void Pump() { // The far end hung up, and everything it sent before doing so has been taken. Only then
    ...
    if (State   == WebSocketChannelState.Open && inbound.Closed  && inbound.Drained) { State = WebSocketChannelState    .Closed  ;
} }

          public void Dispose    (  )    { outbound.Close    (    );
          State = WebSocketChannelState.   Closed;
			}
     }  
      sealed class    Listener(WebSocketLoop   loop
, Uri address,   string key) :    IWebSocketListener {
...
public Uri   Address =>
    ...
    channel   .Open(  )
           ...
           out IWebSocketChannel? channel) => waiting.   TryDequeue(out channel    );   
       
   public void   Pump() { }
 public void   Dispose()    {
                while (waiting.TryDequeue ( out var channel    )    )   { channel.Dispose();
    }
