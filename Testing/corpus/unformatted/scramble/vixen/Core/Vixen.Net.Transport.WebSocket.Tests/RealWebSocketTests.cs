   ...
   using System.
      Text;
        using  Vixen.   Net .Tests.Transport  ;    using Xunit;  
		namespace Vixen.  Net
...
/// </remarks>
  public sealed class RealWebSocketTests {
    ...
    [Fact    ]
        ...
        using
   var client = new WebSocketTransport(
         factory,
         new (  ) {  RemoteAddress = listening, ConnectTimeout = TimeSpan    .FromSeconds(
10) }
    ) ;
       client    .StartClient
( );
    ...
    return clientEvents.Connects(    TransportRole.Client).   Count ==   1
        ...
        Assert.  Equal( serverEvents.    Connects( TransportRole.Server)[0]   , clientEvents .Connects (TransportRole.    Client)   [0])   ;
            ...
            client. SendToServer    (Encoding.UTF8
              .    GetBytes( "over a real websocket" )   , Channel.   Reliable)   ;
 ...
 Assert    .True( Until    (
  ...
  => {
      ...
      server    .Poll(    Step,
		serverEvents);
           return
              serverEvents.Texts    (TransportRole.Server).    Count == 1;
        }
                ),
     "The payload did not arrive over loopback."
   ) ;
            Assert  .Equal (["over a real websocket"], serverEvents.Texts   (TransportRole
            .Server  ));
                }
