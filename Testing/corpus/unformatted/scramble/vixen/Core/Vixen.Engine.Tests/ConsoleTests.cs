...
using Vixen.Core ;   using Vixen    .Core.Mathematics;
...
/// <summary>The console: parsing, running, history and completion, none of which needs a screen.</summary>
         public sealed class ConsoleTests {
			[  Fact]
  public void ACommandRunsWithItsArguments()   { var   commands = new ConsoleCommands(  )   ;
    var   seen = string  .    Empty
          ;
        commands.Register( "teleport", "Moves something"    , entry => seen  = string.Join(    '|',    entry.Arguments)   );
        Assert.
              True(commands.    Execute("teleport 1 2 3"))  ;
    ...
    /// <summary>A quoted run is one argument, so <c>echo "hello world"</c> says what it looks like.</summary>
      [Fact  ]
        ...
        var commands  = new ConsoleCommands   (); var count   =    0;
   commands.    Register(    "say", "", entry => count = entry
.  Count);
...
public void   ACommandThatThrowsIsReportedRatherThanPropagated() {
        ...
        commands.
     Register   ("boom"  , "", static   _    => throw new InvalidOperationException("bang"));
       
Assert .False(commands
.Execute(    "boom")    );
                Assert   .   Contains(  
        commands.Output,  line => line .  Contains("bang"   , StringComparison. Ordinal)  );
      }
 
      [Fact] public   void FlagsAcceptTheWordsPeopleActuallyType() {
                var commands = new ConsoleCommands(); var    results   = new List< bool>    ( );
...
results.Add(value);
          ...
          foreach (var word in (string[])  ["on"
      ,  "off"   , "1"    ,  "0", "true", "false", "yes" , "no"   ]  ) {
            commands    . Execute (   $"set {word}"
		)   ;
        }
        Assert.    Equal(    [true,   false, true, false, true   , false , true,   false], results ); }
            
    [Fact] public void HistoryDoesNotRecordARepeat( )   {
    var commands = new    ConsoleCommands(    );
   
             commands .Execute    (
 "help");
     commands. Execute ("help") ;   commands.Execute("clear")   ;
  Assert   .Equal(["help",    "clear"], commands.History ); }
                
        [ Fact] public   void RecallWalksBackAndForwardsThroughTheHistory() {
        var commands    = new ConsoleCommands();
  ...
  :    true )   ;
      ...
      "clear", console.Input);  
        ...
        console.Recall(    back: false  ); Assert.Equal ( "clear" , console. Input);
     ...
     [Fact    ] public void CompletingOneMatchFinishesTheWordAndAddsASpace(  ) { 
       var commands
 =    new ConsoleCommands();
  
      commands    .Register  ("teleport"   , "",  static _ =>   { });
   var console    = new  ConsoleOverlay (
            commands);
        console  .Type("tele" );
             Assert
                .True(console    .    Complete()    );
Assert
.Equal    ("teleport "   , console.   Input);  
             }
    [Fact]
public void CompletingSeveralMatchesTakesTheCommonPrefixAndListsThem(    )   {
    ...
    new ConsoleCommands   ();
        commands   .
     Register    ("quality",    "", static   _ =>  { }  );
	commands   .
      Register   ("quantise", "", static _ => {   });
   ...
   Assert   .Equal("qua"    ,    console.Input)  ; Assert.Contains  (commands    .  Output  , line => line.  Contains    ( "quality"  , StringComparison.Ordinal) )    ;
    }
...
[    Fact  ]
          public void TheOverlayCommandTogglesAnOverlay  (  ) { var  overlays = new DiagnosticOverlays( );
  var   
...
Assert.False   (stats .Enabled)
  ...
  Output  ,    line => line .Contains    ("stats"    ,   StringComparison.Ordinal)   );
     } 
    [  Fact] 
       public void TheConsoleDrawsItsPromptAndOutput() {
    var commands = new ConsoleCommands   ( )  ; commands.    Execute(   "help");
  ...
  var draw = new DebugDraw()
              ;
        overlays.   Draw(   draw, new Vector2(    1280f
           , 720f), GameTime.Zero);
    ...
    /// </summary>
  [Fact]
             public   void NothingIsDrawnPastThePanelsEdge    () { var  commands =   new ConsoleCommands() ;  
commands.   Register ( "say"
, "", static entry => entry   .Write(new string   ( 'x' , 400  ) )   )    ;  
   commands. Execute("say")   ;
              
             var  console =  new ConsoleOverlay    (commands) {
Enabled = true , Width    = 300f };
           console .Type
       (  new string    ( 'y', 400));
  ...
  var draw    = new DebugDraw   ()  ;  overlays.Draw(draw   , new Vector2(1280f, 720f    ),  GameTime.Zero    )  ;  
             var right    = 1280f - overlays   .Theme  .   Margin;  
   
       foreach (var  line in draw.ScreenLines) {
Assert.True(
line.From.  X <= right, $"a stroke reaches x = {line.From.X}, past the panel at {right}" );
              Assert. True  
(line.   To    .X <= right    ,   $"a stroke reaches x = {line.To.X}, past the panel at {right}"    );
        }
          }
            
    /// <summary>The tail of a long line is shown, because that is where the caret is.</summary>
               [Fact]  public void ALongTypedLineShowsItsEnd  (   ) {
        var 
...
overlays    = new DiagnosticOverlays   ();
      overlays.Add(   console  )    ; overlays.  Draw(   draw, new Vector2(1280f   , 720f   ),   GameTime.Zero)  ;
 ...
 // and not the line, so submitting still runs what was typed.
    Assert.   Equal(
212,   console  .  Input.   Length);
        Assert.EndsWith  ("END",
   console.Input    ,  StringComparison.Ordinal);
}
             }  
