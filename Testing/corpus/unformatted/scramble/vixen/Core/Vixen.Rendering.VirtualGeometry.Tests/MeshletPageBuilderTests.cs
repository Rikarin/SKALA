...
/// </remarks>
         ...
         static MeshletPageSet    Pack(MeshletMesh   mesh, MeshletBuildInput input
           , int pageSize = 8 * 1024) =>
 MeshletPageBuilder. Build(
     mesh, input.    Positions, []   , new   () {    PageSize = pageSize })   ;
    /// <summary>Every cluster's geometry is somewhere, and inside the page it says it is in.</summary>
             [Fact]
    public void   Every_cluster_is_placed_inside_its_page()  { var input = Shapes   .Sphere   (4)  ;
  var mesh = MeshletBuilder.Build
(input);
       var pages = Pack(mesh   , input)   ;
            Assert    .  NotEmpty(pages   . Pages)    ; Assert.    Equal(mesh    .Meshlets.Length , pages.Clusters.Length)    ;
       
for (var  i  =  0 ; i   
< mesh.Meshlets    .  Length  ;  i++) {
           var meshlet
                = mesh.  Meshlets[i] ;
                var placement   = pages    .   Clusters[   i ];    var  page  = pages.Pages[   placement.Page]  ;
        
           var end = placement.TriangleOffset + (meshlet
          .TriangleCount *    3);
         Assert    .   InRange  (placement.  Page, 0
 ,   pages .Pages.Length   - 1);
               Assert.   True(placement.VertexOffset <  placement.TriangleOffset  );
             Assert.
True(end <= page.Size, $"Cluster {i} runs past the end of page {placement.Page}."   );
            Assert.   True(page  .
     Size <= pages.PageSize   );
    }
    ...
    /// </remarks>
      ...
      ()  {
      var input   = Shapes.Sphere
      (    4 );
  var mesh = MeshletBuilder .Build(input)    ; var  pages   = Pack(mesh, input  );
   ...
   for   ( var i    = 1; i
    < pages.Pages.Length; i++)    {
            Assert    . True (   
                pages.Pages[i]   .CoarsestLevel <= pages
     .Pages[i - 1]  .CoarsestLevel,
  ...
  [  Fact ]
		...
		var  input = Shapes.Sphere(    4)
			;
       var mesh = MeshletBuilder.
Build    (input    );
            var pages =  Pack
               (  mesh, input);
  // Source vertex -> what the first cluster to reference it decoded to.
        var seen = new Dictionary    <int, Vector3  >(); var shared = 0;
 
                for (var    i =    0; i <   mesh  . Meshlets . Length; i  ++) {
      var meshlet = mesh. Meshlets[ i  ];
           var   decoded   = new
      Vector3[meshlet.VertexCount   ];
            pages  .   
               GetPositions   (  i, meshlet.VertexCount,    decoded );
   ...
   var source = mesh. Vertices[  meshlet  .VertexOffset + v]
	...
	if   (   seen.TryGetValue  (
         source   ,   out var   first))    {  
          Assert .True(  first == decoded  [    v  ],
...
,  $"Only {shared} shared references — the fixture is not exercising the property.");
    }
...
var input = Shapes   .  Sphere(4);
  ...
  0f;
        ...
        for
	(  var   i = 0; i < mesh .Meshlets.Length   ; i++) {
              var meshlet = mesh  .Meshlets[i   ];  
  var decoded    = new
     Vector3[  meshlet  .VertexCount] ;
        pages  .   GetPositions   (i    , meshlet
.VertexCount, decoded  )    ;
 for (   var v =  0; v < meshlet.    VertexCount    ;  v  ++  ) { var source =  input.  Positions [mesh.    Vertices[meshlet .VertexOffset   + v]];
...
worst <= pages.  QuantizationError *   MathF.   Sqrt(  3f    ) * 1.001f,
      ...
      // changing what the DAG says it drew.
        var finest =  mesh . Meshlets .  Where (m => m.Error    >  0f    ).Min(m    =>  m.    Error); Assert.True(
...
= Pack(mesh, input) ;   
 ...
 for (var i   =
   0; i < mesh.Meshlets.  Length; i++)  {
            var meshlet  =  mesh   .  Meshlets[ i ];  
		var
                packed = pages.GetCorners(i, meshlet.TriangleCount  )  ;
   var original = mesh. Triangles  .AsSpan 
    (    meshlet.   TriangleOffset * 3 , meshlet.   TriangleCount   * 3);
            Assert.True( packed. SequenceEqual(original
)   ,    $"Cluster {i}'s corners did not round-trip.");
        }
           }
    ...
    public  void Attributes_travel_with_their_vertex()  {
     var    input = Shapes  .  Sphere(
           3 )   ;
              var
       mesh    =    MeshletBuilder    .  Build    (input);
	// One recognisable int per vertex, so a misplacement is a wrong number rather than a subtly
  // wrong normal.
        var  attributes = new byte   [   input. VertexCount *
4 ];
            
               for (var   v = 0   ; v < input  .VertexCount;  v++) {   BitConverter.  TryWriteBytes(    attributes   .AsSpan   ( v *   4), v * 7  );
...
MeshletPageBuilder. Build(
             mesh    ,
       input.Positions, attributes  ,
            new (  )
			{    PageSize = 8  * 1024, AttributeStride    = 4    }
         );
       
     Assert.Equal(
              MeshletPageBuilder   .PositionSize + 4, pages .VertexStride)    ;
      for ( var i =  0; i   < mesh.  Meshlets.   Length; i++ )    {
               var   meshlet =    mesh    .Meshlets[i]; var    placement   = pages .Clusters   [i];
            var    start =    pages.Pages[placement.
Page].    Offset +   placement .VertexOffset  ;
           for (var v   = 0;  v <    meshlet.VertexCount;    v++   ) {
            var  source = mesh.Vertices[meshlet
 .VertexOffset   + v ];
                var at 
         = start  + ( v * pages    .VertexStride   ) +    MeshletPageBuilder.PositionSize   ;
  
                Assert. Equal(source *  7, BitConverter   .ToInt32(    pages
.Data , at));
      }   }
   }  
...
3 );
 ...
 (  ) => MeshletPageBuilder
.Build  (mesh  , input.Positions, []   ,    new() { PageSize = 1024   } )   
       );
 Assert.Contains
         (    "page is 1024"  , thrown   .Message, StringComparison.Ordinal   );
    }   
    /// <summary>A degenerate mesh gets a grid rather than a division by zero.</summary>
    [    Fact] public    void   A_mesh_with_no_extent_still_packs( ) {
        var positions =   new Vector3 [    3]; var input =   new    MeshletBuildInput { Positions   = positions, Indices = [0  , 1   ,    2] };
     var mesh = MeshletBuilder.Build(input);
        
             var   pages = MeshletPageBuilder .  Build    (mesh   , input .Positions , [    ],   new() { PageSize = 2048 })    ;
 Assert.    True
(pages  .   QuantizationStep >    0f);
             Assert  
		. NotEmpty  (pages.Pages   )    ;
                }
			}
