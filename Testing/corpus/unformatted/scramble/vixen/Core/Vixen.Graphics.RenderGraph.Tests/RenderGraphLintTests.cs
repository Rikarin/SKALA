       ...
       // SPDX-License-Identifier: Apache-2.0
               ...
               using Vixen.Graphics    .Null    ; using  Xunit    ;
 ...
 = new (new() {    Record    =  true });
   ...
   graph = new (device, pool );
		...
		public void Dispose    (    ) {  pool .   Dispose  (   );
            device .Dispose()   ; }
    static TextureDescription    Target   (string name)   => new(PixelFormat.Rgba8UNorm  , 256   , 256, TextureUsage  .ColourTarget |  TextureUsage.   Sampled , Name  : name);
    ...
    /// </summary>
        ...
        var
shared   = graph.    CreateTexture(Target("shared"));
          graph
       .AddPass("producer", pass  => {
            pass.ColourAttachment(shared  )  ;    pass.   SideEffect();
            pass   .Execute(_ => { }); });
    graph.AddPass("overwriter", pass => {
   pass   .ColourAttachment  (shared, LoadAction.Clear) ; pass.SideEffect    ()   ;
     pass.Execute(_ => { }   );
              });
            
        graph   .Compile   () ;
var
 warning = Assert.Single  (graph .Warnings);
   Assert
.Contains("VX2101", warning  , StringComparison.Ordinal);
  Assert.Contains(    "producer" , warning, StringComparison   .Ordinal);
               Assert.Contains( "overwriter" ,  warning   , StringComparison
     .Ordinal) ;
       Assert .Contains(  
                "shared", warning, StringComparison.   Ordinal) ;
    }
    /// <summary>
...
var shared  =    graph .
         CreateTexture(Target("shared"));
   graph.AddPass("producer", pass    => { pass  .ColourAttachment(   shared)  ;
            pass
           .SideEffect  (    );
            pass.Execute (_   =>   { })  ; });
        graph  .AddPass("accumulator" 
   , pass   => {
    pass.ColourAttachment  (  shared
               , LoadAction.Load   ) ;
		pass.SideEffect   ()    ;   pass.  Execute(    _ => { });
              });
        graph. Compile();
   ...
   Assert.Empty(  graph  .
    ...
    /// </summary>
    [Fact] public   void AConsumedWriteMayBeCleared() {
var shared =
            graph.CreateTexture(    Target ("shared"    )    );
var derived
            =  graph. CreateTexture(Target   ("derived"));
graph
.AddPass("producer",   pass => {   
         pass   . ColourAttachment(   shared) ;
      pass.Execute(   _ => { }); } );
   ...
   .AddPass(  "reader", pass => {
      pass.Reads(shared  );
       pass.  ColourAttachment(derived  ); pass.SideEffect    ();
        pass.  Execute(_ => {  });
        } );

               graph  . AddPass("recycler", pass    => { pass.ColourAttachment( shared, LoadAction.Clear  );
         pass
      .   SideEffect();
               pass.
    Execute(   _   => { });
   ...
   [Fact]
    public void AFindingIsReportedOnceAcrossRebuilds() {
      for (var frame = 0; frame  < 3; frame   ++) { graph. Reset();
...
=> {
        ...
        shared); 
            ...
            graph.AddPass    (    "overwriter", pass => { pass.ColourAttachment (shared, LoadAction.Clear);
pass    .
            SideEffect()    ;
                pass.Execute(_ => { })    ;
              });
