...
[    ] BoundsOf ( List <  FoliageInstance> instances, float radius  ) =>
       [.. instances.
      Select(    instance =>  new InstanceBounds(   instance.  Position,  radius *  instance.  Scale))];
    ...
    /// <summary>And they land at the same levels.</summary>
     ...
     void  TheyLandAtTheSameLevels  (    ) {
        var (batch,   view , settings    , lods) =    Fixture(1f, 60f, 120f);
           ...
           culler.
Cull(BoundsOf   (instances,  batch. Radius),  [], in settings  , lods)  ;
        var
       (counts, runs, _) =   Dispatch(instances,   in batch, in view);
                Assert.Equal(3,
            culler  .LevelCount  )   ;
     for (var  level 
			=  0 ;    level <  culler.LevelCount; level  ++) {
...
Assert    . Equal(culler.Parameters[slot    ]
 ...
 Assert. True(
			faded >    0, "nothing was in the fade band, so the comparison proves nothing."   ) ;
  }
...
.    Sum() ) ;
       ...
       Assert.Equal(
             host, runs.    SelectMany(run => run    ).Order().ToArray    (  ))    ;
    }  
 ...
 low  =    high    with    {   DensityScale = 0.3f };
  ...
  var source   = Source   ();
           ...
           Assert   . Contains("0x9E3779B1u"  ,  source,
...
Fact   ]
    public   void TheShaderStillDividesByTheCastMaximum() {
               Assert.  Matches   (
                new Regex  (@"UnitScale\s*=\s*4294967296f"), Source());
      }
