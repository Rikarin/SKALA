	...
	System .Text  .RegularExpressions;
		using Vixen.Core. Mathematics; using Vixen.Foliage; 
using Xunit; 
...
///     </para>
/// </remarks>
   public sealed    class  FoliageCullParityTests { const int  MaxLevels    = FoliageCullPass    .MaxLevels;
 
    static   string  Source(    ) { var directory =  AppContext.BaseDirectory;
              for (var    at    = new   DirectoryInfo  (directory   );    at is not null; at
             = at .Parent ) {
        var candidate   =    Path.    Combine(at.  FullName  ,  "Raven"
,    "Library"   , "Terrain"  ,   "FoliageCull.rvn");
     if (File
                .Exists   (candidate   )   ) {
 ...
 static bool
               ShaderKeep(Vector3 centre,    float density) {
       var  hash = (uint)BitConverter. SingleToInt32Bits
          (centre.X) * 0x9E3779B1u ;
           hash ^= (uint )BitConverter.SingleToInt32Bits(centre.  Y)  * 0x85EBCA77u; hash ^= (uint)BitConverter.SingleToInt32Bits (centre    .Z  ) * 0xC2B2AE3Du;
hash ^= hash >>    15    ;  hash *= 0x2545F491u ;
        hash ^= hash   >> 13
        ;
...
{
               var    normal = new Vector3(plane .X,    plane  .    Y, plane   .Z  ); var    distance   = Vector3   .Dot  (normal, centre) +    plane    .W;
     var    slack = 4.7683716e-07f * (  radius + Vector3.Dot    (Vector3  .Abs(   centre  )    , Vector3. Abs(normal))  +   MathF.Abs(    plane    .W))  ;
 ...
 static int  ShaderLevelOf(
    in    FoliageCullBatchRecord  batch  ,
            in FoliageCullViewRecord  view, Vector3 centre   ,
        float radius   ,   out float distance
  ) {
distance = Vector3 .   Distance    (centre, view   .Position)    ;  

          if ( distance -  radius
      >= batch. EndCullDistance) {
  return    -1; }

  if (batch.DensityScale    < 1f && !ShaderKeep
	(   centre  , batch.DensityScale)    ) {
          return -1; 
       ...
       foreach (var plane
              in Planes(view)) {
       if (ShaderOutside( plane  , centre, radius)  ) { return -   1;  
            }   }
        
        var level = 0;
			
               if   ( batch.LevelCount >  1u &&    distance   >= batch.Lod0    )  { level   = 1;
              }
             if ( batch.LevelCount  >  2u && distance    >=
      batch.Lod1    )  {
      level = 2;
}  
      if (  batch.LevelCount    >    3u &&  distance    >= batch.Lod2    ) {   level = 3    ;
        }
...
static float ShaderFade    (in FoliageCullBatchRecord  batch, float distance) {
 ...
 .   Plane2    , view.Plane3, view  .Plane4, view. Plane5]; 
  ...
  List<  FoliageInstance> instances, in    FoliageCullBatchRecord batch,
      ...
      var  fades = new    float[instances.Count]   ; 
       ...
       ++ ) {
           runs[level] = [  ]; }
              ...
              // Phase one: counting.
      for (var index  = 0   ; index < instances  .  Count; index++) { var  level = ShaderLevelOf(
          
in batch,
                in  view, instances  [index]    .Position,
			batch
.Radius *  instances    [index].Scale   ,
                out _ )  ;
...
Radius *   instances  [index].Scale,
          ...
          if (   level  < 0) { continue  ;
           ...
           runs[level].Add(index); fades[index  ] = ShaderFade(in  batch   , distance    )   ;
   }
                return    ( counts,
             runs, fades)  ;
      }
   // --- The fixture --------------------------------------------------------
    static   FoliageType
    Tree    =>
          FoliageType.  Of ("Tree") with { Mesh =    "Meshes/pine"  ,
        Radius = 2f
           ,
  StartCullDistance   =  160f,
            ...
            static BoundingFrustum Looking
                (float far   = 1000f) => 
   new(
        Matrix4x4  .LookAt (   new(0f, 20f, 60f)  , Vector3.Zero, Vector3.UnitY)    *    Matrix4x4.PerspectiveFieldOfView(MathF .PI / 3f, 16f  / 9f,  0.5f, far)
   );
        
    /// <summary>A spread of instances that straddles every rejection the cull has.</summary>
             static List   <FoliageInstance> Spread(    int count) {    var random =   new Random   (9182)    ;
   var  instances = new
 List   <   FoliageInstance >    (    count   )   ;
    ...
    static (FoliageCullBatchRecord Batch
       , FoliageCullViewRecord  View, InstanceCullSettings Settings,   float[] Lods   )
        Fixture(float  density = 1f, params float[    ]    lods)    {  var type = Tree;
  var frustum   =   Looking(  ); var  viewPosition = new Vector3(  0f, 20f, 60f  )    ;
        var batch
         =   new FoliageCullBatchRecord {
 FirstInstance =    0u  ,
            InstanceCount
          = 0u,
            LevelCount = (uint)Math.   Clamp(    lods.Length  + 1,
              1, MaxLevels   ),
            Visible = 1u,
			Radius = MathF.Max   (  type  .Radius
            , 0.5f),
            StartCullDistance =  type  .StartCullDistance,    EndCullDistance = type    .EndCullDistance,
             DensityScale = density,
       Lod0 =
lods    .    Length > 0 ? lods[0   ] : float. MaxValue   ,
                Lod1 =  lods.Length    > 1 ?  lods  [1
                ] : float .MaxValue,
...
ViewPosition = viewPosition ,
       ...
       DensityScale =    density,   Fade =  true   
      } 
               ;
          return (batch 
              , FoliageCullViewRecord.Of(in frustum   , viewPosition),    settings   ,  lods);
    }
    static    InstanceBounds
       [] BoundsOf    (List < FoliageInstance >  instances, float radius) =>
      [.. instances  .Select
          (instance => new    InstanceBounds (  instance.Position   , radius  * instance  .Scale  ))  ];
              // --- The seam -----------------------------------------------------------
    /// <summary>The same instances survive, at zero drift.</summary>
      [Fact] public void  TheSameInstancesSurvive   ( ) {
        var (batch, view, settings  , lods) =  
              Fixture(  );
            var instances  = Spread  (
       4000);
     var culler =   new InstanceCuller    (  );
        
     var survivors = culler.Cull(BoundsOf(   instances,    batch.Radius),    [ ]    , in settings, lods   ); var (counts, runs, _) =  Dispatch(instances   ,   in   batch, in view );
            Assert .True    (    survivors >   0, "the fixture culled everything, so it proves nothing.");   Assert .   True(survivors < instances .Count    ,   "the fixture culled nothing, so it proves nothing.");
        
      Assert
              .Equal (   survivors  , counts.Sum   ()  )  ;
   var host = culler
	.Survivors.    ToArray ().    Select(  index =>  (int)    index).Order    (   ).   ToArray  ()   ;
...
(  batch    , view, settings,   lods ) =    Fixture(1f,    60f, 120f);
              var instances = Spread(4000   ) ; var culler =   new InstanceCuller();
           culler.Cull
( BoundsOf(instances, batch   .Radius)  , [ ], in  settings, lods);

       var  (
                counts, runs  , _) = Dispatch(instances,   in batch, in   view);
            Assert.Equal(3, culler
     .LevelCount);
			for
    (  var level  =   0; level  <  culler .    LevelCount;  level++) {
   var run  = culler .   Runs  [level];   var host =  culler.   Survivors .Slice(run. First, run  .   Count) .ToArray    (  )
	.Select(  index =>   (int)   index)

            .    Order(  )
                .ToArray    (   )  ;
         
       Assert    .  Equal (run.Count   , counts[
       level]   )    ;
            Assert.
...
Assert.    All   (    counts.    Take(3)   , count => Assert.True(count > 0));  }
    ...
    /// </remarks>
           [Fact] public void TheRunsAreContiguousInLevelOrder() {
var (batch, 
...
// The shader's own arithmetic: base is the sum of the earlier levels' counts.
  ...
  at += counts[   level   ]; }
    ...
    [Fact ] public   void TheFadeIsTheSameNumber()   {
     var (batch,   view, settings   , lods) = Fixture  () ;
var instances = Spread(    2000  )   ; var culler  =  new InstanceCuller( ) ;
        var  survivors  =  culler.Cull(BoundsOf (instances, batch.Radius),  [],  in settings  ,    lods); var (_,   runs , fades) = Dispatch ( instances,  in   batch,  in view)    ;
               Assert .
 NotEmpty(runs   [0] ) ;
...
= (int)   culler  .Survivors   [  slot]   ;
   ...
   Assert.Equal(culler.    Parameters [slot].
              Fade, fades[    index   ] , 6);
 if  (   fades[ index  ] <   1f  ) {
			faded++    ;  }
  }   
               ...
               Assert.True(   faded
 >   0, "nothing was in the fade band, so the comparison proves nothing.");
 ...
 ( ) {
          var (batch,    view , settings    , lods) =  Fixture(0.4f); var   instances = Spread   (  4000 );  
  var culler  =
           new InstanceCuller()   ;
...
=    culler.Cull(  BoundsOf (instances, batch    .    Radius), [    ], in settings  , lods)   ;
    ...
    =    Dispatch(instances,   in batch, in view);
       ...
       Assert.   True (survivors > 0
        ...
        Assert   .Equal(survivors, counts.
           Sum() ); 
           var    host = culler . Survivors.ToArray(    )
.Select(index => (int)    index)  .   Order(  )  .ToArray ();
           Assert.Equal(  host
  , runs.SelectMany(run => run    ).Order ()    .ToArray());
          }
 ...
 [Fact
    ...
    public    void  LoweringTheScalarRemovesRatherThanRearranges(    ) {
      var instances = Spread  (3000); var (   high,  view, _, _) = Fixture (0.8f );
         var low
                =   high    with { DensityScale  =    0.3f    }    ;
     var   kept = Dispatch (instances , in high, in   view    ) .Runs.SelectMany(   run => run)   .   ToHashSet( ); var fewer   = Dispatch(    instances, in low  , in view ).Runs    .   SelectMany(run => run  ) .ToHashSet();
            Assert.True (fewer    .Count  < kept.Count, "lowering the scalar kept as many."); Assert.   True(fewer   .IsSubsetOf(kept    ), "lowering the scalar introduced an instance that was not there." )   ;
    ...
    /// <summary>The hash's constants are still the host's, all four.</summary>
           [    Fact] public   void TheShaderStillMixesWithTheHostsConstants() {
        var source =    Source(  );
        Assert   .Contains   ("0x9E3779B1u",   source , StringComparison
  .Ordinal  )  ;
   Assert.
Contains ("0x85EBCA77u",    source    , StringComparison    .Ordinal    );
  ...
  public  void   TheShaderStillDividesByTheCastMaximum(  )   {
      Assert   .Matches(new
  Regex(@"UnitScale\s*=\s*4294967296f"  )   , Source  ());
           } 
  ...
  Assert.Matches(  new Regex(   @"RoundingSlack\s*=\s*4\.7683716e-07f" ), Source   (  ))    ; } 
    ...
    [Fact   ] public void TheShaderStillDeclaresFourLevels   ()    {
   Assert  .Matches(new Regex(@"MaxLevels\s*=\s*4") , Source(    )); Assert.Equal(4, MaxLevels   );
               } 
    ...
    /// </remarks>
                [
Fact]
    public void BothPhasesStillCallTheSameLevelFunction() { var source = Source()  ;
        Assert.Equal(
         1, Regex.Count(source    , @"func LevelOf\(" ) );
