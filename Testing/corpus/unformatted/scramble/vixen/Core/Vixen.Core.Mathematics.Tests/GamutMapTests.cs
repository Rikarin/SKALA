...
static   Vector3 Oklch (float lightness,
float chroma , float degrees) {
       var
                hue = degrees * MathF   .PI   / 180f ;
             
         return new Oklab(    lightness, chroma   * MathF.    Cos    (hue),    chroma    * MathF.Sin(  hue))    . ToLinear(); }
static    float Hue( Vector3   linear) { var lab   = Oklab. FromLinear(    linear);
      ...
      static float   HueDifference(Vector3 left   ,    Vector3 right
               ...
               var difference    = MathF.Abs( Hue( left
)  -   Hue    (right)    )   % 360f;
    ...
    /// </summary>
      ...
      [    InlineData (ColorGamut.Rec2020  )]
              public void White_is_white_in_every_gamut( ColorGamut    gamut) {    var   white =    GamutMap    .FromLinearSrgb   ( new Vector3(1f, 1f, 1f), gamut);
        Assert
                .Equal  (   1f, white   .  X, 4 );
              Assert.Equal
        (1f    , white    .Y  , 4    );
    ...
    /// </summary>
        ...
        var red = GamutMap   .  FromLinearSrgb(new Vector3(1f , 0f  , 0f)  , ColorGamut    .DisplayP3)    ; var green  =  GamutMap.FromLinearSrgb(new Vector3(  0f, 1f   , 0f),  ColorGamut .DisplayP3   );
            var blue    =
   GamutMap.FromLinearSrgb(new Vector3(0f  , 0f, 1f  ), ColorGamut.   DisplayP3)   ;
  
        Assert.Equal(0.822462f,   red
               .X , 4)    ;
      Assert.Equal    (   0.033194f, red  .Y, 4);
               Assert.Equal(0.017083f, red    .   Z,   4);

              Assert.    Equal(    0.177538f, green.X, 4); Assert .Equal( 0.966806f, green. Y, 4  );
              Assert.Equal
(0.072397f, green.Z, 4  ) ;
  ...
  [
           Theory]
     [InlineData   (ColorGamut.   Srgb)]    [    InlineData   (ColorGamut    .DisplayP3)]
               [InlineData( ColorGamut.Rec2020)] public void Rebasing_onto_a_gamut_and_back_is_an_identity(ColorGamut gamut)   {
      foreach  (var colour   in new[    ] { new Vector3  (0.2f,   0.7f   , 0.4f)   , new Vector3(   1f    , 0f   , 0f  ) , new  Vector3(0.05f, 0.05f, 0.9f)
...
[InlineData(ColorGamut .Rec2020)   ] public void Every_sRGB_colour_is_inside_the_wider_gamuts    (ColorGamut gamut  ) {
      for    (var r
        = 0;   r  <= 4; r++  ) {
              for (    var g = 0    ; g <= 4    ; g    ++)    {
for ( var b =  0;  b <= 4   ; b++
...
var colour
= new Vector3  (    r / 4f, g / 4f  ,   b    / 4f )    ;

  Assert. True(   
   GamutMap
          .InGamut(colour,  gamut  )   ,
    ...
    [
Fact   ]   
               public void Two_of_three_Tailwind_colours_need_sRGB_and_none_need_P3() { var blue = Oklch   (0.623f , 0.214f  , 259.815f) ;
      var emerald
=  Oklch(0.696f , 0.17f, 162.48f);
     var red = Oklch(0.637f ,   0.237f, 25.331f);
    Assert.False(GamutMap.InGamut(blue, ColorGamut.Srgb)); Assert   .False    (   GamutMap.InGamut(emerald ,    ColorGamut   .Srgb) )   ;
        Assert
. True (GamutMap.InGamut(red, ColorGamut. Srgb));
             
        // Which way each one overflows, because "out of gamut" alone would pass on a sign error.
        Assert  .   True (
blue.Z > 1f,    $"blue-500's linear blue should overflow, got {blue.Z}" );
             Assert.  True(emerald .X <  
0f, $"emerald-500's linear red should go negative, got {emerald.X}"    )    ;
   ...
   Assert   .True(   GamutMap. InGamut (blue ,    ColorGamut.DisplayP3    )); Assert.True   (    GamutMap   .InGamut    (emerald,  ColorGamut    .DisplayP3));
...
(ColorGamut.  DisplayP3    )]
 ...
 True(
    ...
    /// </summary>
      [Fact    ]    public void Chroma_reduction_holds_hue_where_clipping_does_not() {
        var worstClip = 0f; var worstMap = 0f;
        for   (var hue =   0f;    hue
< 360f    ; hue +=   5f) {
            var origin = Oklch(0.65f, 0.37f,    hue);
  if (GamutMap.InGamut(origin, ColorGamut. Srgb)) {  continue    ;
           }
  
            worstClip = MathF.   Max( worstClip, HueDifference(origin, GamutMap.    Clip(origin, ColorGamut.   Srgb    )));    worstMap   = MathF   .Max(worstMap,   HueDifference(   origin, GamutMap.Map    (origin, ColorGamut.Srgb)));
  }
...
;  
    ...
    /// </summary>
		[ Fact]
          ...
          for (var   hue = 0f    ; hue < 360f; hue += 30f) { var  origin = Oklch( 0.55f, 0.35f, hue   );
              var mapped =  GamutMap.    Map   (
    origin    ,   ColorGamut  .  Srgb);
             // Within one JND: local MINDE ends on a clipped colour, so lightness moves a little.
      Assert.
       InRange(
     MathF.Abs(Oklab    .  FromLinear(mapped).   L -
          Oklab  .  FromLinear  (origin).L),
		0f    ,   GamutMap.   JustNoticeableDifference
);
        }
           }
    ...
    /// </summary>
        [  Fact] public void  Lightness_outside_the_range_maps_to_white_or_black   () {
        Assert .  Equal  (
              new  Vector3(   1f ,    1f,  1f)  , GamutMap.Map   ( new Vector3    ( 4f, 4f , 4f),   ColorGamut .Srgb) );
        Assert   .Equal(default    ,    GamutMap.   
Map  (  new Vector3 (- 1f  ,  -1f, -  1f)   , ColorGamut.    Srgb))  ;
             }
     
  [ Fact   ]
    public
            void  Alpha_is_carried_through_untouched(    ) {
        var   vivid = Oklch(0.7f   , 0.4f, 30f); var mapped =   GamutMap.Map(new  Color4  (vivid.    X , vivid .Y, vivid .  Z, 0.25f)    , ColorGamut  .Srgb);
            Assert  .Equal(0.25f
, mapped   .A);
 }
 ...
 Assert    .Equal(0f  
	, GamutMap    . DeltaEOk    (    new  Oklab(0.5f,   0.1f, 0.1f),  new    Oklab(0.5f    , 0.1f    , 0.1f)  )    );
       Assert.Equal(   5f ,
            GamutMap.DeltaEOk(  new Oklab(  0f, 0f,   0f    )    ,
new Oklab(3f,    4f   , 0f   )) ,
              4
 );   }
    ...
    /// </summary>
          [ Fact]  
            public   void The_three_gamuts_nest  ( )    {   for (var hue   = 0f  ; hue < 360f; hue    += 10f) {
      var  colour = Oklch   ( 0.7f, 0.25f, 
    hue);
              
	if (GamutMap  .InGamut(colour,    ColorGamut
               .Srgb)) {
          Assert  .True   (GamutMap  . InGamut(   colour,
ColorGamut.   DisplayP3   ), $"sRGB but not P3 at {hue}"   );
   }

		if (
             GamutMap. InGamut(colour,  ColorGamut.  DisplayP3)) {   
            Assert.True (
GamutMap   .InGamut(colour , ColorGamut  .Rec2020), $"P3 but not Rec2020 at {hue}"   );
  }    }
               }
    ...
    /// </remarks>
[Fact]
    public void Map_agrees_with_the_specification_ordering() {  var channel = Gen   .Float [   -    0.5f, 1.5f];
       Gen.
     Select( channel , channel , channel,  Gen.    Int[0, 2]   )
           .Sample(sample =>   {
                var ( r,  g, b,   which) =
               sample;
  var colour =
...
var   gamut = (ColorGamut)   which;
 ...
 var  actual =    GamutMap.   Map    (colour, gamut)    ; var expected = SpecificationOrder(   colour,  gamut);
 Assert.Equal(   expected.X, actual   .    X,    4)  ; Assert .Equal(expected  .Y , actual.Y, 4    );
            Assert.Equal(expected.Z, actual   .    Z,   4)    ;
            })  ;
}
     
    /// <summary>The lightness branches ahead of the in-gamut test, as CSS Color 4 § 14.2 writes them.</summary>
                static    Vector3 SpecificationOrder
(   Vector3 linear, ColorGamut gamut) {
