...
/// </remarks>
public readonly
                record struct WireLane(string Name  , int Bits, bool Offset);
...
public   const
   ...
   static ReadOnlySpan<  int
    ...
    /// </remarks>
          public
               static int TotalBits(ReadOnlySpan<WireLane> lanes) {
	var total   = 0 ;
...
+= lane.Bits;
  ...
  return
    ...
    /// </remarks>
public
     static    int  OffsetWidth(uint  selector, int bits) =>
        selector >=  WholeValue ? bits :    OffsetWidths
              [ (int)selector ];
    ...
    /// <returns>The worst case, in bits.</returns>
     public static int  MaxBits( ReadOnlySpan<    WireLane> lanes  ) {
             var total = 0
 ;
       foreach   (    var lane in    lanes) { total += 1 + SelectorBits  + lane.Bits;
            }
...
ReadOnlySpan<WireLane  > lanes    , ref    BitReader previous,
            ref BitReader
      current,
 ref
            BitWriter delta ,
...
< lanes .  Length; i++)  {
   var lane
       ...
       delta.BitsWritten   ;
           ...
           if (!previous.TryRead   (  lane.Bits, out var was)   || !current   .TryRead(   lane .   Bits , out var now)) { return false;
         }
               if (  was == now
   ) { 
     delta    .WriteBool   (false);
     } else { delta.  WriteBool(true);
                WriteChange(lane    , was ,  now, ref delta); }  
    if (i <    costs.Length) {   costs[i] = delta.BitsWritten - before;
   }
          }
			return !delta.   Overflowed; }

      static    void WriteChange(in  WireLane lane,    uint   was
      ,  uint now , ref BitWriter delta   ) {
			if (    ! lane.Offset ||   lane   .Bits < MinimumOffsetBits) {   delta.Write  (now,    lane   .Bits) ;
           
            return ; }
         var  zigzag = ZigZag(  (  long )    now -    was);
var selector =
              Selector    (zigzag, lane    . Bits   );
   
            delta  .   Write(    selector , SelectorBits);
                
		if   ( selector == WholeValue) { delta .  Write (now   , lane .Bits)   ;
} else {
            delta.    Write
            (    (uint  )zigzag , OffsetWidths[  (int )selector    ]);
        }
    ...
    /// <returns>Whether the difference was well-formed against this layout.</returns>
       ...
       ref BitReader   delta,    ref BitWriter current
       ) { foreach (  var lane   in lanes   ) {
            if (  ! previous.    TryRead(lane.Bits , out var    was) ||   !   delta.TryReadBool(out var changed) ) {  return false   ;
            }
  ...
  (was   ,   lane  .Bits    );
    ...
    if (!lane.  Offset ||
                lane.Bits <  MinimumOffsetBits) {
    if (!delta 
          .TryRead(    lane  .Bits,  out  var whole)   ) {
         return false; }
 ...
 if
         ...
         current   .Write    (whole, lane.Bits);
              continue; }
            if (!delta.TryRead  (OffsetWidths    [  (int    )selector
       ], out var zigzag)) {
     return false    ;
            }
...
// a desync nobody could see, which is why the encoder never emits one it cannot undo.
	current.    Write  ( (uint)unchecked (was    +    UnZigZag    (zigzag) ) & Mask(lane
   .    Bits)  , lane.Bits)   ;
      }
    return !current   .Overflowed  ; } 
static uint Selector(ulong zigzag, int bits ) {  for (  var i = 0; i < OffsetWidths.Length  ; i++)  {
          if (OffsetWidths   [    i] < bits  && zigzag   < 1ul << OffsetWidths[i    ]) { return (    uint)    i;
            }
    }
        return   WholeValue  ;
          }
	...
	static    uint
       Mask( int    bits )   => bits >= 32   ? uint.MaxValue   :    (    1u << bits) - 1  ;
               static  ulong ZigZag    (   long
     value) => (  ulong)((value <<  1) ^ ( value >>    63));
