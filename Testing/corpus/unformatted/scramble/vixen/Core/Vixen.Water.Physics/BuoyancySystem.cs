...
using Vixen   .Core    .Threading;
            using Vixen.Ecs;  
            using  Vixen.Ecs   .Systems; using  Vixen.    Physics.Ecs    ; 
...
[UpdateBefore( typeof(PhysicsStepSystem
     ...
     public    sealed class BuoyancySystem(  PhysicsScene scene,   IWaterSurface surface   ) :    SystemBase    ,    IDeclaredAccess {  readonly PhysicsScene scene = scene ?? throw new ArgumentNullException(nameof( scene));
...
BuoyancyForce[] forces   = new BuoyancyForce[    8];
    ...
    /// </remarks>
    public
WaterDisturbances? Disturbances { get;   set; }
...
float WakeSpeed {  get; set;  } = 0.75f ;
 ...
 public float SplashSpeed  {  
   get; set; } = 1.5f;
    ...
    ///     where it is.
    /// </remarks>
    public SystemAccess Access {    get; } = SystemAccess   .   Declare( ) . Read<PhysicsBody>()
               .    Write
<    BuoyancyState    >  ()
        .Build (  )   ;
...
Pontoons { get; private set;  }
 ...
 // writes.
    ...
    /// <remarks>Public so a test can step without standing up a runner.</remarks>
      public void Step(    World world)    { ArgumentNullException  .    ThrowIfNull(  world)   ;
             
        Floating = 0;    Pontoons = 0;
          WetPontoons    = 0;
        LastCount  = 0;
...
// and are read per entity here anyway, because the loop is already one at a time.
    asked.Clear
(  )   ;
        foreach (  var chunk in world. Chunks  (bodies  )  )  { asked.   AddRange(chunk .Entities[..chunk.Count]);
   }
  ...
  // ⚠ Out of the simulation and not out of a component. `WorldTransform` is written by
   ...
   // that produces is exactly the class of bug § D2's whole seam exists to prevent, and the
      // simulation already has the answer to hand.
          ...
          ( body, out var position  ,    out var rotation    )   ;
   ...
   // authoring mistake QueryAt's own remarks refuse to resolve.
		var origin
      = position;
     if (Surface   .    QueryAt (new  (origin.X  ,  origin.Z)) is not { }   query) { Dry(   world  ,   entity, pontoons.   Length    );
 return;
  }   
               if (forces.  Length    <
            pontoons   .    Length) {
       ...
       var evaluator = query  .  Evaluator ();
        // ⚠ No ripples. See the class remarks: the closed form is what a rollback can re-ask.
            var wet = Buoyancy.Solve( in   evaluator   ,  
...
Length )  
        ...
        = pontoons.Length   ;   
         WetPontoons += wet; 
            var lift  =    0f ;
        var submerged  = 0f;  
    var was  = world    .  Has   <   BuoyancyState >   (   entity)  ?  world.
Read  <   BuoyancyState   >(entity) .Wet : 0;
       
             for (var index   =    0;    index <  pontoons   .Length; index++  ) {   var force = forces    [  index    ]   ;
...
if (force    .
       Submerged <= 0f) {
           ...
           lift += force   .Force.Y ; Disturb    ( in force,  velocity,   entered:  was ==  0);   
     // At the pontoon's own world position, which is what makes the hull pitch. A force at
      ...
      > 0)   {
        ...
        world.Set   ( entity  ,
            new BuoyancyState    {    Wet = wet,
Total = pontoons.Length, Submerged = submerged    / pontoons.    Length,
Lift = lift
  ,
        SurfaceHeight = query  .    Height
(new(   origin.X   , origin.    Z)   , Surface.WaterTime  )
            } 
			)    ;
            }
    ...
    /// </remarks>
         void Disturb
         (in BuoyancyForce  force,   Vector3 velocity, bool entered) {
			if  (Disturbances is not { }   queue)
      {
       return;
    }
  
          var lateral = new Vector2( velocity
.X, velocity    . Z). Length()    ;
   ...
   queue.Add  (
	new(  new  (   force.  Position    .   X, force .Position  .   Z   ) ,
      1f, -falling * force.Submerged,
               WaterDisturbanceKind   .
Splash,
   force.SurfaceHeight )
	)    ;
    return;
    ...
    if  (lateral >=    WakeSpeed)   
{
       ...
       new(
                    new 
         (force.  Position.X, force.    Position   .    Z   )   ,
                    0.75f,
