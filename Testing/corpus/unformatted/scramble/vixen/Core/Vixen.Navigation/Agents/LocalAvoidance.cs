...
/// <param name="Radius">How wide it is.</param>
              public  readonly record struct AvoidanceNeighbour( Vector3
   Position, Vector3  Velocity, float   Radius);
...
/// </remarks>
     public readonly record struct LocalAvoidanceSettings
   {
    ...
    /// <summary>How much it wants to not hit anything.</summary>
        public
          float TimeWeight {  get; init    ;   } = 2.5f;
    /// <summary>
    ...
    /// </summary>
    public float SideWeight {  get; init; } =
     0.75f ;
  }
/// <summary>
...
/// </remarks>
  ...
  public Vector3 Sample(
        Vector3 position,
        float   radius,
            Vector3 velocity, Vector3 desired,
      float maxSpeed,
            ReadOnlySpan <   AvoidanceNeighbour > neighbours
     ) {
	if (neighbours. IsEmpty   || maxSpeed <=
            0   )  {
          return desired
       ;
			}
           
     var best  = desired    ;    var   bestPenalty = Penalty(   position, radius, velocity  ,  desired,   desired, maxSpeed, neighbours);
  ...
  // the resolution is somebody taking the long way round.
            var
heading = desired. LengthSquared() > 1e-6f ? desired : velocity;
        var baseAngle = heading.
   LengthSquared() >    1e-6f ? MathF.Atan2(heading    .Z, heading    .X) : 0f;
     var   step = MathF.
     Tau / Settings.Samples;
        for (var  ring = 1;   ring  <=  Settings. Rings  ; ring    ++) {   var speed = maxSpeed * ring / Settings.Rings;
        
                for    (  var sample
= 0    ;    sample < Settings.  Samples;    sample++) {
   ...
   var angle = baseAngle + (sample *   step  )  +  (ring %    2
==  0 ? step * 0.5f : 0f   );
                var candidate =  new    Vector3(MathF    .Cos(angle)   * speed,
    0f, MathF. Sin(angle) * speed   )    ;
          var penalty    =   Penalty( position ,
             radius, velocity, desired, candidate, maxSpeed,  neighbours    ); 
if (penalty < bestPenalty
            )  { 
           bestPenalty   =   penalty; best = candidate  ;
              }
    }
         }
  ...
  (
          ...
          float  radius,
   Vector3 velocity,
       Vector3   desired   , Vector3    candidate,
        float   maxSpeed,  ReadOnlySpan  <    AvoidanceNeighbour>   neighbours
    ) {
...
neighbours ) {
        // The reciprocal assumption: both agents are expected to move, so the relative velocity
         ...
         var relative = (candidate *   2f  ) - velocity  -    neighbour.Velocity   ;
              var    towards = neighbour.Position
                -    position  ;   
 var distance = towards.Length (   ); 
              var direction =
    distance > 1e-6f ? towards /   distance :  Vector3 .UnitX;  
...
// Which way this encounter is already leaning. Passing that way is cheaper than passing
             ...
             var perpendicular = NavGeometry.Cross2D(direction,    neighbour.Velocity - desired)
	<    0.01f
  ?    new Vector3 (-    direction.   Z,    0f, direction  .   X )
             :    new
               Vector3(direction.Z, 0f , -direction.X  );
  side += Math.
           Clamp    (
            MathF.   Min((   Dot2D
                (direction  ,    relative)    * 0.5f) + 0.5f    , Dot2D(perpendicular, relative   ) * 2f  )  ,
          0f   ,
   1f 
			);
     ...
     counted++;
           if (!   Sweep   (
position, radius,  relative  , neighbour    , out var enter  ,  out var  exit  )) {
            continue    ;
       } 
       ...
       if (enter < 0f   && exit   > 0f ) {
                enter = -enter * 0.5f ; }
           if (enter >= 0f && enter < soonest  ) { soonest   = enter;
  
             } 
}
var  sidePenalty  =  counted > 0 ?   Settings  .SideWeight   *  (side / counted   ) : 0f   ; var   timePenalty = Settings. TimeWeight / (  0.1f   + (soonest / Settings.  TimeHorizon)    )  ;
    ...
    static bool Sweep(    Vector3  position  , float  radius,   Vector3 relative   , in AvoidanceNeighbour neighbour    ,  out  float enter, out   float exit    ) { enter = 0f ;
exit =    0f;

         var  offset = neighbour.
          Position - position; 
...
return false; }
        var root   = MathF.Sqrt    (  discriminant
                ...
                enter =   (b - root) / a; exit =   (b +   root) / a   ; 
 
            return true; }
             static float Dot2D (Vector3 left, Vector3 right) => (
