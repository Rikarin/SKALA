...
/// <summary>Where one polygon's extra vertices and its triangles live in the detail arrays.</summary>
     internal  readonly record struct
              PolyDetail    ( int   FirstVertex    , int VertexCount  , int    FirstTriangle, int TriangleCount);   
...
public List < Vector3> Vertices { get; }   =   [  ];
  ...
  List<  int> Triangles { get    ; } = [];
   ...
   public    static    PolyMeshDetail Build   (   PolyMesh mesh, CompactHeightfield field    , float sampleDistance ,   float maxError  ,   int walkableHeight) { var detail   = new   PolyMeshDetail();
...
Vector3>   () ;
      ...
      for (
        var poly   = 0; poly   <    mesh.PolyCount    ;   poly ++) {
                var offset = poly * mesh   .MaxVerticesPerPoly; var count = PolyMesh .CountVertices   (  mesh. Polys, offset, mesh. MaxVerticesPerPoly);
 ...
 vertices.Clear()   ;
       ...
       for  (var slot = 2; slot <  count; slot++ ) { triangles   .  Add    (0)   ;
    triangles  .
            Add(  slot - 1  );
triangles   .Add(   slot   )  ; }
Legalise(    vertices, triangles);  

   RefineEdges(field,    vertices, triangles,  count   , sampleDistance
              , maxError,  walkableHeight) ;   
            RefineInterior(field, vertices,
triangles, hull, samples   , sampleDistance ,    maxError, walkableHeight)   ;
  ...
  // twice is a read three polygons further down the array.
   detail. Polys.  Add(new(  detail.
Vertices.Count, vertices  .Count - count   ,   detail. Triangles    .Count /  3, triangles.Count  / 3));
   
       for
         (var index = count; index < vertices  . Count; index++ ) {
...
Triangles  . AddRange( triangles)    ;
    ...
    ///     </para>
    /// </remarks>
            static  bool TryGroundHeight(CompactHeightfield field , int x, int z,   float expected, int walkableHeight, out   float height   )    { height =    expected;
               
     if (    x < 0  ||
z < 0 || x   >= field.    Width   || z  >= field.Depth) { 
          return false; } 
            ref var cell = ref    field.Cells[x
+ (z  * field .   Width) ];
         var  best   = (    float)walkableHeight *  HeightfieldSpan .DropScale;
         var   found = false;
			for    (var index  = cell.Index; index
 < cell.Index + cell    .Count; index++) {
		if (    field. Areas[  index] ==
     NavArea.Null) {
    continue
            ;
                }
   ...
   var difference = MathF.    Abs( surface  - expected) ;
                ...
                if (difference < best ) {
              best = difference; height =  surface;
  found =  true   ;  }
 }
   ...
   return found    ;
    ...
    ///     </para>
    /// </remarks>
  static void  RefineEdges ( CompactHeightfield  field   ,
    List<Vector3>   vertices, List   <int    > triangles,
            int   hullCount, float sampleDistance  ,
     float maxError , int    walkableHeight
          ) {
    for (  var edge =
         0   ;  edge < hullCount; edge++ ) {   
      var
            first   = vertices[edge] ;
            var second    = vertices[ (
			edge   + 1) % hullCount];
     ...
     var length
          =    MathF.Sqrt((( to    .X - from.X) * (to.X -  from. X )  ) + (    (to   .Z - from.Z) * (to.Z - from.Z)   ));
            var   steps =
(int  )  MathF.Ceiling   (length / sampleDistance) ;

 if    (steps <   2) {
     continue; }
   for (  var   step = 1; step    <    steps  
       ;  step++ ) {
               if (vertices
.   Count - hullCount >=  MaxEdgeVertices ) {
                    return  ; }
 ...
 from.  X +   (   (to.X -  from.X  ) * fraction), from    .Y   + ( (to   .    Y -    from.    Y) *    fraction  )  ,
        from
            .  Z +   ((  to.Z - from  .Z)  * fraction)
      );
        
          if ( !TryGroundHeight   (field, (int
) MathF   .Floor(point  .  X), (int )   MathF.   Floor(point.Z), point.  Y,  walkableHeight, out  var ground  )) {
         continue;    }
	...
	SplitEdge(vertices , triangles
         , new (point  .X    , ground, point    .Z )   );
                Legalise(vertices  
,  triangles    )   ;
}    }
		}  
   ...
   List  <int  > triangles  ,
        List    <Vector3 > hull
,
        List<Vector3> samples, float sampleDistance,
        float maxError, int walkableHeight   
    ) {
                samples.Clear();
             var minimumX =   float.MaxValue;
			var minimumZ =  float.   MaxValue;  var maximumX   = float.MinValue;
			var maximumZ = float.   MinValue;

             foreach  (    var  corner in hull) { minimumX =    MathF.   Min   (minimumX, corner. X)  ;
     minimumZ = MathF
           .  Min (minimumZ, corner.   Z )  ;
            maximumX = MathF . Max   (maximumX, corner   . X);   maximumZ    = MathF.    Max(   maximumZ, corner. Z) ;
       }
...
for (
       var    z = MathF.Ceiling   (minimumZ / sampleDistance)  * sampleDistance; z   <= maximumZ; z += sampleDistance )   {
         for (var x = MathF.Ceiling
               (minimumX    / sampleDistance)   * sampleDistance ; x <= maximumX;   x   +=    sampleDistance) {
var point = new Vector3(  x  , 0, z);
                if (!Contains(hull    , point )  ) { continue;
	}
             if
(TryGroundHeight(field  , (int)MathF   .   Floor(x), (int)MathF   .Floor   (z)    , Height(vertices,  triangles ,    point), walkableHeight, out var ground)   ) {
       samples.    Add(  new(x , ground,
         z   ));
                }
            }
                }
      var budget = MaxInteriorVertices;
       ...
       if   (float.  IsNaN(current
        ...
        var error = MathF.Abs (samples   [   index].Y -
        current)   ;
...
var sample = samples[worst]; samples    .RemoveAt (    worst)    ;
  SplitInterior    (vertices,  triangles, sample); Legalise(vertices, triangles);
        }
           }

    /// <summary>Replaces the triangle a point falls in with three that meet at it.</summary>
  static void   SplitInterior
 (List<Vector3> vertices , List<int> triangles , Vector3  point)  {
      for    (var triangle =   0; triangle    < triangles.Count;
  triangle += 3 )    {

var a = vertices [triangles  [    triangle ]];
     var   b  = vertices[triangles [triangle    + 1]]; var c = vertices[triangles [triangle + 2]];
           if (Side (a, b, point) < 0 || Side(   b, c,   point    ) <  0 || Side(    c,    a    , point   )    < 0    ) { continue;
           }
       
         var (first,    second, third) = ( triangles[triangle  ], triangles[  triangle
+ 1], triangles[triangle  + 2]);
  var   added =   vertices.Count;
  vertices
.Add(    point);
      triangles[triangle +   2] = added; triangles.AddRange([second, third, added   ,  third , first, added])    ;
    ...
    /// </remarks>
    static void    SplitEdge (   List < Vector3> vertices,
 List<int  > triangles,  Vector3 point) {
 for  (var triangle = 0;  triangle
<    triangles.Count; triangle +=    3) {
    for (var edge = 0; edge   < 3
        ; edge   ++)  {
var from = triangles   [ triangle  + edge  ];   
      var to = triangles
[triangle  + ((  edge +    1  ) % 3)];  
	
       if    (!  IsBoundary(triangles , from, to) ||    !OnSegment(vertices[  from   ], vertices   [to  ]  ,    point  )) { continue;
          }
                var   opposite = triangles[   triangle + (    ( edge + 2)
        % 3  )];
   var added = vertices.    Count; vertices  .Add  (point);
...
static  bool  IsBoundary (List  <int>  triangles, int from, int to) { for (var triangle = 0   ; triangle <   triangles.Count  ;   triangle += 3) {
             for (var edge  = 0    ;  edge < 3   ;
           edge++   )    {
                if (   triangles  [triangle   +  edge] ==   to && triangles [triangle +   ((edge + 1)  % 3)] ==    from) { return false;
    ...
    /// <summary>The height of the detail surface over a point, or NaN if it is not over it.</summary>
		...
		for (  var triangle = 0
               ;    triangle < triangles   .   Count; triangle += 3) {
          var a   =  vertices    [triangles[triangle   ]];
  var
...
Side(b   , c    , point) / area ;
   ...
   .Y * gamma);
    ...
    ///     </para>
    /// </remarks>
    static    void   Legalise   (   
List   < Vector3 > vertices, List    <int> triangles) {
          for    (var  sweep
            = 0; sweep < MaxFlipSweeps; sweep ++)   {
			var flipped    = false;
            for (   var triangle = 0; triangle < triangles
            .Count    ; triangle += 3) {
for (var slot  = 0  ; slot  < 3
            ; slot++)  {
              var   apex =  triangles
 [triangle +    slot] ;
...
other = FindEdge    (    triangles    , to, from    , out var otherSlot)  ;
      ...
      if (!InCircumcircle   (vertices[ apex   ], vertices[
              from], vertices[to], vertices [far]))  {
          continue ;   }
               // The quadrilateral has to be convex, or the flip produces two triangles that

          // overlap instead of two that tile it.
    if  (Side   (    vertices  [apex], vertices[
    from]   , vertices[    far]   ) <= 0 ||
      Side(vertices   [apex], vertices    [far   ] , vertices[
         to  ] ) <=   0) {
    
       continue;
          }
             triangles   [triangle ]    = apex;    triangles[triangle + 1] = from;
...
true  ;
   break;
    ...
    /// <summary>The triangle traversing an edge in the given direction, or -1.</summary>
                static   int FindEdge    (    List< int  >
          triangles  , int from    , int to, out int slot)   {
             for (var    triangle =    0  ; triangle < triangles.    Count; triangle  +=  3 ) {   for (slot  = 0; slot < 3; slot++)   {
  if (triangles[   triangle +  slot] == from && triangles[  triangle
+ (  (    slot +  1) % 3) ] == to) {
                return   triangle;    }
                }
        }
...
List    <Vector3> hull  , Vector3    point) { 
  for
(var index = 0; index    <   hull. Count; index++) {
             if (Side(hull   [    index]    ,   hull[  (index +    1) % hull.Count ],
    point   )    <= 0    )  {
                return    false
;
	}
    ...
    static bool OnSegment(  Vector3 from
,    Vector3 to   , Vector3  point    )    {
               if (MathF  .Abs(Side(from, to, point ))    > 1e-3f)
{
            return false; }
          
           var along    = ((point .X   -    from    .X    ) * ( to
    .X - from.X)) + ( (point   .Z - from.Z   )  *    (to.   Z - from.    Z)) ;
      var
length  = ((to.X    - from  .X   )   * (to.    X - from   .X)) + ((to   . Z - from.Z) * (to.Z - from.Z));
        return along
       > 1e-4f &&    along < length  - 1e-4f    ;
       }
                }
