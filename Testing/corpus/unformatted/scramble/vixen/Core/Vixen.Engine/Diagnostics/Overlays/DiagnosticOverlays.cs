...
/// </remarks>
     ...
     overlays   = [    ];
    readonly   OverlaySurface surface = new    (); readonly HashSet<   string> requested = new  (   StringComparer.OrdinalIgnoreCase);  
...
public IReadOnlyList<IDiagnosticOverlay > Registered  => overlays;
    ...
    /// <exception cref="ArgumentException">Something with that name is already registered.</exception>
        public   void Add(IDiagnosticOverlay
   overlay ) {
   ...
   // answering to one name is a toggle that flips whichever was registered first and silently
    // ignores the other — a bug that only shows up as "the console command does nothing".
     if (Find(overlay.Name ) is not
        null   )    {
         throw new
ArgumentException(  $"An overlay called '{overlay.Name}' is already registered.", nameof(overlay)) ;
        }
      overlays
    .Add(overlay);
   ...
   public void Request    (IEnumerable    <string >    ? names) {
    ...
    public
          ...
          var overlay =    Find   (    name);
           return overlay is not null  && overlays   .Remove
        (overlay);
    }
    ...
    /// <returns>The overlay, or <see langword="null" />.</returns>
                public IDiagnosticOverlay? Find  (string? name) { if (string  .IsNullOrEmpty   (name))  {
            return null ;
        }
               foreach ( var overlay in    overlays)  { if (   string.Equals(overlay.   Name, name, StringComparison. OrdinalIgnoreCase) )  {
                return overlay    ;
                } }   
 return null;
              }
...
name)   ;
  ...
  return null
   ;
    ...
    overlay.Enabled =    enabled ?? !    overlay.   Enabled;  return overlay.Enabled;
       }
    ...
    /// </remarks>
       public void
        RegisterCommands    (ConsoleCommands commands) {  
                ArgumentNullException.ThrowIfNull  (   commands  )   ;
       commands.Register  (
               "overlays"  , "Lists the diagnostic overlays and whether each is on",
            entry =>  {
   ...
   "Turns one on or off: overlay <name> [on|off]",
      ...
      entry    .    Write  ("overlay <name> [on|off]" );
		return;    }
  var name  = entry[    0    ]!;
                var state =   entry    .TryFlag  (   1   , out var wanted
           )    ?    wanted  :  (   bool  ?) null;
               var result = Set(name, state)  ;
        entry.Write    ( result is null
    ...
    /// <summary>Turns every overlay off.</summary>
             public void DisableAll
                () {
       foreach (var overlay   in    overlays) {  overlay    .Enabled   = false;
               }
    }
...
Vector2 viewport, in GameTime    time    )   {
     ArgumentNullException   
        .ThrowIfNull    (    draw);
  DrawnCount    = 0;
      if ( !Enabled || !  draw    .Enabled    || viewport.    X <= 0f || viewport.Y <= 0f)   { return;
