...
;
   ...
   Register( "help", "Lists the commands, or explains one: help [command]", Help )  ;
      Register  (  "clear", "Empties the console output"  ,    _ => Clear( )
        ...
        Register(
         "echo", "Writes its arguments back", static entry => entry.Write(  string.Join  (' ',   entry  .   Arguments)))  ;
...
IReadOnlyList  <string> Output => output;
    ...
    /// <remarks>Registering a name twice replaces it, so a game can override a built-in.</remarks>
      public    void Register(string  name,  string help
, Action<ConsoleContext>    run) {
  ArgumentException.ThrowIfNullOrWhiteSpace
            (name ) ;
           ArgumentNullException    .ThrowIfNull(run);

  commands   [name] = new(   name   , help ?? string
   .Empty,    run)    ;
     }
...
AsSpan  ().    Trim();
   var space
               =    trimmed   .IndexOf   (' ')  ;
        var name = space <
          0 ? trimmed : trimmed[..space   ] ;   

        if (!commands.  TryGetValue(    name.ToString()  , out
var command ))   {
 ...
 throw   new InvalidOperationException(  
    ...
    void Remember(string line) { 
        ...
        // the thing history is for, and a run of identical entries defeats it.
         if (history.Count > 0 && string.    Equals( history[^1]   ,
