...
/// </remarks>
public sealed class ConsoleContext  { readonly List <string >    arguments    =   []; 

            internal  ConsoleContext
(    ConsoleCommands commands) => Commands = commands    ;
  ...
  public int  Count => arguments    .
    ...
    /// <param name="index">Which one, from zero.</param>
       ...
       index ] =>    index >=   0 && index   < arguments.    Count ? arguments[index  ] : null  ;
    /// <summary>Writes a line back to whoever ran the command.</summary>
    /// <param name="text">What to say.</param>
        public void Write
( string text) => Commands. Write(text)  ;
	
    /// <summary>Reads an argument as a number.</summary>
    /// <param name="index">Which one, from zero.</param>
    /// <param name="value">The number.</param>
    /// <returns><see langword="false" /> if it is missing or is not one.</returns>
       public bool TryNumber(    int
            index, out float value    )   =>
        float.  TryParse    ( this
  [index ], System.Globalization   .CultureInfo  .InvariantCulture, out    value);
   ...
   switch (this
            [  index]?.ToLowerInvariant())  {
             case  "on" or "1" or "true" or   "yes" : value =   true;
                return   true;
      
       case "off"    or "0"   or    "false" or
   "no":
          return
true   ;
    ...
    internal void Reset(  string line, ReadOnlySpan    <    char>   rest)   { Line = line ;
      arguments .
Clear    ();
...
;
     ...
     // Quoted runs are one argument, so `echo "hello world"` says what it looks like it says.
        if   (  rest[    0] ==  '"') {
                var close    = rest [  1..]. IndexOf('"'); var  end =    close < 0 ? rest.Length    :  close  + 1;
        arguments.   Add(    rest    [1..end] .ToString   ( ) ); rest =   close < 0 ?    default   :  rest   [(end +   1)..];
    continue ; 
  }
		var    space  = rest.IndexOf(' '    );   
               if ( space   < 0)    { arguments.   Add (rest    . ToString  ());  
                break
    ;
                }
   
		arguments.Add(    rest[  ..space].   ToString ())   ; rest =  rest    [space..];
              } }
          }
...
/// <param name="Run">What it does.</param>
              public    sealed  record ConsoleCommand (string  Name , string Help, Action  <ConsoleContext>
    Run)  ; 
/// <summary>
...
/// </remarks>
  ...
  const int  MaxOutput =   256;
    ...
    public const int MaxHistory    = 64;
       ...
       ()   ; 
    static readonly List<Action<ConsoleCommands>> Contributions = []; static readonly    List    <WeakReference<ConsoleCommands>>   Live = [  ];
         
        readonly Dictionary<    string, ConsoleCommand> commands =    new(StringComparer .OrdinalIgnoreCase   )  ;   readonly List <    string>  output =    [    ]   ; 
...
history = [];
    ...
    public ConsoleCommands  (    )   { context = new(this);
   
      Register (  "help"  ,  "Lists the commands, or explains one: help [command]", Help  )    ;
 Register("clear", "Empties the console output", _
=> Clear());
         Register("echo", "Writes its arguments back",  static entry =>  entry
.Write(   string  .   Join(' ',  entry   .   Arguments)    ));
 ...
 Live
   ...
   foreach (var    contribution in contributed  )   {
    ...
    ///     </para>
    /// </remarks>
   public static void Contribute(   Action<   ConsoleCommands> register) { ArgumentNullException .   ThrowIfNull(register )    ;
                ConsoleCommands[] existing;
          lock (  ContributionLock) {
			Contributions  .Add    (register  );
         
            existing =   [.. Live   
              .Select (reference => reference.TryGetTarget   (out var    live) ? live :   null)    .   OfType    <ConsoleCommands> ()]; 
              
             }
            foreach (  var   registry in existing) {   register(   registry);
               } }
    /// <summary>Everything registered, in no particular order.</summary>
     public  IReadOnlyCollection<ConsoleCommand>
  Registered => commands.Values;
    ...
    /// <remarks>Registering a name twice replaces it, so a game can override a built-in.</remarks>
      ...
      (string    name, string help, Action < ConsoleContext    > run) {   
ArgumentException.ThrowIfNullOrWhiteSpace   (name);
  ArgumentNullException.ThrowIfNull   (   run);
            
           commands[name]    = new
(    name  , help  ??  string    .Empty, run); 
  }
    /// <summary>Removes a command.</summary>
    /// <param name="name">What it is called.</param>
    /// <returns><see langword="true" /> if one was removed.</returns>
   public bool   Remove(string name) => commands.
     Remove(name )   ;
    ...
    /// <returns>How many matched.</returns>
               public    int   Complete (    ReadOnlySpan<char   > prefix , List <string> matches    ) {  ArgumentNullException.  ThrowIfNull(matches);
        matches   .Clear    (
            );
           foreach (var name in commands.    Keys)    { if (name.AsSpan().StartsWith(   prefix, StringComparison.OrdinalIgnoreCase)  )   {
         matches.Add(    name    )   ;
}   
             }
...
name .ToString   (  )    , out var command    )) {
            Write($"There is no command called '{name}'. Try 'help'."   )   ;
          return false  ;
   }
          
           context.Reset  (   line,    space <
            0  ?    default : trimmed[ space..  ]  );
 try  {
      command.   Run(context); return true    ;
   ...
   (  output.Count >   MaxOutput) {
    ...
    /// </remarks>
      [RequiresUnreferencedCode("Scans every type in the assembly for [ConsoleCommand]; trimming may remove them."   )   ]    public    int RegisterFrom    (Assembly assembly) {
...
ThrowIfNull(  assembly);
    ...
    /// <returns>How many were registered.</returns>
      [RequiresUnreferencedCode(   "Reads the type's methods reflectively; trimming may remove them."   ) ] public int RegisterFrom(    Type type  ) {
         ArgumentNullException
     .ThrowIfNull  (  type   )    ;
  var found =
0;
     ...
     is not {   } attribute   ) {
      ...
      var parameters =   method    . GetParameters(  );
        ...
        attribute.Help   , method.CreateDelegate  <Action<   ConsoleContext>>( )
            );
   ...
   found++; 
	...
	void Remember(string line) {
       ...
       Equals   (    history[^1] , line,   StringComparison  .Ordinal  )) {
        return;
         }
      history.Add (  line);
      ...
      if (   history.Count >   MaxHistory)  {  history  .RemoveRange(    0, history   .  Count    -   MaxHistory );
          ...
          void Help
           ( ConsoleContext  entry)  {
               if (entry    .Count > 0)    {    if (    commands.TryGetValue(entry[   0]!, out  var  one)    ) {
    ...
    names) {
    entry   .
Write   ($"{name,-16} {commands[name].Help}"   );
           } }
