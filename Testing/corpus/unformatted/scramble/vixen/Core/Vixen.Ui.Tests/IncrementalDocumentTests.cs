...
using   System.Globalization;
        using  CsCheck;
     using Vixen.Ui.Styling; using Xunit;  
...
/// </remarks>
    ...
    [Fact]  public    void    An_incremental_pass_produces_exactly_what_a_cold_one_produces() {   
       var incrementalPasses = 0;    var styledElements =  0;
     var stateMutations
= 0    ;
        Gen.Select(Gen  .   Int  [ 0,
              100_000    ], Gen.    Int   [2, 4 ],   Gen.Int[2  , 3], Gen   .   Int[ 4, 12]).    Sample   (    shape => {  
               var  (seed, depth,   breadth, mutations)
               =    shape;
               var css    = Stylesheet(seed); var random = new Random(   seed ^ 0x51D2   );
using var    live = new UiDocument(   400f    , 300f);
              live
 .    Load(    css)   ;
        var elements = Build
            ( live, seed,   depth, breadth )   ;

                // The baseline pass. Everything after it must be incremental.
			
      live.Update  ();
          
               for ( var    m = 0;    m <   mutations; m++)    { stateMutations  += Mutate(    elements,   random    ) ? 1    : 0    ;  
 }
   live.   Update
               (  );
...
elements,  reference)   ;
 ...
 Assert.  True(  cold.LastPassWasCold   );
             ...
             for    (var   
 i    = 0 ; i  <  elements.Count  ; i++) {
             styledElements +=    elements[i  ]  .
      Style.Count > 0 ?  1 :    0;
          
                    Assert    .Equal(
           Describe(cold ,    reference[ i
]    .   Style   )   ,
...
.Load("""
        ...
        for (var r = 0    ;    r < 100;  r++)   { rows[ r  ] = document.Root    . Add(  "div",   classNames   : "row"  )   ;
              for ( var  c = 0    ; c < 100  ; c++) {
   rows[r].Add("div", classNames  : "cell") ; }
         ...
         document
     .Update () ;
               Assert.True(  document.   LastPassWasCold); Assert   .  Equal  (  10_101, document .StylesResolved)   ;
...
, document.StylesResolved  )   ;
    ...
    public void A_scroll_resolves_nothing_at_all(
        ...
        var box = document.Root
               .Add("div", classNames : "box"  );
      
        document.  Update   ()    ;
        box.    OffsetY
=    - 40f;
 
             Assert.    True(document.Update()   );  Assert. Equal(    0   ,   document .    StylesResolved);
       Assert    .
Equal(0,  document    .StylesApplied   )   ;   
     // And it really did move — a pass that resolved nothing is only free if it still did its job.
        Assert.Equal  (-40f  , box.AbsoluteTop, 0.001f    );  }
               ...
               [Fact]
            public void An_inline_style_survives_an_incremental_pass( ) { // The bug wiring this up found. `StyleUpdater.Resolve` did not pass the element's inline
...
"""
        ...
        var box = document.   Root   .   Add   ("div", classNames
           : "box")   ;
      box.SetStyle("width",    "137px")   ; document.Update    ();
Assert
               .Equal(  137f, box. Width    , 0.001f);
  // Now force the incremental path and check the inline block is still there.
 box.AddClass("marked"   )    ;
    document.  Update  ();
Assert.False(document   .LastPassWasCold); Assert.    Equal(  137f , box.   Width ,  0.001f)    ;
              }
  ...
  document
          .Load("root { width: 400px; }");
           
         var  box    = document  .Root.Add("div");
            document.Update(   )   ;
       // A class change is narrow.
        box.    AddClass   ("a");
       document .Update( );  Assert.False(document  .LastPassWasCold  );
    ...
    var fresh =  document.Root.Add("div");
        document.Update( ); Assert.True(document  .LastPassWasCold   );  
             Assert.NotSame(ComputedStyle.    Empty,
                fresh.Style);
 // Nor is an inline style, nor a move, nor a stylesheet.
   ...
   document  .Update  (    )   ;   
             Assert
             . False (document.LastPassWasCold)   ;
...
"""
        ...
        """);
            ...
            var first   = document.Root. Add  ("div"   )   ; var second = document.    Root.Add(  "div");
var third = document .    Root.Add  (  "div"    );    document.   Update();
     // Three changes, one pass. A record that kept only the last would leave two of them unstyled
              // and look completely correct on any test that made one change at a time.
                first    .    AddClass
            ("marked");
    second .AddClass("lit")    ; third.    AddClass("marked");  
	document    .  Update  ()   ;
        
        Assert.False(document .LastPassWasCold);
     Assert. Equal    (33f , first.Width, 0.001f)   ;  Assert.   Equal(    21f,   second.   Height, 0.001f  );
           Assert.Equal
(33f,  third .Width, 0.001f);
            }
         [Fact] public    void    An_ancestors_state_still_reaches_its_descendants()   {
    ...
    // pass scoping is `StyleUpdater`'s to get right rather than `StyleEngine.ResolveAll`'s.
     using var document = new UiDocument
		(400f, 300f   );
        document.Load("""
            root { width: 400px; height: 300px; }
...
;
    ...
    /// <summary>A stylesheet whose rules reach in every direction a change has to be chased.</summary>
                static string Stylesheet(int    seed)   {   var    random  = new Random  (seed) ;
var  css  =   new System
      . Text.StringBuilder("root { width: 400px; height: 300px; }\n");
            for   (   var i =    0   ; i  < 6 ;  i    ++) { var left =  Names    [   random.Next(   Names .Length)   ];
         var right = Names[random
.Next(Names   .Length   )];
 var width   =
        (i + 2) * 3 ;   
            css  .   Append(
    ...
    /// </remarks>
             static List<UiElement>    Build(  UiDocument document    , int seed,   int depth, int breadth  ) { var  elements    = new List <  UiElement> () ;
         Descend(document.
  Root , depth, breadth, elements, new   Random(seed  ^ 0x2B0D    ));
        return elements; }
    static void   Descend  (
UiElement parent  , int depth, int breadth, List<   UiElement>   elements, Random  random) {
            if   (depth == 0) { return    ;
           }
...
var child =  parent.   Add("div");
            ...
            for (var n =  random.Next(3); n
             > 0; n--) { 
                child.    AddClass(Names[random. Next(Names.Length)] ); }
              
       elements.Add
            (child);
    Descend(child , depth -    1    , breadth,
          elements, random);
        } }
          ...
          static    bool    Mutate  (List<UiElement> elements, Random    random)
{
        ...
        if (random.Next(3)  ==   0   )  { element.State ^= ElementState  .Hover;
		return true;
        }
  var 
   ...
   return false;
    ...
    static void Copy   (
            List<UiElement> from    , List<    UiElement   > to    ) {
        for (var i =    0 ; i < from
              .  Count; i++)   {
foreach  (var    name in Names )    {
                if (from[ i  ].HasClass (  name))   { to[    i    ] .AddClass(  name);
           }
 else {
  ...
  to[i].State = from[  i].State; }
    ...
    /// </remarks>
     static string Describe(UiDocument document,    ComputedStyle style) { var    text =  new System.Text .StringBuilder   (  );  
         
           for (var i    = 0; i < style.Count; i++) { text   .Append(document.  Styles   .Properties.NameOf(style    .Properties[i]   ))
 . Append( '=') .Append(document   .Styles    .  Values  . NameOf   (style.Values  [i]))
  .   Append(';');
  }
