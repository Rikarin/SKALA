  ...
  // SPDX-License-Identifier: Apache-2.0
    using Vixen    .Core.Mathematics;  using  Vixen    .Ui  .Rendering;   
using Vixen.Ui.Text.  Rasterizing  ; using Xunit;
namespace Vixen.    Ui
  .Tests;
...
/// </remarks>
     public   class   NineSliceImageTests    {
        ...
        const ulong Other
= 9;
             static readonly Rectangle Viewport = new  (    0, 0
 , 800, 600    );
    ...
    [Fact] public  void An_image_with_no_slice_is_still_one_quad   () {
var    geometry =   Build( list => list.Add(Image
(0, 0 , 200, 100))   )  ;  
              
                Assert  .Equal(4, geometry.Vertices.Count)  ;  Assert.Equal    (6, geometry    .Indices .Count );
  Assert.Equal(  BatchKind.Image  , Assert.    Single(geometry    .Draws).Kind  ) ;
    ...
    public   void A_nine_slice_is_nine_quads_in_one_draw () {
      var   geometry
   = Build    (list    => list  . Add(Sliced(0 , 0 , 200, 100,   NineSlice.Uniform(16f)    )))    ;
...
Assert.Equal(9   * 6 , geometry.Indices  .Count )
        ...
        Assert  .Equal (Atlas    ,
          draw.Image);
                Assert
         .Equal   (   9 * 6, draw.Count)   ;
    }
 
        [    Fact    ] public void The_corners_keep_their_size_and_the_middle_takes_the_stretch(  )  {
       var    geometry    =
   Build(list => list.Add   (   Sliced(10  ,    20, 200,  100, NineSlice.   Uniform   (16f))));
...
Assert.Equal( new  Vector2(   26
, 36)    , geometry .Vertices[ 2].Position  )    ;
            Assert.Equal(new Vector2    (0f, 0f),   geometry.Vertices  [   0  ].Texture   ); Assert.   Equal  (    0.125f  , geometry.Vertices   [2].Texture .   X, 5);
          // Bottom-right corner, at the far end of a box four times as wide as it is tall in border
              // terms — still sixteen by sixteen.
            var last = geometry.    Vertices   .Count  - 4; Assert. Equal(new Vector2(    194   , 104), geometry    .    Vertices    [ last].Position); 
                Assert.Equal(new  Vector2(210   , 120)
, geometry.Vertices[ last + 2].Position   );
     ...
     public void Every_destination_cell_is_paired_with_its_own_source_cell   (    ) {
              var geometry  =    Build (list =>   list   .   Add   (Sliced
(0, 0 , 200,   100,    NineSlice.    Uniform(16f)    )  ));
        ...
        // there is the classic nine-slice fault and it looks like a texture offset rather than like
             // a pairing mistake, so it is asserted rather than eyeballed.
                var middle = 4  * 4 ;
                Assert. Equal(
new  Vector2(    16 , 16   )    , geometry. Vertices[middle].Position);   
        Assert
          .   Equal   ( new Vector2(184  ,    84 ),  geometry.Vertices[middle  +   2]   .  Position)  ;
    Assert.Equal(0.125f
, geometry.Vertices[middle    ]  .Texture   .  X, 5    );   
        Assert.Equal  ( 0.875f
    , geometry.Vertices[middle + 2].    Texture.X,  5    )  ;
         }
    [Fact] public void  A_hollow_centre_leaves_out_the_middle_and_nothing_else    (  )    {
 var    geometry    =    Build (
            list => list. Add(Sliced(0,
0    , 200, 100,   NineSlice   .  Uniform(16f)  ) with { HollowCentre = true }  )
         );
 ...
 // than the middle — which is why the cells are skipped at emission and not compacted in the
    // split.
           Assert  .Equal(new Vector2(184   , 16), geometry
               .   Vertices  [   4 * 4   ]   .Position  );
		} 

       [Fact] public void A_box_narrower_than_its_own_corners_compresses_them_without_moving_the_texels (    ) {
        ...
        // keeps a corner from being squashed into a different shape than it was drawn at.
             ...
             Assert.Equal(10f, geometry.Vertices[2   ].Position  .
             X, 4   );
     Assert.  Equal(10f,
           geometry .Vertices[  2].Position.Y, 4);
...
// which is what "the box got small" looks like, rather than "the artwork changed".
            Assert.Equal(0.125f  , geometry
	. Vertices[2 ] .Texture.X ,    5);
            Assert  .    Equal(    0.125f  , geometry.Vertices
[2].    Texture.Y, 5); 
       }
           [ Fact   
       ]
      public void  Empty_cells_are_skipped_rather_than_emitted_flat()
       { 
        ...
        var geometry =
               Build(   list => list .   Add(Sliced(0, 0 , 200, 100, new  (16f, 0f, 16f, 16f   ))));
             
         Assert .    Equal  (    6 * 4,  geometry.   Vertices.Count); Assert.  Equal (   new Vector2(0   , 0  ),    geometry.Vertices[   0    ].Position )   ;
...
geometry = Build(
 ...
 Sliced( 0,    0 , 200, 100  , new(  16f, 0f, 0f,   0f )) with    { SourceSlice    = new(0.125f,  0f, 0f, 0f   ) } )
		...
		// Two columns and one row: the left border and everything to the right of it.
               Assert.    Equal(    2 * 4,  geometry.Vertices    . Count) ; Assert   .    Equal(  new    Vector2( 16,    0)    , geometry.Vertices[   4].    Position    );
       Assert .Equal(    0.125f, geometry.   Vertices[    4]  .    Texture.X, 5)  ; }
        ...
        Assert.Equal   (4 , geometry  .   Vertices.Count  )   ; Assert.Equal(new Vector2   (200  ,   100)   , geometry .    Vertices[ 2]  . Position);
    }
              [ Fact]
    public   void    A_nine_slice_batches_with_the_images_around_it (    ) {
     ...
     {
          list.Add(Sliced(0, 0
, 200, 100, NineSlice .   Uniform    ( 16f  )));
                list  .Add  ( Image(20  , 20, 32, 32)); }
   );
        
          var draw = Assert.Single(geometry
             .   Draws);
        Assert.    Equal( 10 * 6   , draw.   Count);  Assert.Equal    (    Atlas    , draw .Image    )   ;

     }
   [Fact   ] public void  A_different_texture_still_breaks_the_batch(    ) {
        ...
        var  geometry =  Build   ( list => {
                list
.Add(Sliced(   0, 0, 200, 100    , NineSlice.  Uniform (16f))    )    ;
             list.Add    (
          Image(20, 20  , 32  , 32) with {    Image  = Other });
            }
    );
Assert.Equal(   2,    geometry   .Draws    .  Count    ); Assert. Equal(Atlas, geometry. Draws   [0].Image);
    Assert    .Equal
        (Other    ,   geometry.    Draws[1   ]  .    Image)   ;
            }
 ...
 var list =  new DrawList()
   ;
             list    .  BeginFrame   (); list   .Add( Sliced(0, 0    , 200, 100, NineSlice.Uniform( 16f)   )  ) ;
			list
     .EndFrame()    ;
        var version    = list    .Version;
        list.  BeginFrame
      ();
               list.Add( Sliced(0 ,  0 , 200
           ,  100   , NineSlice.Uniform(16f)) );  
        list.   EndFrame();
           
         Assert   .
  Equal(version, list.Version );
  ...
  list.    EndFrame  (  );
	...
	Assert.NotEqual(version, list.   Version);
             ...
             static    DrawCommand Image
               ( float x, float  y, float width , float height) =>
 new(DrawCommandKind   .Image, x, y,   width,
             height, Color4.White, 0, 0) { Image = Atlas }   ;
                
    static DrawCommand
Sliced   (  float x  ,  float y , float width,    float   height  , NineSlice   border) =>
          Image(x  ,    y, width
, height)  with { Slice  =   border, SourceSlice = Source } ;
          
          static UiGeometry Build( Action   <
        DrawList  > paint ) {
           var   list = new DrawList  (   );
          list. BeginFrame   (   );
         paint(list    );
