...
public class LayoutStyleBridgeTests {
    ...
    public void An_element_with_no_declarations_gets_CSS_initial_values_and_not_Yoga_s() {
        ...
        Assert.NotEqual(
            LayoutStyle.Default.PositionType,
            style
                .PositionType
        );
    }
    ...
    public void A_relative_length_is_resolved_against_its_context(string css, float expected) {
        ...
        Assert.Equal(
            LayoutUnit.Point,
            style.Dimensions[(int)Dimension.Width]
                .Unit
        );
    ...
    public
        ...
        var style = new BridgeFixture().Build(
            "width: 1.5em",
            LengthContext
                .ForViewport(1000f, 500f)
                .WithFontSize(20f)
        );
        Assert.Equal(
            30f,
            style
                .Dimensions[(int)Dimension.Width].Value,
            Tolerance
        );
    }
    ...
    static LayoutSize MeasureThreeWords(in MeasureRequest request) {
        ...
        return new LayoutSize(
            ...
            == MeasureMode.Exactly
                ...
                : MathF.Ceiling(words / perLine) * 10f
        );
    }
    ...
    public void Rem_ignores_the_element_s_own_font_size(
        ...
        Assert.Equal(
            80f,
            style.Dimensions[(int)Dimension
                .Height].Value,
            Tolerance
        );
    }
    ...
    public void A_keyword_the_bridge_does_not_know_leaves_the_initial_value_alone() {
        ...
        Assert.Equal(
            LayoutStyleBuilder.CssInitial.JustifyContent,
            style.
            JustifyContent
        );
    ...
    public void Gap_puts_the_row_first_because_that_is_what_CSS_says() {
        ...
        Assert.Equal(
            1f,
            longhand.
            Gap[(int)Gutter.Row].Value,
            Tolerance
        );
        Assert.Equal(
            2f,
            longhand.Gap[(int)Gutter
                .Column].Value,
            Tolerance
        );
    }
    ...
    public void Inset_is_the_one_shorthand_the_bridge_expands_itself() {
        ...
        Assert.Equal(
            4f,
            four.Position[(int)Edge.Left].Value,
            Tolerance
        );
        ...
        Assert.Equal(
            5f,
            two.
            Position[(int)Edge.Vertical].Value,
            Tolerance
        );
        Assert.Equal(
            6f,
            two.
            Position[(int)Edge.Horizontal].Value,
            Tolerance
        );
    }
}
