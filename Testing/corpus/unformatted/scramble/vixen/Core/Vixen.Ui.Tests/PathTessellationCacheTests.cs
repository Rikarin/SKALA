		...
		// SPDX-License-Identifier: Apache-2.0
              ...
              using Vixen .Ui; using Vixen    .Ui.Rendering   ;
...
new PathBuilder()
      . MoveTo   (new Vector2    ( 20,  20)   ) .LineTo(new Vector2    (  120, 30  ) )
.CubicTo ( new
        Vector2(  140, 80 ),   new Vector2(60 , 120),   new Vector2   ( 20,    90) )
            .  Close();
     
  [Fact]
            public
void The_same_path_drawn_again_is_not_tessellated_again  (   ) {
                var builder = new    UiGeometryBuilder(); var    glyphs = Cache(   )   ;
		builder   .Build( Drawn  (
Blob   (    ) , Color4  .White)  , glyphs, Viewport);
...
builder .Build(Drawn   (Blob()
    , Color4 .White), glyphs, Viewport)  ;
...
= Snapshot  (builder.Build    (Drawn  (Blob(), Color4.White    ), glyphs, Viewport));
    ...
    ///     down a list.
    /// </summary>
     [Fact ] public void Recolouring_a_path_reuses_its_triangles_and_still_changes_the_colour (   ) {
           var  builder   =  new UiGeometryBuilder
          ();
        var glyphs =   Cache( );
            
			var   pale = builder.Build   (  Drawn
(    Blob(),  Color4    .    White), glyphs  , Viewport    );
               var paleColour    =   pale.
         Vertices[0    ].   Color;  
               var deep = builder    . Build
(  Drawn(Blob    (), new  Color4(   1f,   0f   ,  0f, 1f)),    glyphs,   Viewport   );
            
    Assert .Equal   ( 0    , builder.  TessellatedPaths  );  Assert.   NotEqual   (paleColour   , deep. Vertices[0  ]  .    Color  ) ;
             Assert.   Equal(new Color4  ( 
            1f, 0f, 0f, 1f), deep.Vertices[0].Color  );
   }
		[
               Fact]
 public  void  A_path_whose_geometry_moved_is_tessellated_again() {    var builder = new UiGeometryBuilder  ();
            var glyphs = Cache( );
 builder . Build  (  Drawn
          (Blob(    ),    Color4.White ), glyphs ,   Viewport);
...
LineTo    (new Vector2   (120, 30))  
            .CubicTo (new Vector2(140  , 80),
new Vector2(60, 120 ),  new Vector2(20   , 90  ))
         .Close();
		builder.   Build(Drawn(moved , Color4 .    White   ) , glyphs, Viewport)
;
             
              Assert.  Equal( 1, builder.    TessellatedPaths ); Assert.Equal(2, builder.CachedPaths);
    }
    ...
    /// </summary>
              [Fact]
    public void A_stroke_that_changed_width_is_tessellated_again()    { var builder = new UiGeometryBuilder ()    ;
    var glyphs = Cache();
       builder.Build(Stroked(Blob(  ), 2f   )    , glyphs  , Viewport)    ;
        builder   .
               Build   (Stroked   (Blob(   )    , 6f   ), glyphs    , Viewport   );
    ...
    /// </summary>
          ...
          [  Fact]
      public void Changing_the_fringe_tessellates_again() {  var builder = new UiGeometryBuilder();
var glyphs = Cache()  ;

         builder.   Build(Drawn
                (Blob(  ), Color4.   White), glyphs, Viewport   )   ;
       builder.    Fringe =  0f; builder.   Build(Drawn(Blob(), Color4.White), glyphs    , Viewport);
...
Fact]
   ...
   for (   var
         ...
         .    LineTo    (new Vector2    (120    , 30)) .    LineTo    (new  Vector2(    20 , 90   )    )
                .Close(    )
 ;   
  builder  .Build(Drawn(  moved
,   Color4.White),    glyphs   , Viewport);
        }   
 ...
 .MoveTo(new Vector2( 27, 20
   ...
   static DrawList Drawn
             (PathBuilder path, Color4 color)  {
             var list =   new
DrawList   (    )  ;   
    ...
    ,  
                Length = path.Count  ,  FillRule = PathFillRule.NonZero
  }
        )   ;
         
           list.EndFrame(    );   return list ;
       }

  static DrawList Stroked  (
            PathBuilder path,   float thickness    )  {
...
Offset   =
           list.AddPath(path),
                Length = path. Count    ,
              Join    = LineJoin.Miter  , Cap = LineCap. Butt
           }
       )   ;
