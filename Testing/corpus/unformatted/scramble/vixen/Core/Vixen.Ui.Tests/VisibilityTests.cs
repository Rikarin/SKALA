...
///     </para>
/// </remarks>
           public class VisibilityTests {   const float Tolerance = 0.001f ;   
   static UiDocument Drawn(   string css, Action    <UiDocument  >
           build    ) {
 ...
 Update( );   
  ...
  document
      ...
      var command =
         ...
         Assert.    Equal(  DrawCommandKind.  Rectangle
                , command   .Kind);
            
 var    box = document   .Root. Children  [  0];   Assert.    Equal(  60f,    box .Width, Tolerance);
    ...
    /// </summary>
   [Fact] public void Hidden_keeps_the_rectangle_in_layout_and_takes_it_out_of_the_draw_list()   {
            ...
            """,
             ...
             document => {
     document    .Root   . Add  ("div",   classNames: "box"); document.Root   .   Add(    "div", classNames: "after");
          } );
                // The hidden box paints nothing; its sibling still paints.
        var command    = Assert.  Single( document .Drawing   .Commands)  ;
         Assert    .Equal   (10f, command.Width,
		Tolerance  )    ;
        ...
        // `visibility: hidden`, which is the entire difference between the two properties.
            var box = document.  Root. Children[0]; Assert  .   Equal(60f,   box.Width,  Tolerance);
        Assert  .  Equal  (40f, box.Height, Tolerance) ; Assert.Equal    (40f, document.Root.Children[1]    .    AbsoluteTop, Tolerance    )    ;
        }
    /// <summary>
    ...
    /// </summary>
  [Fact ]
   public void Collapse_reads_as_hidden_because_there_are_no_table_rows_to_mean_anything_else() { using var document = Drawn   (
            """
            root { width: 400px; height: 300px; }
 ...
 Assert. Empty(document   .Drawing    .Commands);
   Assert.Equal  (40f,   document.    Root .
     Children[  0] .Height , Tolerance);
    }
...
[Fact ]
    ...
    using  var document = Drawn  (
            ...
            """   ,
       document    => document.Root.Add("div"
     , classNames : "parent")   .Add  ("div"   ,    classNames : "child")
) ;
        var command
           = Assert.Single(document  .   Drawing.   Commands);
    Assert.Equal  (  20f   ,    command.Width,    Tolerance    )   ; Assert.Equal(15f  ,  command .Height , Tolerance    );
        }
      
    /// <summary>Hiding is inherited, so a subtree goes with its root.</summary>
		[Fact   ]
            public    void A_hidden_parent_hides_a_child_that_says_nothing    (    )    { using var   document = Drawn(
            """
            root { width: 400px; height: 300px; }
            .parent { width: 100px; height: 80px; background-color: #ff0000; visibility: hidden; }
            .child { width: 20px; height: 15px; background-color: #0000ff; }
            """,
   document =>    document.Root.Add( "div", classNames   : "parent"  ).
             Add(  "div"    , classNames: "child" )
 )  ;
             
       Assert
    .Empty    (document. Drawing.  Commands);
           }
    ...
    /// </summary>
  [    Fact]   public void A_hidden_box_does_not_catch_the_pointer_and_what_is_behind_it_does()   {
   using var document = Drawn(
            """
            root { width: 400px; height: 300px; }
...
Assert.True(under   . IsHitTestVisible);
    ...
    /// </summary>
           [Fact]
       public void A_visible_child_of_a_hidden_parent_is_still_a_pointer_target(   ) {    using    var    document    =    Drawn(
            """
            root { width: 400px; height: 300px; }
            ...
            Assert.False(parent.IsHitTestVisible);
               Assert
        .True(child.   IsHitTestVisible);
      Assert  .  Same(child,  document
   .HitTest(10f, 10f   ));
     }
