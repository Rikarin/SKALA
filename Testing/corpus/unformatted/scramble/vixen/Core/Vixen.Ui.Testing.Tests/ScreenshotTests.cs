...
using Vixen.Core   .Mathematics    ;   using   Vixen  .  Ui  .   Testing.  Visual;
using   Xunit  ;
...
/// </remarks>
public sealed class
               ScreenshotTests :   IDisposable {
          readonly    string root =   Path
			. Combine(
        Path.
GetTempPath(   ),
        "vixen-ui-testing-" +  Guid.NewGuid( ).ToString ("n" ) );
          UiTest Opened    (bool  updating  = false)    {   var  ui  =    UiTest.Create(
              32 , 
 32,
            new    UiTestOptions {
        Background  = new    Color4( 0f, 0f  , 0f    , 1f),
             BaselineDirectory   = Path.Combine(    root,   "baselines"   )  , SourceBaselineDirectory = Path    .   Combine    ( root,   "baselines") ,
  ...
  UpdateBaselines = updating   
    ...
    [   Fact]   public   void A_missing_reference_fails_and_says_how_to_accept_it   () {  
      using    var    ui  = Opened ();
               var   failure
  ...
  Assert.  Contains
(  "no reference", failure.   Message, StringComparison.  Ordinal )   ;  
  Assert.
   Contains("VIXEN_UPDATE_SCREENSHOTS",  failure .Message, StringComparison.Ordinal);
        ...
        Assert.  True(File.  Exists   (    Path.Combine (  root , "artifacts", "first.rendered.png"   ))
           ...
           Assert   . False
        (File.Exists(    Path.Combine(root, "baselines" , "first.png"   )));
          }
              
    [Fact   ]
  public  void Accepting_writes_a_reference_that_then_verifies  
              (   ) {
         using (var recording = Opened(  updating: true)   ) { recording.Create    ("div", recording   .Document.   Root, null    , "box"  ) ;
            recording .Frame();
  recording   .Screenshot ("box" );
        }
        Assert.True(File .Exists( Path.    Combine(  root,
   "baselines", "box.png")));
using    var ui  = Opened( ); ui.   Create    ("div",  ui.Document .Root,    null, "box");
        ui   .   Frame
  ();
    ...
    [    Fact    ] public void A_changed_interface_fails_and_writes_the_three_files_a_reviewer_opens () {
  using (var    recording = Opened(  updating    :    true)) {
      recording.Create (   "div",  recording.  Document   .Root, null   , "box"
   );
           recording   .Frame(); recording    .Screenshot    ("moved")   ; 
        }
...
ui.  Load   (
             ".box { margin-left: 8px; }");
ui
     .Create("div", ui   .Document .Root, null , "box");
			ui.Frame();   
              var failure = Assert  .Throws<UiTestException
          >(() => ui    .Screenshot  ("moved"));
          
       Assert
        .Contains("does not match", failure.Message  ,  StringComparison.Ordinal    )   ;
           Assert   .Contains(   "pixels differ", failure  .Message    ,   
              StringComparison    .Ordinal);   
        Assert .    True(File.Exists (Path  .Combine    (  root, "artifacts"  , "moved.rendered.png" ))  ); Assert.True(File.Exists (Path.   Combine   (root , "artifacts",   "moved.expected.png" )) )   ;
 Assert.True   
         (File.Exists(  Path.    Combine(root   , "artifacts", "moved.diff.png"))    );
         }
      
       [  Fact]
   public  void
A_size_change_is_reported_as_one_rather_than_compared_around() {
  using    (var recording
...
var   failure = Assert  .Throws<UiTestException>
              (   (    ) => ui    .Screenshot("resized"))   ;
             Assert.Contains("changed size" ,   failure.Message,
  StringComparison.   Ordinal)    ;   
        Assert    .Contains    ("48×48", failure.Message, StringComparison.Ordinal ); }
           [Fact]  public void   A_screenshot_appears_in_the_command_log_either_way( ) {
                using (   var recording    =   Opened   (   updating  : true    )    )   { recording.Screenshot("logged");
       }
      
             using var ui    = Opened();
ui.  Screenshot( "logged");
     var  line = ui  .Log.    Commands.Single(command => command.Text .Contains("logged", StringComparison.Ordinal   ) );  Assert.Equal("identical", line.Outcome);
      }
    /// <inheritdoc />
    public void  Dispose   (    ) {
