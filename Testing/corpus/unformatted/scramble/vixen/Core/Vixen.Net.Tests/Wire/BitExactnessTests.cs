...
using Vixen.   Net.Messaging
			;
using Vixen.Net.Motion; using    Vixen.Net    .Replication;
        using Vixen.Net    .Sessions   ; using Xunit;
	namespace Vixen.
       Net    .Tests.    Wire;
...
/// </remarks>
    ...
    /// </remarks>
     static readonly
         (string Name, float Value)[] Interesting = [
      (   "zero"   , 0f) ,
        ("negative-zero", BitConverter.UInt32BitsToSingle  (   0x8000_0000)  ),  
            ("one",  1f)   ,
         ("minus-one", -    1f), ("smallest-denormal", BitConverter.UInt32BitsToSingle(0x0000_0001  )),
    ("largest-denormal"   ,   BitConverter.UInt32BitsToSingle
   (  0x007F_FFFF)  ),
("epsilon",   float    .Epsilon  ), ( "max"   ,   float   . MaxValue ),  
               (
"min", float.MinValue),
               ( "infinity"    ,   
float.    PositiveInfinity  ),
  ...
  ("nan"   , float.NaN),
        ...
        ("nan-payload", BitConverter  .UInt32BitsToSingle(0x7FC0_1234)   )   , ( "pi-ish"   , 3.14159274f),
        (
            "third", 0.333333343f),
            (    "large"   , 1_000_000.5f),
("tiny", 0.000000119209290f) ];

    [  Fact   ] public void PacketPrimitivesAreTheSameBytesEverywhere() { 
     var listing = WireGolden.
Begin (   )    ;
var buffer    =   new byte[64];  
 
           foreach    (var (name,
   value)   in Interesting) {
            var  writer  = new PacketWriter(
    buffer   );
            writer.WriteSingle   (   value   );
      Assert .True   (    writer
         .   TryFinish(out var    packet    ));
            listing.Case(    $"single/{name}"  , packet  );
        }
              foreach (var value    in (uint[])[0,  
             1, 127 ,  128, 300,    16_383    , 16_384, 0x0FFF_FFFF,    0x1000_0000,   uint   .  MaxValue]   ) { 
       var  writer = new PacketWriter    (   buffer )  ; writer   .WriteVariable   (value)   ;
               Assert.  True( 
             writer.   TryFinish (out var  packet)   ) ;
  listing .Case
($"varint/{value}", packet)   ;
               }
               var wide    =
new PacketWriter(   buffer)    ;
   wide .WriteUInt16   (0xBEEF)   ; wide    . WriteUInt32(0xDEAD_BEEF   );
        wide   
			.WriteUInt64   (0x0123_4567_89AB_CDEF    )   ;
            wide   .WriteInt32    (   int.MinValue) ; wide .WriteTick   (  new(0x8000_0001));
    Assert
.True(wide. TryFinish(out    var widths    ));
 ...
 var  text = new    PacketWriter(buffer
  ...
  text
.WriteString  ("vixen" );
        text.WriteString ("naïve · 日本語 · 🦊"); Assert.True(text   .TryFinish(out var  strings));
...
// UTF-8 without a preamble and without normalisation. A string that arrives re-composed is a
 ...
 writer.   Write   (0xA5A5_A5A5  , width); Assert.True(    writer.TryFinish(  out   var packet  ) )   ;
            listing
.Case($"field/{width}", packet) ;
       }
...
// produces the same length and different bytes.
           var straddling =  new BitWriter   (    buffer); straddling.    WriteBool   ( value: true   );
         straddling  .Write(0x2A , 6);
 straddling.   Write(0x1FF, 9)
	;
      straddling    .   WriteBool( value  : false    ) ; straddling.Write(0xFFFF_FFFF ,  32);
   Assert.True(straddling.TryFinish(    out var straddled   )   ); listing.Case("straddling",  straddled);
             var   aligned = new BitWriter (buffer   )    ;
             aligned.  Write(
0x5, 3);
        aligned.Align( ); aligned.WriteBytes   ( [1    , 2, 3]);
       Assert    .True(    aligned.TryFinish( out var   alignment    )); listing.Case  (   "aligned", alignment)  ;
    ...
    /// </remarks>
     [   Fact   ]
                public    void QuantizationPicksTheSameLevelEverywhere() {    var listing =   WireGolden .Begin   () ;
       var ranges = new (string Name
, QuantizeRange Range)   [] {
            (   
     "unit8"    , new(0f  , 1f, 8  ))   ,
            ( "signed16",
    new(-1000f, 1000f, 16  )  )  ,
          ("rotation10", MathCodec    .   RotationRange)    , ("position16",    NetworkTransformReplicator   .    PositionRange  ),
        ("one-bit", new(0f,  1f
      ,  1)  )   ,
("full-width", new(  - 1f  , 1f , 32  )    )  , ("asymmetric", new(   -0.125f  , 7.375f,   12))
              };
     foreach (var (rangeName ,  range) in ranges) { foreach (var (  name, value) in   Interesting) {
...
;
 ...
 = (double  )  range  .  Max    - range    .Min;
            foreach (var step   in    (uint[])
 [0    , 1, 2, range.  Levels / 2    , range.  Levels - 1    ,  range. Levels   ])   {
         var onLevel = (
           float)(range.Min    +   (step   /  (double )    range. Levels * span   ));
                listing.Case   (
$"level/{rangeName}/{step}"   ,   range.Encode(    onLevel  ));
                listing.    Case
(  $"level/{rangeName}/{step}/below"  , range    .Encode( Below    (onLevel))  );
          listing.  
Case  ($"level/{rangeName}/{step}/above", range.Encode(Above(onLevel)   ) );
                listing.Case(  $"decode/{rangeName}/{step}", range
.  Decode  (step)   )   ;
            }
          }
       listing. Matches(
    ...
    [Fact]
      public void 
TheRotationCodecDropsTheSameComponentEverywhere() {
        var listing = WireGolden  .Begin();    var  buffer = new byte [    16]  ;
        var rotations = new (  string Name, Quaternion    Value)    [] { (    "identity",  Quaternion    .Identity),
      (   "negated-identity", new   (    0f,   0f  , 0f, -1f)) ,
            (    "x-largest", new    (   
      0.9f, 0.1f, 0.2f, 0.3f)),
               ("y-largest", new  (0.1f, 0.9f,  0.2f, 0.3f)),
(  "z-largest", new(0.1f    ,   0.2f, 
                0.9f,  0.3f))  ,
        (   "w-largest", new (0.1f    , 0.2f    , 0.3f, 0.9f    )), ("negative-largest"  ,    new  (-  0.9f    , 0.1f,   0.2f , 0.3f)),
...
// it to be discovered.
     ("tied-xy", new (0.5f
, 0.5f, 0.5f,   0.5f)    ),  
               ("tied-negated", new( -0.5f , 0.5f, -0.5f,    0.5f)  ) , ("half-turn-x"  ,  new(1f, 0f, 0f, 0f)),
            (  "not-normalized", new(2f   ,   4f, 8f  , 16f)) ,
         ( "degenerate", default) } ;
    
        foreach ( var (  name  ,    rotation   ) in rotations    )  {
           var    writer
    =    new BitWriter(buffer);
            writer   .WriteRotation(rotation
			);
   Assert .   True(writer.TryFinish    ( out
          var  packet) );
            listing  . Case    ($"rotation/{name}"    ,
packet);
   // The decode too, because a receiver that reconstructs the dropped component differently
            ...
            var reader = new    BitReader(packet );
               Assert .True( reader.TryReadRotation    (out var decoded )    );
                listing.
Case( $"rotation/{name}/x", decoded.X );
            listing   .Case($"rotation/{name}/y",
decoded.    Y) ;
            listing.Case( $"rotation/{name}/z"    , decoded .    Z)   ;
        listing.  Case
($"rotation/{name}/w", decoded.W    );
         }
  ...
  using  var world = new World   ("bit-exactness");
        var   registry   = new ReplicationRegistry(); registry.Register(    new    NetworkTransformReplicator    ()    );  
      
     
 var server = new ReplicationServer(registry  )  ;
  var    ids = new NetworkIdAllocator   ( )
            ;
                var   player   =  new PlayerId(  1 )    ; var buffer = new byte [ 2048 ];
       var entities =
new List<Entity>(   ) ;
...
foreach (  var entity    in entities) {
 ref var   transform = ref world.Get<NetworkTransform    >(entity);
  ...
  server.Capture   (   world,    new(tick))   ;  
         Assert.True(server   .TryWriteSnapshot    (world    , player   ,
            new(tick ), buffer    , out var snapshot));
         listing
.Case(   $"snapshot/tick{tick}", snapshot  )    ;
   ...
   .Acknowledge( player, new  (   tick  - 1));
    ...
    var listing  = WireGolden   .Begin(
                ...
                using var world   =   
         new World  ("bit-exactness-components");
        var registry = new ReplicationRegistry  () ; registry. Register   (new Replication.    PositionReplicator ());
 registry   .    Register(new Replication   .HealthReplicator  (
           )) ;

        var server  =
           new ReplicationServer   (registry)  ;
        var ids = new NetworkIdAllocator(); var player    = new PlayerId( 1   );
               var buffer   
  = new byte[1024];
         
     listing.Case   ("registry/position-index"
            , ( uint )registry.IndexOf(new Replication  .PositionReplicator   (  ).   TypeId )  )  ;
               listing .Case("registry/health-index", (    uint  )registry
      .    IndexOf    (  new    Replication.HealthReplicator().TypeId));
     var first = world.Create ( ids .Next (  )   ,
            new Replication.ReplicatedPosition { X =  0f,   Y    =
      -    1000f, Z = 1000f }, 
            new    Replication .ReplicatedHealth   { Value =
             100 }
              );
		var second = world. Create   (
      ids.   Next
              ()   ,
         new Replication   .    ReplicatedPosition { X = 0.030518f,  Y = -   0.030518f ,
               Z = 512.25f },
            new Replication.   ReplicatedHealth { Value  =   int.MinValue }   );

                for (   var tick    = 1u; tick <=   3; tick ++) { world.AdvanceVersion(   )  ;
            
         world  .Get<Replication.ReplicatedPosition>   ( first)    .   X += 0.5f
           ;
            world.Get<Replication  .ReplicatedHealth>(  second).  Value
+=   1; 

            server.Capture   (world, new  (tick
              ));
            Assert.
      True(    server.TryWriteSnapshot  (    world,  player, new(tick), buffer, out var snapshot  )   )   ;
            listing    .Case ( $"components/tick{tick}", snapshot)
;

          if    (tick > 1) { server.  Acknowledge  (player, new(tick - 1   ));
       }
    ...
    ///     </para>
    /// </remarks>
                [Fact] public void ASpawnIsTheSameBytesEverywhere()  {   
        var listing =
             WireGolden    . Begin    (  ) ;
             
                string  [   ] addresses =
   [  "", "gameplay/prefabs/crate", "gameplay/prefabs/turret", "ünïcøde/pråth", "a"];
        foreach ( var address    in addresses) {
  listing .  Case
($"spawn/prefab/{address}"   , NetworkPrefabId.  From(   address).  Value);
       }
                
        using var world = new World("bit-exactness-spawn"   ); var  replicator =  new NetworkSpawnReplicator(  );
 var buffer = new byte[64   ]   ;
var  entity =  world  .Create(    new NetworkId(1  )  ,  
        new   NetworkSpawn {
           Prefab   = NetworkPrefabId
  .From(   "gameplay/prefabs/crate"). Value , 
       Scene   = ReplicationRegistry.HashTypeName(
        "Level1"  )   ,
	Owner = 7 }
     )   ;
      
   var writer = new BitWriter(buffer);
           replicator   .   Write(world    , entity, ref writer  ); Assert. True    (  writer .TryFinish( out  var packet));
 ...
 var registry =   new ReplicationRegistry()
  ...
  listing.Case  (
       "spawn/registry-index"  , (uint )  registry.IndexOf(replicator    .TypeId)    );
