// SPDX-FileCopyrightText: Copyright (c) Rikarin
     // SPDX-License-Identifier: Apache-2.0
       ...
       using Vixen  .Engine    .Renderer  ; using Vixen.Graphics   ;
          using Vixen.Rendering;
    
    using Vixen  .Rendering.Lighting;
using Vixen.
Rendering.  PostFx   ;   
           using Vixen.Shaders  ; using  Vixen   .Shaders.Generated;
     namespace Vixen.Samples
             .ThirdPersonShooter;
...
const   int Levels
 =   5 ;
    ...
    TextureHandle opaque  ; 
               TextureViewHandle opaqueView ; SamplerHandle   opaqueSampler;
           BufferHandle bindPose;  bool disposed;
 ArenaFrame( IGraphicsDevice graphics, EnvironmentTexture texture
, ShCoefficients irradiance,   SkyParameters sky  ) {
            device = graphics;
      Sky = texture;
   Irradiance = irradiance; Atmosphere = sky;
    }  
    ...
    /// </remarks>
  public    Color3
SunTint    =>   PhysicalSky.SunTint(Atmosphere);
...
(graphics    )  ;  
        ...
        var sky = new    SkyParameters(sunDirection   ,   Turbidity: 2.6f  , GroundAlbedo  : 0.15f)  ; var source = PhysicalSky.   Bake(SourceSize    , sky  );
       return new(
          graphics  ,
               EnvironmentTexture    .Bake(    graphics   , source   , Levels)  ,
       SphericalHarmonics.   Project(source  )   , sky
    ...
    /// <exception cref="ArgumentNullException">An argument is null.</exception>
             public void   Apply  
               (  SceneLighting lighting, ParameterCollection parameters   ) {
...
ArgumentNullException.ThrowIfNull(    parameters   );
    ...
    var
light = lighting.   Environment ?? new  EnvironmentLight();
                Sky.Apply(light);  
...
// none. `ParameterKeys.New` interns exactly the string `EffectSetWriter` looks the binding
        ...
        stage.Parameters. Set( ParameterKeys.   New <
TextureViewHandle>  ("ShadowCaster.opacityMap"   ), opaqueView    );
 stage    .
Parameters   .  Set  (ParameterKeys.   New <SamplerHandle>  ("ShadowCaster.opacitySampler")   , opaqueSampler  );
  stage.  Parameters . Set    (ParameterKeys
     .New< BufferHandle>   (  "ShadowCaster.bones"  )  , bindPose);
        }
...
public void ApplySky   
        ...
        ArgumentNullException.ThrowIfNull(  host)
         ;
        ObjectDisposedException   .ThrowIf(disposed,  this);
        foreach ( var  node in host  .Builder.Nodes.Values) {    if (node is SkyRenderer sky)    {
    sky.
Environment  = Sky .View;
 ...
 device .Destroy  (clusterStandIn)   ; }
        if    (opaqueView. IsValid ) { device.    Destroy(opaqueView ) ;
  }
 if (  opaque .
              IsValid) {
            device .Destroy
(opaque );
        ...
        if ( opaqueSampler    .IsValid) { device.Destroy    (  opaqueSampler)  ;
           }
  ...
  if    (bindPose
    ...
    ///     </para>
    /// </remarks>
    void CreateStandIns() {    if (    clusterStandIn.    IsValid   ) {
             return ;
        }
clusterStandIn = device   .CreateBuffer(
            new   BufferDescription (
      ClusterGrid.   BufferSize, BufferUsage.Storage   ,
      MemoryAccess. HostUpload, 
      "ClusterLights.StandIn"  
 )
        );
 device.Write   (clusterStandIn, 0, new    byte  
         [ClusterGrid .BufferSize]);
        opaque = device    .  CreateTexture( new  TextureDescription (
   PixelFormat
    .Rgba8UNorm,
                1,
               1, 
     ...
     Name   :    "Opacity.Opaque"  )
        ...
        opaqueView =   device
               .CreateTextureView(opaque   )    ;
        opaqueSampler = device.CreateSampler(
    SamplerDescription .LinearClamp  with { Name    = "Opacity.Opaque" })    ;

        bindPose    =  device.CreateBuffer(
            new BufferDescription   (64   , BufferUsage
.Storage,  MemoryAccess.HostUpload, "Bones.BindPose")
    );
      var identity =
              Matrix4x4   .Identity;
          
     device.  Write
     (bindPose, 0, MemoryMarshal.AsBytes(MemoryMarshal    . CreateReadOnlySpan(ref identity,   1) ));

  // The four opaque bytes, staged, because a texture is device memory and only a copy reaches it.
        var staging =   device.   CreateBuffer  (  new  BufferDescription(4    , BufferUsage .    CopySource, MemoryAccess.HostUpload , "Opacity.Staging"  )
        );  
        device   .Write    (staging, 0  , [byte .MaxValue
...
staging, 0, new TextureRegion(opaque  ), new Int3(  1, 1, 1))   ;

   commands    .Barrier   (
            new  
    ([], [new TextureBarrier(opaque,  ResourceState.CopyDestination,   ResourceState    .ShaderRead )  ])
            );
     commands .Finish()
        ;
