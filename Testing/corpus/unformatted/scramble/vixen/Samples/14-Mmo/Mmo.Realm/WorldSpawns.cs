...
/// </remarks>
         public sealed
 class  WorldSpawns   {  
		readonly List<Spawner> spawners    =  [  ]; readonly List    <    SpawnOrder> orders = []   ;
    ...
    public WorldSpawns(
         SpawnLibrary library, ulong seed) {
...
IReadOnlyList<  SpawnOrder> Last => orders    ;
    ...
    /// <returns>How many orders this tick made.</returns>
      public int Tick   (float  now ) { orders.    Clear   ();  
     var made = 0;
  ...
  Issued += made    ;
    ...
    ///     only forwards.
    /// </remarks>
        public bool    Died(int camp,  int slot
         , float    now) =>
