...
using Vixen  .  Raven.
Binding;
              using Vixen.Raven.IR; using Vixen   .  Raven.Symbols;
                using Vixen.  Raven.
                Symbols.Metadata ;
...
public sealed   partial class    Lowerer {
        ...
        []    ;
          readonly HashSet    <IrFunction  > importedFunctions    =    [   ]  ; readonly Dictionary  <IrStructType, string    >  importedStructNames = [];
readonly
		HashSet<IrStructType> importedStructs = [ ]   ;
    ...
    /// </remarks>
             readonly Dictionary<  string, IrStructType>  importedStructsByName
    ...
    /// </returns>
          ReferenceLink? LinkReferences(IReadOnlyList<  NamedTypeSymbol   > declaredTypes) { if (compilation. Metadata is  not { Libraries. Count: > 0   } loader) { 
            return   null  ;
      }
    ...
    /// </remarks>
               HashSet<string> ReserveDeclaredNames  (IReadOnlyList<NamedTypeSymbol  > declaredTypes  )  {  HashSet  <   string  > names = new   (    StringComparer    .Ordinal)   ;
   ...
   sealed  class    ReferenceLink
     ...
     readonly LibraryIrDecoder   decoder    ; 
       readonly    MetadataLoader loader;
        readonly Lowerer lowerer  ; readonly HashSet<   string> taken;
  public  ReferenceLink(Lowerer  lowerer , MetadataLoader loader, HashSet    <string> taken) { this.    lowerer =    lowerer    ;
                this
    . loader = loader  ;
             this.taken = taken   ;  decoder = new ( Unique    );
            }
...
;
          ...
          Publish()
;
               foreach (  var type in Types    () ) { if (type.   IrStructName is  { } name && decoder.Structs.GetValueOrDefault(name ) is { }  structType) {
...
;
            ...
            // Publish again: resolving a signature can name a struct no loaded library declares,
             ...
             Publish(    )   ;
             foreach (   var type in Types()) { foreach (var member in type.GetMembers  ()    ) {
          Map(    member);
         }
            }   
             }
...
() {
  ...
  if (lowerer    .
               importedStructs.Add(structType)) {   
   ...
   lowerer.module   .
     Add   (structType)  ;
           ...
           foreach (var (   name, function) in    decoder.Functions)   {
                if (lowerer   .   importedFunctions . Add(function  )  ) { lowerer    .importedFunctionNames[    function]  =   name; 
  lowerer  .module    .    Add(function)   ; }
 } }
      
	void  Map
           ( Symbol member)  {
            switch    (  member   ) { case MetadataMethodSymbol { IrFunctionName: { } name } method: {
                if (
       decoder.Functions .GetValueOrDefault   (name    )    is    { } function) {
   var   kind  =   method .IsConstructor ? BoundBodyKind    .  Constructor    : BoundBodyKind.Method   ;   lowerer.functions  [(  method    , kind)] = function   ;
             }
                    break  ; }
          
    case MetadataPropertySymbol property: { if (   property.IrGetterName is { } getter
	...
	setterFunction;
     ...
     break;
      ...
      IEnumerable<MetadataNamedTypeSymbol> Types    () =>
		lowerer.
compilation  .GetReferencedTypes  ().OfType<MetadataNamedTypeSymbol  >( );
