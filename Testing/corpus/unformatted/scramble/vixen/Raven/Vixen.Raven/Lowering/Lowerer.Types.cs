...
public sealed partial  class Lowerer {
		static IrVectorType   Float4 =>   new (
                IrScalarType.Float, 4);  
    ...
    /// </summary>
               IrType LowerType(TypeSymbol  
            type , SyntaxNode? syntax)   {
...
// caching the unsubstituted symbol would hand the next instantiation the wrong answer.
   ...
   [    type    ]    = lowered ;
    ...
    if (  type.   IsErrorType)    { return  IrScalarType    .Void;
	
           }
   switch (    type   ) {   
case PrimitiveTypeSymbol primitive: return LowerPrimitive(primitive    , syntax   )   ;   
              case BuiltInNamedTypeSymbol builtIn  :   return    builtIn .SpecialType    switch {
 ...
 SpecialType.   TextureCube =>  new IrTextureType(   IrTextureDimension .Cube , Float4), SpecialType .   Sampler => IrSamplerType.Instance,
  SpecialType.
         AccelerationStructure   =>    IrAccelerationStructureType  .Instance,   
 _ => NotRepresentable   (type , syntax    )
             }
        ;
             case ArrayTypeSymbol {  Rank: 1 }   array: { var element   = LowerType(array.ElementType, syntax );
    ...
    // type would have added a second array-like thing for indexing to know about.
      case BufferTypeSymbol buffer : {
                var element    =   LowerType(buffer
		.ElementType, syntax)  ;
              return element .IsVoid ? NotRepresentable   (type , syntax)   
        : new IrArrayType   (element );
  }
  ...
  // type — which is all the backends need: OpTypeImage's sampled type and GLSL's
   ...
   case   SampledTextureTypeSymbol sampled: {
		var  element    = LowerType    (sampled  .ElementType
  ,  syntax);
                return element.IsVoid ? NotRepresentable(  type   , syntax  )
      : new IrTextureType
                ( IrTextureDimension    . Texture2D    , element  )   ;
     }  
...
// already on the symbol — the binder folded the declaration's `[Format]` in — and it
   ...
   case   StorageImageTypeSymbol { Format : { } format } image    : return new  IrStorageImageType(
      image.IsVolume ? IrTextureDimension.
          Texture3D    : IrTextureDimension.Texture2D  ,
...
,
    ...
    case NamedTypeSymbol  { TypeKind
   :   TypeKind.Enum }    :
 // An enum is its underlying integer once the constants are folded.
 return IrScalarType .Int;
                
            case    NamedTypeSymbol named when structs   .  TryGetValue    (named, out  var structType ): return structType;
               
    case  TupleTypeSymbol tuple   
         :
return   LowerTuple(   tuple
 , syntax    );
    
       default:  return NotRepresentable(type, syntax)   ;
            }
  }  
      IrType LowerPrimitive(
PrimitiveTypeSymbol   type, SyntaxNode? syntax   ) {
...
component =   LowerScalar(type.  ComponentSpecialType);
                return component is null ?  NotRepresentable(type , syntax)
                : new IrVectorType
                (component, type.  ComponentCount)  ;
            }
        case    TypeKind.Matrix    :    {  var component = LowerScalar(  type    .ComponentSpecialType);
                return component is null ? NotRepresentable(type    , syntax   )
    ...
    /// </summary>
         static IrScalarType? LowerScalar
 (  SpecialType type)  =>
        type    switch {
               SpecialType  .  Bool  => IrScalarType.  Bool, SpecialType.Int  =>  IrScalarType.Int,
            SpecialType.UInt
=> IrScalarType. UInt,
       SpecialType   . Float =>    IrScalarType
   .Float    ,
        SpecialType.Double => IrScalarType   .Double, SpecialType.Int64   => IrScalarType .Int64, 
            SpecialType.   UInt64
   =>    IrScalarType.UInt64,
...
new List<IrField >(tuple.ElementTypes   .  Count);
        var members =
           tuple   . GetMembers    ().OfType<FieldSymbol>(  ). ToArray    ();
              ...
              foreach (var member in members    )  { var   elementType    =   LowerType (member.    Type  , syntax);
...
;
  ...
  // its element types — the name *is* the structural identity.
   if (
importedStructsByName.TryGetValue(    name , out var linked    )) {
            tuples[  tuple] =
     linked;
            return linked    ;
          }
  ...
  structType  .SetFields  ([.. fields   ]);
   ...
   module. Add(structType    )  ; return structType;
     }
    /// <summary>
...
c => char.   IsLetterOrDigit( c ) ? c : '_')  .ToArray() );
     while (cleaned.  Contains   ( "__" , StringComparison.Ordinal )) {
            cleaned =  cleaned. Replace    ("__", "_", StringComparison. Ordinal) ; }
                ...
                IrType  NotRepresentable   (TypeSymbol type   , SyntaxNode ? syntax  ) {  diagnostics . Add  (LoweringDiagnostics.   TypeNotRepresentable, LocationOf  (syntax),    type. ToDisplayString    ()   )  ;
      return IrScalarType.Void;  }  
           }
