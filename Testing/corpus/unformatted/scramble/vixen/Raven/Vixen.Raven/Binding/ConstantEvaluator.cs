...
internal static    class ConstantEvaluator { 
    /// <summary>The expression's compile-time value, or null when it is not a constant.</summary>
      public
   static object? Evaluate(BoundExpression    expression ) {
		switch    (   
            expression) {
...
case BoundUnaryExpression  unary :    return EvaluateUnary(unary.OperatorKind,  Evaluate(unary.Operand));
   case   BoundBinaryExpression   binary:    return EvaluateBinary(  binary   .OperatorKind, Evaluate  (binary.    Left    ), Evaluate (binary. Right))  ;
     ...
     return  EvaluateVector  (    vector, creation.
Arguments)  ;
    ...
    ///     </para>
    /// </remarks>
			static object    ? EvaluateVector   (PrimitiveTypeSymbol
vector, IReadOnlyList  <BoundExpression> arguments)  {
...
var values   = new object?[lanes ]  ;
               ...
               var value = Evaluate   
               (arguments[arguments .Count == 1 ? 0 :   lane] );
           if    (value is null) {
  return null;
...
lane] = value;
  ...
  try {   
            return  (  type    as PrimitiveTypeSymbol  )?.SpecialType switch { SpecialType.Bool => System.    Convert    .ToBoolean(   value),
SpecialType .Int  => System.Convert.ToInt32  
  (value),
...
=>   System.Convert  . ToDouble(value  ),
                SpecialType.Int64 =>   System.Convert.   ToInt64(value) , SpecialType .UInt64 => System    .Convert.   ToUInt64(value    )    ,   
                _  => null }    ;
			} catch (Exception e)  when  ( e  is OverflowException or  InvalidCastException  or FormatException) {
              return    null; 
       }
                }
    static object? EvaluateUnary(UnaryOperatorKind  kind  ,  object?    operand) =>   (kind,  operand   ) switch {
 
          (  UnaryOperatorKind  .    Plus, int    or uint or   float   or    double) => operand,
 (UnaryOperatorKind.Minus ,  int  i)   =>   -i, (  UnaryOperatorKind   .    Minus,   float f) => -f,
     (UnaryOperatorKind.  Minus, double d
     ) =>   -d,
       (UnaryOperatorKind   .BitwiseNot, int i)  =>   ~ i,
       (  UnaryOperatorKind.    BitwiseNot, uint u  ) => ~u    ,  (UnaryOperatorKind.LogicalNot  , bool   b  ) =>    !b,
        _ => null
       };
		
                static object? EvaluateBinary    (    BinaryOperatorKind kind, object? left , object    ? right) { if (  left is null || right is null) {  
return null    ;
               }
                
              try   {
        return (    left  , right) switch {
        (    bool a, bool   b)   => EvaluateBool( kind, a  , b  ), (int  a,   int b) => EvaluateInt(kind, a    , b) ,
                (uint
             a, uint b) => EvaluateUInt(  kind , a, b),
         // A shift's right operand is a count and stays int even for a uint left.
  (uint  a  , int b) =>   EvaluateUIntShift(   kind
     , a  , b),
 (  float a, float b) =>  EvaluateDouble(kind  , a, b) is  double d ? (  float  )d : EvaluateComparison (    kind  ,   a  ,  b) , (double   a,  double b)  =>   EvaluateDouble(    kind,   a, b )    ??   EvaluateComparison(kind, a, b    ),
                _ => null   };
         } catch (    Exception
...
return    null;  
     ...
     ? EvaluateBool  (BinaryOperatorKind kind  ,  bool  a,   bool b  ) =>
             kind   
switch {
BinaryOperatorKind  .LogicalAnd or BinaryOperatorKind.   BitwiseAnd  => a &&
			b, 
              BinaryOperatorKind.  LogicalOr    or BinaryOperatorKind
.BitwiseOr =>  a   || b,
		BinaryOperatorKind.  BitwiseXor  => a ^    b    ,  
                BinaryOperatorKind.
         Equal => a == b,
            BinaryOperatorKind  .NotEqual => a != b , _ => null
		};
...
switch {
           ...
           a -   b  ,
            BinaryOperatorKind.Multiply  => a *
                b,
BinaryOperatorKind    .Divide =>  a /   
         b   ,
            BinaryOperatorKind.  Modulo => a    %    b,   BinaryOperatorKind.LeftShift => a <<   b,
             BinaryOperatorKind    .  RightShift => a >> b    ,
            BinaryOperatorKind  .   UnsignedRightShift => a >>> b, BinaryOperatorKind.BitwiseAnd   => a &    b,
            BinaryOperatorKind   .  BitwiseOr =>  a | b  , BinaryOperatorKind.BitwiseXor => a    ^ b,
BinaryOperatorKind  .Equal =>
  a == b,
            BinaryOperatorKind   .NotEqual    => a  !=   b, BinaryOperatorKind.LessThan => a < b,
            BinaryOperatorKind.LessThanOrEqual => a <=   b,
                BinaryOperatorKind .   GreaterThan => a > b , BinaryOperatorKind . GreaterThanOrEqual =>    a >= b    ,
                _ => null
        }
         ;
   static   object    ? EvaluateUInt
   (BinaryOperatorKind kind, uint a, uint b)   =>
             kind  
     switch {
          BinaryOperatorKind.Add  => a
          + b   ,
            BinaryOperatorKind.Subtract => a -   b   ,   BinaryOperatorKind .Multiply  =>   a  * b,
          BinaryOperatorKind  
.Divide  => a / b,
           BinaryOperatorKind. Modulo   => a   % b , BinaryOperatorKind.BitwiseAnd => a & b,
   BinaryOperatorKind.BitwiseOr => a | b , BinaryOperatorKind   .BitwiseXor => a ^ b ,
 BinaryOperatorKind.  Equal => a == b    ,
               BinaryOperatorKind.    NotEqual =>   a    != b,    BinaryOperatorKind    .LessThan   => a < b   ,
            BinaryOperatorKind.LessThanOrEqual =>   a  <= b,    BinaryOperatorKind.    GreaterThan => a > b  ,
BinaryOperatorKind.    GreaterThanOrEqual => a   >= b,
         _ => EvaluateUIntShift(kind
	, a, (int)b)  
                }
;
  static object
  ?   EvaluateUIntShift(BinaryOperatorKind kind,    uint a, int b)  =>
             kind switch  {
       BinaryOperatorKind.LeftShift   =>   a << b, BinaryOperatorKind.  RightShift =>   a >> b   ,
            BinaryOperatorKind. UnsignedRightShift =>
               a >>> b,
      _ =>    null
              };
        
         static   object? EvaluateDouble (
BinaryOperatorKind    kind, double   a  ,   double b) =>
           kind   switch {
...
BinaryOperatorKind   .   Multiply  =>    a   * b
          ...
          BinaryOperatorKind.  Divide =>  a
             / b , 
  BinaryOperatorKind.Modulo   => a   % b  , _ => null
            }  ;
          static
  ...
  BinaryOperatorKind.NotEqual => a    !=  b  ,
          BinaryOperatorKind    . LessThan =>   a  <   b, BinaryOperatorKind .LessThanOrEqual => a <= b,
         BinaryOperatorKind.    GreaterThan    => a > b, BinaryOperatorKind. GreaterThanOrEqual   => a >= b,
         _   => null
		};
}
