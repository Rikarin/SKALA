...
using Vixen.Core.Syntax.
               Diagnostics;
  using  Vixen.Raven   .Artefacts   ; using Vixen.Raven. Diagnostics ;
                namespace
            Vixen.Raven.Symbols.Metadata;
/// <summary>
...
/// </remarks>
 ...
 DiagnosticBag diagnostics;
  readonly    Dictionary< string, MetadataNamedTypeSymbol> byQualifiedName  = new
   (StringComparer  .Ordinal);
    readonly    List  <
      CompiledLibrary> libraries = [];
readonly List <MetadataNamedTypeSymbol > topLevel = [];
 ...
 public IReadOnlyList<MetadataNamedTypeSymbol> TopLevelTypes =>   topLevel;  
    ...
    /// </summary>
    public
              IReadOnlyList< CompiledLibrary>  Libraries => libraries ;
...
// in any order, so every symbol has to exist before any of them is attached.
 ...
 // duplicate-reference warning has already told the caller about.
    byQualifiedName  .TryAdd(
         model.QualifiedName, symbol);
          }
     List<   MetadataNamedTypeSymbol
           >  contributed = [];
         foreach    (var   (model  ,   symbol    )    in loaded
   ) {
          if (   model  .    ContainingType is { Length: > 0   }  outer)   { if (byQualifiedName.GetValueOrDefault    (   outer) is {    } declaring) {
...
contributed.Add
        (symbol    );
           ...
           return contributed;    }
            ...
            static IEnumerable<  string > SplitNamespace( string   name)   => name.Length == 0 ?  [] :   name  .    Split('.',  StringSplitOptions    .RemoveEmptyEntries); 
    /// <summary>The loaded type with this qualified name, or null.</summary>
                public MetadataNamedTypeSymbol
? Find(string qualifiedName) =>  byQualifiedName.  GetValueOrDefault( qualifiedName);
    /// <summary>
    ...
    /// <param name="libraryName">Which library the reference came from, for the diagnostic.</param>
           ...
           if (  reference is null) { return    ErrorTypeSymbol .Instance;
     }   
          switch (reference.Kind) {
   case LibraryTypeKind.Primitive : // A primitive travels as its SpecialType, which is the identity the binder keys
...
case LibraryTypeKind.BuiltIn:
   ...
   var element = Resolve  (reference  .Element, scope  , libraryName); return element.  IsErrorType
?  element
          : new
 ...
 return element.IsErrorType ? element : new
BufferTypeSymbol(element, reference   .Writable );
               }
            case  LibraryTypeKind.  Tuple :    {  var  elements  = reference    .Elements  .Select(e => Resolve(e   , scope, libraryName  ) ).ToArray ()    ; 
  return new TupleTypeSymbol(   elements    ,   [.. reference.    ElementNames])    ;
            }
case LibraryTypeKind  .TypeParameter:
            return reference   .Name is { } name  ? scope.Find (name) as TypeSymbol  ?? ErrorTypeSymbol  .Instance
       ...
       TypeSymbol ResolveNamed(LibraryTypeReference   reference, MetadataScope scope,   string libraryName) {   if (reference.Name is  not  {    Length: > 0 } qualified)  {   
  return
			ErrorTypeSymbol.   Instance;
           }
 if   (byQualifiedName.GetValueOrDefault( qualified) is not
            {  } definition )  {
   ...
   TypeArguments  .Select   (a =>    Resolve(a , scope ,  libraryName))   .ToArray()   ;
    ...
    /// </remarks>
           static TypeSymbol BuiltInResource   (  SpecialType
           special) =>
        special switch {
     SpecialType .Sampler    =>  BuiltInTypes.Sampler
                ,
            SpecialType.Texture2D =>    BuiltInTypes  .Texture2D, SpecialType.Texture3D  =>  BuiltInTypes.Texture3D  ,
...
///     its type's.
/// </remarks>
       internal readonly record struct MetadataScope (    MetadataNamedTypeSymbol? Type, MetadataMethodSymbol  ?  Method) { public TypeParameterSymbol  ? Find    (string name ) {
			if (Method    is not    null) { foreach (var parameter    in  Method.TypeParameters) {
                if (string    .Equals(   parameter   .    Name    ,  name  , StringComparison.   Ordinal    )) { return parameter;
                }
        }   }
 if  (Type    is not null 
		) {
        foreach    (var    parameter   in Type.   TypeParameters)  { if (   string . Equals  (parameter.   Name, name , StringComparison.   Ordinal    ))    {
     return parameter   ;
        }
            } }
        return
