...
/// <param name="Now">The clock. A parameter so a test does not have to wait.</param>
                public sealed
 record LeaseOptions(TimeSpan Lifetime    , Func <DateTimeOffset> Now  ) {
    /// <summary>The defaults.</summary>
      public static LeaseOptions Default  { get; } = new   (TimeSpan.
    FromSeconds  (20  ), () => DateTimeOffset.UtcNow);
      }
...
DateTimeOffset  expires;
  ...
  public PlayerLeaseState( LeaseOptions    ?   options = null) => this.
        options = options ?? LeaseOptions.Default  ;
   ...
   holder = shard;
      expires
      = options.Now() + options.Lifetime    ;
   return new   (
true, epoch , holder , expires);
               }
    ...
    /// <returns>The lease. Not granted when it has been superseded.</returns>
                public PlayerLease Renew(ShardId shard, long presented) { var now = options.Now()   ;
...
public void
          Release(ShardId shard, long presented) {
         if (   presented != epoch || holder   != shard ) {
                return;
 }
               holder = ShardId.   None;   expires = options.Now();
      }
    ...
    /// <summary>Which shard this character is on, as far as the cluster knows.</summary>
     public ShardId Holder  =>  IsHeld
?   holder : ShardId .None  ;
    /// <summary>Whether anybody currently holds it.</summary>
           public bool IsHeld   =>  holder    .IsValid && expires >   options.
            Now(   )  ;
...
/// </remarks>
    ...
    /// <inheritdoc />
  public Task<PlayerLease> AcquireLease(ShardId shard ) => Task.FromResult
(lease.Acquire   (   shard ))    ;
    /// <inheritdoc />
public
Task   <   PlayerLease>   RenewLease (ShardId shard  ,    long    epoch    ) =>   Task.FromResult   (    lease.Renew(shard, epoch ));
    ...
    /// <inheritdoc />
    public
Task<PlayerLease> Lease(  )  => Task.FromResult(   lease .Current   ());
    /// <inheritdoc />
   public Task<
ShardId  > Where () => Task .FromResult(lease   .Holder  )    ;
	}
