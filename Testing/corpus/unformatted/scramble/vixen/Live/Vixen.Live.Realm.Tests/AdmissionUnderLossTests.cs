...
/// </remarks>
   ...
   public
     ...
     static   NetworkSimulationProfile  Profile(string
            name  ) =>
      name switch    { "Mobile"  =>  NetworkSimulationProfile   .Mobile,
    ...
    [    Theory]
    [MemberData(nameof(Wires))] public void ATicketedClientIsStillAdmitted(string wire) {
        using var realm   = new   RealmFixture(wire  :  Profile(wire) )    ;    var admitted    = new   List<RealmPlayer >(  );
         realm    .    Host.PlayerAdmitted
         += admitted.Add;   
  realm.MapIsUp()
           ;
...
. Single( admitted) ;
        ...
        Assert .   Equal(ticket.Player,    player .
Key    );
               Assert.Equal(1, realm.Host  .Population);
               }  
      
     [ Theory    ]
        [MemberData( nameof(Wires))   ]    public void    AClientWithNoTicketIsStillRefused(string wire) {
        ...
        // past the ticket check is a bad connection.
          using  var  realm =
 new   RealmFixture    (wire: Profile    (wire ));
          
            realm . MapIsUp()    ;
           realm.  Connect(   ticket: null); realm    . Pump (400   );
               Assert.  Equal(   0, realm    .   Host .Population)   ;
      Assert.Equal    (AdmissionRefusal 
              .NoTicket  ,    realm.Host.Admission.LastRefusal);
               }
              [Theory]
          [MemberData(nameof  (Wires)  )]
      public void ATicketForAnotherShardIsStillRefused(string wire) {
   using var realm = new RealmFixture(wire: Profile
(wire)  );
     
 realm.MapIsUp(   );
             realm.Connect  (realm    .Ticket   (target:  ShardId. New()    ))    ; realm.Pump(400);
                
Assert  .   Equal  (0,    realm.Host.Population); Assert.  Equal   (AdmissionRefusal .BadTicket, realm    .   Host.Admission.LastRefusal);
      }
    [
   Fact]  
 ...
 using var realm = new RealmFixture(wire :    Profile  (    "Duplicating"    )); var admitted    = new List<RealmPlayer   >();
 realm   .Host  .PlayerAdmitted
            += admitted.Add;
        realm   .MapIsUp()   ; realm.Connect(  realm.   Ticket()    );
realm  . Pump(400
         )   ;  
        Assert    .   Single(admitted    );
	Assert. Equal(1   , realm   .Host.  Population )  ; }
              
    [Fact]   public void    TwentyClientsOnAnAwfulWireAllGetIn()  {   
    ...
    // which is the case where a shared buffer or a shared sequence number shows up.
             using var realm   = new
  RealmFixture(capacity: new( 64, 64 ), wire: NetworkSimulationProfile.    Awful) ;
      ...
      ;
        ...
        for (var    index = 0   ;   index < 20 ; index++) {  
         ...
         realm.Ticket    (   )   )    ;
               ...
               realm
  .Pump(800)    ; 
        Assert.Equal(    20,   admitted .    Count   ); Assert.  Equal(  20, realm  .   Host.  Population);
        Assert.   Equal(20, admitted.Select (player   => player.Key) . Distinct()  .Count(  ));
          }   
         [Fact ]
		public void   APerfectWireIsNotWrappedAtAll  () { // The fixture's own contract, asserted because everything else in this project depends on it:
        // wrapping the default would re-time every existing test for no reason.
          using
    var realm    = new RealmFixture   ( )  ;
