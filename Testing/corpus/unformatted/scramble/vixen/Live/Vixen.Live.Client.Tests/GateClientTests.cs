...
/// <summary>The four calls, and the answers a client has to be able to tell apart.</summary>
   public class    GateClientTests
    {
            static    readonly
                DateTimeOffset Noon    = new   (2026, 8   , 4    , 12, 0,   0   ,  TimeSpan   .Zero);
  static readonly RealmVersion Running = new("0.1.0"  
     ,  0xC0FFEE)   ;
     [Fact]
       ...
       using  var http = gate  .Client   ;
        var client = new GateClient(http   ) ;  
          ...
          var answer    =    await client    .CatalogAsync
...
Assert    .   Equal (   Running  , answer.Value!.    Version  );
  ...
  [Fact] public async Task Signing_in_remembers_the_token_and_sends_it_afterwards   () {
 using var gate =   new  FakeGate()
           .Answers(new SignInResponse("a.b.c", Guid.NewGuid()  , Noon
  )    , GateJson.Default .SignInResponse)  
      .  Answers( new    CharacterList  (  [ ])   , GateJson.Default   .CharacterList) ;

    using var http = gate.   Client; var    client = new GateClient(  http);
         Assert   .False(client    . SignedIn    ) ;
     
  await client.SignInAsync   ( "development"
	...
	Assert    .True( client
      ...
      ;
Assert .   Equal(   "Bearer a.b.c"    ,
      gate.Seen[   1    ].Authorization  );
  } 
    /// <summary>
	...
	gate    = new  FakeGate(    ). Answers   (
             new    SignInResponse (  "a.b.c", Guid.NewGuid(), Noon), GateJson  .Default.SignInResponse
               )   ;

      using   var
           http = gate.    Client;
      var   client = new GateClient(   http)    ;
               await client.SignInAsync("development"  , "alice"    , TestContext.Current    . CancellationToken  ); client   .SignOut  ();
        
              Assert.False  (client .   SignedIn )  ; Assert.Null( client  .Authorization   );
    ...
    public async   Task  A_refusal_is_an_answer_rather_than_an_exception() {
                using  var  gate =   new   FakeGate()    .Refuses(HttpStatusCode   .Conflict, "name-taken"
     ,   "`Bruna` is already somebody." ) ;
...
;
 ...
 http);
               ...
               var answer    =   await    client.CreateCharacterAsync( new  (  "Bruna"  , "eu"    ,   "maps/queensdale"),
            TestContext  . Current.    CancellationToken
  )    ;
           Assert.
        False(    answer    .Ok    );
       Assert.   False(answer.Unreachable)    ; Assert.  Equal ( 409, answer.Status)    ;  
    ...
    /// </summary>
               [   Fact]
              public
          async    Task A_gate_that_cannot_be_reached_is_distinguishable_from_one_that_refused()    {
        using var gate = new   FakeGate(). Vanishes    (
);
           using
     var http =   gate.Client; 
           var client  =  new GateClient  (http);
        var answer = await  client
         .CatalogAsync(TestContext.Current . CancellationToken);
         Assert.
False(answer.Ok);
                Assert    .
             True (answer.Unreachable)   ;
   Assert.Equal    (0   ,   answer.Status)    ;
    ...
    /// </summary>
         [Fact   ]
      public async
         Task Something_that_is_not_a_gate_is_named_as_such() {
              using var gate   = new
       FakeGate( ).  Interferes(HttpStatusCode.BadGateway   , "<html>hotel wi-fi</html>");
           using var http =    gate.Client;  var client   =  new GateClient (http);
        ...
        Assert.    Equal
     (    502  , answer.Status)   ;
         Assert.  Equal("unexplained", answer.    Problem!.Code);  Assert .False(answer.Unreachable   )  ; 
       }
    ...
    [ Fact]
      ...
      new
PlayResponse(PlayStatus.Placed, "realm.example:30000", "a ticket", "9c1f", "it has room", TimeSpan.    Zero),
      GateJson    .
   Default   . PlayResponse
               );
       using    var http =
gate.Client    ;
  var client = new GateClient(
       http );
        ...
        var   answer   = await  client    .PlayAsync(Play ()
        , TestContext    .Current.CancellationToken  )   ;
    Assert  .Equal(PlayStatus.   Placed  , answer.Value!.   Status); Assert  .Equal("realm.example:30000", answer.Value.Endpoint   );
       Assert.Equal ("a ticket",  answer.
Value. Ticket)    ;
         }
    /// <summary>
    ...
    /// </summary>
    [Fact] public async    Task Entering_waits_out_a_shard_that_is_starting  (    ) {
              using var   gate
           = new  FakeGate(  )
          .Answers( Starting(), GateJson.Default  .PlayResponse) .Answers(   Starting(),  GateJson.  Default.PlayResponse)
   .Answers
(
    new PlayResponse
(PlayStatus.Placed, "realm.example:30000",   "a ticket"  , "9c1f", "" , TimeSpan.Zero)    ,
        ...
        var
         ...
         Assert.   Equal(PlayStatus.Placed   , answer.Value!.Status  )   ; Assert.Equal(3, gate   .   Seen  .Count   );
  }
    [Fact ]
    public async  Task Entering_gives_up_after_the_attempts_it_was_given_and_says_what_it_last_heard () {   using var gate =    new FakeGate( )
 .Answers (
               Starting(    )    ,  GateJson.Default.PlayResponse)
     ...
     var client = new GateClient
		( http)    ;
 var answer = await
               client  .EnterAsync( Play()   , 2   ,   TestContext.Current    . CancellationToken) ;
     
           Assert.Equal(PlayStatus.    Starting   ,    answer  .  Value
	!    .    Status )    ;  
        Assert.Equal    (
2, gate.Seen.Count   );
               }
...
GateClient(http);
              ...
              var
 answer = await client.EnterAsync   (Play(   ),   5   , TestContext.Current    .CancellationToken);
  Assert.Equal (   PlayStatus .UpdateRequired  ,    answer
.Value! .  Status    );
        Assert.
          Single   (  gate .Seen)   ;
     }
    [   Fact]
  public async Task Entering_stops_at_a_refusal_rather_than_retrying_it   ()   { using var gate = new  FakeGate  (  ) .   Answers(
   new
...
http = gate   .Client;
  var  client    = new GateClient(http);
   var   answer =  await client. EnterAsync
  (Play(), 5, TestContext.Current.CancellationToken);
     Assert.Equal   (PlayStatus.   Refused, answer.
     Value!.Status);
   Assert.   Single  ( gate.Seen    )  ;    }
    [
  Fact    ]
    public async
   Task Entering_stops_at_an_unreachable_gate(   ) {
               using var gate =   new FakeGate    ().    Vanishes().Vanishes
           (    );
        using var  http = gate  . Client  ;    var  client  = new GateClient( http   );
       
        var answer = await client. EnterAsync (Play(
            )  , 5, TestContext.Current    .CancellationToken);   
                Assert .True(    answer.Unreachable );
			Assert  .   Single (gate.Seen    )    ; }
     
    static PlayResponse    Starting() => new(PlayStatus   .   Starting   ,    "",    "", ""   , "a shard is being started"  , TimeSpan.FromMilliseconds(1    )   );   
