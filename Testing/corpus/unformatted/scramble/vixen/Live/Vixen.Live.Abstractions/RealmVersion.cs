...
/// <param name="Content">The catalog's <c>BuildHash</c>.</param>
           [ System .Text.Json
.Serialization.    JsonConverter (typeof(  RealmVersionJsonConverter)) ]
...
CultureInfo.InvariantCulture,    out var  content
    )
     ) {
      return  false;
                }
   ...
   public override string ToString    () =>
        IsValid ? string .Create
(CultureInfo.InvariantCulture, $"{Build}+{Content:x16}" )   : "no version"  ;
