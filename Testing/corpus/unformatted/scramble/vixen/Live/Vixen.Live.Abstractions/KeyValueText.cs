 ...
 var key =  Unescape  (pair[..separator    ]    ) ;
  ...
  pair [ ( separator + 1)..]  )  )) {
      ...
      return    false  ;
               ...
               return true; }
    static void  Escape(    StringBuilder text, string    value ) { foreach (var character in value) {
               switch (character    ) {
    case
       '%':
                    text.  Append("%25")
...
break    ;  
     ...
     text.Append    (character)  ;   
             ...
             static string  Unescape(    ReadOnlySpan<char> value) { if (value    .   IndexOf('%')  < 0) { 
           return value .    ToString(   )  ;
      }
	
        var text =   new StringBuilder(value.Length
);
             for (  var index = 0; index < value.Length; index++) { if (  value    [index    ] == '%' && index +    2   < value.Length) {
                var pair
    ...
    2;
           continue  ; }
                ...
                if (    pair  .
Equals    ("3B" , StringComparison .   OrdinalIgnoreCase  ))   {
           text
             .Append(';');
           index
                += 2;
