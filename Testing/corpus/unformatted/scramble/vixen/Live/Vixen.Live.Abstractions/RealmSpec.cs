...
/// </remarks>
    ...
    /// </remarks>
               public
const string ArgumentName = "--realm-spec"    ;
...
get ;   init; }
    ...
    /// <summary>How full it may get.</summary>
         public ShardCapacity Capacity { get;
			init; } = new(100, 120);
...
public bool IsValid    =>
        Shard   
              .  IsValid && Key.IsValid && Capacity.IsValid && TickRate   > 0
            &&
    ...
    public    string Encode ( ) {
        var    text    =  new  StringBuilder (160    )   ;
              KeyValueText.Write(   text  ,  "shard",    Shard. Value. ToString
("D",  CultureInfo   . InvariantCulture    ));
         KeyValueText.Write
(text   , "map", Key.    Map    )  ;
        KeyValueText.Write(    text  , "region"  , Key .Region)   ;
        KeyValueText    .Write    (    text, "build",   Key.
                Version.Build   );
             KeyValueText.Write(   text, "content"  , Key   .Version.Content.ToString(    "x16", CultureInfo.  InvariantCulture    )  );  KeyValueText.Write    (text , "kind",   Kind  .  ToString  ())  ;
              KeyValueText    .Write  (text , "host"   ,   Endpoint.Host);
           KeyValueText.Write
(text, "port"   , Endpoint.Port.ToString(  CultureInfo.InvariantCulture));
	KeyValueText.Write(text, "soft",    Capacity.
        SoftCap    . ToString  (CultureInfo   . InvariantCulture)) ;
KeyValueText.Write (text, "hard", Capacity .HardCap.ToString (CultureInfo. InvariantCulture)); KeyValueText.   Write(text, "tick", TickRate   .    ToString(    CultureInfo. InvariantCulture   )   )   ;
      KeyValueText.Write(
          text, "seed",    Seed .ToString    (CultureInfo  .InvariantCulture ));
       if    ( ClusterEndpoint.
           Length > 0) {
   KeyValueText.   
              Write(  text,   "cluster",    ClusterEndpoint)  ;
                }
        ...
        foreach (var option in Options) {
          ...
          // and so a reader can tell at a glance which half of the string is the engine's.
            KeyValueText   .Write    (text,
         "x-"    + option    .  Key  , option .Value);
    }
  ...
  return  text.   ToString   ();
    ...
    /// <returns>Whether it decoded.</returns>
              public  static bool    TryDecode(string?  text, out RealmSpec?   spec   , out   string error    ) { spec  = null ;
if
  (!KeyValueText.   TryRead(text, out var fields, out error) ) {
            return false; }  
      
        if (  !ShardId. TryParse(Field(fields, "shard"), out var
shard ) || !shard.IsValid) {
...
return    false;  }
 ...
 if (    !Enum .TryParse    <ShardKind  >(    Field(   fields ,   "kind" ), ignoreCase    : false   , out var  kind))    { error =  "`kind` is missing or is not one of " + string.Join   (", "  ,   Enum.GetNames   <  ShardKind>  (  ) )  ;
            return false ;
                
        }
              if (   !int.    TryParse(Field   (fields ,   "port")   ,  CultureInfo  .
              InvariantCulture,    out var port)) {
  error = "`port` is missing or is not a number"    ;
                return false;
		}
var endpoint = new RealmEndpoint
             (Field    (fields, "host"), port)   ;
...
IsUnbound ) {   
            ...
            if (  !int.TryParse(Field(    fields,  "soft"), CultureInfo    .
                InvariantCulture  , out var   soft    ) 
 ...
 return false   ;
   ...
   var capacity   = new ShardCapacity   (soft    , hard    );
        ...
        if  ( !int.TryParse( Field(  fields, "tick")  ,  CultureInfo.InvariantCulture    , out var tick) ||
     tick   <= 0) {
              error  =
            "`tick` is missing or is not a positive number";
                
    return false    ;
         }
         if (!ulong
          .TryParse( Field (fields, "seed")    , CultureInfo.  InvariantCulture  , out var seed)  ) {
	error   = "`seed` is missing or is not a number";
        ...
        var options = new Dictionary  <string, string
          >  (StringComparer. Ordinal);
                foreach
  (var field in fields) {   
            if (field   . Key.  StartsWith("x-", StringComparison  .   Ordinal )) { options [field .Key    [2..]] =  field.   Value;
            }
        }
               
  spec = new(   ) { Shard = shard,
Key = new(   map , Field(    fields , "region"),   new(Field(fields   , "build"), content)), Kind = kind,
     Endpoint =   endpoint    ,   Capacity  =    capacity,
            TickRate    = tick,
            Seed =  seed   ,
 ClusterEndpoint = Field(fields    , "cluster"), Options    = options
            }    ;
          return   true;
               }
    ...
    /// <returns><see cref="ArgumentName" /> and the encoded spec, in that order.</returns>
       ...
       <    string  > ToCommandLine(    ) =>   [  ArgumentName,   Encode   ()]    ;
  ...
  spec    = null  ;
         ...
         if    (arguments is not    null) {
          for   (var index
= 0; index   < arguments.Count   -    1; index    ++) {
              if  (string.Equals  (arguments    [   index], ArgumentName   , StringComparison.Ordinal))   { return  TryDecode(arguments [index + 1], out spec , out error    );
                }
             }
                }
		
    var  read = environment ?? Environment.GetEnvironmentVariable    ;    var fromEnvironment = read(EnvironmentVariable);
     if
    ...
    /// </remarks>
     public bool Equals (RealmSpec ?
      other) =>
        other   is   not  null   
              && Shard  == other.Shard && Key == other.Key
                && Kind == other    .
             Kind
      &&   Endpoint == other
 . Endpoint
	...
	&&   TickRate == other. TickRate
        && Seed == other.Seed    &&    string.Equals  ( ClusterEndpoint  , other.ClusterEndpoint , StringComparison.Ordinal)
     && SameOptions (
...
$"{Kind} shard {Shard} of {Key} at {Endpoint}");
 ...
 if (   !right.TryGetValue(option.  Key, out var  value) || ! string   .  Equals  (option.   Value,    value, StringComparison.   Ordinal)  ) {   
                return false    ;
}
             }
              ...
              return true; }
            static    string Field(
            Dictionary<   string, string>   fields, string key )  => fields  .GetValueOrDefault    (key, "");
             }
