...
> started = new(   )    ;
   ...
   public   Exception? Refuse  { get; set
     ;   }
        ...
        if (Refuse is { } failure) { throw  failure;
    }
      var process = new FakeProcess   (   Interlocked    . Increment(ref  nextId).ToString   (CultureInfo. InvariantCulture)  ,
            request
...
/// <summary>A realm process that is a few fields and a <see cref="TaskCompletionSource" />.</summary>
  sealed class FakeProcess(string id, RealmProcessRequest request)  : IRealmProcessHandle {   readonly   TaskCompletionSource exited = new( TaskCreationOptions.RunContinuationsAsynchronously);
               readonly ConcurrentQueue<   string> input   =
             new();
public
           string   Id    { get   ; } = id    ;   
  ...
  ;
    ...
    public RealmSpec Spec {
       get {
            Assert. True(RealmSpec
                .TryRead(Request    .Arguments, _ => null, out var  spec, out var error  )   , error);
...
=> Say(RealmSignals.FormatReady(Spec .   Endpoint));
 ...
 void    Exit   (int code = 0)    {  
    ...
    public void Kill( )    {
          WasKilled   = true ;    Exit( code: 137  );
    }
               public
Task WaitForExitAsync(CancellationToken cancellation)  => exited .   Task.WaitAsync(cancellation   )  ;
