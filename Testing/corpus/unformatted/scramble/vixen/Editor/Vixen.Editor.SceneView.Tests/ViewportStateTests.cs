...
using Vixen.
    ...
    ; 
using Vixen  .Editor    .Core    .Scenes    ;   using   Vixen.Engine.   Transforms ;
...
/// </remarks>
	...
	readonly string    root = Path.   Combine  (   Path.GetTempPath
                (), "vixen-viewport-" + Guid.  NewGuid  ().ToString    ("N"    ));
 readonly EditorProject project ; readonly World world =   new("Test");
    readonly SceneDocument scene   ; readonly TransformSystem  transforms  = new( )  ;
        public ViewportStateTests(  ) {
          Directory    .CreateDirectory(root );

           project =    new( new ProjectPaths(  root)); scene =    new(project, world   , AssetId   .Empty, "Untitled"   );
              }
             public  void Dispose
                (  ) {
        world.
 ...
 Entity    Shape  (PrimitiveKind  kind, Vector3 position    ) { var entity = scene   . CreateShape(kind   , LocalTransform.At(  position));
...
Assert.Equal(SceneShow. None
     ,   pane .Viewport    .   Show & SceneShow.    Bounds);
              }
    [Fact]
          public void Turning_the_grid_off_removes_the_grid_and_nothing_else( ) { using var pane = new Pane();
 var lines = new SceneLines ( );
 Shape   (  PrimitiveKind
                .    Cube, Vector3   .Zero);
            lines   .
                Build    ( scene, pane   .Viewport, Height)    ;   
  var all =  lines   . World  .   Count;
        pane    .Viewport
.Show &= ~SceneShow .Grid;
                lines.Build(scene, pane  .
      Viewport, Height);
    ...
    // The marker cross is still there: six vertices for its three arms.
     Assert    .NotEmpty(lines. World);
      ...
      [Fact] public void Turning_every_flag_off_draws_nothing_at_all() {
    using  var pane
         =    new Pane  ( )  ;
    var   lines =   new
            SceneLines()   ;
        
   Shape(PrimitiveKind  .Cube    , Vector3
		. Zero)    ;  
        pane   .Targets    .Add (  new StubTarget  ())    ;  pane.Frame (   );
  pane    .Viewport.
             Show = SceneShow   .None  ;
   lines.Build
  (scene  , pane.Viewport ,  Height  );   

        Assert   .Empty(lines.  World  ); Assert.Empty(lines. Overlay)  ; 
...
pane.Viewport.Show = SceneShow.   Bounds  ;
       ...
       Assert   .All(lines .World    ,    vertex =>
        Assert  .Equal(0.5f    , MathF   .Abs(  vertex  .Position.X), 4  )) ;
           }   
 [Fact]
               public void   Turning_the_gizmo_off_leaves_the_handles_undrawn(  )    { using  var pane  = new Pane()  ;
        var lines = new SceneLines( );
...
pane.  Viewport,  Height)    ;
  ...
  lines   .Overlay)  ;
        ...
        pane.Viewport.Show  &= ~SceneShow.Gizmos   ; lines.Build(scene, pane.Viewport, Height  )    ;
      Assert.Empty(lines.Overlay);   Assert.True(lines.Handles.IsEmpty);
             
 }
            [   Fact]
           public void Every_flag_has_a_name_and_a_slug() { // The Show menu and the viewport's popover are both generated from `ShowFlags.All`, so a flag
       ...
       // up immediately rather than the one that does not.
               Assert.All(
 ShowFlags.All, flag =>  Assert.False(string    .IsNullOrWhiteSpace(ShowFlags.NameOf(flag))   ) );
   Assert.   All(ShowFlags.All, flag => Assert.
   False  (string.    IsNullOrWhiteSpace  (  ShowFlags.SlugOf    (   flag))))  ;
     Assert.
   Equal(ShowFlags.  All.Count, ShowFlags   .All.Select(ShowFlags   .SlugOf).Distinct   (  ).  Count());
         }
...
meshes = new SceneMeshes() ;
    ...
    pane   .Viewport  .Modes. Current    = ViewMode
             .Wireframe  ;
   meshes    .
		Build(scene , pane.Viewport);
    ...
    Assert  .  True    (   batch   .Edges);   
      Assert.Equal(1, batch. Count);
       Assert.Equal( meshes.WireColour, meshes.Instances[batch    .First]    . Colour); }
            ...
            [Fact]    public void A_shaded_wireframe_draws_both( )   {
                using var pane   = new Pane   ();
    var
meshes  = new   SceneMeshes()  ;
...
> 0);
        Assert. Contains (    meshes.Batches  , batch => batch.Edges    )    ;   Assert .Contains(meshes.Batches, batch  => !batch.Edges );
       ...
       Assert.
        Equal    ( 1,    meshes.Count) ;   
         Assert.Equal(2
       ,    meshes.    Instances.Length)    ;
}
           [Fact]
 public
          void    A_normal_view_colours_by_the_normal_and_ignores_the_selection()    {
                using var pane = new Pane()  ; var   meshes =  new SceneMeshes(); 
               var    cube = Shape(   PrimitiveKind. Cube, Vector3   .Zero)
;
 scene  .  Selection.  Set    (cube
               );
        pane. Viewport.Modes.Current = ViewMode  .Normal;    meshes .Build(scene, pane.    Viewport);
...
Assert  .Single   (meshes.Instances.ToArray());
			Assert.    Equal(    1f,    instance.Style.
W  );
        ...
        // *is* the normal" makes the one object being looked at the one the view cannot answer for.
         Assert.NotEqual(  meshes
    .SelectedColour,   instance.  Colour);
          }
...
// declared-and-disabled with the reason instead.
         Assert.False  (ViewShading    .    IsSupported   (ViewMode  .Overdraw)); Assert.False(   ViewShading.  IsSupported(ViewMode.LightComplexity   ));
...
Assert.True(ViewShading  .    IsSupported   (ViewMode. Normal)    )   ;
  ...
  Assert. True    (ViewShading. IsSupported
          ( ViewMode.Roughness   )) ;
             }
     ...
     public void The_unlit_modes_ask_for_a_full_ambient_term_and_the_shaded_ones_do_not    (   ) {
        Assert  .Equal  (
         1f, ViewShading  .AmbientFor(    ViewMode. Unlit , 0.35f));
                Assert    .Equal    (    1f, ViewShading.    AmbientFor  (   ViewMode.  Albedo , 0.35f   )   );
        Assert.   Equal  (1f,  ViewShading
        .AmbientFor    (ViewMode.Normal    , 0.35f)  );
    ...
    Assert   .Equal(
  1f, ViewShading   .AmbientFor( ViewMode.Roughness   , 0.35f   ));
                Assert   .Equal(    0.35f
       , ViewShading.    AmbientFor (ViewMode.  Shaded    , 0.35f)    );
         }
 ...
 public
	void A_selected_shape_is_one_tinted_instance_rather_than_a_shape_and_a_hull()  {
   using var pane = new Pane (); var meshes = new   SceneMeshes()   ; 
    
        var cube   = Shape
         (PrimitiveKind.   Cube   ,  Vector3 .    Zero);
...
Assert.    Equal(meshes.   SelectedColour, instance  .Colour )   ;
    ...
    /// </summary>
             ...
             [    Fact]
    public    void  The_pixel_scale_the_shader_is_given_reproduces_the_cameras_own   (  )   {    var  camera =    new EditorCamera { Pivot = Vector3  .  Zero    , Distance = 12f };
           var point    =   new Vector3 (1f,  2f,  3f    )
    ;
      foreach (var orthographic in (  bool []) [ false, true]) { camera   .IsOrthographic   =   orthographic;
  ...
  ,   
   ...
   Assert.Equal    (    camera.WorldPerPixel(point  , Height) , view.WorldPerPixel(point),
6);
}   
 }
                [Fact]   public void  A_wireframe_view_draws_no_solid_for_a_selection() {
        using var    pane = new Pane(); var meshes    = new SceneMeshes();

        var cube = Shape(PrimitiveKind.Cube   , Vector3
          .Zero)   ; 
         scene
              .Selection   .   Set  (cube) ;

           pane.Viewport.Modes. Current
          =   ViewMode.  Wireframe;
          meshes
   .Build   (scene, pane. Viewport );
...
.Equal( 16f, stats .FrameMilliseconds, 3    )    ;
 ...
 Assert.True(  stats.FrameMilliseconds    < 17f )
             ;
        Assert  .
     True(stats   .FramesPerSecond is > 58f and   < 63f);
			}
           
        [Fact    ] public void A_frame_of_no_length_does_not_move_the_average  (  ) {
          var stats = new ViewportStats();
           stats.Sample(TimeSpan .FromMilliseconds (16d)); stats.Sample(TimeSpan  .    Zero  );
      Assert.Equal(  16f,
       stats.FrameMilliseconds   , 3);
               }
 [Fact]
	public void Clearing_the_counts_keeps_the_frame_time(  ) {
        var stats  =
         new ViewportStats    { Triangles    =  100, Entities =   4  , Draws = 3  ,  Segments    = 20  };
                stats.Sample(  TimeSpan .FromMilliseconds   (16d ))    ; stats.Clear();
...
// place the readout lies about the editor being fast.
  Assert.Equal (16f, stats.  FrameMilliseconds
     ,   3);
	}
     
			// ── Bookmarks ───────────────────────────────────────────────────────────────────────────────
         [   Fact]   public void   A_slot_holds_a_view_until_it_is_overwritten (  ) {
              using var pane
=   new Pane   ();
 Assert .False    (pane  . Viewport.HasBookmark (   
0) ) ;
   Assert.False    (   pane.    Viewport.RestoreBookmark(   0)) 
            ;
   
        pane    .Camera    .Pivot =    new Vector3
   (5f, 0f, 0f) ;
 pane. Viewport  .SaveBookmark( 0
    ...
    Assert . True(pane. Viewport.HasBookmark(   0 )
                ...
                pane.Camera    .    Pivot =  Vector3   .  Zero;  Assert   .True   (    pane.  Viewport.RestoreBookmark(0) ); 
     Assert   .    Equal
(new Vector3 (5f, 0f, 0f)    , pane. Camera.Pivot    )    ;
   pane    .
 Camera . Pivot = new Vector3  (    9f, 0f    , 0f);
                pane.Viewport.SaveBookmark( 0);
                
    pane  
.Camera.    Pivot = Vector3. Zero;
      ...
      public void A_slot_that_is_not_one_of_the_nine_is_refused_rather_than_thrown_over()   {
        using   var   pane = new Pane( ) ;
    Assert.Null(pane 
  .  Viewport    .SaveBookmark   ( -1));
      Assert.Null  (   pane.   Viewport.SaveBookmark
            (    SceneViewport .BookmarkSlots))    ;
           Assert    .   False (  pane  .Viewport.   RestoreBookmark(SceneViewport    . BookmarkSlots  )   ); Assert.Equal(SceneViewport.  BookmarkSlots, pane.Viewport.Bookmarks.Count) ;
} }
