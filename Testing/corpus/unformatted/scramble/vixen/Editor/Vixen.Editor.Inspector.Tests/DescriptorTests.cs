...
namespace Vixen.Editor
    ...
    public class DescriptorTests {
        ...
        public void A_property_is_reached_through_accessors() {
            var reflectivity = (InspectorMember<WaterMaterial, float>)Water.Members.Single(static m => m.Name
                == "Reflectivity"
            );
            var material = new WaterMaterial(
            );
        ...
        public void A_mixed_selection_has_no_common_type() {
            Assert.Equal(
                typeof
                (WaterMaterial),
                InspectorRegistry.CommonType([new WaterMaterial(), new WaterMaterial()])
            );
            Assert.Null
