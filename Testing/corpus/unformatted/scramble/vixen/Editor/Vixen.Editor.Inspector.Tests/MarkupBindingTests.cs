       // SPDX-FileCopyrightText: Copyright (c) Rikarin
            ...
            using  Vixen.Core    ; using Vixen  .   Editor. Core; 
		using   Vixen.    Ui;
using Vixen.Ui.Controls; using Xunit  ;
...
/// </remarks>
        public   class MarkupBindingTests   :
   IDisposable {
			readonly UiDocument    document =   new(400f, 800f)  ;   readonly string    root  = Path   .Combine(Path.GetTempPath(), "vixen-markup-" +  Guid.NewGuid  () .ToString  ("N")    );
    readonly EditorProject project;
          readonly  ScratchDocument edited;

       public  MarkupBindingTests    (   ) { InspectorTheme.    Install(document );
        Directory
 .CreateDirectory  (root);
  
        project    = new    (   new ProjectPaths(root))   ; edited    = new(    project) ;
          }
public void Dispose  () {
       document   .  Dispose();
  ...
  if   (   Directory.    Exists  (root)) {
	...
	InspectorTarget Target  (  params object    [ ]
objects) => new( objects, edited);
    InspectorTarget Target  (  IPrefabSource
	prefab, params object [] objects) => new(objects, edited,  prefab    );
       [    Fact] public  void A_property_field_draws_the_row_the_generated_inspector_would() {
 var    water = new WaterMaterial  ();   var    field =    document .Root    .  Add<PropertyField >();

        field.Path = "Roughness"   ; document    .  Update();
 ...
 var water =  new WaterMaterial( )   ; var   field    =  document   .Root.Add  <PropertyField  >(  )    ;
         ...
         = "Roughness";
            document.Update   ();
  MarkupBinding. 
      Bind(    document. Root   ,    Target(water   )   )   ;
              Assert .True (field    .   
	Row!.Field   . Write(0.75f))    ;
              Assert .Equal
                (0.75f, water.Roughness);
           edited.Stack.Undo   (  );
	...
	public void A_path_that_names_nothing_says_so_in_the_panel(   )
       {
        var    field =    document    .Root.  Add < PropertyField>();
         field. Path
             =  "Roughnes";
              document.Update();
        Assert    .Equal(0    , MarkupBinding.Bind(document.Root, Target(new WaterMaterial())    ));  Assert   .   Null (  field  .Row)   ;
        var text    = Descendants
             (field). OfType<TextBlock  >() .Single()    ;
    
    Assert. Contains( "Roughnes"    ,    text  . Text   , StringComparison.  Ordinal)   ; }   
        [
             Fact]
    public void A_control_carrying_binding_path_reads_the_member_and_writes_it_back()   { var water    = new   WaterMaterial { Roughness = 0.4f   };
        var slider
              = document.  Root   .Add<    Slider>();
		
 slider.Minimum  =    0f;    slider.Maximum =   1f ; 
slider  .SetAttribute   ( "binding-path"    , "Roughness")    ;
          document.Update(  )   ;
    
 Assert.    Equal
           (1, MarkupBinding.Bind   (document  .Root, Target(water)));
...
slider.Value = 0.9f;
   ...
   [Fact]
		public void
             A_numeric_input_writes_the_members_type_rather_than_its_own(   ) {
             var water = new    WaterMaterial  ( )   ;
     var number =   document.   Root .Add <NumericInput  >() ;
       number   .SetAttribute  ("binding-path",   "Roughness"); document.Update   ();
...
MarkupBinding .Bind (document   .Root , Target   (first,   second
    ...
    /// </summary>
    [Fact]
                public  void   The_walk_does_not_descend_into_a_row_a_drawer_built() { var field    = document.    Root. Add<PropertyField   >();
...
Assert.Equal   (  1, MarkupBinding. Bind   ( document    .  Root    ,    Target (  new WaterMaterial())));   }
    ...
    /// </summary>
        [Fact   ]
    public void A_property_field_keeps_its_reset_button_and_override_bar_in_step()  {    var water = new WaterMaterial   ();   
        var  
   field    = document .Root   .  Add<  PropertyField >   ();
     field .    Path = "Roughness"; document .Update( );
    ...
    new StubPrefab {   Roughness = 0.2f  }    , water)   );
            var row    = field.  Row   ;
      Assert.NotNull(row
    )   ;  
    Assert   .True(row.   Reset  . HasClass (    "hidden"   ))   ; Assert.   False    (row .HasClass("overridden")); 
      
           Assert .  True    ( row.   Field.Write
(0.75f));
   Assert  .   False    ( row.  Reset.HasClass("hidden") ); Assert.   True(row.  HasClass( "overridden"));
 }
    ...
    /// </summary>
        ...
        var    water   = new WaterMaterial    ()    ;
var    field = document .  Root.Add  <  PropertyField>()    ; var target = Target(    water);
 ...
 MarkupBinding
        .Bind(document.Root,    target);
     
		Assert.NotNull   (first  ) ; Assert.True  (first.    IsRemoved    )   ;
...
0.75f  ,    water. Roughness)  ;
    ...
    /// <summary>A prefab that has one member and agrees with whatever it is asked about it.</summary>
          sealed class
			StubPrefab   : IPrefabSource {
      public  
         float Roughness   { get; init; }
   
           public bool IsOverridden(   object target   ,    InspectorMember member) => TryGetPrefabValue(target   ,    member, out   var  original) && !   Equals(   member.GetBoxed(target )    , original);
               
   public  bool  TryGetPrefabValue   ( object
             target , InspectorMember  member,    out object? value) {
                value =    member    .Name  == "Roughness"
  ? Roughness : null   ;
           return   member
         .    Name    == "Roughness";
         }
            }
     static  IEnumerable    <UiElement> Descendants  (UiElement element ) {   foreach (var child in element   .Children   ) {

           yield return child;  
			
            foreach
     (var found in Descendants( child)  ) { 
          yield return found;
            }
    }
    } }
