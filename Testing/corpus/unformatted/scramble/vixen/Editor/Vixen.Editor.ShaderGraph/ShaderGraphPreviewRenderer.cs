...
using    Vixen .Editor.    NodeGraph   ; using Vixen    .   Graphics   ;
               using Vixen.ShaderCompiler   ; using Vixen.Shaders  ;
...
int Size = 64;
    /// <summary>What a preview target is, and what its pipeline is built for.</summary>
          public
const PixelFormat Format =  PixelFormat.Rgba8UNorm  ;
    readonly IGraphicsDevice device;
    readonly NodeTypeRegistry   
registry;
  readonly
		IPreviewImages? images;
...
;
    ...
    /// </remarks>
           readonly
Dictionary  <(NodeGraphModel Graph, NodeId    Node), Entry> entries =  [];
...
Refusals    {   get; private set;  } 
    ...
    /// </remarks>
      public bool TryGet(
NodeGraphModel graph    , GraphNode node,
               NodeTypeDefinition  definition, out NodePreview preview 
    )   {
        ArgumentNullException    .ThrowIfNull(   graph ); ArgumentNullException.ThrowIfNull  (node   ) ;
...
.Id    )    ;
        ...
        if (entries  .TryGetValue(    key , out
              var   entry    )    && entry   .Revision  == revision)   {
  // ⚠ The first of three gates, and the only one that runs sixty times a second. Nothing
     ...
     // revision on. Without it, a canvas of fifty nodes walks fifty closures and builds fifty
        // strings on every frame in which nobody did anything.
              Touch(  key   );
           return Answer(entry    ,    out
preview)  ;
        }

        var compilation =   ShaderGraphPreview   .Compile(graph    , node
              .Id, registry);
...
key) ;
  ...
  // renaming a property, editing a value on a node this one does not depend on — all of
   ...
   if  (!string.   Equals(  entry.Source, source.Source, StringComparison.Ordinal)) { entry  .Source =  source.Source    ;
...
(key    );
     ...
     } else {   entries[key] = new Entry { Source = source.Source,   Revision = revision };
           dirty.  Add(key)   ;
            Evict (    );  
...
return
 false;
   ...
   preview  =
			new NodePreview(   new   Color4(1f ,   1f,    1f, 1f ), Image  :  entry   .  Image );
   ...
   return true;
    ...
    /// </remarks>
              public int Update()  { ObjectDisposedException.    ThrowIf(  disposed, this);
    ...
    NodeGraphModel   Graph   ,  NodeId Node)> built    = [];
               for   (   var index =  0; index <  taken   ; index  ++) {    var    key =   dirty[index  ];
                if (  entries .TryGetValue(key
      , out var entry) && Build(key   .Node   , entry))    {
              built.Add( key);
       }
}
    ...
    dirty.    RemoveRange    (0 , taken);
        ...
        using    (var commands   =
   device. BeginCommandList  (QueueKind.Graphics, "shader graph previews")) {
...
disposed =    true;
    ...
    /// <summary>Compiles one preview's Raven and makes everything the draw needs.</summary>
                bool Build(NodeId id, Entry   entry   ) { Release    ( entry  );
     Compilations ++  ;
        
  Effect    effect;
	try {
           var   data =
    Compile(entry.Source   );
...
if (binding  .Kind is not (DescriptorKind.UniformBuffer or  DescriptorKind.    DynamicUniformBuffer)) {   entry.Refusal =
  ...
  +
    ...
    Refusals++   ;
        ...
        var block = effect.  BlockOf    (DescriptorSetSlot.PerMaterial);
                if (   block. Exists  ) {
            entry.Constants =   device   . CreateBuffer ( new(block.Size, BufferUsage.    Uniform,   MemoryAccess.HostUpload, "shader graph preview constants"  )
    );
      
            device
		.   Write    (entry.  Constants   , 0,   Transforms   (block )  )  ;
        entry. Descriptors = device.CreateDescriptorSet( effect .SetLayouts    [(int)DescriptorSetSlot.PerMaterial   ],
    ...
    ;
           ...
           device  .
   UpdateDescriptorSet (entry .Descriptors    , [DescriptorWrite.Uniform(   block.Binding , entry.Constants )]);
          }
         
        var (  vertices    , layout)  =   Geometry(effect   );
   if (vertices.Length > 0) {
             entry .Vertices = device. CreateBuffer(
        new( vertices  .   Length *  sizeof(float),
          BufferUsage.Vertex,
                    MemoryAccess .
   HostUpload,
         ...
         device.Write(entry
             .Vertices  , 0    , MemoryMarshal    .AsBytes<float    >(vertices   )   )    ;
           }   
              entry  .Vertex = device.  CreateShader (ShaderStage.Vertex , Bytecode(effect   , ShaderStage .Vertex) ,
           "preview vertex");
 entry.
Fragment = device. CreateShader(ShaderStage   .Fragment    , Bytecode(effect   , ShaderStage.Fragment)    , "preview fragment"    );

        entry.Pipeline =    device.    CreateGraphicsPipeline( new   (
               entry.Vertex, entry.Fragment  ,
          effect   .Layout, 
    [    new
   ColourTargetState(    Format,  BlendState . Opaque)  ],
             layout.Length > 0
  ...
  // and that failure is indistinguishable from a preview nobody implemented.
        Rasterizer: RasterizerState  .TwoSided, DepthStencil : DepthStencilState. Disabled  ,
    ...
    void Draw(ICommandList commands, Entry entry) {
       commands.    Barrier(   new BarrierGroup([], [
          new TextureBarrier  (  entry.  Texture,    entry   . State , ResourceState   .ColourTarget )]    )) ;
                
    commands.   BeginRenderPass
(
			new RenderPassDescription( [new ColourAttachment( entry.    View  , LoadAction.Clear,  StoreAction.Store, new  Color4(   0f, 0f, 0f, 1f))   ],
   name: "shader graph preview"
              )
 );
			
commands.  SetViewport(new    Viewport(0f   , 0f, Size,    Size   ))  ; commands.SetScissor( new   ScissorRect(0, 0, Size    , Size  ) );
           commands.  BindPipeline(   entry   .    Pipeline);

        if   (
entry    .   Descriptors. IsValid ) {
               commands.BindDescriptorSet(    DescriptorSetSlot    . PerMaterial
  , entry.Descriptors);
}
      ...
      Vertices. IsValid  ) {
        ...
        commands.    Draw    (Quad.Length  ); commands.EndRenderPass(  );

        commands
	. Barrier(
         new  BarrierGroup ([], [new TextureBarrier (entry .
  Texture,    ResourceState.    ColourTarget   , ResourceState.  ShaderRead ) ] )
    ...
    void Ensure(NodeId id, Entry   entry  ) { if (   entry.   Texture. IsValid ) {
            return; }
...
entry.Texture    =   device  .    CreateTexture(
 ...
 Format ,
  ...
  .CreateTextureView    (entry. Texture   );
    ...
    entry .Image =   images?.   Register    (entry
.View) ??    0;
  ...
  ShaderGraphPreview    . Name + ".rvn", source   ) ])
                .TryGet(    EffectKey.Of(ShaderGraphPreview.Name));  
          static ReadOnlySpan    <byte
          >   Bytecode(Effect  effect  , ShaderStage stage) {
                foreach (var
compiled in effect.Stages)    {
     if (compiled.Stage == stage) { return compiled.Bytecode.AsSpan    ( ); 
         } }
  return  [];
              }
    ...
    block   .Size];
        ...
        foreach   (var parameter in block. Members) {
              if  (parameter.Size   < 64 || !IsTransform (parameter. Key.Name)) {   continue  ;
         }
...
return bytes  ;
    ...
    /// </remarks>
      static    bool  IsTransform   (string name)   {   
            var bare =
          name   .    AsSpan ()   [(name  .   LastIndexOf  ('.')    + 1)..];
...
lanes == 0   ) {
    continue    ;
              ...
              elements    .Add(new    ((uint) input  .Location   ,  FormatOf(    lanes    ),    offset)); readers.Add ((lanes  ,   ReaderOf   (input . Name) )); 
...
List<float> ( Quad  .Length * Math  . Max    (1    , stride  /  sizeof (   float    )))    ;
             ...
             foreach (var corner    in    Quad)   {  foreach (var (   lanes,   of   ) in    readers  ) {
    var value =
			of    (corner);
			...
			for (    var lane = 0;   lane <
               lanes   ;  lane ++)    {
     floats.Add(lane
      switch { 0 =>  value .X , 1 =>    value.Y   , 2 => value.  Z, _ => value.W   }  );
   }
               }
      }
     return ([.. floats],  new Layout
               (stride, [.. elements]))  ;
           }
	static int
          LanesOf (ShaderValueKind  kind) =>  kind switch {
     ShaderValueKind.  Float =>  1    ,
        ShaderValueKind.Float2 => 2 , ShaderValueKind   .Float3 =>   3,
           ShaderValueKind.Float4  =>   4, _ => 0
    ...
    /// <summary>What one named vertex attribute holds, corner by corner.</summary>
             static Func  <Corner   ,  Vector4 > ReaderOf(string name) => name switch   {  "texcoord" =>  corner => new Vector4(  corner.Uv.X, corner  .  Uv    .   Y ,    0f, 0f),
    ...
    // one whose normal is zero — which normalises to a NaN and shades as nothing.
      "normal" => _ => new Vector4
      (0f, 0f, 1f, 0f),
              "colour"
         => _ =>    new Vector4(1f, 1f, 1f  , 1f)  ,
_ =>   corner    => new Vector4(corner    .
             Position  .X, corner.Position.Y ,  0f,   1f )
             };
  ...
  if (watched .    TryGetValue(  graph,  out var   watching)) {   return watching .  Revision;
        }
      
			var
               state = new Watched   ();
  
                state.   Handler = _  => state.Revision++   ; graph.    Changed +=    state. Handler  ;
  watched[graph] = state ;
...
Pipeline)    ; 
  ...
  if  (entry.   Vertex    .  IsValid) {
    device.Destroy(  entry.   Vertex    ); entry  .  Vertex = default;
        }
               if
  ( entry  .Fragment.IsValid    ) {
         ...
         if (entry.Descriptors.IsValid  )  { device. Destroy(entry  .Descriptors  )   ;
            entry.   Descriptors = default
   ;
      
     }
              
      if (  entry  .Constants.IsValid) {   device .  Destroy    (entry.Constants);
            entry    .Constants =
         default;
}
      if   ( entry . Vertices    .IsValid) {    device  .Destroy(entry. Vertices ) ;
 entry.Vertices =   default; }
    ...
    void Destroy (    Entry entry)   {
        Release    (entry)
      ;
   if (!entry  .   Texture  .IsValid  ) {   return;  
        }
        ...
        if    (  entry   .  Image != 0)    { images ?.Release  (   entry.Image);
 
       entry.Image =    0; 
        }
   
   device.  Destroy(  entry.    View); device.Destroy(entry.Texture    );
...
Length => Elements.Length; 
    ...
    /// <summary>One graph's revision, and the handler that moves it.</summary>
       sealed class Watched { public int Revision = 1;
...
get  ; set;   }
   ...
   public int Revision {    get ; set ; }
    ...
    ; set; }
          ...
          public TextureHandle  Texture  ;   public TextureViewHandle View;
        public ResourceState State   = ResourceState.Undefined; public    ulong    Image;
