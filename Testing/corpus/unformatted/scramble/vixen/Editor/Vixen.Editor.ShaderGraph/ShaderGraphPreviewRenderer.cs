...
float[] Vertices ,   Layout Layout    ) Geometry  (Effect effect) {
   ...
   <    lanes; lane++) {
               floats.Add  (
      lane switch { 0 => value    .X,   1 => value .Y  , 2 => value.Z,   _  => value.W })  ;
     }
               } 
           }
	return ([.. floats],
...
_  => corner => new Vector4 
             (corner.Position .  X,   corner .Position.Y, 0f, 1f)
             } ;   
...
(entry  );
 ...
 entry.View  = default;
    ...
    /// </remarks>
    readonly record   struct  Corner  (    Vector2
Position, Vector2   Uv);

       static readonly Corner   [   ] Quad   = [
        new    (new Vector2(
-    1f, 1f), new Vector2(0f  ,   0f)  ),  
...
,    new Vector2 (1f  ,  0f)    ), 
   ...
   new Vector2(-1f,  -1f)  , new Vector2   (0f   , 1f))  ,
    ...
    readonly record struct  Layout(int
                Stride, VertexElement[] Elements) {
