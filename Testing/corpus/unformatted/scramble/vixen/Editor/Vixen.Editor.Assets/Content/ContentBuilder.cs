...
/// </param>
                public readonly  record  struct   BuildableAsset(    string Address,
    ObjectId Id,
              string    Group,
...
/// <param name="Hash">The hash of those contents.</param>
	...
	string Name, string  FileName   , ReadOnlyMemory <    byte> Bytes  , ObjectId Hash);
...
ThrowIfNullOrEmpty  (target   );
    ...
    /// <exception cref="ArgumentException">An asset names a group that does not exist.</exception>
             public ContentBuildResult Build( IEnumerable<AddressableGroup>  groups,
        IEnumerable <BuildableAsset
> assets  ,
        IOdbBackend chunks 
        ) {
...
.ThrowIfNull(chunks);
             var  policies = new
Dictionary<string,   AddressableGroup    > (StringComparer    .Ordinal );
       foreach (var group in    groups)    {    if (    !policies.TryAdd  (group .Name   , group))  {
   ...
   var ordered = assets    .OrderBy(   
asset   => asset  .Address, StringComparer.    Ordinal)   .ToArray(   );
         var planned = new SortedDictionary<string,  List  <BuildableAsset>    >(StringComparer   .  Ordinal
		);
        var bundleOf   = new Dictionary<
      string  , string>(   StringComparer  .Ordinal    );

                foreach   
               (var   asset   in   ordered)   { 
              if    (! policies.TryGetValue(asset    .Group  , out var
 group    ))  { 
...
assets)
     ...
     if ( !
      group.IncludeInBuild    ) {
             diagnostics.    Add (
   new (  ImportSeverity   .   Information,
  ...
  "build and nothing can address it."
   ...
   continue;
            ...
            var name  
=   BundleNameFor(group , asset   ,   diagnostics);
            if (  !planned. TryGetValue(   name, out var    contents)) {
                contents   = [ ]  ; planned[name ] = contents;
  }
      ...
      contents.Add( asset)    ;
        ...
        var   built
= ImmutableArray.CreateBuilder<BuiltBundle>  (planned    .Count   ) ;
       var    described =   ImmutableArray.CreateBuilder<   CatalogBundle>
  (    planned.Count  )  ;
                
         foreach
 (   var (name, contents) in planned) {
     var group = policies   [  contents [0].Group]  ; var    writer   = new BundleWriter();

 foreach (var asset   in contents) { if (   chunks .  TryRead    (    asset.   Id,  out    var blob) )   {   
               using (  blob    ) { writer. Add(asset.  Id    , blob.Bytes.ToArray   ())    ;
       }
     continue;  
  ...
  ImportSeverity    .   Error  ,
   ...
   name,
                group.LoadPath == 
              ContentProvider.  Remote ?   Join   (  group.RemoteUrl, fileName) : string   .Empty,
 hash ,   
         bytes .Length   , Crc32.HashToUInt32(bytes) ,
			group
      .Compression,
   [  ]
           ) ) ;   
        ...
        var  entries = ordered
.Where(asset => bundleOf    .ContainsKey(asset
        .   Address    ))
   .Select    (asset => new CatalogEntry(    asset.Address,
           asset    .    Id   ,   
            bundleOf    [    asset.Address
  ]   ,
        policies[    asset.Group   ].LoadPath,
                asset.Dependencies.IsDefault ? [] : asset.Dependencies,
        asset.Labels .IsDefault ? [  ] : asset    .Labels ,  0   ,
     asset.    Reference
                )
       )  ; 
            var
           catalog = new ContentCatalog(CatalogVersion, BuildHash(built  ) , Target,  entries, described)  ;
                return new(   catalog, built.
ToImmutable(),   diagnostics.   ToImmutable(  ));
    }
    /// <summary>Which bundle an asset belongs in, from its group's packing policy.</summary>
static string  BundleNameFor( AddressableGroup group   ,
BuildableAsset asset,   ImmutableArray<ImportDiagnostic   >.Builder diagnostics
) {
		switch    (group.Packing) { case    BundlePacking  .PackSeparately:   
                ...
                return $"{group.Name}_{ContentHash.Compute(System.Text.Encoding.UTF8.GetBytes(asset.Address))
                    .ToString()[..12]}";
 case    BundlePacking.PackTogetherByLabel: { if (asset  .  Labels.IsDefaultOrEmpty) {
    return group  .   Name;
 }  

       var labels = asset.Labels    .Sort(  StringComparer.   Ordinal)
; 
               if (labels.Length > 1 )    { diagnostics.Add(
       new   (
...
;
    ...
    /// </summary>
           static  ObjectId BuildHash( ImmutableArray   <BuiltBundle >    .
Builder bundles) {
 ...
 .Hash.    WriteTo(    buffer   );  
         hashing    .Append   (buffer)    ; }
             ...
             hashing
 . GetHashAndReset (   buffer    )   ;
        return    ObjectId   .FromBytes(buffer )
;

    }

			static    string   Join(string prefix  ,
string fileName)  =>
        prefix    .Length
         ==   0 ? fileName
