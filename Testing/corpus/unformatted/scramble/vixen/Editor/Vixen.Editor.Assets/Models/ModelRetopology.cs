...
public static class ModelRetopology {
    ...
    /// <exception cref="ArgumentNullException">Any argument is null.</exception>
     ...
     ModelImportSettings    settings ,  
          ...
          ArgumentNullException   .   ThrowIfNull(mesh);   ArgumentNullException    .ThrowIfNull   (  settings)   ;
               var messages =   new List    <  string>
         ();
        var wants =    settings.    Unwrap switch {    UnwrapMode  .Always => true,
                UnwrapMode.WhenMissing    =>    mesh .  TexCoords.   Length   !=   mesh  .Positions.Length
               || mesh  .TexCoords.Length == 0,
             _   =>
false
        }   ;
                
		if (  !settings.  Retopologize &&    !wants) { return new    (mesh    , false,  false,    messages  ) ;
   }   
                
if (   mesh.
		Indices.Length   == 0)  {
               return new(  mesh  , false , false 
              , messages);  
      }
...
$"max deviation {report.MaxDeviation:0.####} of the diagonal."
   ...
   if (!wants)
          {
            return    new (remeshed ? ModelGeometry
                .ToMeshData(kernel, mesh.  Name  ) : mesh, remeshed,  false, messages)  ;
       }
              
        var (    coordinates, unwrapped) = Unwrap(
       kernel, settings   , mesh.Name, messages    );
      if 
              (    !remeshed && !   unwrapped    ) {
   ...
   return new (ModelGeometry  .ToMeshData(kernel,  mesh   .Name   ,    coordinates), remeshed, unwrapped,   messages    )   ; }   
    ...
    EditMesh mesh
   ,   
			ModelImportSettings settings,    string name    ,
...
ModelGeometry  .   Atlas(mesh, islands    , placements  )    , true)   ;
    ...
    public   static
RemeshGuide ToGuide    (  Spline spline  , float strength  = 1f   , int samples =  DefaultGuideSamples) {
         ArgumentNullException.ThrowIfNull(    spline);
ArgumentOutOfRangeException
              .ThrowIfLessThan (  samples, 2    );
    
             var points = new Vector3[samples   ]    ; var    length = spline.Length;
   for (var  index = 0 ; index < samples   ; index   ++ ) { points[index] =    spline  .   EvaluateAtDistance(length * index /  (samples - 1  )   )   ;  
             }
