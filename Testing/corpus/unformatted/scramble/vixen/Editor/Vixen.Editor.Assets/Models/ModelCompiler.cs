...
ModelCompiler {
    ...
    /// <exception cref="ArgumentNullException">The mesh or the report is null.</exception>
               public static
    MeshletMesh? CompileMeshlets(
           MeshData mesh   ,
               MeshletBuildSettings  settings,
        Action<
   ImportSeverity,  string    >  report
 ...
 if (mesh  .   TriangleCount == 0    ) {    return null;
        ...
        var input    = ToBuildInput(mesh );
              var built   = MeshletBuilder .Build
   (   input , settings);
 var problems = MeshletValidator   .
Validate(built, input);
 ...
 report(
    ...
    /// </remarks>
 public const  int PageAttributeStride
             =    10;
    /// <summary>
    ...
    /// </remarks>
    public   const
int SkinnedPageAttributeStride = PageAttributeStride +  MeshletPageBuilder.  InfluenceSize  ;
    ...
    ///     </para>
    /// </remarks>
      public static MeshletPageSet  ? CompilePages( MeshData mesh,
        MeshletMesh meshlets ,
  Action<ImportSeverity, string  >   report
)
    {
               ArgumentNullException    .ThrowIfNull  (mesh); ArgumentNullException.ThrowIfNull(meshlets);
     ArgumentNullException.ThrowIfNull(   report)   ;
		if    (mesh.    IsSkinned && OutOfPalette
             (mesh  )  is { } offender    ) {
	report    (
               ImportSeverity.
Error    ,
...
$"index in one byte — so a skeleton it can page has at most "
     ...
     return null   ;
             ...
             try   { return MeshletPageBuilder    .Build(
        meshlets,
                mesh .Positions,   
                PageAttributes(mesh) , new   ( ) {
            AttributeStride  =
  mesh .IsSkinned ? SkinnedPageAttributeStride    : PageAttributeStride,
 ...
 ;
    ...
    /// </remarks>
    static    byte   []   PageAttributes  (MeshData    mesh    ) {   var   stride  = mesh.IsSkinned ?    SkinnedPageAttributeStride : PageAttributeStride  ;
        var attributes = new byte   [    mesh.   Positions   .Length * stride   ] ;

        for (
	var i = 0  ; i < mesh.Positions.Length; i ++) {
         var  normal =    i
                <  mesh. Normals   .Length ? mesh.Normals[i] : Vector3.   Zero;
            var uv = i < mesh.TexCoords.    Length ?   mesh.TexCoords  [  i]    :  Vector2.Zero;  var at = i * stride;
   Half
                (attributes.AsSpan   (at    )    , normal.   X   )   ;
      Half(attributes.  AsSpan(at + 2)  , normal  .    Y); Half  (attributes.   AsSpan( at + 4  ), normal.Z  );
			Half(  attributes.AsSpan (   at  + 6),    uv. X   );
            Half(attributes   .AsSpan(
at + 8  ), uv. Y);

        if (mesh.IsSkinned   ) { Influences(    attributes .  AsSpan(   at +   PageAttributeStride)  , mesh,   i);
         } }   
    ...
    ///     </para>
    /// </remarks>
   static void    Influences(
              Span<byte  > destination ,   MeshData mesh , int vertex) {
 var at = vertex    *   4;
 if ( 
               at  +   4 >   mesh.BoneWeights    .Length)  {
            destination[..MeshletPageBuilder.InfluenceSize].Clear( )    ; destination[4] = byte.  MaxValue;
       return
;
}
         for (var i = 0   ; i < 4; i++)
...
destination[i] =    (    byte  )Math. Clamp (index, 0  , MeshletPageBuilder .MaxBones -   1); destination[ 4 + i] =   (byte)MathF   . Round(    weight * byte.  MaxValue    );
          } }
     ...
     foreach (var    index in mesh.BoneIndices)    {
		if (index < 0 || index >= MeshletPageBuilder  .  MaxBones    )   {
           ...
           return null    ;
             ...
             static   void  Half  (    Span<
byte> destination, float value    ) =>
          BitConverter.
...
MeshletBuildInput ToBuildInput    (MeshData mesh)   =>
 ...
 Normals =  mesh  .Normals , Tangents = mesh.Tangents,
