...
using Vixen. Core; using Vixen    .Core.Serialization;
 using Vixen.  Core.  Yaml; using  Vixen.   Core  .Yaml.Meta;
   using Vixen. Water;
...
/// </remarks>
   ...
   string text;
      ...
      using var   reader = new StreamReader (    source);
              text = await
           reader   .   ReadToEndAsync(  cancellationToken    ).ConfigureAwait (   false);
        }
      ...
      waves = YamlSerializer.Parse<    WaterWavesAsset>(text);  
        ...
        context  .
     Report    (   ImportSeverity .Error  , exception.Message) ;

	return context  .Finish   (   ); 
        }
    
        if (    waves.Name .   Length == 0)   { waves.  Name =   Path. GetFileNameWithoutExtension  (context. SourcePath.Value  )   ;
        }
             ...
             Advise(context, waves. Spectrum) ;
                context .Write (SubAssetId. Main    , WavesType, Serializer.    ToBytes
          ( waves )) ;
    ...
    /// </remarks>
       ...
       if (spectrum   .    WindSpeed
<= 0f    || spectrum    .   AmplitudeScale <= 0f    )   {
   context.Report(
                ImportSeverity.Warning   , "Every wave in this sea state has zero amplitude, so the surface is a mirror. That is "
                + "a legitimate dead calm and it is also what a file nobody finished looks like — the " + "wind speed or the amplitude scale is zero."
      )
         ;
			}
              if (spectrum.DirectionalSpread   <= 0f) {   context.Report   (
