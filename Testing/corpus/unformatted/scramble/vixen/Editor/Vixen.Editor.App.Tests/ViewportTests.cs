...
public void  Four_panes_have_four_cameras_and_four_view_modes()   {
      using var fixture =   EditorSession.Start(  )
           ;  
       fixture  .   Open( "scene")   ; fixture.   Run (   "scene.panes-quad" )   ;
...
var panes = fixture.    Viewports;
    ...
    // perspective views is one every user then configures by hand — and it is also the layout in
      // which a presenter shared between panes is invisible, because all four would agree.
        Assert
               .True (panes .Take   (3  ).All(  pane    => pane    .Camera. IsOrthographic)   );  
       Assert   . False(panes[3].
               Camera   .IsOrthographic ) ;  
        Assert.    Equal
         (4  , panes.Select(  pane =>  (  pane    .Camera .Yaw, pane    .Camera    .Pitch   ))   .Distinct(  ).Count(   ))    ;
  ...
  // the wrong axis.
           panes[0
].Modes .  Current = ViewMode.Wireframe;   
              Assert.Equal (ViewMode.Wireframe   ,    panes   [0].    Modes .Current)  ; Assert.  All(panes.Skip  (   1  ),   pane => Assert.    Equal (  ViewMode  .Shaded, pane.Modes.  Current)  );
         }  
   [Fact]    public    void A_scene_command_acts_on_the_focused_pane_and_not_its_neighbours() {
             using
  ...
  Assert  .  Same   (panes    [  0]    , fixture.Viewport )  ;
           ...
           Assert.  Equal(ViewMode.  Wireframe
,  panes    [  0]. Modes.Current);
 Assert.Equal(ViewMode .Shaded, panes   [
               1]. Modes.    Current    )    ;
...
// Clicking in a pane focuses it, and the next command follows the work rather than the
   ...
   fixture. Frames(    1  )
              ;
    Assert  
              . Same    (    panes[1    ],   fixture    .Viewport);
  
             fixture  .   Run    ("scene.view-mode-unlit");
         
            Assert. Equal    (ViewMode.   Wireframe, panes[ 0].Modes.Current); Assert.  Equal(ViewMode.Unlit, panes[   1].Modes.Current);
            }
    /// <summary>
	...
	using  var    fixture =
      ...
      fixture    .Frames(2)  ;
     Assert.    True(fixture.
           Shell.Workspace.IsOpen (    "hierarchy"));

       fixture. Run    ("scene.maximise")   ;   fixture .Frames (2);
...
Shell .Workspace.    IsOpen("hierarchy"));
      ...
      fixture
        ...
        // the default layout would silently throw away every splitter the user had dragged.
                Assert.
          True(fixture   .  Shell.   Workspace.IsOpen    ("scene"));  
             Assert.True(    fixture  .    Shell .Workspace.
IsOpen("hierarchy"));
       }
    /// <summary>And it does something in the single-pane layout, which is where it did nothing.</summary>
   [Fact]    public  void Maximise_works_in_the_layout_that_is_already_one_pane    () {
             using var fixture  =    EditorSession   .Start();
		fixture .  Open(    "scene" )   ;
...
Assert.False(fixture.Shell  .Workspace.  IsOpen(  "inspector" ))
    ...
    [    Fact]
     ...
     fixture.   Open ("scene"); fixture.Frames(1);  
       ...
       Assert.True    (fixture.CanRun("scene.view-mode-normal") ); Assert.False  (fixture.CanRun("scene.view-mode-overdraw"));
   Assert.False
  ...
  // material now, so the menu line enabled itself.
        Assert.    True(  fixture.CanRun (
       "scene.view-mode-roughness"));
            Assert. True( fixture    .Shell.  Commands.TryGet("scene.view-mode-overdraw"  , out  var
          command) )    ;
              Assert.   False(    string .IsNullOrWhiteSpace( command!.Unavailable. Text));
      }
  
              [Fact]
       public void The_show_flags_are_per_pane_and_the_grid_has_exactly_one_command ( ) { using var fixture =    EditorSession . Start   ( ) ;
		fixture.Open(    "scene"); fixture.Run("scene.panes-side-by-side"   );   
           fixture. Frames    (2   );
    var
  panes = fixture. Viewports    ;  
           fixture  .  Run(    "scene.toggle-grid" );
         Assert
               .Equal (SceneShow    .None, panes[    0].Show &   SceneShow  .Grid);
  Assert
             .Equal(    SceneShow   .Grid,  panes    [1] .Show & SceneShow  .Grid);
...
Assert
               .Equal ( SceneShow.Bounds , panes[0].  Show   &    SceneShow.Bounds)   ;
      Assert.Equal(    SceneShow.    None,    panes[  1].Show & SceneShow.Bounds);   }
 [Fact    ]
      public void A_bookmark_is_saved_by_one_command_and_recalled_by_another( ) { using var    fixture = EditorSession.Start  (   )  ;
...
Camera.Pivot =    Vector3.Zero    ;
  fixture    .
			Run("scene.bookmark-go-1")   ;
	Assert.Equal(   new Vector3(   7f , 1f, 2f), pane.Camera.Pivot);
	
      }
             
          [ Fact]   public void Snap_to_floor_drops_the_selection_onto_what_is_under_it_and_undoes() {
       using var fixture
=    EditorSession.Start()  ;
  ...
  var scene  = fixture    .Scene ;
             var
                crate = scene   .Entities .   First (entity    => scene    .  NameOf(entity) == "Crate")  ;  
              scene.
           Selection.  Set([    crate] );   
        fixture  .Frames(
		1);
        var
           before = fixture.Scene  .World . Read    <Vixen  .Engine.Transforms    .LocalTransform   >(crate).Position; 
  
			Assert.True   (fixture.CanRun("entity.snap-to-floor" )) ; fixture.    Run ("entity.snap-to-floor");
             fixture.Frames(    1) ;
   
            var after = fixture.Scene . World.Read   <Vixen.   
               Engine    .Transforms.  LocalTransform    >(crate)  .Position;
...
.Y    , after .Y);
 ...
 fixture.Run ("edit.undo"); fixture    .  Frames(1);
  
       Assert.Equal(    before
              .  Y   , fixture.Scene.World.Read< Vixen  .Engine  .Transforms.LocalTransform  >(crate)  .Position. Y,  4); 
            }

       [Fact] public void Move_to_view_puts_the_selection_where_the_camera_is_looking( ) {
	using  var fixture = EditorSession.Start  (  );
           fixture.Open   ("scene"); fixture    .Frames(    1)   ;
...
scene = fixture.Scene;
        var crate
= scene  .   Entities.First(entity => scene.NameOf(    entity) == "Crate") ;
        ...
        Assert  .True 
(fixture.CanRun("entity.move-to-view"));
        fixture   .Run("entity.move-to-view"
		);
        fixture.Frames
  (1);
              var position =   fixture.
Scene    .World    .Read  <Vixen.Engine.Transforms.    WorldTransform > (crate ).   Value  .Translation    ;
        ...
        Assert.True
(   Vector3.NearEqual  (  position, new Vector3(12f, 3f,  -4f  )  , 1e-3f)) ;
               }
    ...
    /// </summary>
               [  Fact  ]
             public  void  What_a_pane_is_drawing_survives_a_restart() {    using    var fixture = EditorSession.Start() ;
             fixture.  Open   ("scene")    ;
      fixture.
     Frames(    1) ;
  ...
  fixture  .Viewport!   ;
        ...
        Assert.True(  (pane
.Show    & SceneShow.Grid) != 0  , "the grid should start on")  ;
...
// generated ones and is deliberately not registered twice — see `ShowFlagCommands`.
             fixture .  Run    ("scene.toggle-grid")   ;  fixture.Frames( 2);
           
   Assert.   True(
          (fixture.   Viewport!.Show & SceneShow   .Grid) == 0);
        fixture    .  Restart(); fixture   .Open("scene");
           fixture.Frames(   2);
        Assert  .True (
        (  
           fixture.Viewport!  .Show  & SceneShow.Grid) == 0,
        ...
        Assert.
     True(  ( fixture.Viewport!.    Show & SceneShow.Gizmos) !=   0 );
 }
    /// <summary>The view mode goes the same way, through the same entry.</summary>
          [Fact] public    void So_does_the_view_mode  () {
	using var
fixture = EditorSession .Start   (    ) ;
        fixture.Open   ("scene"); fixture.Frames (1);
   ...
   .Frames(2   ) ;
     fixture.Restart  (
         ...
         fixture.Open("scene")  ;    fixture.   Frames( 2 );  
        Assert.   Equal    (ViewMode
        .Wireframe, fixture.Viewport!   .  Modes.Current    )   ;
              }
}
