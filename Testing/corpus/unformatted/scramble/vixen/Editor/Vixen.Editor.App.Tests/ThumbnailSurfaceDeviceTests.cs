// SPDX-FileCopyrightText: Copyright (c) Rikarin
       // SPDX-License-Identifier: Apache-2.0
                ...
                using Vixen.Graphics    ; using Vixen. Graphics   .Vulkan    ;
  using Vixen.Rendering; using Vixen .Shaders.Generated    ;
...
/// </remarks>
   ...
   const int Size  = ThumbnailCache .Size    ;
    ...
    /// <summary>A device, or a skip — or, when one was required, a failure.</summary>
       static    VulkanDevice  Open( ) { if (  VulkanDevice.   TryCreate(  new( ), out   var device  , out var reason)  ) {
  return device!;
     }
     if   (
    ...
    /// </remarks>
     static UiShaders Shaders( IGraphicsDevice  device    )   => 
      new   (
		Load(  device    , ShaderStage  .Vertex
 , "UiVertex.vert.spv"),
 ...
 Locations = new(  UiVertexKeys   .PositionLocation,
           UiVertexKeys   . TexcoordLocation    ,    UiVertexKeys   .VertexColourLocation    ,
     UiVertexKeys   .VertexShapeLocation  )
            } ;
   static ShaderHandle Load (   IGraphicsDevice  device, ShaderStage stage   , string name ) { var  path = Path.    Combine (AppContext.BaseDirectory, "ToolShaders"  ,   name )  ;

         Assert.True  (   File
.Exists(    path)   , $"the upload test needs {name} beside the test binary"    );
        return  device.CreateShader   
                (stage, File.ReadAllBytes  (path    )   , name  );
         }
 ...
 for (var y =  0;   y < Size    ; y++) {   var row = flip ? Size - 1   -    y :  y;

            for ( var x = 0    ; x    < Size   ;  x++) {  var at = ((   row * Size    )  + x )  * 4;
pixels    [   at] = (   byte)(x * 255 / (Size -  1    )   ) ;
 pixels[at +   1] = (  byte )(y *
               255 / (    Size - 1  ));
 pixels   [at + 2] = 255    ; pixels[   at +  3   ] = 255;
} } 
    ...
    /// </remarks>
          ...
          {
                var image =
         surface.Upload  (   Size  ,  Size , source);
       Assert   .NotEqual    ( 0ul,    image  )    ;
     const int Bytes =
     Size *  Size   * 4  ;   
          var readback =  device   .CreateBuffer    ( new(Bytes, BufferUsage.CopyDestination ,    MemoryAccess.HostReadback, "thumbnail readback")
      );
        device .  BeginFrame();
        Assert.  Equal(1, surface. Flush
           ()  ); 
          
               var texture    =  surface.TextureOf
   (image   );
        Assert   .    True(texture.IsValid  
, "the surface has no texture for the image it just handed out")   ;
          
        using
         (  var commands =  device.BeginCommandList(QueueKind  .Graphics   , "thumbnail readback"   ))    {
            commands.Barrier(
       new BarrierGroup([], [new TextureBarrier(    texture, ResourceState  .ShaderRead,    ResourceState .  CopySource  )])   
    )  ;  
      commands   . CopyTextureToBuffer(new  
               TextureRegion(    texture)    , new(Size, Size   , 1)    , readback, 0) ;
            commands. Barrier   (
...
(byte[  ] pixels)   {
     HashSet<    int> seen =    []; 
        for (var at = 0; at + 3 < pixels.   
            Length   ; at += 4    ) {
    seen  .   Add((pixels[at]   <<  16   ) | (  pixels[  at +
            1    ]    <<  8) | pixels[at + 2    ]    );
  }
  return seen.Count    ; 
        }
    static  double Mean(byte [] pixels  )   {  long sum  = 0;
for (var    at
= 0; at +  3 < pixels.   Length   ;    at += 4) {
  sum += pixels[
               at] + pixels  [at + 1] + pixels[at  +   2]; 
       }
         return sum
       / (double  )( pixels. Length / 4 * 3 )  ;
    }
        
	static byte Channel    (byte[ ]  pixels, int x  , int
 y, int channel) => pixels[(    (  ( y * Size) +   x) * 4) + channel];
    ...
    /// <summary>The uploaded texture holds the gradient, and holds it the right way up.</summary>
        ...
        using (    var device = Open()  )  { VulkanDiagnostics.Reset (   );
  
 var shaders = Shaders    (  device
         );
  var renderer =
         new UiRenderer(  device, shaders, new RenderOutput([ PixelFormat.    Bgra8UNorm])); 
	
		using var surface =   new ThumbnailSurface(device   , renderer
);
       
            var    pixels = Round(   device
               ,    surface, Gradient(    )); 
            
           Assert
            .Equal (1, surface    . Submitted);
      Assert.Equal
   (  0 , surface.  Waiting);
  
            Assert.True(
      VulkanDiagnostics   .ErrorCount  == 0, "the upload produced validation errors, so its picture means nothing: "
                + string.Join(
     Environment.NewLine,  VulkanDiagnostics  .Messages)
            )
             ;

 var distinct   = Distinct(pixels); var mean = Mean(    pixels);
...
1000   , $"the thumbnail holds {distinct} distinct colour(s), which is not a gradient");
  ...
  Assert   .True   (   Channel(pixels ,   0 , Last, 1    ) > 220,
         ...
         // And the other axis, which no vertical flip would disturb: red is the column.
           Assert
           .True(   Channel(pixels, 0,    0, 0  ) <  32)    ;
           Assert
                .True(Channel( pixels, Last, 0,  0) > 220)   ;
    ...
    [    Fact]
                public void
         A_flipped_picture_has_the_same_histogram_and_different_corners () {
     using (   var   device =    Open  ())    {    var shaders =   Shaders(device   );
   var    renderer = new UiRenderer(device, shaders, new RenderOutput([PixelFormat.Bgra8UNorm])) ;
   using var surface
  =    new ThumbnailSurface( device, renderer   );
    var upright = Round    (device    ,    surface, Gradient()); var    flipped  = Round  (  device, surface   , Gradient    (    flip:  true));
  
     Assert    .Equal(Distinct  (upright), Distinct( flipped) ); Assert.Equal  (   Mean(upright ),   Mean(flipped    ),    6 );
            const int Last
   = Size - 1;
 ...
 Assert .True(Channel   (flipped, 0, 0 , 1) > 220)   ; Assert.  True(  Channel   (flipped  , 0, Last, 1)  < 32);
    ...
    /// </remarks>
    [  Fact] public void A_released_image_leaves_no_copy_behind() {
        using (var    device = Open ()) { VulkanDiagnostics.Reset    () ;
     var
...
renderer = new UiRenderer(device, shaders, new RenderOutput([  PixelFormat.Bgra8UNorm ]))   ;
          ...
          Assert   .
      Equal(1, surface.Waiting);

            surface  .  Release(image);
            surface.Retire(  );   
 Assert.
          Equal( 0   ,    surface.Waiting);
      device.BeginFrame( )    ;
