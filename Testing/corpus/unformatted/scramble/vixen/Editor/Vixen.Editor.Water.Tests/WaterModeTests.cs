...
.Mathematics;  
             using Vixen  .
Editor  .Ui  ;
   using Vixen.Input; using Vixen   .    Ui    ;
            using   Vixen.Water;
               using   Xunit; 
...
/// </remarks>
    ...
    static (EditorShell Shell  , WaterMode Mode) Built(   ) { var shell = new EditorShell(1280f   , 800f  ) ;
...
() {
 ...
 using var _shell =    shell;   
          ...
          foreach (var   tool in    WaterMode.Tools) { var   id    = WaterMode.ToolCommand  ( tool);
          
                Assert.True(  shell   
.Commands.TryGet( id  , out var command)  );
    Assert.Equal(WaterMode
.WaterContext, command!   .Context);
        // Registered, listed, rebindable — and disabled, because nobody is in the mode.
          Assert
.False (command .CanExecute);
      }

    foreach (    var    id in WaterMode.
             Commands ) {
              Assert.True  (shell    .Commands.TryGet(id, out _ ),
$"{id} was not registered."    );
    }  }
   [Fact  ]
    public void Leaving_the_mode_takes_every_command_back_out() { var (shell, mode ) = Built  ();
  using var    _shell  = shell
;
               mode .
                Unregister(shell    );
             foreach (var tool   in WaterMode. Tools )  { Assert.    False   (shell   .    Commands    .TryGet   (WaterMode .ToolCommand(   tool)   , out _   ))    ;
   }
      
      foreach
             (var id  in    WaterMode. Commands) { 
            Assert.False    (shell
.Commands   .TryGet(id,   out _), $"{id} was left behind.");
    }

        for (var  slot =  0; slot < WaterMode.SlotCount; slot++ )  {    Assert.False (    shell.Commands.TryGet(    WaterMode   .SlotCommand(slot),    out  _)  );
            }
          }
    /// <summary>The digits are the tools while the mode is active, and nobody else's after.</summary>
        [Fact]
          public  void Entering_the_mode_claims_the_digits_and_leaving_it_gives_them_back() {
               var (shell, mode  )  =   Built   (    ) ; using var    _shell = shell;  
        shell
          .  Commands .Add   (    "scene.bookmark-go-2", new StringId    ("test.bookmark",  "View 2"), ()   => { });
               shell. Keys.SetDefault("scene.bookmark-go-2",  new    KeyChord    (InputKey.   Number2  , ModifierKeys   .None) )  ;
     
       var    chord =
        new    KeyChord(InputKey. Number2  , ModifierKeys  .None   )    ;
   Assert   .Equal  ("scene.bookmark-go-2", shell.Keys.CommandFor(    chord, shell    . Context))  ;
          
             shell.Modes.Activate (  WaterMode    .ModeId)   ; Assert.  Equal (WaterMode.   SlotCommand(1)   , shell.Keys .CommandFor(    chord, shell.Context   )    ) ;
             
    shell.Modes.    Activate ("select")   ; Assert   .Equal("scene.bookmark-go-2", shell.    Keys.CommandFor(    chord,   shell.Context   )); 
          Assert.NotNull(    mode    )
                ;
            }
    /// <summary>Selecting a tool through a slot is selecting it, and past the last one does nothing.</summary>
[Fact]
    public void  A_slot_selects_its_tool_and_a_fourth_does_nothing( ) { var (shell    ,  mode)   = Built( );
  using  var
                _shell = shell    ;
                
        Assert  . True (mode.    SelectSlot(
1 ));
              Assert.Equal(
WaterTool.    Profile,   mode.Tool) ;
              Assert.   False  (mode.SelectSlot   (
3)    )    ;
        Assert
.  Equal(    WaterTool  .Profile, mode  .Tool);
    }
 ...
 =   Built();
     ...
     mode.Editing   . Add(new   (0f, 0f, 0f)
                ...
                mode.
             Editing.Add(   new(10f, 0f, 0f  ) );
         
        Assert.Equal
                (2, mode. Editing.Points.Count);
...
mode .    Deactivated() ;
    ...
    /// <summary>And so does changing tool, because a draw belongs to the tool that started it.</summary>
              [    Fact] public void   Changing_tool_drops_the_draw  ()   {
        var (shell,    mode)    =
  Built   ()  ;
     using var _shell  = shell;
mode.Editing    .Add (    new(0f   , 0f, 0f)   ); mode .Tool = WaterTool.Profile;
   ...
   // --- The gesture --------------------------------------------------------
    ...
    /// <summary>A lake is three points and a closed curve.</summary>
         [Fact]  public    void Drawing_a_lake_makes_a_closed_curve() {
          var (shell   , mode) = Built   (  ); using var _shell = shell   ;
        Spline? laid =  null; WaterBodyKind?    kind   = null;
          ...
          mode   .  Editing.Add(   new (20f , 4f    ,    20f)
           ...
           mode.Editing 
            .Add(  new(   0f   ,  4f   , 20f ));
               ...
               Assert .NotNull  (mode
.Finish    (  ));
Assert.NotNull(laid)    ; Assert.True(  laid   !  .IsClosed, "a lake's curve has to close, or it has no inside.");
     Assert . Equal   (   4 , laid.Points.Length) ; Assert.  Equal(WaterBodyKind .    Lake, kind  );
             
              
    // And the gesture is over, so the next click starts a new one rather than extending this.
       Assert   
     .Empty(mode.Editing   .Points);
		}
		
    /// <summary>A river is an open curve, and two points are enough.</summary>
      [   Fact]
            public    void Drawing_a_river_makes_an_open_curve(  ) {  var (shell, mode    )   =    Built(   );
   using  var _shell = shell;
	Spline? laid = null ;
...
, mode )  = Built()    ;
        ...
        mode.Editing.Kind =    WaterBodyKind    .Lake   ; mode  .Editing.Add(new(0f , 0f ,    0f))  ;
         mode.Editing    .
        Add (    new(10f    , 0f  , 0f)   )  ;
  Assert.  False    (mode.Editing    .   CanCommit)    ; Assert.Null(mode.Finish    ()) ;
          // And it is still in flight, so the author can carry on clicking.
       Assert
              .Equal(2 , mode.Editing.Points  .  Count) ;
	...
	using  var _shell    = shell;
              mode
.  Editing.Add    (new(0f  ,  0f,    0f))  ;
			Assert. 
               True   (mode.Key   (null!, new KeyEvent  { Key = InputKey. Escape   }    ));
            Assert .Empty(mode    .   Editing   .Points  );
