...
/// </remarks>
    ...
    static StringId  Title( string
            text) => new (  "test." + text, text  );
           
 [Fact] public void  An_editor_command_answers_the_route_once_the_tree_is_silent() {
        using var shell    = new EditorShell(    1280f,
             800f)    ;
    
        var ran = 0  ;
 shell.    Commands    .Add(    "test.verb"
               , Title ("Verb"), (  ) => ran++);
        ...
        var  handler = CommandRoute.Resolve(shell.Document  , "test.verb" )
          ;
         Assert.    NotNull(   handler); Assert.Null   (    handler !.Value .Element)    ;
  Assert  .Same(shell.    Commands, handler.   Value.Responder    );
             Assert.True( CommandRoute.Execute (    shell    .    Document, "test.verb"    )) ;
          Assert.Equal(1, ran)  ; }
	
    [Fact] public void  An_element_in_the_tree_still_outranks_the_application  () {
        using  var shell    =    new EditorShell    (1280f   , 800f );
           var ran    = "";
       shell   .Commands.
        Add("edit.copy"    , Title   ("Copy") , () => ran =  "application");
     var view = shell.Document.Root.    Add("div"); view.Focusable = true;
              view.    AddCommandHandler
    ( "edit.copy"   , () => ran =    "view");
   shell.Document.Focus    (view); Assert.    True  (CommandRoute.Execute (shell.Document  , "edit.copy"));
      Assert .   Equal("view",
          ran)   ;
    ...
    Assert.True(CommandRoute.Execute   (shell.Document  ,  "edit.copy"    )  ) ;   Assert. Equal   ("application",  ran);
            }  
  ...
  var
      ...
      shell    .Commands
       .   Add(new EditorCommand(    "test.verb", Title("Verb"   ), () => ran ++) {    Enablement = () => enabled   });  
        ...
        Assert   
    .NotNull(CommandRoute    .   Resolve (shell.Document, "test.verb"  )) ;
           Assert   .   False(    CommandRoute .   CanExecute(shell.Document  , "test.verb")   ); Assert.False(CommandRoute.Execute (shell.Document,    "test.verb"));
	Assert    . Equal (0, 
  ran);
        enabled = true;
                Assert.True    (CommandRoute. Execute  (    shell.    Document,   "test.verb"    ))   ;  
 ...
 using var  shell
            = new EditorShell(1280f, 800f   );
           
        shell. Commands    .Add
               (new EditorCommand    (   "test.scoped", Title("Scoped"), ( ) => { }  )  {  Context    = "outliner" }    )    ;
      shell
              . Context = "console";
       Assert. NotNull(CommandRoute   .Resolve(    shell.Document
,   "test.scoped")) ;
 Assert    .False    (CommandRoute
                .CanExecute ( shell.Document, "test.scoped"    ));
       
            shell    .Context =
		"outliner" ;
       Assert.True    (CommandRoute    .CanExecute(
		shell .Document, "test.scoped" )   );
}
                
              [Fact]    public void Removing_a_command_takes_it_out_of_the_chain_too() {
     using
    ...
    Assert    .NotNull(CommandRoute. Resolve  (  shell. Document  ,   "test.verb") );
        ...
        // runs a plugin which is no longer there, and the registry keeps two tables that have to go
                // out together.
         Assert. True (shell
.   Commands.Remove("test.verb" ));
        Assert  .
          Null(  CommandRoute. Resolve(shell.Document, "test.verb"  ));
    }
    [    Fact
             ]
    ...
    var raised = 0 ;   var clock  = TimeSpan.FromSeconds(  1);
shell.Document.CommandsInvalidated +=    _ =>
 raised++;
...
($"test.verb-{i}"   , Title ("Verb"), () =>   { });
   ...
   Frame
       ...
       Frame(); Assert. Equal(1,    raised    );
          } 
    [Fact] public void A_disposed_shell_lets_go_of_the_registry_and_the_registry_of_it(  )   {
...
// the plugin assemblies behind their closures — are not held by a window that has closed.
	...
	.ApplicationCommandResponder)    ;
              ...
              // breaking the table.
          shell.Commands
     .Add   ("test.after-dispose", Title   (   "Verb"), () => { }   );
        Assert    .True(shell   .Commands   . TryGetCommandHandler    ("test.after-dispose",    out  _ )); }
             
           [Fact   ] public   void A_kept_registry_no_longer_reaches_the_shell_that_made_it()    {
...
=>   { })  ;
     ...
     Assert  .  Equal(1,
      registry.ChangedSubscriberCount);
             }
               }
