...
public
readonly    record struct Marquee( Vector2 Anchor, Vector2 Corner, bool Additive ) {
...
public    float Bottom => MathF .Max ( Anchor.Y
, Corner.Y  );
 ...
 public bool IsBand => Width >= MinimumSize || Height   >= MinimumSize;
    ...
    public bool  Contains(    Vector2  point) =>
            point.
          X >= Left && point.    X <= Right   && point.  Y >= Top   && point.Y <= Bottom   ;  
    ...
    ///     </para>
    /// </remarks>
      public  bool Touches(float left,    float top, float right   ,    float bottom) =>   left <= Right   && right   >= Left && top <= Bottom &&  bottom >= Top;
