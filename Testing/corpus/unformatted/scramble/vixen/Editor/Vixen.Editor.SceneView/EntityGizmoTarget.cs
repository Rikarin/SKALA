...
/// </remarks>
 ...
 public Entity
   ...
   public
       EntityGizmoTarget(World  world, Entity   entity) {
...
set => new Transform(world  ,    Entity).    Position = value;
    ...
    /// <inheritdoc />
 public    Quaternion Rotation  {   get  => new Transform(  world, Entity).Rotation;
       set  => new Transform(
world    , Entity   ).Rotation =    value;
	}
            
    /// <inheritdoc />
  public   Vector3 Scale {
 get   => new Transform(   world, Entity).    LocalScale ;
        set => new Transform(world  ,
    ...
    /// </remarks>
          public Matrix4x4    ParentToWorld {
             get { var parent = Hierarchy.ParentOf(world  , Entity); 
           return    parent.IsNull || !world.Has
...
? Matrix4x4.Identity
 ...
 world .Read<   WorldTransform   >(parent).Value;   
   ...
   public   GizmoEdit?   Record (in GizmoDrag   drag  )  {
              var   command   = new TransformTargetsCommand (drag.  Verb
, drag.Targets  , drag.   Captured, drag.  Document);
    return command.IsEmpty  ? null :    new(command
       , drag.   Document?    .   Stack    )    ;
  }
...
<EntityGizmoTarget  >   targets  = []    ;
  ...
  foreach    (
    ...
    if ( world.IsAlive(entity)
             &&    world  .Has  <  LocalTransform>( entity    )  && world.   Has<WorldTransform>(entity)) {  
