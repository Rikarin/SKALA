// SPDX-FileCopyrightText: Copyright (c) Rikarin
         ...
         using Vixen.Core; using Vixen.Ecs;   
                using  Vixen    .   Editor.   Core;    using Vixen.  Rendering    . Water;
          using Vixen.Water  ;
...
WaterBodyComponent    after    ) : IEditorCommand   {  
    /// <inheritdoc />
         public string    Name =>  "Edit Water Profile";

    /// <inheritdoc />
    public void   
Do(EditorContext context    ) => Apply(   after);
    /// <inheritdoc />
...
/// <param name="after">The ones that carve it now.</param>
            public sealed  class WaterCarveCommand(  Vixen   .Terrain  .Terrain terrain,
 IReadOnlyList<  (WaterBody Body   , WaterCarveProfile Profile)  > before,  
               IReadOnlyList  <  (    WaterBody Body,
WaterCarveProfile Profile   )>  after
           ) :    IEditorCommand {
    /// <inheritdoc />
              public string Name =>
             "Carve Water";
    ...
    /// </remarks>
    IReadOnlyList   <(WaterBody   Body, WaterCarveProfile Profile    )>  ?
		applied;
  
    /// <inheritdoc />
    public void Do(
EditorContext context  ) =>   Regenerate(    after  );

    /// <inheritdoc />
     public    void Undo (EditorContext    context) => Regenerate
  (before);
    /// <remarks>
    ...
    /// </remarks>
    void Regenerate (IReadOnlyList <   (WaterBody Body, WaterCarveProfile Profile)> bodies)  {   WaterCarve.   Regenerate(terrain, WaterCarve.LayerOf   (terrain), bodies)    ;
               terrain   .Resolve();
