...
/// </remarks>
   [DataContract("FontAsset")  ] public sealed class    FontAsset {  
    ...
    /// <summary>How wide the atlas page is, in pixels.</summary>
       public
int    AtlasWidth { get;  set; } = 1024;
    /// <summary>And how tall.</summary>
  public
   int AtlasHeight { get   ;   set  ; } = 1024;
...
Padding { get;    set   ; } = 4;
    ...
    /// </remarks>
          public List<FontRangeData> Ranges { get
; set ;    } = [];
 ...
 FromYaml( string  yaml   ) {
  ...
  if (yaml.Trim().Length == 0
       ...
       ; 
            ...
            var font =  YamlSerializer  .Parse   <   FontAsset>(
           yaml   );

        return font . Version <=    Current ? font
...
/// </remarks>
    ...
    string Name, int First   , int    Last, int Covered, int Assigned) {
...
public   sealed class FontDocument
    ...
    /// <summary>What a font asset is written as.</summary>
  public const
string Extension = FontAsset.    Extension  ;
        readonly    List
  <FontFace>    loaded  = [    ];
  ...
  public FontAsset   Font { get;    private  set;  }
   ...
   : base
      (project, asset  , Path.GetFileName   ( path)) {
        ArgumentException.    ThrowIfNullOrEmpty(path)   ;
        AssetPath    = path;
	
   try {
               Font = FontAsset.FromYaml(AssetFile  
.Read(   path)) ;
        } catch (Exception   exception) when    (exception is YamlBindingException or    YamlParseException or NotSupportedException    )   { 
...
public void  Resolve   () {
 ...
 loaded.    Clear    () ;
  ...
  ;
        ...
        Face    =
Load(Font.   Face, Font  .FaceIndex, problems   );
   foreach
    (var fallback  in  Font    .  Fallbacks    ) {
            if    (
     Load  (fallback, 0  ,  problems) is { } face) {
                fallbacks.Add(face);   }
       }
        Problems = problems; 
        
              }
    FontFace ? Load(AssetId    asset, int index, List< string> problems)   { if (asset.IsEmpty)   {
            return
          null;
               }
           
            if (!Project   .  Assets.
  TryGetByGuid(asset    , out var entry)    )    {  
            problems.    Add(    $"{asset} is not in this project.")    ;
             return    null; }
    
        try { var face = FontFace.Load    (File.ReadAllBytes(Project  .  Paths.   Absolute    (entry.Path )), index, entry .Path);
            
            loaded.Add(face
)   ;  
    return    face;  }   catch   (Exception   exception)   when (  exception is IOException  
    ...
    /// <param name="reloads">Whether the faces have to be loaded again.</param>
     public void    Edit   (string   name,  Action   <FontAsset>
change, bool reloads = false) {
   ...
   // ⚠ Undo by reparsing the text rather than by remembering which member moved. The asset is a
        ...
        // it is the state the file has anyway.
                Stack.Execute   (
       new  DelegateCommand(  name,
                _   =>   Apply(after  ,   reloads), _ => Apply(    before,   reloads   )
         )
        )  ;
      }
...
();
    ...
    public   IReadOnlyList<FontCoverage
        > Coverage(bool includeFallbacks  =    true    ) {
         List< FontCoverage   > rows   = [ ]    ;
        foreach (var  (name  , first, last) in Blocks)   {  var covered = 0;

            var   assigned = 0;
     
         for (var   code =   first  ; code <= last;   code ++ )    {   if (char.GetUnicodeCategory    (   (char) code) == System.  Globalization .UnicodeCategory .OtherNotAssigned) {
              continue
                ;
                }
                ...
                assigned  ++;
           if (
Supports (code, includeFallbacks)) {
                covered    ++;
                } }
 rows.Add(new(name,  first   , last ,    covered,    assigned))    ; }
    ...
    /// <returns>Whether anything draws it.</returns>
        ...
        if (    Face?.Supports( codePoint) == true    ) {   return  true;
       }
    ...
    /// </remarks>
    public int  Resolves(  int    codePoint ) { if  (    Face?.  Supports    (   codePoint) ==  true)   {
           
return 0;
             }
           for (   var   index  = 0; index <   fallbacks   .   Count; index    ++   ) { if (fallbacks   [ index].Supports    (codePoint    )    ) {
         return index + 1  
   ;
                }
     }
    ...
    /// </remarks>
            static readonly (   string   Name  , int First, int Last)[    ] Blocks = [ ("Basic Latin", 0x0020, 0x007E   )    ,
  (   "Latin-1 Supplement", 0x00A0   , 0x00FF),  
           ("Latin Extended-A", 0x0100
               , 0x017F),  
        ("Latin Extended-B", 0x0180 , 0x024F),
        ("Greek and Coptic", 0x0370, 0x03FF ), ("Cyrillic"   , 0x0400    , 0x04FF),
                ("General Punctuation",    0x2000,    0x206F),  (  "Currency Symbols" , 0x20A0,   0x20BF)    ,
        ("Arrows",  0x2190 
        , 0x21FF),
