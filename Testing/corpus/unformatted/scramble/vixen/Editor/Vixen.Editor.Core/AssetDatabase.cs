...
namespace Vixen.Editor
    ...
    public sealed class AssetDatabase {
        ...
        public bool TryGetByPath(
            string relativePath,
            [MaybeNullWhen(false)] out AssetEntry entry
        ) =>
        ...
        public ScanReport Scan(ScanOptions? options = null) {
            ...
            Parallel.For(
                ...
                candidates.Count,
                index => {
                    claimed[index] = survey.Sidecars.ContainsKey(candidates[index].Absolute); found[index] = Read(
                        candidates[index],
                        survey,
                        previousEntries,
                        previousStamps,
        ...
        static bool IsFresh(
            ...
            previousStamps.TryGetValue(relative, out var recorded)
            && recorded == current
            && current.WrittenUtc < cutoff;

        /// <summary>
        ///     The project-relative form of a path the walk produced, which is known to be under the root.
        /// </summary>
        /// <remarks>
        ...
        void Insert(AssetEntry entry, MetaStamp stamp, ScanOptions options, ConcurrentBag<AssetIssue> issues) {
            ...
            var message =
                ...
                + (incomingMatches
                    != existingMatches ? "The one whose recorded sourceHash still matches its file kept it."
            ...
            issues.Add(
                new(
                    ...
                    DuplicateGuid,
                    loser.Path,
                    $"{message} '{loser.Path}' was re-GUIDed to {minted}."
                )
            );
        }
        ...
        void Quarantine(Survey survey, bool[] claimed, ScanOptions options, ConcurrentBag<AssetIssue> issues) {
            ...
            foreach (var owner in survey.Sidecars.Keys.Order(StringComparer.Ordinal)
                ...
                if (!options.QuarantineOrphanMeta) {
                    issues.Add(
                        new(AssetIssueKind.MetaOrphaned, relative, "Its asset is gone. It was left where it is.")
                    );
                    continue;
                }
