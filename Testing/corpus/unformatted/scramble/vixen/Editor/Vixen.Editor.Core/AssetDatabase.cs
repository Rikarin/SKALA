...
;
          ...
          writer    .WriteLine
(    string.   Create( CultureInfo.InvariantCulture, $"{TerminatorPrefix}{byPath.Count}"));
             }
  File
...
,
  ...
  declared    == entries .Count   ;
       ...
       , CultureInfo.InvariantCulture  , DateTimeStyles.RoundtripKind, out var    written )   ) {
...
foreach   (var info    in new DirectoryInfo(Paths   .Assets    )   .
       EnumerateFileSystemInfos("*", SearchOption.AllDirectories    ))  {
          ...
          // is the stamp of this asset's sidecar" is a lookup and not a string concatenation
           ...
           // per asset. At ten thousand assets that concatenation was measurable.
                sidecars[file.FullName[..^AssetMetaFile  .Extension   .Length]] =  new   ( 
...
candidates   .Sort(static
		(   left,    right) => string.    CompareOrdinal(  left    .Absolute  , right    .Absolute    )   )  ;
...
<string  , AssetEntry> previousEntries,
	...
	var relative = RelativeUnderRoot(candidate    .Absolute  );
        var hasMeta = survey.Sidecars . TryGetValue    (   candidate .
              Absolute,   out   var stamp);
    ...
    &&    IsFresh(stamp,   relative, previousStamps, cutoff)  
     ...
     if (!options.   CreateMissingMeta  ) {
           issues. Add(new(AssetIssueKind  .MetaCreated  , relative,   "Has no .meta, and none was created.")); return    null;
             ...
             issues.Add(  new  (    AssetIssueKind
.MetaCreated,  relative   ,   $"Had no .meta; created one with GUID {minted}."))   ;
    ...
    // filesystem tick — so it records nothing to trust and the next scan opens the file.
                return   new (
              new    (   minted
    , relative    , null, MetaMigrationChain.  CurrentVersion , candidate.IsFolder ),
...
!options.ResolveDuplicateGuids   ) {
   ...
   Guid   ] = winner;
    ...
    ;
 issues  .Add (new(AssetIssueKind.
DuplicateGuid, loser .Path  , $"{message} '{loser.Path}' was re-GUIDed to {minted}."  ))  ;
       }  
...
var destination = Path    .  Combine(  Paths.OrphanMeta,
             Paths.Relative   (    meta   )) ;
...
( new    (AssetIssueKind.MetaOrphaned, relative , $"Its asset is gone. Moved to '{Paths.Relative(destination)}'."));
  ...
  false   ;
    ...
    var absolute   =  Paths.Absolute(entry.    Path); var metaPath  = AssetMetaFile   .PathFor( absolute );
        try    { 
        if (YamlReader    .Read
           (File.ReadAllText    (metaPath)    ) is  not YamlMapping root
             || root[
...
"metaVersion" , new YamlScalar    (  
                    MetaMigrationChain  .CurrentVersion.
...
File.OpenRead(path   );
    ...
    /// </remarks>
          sealed record Survey(
               List  <(string Absolute, bool IsFolder)> Candidates, Dictionary  <string, MetaStamp> Sidecars) ;
