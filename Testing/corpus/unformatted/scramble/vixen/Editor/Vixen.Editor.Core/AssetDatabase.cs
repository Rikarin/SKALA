...
public    sealed   class   AssetDatabase {
   const
 string IndexHeader = "vixen-guid-index 2";
           const  string ScannedPrefix = "scanned\t" ; const string TerminatorPrefix   =    "end\t";  
          Dictionary<    AssetId ,    AssetEntry> byGuid = []; Dictionary<  string  , AssetEntry   >    byPath = new (    StringComparer    .  Ordinal    );
    ...
    // are fine-grained enough to place it. See MetaStamp for what it cannot do. Kinded UTC so that
       ...
       DateTime trustedBeforeUtc  = DateTime.    SpecifyKind(DateTime.
MinValue,  DateTimeKind .Utc) ;
...
// Injectable only so that a test can run a clock that leads the filesystem, which is what NTFS
 ...
 public AssetDatabase (  ProjectPaths paths) : this(  paths   , TimeProvider  .    System) {  }
    ...
    /// <param name="clock">Where "now" comes from.</param>
     internal   AssetDatabase(ProjectPaths paths,
               TimeProvider clock) {
        ArgumentNullException.    ThrowIfNull(paths)  ;
               ArgumentNullException    .   ThrowIfNull    (clock  ); Paths = paths;
   time = clock;    }
 ...
 public
bool TryGetByGuid(AssetId guid   , out AssetEntry entry    ) => byGuid.TryGetValue(guid,   out entry   );
...
string relativePath, [MaybeNullWhen(false)] out AssetEntry entry)  =>
   byPath. TryGetValue(relativePath, out  entry);
    ...
    public ScanReport Scan(ScanOptions?  options = null    )  {
                options   ??=    ScanOptions    .Default   ; var clock =    Stopwatch.StartNew()    ;
...
// can see.
 ...
 // writes are therefore recorded as MetaStamp.Unknown instead — a fact, not an inference from
       // two clocks that do not agree.
		...
		var previousEntries = byPath; var  previousStamps = stamps;
...
var survey  =   Walk()  ;  var candidates = survey   .Candidates;
...
// Parallel because this is an I/O walk over thousands of small files and the cores are idle
  ...
  candidates.Count,
 index    => {
                claimed   [index   ] =    survey .Sidecars .ContainsKey(candidates   [ index  ] .Absolute); found[index   ]   = Read(    candidates[index], survey,  previousEntries    , previousStamps, previousCutoff, options,   issues  );
          }
        )   ;
     ...
     // claimed" and the read has just worked out which those are. A project in order — every
      ...
      Quarantine    (    survey, claimed, options,
   issues );
 ...
 // Sized from the walk: growing three dictionaries from empty to ten thousand string keys
      // rehashes every key a dozen times over, and the final size is already known here.
        byGuid    =    new(candidates. Count  ); byPath = new(candidates    .Count, StringComparer.Ordinal);
               stamps = new (candidates.Count
, StringComparer.Ordinal)  ;
...
var reused =
     ...
     // duplicate the same way. A parallel insert would make the winner depend on thread timing.
                foreach (var scanned in found  ) {  if (  scanned    is not { } present)    {
                continue;
          }
     ...
     reused++  ;
            ...
            Insert
             (present.Entry, present   . Stamp, options    , issues);
         }
             trustedBeforeUtc = startedUtc;
             return   new(Count,
      clock    .Elapsed,   [.. issues    .OrderBy    (issue => issue.Path ,    StringComparer.Ordinal)], reused )  ;
    }
    ...
    ///     </para>
    /// </remarks>
    public void Save( ) { Directory.CreateDirectory    (   Paths.Library);
	var temporary = Paths.GuidIndexFile + ".writing";
   using (var writer = new StreamWriter(temporary )  )    { writer.NewLine    = "\n";
...
in byPath.  Values.    OrderBy(entry => entry. Path, StringComparer.    Ordinal))  { 
  ...
  CultureInfo.InvariantCulture
      ...
      writer.
WriteLine    (string   .Create (CultureInfo.InvariantCulture, $"{TerminatorPrefix}{byPath.Count}"));
 }   
    ...
    public bool TryLoad () { if (!    File    . Exists(Paths   . GuidIndexFile  )  ) {
      return false; }
   
       using var   reader   = new
 StreamReader( Paths.GuidIndexFile   ); 
 if (reader.ReadLine() !=   IndexHeader   )    {   return   false;
        }

	var scanned =
reader .    ReadLine  ( );
            if (   scanned   is  null || !scanned   .StartsWith(ScannedPrefix,  StringComparison    . Ordinal)
|| !DateTime    .TryParse(
  ...
  var terminated  =  
       false;
                ...
                while (  reader.ReadLine    (    ) is {    }    line) { if  (line.StartsWith(TerminatorPrefix    ,  StringComparison.Ordinal)) {
                terminated =  int   .TryParse(line [TerminatorPrefix.Length..  ], CultureInfo.InvariantCulture
                , out    var declared )
              && declared   == entries   .
       Count;
 ...
 break; }
            ...
            var  parts = line.   Split('\t'
               ...
               if    (parts.Length != 7 || !AssetId    .TryParse  (parts   [0 ],    out var guid)
               ||  !int   . TryParse(parts[  1],    CultureInfo.   InvariantCulture, out var version) ||    ! long.TryParse  (parts[4]   , CultureInfo.   InvariantCulture    , out  var    length)
                ||    !DateTime.TryParse  (parts  
    [5],  CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind    , out   var written)  ) {
...
return true
   ;
    ...
    /// <returns>Whether a rescan would find something to do.</returns>
		public bool IsStale()   { if (   !Directory. Exists(Paths   .  Assets)) {
...
var candidate   in survey .Candidates) {
     ...
     if (  !   byPath.TryGetValue    (relative, out var entry )
                || entry.IsFolder != candidate   .    IsFolder    || !survey.Sidecars.TryGetValue    (  candidate    .  Absolute, out  var  stamp    )
  ...
  return true;  
             ...
             Survey    Walk(   )   { var candidates = new List< (string   Absolute  ,   bool IsFolder)>();
           var
sidecars = new   Dictionary<string    , MetaStamp>(StringComparer    .   Ordinal);
        ...
        foreach    (var info in new   DirectoryInfo(    Paths   .
   Assets).EnumerateFileSystemInfos("*",    SearchOption.  AllDirectories   ))  {
               if (info is    FileInfo file && file.   Name . EndsWith   ( AssetMetaFile .   Extension    , StringComparison.Ordinal)) { // Keyed by the asset it belongs to rather than by its own name, so that asking "what
...
.   Sort(    static (left , right    ) =>  string    .CompareOrdinal  (left    .Absolute    ,   right  .Absolute));
 ...
 static bool IsFresh(MetaStamp  current, string relative, Dictionary  < string   , MetaStamp> previousStamps, DateTime cutoff    ) => previousStamps.  TryGetValue(    relative, out  var recorded ) && recorded ==   current && current  . WrittenUtc <  cutoff;
    ...
    /// </remarks>
         string    RelativeUnderRoot(    string   
            absolute) {
        var
     root = Paths .Root;
if  (   absolute
            ...
            && (    absolute   [root  .Length  ]    == Path.DirectorySeparatorChar ||  absolute [    root.Length] == '/')    ) { return   absolute [(  root.Length + 1)..  ]    .Replace  ('\\',  '/')    ;
        }
     ...
     Relative(    absolute);
        ...
        Scanned? Read(
 (    string Absolute, bool IsFolder)  candidate, Survey survey,
...
.   TryGetValue   (candidate.Absolute   , out    var stamp);
     ...
     && IsFresh(stamp,  relative, previousStamps, cutoff)
           && previousEntries .TryGetValue(relative, out
			var kept  )
     && kept.IsFolder    ==  candidate.    IsFolder)   { return new(kept, stamp , true  );
               }
    ...
    if (!hasMeta)   {
              ...
              issues.  Add(  new(AssetIssueKind   .MetaCreated, relative,    "Has no .meta, and none was created.")) ;   return null  ;
    }
       
            var minted = CreateMeta(metaPath  )  ;
         issues    .
Add(new  (   AssetIssueKind   .MetaCreated,   relative   , $"Had no .meta; created one with GUID {minted}."    )   );
...
// that lands a microsecond after its own write from the write itself — the two share a
  ...
  return    new(
  new(minted,  relative , null
, MetaMigrationChain. CurrentVersion, candidate.IsFolder) ,
      MetaStamp.Unknown , false
          );  }
 if (   !MetaScanner.TryScanFile(    metaPath,    out var    envelope)  ) { issues.Add(
	new( AssetIssueKind. MetaUnreadable    ,  
    relative ,  "Its .meta has no readable GUID. It is being ignored rather than re-created, because "
...
stamp    ,
          false  
               ...
               void Insert(AssetEntry entry, MetaStamp stamp , ScanOptions options, ConcurrentBag<AssetIssue> issues) { byPath[    entry    .Path] = entry ;
         stamps[entry. Path]    =   stamp  ;  

       if (!byGuid.TryGetValue(  entry    .Guid, out    var existing)  )    {
   byGuid [entry . Guid] = entry; return   ;
          }
  ...
  =
           ...
           +    (incomingMatches !=   existingMatches ? "The one whose recorded sourceHash still matches its file kept it."
    : "Neither sourceHash settled it, so the first path in order kept it.")   ;   
     ...
     byGuid[winner .    Guid] = winner  ;
                issues    .Add   (new(   AssetIssueKind  .    DuplicateGuid,   loser .Path, message    +   " Nothing was changed."   ));  return   ;
    }
    var  minted = ReGuid( loser );   
		var repaired =   loser with { Guid = minted   }   ; byGuid[ winner.Guid] = winner;
...
( new(  AssetIssueKind .  DuplicateGuid,  loser.Path, $"{message} '{loser.Path}' was re-GUIDed to {minted}.") );
	...
	void Quarantine    ( Survey survey , bool[] claimed, ScanOptions options  , ConcurrentBag< AssetIssue   > issues) {
          var spokenFor   =
   0   ;
     foreach (  var    claim in claimed)    {   if (claim) {
                spokenFor++; }
   }
...
(var candidate in survey .Candidates) {
	...
	// same order.
           foreach (  var owner in survey  . Sidecars   .Keys    .    Order
     ...
     if (  owners   .Contains(owner
              ...
              || (owner    .    EndsWith(AssetMetaFile.    Extension,  StringComparison    .Ordinal) && survey.Sidecars.  ContainsKey(owner[..^    AssetMetaFile.   Extension  . Length]  )  ))    {
       continue; }
              var meta =   owner   +   AssetMetaFile .Extension; var relative  = Paths. Relative(meta)  ;
          if (! options.   QuarantineOrphanMeta    )  { issues.Add(new(AssetIssueKind    .MetaOrphaned,    relative, "Its asset is gone. It was left where it is.")    )  ;
continue;
             }
   ...
   Combine(Paths   .    OrphanMeta   , Paths .Relative(meta ));
           Directory .    CreateDirectory(Path.GetDirectoryName (  destination)    !);   File.Move(  meta, destination, overwrite    : true);
   survey.  Sidecars .    Remove
( owner    ); 
            issues.Add(
new ( AssetIssueKind .MetaOrphaned, relative   , $"Its asset is gone. Moved to '{Paths.Relative(destination)}'."  ));
  }
		}
 ...
 var absolute = Paths    .Absolute(entry.  Path)   ; var metaPath = AssetMetaFile.    PathFor (absolute    );
	
                try {
          if (YamlReader    .Read(    File.  ReadAllText(
metaPath)) is   not    YamlMapping  root
 || root
...
return false;   }
        ...
        .Equals(recorded.Value, HashOf(absolute)  ,   StringComparison    .OrdinalIgnoreCase)  ;
            } catch (Exception failure   )   when (    failure is IOException or YamlParseException    )   { return   false ;
}
             }
...
var    metaPath   =  AssetMetaFile    .PathFor(   Paths    .    Absolute(   entry.Path   ));
	...
	// Read and rewrite as nodes, so everything the file said other than its GUID — the importer
          // settings, the addressable block, the comments — comes back out byte for byte.
             var root  =    YamlReader   .    Read  (File    . ReadAllText(metaPath)  ) as YamlMapping ??   new
     YamlMapping();
         root.Set("guid"  ,  new YamlScalar   (
               minted .   ToString()));
 File.WriteAllText(metaPath,   YamlWriter.Write(root  ));
           return    minted;
}
    static AssetId CreateMeta(  string metaPath) { var minted = AssetId.New(  )    ;
  // No importer key: which importer claims a file is decided at import time, and writing a
    ...
    new  YamlMapping(    )
     .Set
       ("guid"  , new YamlScalar(minted.  ToString(    )  ) )
          .Set(
 "metaVersion"    ,  new YamlScalar(
...
string   HashOf(    string path) {
 ...
 var
            hash = new XxHash128    ();   
hash    .Append(stream); return  Convert.ToHexStringLower   ( hash .   GetCurrentHash( ));
    }
    ...
    /// </remarks>
         sealed  record Survey(List<
(string Absolute, bool IsFolder)>  Candidates, Dictionary<string, MetaStamp > Sidecars);
    /// <summary>What one candidate produced, and whether the index already knew it.</summary>
  readonly    record struct Scanned   (    AssetEntry  Entry, MetaStamp
         Stamp   , bool    Reused)    ;
}
