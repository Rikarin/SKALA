...
// SPDX-License-Identifier: Apache-2.0
               ...
               using    System.Globalization  ; using Vixen.    Core   ;
using Vixen   .Core.Mathematics;    using Vixen.   Core.Yaml  ;   
using Vixen.Ecs;
         using    Vixen.
         Editor .Assets.Scenes   ;   
               using Vixen
         .Editor   .Core.Scenes   ;
        using Vixen .Engine.Transforms; using Vixen .Physics;
using Vixen.
       Physics.Ecs;   
...
;
 ...
 using   var physics = new PhysicsScene (world)    ;
  ...
  // The rigid body, which is what a scene cannot yet carry on its own — see the class remarks.
          world.   Add (dinghy,  Collider.Of
         (physics   . Shapes.    Sphere(0.75f))    )    ;
  world.Add    (dinghy
 , RigidBody .Dynamic (   ) with {    Mass = 400f,  AllowSleeping = false })  ;   
        var zones =   Fold(world)  ; var buoyancy = new BuoyancySystem(   physics    ,  zones  )   ;
    ...
    zones. WaterTime = index * Step
     ...
     physics   .Synchronize(Step);
			buoyancy
.Step(world)    ;
         physics.Step(Step)  ; physics.  Writeback  () ;
		}
            var state =
            world    .Read  <BuoyancyState>    (dinghy) ;
           var   settled  = world.   Read   <
LocalTransform >(dinghy   )   . Position;
               Assert
		.True(state.    IsFloating , "the crate authored in the scene never touched the water")    ;
         var query = zones    .  QueryAt(new( settled.  X,  
       settled .Z))  ;
        Assert.NotNull(
    query);
...
// rather than sunk to the bed twelve metres down or hovering above the swell.
                var
  surface = query  .  Height   (new(settled  . X, settled   . Z ),   zones .WaterTime) ;
        Assert.InRange(settled.Y, surface - 0.75f,
       surface + 0.75f) ;
      }
...
Fold(world) ;
           ...
           Assert
    .Equal(1, zones.ZoneCount)  ;
        Assert.  Equal(
               1, zones.BodyCount);
			Assert. Equal(  0   ,    zones.    ZonelessBodies) ;
    Assert.Equal   (0, zones .UnresolvedBodies);
        Assert.Equal (0
  , zones.UnresolvedWaves)   ;
             
        var
query   =    zones    .QueryAt(   Vector2.Zero)   ;   
            
              Assert.   NotNull   (query); Assert.    True(query.MaximumAmplitude  >   0f, "the sea state off disk sums to a dead flat sea"  )    ;
   ...
   Assert.NotEqual(query.
              Height (Vector2.Zero, 0f  ) , query.    Height(Vector2.Zero, 1.7f   ), 4)   ;
 }
  ...
  // The transforms first: the fold rasterises a body where `WorldTransform` says it is, and a
		...
		// scene that has just been instantiated has only written the local ones.
                new    TransformSystem() .Resolve(
                world);
        var zones = new WaterZoneSystem(new RenderView( "test")) { Splines = new PondSpline  (    ),
    Waves = new    FileWaves()   ,
...
(problems);
 ...
 ;
  ...
  sealed   class PondSpline  : IWaterSplineSource {
              public Spline
         ? SplineFor(string  name   , in Matrix4x4 placement) {
            if  (!string. Equals   (name, "water/pond", StringComparison.Ordinal)    )    {   return null;
             }
               var transform = placement;
             
    Vector3[] ring = [
    new  (   -24f    , 0f, -24f ),   new   ( 24f ,    0f, -24f)  ,  new (  24f  , 0f
 , 24f    ) , new(-24f  ,   0f   ,  24f)
                ];
...
ring  [index ]   =    Matrix4x4  .TransformPosition(ring   [index ],  transform)    ;
    ...
    sealed class FileWaves :    IWaterWaveSource   {
        public WaterWaveSpectrum?    SpectrumFor(string name) {  if (!string.Equals(name,    "water/pond", StringComparison    .Ordinal)   ) {  
                return null   ; }
    
       var   path = Path    . Combine(AppContext .BaseDirectory, WavesFile);
         
 Assert   .   True(
File.Exists (  path),    FormattableString    .Invariant(    $"{WavesFile} was not copied beside the test")) ;
		var asset   
	= YamlSerializer.Parse <  WaterWavesAsset>   (  File .ReadAllText    (  path));
             Assert   .    NotNull( asset    )  ; Assert .Null  (asset   .Validate    ()    );
