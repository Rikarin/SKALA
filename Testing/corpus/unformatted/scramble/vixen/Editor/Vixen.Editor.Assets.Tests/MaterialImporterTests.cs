...
Vixen.Editor.Assets  .Materials;
            using Vixen.Rendering.Materials; using  Xunit;
      namespace Tests    ;
/// <summary>
...
public   sealed    class MaterialImporterTests   {
 static    readonly
   AssetId   Albedo = new (  Guid.    Parse("9e8a44c9930c64e388ca034c5fe4c426"));
    [Fact] 
              public void ItClaimsTheMaterialExtensionAndNothingElse()
{
          var importer =  new MaterialImporter();
     
        Assert.Equal    (   "MaterialImporter",    importer
             .Name   );
    ...
    /// </remarks>
 [    Fact]    public   void TheArtifactTypeIsTheContractOfWhatIsWritten(  )  {
        Assert.True(    TypeRegistry .TryGetByAlias(MaterialImporter   .MaterialType
, out var descriptor)    );
  Assert   .Equal(  typeof(   MaterialContent)
           , descriptor.  Type   );
	}
    ...
    /// </remarks>
    [Fact    ]
           public async Task AMaterialWithFeaturesCompilesToAChunkThatReadsBack()    { var (_, result) = await  Import(
            """
            shader: ForwardPlus
            ...
            var    artifact = Assert.Single(result.    Artifacts   );    Assert. Equal(    MaterialImporter  .MaterialType, artifact.Type  );
  Assert
            .   Equal   (SubAssetId.Main   , artifact.SubAsset );
        
        var content   =  Serializer. Read<MaterialContent
   >( artifact.Content.ToArray(    ));
              
        Assert .   Equal  ( "ForwardPlus"   ,    content .    Shader  ); Assert .Equal(  2, content   .    Features.Length    ) ;
	var workflow = Assert.  IsType<    MetalRoughnessFeature >    (content.   Features[0
   ]  );   
        Assert.Equal (   1f, workflow   . Metalness )    ;
              Assert
.Equal(    0.25f , workflow.    Roughness);
Assert
     .  Equal(0.8f  ,  workflow.BaseColor. X, 1e-6f  );
    ...
    /// </remarks>
               [Fact]
     public async   Task ATextureIsBothCarriedAndDeclared() { var (context, result) = await Import    (
            """
            features:
        ...
        Assert.Contains (  Albedo
,    context.AssetDependencies ) ;

            var content =    Serializer .  Read<    MaterialContent   >(Assert.Single(  
result.Artifacts  ).Content.ToArray(    ));
         var texture
       = Assert.Single (content. Textures);
           Assert .  Equal("baseColorMap"   ,   texture.
            Parameter) ;
    Assert.Equal
             (  Albedo, texture.Texture    .  Asset  );
           }
    /// <summary>
    ...
    [   Fact]
             public async
  Task   AShadingModelThisBuildDoesNotHaveIsAnError() {
               var (_,
result   ) = await    Import(   "shading: ToonShading\n")   ;
  ...
  public    async Task AMaterialThatWouldCompileWrongIsRefusedWithTheCompilersOwnWords() {
            ...
            """
                ...
                Assert. False    (result.Succeeded    ); Assert.   Empty(  result.Artifacts)    ;
     Assert.Contains    (
           "NormalMapSurface", Assert .   Single(result .Diagnostics) .Message   ,
    ...
    /// </remarks>
          [ Fact    ] public    async Task AMaterialWithNoFeaturesIsBuiltAndQuestioned() {
			var (    _,   result   )   =    await Import(    "shader: ForwardPlus\n") ;
            Assert  .  True(result.Succeeded);   Assert.    Single (  result.Artifacts    ) ;
      Assert
.Equal(ImportSeverity.Warning, Assert   .Single(    result.    Diagnostics  ).    Severity);
  }
    /// <summary>A material from a newer editor is refused by number rather than half-read.</summary>
            ...
            var
(_ , result) = await    Import($"version: {MaterialContent.Current + 1}\n");
...
.   Artifacts);
    ...
    [Fact] public async Task AFeatureThisBuildDoesNotHaveIsReported() {
      ...
      VirtualPath   ( "/Assets/hero.vxmat" ) ;
        var    files    =   new  MemoryFileProvider (   )  ;    files.Seed   (   path, text);
            
        var importer = new MaterialImporter(   );
       var context = new ImportContext
	(    AssetId    .New    (   )  , path , importer.CreateSettings(  ), files,    importer .Name,    "Windows");
