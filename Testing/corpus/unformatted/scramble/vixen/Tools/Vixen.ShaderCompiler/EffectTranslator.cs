...
using Vixen.Raven.Artefacts; using    Vixen.Raven.IR    ;
           using Vixen.Raven.    Reflection  ; using Vixen    .   Shaders ;  
...
public static class  EffectTranslator {
    ...
    /// </param>
      public static EffectData Translate(CompiledEffect
                effect, ShaderComposition composition = default ) {
        ArgumentNullException.ThrowIfNull   (effect
        ) ;
               var reflection = effect   .Reflection ;  var    block =    BlockOf(reflection  );
	
  return  new()  {
            ShaderName = effect.Name, Target    = effect  .Target    ,
             SourceHash =
     effect   .    SourceHash    ,
...
Stages   =    [.. effect
 ...
 Parameters =   [
		...
		VertexInputs = [
                .. reflection   .   VertexInputs.Select(  input => new EffectVertexInputData    (input.Name, input    .Location    ,
                Kind(    input.Type)) ) 
            ]   ,   PushConstants = [
           .. reflection.  PushConstants .  Select( range => new EffectPushConstantData    ( 
          Stages (range.Stages)  , range.Offset,
     range    .   
           Size  ,
         [
 .. range.   Members.Select(
         member  => new  EffectPushConstantMember (member.   Name    ,   member.Offset   ,
member.Size)
 ) ]
                )
          )
    ...
    static
        ...
        foreach (  var (  name    , value) in   effect.
        PermutationKey) { 
            var declared = effect .Reflection.Permutations.   FirstOrDefault  (permutation => permutation    . Name ==  
  name  );
          yield return new    (
  ...
  value == "default" ?  declared ?.    DefaultValue ??   value :
            value,
       Kind(declared    ?.
 Type   )    ,
   declared?.DefaultValue ?? string  .    Empty
     ...
     {
             foreach (   var    binding  in   set.  Bindings    )    { yield    return new(
   binding    .Name
           , 
    (DescriptorSetSlot    )  set    .   Set, (uint)   binding.Binding ,
                    Kind(binding    .Type),
...
binding.
    ...
    ///     </para>
    /// </remarks>
    static    IEnumerable<EffectParameterData> Parameters(string shaderName    ,  RavenReflection reflection) { foreach (   var block in Blocks(reflection)   ) {
            foreach  (var parameter in   Parameters(shaderName,  reflection,    block) ) { yield return parameter;   
 }
}
             }
    ...
    /// </remarks>
           ...
           > Blocks(   RavenReflection reflection) {
               foreach (var set in reflection .Sets)   { foreach (var binding in set  .Bindings  ) {
             if ( binding.
      Type ==   DescriptorType. UniformBuffer &&    binding.Members.Length > 0)  {
  ...
  static IEnumerable< EffectParameterData  >    Parameters(
         string    shaderName  ,  RavenReflection reflection   , UniformBlock block ) {
      var slot = (DescriptorSetSlot)block  .Set   ;
   
        foreach (var  parameter in   reflection.Parameters) {    if (  parameter.Set  != block.Set || parameter.Binding !=   block.Binding.Binding)    {
                continue 
        ;
        }
...
;
            ...
            var count = ElementCount  (block
.   Binding, parameter.  Name[..marker]);
           for (
...
(    shaderName , name ),
kind    ,
           parameter   .Offset + (index * parameter.
   ArrayStride),
     parameter.   Size, slot   
                );
       }    }
   ...
   static
        ...
        , arrayName  , StringComparison.  Ordinal)   ) {
   return member.Type    .ArrayLength ?? 0;
    ...
    /// </remarks>
static UniformBlock   ? BlockOf(RavenReflection   reflection    ) { foreach  ( var set in reflection   . Sets    ) {
 ...
 if (binding.   Type   == DescriptorType.UniformBuffer &&    binding.Members.Length > 0) {
   ...
   binding);
    ...
    /// </remarks>
        static string Qualified(string shaderName   , string name
)    => $"{shaderName}.{name}"    ;
  ...
  .ShaderStage Stage( RavenStage stage ) =>  
         stage switch {
            RavenStage    .Vertex => Vixen.Graphics  .ShaderStage  
                .  Vertex,
             RavenStage  . Fragment =>  Vixen  .Graphics .
       ShaderStage.Fragment    ,
       RavenStage.Geometry =>  Vixen. Graphics.ShaderStage.   Geometry,    RavenStage .   Compute => Vixen.Graphics.ShaderStage   .   Compute ,
...
Vixen.Graphics. ShaderStage Stages (    ShaderStages stages   ) {
        var result =   Vixen.Graphics.  ShaderStage.    None
            ;
              if  (stages.HasFlag(ShaderStages .Vertex  ))    { result |=   Vixen.Graphics. ShaderStage.Vertex;

        }   
			
     if
             (stages.HasFlag   ( ShaderStages.  Fragment))  {
   result |= Vixen.  Graphics .ShaderStage. Fragment; }
        if  (   stages.
			HasFlag(ShaderStages.    Geometry    ))    {
    result |= Vixen 
  ...
  result  |= Vixen. Graphics
   ...
   static DescriptorKind  Kind (   DescriptorType type
     ...
     type switch { DescriptorType.  UniformBuffer =>    DescriptorKind. UniformBuffer,
DescriptorType
        .StorageBuffer =>   DescriptorKind.StorageBuffer,
        DescriptorType.SampledTexture =>    DescriptorKind   .SampledTexture   ,
   DescriptorType.Sampler =>  DescriptorKind .Sampler, DescriptorType.    StorageImage => DescriptorKind.StorageTexture,
...
.    AccelerationStructure =>    DescriptorKind.AccelerationStructure,
 _ =>  DescriptorKind   .UniformBuffer
    ...
    /// </remarks>
           static ShaderValueKind Kind(  ShaderDataType? type  )    {  if (  type is null  ||    type.IsStruct)    {
     ...
     (3, 3)   => ShaderValueKind    .Matrix3x3,
        ...
        return type .Scalar switch {   IrTypeKind.    Float =>   type    .  Rows switch {
      1  => ShaderValueKind.Float,
2
            =>   ShaderValueKind    .Float2   ,
   3    => ShaderValueKind. Float3, 
         4 => ShaderValueKind. Float4, _    => ShaderValueKind .   Unknown
         },
        IrTypeKind   .Int => type   .  Rows switch  {   1   => ShaderValueKind .   Int ,
    2 => ShaderValueKind   .
   Int2,
...
_ => ShaderValueKind.Unknown
            ...
            ,
            IrTypeKind.  UInt => type . Rows == 
              1 ? ShaderValueKind.   UInt : ShaderValueKind    .Unknown   ,
           IrTypeKind  .Bool => type    . Rows == 1 ?    ShaderValueKind.Bool :  ShaderValueKind.    Unknown,
  IrTypeKind   .Double => type
			.Rows == 1  ? ShaderValueKind. Double :   ShaderValueKind  . Unknown    ,
            _ => ShaderValueKind.Unknown
 };
    }
