...
Composition = [.. composition.Slots
          ...
          Stages =
       [.. effect .   Modules    .Select(module => new EffectStageData(Stage (    module.  Stage)   , [.. module  . Bytes    ],   module.Name)    )] ,
 Bindings = [.. Bindings(reflection)  ]  ,
               ConstantBufferSize = block?.Binding.Size ?? 0   ,
		Parameters = [.. Parameters(effect.Name   ,
reflection)]    ,
                VertexInputs
= [
    ...
    /// </remarks>
      ...
      IEnumerable<UniformBlock> Blocks(RavenReflection reflection) {   
        foreach (   var set in reflection.Sets ) { foreach (var binding   in set.Bindings) {
               if (  binding.  Type ==
   DescriptorType. UniformBuffer && binding   .Members .Length    > 0) {   
...
kind = Kind(    parameter  .Type);
                var marker = parameter  .  Name.IndexOf ("[]", StringComparison
            .Ordinal);
    if (   marker <  0) {
                yield return    new  ( Qualified (shaderName,
             parameter.Name) , kind ,    parameter.Offset, parameter .Size    ,    slot);
                continue;
            }
...
kind ,
           parameter 
   .Offset + (    index   * parameter.ArrayStride),
     parameter.Size, slot  
                );
       }   }   
           }
...
(string shaderName, string name)   => $"{shaderName}.{name}";
...
IrTypeKind   .Int => type   .  Rows switch  {   1   => ShaderValueKind .   Int ,
   ...
   IrTypeKind  .
UInt => type . Rows == 1 ? ShaderValueKind   .UInt :    ShaderValueKind.   Unknown  ,
  IrTypeKind.Bool =>    type .Rows == 1    ? ShaderValueKind.Bool  : ShaderValueKind    .Unknown   ,
IrTypeKind.Double => type.Rows  == 1 ? ShaderValueKind .
Double   :  ShaderValueKind .    Unknown,
            _ => ShaderValueKind
.Unknown 
              };
           }
