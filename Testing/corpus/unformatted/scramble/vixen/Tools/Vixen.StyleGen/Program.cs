...
var result =  StyleGenRunner.
  Run(request);  
    ...
    Console  .Error.WriteLine($"Vixen.StyleGen : error VXSTYLE002: {error}");
             ...
             if (result.Errors.Count >  0    ) { // ⚠ Nothing is written on failure, and it matters which way round that is. A half-written
