...
/// <summary>One node of a talent tree.</summary>
 ...
 set;   } =  string .Empty;
  ...
  ;   }
    ...
    public List
       <ModifierDefinition    > Modifiers { get   ; set; } =    [    ];
...
public List<TalentNodeDefinition> Nodes {   
    ...
    public override void CollectTags(ICollection<string> tags) {
      ...
      foreach  (var node in Nodes  ) { foreach  (var   tag in   node.GrantsTags    )  {
                tags .Add(tag    );    }
			}
         }
}
...
public   sealed record    SpecialisationDefinition
            : Definition {
    /// <summary>What it is called in the UI.</summary>
  public    string DisplayName {    get; set
    ; } =   string.Empty    ;
    ...
    /// <summary>What choosing it does to the character's stats.</summary>
         public List<   ModifierDefinition>
         Modifiers { get; set; } =  [];
    /// <inheritdoc />
          public    override    void CollectTags   (ICollection   <string > tags) {
        ArgumentNullException .ThrowIfNull   ( tags)  ;
       ...
       foreach   (   var requirement  in Requirements) {
              if (  requirement
             .    Kind !=  RequirementKind.Value && requirement.Subject   .  Length >    0)    {
...
{    get;   set; }   = string.  Empty ;
 ...
 ; set; } = string.Empty;
    ...
    /// <summary>Its tiers, in ascending order of skill.</summary>
			public List<ProfessionTierDefinition> Tiers {  get
; set    ; } =  [  ]  ;
    /// <inheritdoc />
   public
           override   void CollectTags(ICollection<  string>  tags) {
  ...
  .Length   > 0 )   {
            tags.Add(Tag
             ...
             if ( tier.Tag.Length > 0) { tags  .  Add(tier   .Tag)  ;
      }
        }   }
        }
/// <summary>One standing with a faction.</summary>
   ...
   public
     sealed class    ReputationRankDefinition   {
    /// <summary>What it is called in the UI — <c>Honoured</c>.</summary>
        public  string
            DisplayName { get;   set ; } = string.Empty;  
