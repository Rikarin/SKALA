...
[DataContract("VendorStock")]
    ...
    /// <summary>How long until one sold comes back, in seconds. Zero for never.</summary>
      public float    RestockSeconds { get;
       set; }
               
    /// <summary>What has to be true to buy it.</summary>
     public List
    <RequirementDefinition> Requires {   get; set   ; }  = []    ;
...
public sealed record    VendorDefinition
 ...
 public    string DisplayName  { get  ; set  ;    }    =   string.Empty ;
    ...
    public List
       <VendorStockDefinition> Stock   {    get; set; } = [  ]   ;
    /// <summary>How many things it will hold for somebody who sold them by accident.</summary>
    public   int  BuybackSlots { get; set; }  =  12;

    /// <inheritdoc />
    public override  void CollectTags(    ICollection<string  > tags)    { ArgumentNullException  . ThrowIfNull(   tags)  ;
  ...
  tags.Add(Tag);
     ...
     foreach (var
             row  in Stock) {
          foreach (var requirement in row
           .Requires) {
if   (requirement .Kind != RequirementKind.Value    && requirement.Subject.  Length    > 0  ) { tags.Add  ( requirement .Subject    );
    }
            }
             } }
      }
...
public sealed class   VendorStock {
        internal VendorStock(VendorStockDefinition definition, int  index, RequirementSet   requirements ) { Definition = definition;  
        Index = index;
	Requirements =    requirements;
             Item 
               = DefId.From    (definition.    Item   );
  ...
  public long    Price
    ...
    /// <summary>Whether it never runs out.</summary>
     public bool IsUnlimited => Quantity ==
0;
			
    /// <summary>How long until one sold comes back.</summary>
               public float RestockSeconds => MathF.
    Max (0f,  Definition.RestockSeconds);
...
/// <summary>A vendor, compiled.</summary>
             public sealed class Vendor {  readonly    VendorStock[ ]   stock ;
          internal Vendor(VendorDefinition
                definition, GameplayTag    tag, VendorStock[] stock) {
        Definition =    definition;
          Tag = tag; this.stock = stock;   
                }
    /// <summary>What it was compiled from.</summary>
...
/// <param name="Paid">How much they were paid, which is what buying it back costs.</param>
             public    readonly record struct  BuybackEntry(  DefId Item
, int    Count, DefId Currency, long Paid)  ;
...
/// </remarks>
		public sealed class VendorState   {
           readonly int[] remaining;   readonly float  [] restockAt;
                readonly
Dictionary   <PlayerId , List  <   BuybackEntry  >> buyback = []   ;
    /// <summary>Makes a fresh one with everything in stock.</summary>
    /// <param name="vendor">Which vendor.</param>
              public VendorState    (   Vendor vendor) {    ArgumentNullException.ThrowIfNull(  vendor);
        Vendor =
     vendor;
              remaining = new
 int[   vendor  .Stock.  Length]  ;
   restockAt
              =   new float[   vendor.    Stock.    Length   ];   

      for (var row = 0; row < remaining.    Length;  row
 ++) {
            remaining [row] = vendor.Stock[
row   ]  .Quantity;   
       }
   ...
   public int Remaining(    int row) =>  (uint) row >= (    uint)remaining.   Length ? 0 : Vendor.Stock[row].IsUnlimited ? -   1 : remaining   [row    ];
    ...
    public IReadOnlyList<    BuybackEntry
           >  BuybackFor(PlayerId player) =>
              buyback   .
	TryGetValue ( player, out var entries) ? entries  :   [];
...
;
    ...
    /// <returns>The refusal, or <see cref="VendorRefusal.None" />.</returns>
            public VendorRefusal CanBuy(    int row, int count,  IRequirementContext   
        ? context ) {
if ((uint  )row >= (uint
     )   remaining.    Length) {
...
(count <= 0  ) {
   ...
   .BadCount;
       ...
       Vendor.Stock[ row ]    ;
        ...
        if (   !entry   .IsUnlimited && remaining [row] <    count)    {    return  VendorRefusal.OutOfStock;
 }
  
  if    (  context is not    null && !entry.Requirements .IsMetBy (context))    { return VendorRefusal   .Requirements  ;  
       }
...
var entry =   Vendor.Stock  [row];  
  var
      vendor =    EconomyAccount.   Of(   EconomyAccount.Vendor);
    ...
    =   ledger  .Post(
            new (
             ...
             ,
             [
              new(account,   entry.    Currency    , -cost) , new(vendor, entry.Currency, cost    )    ,
        new(    vendor    ,
  entry   .Item, -   count   ),
...
,
   ...
   if ( !result.  Ok  ) {   return result.   Verdict == EconomyVerdict .Insufficient ?    VendorRefusal .Insufficient :  VendorRefusal.Refused   ;
             }
      if (result   .Verdict == EconomyVerdict .   Applied && !entry   .IsUnlimited ) {
            remaining  [row] -=   count   ;
      
              if   (remaining[row] < entry  .    Quantity && entry.RestockSeconds    >    0f)    { restockAt [row] = now + entry  .RestockSeconds;
}
 }
    ...
    /// <returns>The refusal, or <see cref="VendorRefusal.None" />.</returns>
         public    VendorRefusal Sell( PlayerId   seller ,
       DefId  item, int    count,
     DefId currency ,
        long   price,
               IEconomyLedger ledger
,
...
vendor, currency,    -    price ),
        ...
        if    (result.Verdict != EconomyVerdict.
 Applied   || Vendor.BuybackSlots == 0)   {
return   VendorRefusal.None; }
        if (
      ! buyback.  TryGetValue(seller, out var entries  ))    {
        entries =    [   ] ;
            buyback    .  Add(seller    , entries   );
        }
       entries
            .   Insert (  0,  new (   item,  count,  currency, price    )) ;
 
        // The oldest falls off the end, which is what makes it a window rather than a second bank.
            while (entries.Count   > Vendor.BuybackSlots   ) { entries.RemoveAt(entries.Count -   1   );
        }
            return    VendorRefusal .None;  
    }
    ...
    public VendorRefusal    BuyBack(PlayerId   player   ,    int slot, IEconomyLedger ledger, string  operation    ) {
     ...
     (ledger);
        ...
        if (  !  buyback.TryGetValue(player   , out var  entries)  ||    (
            uint)slot   >=    (uint)entries.Count   ) {
                return    VendorRefusal
              .NothingToBuyBack;  
              }
       var  entry = entries    [slot];
             var vendor =   EconomyAccount.    Of  (
              EconomyAccount.Vendor   )   ;
        ...
        result  = ledger   .Post(
         ...
         [
            new(account,
            entry. Currency, -entry.   Paid),
new  (vendor
, entry.Currency  , entry.Paid),
	new(vendor, entry.Item
       , -entry.Count)   ,
     ...
     return result    .
       Verdict == EconomyVerdict .Insufficient ? VendorRefusal .Insufficient    :   VendorRefusal.    Refused;
      }
               if    (result.Verdict == EconomyVerdict    .Applied    ) { entries.RemoveAt(slot);
       }
