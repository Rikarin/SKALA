...
var width = BinaryPrimitives.ReadInt32LittleEndian(    dib[   4..  ]); var  signedHeight    = BinaryPrimitives.ReadInt32LittleEndian(  dib[8..]);
         var bits  = BinaryPrimitives.    ReadUInt16LittleEndian(dib
 [14..]   );
  ...
  // A V4 or V5 header carries the masks in the header itself.
          redMask
           = BinaryPrimitives    .  ReadUInt32LittleEndian(dib[40..]   )  ;
                greenMask = BinaryPrimitives   .ReadUInt32LittleEndian
   (    dib[  44..])    ;
...
offset +=    paletteEntries * 4   ;
       ...
       if (offset
< 0 || stride <=   0   || (long    )  offset + (long) stride *    height > dib.   Length    ) {   
return  false
       ;
...
// interesting one — it is only ever this arithmetic.
            var source = dib  .Slice  (offset  + (topDown ? y   :  height - 1 -
   y)   * stride, stride  );
      var  destination =   
          ...
          _ => BinaryPrimitives .ReadUInt16LittleEndian(   source
       [( x *  2)..])
      };
		var alpha   =    alphaMask ==
0  ? (byte)255 : Extract   (value  ,  alphaMask) ;
		alphaSeen |= alpha   != 0;
    destination   [x
...
destination [  x *    4 +  2] = Extract    (   value,  blueMask   ); destination[x * 4    +    3] = alpha;
   ...
   for (
           var index = 3; index <   pixels.  Length;   index += 4) {
             pixels[    index
] =    255;
         } }
...
BinaryPrimitives.
WriteInt32LittleEndian(header[20..], stride  * height);  
