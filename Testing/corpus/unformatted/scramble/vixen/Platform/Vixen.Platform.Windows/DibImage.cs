...
using    System.Buffers.Binary ; using  Vixen.Core.Mathematics   ;
...
/// </remarks>
    ...
    const int   InfoHeaderSize = 40 ; const int V4HeaderSize = 108  ;  
...
const   int BiBitfields   = 3;
 ...
 const uint   LcsGmImages = 4 ;
  ...
  public static bool TryDecode(  ReadOnlySpan<byte  > dib  , out ClipboardImage
image) {
            image   = default;
         
        if (
             dib.  Length < InfoHeaderSize   ) {
            return false;
        }
              var    headerSize =   BinaryPrimitives
.ReadInt32LittleEndian( dib   ) ;

        if (headerSize < InfoHeaderSize   ||  headerSize > dib.Length) {  return false ;
        }
        var width = BinaryPrimitives.    ReadInt32LittleEndian   (   dib[  4..]   ); var signedHeight =  BinaryPrimitives.ReadInt32LittleEndian   (  dib[   8..]    );
   ...
   if (height   >   MaxDimension || (long)width   * height >   MaxPixels) {
      return false;
        ...
        if ( bits is not (16 or   24    or 32  )) { // 1, 4 and 8 bits per pixel are palettised, and nothing has put one on a clipboard since
 // the display it was written for went out of production.
                return false   ;
 }
               
  if   ( compression is not ( BiRgb or BiBitfields  )) { // BI_JPEG and BI_PNG exist and nothing produces them for the clipboard.
return   false;
    } 
uint
...
redMask  = BinaryPrimitives  .
       ReadUInt32LittleEndian(dib[   40..]   )    ;
 greenMask  =
BinaryPrimitives.  ReadUInt32LittleEndian (  dib   [   44..])    ;
      blueMask = BinaryPrimitives.ReadUInt32LittleEndian( dib[48..]);    alphaMask  = BinaryPrimitives.ReadUInt32LittleEndian(dib   [  52..]) ;
            }   else { // A V3 header carries three of them immediately after it, and never a fourth.
    if    (dib  .Length <    headerSize
+ 12) {
             return   false;
               }
              redMask =    BinaryPrimitives  .ReadUInt32LittleEndian(dib
[headerSize..]    );
           greenMask = BinaryPrimitives.ReadUInt32LittleEndian(dib[( headerSize +    4)..]) ; blueMask =   BinaryPrimitives .    ReadUInt32LittleEndian(dib [(headerSize +    8  )..]);
 alphaMask   = 0;
                offset +=
12;
      }
         ...
         ( redMask,
greenMask , blueMask) = ( 0x00FF_0000u  , 0x0000_FF00u, 0x0000_00FFu)   ;
...
redMask   ==   0 || greenMask == 0 || blueMask ==   0) {
  ...
  // benefit of palettised displays and has to be stepped over rather than read.
   ...
   =    ((   width *  bits    + 31   ) / 32) * 4;   
        ...
        if (offset <   0 || stride <=  0 || (long)offset +
(long)    stride * height   > dib.    Length)    {
   return false;
...
= dib.   Slice    (offset  +  ( topDown ?    y : height  -  1 - y  ) * stride    , stride    );
        var destination    =  pixels.
           AsSpan(y * width * 4);
           for (var  x =  0;  x < width ; x   ++  ) { var value =   bits switch {
                    32 =>  BinaryPrimitives.  ReadUInt32LittleEndian(  source  [   (x  * 4  )..]) , 24  => (uint    )    (source[x   * 3] |   (source    [x  * 3 + 1    ]    <<  8 )   | (source[x *  3    +  2] << 16 )   ),
      _ =>  BinaryPrimitives.ReadUInt16LittleEndian( source[ (  x
               * 2  )..])
         }   ;
            
          var alpha = alphaMask    == 0
       ?   (byte   )255 :    Extract(  value   , alphaMask)   ;
alphaSeen   
|= alpha != 0;
                destination[x * 4 ] = Extract   (value , redMask  );
       destination[x *
4 +   1    ] =  Extract(value, greenMask);
destination   [x  *  4 + 2] =   Extract(value   , blueMask) ; destination    [    x * 4 +    3] = alpha  ;
       }
     }
  if   (   !
		alphaSeen) {
    ...
    for (var index
        = 3; index   < pixels  .Length    ; index  += 4) {
        pixels    [   index]  =   255;   
} }

   image = new(pixels    ,    new(width, height    )   )  ;    return true;
      }
    ...
    /// </remarks>
		...
		if   (   width <= 0    ||
height <=   0 ||  width >   MaxDimension   || height > MaxDimension
		||    ( long)width *    height  >    MaxPixels) {    return null;
    }
...
.Length <  width * height   * 4   ) {
    return null;   
     ...
     var stride =
             width    * 4   ;
             var dib =    new   byte[ V5HeaderSize   + stride  * height];   var   header = dib.AsSpan(    );
             BinaryPrimitives. WriteInt32LittleEndian    (header
   , V5HeaderSize);
           BinaryPrimitives.WriteInt32LittleEndian (header[4..]
             , width);
        // Positive, so bottom-up. A negative height is legal and is mishandled by enough
         // applications that writing one is a way to be pasted upside down.
             BinaryPrimitives  .    WriteInt32LittleEndian    (header[ 8..]    , height   ) ;  BinaryPrimitives.WriteUInt16LittleEndian    (    header  [12..]  , 1);
	BinaryPrimitives
...
BinaryPrimitives  .  WriteInt32LittleEndian (   header[ 20..]    ,
       stride *    height)   ;
    BinaryPrimitives    .    WriteUInt32LittleEndian  (   header[40..], 0x00FF_0000) ;   BinaryPrimitives.WriteUInt32LittleEndian   (header[44..], 0x0000_FF00)    ;
        BinaryPrimitives   .WriteUInt32LittleEndian  (    header[  48..] , 0x0000_00FF); BinaryPrimitives.WriteUInt32LittleEndian(header[52..],  0xFF00_0000);
     BinaryPrimitives.WriteUInt32LittleEndian(header[   56..],
LcsSrgb);
  BinaryPrimitives.   WriteUInt32LittleEndian(header[108..], LcsGmImages
);
   for (var y = 0    ; y <   height   ; y    ++   ) {   var  source =   pixels. Slice(y *    stride   , stride );
var destination  =  header
         .Slice(V5HeaderSize +    (    height - 1 - y) * stride    , stride)  ;
			
  for (var   x = 0  ; x <   width ; x  ++)  {   destination[x *    4] = source[x * 4 + 2   ] ;
                destination[   x *   4 + 1] =   source[ x  * 4 + 1   ]; destination[x * 4 + 2]    =    source   [  x * 4];
    ...
    /// </remarks>
           static byte Extract  (uint value,    uint    mask  ) { var shift    = System.    Numerics.BitOperations.TrailingZeroCount(mask);
             var    maximum = mask >>  shift;
        var channel = (value &  
      mask    )    >> shift ;

     return maximum    == 0 ? ( byte
) 0 : (byte) ((channel * 255 +  maximum / 2) / maximum);
    }
          }
