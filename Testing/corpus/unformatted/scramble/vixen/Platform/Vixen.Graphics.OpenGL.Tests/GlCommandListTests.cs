...
/// </remarks>
  public   sealed class GlCommandListTests {
    /// <summary>Recording a whole frame's worth of work makes no GL call at all.</summary>
             [Fact    ]
        public    void RecordsWithoutTouchingGl() { var gl =    new RecordingGlApi   ( );
      using
              var device =   new GlDevice(new(  gl  ))   ; 
  var   pipeline = Pipelines    .Handle(device    , BlendState.Opaque, DepthStencilState.    Disabled); var target    =  Target(device, out var    view);
        gl.Clear  ();
           
        using var commands   =  device    .  BeginCommandList(QueueKind.Graphics, "frame"    ); commands.   PushDebugGroup("opaque");
      commands.   BeginRenderPass ( new  ([  new (
view, LoadAction.    Clear   ,    StoreAction   .Store, new   ( 1f, 0f    , 0f  , 1f  )   )    ] )   );
    commands.SetViewport    (new(0,  0  ,    64, 64)  )    ; commands.BindPipeline(pipeline);   
        commands.   Draw    (
       3) ;
...
var gl = new RecordingGlApi(  );
	using var device = new GlDevice(new(gl))   ; Target(device    , out var view );
   ...
   (    new(   [new  (view  )    ])   );
              ...
              Assert .Throws<  InvalidOperationException  >(   (    )    =>  commands.  BeginRenderPass
      (    new(    [new(view    )]   )));
         }
    ...
    /// </remarks>
    [Fact   ] public void RefusesADrawOutsideAPass() {
               var gl = new RecordingGlApi();
    ...
    /// <summary>Finishing inside a pass is refused.</summary>
          [Fact] public    void  RefusesToFinishInsideAPass    (  ) {
         var gl = new RecordingGlApi  ()    ;  using var    device = new GlDevice(new   (gl ))   ;
      Target   (device, out   var view  ) ;
             using   var commands    = device.   BeginCommandList  (    ); commands    .    BeginRenderPass   (   new(   [new    (view  ) ]));
...
Assert.Throws<InvalidOperationException>(    () => commands.SetStencilReference(1));  }
    /// <summary>Submitting an unfinished list is refused.</summary>
      [    Fact  ] public    void  RefusesAnUnfinishedSubmission() {
var gl = new RecordingGlApi(); using  var device = new GlDevice (new   (  gl))   ;
      using var commands =    device.BeginCommandList();
  ...
  using var commands =  device.
              BeginCommandList( );
     commands.Finish(   )    ; device.    GraphicsQueue.Submit ( [ commands]  );
     Assert.Throws<InvalidOperationException>    (()    => device.GraphicsQueue.Submit(    [commands   ])  )  ; }
    ...
    /// </remarks>
[  Fact] public void RearmsAPooledList () {
 var gl    = new RecordingGlApi (    ); using var device = new GlDevice(    new(gl) );
...
device.BeginCommandList    ();
     ...
     Dispose   ();
      ...
      Assert.Same(first  , second); Assert .   False(second.IsRecorded);
             second   .SetStencilReference    ( 1); second.Finish() ;
             }
   ...
   public void PacksAndUnpacksAnOrigin(int x, int   y ,
          int    z ) {
                var origin    = new Int3(x  , y,   z)  ;
		Assert. Equal(origin    , GlCommandList. Unpack(GlCommandList.Pack    (  origin)   )); }   
    static  TextureHandle Target(GlDevice device  , out TextureViewHandle view ) {   var texture = device.   CreateTexture   (new(
         PixelFormat. Rgba8UNorm, 64,
64,   TextureUsage .  ColourTarget | TextureUsage.CopySource ,
            Name: "target"
    ));
        view  = device    .CreateTextureView(texture)  ; return texture;
       }
                }
