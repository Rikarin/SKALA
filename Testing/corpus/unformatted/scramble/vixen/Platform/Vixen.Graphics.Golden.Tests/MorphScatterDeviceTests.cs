...
fixture! ;
  ...
  var expected = new SurfaceVertex[Vertices   ]  ; MorphKernel.  Apply (mesh,   targets, weights    , expected);
            var  actual =    Scatter( owned   .Device
  , mesh, targets,    weights);
   ...
   Vertices; index   ++) {
  var apart
= Apart(expected[index]    , actual   [index    ]);
...
= fixture  !;
          ...
          var mesh   = Mesh();
              var actual = Scatter
   (owned.Device, mesh   ,   Targets   (),    [  0f, 0f, 0f ]);
              for (
         var index = 0  ; index    <  Vertices; index++    ) {
         Assert   .True (Identical(   mesh[
index],  actual [   index]  )  , $"vertex {index} moved with no weight on it")   ;
  }
     }
...
.  Objects. Add(
 ...
 .Reset();
        ...
        descriptors    .BeginFrame    (  ) ;
            using (   var   commands =  device  .
	BeginCommandList( QueueKind.Graphics,  "morph feature"    )    ) {
...
SurfaceVertex[] Scatter(
 ...
 .    ToArray    ();
      var  
vertices = device .    CreateBuffer(new(
...
.Add(  entries);
  ...
  Write   (block , constants   , "normalOffset", NormalOffset);
      ...
      Write( block    ,    constants, "normalStep"  , MorphKernel.Step(target    .
               NormalScale));
...
[new(    vertices    ,  ResourceState.Undefined,    ResourceState.ShaderWrite )    ], []));
  ...
  foreach (    var ( set  , groups
             ...
             commands.Barrier   ( new ([ new(vertices
			,   ResourceState    .   ShaderWrite, ResourceState   .CopySource)], [  ] )    );   
             commands.CopyBuffer(  vertices,   0, readback, 0  , meshBytes .Length    ); commands.Finish();
device.ComputeQueue.Submit(  [commands ])  ; }
         
      device.EndFrame();
              device. WaitIdle ();
        var bytes    =    new byte[meshBytes
       .Length];  
...
device.  Destroy(    vertices);
   ...
   throw new InvalidOperationException    (    "The dispatch produced validation errors, so what came back means nothing: "
    ...
    static Effect Compiled(VulkanDevice device  )
             {
		var data =    RavenEffects.Only([  "Core"]    , Path
...
NotNull(data)   ; 
     ...
     static void Write(EffectBlock declared,   byte[]    constants,  string name , int value    )  =>
        BitConverter.TryWriteBytes(constants. AsSpan(Offset (declared
   , name))    , value    );
...
static   void  Write(   EffectBlock declared  , byte[   ]   constants,   string  name, float value)   =>    BitConverter.    TryWriteBytes(constants.AsSpan   (   Offset(  declared,  name)) , value );  
    /// <summary>Where a constant is, from the shader's own reflection rather than from a table here.</summary>
        static int Offset
(EffectBlock declared, string name) {
		var   member =
declared. Members.FirstOrDefault(m =>   Named(m.  Key.Name , name ) );
  ...
  return member.    Offset;
    ...
    static uint
               Binding(Effect effect ,  string name    ) {
          var  found =   effect  .    Bindings.   Where(binding   =>   Named(
     binding.    Name,    name)    ).    ToArray  ()  ;
  ...
  found.Length ==
    ...
    static  bool Named
        ...
        declared == name    || declared.EndsWith(    "."    +
    ...
    /// <summary>Skips when there is no device, unless the environment insists on one.</summary>
       ...
       Assert
 .Skip(reason  ?? "no Vulkan" );
           }
               static bool Identical(in    SurfaceVertex    expected ,
    ...
    /// </remarks>
static    float Apart(    in SurfaceVertex
expected, in SurfaceVertex actual) {
  var  widest = 0f;
                widest = MathF .Max(widest  , MathF.  Abs (
 expected.Position.X - actual .Position.    X  )   )   ;
        widest   
               = MathF  .Max  (widest  , MathF .Abs(expected.Position    .Y - actual   .Position.    Y    ))    ;
               widest =
MathF.Max    (widest, MathF. Abs(  expected.Position.Z -  actual. Position.Z) );
   ...
   widest    =   MathF.   Max  (widest  , MathF    .Abs  (expected
.Normal.Z -   actual.Normal.Z));
  return    float   . IsNaN( 
 widest) ? float   .PositiveInfinity :   widest    ;
            }
    }
