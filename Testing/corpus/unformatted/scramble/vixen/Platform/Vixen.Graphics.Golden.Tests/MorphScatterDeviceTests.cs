   ...
   using Vixen  .  Rendering.Ecs   ;
    using Vixen.Rendering.  Features ; using Vixen.Shaders;
      using Xunit;
...
1f / 32768f ;
         ...
         static Vector3 Exact(int
          x, int y,  int z) => new(x   *   Unit, y * Unit,    z * Unit);
    ...
    /// </remarks>
   static SurfaceVertex    [] Mesh() { var mesh = new SurfaceVertex[   Vertices]    ;
               
        for ( var index = 0   ; index
< Vertices  ; index ++) {
mesh[index
           ] = new   () {
          Position =  Exact(index * 71,   index * -37, index *  13)   , Normal = Exact   (   index * 5  , 32768    , index * -3),
                Tangent =    new  ( 1f, 0f,    0f,    1f), TexCoord =    new( index * 0.01f , index    * 0.02f)
 };
        }
...
static MorphTargetData[   ] Targets(    ) { int[    ] jaw = [3  ,  11 , 12, 40    , 41  , 42, 90];
    int[
     ] brow =   [11   , 12    , 13, 60, 91    ]   ;
           int[    ] crease
=   [   12    , 42  , 95];
   return [ MorphTargetData.Encode(
        "jawOpen",
      jaw   ,
               [.. jaw
         .Select(v => Exact(32767, -16384,  8192 + (   v * 3)))   ],   
       [.. jaw.Select(v => Exact(v   * 7
        , -4096,    16384))]
         )  , MorphTargetData   .   Encode(
           "browRaise",
                brow   ,
      [.. brow. Select(v
               =>   Exact(-8192,  32767, v *    -5    ))   ],
                []
          )   ,
            MorphTargetData.
              Encode (
...
11, 32767   , -1024))    ]
 ...
 [ Fact]
	...
	, out var reason   )) {
        ...
        var mesh    = Mesh(   )   ;  var targets = Targets(   ) ;
           float[
   ] weights    = [1f , 1f , 1f];
        var expected    = new SurfaceVertex   [ Vertices];
	MorphKernel.Apply(mesh, targets,
       weights, expected   )   ;
   var actual  = Scatter (owned.Device    ,   mesh    ,  targets    , weights);
      for (var index  = 0; index < Vertices; index++  )    {   Assert .True(
           Identical(expected[index ]   , actual[
            index   ]),
    ...
    [
         ...
         if  (!   Fixture.TryOpen    (out var fixture, out var   reason)) { Skip (   reason );   
            return;
        }
  ...
  var    mesh = Mesh();
            var targets = Targets  (); float[] weights = [0.37f, -0.25f   ,  0.9137f];
  var  expected = new SurfaceVertex   [Vertices]    ; MorphKernel.Apply  ( mesh    , targets ,   weights, expected);  
       
        var actual    =
         Scatter(owned.Device, mesh, targets  ,    weights  );
    var worst
= 0f  ;
         var worstAt    = 0; 
   for   (var index
  = 0; index < Vertices; index    ++ )   {
            var
   apart =    Apart(expected    [index]    , actual[index] );
    ...
    ///     than by accident.
    /// </remarks>
    [Fact   ] public void    No_active_shape_leaves_the_mesh_exactly_as_it_was   () {
  if (    !Fixture.
TryOpen  (out var fixture   , out var reason  )) {
            Skip   (reason 
 );  
              return;
}
...
var owned = fixture!;
     var mesh = Mesh(
              ...
              var   actual   = Scatter(    owned  .Device, mesh, Targets(), [0f
   , 0f  , 0f    ]  );
              
         for (var    index =   0;
         index < Vertices;   index++) {
  Assert  . True
(   Identical(  mesh  [index]   , actual    [index] )   , $"vertex {index} moved with no weight on it");
     }
    }
    // --- The render feature -------------------------------------------------
    /// <summary>
  ...
  fixture!;
        ...
        var    rest  = Mesh()   ;
     var    targets   =  Targets();
float   []   weights = [0.37f, -   0.25f ,
      0.9137f] ;

               var mesh
    = Described   (  rest, targets);
...
// looks exactly like a kernel that is doing slightly different arithmetic and is not.
  ...
  SurfaceVertex[    Vertices]    ;
              ...
              MorphKernel
      .   Apply (packed, targets, weights, expected);
    ...
    Assert.True
    (scene.  TryAllocate(9 , 0,    out _   ));
        var residency = new GeometryResidency(scene);
  
 Assert.   True(   residency.Acquire(GeometryKey .Of   (PrimitiveKind.    Cube) ,    ()  => mesh,    out var    slice, out _))  ; Assert    .NotEqual(0, slice    .    BaseVertex    )   ;
  var    effects    = new EffectSystem() ;
    
effects.AddProvider  (    new Compiling(    new EffectLoader(device) , _ => RavenEffects.   Everything()    )    ) ;

 using var  descriptors    =  new DescriptorAllocator(device, "Morph"); using var morph = new MorphRenderFeature(device    , vertexCapacity:    512, entryCapacity   :  512)    {
              Effects
       =  effects, 
          Pipelines =   new(    device ),
            Descriptors = descriptors   , Source   = scene   
      };

 using
 var system   = new RenderSystem();
        var
stage = system.AddStage(    new("Opaque"));
        var meshes = new
MeshRenderFeature();
              ...
              var id = system .   Objects   .
Add(
           new
   (   ) { Bounds   = new(Vector3    .Zero,   1f   ) , Stages   = stage  .   Mask, FeatureIndex = meshes.Index   }
            
          );
   var draw = new MeshDraw {  InstanceCount =   
 1 };
     scene .Apply(ref  draw, slice);
           Assert    .True    (    morph.Attach(system   , id, GeometryKey.Of(    PrimitiveKind.Cube), mesh   , slice, ref    draw    ))    ; Assert.   True(morph   .  SetWeights (  id, weights ));
         
        // The draw the mesh feature would record now reads the morph buffer, which is the seam the
			// whole feature exists to move.
                Assert.Equal(morph   .  Buffer, draw. VertexBuffer) ; Assert.  NotEqual(scene.Vertices,  draw.VertexBuffer);
 ...
 VulkanDiagnostics. Reset(    );   
     ...
     using (
         var commands = device  .    BeginCommandList    ( QueueKind. Graphics    ,  "morph feature")    ) {
...
True(   morph    .   Record (commands));
 ...
 // declared any other state here would be saying the pass had left it somewhere else.
     ...
     commands.CopyBuffer(
            morph.Buffer
  ,
       (long)draw.VertexOffset * SurfaceVertex.SizeInBytes, readback  ,
                0 ,
              bytes  );
     commands.
      Finish ();
...
Assert.Equal(3, morph.Dispatches);
         Assert    .Null(morph   .
               Degraded    );   
      var    raw =    new  byte[bytes]; device.  Read(readback, 0, raw);
device
     .Destroy  ( readback);
      if ( VulkanDiagnostics.    ErrorCount   > 0) {    throw    new InvalidOperationException  ( 
      "The frame produced validation errors, so what came back means nothing: " +  string.Join(Environment.NewLine, VulkanDiagnostics.Messages    )
            ); }
  ...
  for (var    index = 0; index < Vertices  
;   index  ++) {
var
 apart =   Apart (expected[index]  , actual[index])   ;
  ...
  Tolerance,
    ...
    /// </remarks>
     static MeshData
       Described(SurfaceVertex [] mesh ,   MorphTargetData[    ] targets ) {
  var  positions  = new Vector3[Vertices]; var normals = new Vector3[Vertices];
			var
              texCoords    =  new Vector2[    Vertices  ]  ;
    var indices = new    int[Vertices]    ;
        for (    var index = 0  ; index < Vertices; index   ++) { positions[   index   ] = mesh[index].  Position;
            normals  [  index   ]    = mesh[index  ]
     .  Normal;
   texCoords [    index
     ] =    mesh[index]    .TexCoord;
            indices  [    index   ]    = index   ; }
        return new()  {  
Name = "head"    , Positions = positions  ,
			Normals    = normals,
TexCoords = texCoords,
            Indices =
indices,
   ...
   VulkanDevice device  ,
     SurfaceVertex[] mesh    ,
        ...
        var   effect =   Compiled  (device)   ;  var block = effect.BlockOf   (DescriptorSetSlot.   PerDraw);
               var   layout = effect.SetLayouts[( int) DescriptorSetSlot  .  PerDraw];
        var meshBytes = MemoryMarshal.AsBytes (    mesh
          .    AsSpan()). ToArray    ();
	var vertices = device .CreateBuffer ( new(
   meshBytes.Length, BufferUsage. Storage   | BufferUsage.    CopySource,
            MemoryAccess.  HostUpload
,
 "morph vertices" 
      )    );
        var    readback  = device.CreateBuffer    (    new    (    meshBytes  .Length  ,
        BufferUsage   .
CopyDestination,
     MemoryAccess
           .    HostReadback,
    ...
    device    .    Write(
 vertices   ,   0 , meshBytes)  ;  
 var shader = device.CreateShader( ShaderStage.    Compute,
               effect.   Stages .Single (stage => stage.Stage ==    ShaderStage .Compute  )  .Bytecode.AsSpan(  ), "MorphScatter"   
    );
                
        var    pipeline = device.CreateComputePipeline  
(new(shader    ,  effect.Layout    , "MorphScatter" ));

             List<    BufferHandle    >    owned = [];
           List    <DescriptorSetHandle> sets  = []  ;
             List<(DescriptorSetHandle Set,  int Groups)
        >   passes =    [];   
		for  (var    index = 0   ; index
 ...
 weights [  index] ==    0f    )    {
     continue; }   
        var target = targets [index ];
            var words = MorphKernel.  Pack(target   ); var entryBytes =    MemoryMarshal.AsBytes(words.    AsSpan()   ).ToArray   () ;
            
   var
  entries = device.    CreateBuffer(  new(
    entryBytes.  Length   
   ,
              BufferUsage .  Storage   ,
		MemoryAccess.HostUpload,
              ...
              device   .Write(entries,
             0  , entryBytes);
     owned.Add
                (entries);
            var constants =
   new   byte[Math.Max (16   ,   block   .Size)   ];   
       Write
 (block,   constants  , "entryCount", target.   Count);
            Write (  block, constants,    "baseVertex", 0)    ;
             Write  (   block, constants,    "stride" , Stride)   ;
  Write(block   , constants
        , "positionOffset",   PositionOffset);
            Write(block, constants,
	"normalOffset",    NormalOffset);  
            Write   ( block, constants, "weight"    ,   weights[index]) ; Write(block,   constants, "positionStep", MorphKernel.Step (target .    PositionScale    ));
         Write  (block, constants, "normalStep"    , MorphKernel
.Step(target .NormalScale)   )  ;
             var uniforms = device.  CreateBuffer (new(   
           constants.   Length
  ,   
          BufferUsage.Uniform,  MemoryAccess    .HostUpload,
                $"morph constants {target.Name}"
      ));
               device.  Write    (uniforms, 0, constants); owned.Add(   uniforms);
            var set = device  .CreateDescriptorSet(layout, $"morph {target.Name}")    ;
                
device.UpdateDescriptorSet(
set,
	[
            DescriptorWrite.  Uniform   (   block.Binding, uniforms,   0  , constants.Length), DescriptorWrite . Storage    (Binding   (    effect, "vertices") , vertices)    ,
            DescriptorWrite.Storage(Binding(  effect,    "entries"), entries  ) ]
);
      sets  .Add(   set
       );
passes   .Add( (set, (target   .    Count + 63    ) / 64)  ); }
 VulkanDiagnostics.    Reset(); device.    BeginFrame()    ;

            using (   var commands =   device. BeginCommandList(QueueKind.Compute   , "morph scatter")) {
               commands
.Barrier    (    new  ([new(    vertices, ResourceState .    Undefined, ResourceState.ShaderWrite)], [    ])   );
     commands.    BindPipeline( pipeline  );
         
            foreach (
var (set, groups) in passes) {
                commands.BindDescriptorSet(   DescriptorSetSlot .PerDraw, set    )  ;
             commands.Dispatch(  groups);  commands    . Barrier  (new([new  (   vertices, ResourceState.   ShaderWrite  ,    ResourceState.ShaderWrite)], [   ] ) );
            }
		commands .Barrier(   new
  (    [   new(vertices   , ResourceState.ShaderWrite, ResourceState  . CopySource    )], []  ))   ;
commands.CopyBuffer(vertices  , 0 , readback    , 0, meshBytes.Length); commands.Finish()  ;
...
throw   new   InvalidOperationException(   "The dispatch produced validation errors, so what came back means nothing: "
 +    string.Join(Environment    .  NewLine, VulkanDiagnostics.  Messages    ) );
    ...
    /// </remarks>
    static Effect
  Compiled(    VulkanDevice device) {
  var data  = RavenEffects    .Only(
    ["Core"], Path.Combine ("Pipeline", "MorphScatter.rvn"))
              .TryGet    (EffectKey    .
Of("MorphScatter")   );
             Assert.NotNull    (data );
...
static void Write    (EffectBlock  declared, byte[]    constants  , string name, int value ) =>
    BitConverter. TryWriteBytes( constants. 
AsSpan(    Offset(    declared,   name  ))   , value  );
           static void   Write   (EffectBlock   declared  , byte[] constants   ,    string name    , float value) =>   BitConverter   .TryWriteBytes  (constants  .AsSpan( Offset( declared, name)) , value);
    /// <summary>Where a constant is, from the shader's own reflection rather than from a table here.</summary>
             static int Offset   (EffectBlock declared , string
name) {
        var member =   declared
     .Members.FirstOrDefault  (m => Named( m .Key.Name, name)    )    ;
        Assert.True (
...
member .  Offset;
    ...
    /// <summary>Which binding the shader gave a name, rather than a number written down here.</summary>
        ...
        var found   =   effect.Bindings    .Where
               (    binding =>    Named(    binding  .Name  , name)).ToArray()  ;
  ...
  True(
    found.Length == 1,
     ...
     .Binding;
           ...
           static
  bool    Named(string declared    , string name) =>
      declared    ==    name    || declared
	.EndsWith(  "."    + name, StringComparison.Ordinal)    ;
    /// <summary>Skips when there is no device, unless the environment insists on one.</summary>
            static void  Skip    (string? reason  ) { if (Environment.GetEnvironmentVariable(    "VIXEN_REQUIRE_VULKAN") is "1" or  "true" or   "TRUE"  ) {
      Assert.
Fail(  $"VIXEN_REQUIRE_VULKAN is set and no device could be opened: {reason}") ;
    }

        Assert.Skip(reason ??    "no Vulkan"    ) ;
    }   
      static bool   Identical
 (in SurfaceVertex expected, in   SurfaceVertex actual)    =>
         expected.Position.X == actual.Position.X && expected.Position   .Y ==  actual. Position .Y
   && expected  .Position .Z == actual.Position.
       Z 
 && expected  .Normal.X ==  actual
           . Normal   .X
               &&   expected.Normal .Y == actual    .Normal.    Y  && expected.Normal.Z    ==    actual.Normal    .Z;
    ...
    /// </remarks>
      static
       float Apart(in SurfaceVertex expected  , in SurfaceVertex actual ) {
        var widest = 0f;
       
     widest  = MathF.  Max (widest
             , MathF.Abs(expected .Position.    X  -   actual   .Position  .X  ))  ;
		widest = MathF.Max
(widest, MathF    .Abs( expected   .Position.    Y    - actual    .Position.Y    ));
        widest = MathF .Max  (widest, MathF.Abs  (expected .Position
  .Z - actual.Position.Z));
                widest    = MathF    .Max (  widest , MathF.Abs  (expected.Normal    .X - actual.   Normal.X)); widest   = MathF.Max( widest, MathF.Abs( expected . Normal.Y - actual.    Normal   .Y   )  );
 widest  = MathF    .Max  (widest,
MathF.Abs(   expected.Normal.Z - actual.    Normal   . Z));
               
               return float   .IsNaN(   widest    ) ? float   . PositiveInfinity
:    widest ;
                }
   }
