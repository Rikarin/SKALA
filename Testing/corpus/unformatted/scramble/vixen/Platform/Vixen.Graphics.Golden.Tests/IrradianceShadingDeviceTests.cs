...
;
    ...
    Assert   .True(materials.  BoundCount > 0  ,
...
// ⚠ Before the pixels. A dispatch that carried a reason is a field nothing filled, and the dark
   ...
   return picture; 
    ...
    static TextureHandle[] Fill  (SceneConstants scene, SamplerCache samplers, IGraphicsDevice device  ,   Fixture fixture)   { // No sun and no ambient intensity change. Every photon in the picture came out of the field.
        ...
        ClusterGrid  .
  Apply(  scene.    Parameters,   Camera,  "ForwardPlus");
          var flat =  Flat( 
               device  , fixture)    ;
                var cube = Cube
               (device    , fixture  );
        
    scene.  Parameters.Set(ForwardPlusKeys    .ShadowMap,
 flat.View);
...
.Graphics, "irradiance names"  );
   ...
   new
    (   )    {
      ...
      Format
=  PixelFormat.Rgba8UNorm, Usage = TextureUsage.Sampled, Name   = "unused"
            }
  )
  ;
           ...
           new  ()
{
         ...
         =   1, MipLevels   =   1, ArrayLayers =   6, SampleCount = 1    ,
...
.   Owns   ( (   ) => device.Destroy(texture))   ;
 ...
 Pixel(in Bitmap image   ,  int x    , int y   ) {
        var    offset = image .Offset(    Math.Clamp(x, 0  ,    image
  .Width -    1)   , Math .Clamp    (y, 0   ,  image.    Height - 1   ) );   
         return new(image.  Pixels[    offset] /
               255f, image  .Pixels[ offset + 1    ] /  255f, image .Pixels[    offset + 2  ]   / 255f    );
            } 
   ...
   compilation    .Failed,
    ...
    /// <summary>One radiance from every direction, and surfaces that give back nothing.</summary>
            sealed  class UniformSky    (float radiance ) : IRadianceSource { public Vector3 Sky (    Vector3   direction) => new    (  radiance);
               ...
               public Vector3  Surface(Vector3 position,    Vector3 normal,   Vector3 direction   ) => Vector3.
			Zero;
     }
           
    static void  Skip   (string  
        ?  reason) {
            if (   
