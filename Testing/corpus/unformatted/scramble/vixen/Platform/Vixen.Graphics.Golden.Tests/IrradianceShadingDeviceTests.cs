...
// SPDX-License-Identifier: Apache-2.0
 ...
 using  System    .  Runtime.InteropServices; using Vixen.Core.Imaging;
 using Vixen.
Core.    Mathematics;  
  using Vixen   .    Rendering  ;
     using    Vixen   .  Rendering   .Compositor; using Vixen    .Rendering.DistanceFields   ;
        using   Vixen.  Rendering   .Features   ; using   Vixen.Rendering.IrradianceFields    ;
            using    Vixen.Rendering. Lighting; using Vixen.Rendering.Materials;
using Vixen
.ShaderCompiler;
    using Vixen  .Shaders   ;
     using Vixen.    Shaders
...
using Xunit;
            ...
            namespace   Vixen.Graphics  .Golden   .
      Tests  ;
/// <summary>
...
/// </remarks>
  ...
  const float Radiance  =   0.5f    ;
    ...
    static  EffectKey  Key(ShaderComposition composition   ) =>
             EffectKey.   Of( "ForwardPlus",
           [ new("ForwardPlus.UseIrradianceField", "true"  ),
       new   ("ForwardPlus.UseImageBasedLighting",
     "false"    ),
	new( "ForwardPlus.UseShadows"
       , "false"  ),
         new  ("ForwardPlus.UseReflectionProbe",   
        "false"   )   ,
    new   ("ForwardPlus.UseClusteredLights"
    ...
    public void TheFieldVariantCompilesAndBindsThePoolPerFrame   ()  {  
               if   (! Fixture.  TryOpen   (out    var fixture
        ,  out var reason)  ) {
    Skip   (reason   )   ;
 return
   ;
        }
 using var owned =  fixture !    ; var material = Composed();
 ...
 "irradianceL0", "irradianceL1R",  "irradianceL1G",  "irradianceL1B" }) {
...
[InlineData(Fills
        .   Device)]
        public void ASurfaceIsLitByTheFieldItStandsIn(    Fills fills) { if    (!Fixture  .TryOpen(    out var   fixture  , out var reason)) {
              Skip(reason );
    return;
        }  
       
            using var    owned    = fixture !; var image = Render   (owned, lit   : true, fills);
...
image, 2 , 2);
             ...
             Assert   .True(
corner    .Z >  0.2f && corner. X  < 0.05f, $"the pass did not clear: {corner}"  );
      var centre = Pixel    (image, image  . Width /
        2, image.Height / 2) ;
        ...
        // C × L, per channel. Tolerant of an 8-bit target and of the filler's sixty-four rays, and
         ...
         Assert. 
               Equal (Albedo.X *  Radiance,  centre    .X    , 0.02f);
     Assert  .Equal( Albedo.    Y   * Radiance, centre.    Y, 0.02f
		);
        Assert.Equal(Albedo.Z  *
Radiance, centre.    Z  , 0.02f);
     }
    /// <summary>
    ...
    /// </remarks>
 [Fact]
             public void WithoutTheFlagTheSameFrameIsDark ( ) {    if (!Fixture.TryOpen(out var fixture, out var reason   ) ) {
             Skip  (reason);
  return   ; }
        using var   owned = fixture!;    var image = Render (    owned,    lit  : false);
                var centre
  = Pixel(image    , image.Width   /    2    , image.Height    / 2)   ;
 Assert.True(   centre.   X < 0.02f   ,
...
Fills fills = Fills.Cpu)   {
var device = fixture.  Device  ;  var   material = Composed() ;
        material
               .  Parameters .Set (ForwardPlusKeys .UseIrradianceField, lit    );
        material   . Parameters.Set(ForwardPlusKeys.UseImageBasedLighting, false); material.Parameters.Set  (   ForwardPlusKeys.UseShadows, false  );
 material    .Parameters.Set (  ForwardPlusKeys
    .   UseReflectionProbe, false)  ;
        var loader = new EffectLoader(    device   )   ; var   effects   = new EffectSystem(    ) ;
...
using var samplers = new SamplerCache    (device); using var system =   new   RenderSystem(  );
        using var view = new ViewConstants(    device)    { Descriptors = allocator    ,
    Layout =
          effect!.SetLayouts[   (int)DescriptorSetSlot.    PerView]
        };
 ...
 var    offsets    = new 
		...
		(  [
   new VertexBufferLayout(  Vertex.Stride,
        [
             .. effect  !    .   VertexInputs   .Select  ( (input,    index) =>  new   VertexElement(  (uint)    input    .  Location   , formats[index  ], offsets  [index])
                    )
    ]
      )
      ]   )    ;  
        var meshes
   = new MeshRenderFeature { Pipelines  =   new(   device),  Describer = describer  }   ;
var materials   = new MaterialRenderFeature { Effects = effects, Device  = device, Descriptors   = allocator }; // Its layout is the effect's, which is what makes the non-clustered path drawable at all —
   // see ForwardLightingRenderFeature.Layout.
var    lighting =    new   ForwardLightingRenderFeature { Device = device,
             Clustered =    Clustered
   ,
       Layout = effect.
           SetLayouts    [(int)  DescriptorSetSlot   .    PerDraw]
                } ;
		var transforms
                = new TransformRenderFeature {   Device = device, Scene = scene.Parameters    }    ;
         meshes   .Add   (transforms  ); meshes.Add(materials);
        meshes.Add   (lighting);
        system. AddFeature(
meshes) ;
               materials.PermutationKeys[
"ForwardPlus"] = ForwardPlusKeys   .    UsedPermutationKeys;
        materials
        .    PermutationSources[    ForwardPlusKeys.UseClusteredLights]    = lighting.  PermutationKeys[0    ];
   lighting    .Scene = scene .Parameters 
;
        var camera = new RenderView( "camera")
{ Camera  =   Camera    , Stages    =  opaque    .Mask } ;
        camera .Frustum = new  (   camera .ViewProjection) ;  system   .SetViews ([camera  ])   ;
        var quad = system.   Objects.Add( new() {
           Bounds = new(  new   Vector3 (0f   , 0f    ,   QuadZ  ), QuadHalf   *   1.5f)   ,  Stages = opaque. Mask,
			FeatureIndex    = meshes.    Index
             }
       )   ;
  
              system.    Objects.Data.   Data    (meshes.   Draws   )[quad    .Index] = new(   ) { VertexBuffer = fixture.Buffer    <Vertex>  (Vertex.Quad, BufferUsage.Vertex    ),
    IndexBuffer  = fixture.
            Buffer<ushort>([ 0,  1, 2    ,   2, 1,    3]  , BufferUsage.Index),
  IndexFormat = IndexFormat  .UInt16, Count = 6,
       InstanceCount = 1 };
			system
     ...
     // effect system the material came from; `RavenEffects.Everything()` is what makes that possible
         ...
         using  var dispatch    = fills is Fills  .Device
           ? new IrradianceFieldFill  (device
                ...
                Effects   = effects, Pipelines    =    new   ComputePipelineCache   (  device    )    ,
               Descriptors  = allocator,  SkyColour =    new(Radiance)
  }  :   null   ;
               using var 
probes = new  IrradianceFieldRenderer {
          Name = "IrradianceField"  , Field =    field ,
    Filler =
    fills  is Fills  .Device    ? null : new   TracedIrradianceFiller(new EmptyWorld    (), new UniformSky(  Radiance)  ),
        DeviceFiller = dispatch,
               SceneConstants =   scene, Device = device,   
     ...
     ,
      ...
      ;
        ...
        var unused = Fill(scene,
   samplers, device, fixture);
               
        var  pass =  new   RenderPassRenderer {
Name = "Forward", ClearColour =   Background    ,
         SceneConstants = scene, Children = {   new SingleStageRenderer { View = camera , Stage = opaque,    Constants = view   } }
 } ;
        pass.ColourTargets.  Add("Display" ); pass  .SceneBuffers    [    "lightBuffer"] =  "SceneLights";
        pass  .
            SceneBuffers["clusters"] = "Clusters";
         var display    = fixture.  Owned("Display"   ,
TextureUsage.ColourTarget    | TextureUsage.CopySource);
      var compositor =  new GraphicsCompositor   (   system) {    FrameSize  = new(Fixture.  Side, Fixture.   Side ) ,
            Game =    new SceneRendererSequence {   Children    = {
probes   ,    pass }    }
        };

      compositor.Imports[   "Display"]    = new  ( display.    Texture    ,
        display.View,
		display  .Description   , ResourceState.Undefined,
    ...
    // One light nothing points at, and a cluster list of zeros. Imported rather than declared,
        ...
        var lights =  fixture.Buffer<
PunctualLightData>(  [ default  ],  BufferUsage.Storage )  ;
              var  clusters    = fixture.
              Buffer<byte    >   (new byte[   ClusterGrid.  BufferSize]   , BufferUsage. Storage   );
      
        compositor.BufferImports[  "SceneLights"] = new
               (
    lights ,
			new(   Marshal  .
         SizeOf   <    PunctualLightData>    ( )   ,   BufferUsage .  Storage, MemoryAccess.HostUpload, "SceneLights" ) ,
ResourceState   .    ShaderRead, ResourceState.ShaderRead
)  ;

           compositor   .BufferImports["Clusters"  ]
            =   new (
		clusters, new(   ClusterGrid    .BufferSize  , BufferUsage.Storage    , MemoryAccess.HostUpload, "Clusters"  ),
   ResourceState. ShaderRead, ResourceState. ShaderRead
           );
...
,
                    [.. unused
          . Select   (   texture => new TextureBarrier(texture, ResourceState .Undefined   , ResourceState.ShaderRead ) )  ]
                )
       )
			);
...
// assertion that tells the three apart.
  ...
  scene.Parameters  .Set(ForwardPlusKeys.LightColor    , Vector3  .Zero)    ;
        ClusterGrid  .Apply
(  scene.    Parameters,   Camera,  "ForwardPlus");
        var flat =  Flat(  device  
, fixture)    ;
  var cube = Cube(
          device    , fixture  );
               
                scene.  Parameters.Set(ForwardPlusKeys    .ShadowMap, flat
               .View);
 ...
 scene.Parameters .Set(ForwardPlusKeys.  Probes   , cube    .View)   
          ...
          scene.Parameters.    Set(    ForwardPlusKeys    .  ShadowSampler   , samplers.PointClamp)
               ;  
        scene  .Parameters. Set (ForwardPlusKeys  .EnvironmentSampler,   samplers.LinearClamp) ; scene.Parameters    .   Set( ForwardPlusKeys.    ProbeSampler, samplers.  LinearClamp );
  ...
  var parameters = new ParameterCollection(); var   field =   new IrradianceField (new BoundingBox(   new(-1f),    new(   1f)  )   ,   new    (  1)   )    ;
        using var texture = new   IrradianceFieldTexture
            (field);

           var commands = fixture.Device.BeginCommandList(    QueueKind.
  Graphics, "irradiance names"  );
             
  texture  . Upload(fixture   .    Device,
commands);   
        commands .Finish();
          fixture.   Device.GraphicsQueue.Submit   (  [    commands ]); fixture.Device.   GraphicsQueue .WaitIdle(); 
      ...
      static (TextureHandle Texture, TextureViewHandle  View) Flat    (IGraphicsDevice device , Fixture fixture)   { var texture    =   device .  CreateTexture( 
         new   (    )
      {
         Width    = 4, Height   =    4, Depth =   1   , MipLevels =    1  , ArrayLayers = 1, SampleCount = 1, Dimension = TextureDimension   .Texture2D,
   Format  = PixelFormat
    .Rgba8UNorm, Usage = TextureUsage.Sampled, Name   = "unused"
      }
      );
        var    view   
=  device.   CreateTextureView (  texture);
  ...
  fixture  .  Owns(()    => device
                .  Destroy(   texture    ) );
            return (texture  , view   )   ;    }
    static (    TextureHandle Texture  , TextureViewHandle View) Cube(IGraphicsDevice device, Fixture fixture    ) { var texture    = device.CreateTexture  (
    new( ) {
                Width   =   
4  , Height = 4,   Depth = 1   ,   MipLevels = 1 ,   ArrayLayers = 6, SampleCount    = 1,
       Dimension   =  TextureDimension.TextureCube, Format =   PixelFormat .Rgba8UNorm,  Usage = TextureUsage.Sampled ,  Name =   "unused"
           }  );
...
in   Bitmap  image,    int x,   int y    ) {   
   var offset = image.    Offset(Math.Clamp(x  ,    0, image    .Width
              -   1) , Math    .Clamp(y   ,  0,    image.Height   - 1));
 return new(  image.    Pixels[offset] /  255f,
   image. Pixels[offset    + 1  ] / 255f , image.    Pixels[offset  +   2]    / 255f )  ;
 }
    /// <summary>What the vertex stage reads: position, normal, tangent, texcoord.</summary>
        [StructLayout(LayoutKind.    Sequential
  ) ]
         struct  Vertex  { public const int  Stride =   48;
               public  Vector3 Position    ; public    Vector3 Normal   ;
            public Vector4 Tangent;
       public Vector2  Texcoord   ;
			
           public static Vertex[] Quad  => [ Corner(    -QuadHalf,    -  QuadHalf),
   Corner   (QuadHalf,  -QuadHalf)   , Corner(    -QuadHalf    , QuadHalf),
        Corner(QuadHalf, QuadHalf)
          ]  ;   
    static Vertex Corner(float    x, float   y    )    =>
		new()  { Position = new   (x,  y  , QuadZ    )    ,
       Normal
			= new( 0f, 0f , 1f)  ,
       Tangent = new(1f   , 0f, 0f, 1f), Texcoord = new  ((x /  QuadHalf *    0.5f)    + 0.5f, ( y / QuadHalf * 0.5f) +   0.5f   )   
        }  ; }
    /// <summary>A rough dielectric, so its response is Lambertian and its albedo is its base colour.</summary>
    static Material   Composed    () {   var    compilation = MaterialCompiler   .Compile(
              new() {    ShaderName =   "ForwardPlus"   , 
...
compilation .Failed  ,
 ...
 return compilation.    Material! ; }
    /// <summary>A world with nothing in it, so every ray reaches the sky.</summary>
        sealed class   EmptyWorld : IDistanceField { public float Sample(Vector3 position  ) => 1e6f;
   ...
   public Vector3 SampleGradient (Vector3 position) =>
    ...
    sealed class UniformSky (float radiance) : IRadianceSource { public    Vector3   Sky( Vector3 direction    )  => new( radiance  );
     public
          Vector3 Surface(Vector3    position, Vector3   normal ,   Vector3 direction) => Vector3.  Zero   ;
            }
    static
