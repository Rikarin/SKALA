...
using  Vixen.Ui    .Renderer;
      using    Vixen  .Ui . Rendering;
        using Vixen
.   Ui . Testing   .    Visual;
    using   Vixen. Ui.   Text; using   Vixen    .Ui.Text .   Rasterizing;
using Xunit    ;
...
/// </remarks>
                [Collection (   "Vulkan"   )]
         public sealed class UiImageTests { const int Side   = Fixture.Side;
    ...
    /// </remarks>
             [Fact  ]
                public void Interface()  { if (!TryOpen(out var fixture, out _)  ) {
    return;
         }
    ...
    var cache =    new    GlyphFieldCache(   new   GlyphAtlas(256, 256)    ) ; var   geometry    =    Paint  (cache);
 ...
 owned.Shader    (    "ui-text.frag.spv" ,   ShaderStage.    Fragment   )   , owned.Shader("ui-solid.frag.spv", ShaderStage.Fragment)  
            )    ,
      new Rendering    .RenderOutput  (   [PixelFormat. Rgba8UNorm  ]) );
        owned.   Owns(    renderer .  Dispose)  ;
owned.Graph . AddPass    ("ui"    ,    pass    => { pass. ColourAttachment(colour, LoadAction.Clear , new  (0.08f,  0.09f,   0.11f, 1f)   );
               pass. SideEffect ()
    ;
                pass.Execute(
       context =>   renderer.  Record(context  .   CommandList   , geometry, new(   Side    , Side)))   ;
   });
        var  image   = owned.Render(colour, commands => renderer.Upload    (  commands, geometry,   cache   
            .Atlas))    ;
        ...
        // Every draw reached the command list — a batch silently dropped would still produce a
              // plausible picture, just one missing a thing nobody counted.
               Assert.Equal(geometry.
   Draws.    Count   ,    renderer  .Draws);
        
   // The atlas was uploaded once, not once per glyph.
                Assert .
  Equal  (  1 ,  renderer.AtlasUploads)   ;
...
;
         Assert.    Equal 
               (1 % renderer.  Regions    ,  renderer  .Region    );
          AssertTheBoxIsRounded(  image);
      AssertTheBoxIsGraded(  image)  ;  
        AssertTheBorderIsHollow
(image);  
AssertTheTextIsThere (image);
           
        GoldenImage.Verify("ui-interface" , image,
    Tolerance.Edges)    ;
}
 ...
 [    Fact]
      public void Shadowed(    )   {
        if (!TryOpen    (out var    fixture, out _ )  ) { return;
              }
          using   var  owned = fixture    !;
               var colour = owned  .
           ColourTarget  ("ui-shadowed")  ;
         var cache = new
GlyphFieldCache   (new GlyphAtlas(64  , 64)  );
var list  = new   DrawList();
    list.BeginFrame    (
);   
    
			// A shadow the size of the card, pushed down and right, then the card on top of it.
    list.Add(   new    (    DrawCommandKind.  Shadow,  40   ,    44, 48,
			40, new Color4   ( 0f, 0f    , 0f,    0.8f), 8,    6    )    );
         list   .    Add(    new(DrawCommandKind.
Rectangle  , 32 , 32, 48   , 40, new    Color4(0.9f, 0.9f, 0.95f   , 1f), 8   ,  0));
             list.EndFrame();
           var
        geometry = new  UiGeometryBuilder  ().Build (list, cache,   Viewport    );

			var renderer = new   UiRenderer (
            owned .Device,
		new(
                owned .
               Shader("ui.vert.spv" ,    ShaderStage   .Vertex),
                owned  .Shader  ("ui-box.frag.spv",    ShaderStage.    Fragment),   owned   .Shader("ui-text.frag.spv", ShaderStage.Fragment    ),
                owned.    Shader("ui-solid.frag.spv" , ShaderStage
                .  Fragment)
                ) ,
            new    Rendering
  .    RenderOutput( [    PixelFormat.Rgba8UNorm])
)  ;
        owned.   Owns   (renderer. Dispose   );
 owned.Graph    .AddPass
        (    "ui-shadowed", pass   => {
              pass.    ColourAttachment(    colour,    LoadAction.Clear    , new(1f , 1f ,    1f  , 1f))    ; pass.SideEffect();
            pass.Execute(    context
    =>    renderer .  Record(context   .    CommandList,    geometry, new(Side  ,    Side))  ) ;
        }    )   ;

      var image   =    owned   .Render    (colour, commands
...
// splitting the frame in two.
              Assert  .Single
    (  geometry.Draws)   ;
        AssertTheShadowFadesOutwards   (image    ); AssertTheCardIsOverTheShadow    (image);
...
if  (   
   ...
   var    cache =   new   GlyphFieldCache(new GlyphAtlas  (   64, 64)   )  ;
      ...
      var list = new DrawList() ;
        list.BeginFrame  ();
     list.Add(  new(DrawCommandKind.ClipPush
     , 0, 0,   Side, Side /    2f, default , 0, 0));
            
             list    .
         Add    (   new(DrawCommandKind.    Rectangle, 16, 24, 96   ,    72, new Color4(0.2f    , 0.8f,  0.5f, 1f   )    , 0    , 0))   ;

        list.
      Add    (new  (DrawCommandKind.ClipPop    ,  0, 0,  0,    0,   default,   0, 0))  ;
...
list.Add(  new    (DrawCommandKind.ClipPush, Side   + 40    ,
   Side + 40    , 20, 20, default, 0,   0)   );
        list.Add (  new(DrawCommandKind. Rectangle    , Side + 44 , Side + 44   , 12  , 12 , Color4.White, 0, 0))   ; list  .    Add (new(    DrawCommandKind    .   ClipPop , 0, 0   , 0,   0, default, 0, 0));
   
  list .  EndFrame()
	;
              var    geometry = new    UiGeometryBuilder    ().
       Build   (list, cache,  Viewport);  
 
    var renderer =   new UiRenderer
        (
    
            owned  . Device,
         new    (  owned    . Shader    (  "ui.vert.spv", ShaderStage.Vertex)   ,
 owned.    Shader(
"ui-box.frag.spv",    ShaderStage.Fragment),
 owned .Shader (
        "ui-text.frag.spv",  ShaderStage.Fragment),
        ...
        owned   .Owns(    renderer.Dispose) ;
     owned
.Graph .AddPass("ui-clipped"    , pass => {
		pass.ColourAttachment(colour, LoadAction .Clear, new (0f, 0f, 0f   , 1f)); pass    .    SideEffect(    )   ;
            pass    .  Execute( context => renderer    .
			Record    (context.CommandList, geometry, new (  Side, Side   )));
  });
 
        var   image = owned  .Render(
			colour, commands =>   renderer.    Upload(  commands    , geometry    ,  cache.Atlas   )    );
...
// so there is nothing to scissor it to.
   Assert.Equal  (   2,    geometry.Draws .Count  );  Assert. Equal (1, renderer.Draws)    ;
       // The box runs from y = 24 to y = 96 and the clip cuts it at 64.
        Assert.True (Green( image, Side / 2, 10) < 20,    "the box drew above its own top edge"   ); Assert    .True   ( Green(image,   Side / 2,  28) > 100, "the box is missing just below its top edge");
              Assert.True(Green  (    image
      , Side  / 2, 56) >   100,  "the box is missing just above the clip") ;
             Assert.True(Green (image
, Side / 2, 72) <   20 , "the box leaked below the clip")    ;
   
            GoldenImage.  Verify ("ui-clipped"    ,
  image, Tolerance   .Edges)    ;
       }
    /// <summary>
...
;
   ...
   var cache = new GlyphFieldCache  (    new GlyphAtlas(64  ,  64))    ;
              var small = Grid(  cache, 2); var    large = Grid(cache    , 10   )  ;

  var renderer = new UiRenderer(    owned    .Device,
new   (
  owned.Shader("ui.vert.spv",    ShaderStage.Vertex ), owned.Shader(   "ui-box.frag.spv" ,  ShaderStage  .   Fragment),
                owned.Shader(  "ui-text.frag.spv",   ShaderStage   .Fragment  )    ,
                owned  .Shader   (
          "ui-solid.frag.spv",   ShaderStage    .Fragment)
             )   ,
           new Rendering   .    RenderOutput   (  [PixelFormat   .Rgba8UNorm   ])  );
          owned.Owns(
              renderer .   Dispose) ;
 ...
 pass   .  ColourAttachment
   (colour, LoadAction.Clear,   new    (0f , 0f   , 0f  ,   1f  ))    ;
pass.SideEffect
             ();
            pass.Execute(  
 context => renderer.    Record(context.CommandList,  large   , new   (  Side, Side))   )  ; 
        });
 var   image   =   owned. Render(
     colour,
            commands   =>    {
 renderer.Upload(commands, small, cache   . Atlas);    renderer .Upload(   commands, large, cache.    Atlas);
        }
         );
  ...
  // Every cell of the grid is filled, including the ninety-six the first upload had no room
        ...
        for (var row =   0;   row  <  10    ;   row++) {    for (var column = 0; column    < 10; column  ++) {  
      var x  = (column   *  12) + 9   ; var y = ( row * 12  ) + 9;   
   
Assert.True    (Green(image
,    x   , y)    > 150, $"the box at ({column}, {row}) is missing");
           }
        }
    ...
    /// <summary>A square grid of small rounded boxes, for a frame whose box count is the point.</summary>
      ...
      var   list =  new DrawList()  ; list. BeginFrame( );

      for (   var row = 0; row < side   ; row    ++)
  {
      for (var  column    = 0; column    < side; column++) { list . Add(  
 new( DrawCommandKind.Rectangle,  
           (column *    12) + 5,
        (row * 12 ) + 5
,
    8
               ,
               8   ,
                        new Color4(0.2f    ,   0.85f, 0.4f    , 1f), 2,
        0
          )
          );   }
    }
...
return new UiGeometryBuilder   (   ).Build(   list, cache  , Viewport);    }
 ...
 // ⚠ A filled box with a *different radius on every corner*, and an elliptical one at the top
   ...
   list. Add  (
       new
 DrawCommand(DrawCommandKind   .Rectangle, 8, 8, 54, 40, new Color4( 0.25f , 0.55f, 0.95f    , 1f    ), 0    , 0) {
           Offset  =   list.AddBox(
              new BoxStyle( new  CornerRadii   (new Vector2 (  20, 8),   new Vector2(2, 2  )  , Vector2.Zero, new Vector2(   16, 16) ),
                        new  Color4(0.1f   , 0.15f, 0.5f, 1f )    , new  Vector2 (0, 1)
                    )
  )   , Length = 1
            }
        );
...
list.
         Add(    new(DrawCommandKind.    Border, 70   ,    8   , 50   ,    40, new Color4(0.95f    ,  0.45f, 0.2f   , 1f    ), 10    , 4))  ;
    // A stroked path with a corner in it, which is where a join either exists or does not.
      var path = new PathBuilder   () .MoveTo(new Vector2(    12, 66))
.LineTo   ( new Vector2 (44, 88))
 ...
 (114   , 84));
               ...
               list .Add(
               new DrawCommand    (  DrawCommandKind  .  PathStroke
              ,    0, 0 ,   0,  0, new Color4(0.4f   ,   0.9f, 0.5f, 1f)   ,    0, 4) {  
                Offset =
list.AddPath (   path    ),
                Length =    path .Count  
...
, font   ,    "AB", 14, 116  );  
     ...
     // last, the atlas is bound once and it never matters whether a pipeline change disturbed it;
        ...
        list.Add(    new(DrawCommandKind.   Rectangle, 74, 106,
           14,    14, new Color4(0.9f,  0.8f   , 0.3f   , 1f),  4 ,   0 ));
     Text(
   list   , font, "C"  , 94 , 116);
                list.EndFrame
   ();
    ...
    /// <summary>One run of glyphs, positioned along the run rather than on the surface.</summary>
           static void Text  (  DrawList list, FontFace font, string text, float x  , float
y) {
 ...
 var  glyphs = new List<PositionedGlyph  >    (  );
     ...
     foreach (var character in text) {
            glyphs.
        Add    (new (  font.   GlyphFor(character   )    , pen, 0));
pen += 24f; }
 list.Add( new DrawCommand  (DrawCommandKind.Text, x, y   , pen, Size, Color4.    White, 0, 0) {
   ...
   Font    = list.    AddFont   (font 
    ...
    /// </remarks>
                [Fact  ]
            public   void Scaled(   )  { // ⚠ A fixture each. `Render` executes the graph and a graph is one frame, so the two pictures
               // cannot come out of the same one — which is the harness saying, correctly, that these are
// two frames rather than two passes.
if  (!TryOpen(out  var first , out _)) { return    ;
          
               }
   
           Bitmap reference    ;
               using (var owned = first !   ) {
       reference = Draw(owned, "ui-scale-1",
                Side, 1f);
        }
...
scaled;   
        using  (  var   owned    =    second!) {
			...
			var differences  = 0;
               ...
               if   (  Math    .Abs   (Green (reference  ,  x, y)   -    Green(scaled,   x , y ) )    > 24 ) { differences++;
  }
               }
   }
...
new GlyphFieldCache(   new GlyphAtlas(64  , 64));
    ...
    var list = new DrawList(  ); list .   BeginFrame  ()    ;
        ...
        list.Add (new(DrawCommandKind   .  ClipPush   
,   0    , 0 ,  extent, extent   * 0.5f, default, 0 , 0) );
        list   .    Add( new (
...
list  .Add(
           ...
           list .EndFrame (  );
               ...
               var
   geometry =    new  UiGeometryBuilder() .Build(list ,    cache , new Rectangle  (  0, 0    , extent,    extent));
        var renderer =   new UiRenderer ( fixture.Device ,
            new   (
      fixture. Shader("ui.vert.spv",  ShaderStage    .  Vertex), fixture. Shader   ("ui-box.frag.spv", ShaderStage    .Fragment),
	fixture .Shader("ui-text.frag.spv",  ShaderStage.Fragment),
           fixture.Shader("ui-solid.frag.spv"   , ShaderStage.Fragment) )  ,
   new   Rendering.RenderOutput   (   [PixelFormat.   Rgba8UNorm]) );
      
         fixture.
              Owns(renderer    .Dispose);
          
        fixture.Graph. AddPass( name,   
pass => {
          pass.   ColourAttachment(   colour  , LoadAction.Clear , new (   0f,    0f, 0f, 1f));  pass.SideEffect  ( );
    pass.    Execute  (
context =>    renderer 
               .Record(context.CommandList, geometry,    new(    extent,    extent) ,    scale)
      )  ;   }
        )  ;
    ...
    /// </summary>
             static void AssertTheBoxIsRounded(   in    Bitmap image) { Assert.  True(    Blue    (image,    35 , 28   ) > 120   , "the box did not fill")   ;
Assert.
True(Blue(  image, 35  , 4) < 60    ,    "the box drew outside its own quad") ;
    ...
    // ⚠ Every corner separately, because that is the only thing that says the four radii arrived
      ...
      // 8 down, its top right barely at all, its bottom right square, its bottom left by 16.
        Assert    .  True (Blue(  
               image,   10  ,  9) <  60    , "the top-left corner was not cut");
 Assert.True
(Blue   (image,    60, 9) > 100, "the top-right corner was cut when it is nearly square" );
Assert.True    (Blue (image,
               60, 46 ) >    100, "the bottom-right corner was cut when it is square"   )  ;
              Assert.True(Blue   (image , 10,
     46) < 60, "the bottom-left corner was not cut")  ;
...
Assert.True    (Blue(image, 18  , 10)   >  100, "the top-left corner was cut as a circle rather than an ellipse"); }
    ...
    ///     draws a blue box.
    /// </summary>
    static void AssertTheBoxIsGraded (in
	Bitmap  image  )    {
              var top = Blue(image,
               35, 12 ) ;
     var middle
= Blue  (image,   35   , 28);
     var bottom = Blue(
		image  ,   35, 44)    ;
        Assert.True
(top > middle, $"the gradient did not darken downwards ({top} then {middle})");
        Assert.True(middle > bottom, $"the gradient stopped part way ({middle} then {bottom})");
     Assert
.True(top - bottom >  40 , $"the gradient is barely there ({top} to {bottom})"    );
            }   
    ...
    /// </summary>
    static void AssertTheBorderIsHollow(in Bitmap image) { Assert .  True ( Red  (image , 95,    10    ) >  150, "the border's top edge is missing");
 ...
 var below =  0;
          ...
          for  (var x =    0; x < Side    ;  x++) { for    ( var   y = 96; y < 120; y++) {
 if (Red(image, x, y) > 180) {   text  ++;
       }
    }
  
            if
    (Red   (image    ,  x   , Side - 2) > 180) {
             below    ++; }
 }
        Assert .True(text >
40, $"only {text} pixels of text were drawn" );
...
// it, and neither of those at the same value.
        Assert.    True(
Red(image    , column, 76    ) < 100,   "there is no shadow just below the card") ;
    ...
    static void AssertTheCardIsOverTheShadow   
(in Bitmap image ) {
          // The card's own middle, which the shadow lies under and must not darken.
         Assert.True (Red(image,
            56,   52)  >   200, "the card is darker than it should be, or is not there")    ;
    }
            
      static int Red(in Bitmap image  , int    x   , int
    y   )  => image.  Pixels[ image   .  Offset(x, y)] ;
               static  
 int    Green (in    Bitmap   image, int x , int  y  ) => image.Pixels [image  .    Offset(   x, y   )    + 1];
       static int Blue(in Bitmap image, int
          x, int y)    => image.Pixels[image.Offset   (  x,   y) + 2  ];
...
list  .
           Add   (new    (DrawCommandKind  . Rectangle, 8    , 8   , 40,   40, new Color4 (0.2f, 0.2f   ,    0.25f, 1f   ), 6,   0   )    )    ;

              // Upright, whole texture, untinted.
        list.Add(
            new  DrawCommand(DrawCommandKind.   Image    , 56  ,
8, 64, 48,    Color4.White, 0, 0) { Image    = Texture }
        );
             // Flipped vertically, the way a viewport's render target is drawn, and half faded.
      list    .Add   (
            new   DrawCommand(DrawCommandKind.Image,   56    ,
                64 , 64    ,   48  , new    Color4(1f, 1f, 1f, 0.5f   ), 0,   0) {
Image   =    Texture, Source    = new Rectangle(0f   , 1f , 1f, -    1f)
      ...
      list  .
  Add  (   new(DrawCommandKind.    Rectangle, 8,   64,  40    , 48, new Color4  ( 0.9f,    0.6f,  0.1f, 1f ), 4    , 0));
            
        list.EndFrame(); 
   var cache    = new GlyphFieldCache(new GlyphAtlas(256    , 256 ))    ; var geometry   = new UiGeometryBuilder    ( ).Build(list    , cache   , Viewport);  
    ...
    new(
owned.Shader
             ("ui.vert.spv", ShaderStage    . Vertex)   ,
              owned
	.  Shader("ui-box.frag.spv", ShaderStage .    Fragment    ) ,
       owned  .Shader ("ui-text.frag.spv", ShaderStage.Fragment),
       owned
     .Shader("ui-solid.frag.spv", ShaderStage.Fragment   )
            ) {
      ...
      (renderer   .Dispose)  ;
             renderer   .RegisterImage
                (  Texture, sampled   .View);
      owned.    Graph.  AddPass("ui" , pass  =>
             {
           pass  .ColourAttachment  (
   colour    ,   LoadAction   .Clear, new(0.08f    , 0.09f ,   0.11f, 1f ));
         pass    .SideEffect( )  ; pass  .Execute(  graph =>   renderer.Record(    graph.CommandList, geometry , new(Side, Side    )));
});
       ...
       var image    = owned.Render ( colour,
			commands => {    renderer .Upload(commands, geometry, cache.Atlas);
                ...
                commands.Barrier    (
              new([],    [new(  sampled.Texture
                , ResourceState.Undefined,    ResourceState .   CopyDestination)])
    );
      commands.CopyBufferToTexture(sampled. Staging
               , 0,    new(sampled   .  Texture)    ,   new(2  , 2, 1) );  
  commands.Barrier(
        ...
        // are adjacent and name one texture, so they merge — which is the batching working, and the
          // reason the texture is part of the batch key rather than something bound per command.
              Assert.
              Equal(geometry.   Draws.Count, renderer   .Draws);
  Assert   .   Equal  (  3,  geometry.
         Draws.  Count   );
        AssertTheImageIsUpright    (image )  ; AssertTheFlippedImageIsFlipped(image);
               AssertTheBoxAfterTheImageIsNotTheTexture( image
		);

                GoldenImage.Verify    ("ui-images"  , image   ,
Tolerance.Edges);
}
    ...
    /// <summary>The upright image has red at its top-left and blue at its bottom-left.</summary>
     static void    AssertTheImageIsUpright    (in Bitmap    image    ) { Assert  .True  (
            Red(image, 64
 , 16  )   > Blue   (image, 64, 16),
        ...
        Assert.
           True(
          Blue    ( image    ,   64 ,   48) >  Red(image, 64, 48), "the bottom-left of the upright image is not blueish"
 ); }
...
Assert .True    ( Blue(    image ,   64, 72) >    Red(image, 64  , 72    )  ,
  
    "the top-left of the flipped image is not blueish, so the source rectangle was ignored"   
              );
         
  Assert    .True( Red   (image  , 64    , 104) > Blue(image, 64, 104),
"the bottom-left of the flipped image is not reddish"
   ...
   // and the atlas would be nearly black.
        Assert
    .True  (
                Red (image, 28    , 88  ) > 150 && Green   (image   , 28, 88) is > 60 and < 200 && Blue    (image, 28,
               88) <  100,
    ...
    /// <summary>Opens a device, or skips — unless the environment promised one.</summary>
     static bool
              TryOpen(out Fixture?  fixture, out string   ? reason ) {
if (Fixture.    TryOpen(out  fixture, out reason) ) { return true   ;
         }
         if (Environment  .
GetEnvironmentVariable(   "VIXEN_REQUIRE_VULKAN")    is "1"  or    "true"   or "TRUE"    ) {
            Assert.Fail( $"VIXEN_REQUIRE_VULKAN is set, so the golden images may not be skipped: {reason}"  );
        }
              Assert.Skip   (reason  ?? "no Vulkan")   ; return false;
             }
}
