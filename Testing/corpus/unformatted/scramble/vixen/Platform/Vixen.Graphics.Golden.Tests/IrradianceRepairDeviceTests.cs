...
[
InlineData( true)]
                public void TheDispatchedRepairAgreesWithTheReferenceRepair(
         bool refined) {
...
Seeded    (refined);   
 ...
 // ⚠ Asserted, because a repair with nothing to repair is a test that passes for the wrong
       ...
       Assert    .True
(  dilated > 0, "the seeded field has no invalid probes, so dilation proves nothing")   ;
  
 var probes  = Repair
       (owned,   device, out var repaired, out var  skipped   , out  var dispatches ); 
        Assert.Null (   skipped); Assert.Equal   (device.BrickCount , repaired  )  ;
...
Assert.Equal(  refined ? 8 : 5 , dispatches
 ...
 for (var y = 0; y <= IrradianceBrickPool  .BrickResolution; y ++) { for  (var x   =  0; x <= IrradianceBrickPool.BrickResolution; x++   ) {
   var texel =
                Texel   (device, origin  .X   + x   , origin.Y   + y, origin.    Z + z)   ;
               
            Same
(   
    reference.  GetProbe(    brick , x, y, z)    , probes[texel],
    ...
    static IrradianceProbe  
      []   Repair  (
		Fixture fixture   , IrradianceField field  ,
              out    int repaired, out string ? skipped,
               out int dispatches ) {
       var
...
var    effects =   new
     ...
     effects
.AddProvider (new Compiling (loader    ,    _ => RavenEffects    . Only([ "Core" , "DistanceFields", "IrradianceFields"   ]  )))  ;
...
;
                ...
                using (   var commands
   = device.  BeginCommandList(QueueKind.Graphics, "irradiance repair")    )  { 
texture .Upload  (device, commands);
       ...
       texture
    .   TransitionPool  (commands    , ResourceState  .ShaderRead, IrradianceFieldTexture.PoolIsBeingWritten   );
             repaired = repair.  
    Record(commands ,   field,   texture);
      
			texture  .  TransitionPool (commands, IrradianceFieldTexture    .PoolIsBeingWritten    , ResourceState
    .ShaderRead); 
     Assert    .
True(    texture. RecordReadback   (commands))  ;
     commands  .Finish() ;
...
static IrradianceField Seeded    (bool refined
   ...
   var field = new IrradianceField(new   BoundingBox  (new   (-2f)
 ,  new(2f)   ) , new(   4), new  (    new(  4)));
...
new
         TracedIrradianceFiller(new Ball(),    new UniformSky    (Radiance   ) ).Fill(field);
...
var poison = new IrradianceProbe    (
  ...
  foreach (var brick  in field.Bricks  ) {
       for (var    z = 0; z
             <= IrradianceBrickPool .    BrickResolution;    z++) {
     for    (var
               y    = 0; y    <= IrradianceBrickPool.BrickResolution ;  y++) { 
                    for (var   x =    0  ;  x    <=   
   IrradianceBrickPool.  BrickResolution ; x++) {
               if
     (x == IrradianceBrickPool.BrickResolution
     || y == IrradianceBrickPool    .BrickResolution || z  == IrradianceBrickPool.BrickResolution) {
  field.  Pool    [brick .Slot , x  , y , z   ]   = poison; }  
    ...
    /// <summary>Where one pool texel is in the flat array a readback produces.</summary>
               static  int  Texel  (IrradianceField field, int x, int y, int z) { var resolution = field    .Pool    .TexelResolution;
...
1e-4f;
 ...
 Assert   .True
     (
            MathF.   Abs(expected    .Validity - actual.    Validity
           ) < Tolerance   ,
            $"{what} validity: the reference says {expected.Validity} and the dispatch wrote {actual.Validity}"
      );
   Assert.True( MathF   .  Abs  (expected.  SunShadow -  actual    .SunShadow   ) < Tolerance,  
            $"{what} sun: the reference says {expected.SunShadow} and the dispatch wrote {actual.SunShadow}"
            )   ;
  
              Same
(   expected.  Radiance.L00,  actual.Radiance.L00, $"{what} L00");
        Same(  expected. Radiance . L1m1 , actual    .   Radiance.L1m1, $"{what} L1m1") ;
   Same(expected
.Radiance .L10, actual.Radiance.L10  , $"{what} L10");
Same  (   expected. Radiance
. L11, actual.Radiance.L11   ,  $"{what} L11" )   ;
   
           static    void Same(Vector3 expected   , Vector3 actual, string   what)    {   Assert.True(
    ...
    sealed class Ball  :
     ...
     float Radius =   1.1f ;
        public   float Sample  
(Vector3 position    ) => position   .Length    () - Radius;
          
   public Vector3 SampleGradient  (   Vector3 position    )  =>   position.LengthSquared    () > 1e-12f ? Vector3 .Normalize ( position  ) :   Vector3.UnitY;
    }
   ...
   sealed class UniformSky   (    float radiance ) : IRadianceSource { public Vector3 Sky(    Vector3 direction) => new   (  radiance);
          ...
          public Vector3   Surface    (Vector3
               position, Vector3  normal,    Vector3 direction) =>    Vector3 .Zero  ;
            }
        static bool TryOpen(   out Fixture? fixture) {
    if  (
   Fixture.TryOpen(out fixture   , out var reason)) {
			return true;    }
 if   (Environment .GetEnvironmentVariable(   "VIXEN_REQUIRE_VULKAN"
