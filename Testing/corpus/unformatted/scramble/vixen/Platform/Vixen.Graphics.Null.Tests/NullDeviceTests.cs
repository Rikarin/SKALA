...
namespace
  ...
  public sealed class
   NullDeviceTests : IDisposable {
           readonly
            NullDevice device    =  new(   new() { Record = true }    );
            [Fact] public void ItReportsItselfAsSoftwareRatherThanPretendingToBeAGpu    () {
       Assert.Equal (AdapterKind .Software
           , device.Adapter.Kind);
    ...
    /// </summary>
                [Fact] public    void ADestroyedHandleIsRefusedRatherThanReused ( )   {
       var buffer =
       device.    CreateBuffer   (new    (256,  BufferUsage.Uniform   ,    MemoryAccess.HostUpload    , "Constants"));
       device
.  Destroy(  buffer);
...
public void    EveryResourceThatIsTakenComesBack(
	...
	Assert  .Equal(0
    , device .LiveResourceCount);
    for (var    round =  0  ; round   < 100; round++) { var buffer = device. CreateBuffer(new(   1024, BufferUsage.  Vertex , Name: "Mesh" )    );
			var texture = device.CreateTexture    (new(PixelFormat.    Rgba8UNorm, 64, 64    ,    TextureUsage    .Sampled, Name : "T")); var  view = device.CreateTextureView(   texture ) ;
             var sampler    =   device  .CreateSampler(SamplerDescription    .
     LinearClamp);
  device.Destroy(  view); device.    Destroy  (  sampler);
...
;
              ...
              Assert.   Equal( 
0 ,    device.LiveResourceCount ) ;
            }
                
        [Fact] public void ADeviceLocalBufferCannotBeWrittenByTheHost( ) {
              var buffer    = device.CreateBuffer(new(256,
       BufferUsage .Vertex, MemoryAccess  .   DeviceLocal,   "Mesh")) ;
         
        var thrown  = Assert.   Throws<   InvalidOperationException>( ( )    =>   device.  Write(buffer  , 0, new byte[16  ]))    ; Assert  . Contains(    "Stage it",   thrown.Message, StringComparison    . Ordinal);
             }
    ...
    [Fact]
        ...
        var    buffer =  device.
        CreateBuffer  (new    (16, BufferUsage.Uniform,  MemoryAccess.    HostUpload, "Small"    ));
        device.Write   ( 
buffer, 0,  new byte[16]);
   ...
   public void ReadbackGivesDefinedBytesRatherThanWhateverWasThere( ) {
              var buffer    = device.CreateBuffer( new( 8,  BufferUsage.Storage |    BufferUsage.CopyDestination, MemoryAccess.HostReadback  ,    "Readback")
            
              );
         
        var destination = new  byte[  8 ];
            Array.Fill(destination    , (byte)0xCD); device .   Read  (   buffer, 0, destination   );
                Assert.    All    (destination, value   =>   
    ...
    /// </summary>
        ...
        public  void ADeviceWithoutComputeRefusesAComputePipeline() {   
            using var limited = new    NullDevice(new(    ) { Features   = GraphicsDeviceFeatures
           .  Minimum }); 
   var   shader = limited.    CreateShader    (
			ShaderStage   .Compute, [    1 , 2,    3, 4]);
                var  layout   = limited.
           CreatePipelineLayout (new   ([ ])); 
        ...
        using var    limited  = new NullDevice(   new() { Features
           = GraphicsDeviceFeatures.Minimum });  
          var
  thrown =    Assert   .Throws   <ArgumentException >(
            (    )  => limited  .CreateTexture(   new(
PixelFormat.  Rgba8UNorm    , 8192,   8192  , TextureUsage.Sampled, Name: "Huge"  )   )   
        )    ;
        Assert.   Contains
(  "4096"    , thrown.Message,   StringComparison .    Ordinal ) ;
            }
    /// <summary>
    ...
    /// </remarks>
    [ Fact]   public void    AWriteOfTheWrongKindIsRefused   () {
               var   layout = device.CreateDescriptorSetLayout(
       new    (  DescriptorSetSlot.PerDraw, [  new(0,    DescriptorKind
.   DynamicUniformBuffer  ,  ShaderStage.Vertex  )], "Draw"   )
               );

        var set = device. CreateDescriptorSet   (layout);
       var buffer = device.CreateBuffer   (new  (   256, BufferUsage
    .    Uniform, MemoryAccess.HostUpload, "Transforms"    ));
              var   thrown   = Assert   .
         Throws<ArgumentException  >(
            (  ) => device .   UpdateDescriptorSet
	(    set,   [DescriptorWrite  .  Uniform(0, buffer , 0  , 64 )])
        );
    
        Assert    .Contains("DynamicUniformBuffer", thrown.
...
public void ASwapChainCyclesThroughItsImages( ) {
               using   var swapChain = device. CreateSwapChain
            (new(   SurfaceHandle.None, new(1280,    720),   ImageCount: 3 ));
          
        var
seen = new  HashSet<TextureViewHandle>  ( );

  for ( var  frame = 0; frame  <    3; frame
++   ) {
    Assert. Equal( SwapChainStatus   .Ready,  swapChain  .AcquireNextImage  (out var view )    ); Assert.    True(seen.  Add   (view  ));
   Assert.Equal  (SwapChainStatus
               .Ready   , swapChain.Present   ());
               }
         // And then round again, onto the images it already has.
	Assert.Equal  (SwapChainStatus   .   Ready, swapChain   .AcquireNextImage (out
var   repeated    )    );
    ...
    public  void AnOutOfDateSwapChainReportsItRatherThanHandingBackAnImage( ) {
               using var swapChain   = (NullSwapChain)device  .CreateSwapChain
(new    (SurfaceHandle.   None, new(800   , 600)));
      swapChain.NextStatus =   SwapChainStatus
.OutOfDate   ;

                Assert.Equal  ( SwapChainStatus.    OutOfDate,    swapChain.    AcquireNextImage(out var view )   ); Assert.    False(  view.IsValid);
             
    swapChain    .   Resize
              (   new(1024 ,   768  ));
            
            Assert.Equal(new  Int2(1024,  768), swapChain   .    Size   ); Assert.   Equal  (SwapChainStatus.Ready, swapChain.  AcquireNextImage(out var fresh)    )   ;
Assert.True(fresh.IsValid);  }
  [Fact]
    public void  ASwapChainReturnsItsImagesWhenItIsDisposed
                ( ) {
            var before = device. LiveResourceCount;
 var swapChain = device.CreateSwapChain(new (    SurfaceHandle .    None    , new(   640
, 480), ImageCount:  2 ) )   ;
        swapChain.AcquireNextImage(out _) ; 
	swapChain.AcquireNextImage(
out _);
       Assert.
    True(    device.LiveResourceCount >  before);
   ...
   swapChain.Dispose    ();
      ...
      // a real backend, so this asserts the views rather than the whole count.
        Assert
  .True   (   device.    LiveResourceCount    <   before    + 4 )    ;
 }
                [Fact]
     public    void FramesAreCounted(  ) {
             for (var frame = 0; frame <   5
 ;    frame++)   {
               device.BeginFrame   ()   ; device.  EndFrame( ) ;
}
    ...
    /// </summary>
            [Fact]
    public void ADeviceThatWasNotToldToRecordDoesNot(   )  { using var server  = new    NullDevice    ()  ;
        Assert . Null
(server.Recorder)    ;
        using var list    =
    server .BeginCommandList() ;
		list.PushDebugGroup("Frame"  ) 
         ;
    list    .PopDebugGroup(
              )   ;
   list. Finish(   )   ; server   .   GraphicsQueue.    Submit (    [list])   ;
...
.  PerMaterial , [new (0  , DescriptorKind.StorageBuffer, ShaderStage . Fragment ) ], "Material" )
  ) ;
                var set = device.   CreateDescriptorSet( 
      layout  );
        var
buffer    = device    .CreateBuffer(new    (   256 , BufferUsage. Storage  , Name:    "Instances") );
    ...
    /// </remarks>
          [   
              Fact]
public
     void AnElementOutsideItsBindingIsRefused() {
var    bounded =
device.CreateDescriptorSetLayout (
        ...
        DescriptorSetSlot
       .PerMaterial,
  [    new (0,  DescriptorKind.SampledTexture
 , ShaderStage. Fragment, 4)    ],
          "Probes" )
        );
      
       var  table = device.CreateDescriptorSetLayout(
            new (    DescriptorSetSlot    .PerFrame,
                [new   ( 0, DescriptorKind.SampledTexture , ShaderStage   .Fragment,   0)  ] ,   "Textures"  
               )
...
var   texture = device.  CreateTexture(new(    PixelFormat.Rgba8UNorm,   4,  4 ,    TextureUsage.Sampled,  Name: "Grey"    ) );
  ...
  var tableSet
= device  .CreateDescriptorSet    (table);
         device.   UpdateDescriptorSet(boundedSet, [DescriptorWrite    .
            ...
            device.UpdateDescriptorSet (tableSet   ,
 ...
 public    void    AnUnboundedBindingNeedsTheCapability (
   ...
   using   var minimum =
          new NullDevice(new (   ) { Features = GraphicsDeviceFeatures.  Minimum })    ;

  var refused  =   Assert.
     Throws <ArgumentException>(
      () => minimum    .CreateDescriptorSetLayout( new(
         DescriptorSetSlot
           .PerFrame,
  [new(0,    DescriptorKind.   SampledTexture ,
ShaderStage.    Fragment,   0)  ]  ,
 ...
 Assert.
       Contains("HasBindless", refused.Message, StringComparison . Ordinal) ;
	}
    /// <summary>
    ...
    /// </remarks>
        ...
        using var    limited = new NullDevice( new()    {
             Features = NullDevice. Everything with { HasMultiDrawIndirect
              = true,  HasDrawIndirectCount =  false   }
            }
           );
                
                var arguments   = limited.CreateBuffer (  new(64,   BufferUsage.Indirect,  MemoryAccess   .DeviceLocal) ); var counts   = limited.CreateBuffer(new(4, BufferUsage .Indirect    , MemoryAccess    .DeviceLocal))    ;   
        var target = limited
 .CreateTextureView(
 limited.CreateTexture(new(PixelFormat   .Rgba8UNorm,   8,
8, TextureUsage.    ColourTarget)    )
     );
			
     using var
      list =   limited.BeginCommandList();
           list.BeginRenderPass(
      new([new   (    target) ]   ));
   
    var refused = Assert.  Throws
    ...
    public void  WithTheCapabilityItRecordsItsCeiling(    )
        {
             ...
             var arguments =  capable
.  CreateBuffer   (  new(    64 , BufferUsage.    Indirect , MemoryAccess.   DeviceLocal));
           var counts
= capable.CreateBuffer   ( new   (  4,    BufferUsage  .   Indirect, MemoryAccess   .   DeviceLocal)    )   ;
...
(   );
			list   .   Finish()
               ;
               capable.GraphicsQueue.
        Submit([ list  ]    ) ;
        var  drawn    = Assert.  Single
         (capable . Recorder!.OfKind    (    RecordedCommandKind.DrawIndexedIndirectCount   ))  ;  
Assert.Equal   ((long )arguments.Value.
    Packed,  drawn.A);
       Assert.Equal    ((long)counts  .Value   .    Packed, drawn.  B   )  ;    Assert. Equal(    20, drawn.C);
	Assert.Equal(  3, drawn  
         .D );
           }
			
    public
        void Dispose () => device   .    Dispose();
}
