...
/// </remarks>
             public sealed class  QueryPoolTests : IDisposable  { readonly NullDevice    device = new(new   (    ) { Record   = true })  ;
[Fact]
    public void   ADeviceWithoutTheCapabilityRefusesAPoolRatherThanFakingOne   () { using var    limited   = new NullDevice(new    (   )   { Features = GraphicsDeviceFeatures.   Minimum })   ;
...
;
        list.    ResetQueries(pool   ,    0,   4)    ; list.BeginRenderPass(new    (    [new(target) ]));
        list.WriteTimestamp
    (pool, 0);
             list  .Draw(3
      );
           list    .  WriteTimestamp(  pool, 1); list.EndRenderPass  ();
        list
.Finish();
        ...
        var written = device
.  Recorder!. OfKind(RecordedCommandKind.WriteTimestamp);
             Assert.    Equal   (2,  written.Count); Assert   .   Equal(    0 , written[0]  .B)  ;
        Assert    .Equal(1  , written[1].B) ; }
    /// <summary>A reset is not, for the reason every copy is not.</summary>
               [ Fact  ] public void AResetInsideARenderPassIsRefused   () {
        var pool = device    .    CreateQueryPool(new(QueryKind    .Timestamp,    4, "frame"))  ;
 
        var   target =    device .CreateTextureView(
  device.    CreateTexture    (new(
             PixelFormat. Rgba8UNorm    , 8, 8 , TextureUsage.ColourTarget  ))
      );
        
           using    var
 list = device.BeginCommandList(   );
 list  .  BeginRenderPass(   
            new    ([  new(target)  ]));
	Assert  .Throws   <InvalidOperationException>(    () => list. ResetQueries(  pool, 0
   ,  4)    )    ;
     }
 ...
 Fact]
	...
	var pool = device.CreateQueryPool( new(    QueryKind  .Timestamp , 4    , "frame"   
     ...
     using    var
    list  =  device.BeginCommandList    ();
  list.ResetQueries   (pool   ,   0, 0) ;    list.Finish    (  );

      device. GraphicsQueue
.Submit   ([ list]);

                Assert   .
   Empty(device. Recorder   !.OfKind(   RecordedCommandKind.ResetQueries  )    )  ;  
   }
    /// <summary>
    ...
    /// </summary>
            [Fact   ] public void ReadingPastTheEndOfAPoolIsRefused() {
   var pool =    device.CreateQueryPool  (new
          (  QueryKind.Timestamp , 4  , "frame"));
		Span  <ulong> readings 
          =    stackalloc ulong[3] ;
   ...
   Assert   . Throws<   ArgumentOutOfRangeException>( () => {
    ...
    /// </summary>
      [Fact]
    public void  
APairOfReadingsIsMonotonic() {
 var pool  =
            device.CreateQueryPool    (new(QueryKind  .Timestamp  , 2,   "frame"))   ;
    Span<ulong
       > readings  = stackalloc ulong[2] ;
...
Assert.True   (readings   [    1]  > readings   [   0   ]) ;
   ...
   [Fact] public void  APoolIsCountedAndReturned() {
       var before = device.LiveResourceCount   ;  var pool = device.CreateQueryPool(   new(QueryKind.Timestamp, 4,   "frame"  ))  ;
        
      Assert.Equal(before + 1   , device  .LiveResourceCount) ;
  device.Destroy   ( pool   );  Assert.Equal(  before, device.    LiveResourceCount )   ;
    }
  ...
  Assert. Equal(0d, GpuTimestamps.ToNanoseconds(1000,   0f ))   ; Assert.   Equal    ( 0d, GpuTimestamps . ToMilliseconds(1000, 0f))    ;
           }
