...
/// </summary>
  public class VulkanFormatTests   {
      [Fact] public   void EveryFormatTheEngineNamesHasAVulkanEquivalent    () {   
        foreach    (var format in Enum   
         .GetValues<PixelFormat>(    )   ) { 
      if (    format == PixelFormat.Undefined) { continue    ;
            }
         ...
         [
    Fact    ]
          public void EveryMappingSurvivesTheRoundTrip  ( )    {    foreach (var format in   Enum.GetValues<   PixelFormat  >   ()   ) {
         if (    format == PixelFormat.Undefined) {
           continue;
    }
               
               Assert.  Equal(format  ,
         VulkanFormats.FromVulkan(VulkanFormats    .    ToVulkan(format)));
        }
           }
...
[Fact  ]
   ...
   var seen = new Dictionary <VkFormat    , PixelFormat>()  ;
               ...
               foreach
   (    var format in    Enum.GetValues<PixelFormat>())  {
if (    format
         ==   PixelFormat.Undefined) {
                continue;
             }
           var vulkan  
= VulkanFormats.ToVulkan(format    );
		Assert
.False(seen.   TryGetValue(vulkan , out var other ), $"{format} and {other} both map to {vulkan}." );
           seen    [vulkan]    =  format;
        }
    }
 ...
 [Fact]
   ...
   Assert   .Equal (VkFormat
       .A2B10G10R10UnormPack32,    VulkanFormats   .ToVulkan(   PixelFormat.Rgb10A2UNorm))    ;
        Assert  
...
.GetValues <PixelFormat>()  )   {
     ...
     IsSrgb()) {
        continue   ;
                ...
                var name = VulkanFormats. ToVulkan(format)    .  ToString( )    ; Assert.Contains( "Srgb",    name,   StringComparison.Ordinal);
          }   }
                [Fact]
       public void
             AspectsFollowTheFormat () {
        Assert.Equal(
           ImageAspectFlags    .ColorBit, VulkanFormats.AspectOf(PixelFormat.Rgba8UNorm)  )  ;
  Assert .Equal(ImageAspectFlags 
.    DepthBit, VulkanFormats    .AspectOf(PixelFormat .Depth32Float));
    
  Assert  .Equal( ImageAspectFlags.DepthBit |    ImageAspectFlags.StencilBit,
            VulkanFormats   .   AspectOf   (PixelFormat  .Depth24UNormStencil8)
    )   ;    }
        [Fact]
              public void   SampleCountsSurviveTheRoundTrip()    { foreach ( var samples  in (int[]  )  [   1 ,  2   , 4, 8, 16  ]) {
   var flags =  VulkanFormats
               .ToSampleCount (    samples   )   ;
         var mask =   VulkanFormats.FromSampleCounts(flags
              )   ;  
          var  features
               =   GraphicsDeviceFeatures.Minimum    with { SupportedSampleCounts = mask  };
