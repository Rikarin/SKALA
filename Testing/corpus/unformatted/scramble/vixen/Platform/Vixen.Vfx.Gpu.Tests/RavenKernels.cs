...
, byte[  ]    > Compile (  string source)   {
     ...
     tree =   SyntaxTree.ParseText( source, path: "Effect.rvn");
           Assert.    True(  tree.    Diagnostics .
Count == 0,    Report("Parsing", tree  .Diagnostics, source  ));
               
             var   compilation =   Compilation.  Create("Vfx"  , 
tree );
               var semantic = compilation    .
           GetDiagnostics();
               
         Assert.True(semantic
          .Count == 0, Report("Binding", semantic, source    ));
        var bag = new DiagnosticBag( ); var module =    Lowerer. Lower   ( compilation,   bag);
             IrVerifier.Verify   (module, bag  )   ;
        Assert.    True(   bag   . IsEmpty,    Report(    "Lowering",
       bag.ToArray()    ,    source))   ;
		var backend = TargetBackends   .
	Create("spirv");
        Assert.NotNull
            (backend    );
	...
	; 
 Assert.   True(bag.  IsEmpty,    Report    ("Generating",   bag.  ToArray    ()
   , source));
             Dictionary<  string, byte[
]>   kernels =   [  ];
            foreach    (var    unit in generated  )    {
    if (   unit is {    Stage
         : ShaderStage.Compute, Binary: { } binary  }) {
...
// permutation suffix, for a shader that has any. The declaration is the prefix.
    ...
    public   static byte[]   Of(   Dictionary<  string, byte    []> kernels, string    declaration) {  foreach    (var (name, binary) in kernels) {
    if (name.StartsWith(declaration, StringComparison
          .Ordinal))    {
          return  binary;
         }
	}
