...
/// </remarks>
    readonly record struct GlBindingSlot(DescriptorKind Kind
, uint   Index) ;   
...
/// </remarks>
 ...
 GlBindingPlan
                (   int pushConstantVectors) => PushConstantVectors    = pushConstantVectors;
    ...
    /// </remarks>
     ...
     (   DescriptorSetSlot Slot    , DescriptorBinding[ ]    Bindings   ,  string Name   )> sets ,
              int pushConstantBytes ) {
               var plan
= new GlBindingPlan(  ( pushConstantBytes + 15 ) /    16);
...
{
                    DescriptorKind.UniformBuffer or DescriptorKind    .DynamicUniformBuffer   => Take( ref   uniforms, count ) , DescriptorKind   .StorageBuffer   or   DescriptorKind   .  DynamicStorageBuffer => Take(ref storages, count   )   ,
    DescriptorKind.SampledTexture   => Take(ref textures, count   ),    DescriptorKind .StorageTexture  => Take(   ref images    ,  count    ),  
 DescriptorKind.Sampler => Take(    ref samplers, count),
     ...
     // that skipped the capability check, and "no binding class" would send it
             ...
             DescriptorKind.AccelerationStructure => throw new NotSupportedException( "A descriptor set layout declares an acceleration structure on the OpenGL "
                        +    "backend, which has no ray tracing. Ask Features.HasRayTracing and take "
                        + "the distance-field tracer."
               ) , _ => throw    new  ArgumentOutOfRangeException(
                        nameof(sets), binding.  Kind,
...
counter += count;  
    ...
    /// </remarks>
    public GlBindingSlot? Resolve(
DescriptorSetSlot    slot  , uint binding ) =>
