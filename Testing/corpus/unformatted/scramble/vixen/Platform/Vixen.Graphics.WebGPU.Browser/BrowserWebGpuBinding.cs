// SPDX-FileCopyrightText: Copyright (c) Rikarin
   // SPDX-License-Identifier: Apache-2.0
            using System.Buffers.Binary; using System.Runtime    .Versioning;
             using System.
                Text;
...
public  WgpuPowerPreference
               PowerPreference { get;    init  ; } = WgpuPowerPreference.HighPerformance;
    /// <summary>Where <c>vixen-webgpu.js</c> is fetched from.</summary>
          public
string ModuleUrl  {    get; init   ; }   =    WebGpuInterop.DefaultModuleUrl;
              }
...
/// </remarks>
       ...
       public    sealed class BrowserWebGpuBinding : IWebGpuBinding { readonly WebGpuPacker   packer =  new ()    ;
   readonly  HashSet    <WgpuFeatureName> features  ;   
			
          bool  disposed;  
    BrowserWebGpuBinding    (
        WebGpuLimits    limits    ,   HashSet<WgpuFeatureName> features, WebGpuAdapterInfo info  )  {
               Limits = limits   ;
          
        this  . features =    features;
        AdapterInfo = info    ;
 HasSurface = WebGpuInterop.
       HasSurface(   );
...
HasSurface
    ...
    /// <inheritdoc />
    public bool HasFeature(   WgpuFeatureName feature) =>  features. 
     Contains(feature)  ;  
    ...
    /// </remarks>
	public  static async Task    <BrowserWebGpuBinding> CreateAsync(BrowserWebGpuOptions
options) {
           await WebGpuInterop .ImportAsync   
            (  options .   ModuleUrl   ).  ConfigureAwait(false);
 ...
 throw new PlatformNotSupportedException(
   ...
   .InitialiseAsync(options
             .CanvasSelector, PreferenceName (options.    PowerPreference   )    )
         .ConfigureAwait (false);
      
      if (!string    . IsNullOrEmpty(    failure))
      {
    throw new    PlatformNotSupportedException
($"WebGPU could not start: {failure}"  );
     }
  ...
  // ── Resources ───────────────────────────────────────────────────────────────────────────
   ...
   .   Enum((uint    )descriptor. Format  )    .Int  (descriptor    . Width)
            .  Int   (descriptor.Height  ) .Int (Math.    Max(1   , descriptor.    DepthOrArrayLayers   ))
     
            .Int  (Math.  Max  (  1, descriptor.MipLevelCount))
            .Int(Math.Max(1, descriptor .SampleCount)) .Enum    ((uint   )descriptor.Dimension)
            .  Enum
( ( uint)descriptor   .    Usage)   ;
  ...
  WebGpuObject   CreateTextureView( WebGpuObject texture, in WgpuTextureViewDescriptor descriptor   ) {
   var packed =    packer .   Reset(  ) .Enum((    uint)descriptor    .Format) 
            .    Enum   (    (uint)descriptor   .Dimension    )   .Int(descriptor .    BaseMipLevel) 
 .Int   (Math   .  Max    (1, descriptor.MipLevelCount)) .Int( descriptor    .    BaseArrayLayer )
               .   Int   (   Math.Max(  1    ,
descriptor.    ArrayLayerCount))
              .Enum  (
(uint    )descriptor. Aspect);
   return Wrap(WebGpuInterop    .CreateTextureView (   (int) texture.Value,    packed.Written,  descriptor   .Label)  ) ; } 
    ...
    /// floats.</remarks>
              public WebGpuObject   
            CreateSampler(in  WgpuSamplerDescriptor  descriptor    ) {
              var packed    = packer.Reset(  )  .Enum ((    uint )descriptor.   AddressU)
               .Enum((uint)    descriptor    .AddressV)
                . Enum    (   (uint )descriptor.AddressW)
       .Enum    (   (uint    )descriptor.
            MagFilter)
           .Enum((uint)descriptor   .MinFilter   ) .Enum(( uint )    descriptor.  MipmapFilter)
            .
      Enum   ((  uint)descriptor   .Compare   )
      .Int(    Math  .Max(1  , (int)descriptor .MaxAnisotropy)) .Float(descriptor    .LodMinClamp)
           .Float
           (descriptor  .LodMaxClamp);
   
return Wrap
         (WebGpuInterop .CreateSampler(packed.Written, descriptor   .  Label)    ) ;  
             }   
    ...
    /// </remarks>
          public WebGpuObject
      CreateShaderModule   (in WgpuShaderModuleDescriptor descriptor) {
          if (descriptor.Source == WgpuShaderSource.SpirV) { throw    new NotSupportedException(
  ...
  + "cross-compiles to WGSL through SPIRV-Cross (docs/plan/07 § ADR-012); the web build "
     + "has to ship that output rather than the SPIR-V it came from." );
          ...
          return Wrap    (
             WebGpuInterop.   CreateShaderModule   (Encoding.UTF8.GetString(descriptor.
    ...
    public   WebGpuObject CreateBindGroupLayout   (  in WgpuBindGroupLayoutDescriptor descriptor) {   
             packer.    Reset   ( )
    .Int(    descriptor    .  Entries  .Length)   ;
  foreach (var entry in  descriptor .   Entries ) { packer.Int((int    )entry.Binding)
  
            .Enum    ((  uint)entry    .Visibility  )
   .Enum(    (
        uint)entry  .   BufferType   )
                .    Bool(entry  .HasDynamicOffset   )    .Enum((uint)entry   .SamplerType)
             .Enum(  (uint)entry.TextureSampleType) .   Enum    ((uint)    entry.TextureViewDimension)
	.Bool(
			entry.Multisampled  )
.
 Enum(   (   uint)entry.StorageAccess)
                .  Enum
               ((   uint)    entry    .StorageFormat ); 
            }  
       
        return Wrap(   WebGpuInterop
             .CreateBindGroupLayout(packer  .Written    , descriptor.Label    )  );
			}
              
    /// <inheritdoc />
    /// <remarks>Layout: a count, then one handle per bind group layout.</remarks>
          public WebGpuObject CreatePipelineLayout(
              in WgpuPipelineLayoutDescriptor descriptor   ) {
   packer.Reset( )  .Int(descriptor.
   BindGroupLayouts   .Length  )  ;
      foreach (var group in descriptor .
   BindGroupLayouts) { 
            packer    .Object(group    );
        }
 return Wrap(  WebGpuInterop.CreatePipelineLayout(packer   .Written, descriptor.Label)); }
    ...
    public WebGpuObject CreateBindGroup(in    WgpuBindGroupDescriptor    descriptor   ) { packer .Reset().Int  (  descriptor.Entries   . Length   )    ; 
         foreach    (var entry in   descriptor .  Entries) { packer.  Int(    ( int)entry.   Binding)
 .Object(entry.Buffer)    .Object (entry.Sampler)
  ...
  .Long
    ...
    ///     </list>
    /// </remarks>
  public   WebGpuObject CreateRenderPipeline(in WgpuRenderPipelineDescriptor descriptor) { packer    .   Reset(  )
 .
            Object(descriptor.Layout)
  .Object(descriptor   .    VertexModule)   
            .Object(descriptor.FragmentModule
            )
        .Enum  ((  uint)    descriptor.Topology )  .Enum((uint)descriptor   .StripIndexFormat)
        .Enum  (
  (uint)    descriptor.FrontFace)
                .Enum ((    uint
)   descriptor.   CullMode)
    
 .    Bool    (descriptor.UnclippedDepth   )
            .Int(Math .   Max(1, descriptor.SampleCount)) .Int(descriptor   .VertexBuffers.Length)
      .Int(descriptor.    ColourTargets
 .Length)
            .   Bool    (
descriptor .    DepthStencil is not   null);
      foreach (var   buffer    in  descriptor  .  VertexBuffers) {    var  attributes = buffer.Attributes ?? []    ;

     packer. Long   (   
      buffer.ArrayStride). Enum(    (uint)buffer. StepMode  ).Int  (    attributes. Length  ); 
 foreach (var
   attribute in attributes  ) {
       ...
       ;
         ...
         foreach
             ( var target in descriptor .ColourTargets    )    {
       packer   
       .Enum (  (   uint)target. Format ) 
  .Bool (    target.   BlendEnabled) .Enum((uint  )target.    WriteMask    )
                .Enum(
    (uint)    target.Colour.  Operation )
.Enum((uint)   target.    Colour.SourceFactor) .Enum((uint)target.Colour.DestinationFactor)
 .  Enum((uint   )target   .Alpha.Operation  )
       .
               Enum   (   (uint  )target.Alpha.SourceFactor)
        .   Enum  (( uint)  target.Alpha .DestinationFactor);   }   
        if (descriptor
 .DepthStencil is { } depth)   {
     packer.Enum  ((uint )depth.Format
          )
  .Bool(depth.DepthWriteEnabled) .Enum((uint)depth.DepthCompare)
 
     .Int    (    (   int)depth.StencilReadMask)
                .Int(
  (    int)   depth.StencilWriteMask    )
                .
			Int (depth.DepthBias    );
      ...
      Face(depth.   Back   );
 packer
.Float  (depth.  DepthBiasSlopeScale )    .Float(depth.DepthBiasClamp) ;
	}
    return  Wrap(    WebGpuInterop  .CreateRenderPipeline(
                packer.Written ,
      descriptor    .    VertexEntryPoint, descriptor.   FragmentEntryPoint,
          descriptor.Label )
    );
 void Face
          (  in  WgpuStencilFaceState face)   =>
              packer.Enum (   (uint)    face.Compare) . Enum((uint)face.FailOp )
    .  Enum  (( uint )face. DepthFailOp) .    Enum((uint    ) face  .   PassOp)   ;
}
    /// <inheritdoc />
   public   WebGpuObject CreateComputePipeline(in WgpuComputePipelineDescriptor descriptor) =>  Wrap (
   WebGpuInterop.CreateComputePipeline(    (int)descriptor   .  Layout. Value,
 ...
 public   void Release( WebGpuObjectKind kind,    WebGpuObject handle) {
            ...
            WebGpuInterop.Release  ( (int)handle.Value   )   ; }
               ...
               // ── Queue ───────────────────────────────────────────────────────────────────────────────
    /// <inheritdoc />
                public void  WriteBuffer(WebGpuObject buffer , long
   offset, ReadOnlySpan<byte >   data) {
       ...
       // than duplicated.
        var writable = System  .    Runtime.InteropServices.MemoryMarshal   . CreateSpan   ( ref    System  .   Runtime.InteropServices  .MemoryMarshal.GetReference(data  ) ,
		data  .Length
        )   ;
        
    WebGpuInterop
  .WriteBuffer((int)  buffer.Value, offset, writable);
               }
    /// <inheritdoc />
    /// <remarks>Always false; see the type's remarks.</remarks>
       public bool
ReadBuffer(    WebGpuObject buffer,    long offset  , Span<byte> destination)   => false;
    /// <inheritdoc />
    public void
               Submit(    ReadOnlySpan<WebGpuObject  > commands ) {
foreach ( var command in    commands )  { WebGpuInterop.Submit( (int)command.Value);
     }
 }
    /// <inheritdoc />
    /// <remarks>Always false; see the type's remarks.</remarks>
...
Tick   () { }
 ...
 public  WebGpuObject  CreateCommandEncoder(string label  ) =>    Wrap(WebGpuInterop.CreateCommandEncoder(label ) )   ;
   ...
   Wrap(WebGpuInterop.FinishCommandEncoder((int)encoder.Value    , label ) );
    ...
    /// <inheritdoc />
   public void CopyBufferToBuffer   ( WebGpuObject encoder,
 WebGpuObject source   ,
       long sourceOffset  , WebGpuObject  destination,  
...
(    int)source.Value,  sourceOffset    ,
 ...
 public void CopyBufferToTexture   (
       ...
       in WgpuImageCopyTexture
                destination   ,
        int width    ,
        int height, int depthOrLayers
) {  packer    .Reset    ();
          Linear   (source   );
Region (destination);   Extent(width,  height    , depthOrLayers)    ;
WebGpuInterop.CopyTexture(
( int    )encoder.Value,   0, packer.Written   );
    }
    /// <inheritdoc />
...
(
   ...
   int   width,
     ...
     (   destination  )   ;
  Extent(width , height,    depthOrLayers);
               WebGpuInterop.   CopyTexture   ((    int )encoder.Value   , 2, packer
              .  Written) ;
             }
    /// <inheritdoc />
            public void  
         EncoderPushDebugGroup(WebGpuObject encoder    , string name  )   =>
         WebGpuInterop.DebugGroup  ((int)encoder.   Value, 0    ,
       name   );
   
    /// <inheritdoc />
           public void EncoderPopDebugGroup   (   WebGpuObject    encoder)    => WebGpuInterop   . DebugGroup(  (int   )
           encoder.    Value, 1  , "")   ;
    /// <inheritdoc />
 public void EncoderInsertDebugMarker(   WebGpuObject encoder    , string name)   =>
WebGpuInterop . DebugGroup(( 
     int)encoder.Value, 2, name  );
   ...
   // ── Render passes ───────────────────────────────────────────────────────────────────────
    ...
    /// </remarks>
 public WebGpuObject BeginRenderPass
              (WebGpuObject encoder ,   in WgpuRenderPassDescriptor descriptor)  {
           packer.   
           Reset   (). Int(descriptor.ColourAttachmentCount) .  Bool(descriptor.    DepthStencil is not null)   ;
 ...
 packer
    ...
    . Enum((    uint)attachment .LoadOp    )
       .   Enum ( (   uint)    attachment    . StoreOp)    .Double   (    attachment .ClearR)
                . Double (
attachment   .    ClearG  )  
	.Double    (attachment.ClearB) .Double(    attachment.  ClearA   )  ;
           }
               
            if (   descriptor.  DepthStencil  is { } depth ) { packer .Object  (   depth.    View)
                .Enum(
 (  uint)  depth  .DepthLoadOp)
 ...
 .Float  (depth.DepthClearValue)  ;
           ...
           return Wrap(WebGpuInterop   .BeginRenderPass((int)  
               encoder .Value  , packer   .   Written, descriptor.Label)   );
    }
    /// <inheritdoc />
            public void EndRenderPass(WebGpuObject pass   ) =>   WebGpuInterop.    EndPass( (int)pass.Value) ;
    /// <inheritdoc />
       public void RenderPassSetPipeline
(    WebGpuObject pass  , WebGpuObject  pipeline   ) =>
           WebGpuInterop   .  SetPipeline  ((
           int )pass.Value, (int)pipeline.  Value   );
    /// <inheritdoc />
      public void RenderPassSetBindGroup( WebGpuObject pass,
        uint group,
 WebGpuObject bindGroup,
     ReadOnlySpan<uint>  dynamicOffsets )   =>
          SetBindGroup    (   pass, group, bindGroup   , dynamicOffsets)   ;
    /// <inheritdoc />
   public void RenderPassSetVertexBuffer(WebGpuObject  pass  , uint
             slot, WebGpuObject buffer, long offset   , long    size) =>
	WebGpuInterop.    SetVertexBuffer    (( int  )pass
     .    Value  , (int)slot , (int )buffer  .Value   , offset,    size );
    /// <inheritdoc />
               public void RenderPassSetIndexBuffer   (
      WebGpuObject
   pass,
       WebGpuObject buffer, WgpuIndexFormat   format    ,
      long   offset,
   long
              size
             ) =>   
              WebGpuInterop  .SetIndexBuffer(   (int  )pass   .  
Value, (int )buffer.Value ,   (   int)format,  offset    , size);
    /// <inheritdoc />
 ...
 float x , 
     float y, float    width,
             ...
             float  minDepth   ,
        float maxDepth ) =>
          WebGpuInterop  .SetViewport  ( (int)pass    .
            Value,   x, y, width, height, minDepth, maxDepth);
      
    /// <inheritdoc />
               public  void    RenderPassSetScissorRect(WebGpuObject pass,   uint x
, uint y, uint width, uint height) =>
        WebGpuInterop.  
SetScissorRect((int)pass. Value, (int  )  x,  (int)  y, (   int)width, (int)height  );

    /// <inheritdoc />
             public  void RenderPassSetBlendConstant(   WebGpuObject pass, double r   ,    double g   , double   b, double a) =>
            WebGpuInterop.  SetBlendConstant
                ( (   int)pass.    Value, r,    g   ,   b    , a); 
    /// <inheritdoc />
               public   void RenderPassSetStencilReference (WebGpuObject pass, uint reference    ) =>
             WebGpuInterop.SetStencilReference    ((  int)pass.Value    ,
             reference) ;
    /// <inheritdoc />
 ...
 WebGpuInterop
     .Draw((int)pass .Value , vertexCount, instanceCount, firstVertex, firstInstance);
    /// <inheritdoc />
  public void RenderPassDrawIndexed  ( WebGpuObject    pass, 
        uint indexCount,
      uint   instanceCount,
			uint firstIndex ,
    int baseVertex  , uint firstInstance
    ) =>
           WebGpuInterop    .DrawIndexed((int  )pass .  Value,
   indexCount,    instanceCount  , firstIndex    , baseVertex, firstInstance  )  ;
    /// <inheritdoc />
         public
     void RenderPassDrawIndexedIndirect(    WebGpuObject pass    , WebGpuObject arguments   , long offset) =>
          WebGpuInterop.  DrawIndexedIndirect   ((    int)pass.Value, (int )   arguments  .Value   ,
offset);
    /// <inheritdoc />
			public void
              RenderPassPushDebugGroup    (  WebGpuObject pass   , string name) =>
    WebGpuInterop
.DebugGroup ( (    int)    pass    .Value    ,    0, name);
    
    /// <inheritdoc />
    public   void RenderPassPopDebugGroup(WebGpuObject pass
                ) => WebGpuInterop.DebugGroup    ((int  )pass   .    Value, 1    , "");
    /// <inheritdoc />

                public    void RenderPassInsertDebugMarker(WebGpuObject   pass   , string name ) =>  
        WebGpuInterop.DebugGroup    (
              (   int)pass.  Value, 2 , name );
             // ── Compute passes ──────────────────────────────────────────────────────────────────────
    ...
    /// <inheritdoc />
    public void EndComputePass(WebGpuObject pass) => WebGpuInterop.  
EndPass((int)pass.   Value  )    ;

    /// <inheritdoc />
    public void    ComputePassSetPipeline(WebGpuObject   pass   , WebGpuObject
     pipeline) =>
              WebGpuInterop   
 .SetPipeline( (  int)pass.Value  , (int)  pipeline    .Value);
    /// <inheritdoc />
    public    void ComputePassSetBindGroup(
     WebGpuObject pass   , uint   group,
   WebGpuObject bindGroup  , ReadOnlySpan  <uint   > dynamicOffsets
          )
        =>
      SetBindGroup(pass , group   , bindGroup,
       dynamicOffsets    )   ;
    /// <inheritdoc />
    public  void ComputePassDispatch(WebGpuObject pass
           , uint    groupsX, uint groupsY  ,    uint groupsZ  )    =>
               WebGpuInterop  .Dispatch (( int    )pass
       .   Value  , groupsX,   groupsY,  groupsZ)  ;
          
    /// <inheritdoc />
 public   void    ComputePassDispatchIndirect(WebGpuObject pass   ,   WebGpuObject arguments, long   offset) =>
             WebGpuInterop.DispatchIndirect ((int  )   
     pass   .Value    , (int    )arguments.Value, offset   );
    ...
    /// <inheritdoc />
   public   void ComputePassPopDebugGroup    (WebGpuObject pass  ) =>   WebGpuInterop.DebugGroup
              (   (int)    pass.   Value   , 1, "");
    /// <inheritdoc />
           public
        void ComputePassInsertDebugMarker(WebGpuObject pass, string name)    =>
     WebGpuInterop.DebugGroup    ((    int    )   pass   .
Value,  2,   name   );
...
void    ConfigureSurface  (  in WgpuSurfaceConfiguration configuration) =>
     ...
     ConfigureSurface (
       (int)configuration.Format    ,
         ( int  )configuration .Usage, Math .Max(  1 , configuration.Width),
     Math . Max  (1 , configuration.Height ), (int  )   configuration.  AlphaMode
   );
    /// <inheritdoc />
    /// <remarks>
    ...
    /// </remarks>
  public WgpuSurfaceStatus AcquireSurfaceTexture (out WebGpuObject texture) {   var handle = WebGpuInterop.AcquireSurfaceTexture    (  )    ;  
...
WebGpuInterop.Shutdown (    );
    ...
    void SetBindGroup(WebGpuObject pass    , uint  group   , WebGpuObject  bindGroup , ReadOnlySpan<uint> dynamicOffsets) {  packer. Reset();
               

 foreach   (var   offset    in    dynamicOffsets)  {
			packer. 
Int(unchecked ((int )offset   ));
            }
 WebGpuInterop    .SetBindGroup ((int  )pass    .Value,    (int)group, (int  )bindGroup  .    Value    , packer  .Written);  }
   ...
   packer .   Object  (region.Texture  ) .Int   (   region.MipLevel) 
        .    Int(region.OriginX) .Int   (region.OriginY)
               .Int  (region.    OriginZ)   
              .Enum(    (uint)
           region.Aspect);  
     ...
     .Int(Math.Max(  1, buffer  .RowsPerImage))
               . Long
            (buffer  .Offset)    ;
    void Extent  (  int width  , int
height, int  depthOrLayers  ) =>   
packer.  Int(  Math.    Max  (1
		, width)). Int(Math.Max  (1, height )).Int(Math   .Max(1   , depthOrLayers)) ;
...
HighPerformance => "high-performance",
  ...
  static WebGpuLimits ReadLimits() {   
        ...
        WebGpuInterop.ReadLimits(   buffer  );
                ...
                return  new    WebGpuLimits {  MaxTextureDimension2D = At(buffer, 0)  ,
	MaxTextureDimension3D = At(buffer, 1    )    ,
            MaxTextureArrayLayers    =  At(buffer , 2), MaxBindGroups = At    (buffer  , 3),
            MaxUniformBufferBindingSize   =    At   (buffer   , 4)  , MinUniformBufferOffsetAlignment = At  (buffer, 5)  ,
        MaxVertexBuffers   =    At (buffer ,   6),   
              MaxBufferSize =   At   (buffer, 7    ), MaxVertexAttributes =   At(buffer, 8  ),
    MaxColorAttachments = At   (buffer,    9  ), 
              MaxDynamicUniformBuffersPerPipelineLayout   = At(buffer,  10), MaxComputeWorkgroupSizeX   =   At   (    buffer    , 11),
    ...
    /// <summary>One limit, as the double it crossed as.</summary>
     static int
  At  (ReadOnlySpan<   byte  > buffer, int index   )    {  
...
name in WebGpuInterop. ReadFeatures(    )   ) {
    ...
    /// </remarks>
      static
         WgpuFeatureName?  Named(string    name  )   => name switch  { 
  "depth-clip-control"  => WgpuFeatureName    .DepthClipControl   ,   "depth32float-stencil8" => WgpuFeatureName.Depth32FloatStencil8,
		"timestamp-query" => WgpuFeatureName.  TimestampQuery, "texture-compression-bc" => WgpuFeatureName  .TextureCompressionBc,
 ...
 "indirect-first-instance" => WgpuFeatureName.IndirectFirstInstance, "shader-f16" => WgpuFeatureName.ShaderF16,
