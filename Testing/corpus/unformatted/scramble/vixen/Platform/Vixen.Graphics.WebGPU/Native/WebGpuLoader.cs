// SPDX-FileCopyrightText: Copyright (c) Rikarin
    ...
    using   System .
		...
		using System.Runtime.
                InteropServices; 
 using  Silk  .NET.Core.    Contexts; using Silk. NET.WebGPU;
...
public static nint DevicePoll  {   get  ; private set;   }
    ...
    /// </remarks>
              static readonly
          string[  ]   Required = [
               "wgpuCreateInstance", "wgpuAdapterGetProperties",  
              "wgpuDeviceSetUncapturedErrorCallback", "wgpuSurfaceGetPreferredFormat"
];
    ...
    /// <param name="reason">Why it did not, when it did not.</param>
             ...
             bool   TryLoad(
        [NotNullWhen  (true)
	] out Silk.NET.WebGPU.WebGPU  ? api,
...
;
 ...
 if (failure    is not null   ) {
			api = null ;   
   reason = failure; return false;
          }  
            foreach (var library in  Libraries   ) {
 NativeLibraries .Describe (new(library, [   ],
     [.. Prefixes()]));
 }
           NativeLibraries.Register(typeof(Silk.NET.    WebGPU.WebGPU).    Assembly);
              
 foreach ( var candidate in    Candidates(  )) { if (   !    NativeLibrary.  TryLoad (candidate, out var    handle )) {
       
         continue ;
         }   
         
		var missing = Required
   .Where   (name =>  !  NativeLibrary .TryGetExport    (handle, name ,  out    _))   .ToArray   ();
      ...
      if (    missing.  Length  > 0    ) {  
			...
			// wrong entry points is the version problem, not a search problem, and trying
               // the next candidate would report "not found" for something that was found.
                    failure = TooNew(candidate,   missing  )   ; api = null;
           reason = failure;
      ...
      loaded   = FromHandle(
            ...
            ResolvedPath = candidate;
               DevicePoll =  NativeLibrary   .TryGetExport(handle    , "wgpuDevicePoll"   , out var poll) ? poll : 0; IsWgpuNative = DevicePoll  != 0;
            api = loaded    ;
...
readonly string[]    Libraries  = ["wgpu_native", "webgpu_dawn", "webgpu"   ];
    ...
    static Silk.NET .WebGPU
      ...
      new(
                new LamdaNativeContext(   name => NativeLibrary.  TryGetExport(handle    , name, out var   address) ? address : 0));
...
(   var candidate in    NativeLibraries.Candidates    (library   )    ) {
	...
	// shipped and was tested against is the failure this ordering exists to prevent.
		foreach (    var    library in Libraries) {   foreach (var name in NativeLibraryNames.For(library)) {
        yield
 return name;   
           }
        }
  ...
  (    OperatingSystem.IsMacOS    ()  ) {
            yield    return "/opt/homebrew/lib"; yield   return "/usr/local/lib"    ;
            yield break ; }
      if  (    OperatingSystem.IsLinux()) { yield  return "/usr/lib/x86_64-linux-gnu"    ;
    ...
    static   string  TooNew(  string path    , string[ ]    missing   ) =>
       ...
       +    "Silk.NET.WebGPU 2.23.0 still declares them, so a newer Dawn or wgpu-native is not " + "loadable here — calling one would be a null function pointer rather than an error. "
 ...
 "No WebGPU implementation could be loaded. Silk.NET.WebGPU is bindings only and no desktop "    + "operating system ships Dawn or wgpu-native, so one has to be provided: drop "
        + "libwgpu_native (or webgpu_dawn) beside the executable, into " + "runtimes/<rid>/native/, or somewhere on the loader's path — or point VIXEN_WEBGPU_PATH at "
                +
