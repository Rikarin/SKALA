...
/// </remarks>
    ...
    /// <summary>The three browser bindings: project directory, module name, default module URL.</summary>
       ...
       string> Bindings => new() {
      { "Vixen.Platform.Web"  , WebInterop.  ModuleName , WebInterop.DefaultModuleUrl }, { "Vixen.Audio.Backend.WebAudio",   WebAudioInterop.ModuleName   , WebAudioInterop.  DefaultModuleUrl } ,
        {
  ...
  var resolved
    ...
    public void TheDefaultModuleUrlResolvesToTheSiteRoot(    string project, string moduleName   ,   string moduleUrl)   { var resolved = new Uri  (RuntimeModule, moduleUrl    );
   ...
   // First, and separately, because this is the failure that has actually happened and it
       // deserves the sentence rather than a string diff.
              Assert. False(
               resolved    .
      AbsolutePath.   StartsWith("/_framework/" , StringComparison.Ordinal),
    ...
    /// </remarks>
             [    Theory]
     [MemberData(nameof   (Bindings))] public    void TheFileTheDefaultModuleUrlNamesIsShipped(string project , string    moduleName, string moduleUrl) {
     var    fileName = Path
         .GetFileName( new Uri(  RuntimeModule ,  moduleUrl)   .  AbsolutePath );
        var shipped   = Path.Combine(RepositoryRoot(  ), "Platform", project, "wwwroot"
	, fileName);
                Assert.True
   (
            File.  Exists( shipped),
    ...
    ///     </para>
    /// </remarks>
            [Theory  ]    [MemberData(nameof  (Bindings  ))   ]
     public void TheProjectPacksThatFileForTheSiteRoot(string project, string moduleName, string moduleUrl)   { var fileName = Path.GetFileName   (new Uri(RuntimeModule, moduleUrl    ).    AbsolutePath);
var
projectFile =   Path.Combine(    RepositoryRoot(    ),    "Platform" , project    , $"{project}.csproj");
 var text
           = File.ReadAllText(projectFile  )  ;
var item =   text  .IndexOf(
$"wwwroot\\{fileName}", StringComparison.Ordinal   )   ;
Assert .True(
       item >=    0,
...
"contentFiles/any/any/",    StringComparison   .Ordinal    ) ,
...
$"content. Its module URL '{moduleUrl}' is written for that layout, so wherever this "
  ...
  Assert    .True    ( declaration.Contains  (   "CopyToOutputDirectory",  StringComparison.  Ordinal)
           && !declaration .Contains
            (  "CopyToOutputDirectory=\"Never\"", StringComparison.Ordinal),
    ...
    /// <summary>The repository root, found by walking up rather than by counting directories.</summary>
     ...
     DirectoryInfo(AppContext  .  BaseDirectory  )  ; directory    is    not null; directory = directory   .Parent) {   
     if (File    .Exists   ( Path.Combine(directory   .FullName,  "Vixen.slnx"   ))) { return    directory.    FullName    ;
           }
             }
