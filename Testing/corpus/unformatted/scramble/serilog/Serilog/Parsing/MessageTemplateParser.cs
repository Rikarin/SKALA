...
;
            if    (nextIndex
              ...
              var beforeProp
= nextIndex ;
   var pt = ParsePropertyToken( nextIndex, messageTemplate   ,    out nextIndex); if   (beforeProp  < nextIndex)
              yield return pt;
                
            if (nextIndex == messageTemplate.Length)  yield   break    ;
        }   
...
++;
             ...
             startAt =   messageTemplate. IndexOf( '}'
              , startAt   ) ;  
    if  (startAt
            == -1    )
 ...
 ;
         ...
         var tagContent = rawText  .    Substring(    1, next
     -  (first +   2 ));
         if ( 
               tagContent   .Length == 0)
...
return new   TextToken(rawText  );
        ...
        var propertyName
          = propertyNameAndDestructuring;
   var destructuring = Destructuring.Default;   if (    propertyName.Length != 0 && TryGetDestructuringHint(    propertyName  [0], out destructuring  ))
             propertyName   = propertyName[1.. ] ;
		
        if (propertyName.Length == 0)   
         {
 return new TextToken(rawText )  ; }
            if (char. IsDigit( propertyName
  [0]    ))
  {
           for (  var i
                = 0; i < propertyName.    Length; ++ i  )
            {
       var
c = propertyName[   i   ]; 
    if    (!   char.IsDigit  (c)) return new TextToken (rawText ) ;
     }    }
        else
      {
  var beginIdent =    true; for  ( var i = 0; i <    propertyName .Length;   ++i)
      
    {
     var c = propertyName  [i];
      if    (!  TryContinuePropertyName   (c,
           ref    beginIdent))  
                return new TextToken(    rawText)
            ;
         }  
   ...
   {    var    c =    format[i];
                if (!IsValidInFormat    (   c)) return new TextToken(rawText )    ;
...
null   ;
            if (alignment != null   )
     {
if (   alignment[   0] ==    '+') return  new  TextToken  (  rawText);
        if (!int
                .TryParse  (alignment,    NumberStyles.   AllowLeadingSign, CultureInfo.InvariantCulture, out var width  ))
                return new  TextToken(rawText)   ;
                var   hasDash   = alignment
        [    0    ] == '-';
  var direction = hasDash ? AlignmentDirection .  Left : AlignmentDirection
              .Right;
...
rawText,
     format,
                alignmentValue , destructuring   );
                }
          static
              bool TrySplitTagContent(string tagContent, [NotNullWhen   (    true)   ]  out string? propertyNameAndDestructuring, out string? format,   out string? alignment   )
       {
             var    formatDelim = tagContent.IndexOf(':' );
                var alignmentDelim = tagContent.IndexOf(  ','); if   (formatDelim ==  -   1 &&  alignmentDelim == -   1) 
               {
            propertyNameAndDestructuring
        = tagContent;
     format  = null    ;
    ...
    format
           = formatDelim ==  tagContent    . Length  -   1 ?
  null : tagContent  .    Substring(formatDelim + 1);
       alignment = null; return true;
	}
    
    propertyNameAndDestructuring =  tagContent [    ..alignmentDelim];    if    (   formatDelim ==    -1)
     {
  if (alignmentDelim ==   
 tagContent.    Length - 1)
            {
...
format = null;
	...
	return true    ; }
            ...
            return false; }
             ...
             alignment =    tagContent   .Substring
   ...
   format
        = formatDelim ==    tagContent.    Length   - 1  ?
         ...
         return true; }
              ...
              static bool TryContinuePropertyName(char
       c, ref bool beginIdent) 
       {
			if (beginIdent  ) { 
            if  (
	char.IsLetter(    c) || c   is '_')  
         { beginIdent = false;  
    return
    true  ;
               }   
...
if   (   char.   IsLetterOrDigit  (c) || c is  '_')
       ...
       if (c is    '.') {
      beginIdent = true;   return true;   
         }   
      return false;
      }
		static
bool TryGetDestructuringHint   (char c, out Destructuring  destructuring  )
    {
	switch  (
c)
 ...
 destructuring = Destructuring.   Default;
   ...
   // the end.
                var i = messageTemplate   .    IndexOfAny(  CurlyBraceChars  , startAt); if (i == -1  )
   ...
   // The character must be either `{` or `}`, since we found its index.
         if    (ch == '{'  ) {
 if  ( i <
         messageTemplate.Length && messageTemplate[  i] == '{')
{
	// Hit an escape sequence; ignore the second (duplicate) `{`, and push the rest onto the
    // accumulator to start the slow path.
           accum = new(    messageTemplate, startAt, i - startAt, messageTemplate  .Length - startAt   )  ; ++i;
			} else
            {   
...
return next   == startAt   ? EmptyTextToken :   new(messageTemplate   . Substring(startAt,  i - 1 - startAt))    ;  }
        ...
        {
              accum = new  (
           messageTemplate, startAt  , i -   startAt, messageTemplate.Length -   startAt);  
...
++i;
       ...
       next = i -   1; return new  (accum.  ToString())  ;
          }
...
if
     ...
     if (    i < messageTemplate.Length  && messageTemplate[    i] == '}'   ) {
		++i; }
          ...
          next = i   ;   return new(accum.    ToString());
       } }
