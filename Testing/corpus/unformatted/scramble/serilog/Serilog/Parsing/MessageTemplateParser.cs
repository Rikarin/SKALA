...
== -1)
 ...
 .Length   == 0 )
         ...
         if (!TrySplitTagContent    (    tagContent   , out var   propertyNameAndDestructuring, out var format  , out
               var alignment    ))
  return new TextToken(   rawText);
	...
	if (format   !=  null)   
     ...
     { var    c =    format    [i    ];
                ...
                (alignment   != null)
		{
			...
			if  (!int.TryParse
(alignment,  NumberStyles.AllowLeadingSign    , CultureInfo   .InvariantCulture, out var width))
            return new  TextToken(rawText  );
...
return false; }
              ...
              formatDelim -   alignmentDelim - 1  )  ;
        format    =    formatDelim ==  tagContent.
		Length - 1 ?
             null    :
  ...
  beginIdent = true;    return true;  
       ...
       static bool   TryGetDestructuringHint(char c,
