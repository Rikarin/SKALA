...
public class SequenceValue  : LogEventPropertyValue
    ...
    /// </summary>
   public static
           SequenceValue Empty { get; } =  new( Array.Empty<LogEventPropertyValue>()   )  ;
    /// <summary>
...
_elements  =    elements   ; }
 ...
 public IReadOnlyList<    LogEventPropertyValue>  Elements => _elements  ;
    ...
    public
override void   Render(TextWriter    output  ,   string    ? format = null    , IFormatProvider? formatProvider = null)
              { Guard.AgainstNull(   output)   ;
      
       output .Write ( '[')    ; var allButLast =    _elements    .Length -   1;
        for    (var i = 0   ; i
         < allButLast; ++  i)
...
( ", ");
     ...
     if
         (_elements. Length > 0)
       _elements
[_elements.   Length  - 1]  . Render(output, format   , formatProvider);
        output.Write(']'); }
