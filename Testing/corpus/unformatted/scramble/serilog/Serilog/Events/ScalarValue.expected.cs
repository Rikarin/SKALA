...
public class ScalarValue : LogEventPropertyValue {
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="output"/> is <code>null</code></exception>
    internal static
        void Render(object? value, TextWriter output, string? format = null, IFormatProvider? formatProvider = null) {
        Guard
            .AgainstNull(output);
        if (value == null) {
            output.Write("null");
...
#if FEATURE_SPAN
    internal static void Render<T>(T value, TextWriter output, string? format = null, IFormatProvider? formatProvider =
 null) where T : struct, IFormattable, ISpanFormattable
    {
    ...
    {
        Span<char> buffer =
 stackalloc char[36]; // large enough to fit ISO 8601 timestamp (DateTimeOffset.ToString("O"))
