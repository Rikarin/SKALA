       ...
       readonly struct    EventProperty
{
   ...
   Guard.AgainstNull    (value) ; LogEventProperty.EnsureValidName(name);
...
( out string name, out LogEventPropertyValue  value)
   {
       name =  Name;
     value = Value;   
              }  
   ...
   public bool Equals (EventProperty other
    ...
    {
        return string
.Equals  (  Name    , other.Name) && Equals(Value, other.Value) ;
             }
    /// <inheritdoc />
      public  override
      int GetHashCode()
    {   
        unchecked {
            return ((Name != null   ? Name.  GetHashCode    ()
	: 0) *   397) ^ (   Value !=    null ? Value.GetHashCode() : 0);
               } }
  }
