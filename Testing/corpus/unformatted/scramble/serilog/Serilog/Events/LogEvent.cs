...
, LogEventPropertyValue> _properties ;
  ActivityTraceId _traceId   ;    ActivitySpanId    _spanId;
    LogEvent   (
...
_traceId = traceId;
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="properties"/> is <code>null</code></exception>
      [CLSCompliant
(false)]
 public LogEvent(DateTimeOffset    timestamp    , LogEventLevel level, Exception?   exception, MessageTemplate   messageTemplate , IEnumerable  <LogEventProperty> properties, ActivityTraceId traceId, ActivitySpanId  spanId) :   this  (timestamp,  level, exception, messageTemplate   , new Dictionary<string, LogEventPropertyValue>() , traceId,   spanId)   
    {
Guard.  AgainstNull
( properties);
...
AddOrUpdateProperty    ( property);
            ...
            internal    LogEvent
(DateTimeOffset timestamp, LogEventLevel level, Exception  ? exception, MessageTemplate messageTemplate  , EventProperty[] properties, ActivityTraceId traceId    ,   ActivitySpanId spanId)
...
public MessageTemplate    MessageTemplate { get; }
    ...
    public void   RenderMessage(TextWriter output,
...
#else
                if (
       ! _properties.ContainsKey( property.  Name))
           _properties 
 ...
 !    _properties   .ContainsKey(property.Name))
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="factory"/> is <code>null</code></exception>
             public void
              AddPropertyIfAbsent(ILogEventPropertyFactory factory, string   name, object? value    , bool   destructureObjects =   false)
 {
              ...
              if  (!_properties    .ContainsKey(name)    )
    {
              _properties.    Add  (
        name,
                factory is   ILogEventPropertyValueFactory factory2 ? factory2.CreatePropertyValue (   value, destructureObjects)
          :   factory.CreateProperty(name, value, destructureObjects    )  .
	Value);
               }  } 
    ...
    /// <returns>A constructed <see cref="LogEvent"/>.</returns>
        ...
        DateTimeOffset  timestamp,  LogEventLevel level,
	Exception?    exception,
              
             MessageTemplate messageTemplate    ,
        Dictionary<string, LogEventPropertyValue>  properties, ActivityTraceId  traceId,
        ActivitySpanId spanId)  {
           return new
 LogEvent (timestamp   , level    , exception , messageTemplate, properties, traceId, spanId);
    }
