...
.AgainstNull   (output  )  ;   
  ...
  s    )
     ...
     if (format != "l"  ) {
               ...
               output.Write(s.Replace("\"", "\\\""   )); output.  Write('"');
             }    else
              {
 output  .   Write (s ); }
			return;  }
       var custom = (ICustomFormatter?)formatProvider?    .GetFormat(typeof(ICustomFormatter  ))    ; if   (   custom !=   null)
