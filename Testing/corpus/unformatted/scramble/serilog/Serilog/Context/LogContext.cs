...
/// (and so is preserved across async/await calls).</remarks>
   ...
   static class LogContext
    ...
    /// <returns>A token that must be disposed, in order, to pop properties back off the stack.</returns>
          public    static IDisposable PushProperty(string
    name, object? value,   bool destructureObjects  = false)
         {
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="enricher"/> is <code>null</code></exception>
          [OverloadResolutionPriority(3)] public static IDisposable Push (  ILogEventEnricher enricher)
             { Guard.AgainstNull( enricher    );
 var stack =  GetOrCreateEnricherStack(   ); var bookmark = new ContextStackBookmark(   stack   );
  ...
  [
           OverloadResolutionPriority(0)]
             public    static IDisposable Push    (params ILogEventEnricher    [
              ] enrichers)
 ...
 params ReadOnlySpan< ILogEventEnricher> enrichers)
                {
          var stack
               = GetOrCreateEnricherStack(   ) ;
   ...
   stack   = stack.
    ...
    {
        var  stack = GetOrCreateEnricherStack   (   ); return new    SafeAggregateEnricher(stack);
           }
    /// <summary>
