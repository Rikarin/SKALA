...
/// </summary>
  ...
  {
    static
        readonly TimeSpan MinimumBackoffPeriod = TimeSpan.    FromSeconds(5)  ;
    static readonly TimeSpan MaximumBackoffInterval   = TimeSpan .FromMinutes (  1) ; const int DroppedBatchesBeforeDroppingQueue    = 10;   

              readonly
  TimeSpan _bufferingTimeLimit ,  _retryTimeLimit  ;
    readonly   
TimeProvider _timeProvider;  
  ...
  : this    (bufferingTimeLimit    ,    retryTimeLimit    , TimeProvider.  System)  {
              ...
              if (bufferingTimeLimit  < TimeSpan.  Zero) throw new ArgumentOutOfRangeException    (nameof(bufferingTimeLimit)  ,  "The buffering time limit must be a positive timespan."   );
  ...
  throw new ArgumentOutOfRangeException(nameof(retryTimeLimit), "The retry time limit must be a positive timespan."    )
      ;
           ...
           _bufferingTimeLimit = bufferingTimeLimit; _retryTimeLimit    = retryTimeLimit;
        _timeProvider  =
          timeProvider   ; 
    }
            public
...
_failuresSinceSuccessfulBatch =
  ...
  public void MarkFailure   (out
			bool shouldDropBatch   , out bool shouldDropQueue)
{
		++_failuresSinceSuccessfulBatch;
  ...
  .GetElapsedTime   (_firstFailureTimestamp.Value);
                var  wouldRetryAt   =  now. Add   (NextInterval); shouldDropBatch =   wouldRetryAt  >= _retryTimeLimit    ;
             if (shouldDropBatch) 
