...
#endif
           { 
    readonly ILogger _logger   ; readonly bool _attemptDispose;
       
             public SecondaryLoggerSink(ILogger logger  , bool attemptDispose = false) {
            _logger   = Guard   .AgainstNull( logger )    ; _attemptDispose    = attemptDispose;
    }
        ...
        var copy = logEvent.Copy    ()   ; _logger    .Write(copy)    ;
               }
   public   void  Dispose()
	{
        if (   !_attemptDispose)
