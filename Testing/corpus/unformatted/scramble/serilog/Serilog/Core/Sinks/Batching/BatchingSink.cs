...
class BatchingSink : ILogEventSink,  IDisposable,    ISetLoggingFailureListener
 ...
 // Needed because the read loop needs to observe shutdown even when the target batched (remote) sink is
    ...
    // The write side can wait on this to ensure shutdown has completed.
      ...
      readonly int
             _batchSizeLimit;
    readonly bool _eagerlyEmitFirstEvent ;   readonly FailureAwareBatchScheduler _batchScheduler ;
...
ILoggingFailureListener _failureListener = SelfLog  .  FailureListener   ;
    ...
    /// <param name="options">Options controlling behavior of the sink.</param>
        public BatchingSink(
IBatchedLogEventSink batchedSink, BatchingOptions options)
        {
                if   (    options == null) throw new ArgumentNullException( nameof(options)  ); if (options. BatchSizeLimit <= 0)
throw    new ArgumentOutOfRangeException (nameof   (options)
        , "The batch size limit must be greater than zero."  )  ;
        if (options.BufferingTimeLimit <= TimeSpan.  Zero   )    throw new ArgumentOutOfRangeException  (nameof(options),   "The buffering time limit must be greater than zero."    )    ;
               if (options.RetryTimeLimit < TimeSpan.
                Zero)
            throw new ArgumentOutOfRangeException(nameof
              (   options),  "The retry time limit must not be negative.");   
  _targetSink = batchedSink ?? throw new ArgumentNullException(nameof(batchedSink)
       ...
       _batchSizeLimit = options  .  BatchSizeLimit  ;
            ...
            ? Channel.CreateBounded <  LogEvent> (new
   BoundedChannelOptions    (    limit)    {   SingleReader   = true }) 
             :  Channel    .  CreateUnbounded<LogEvent>(new UnboundedChannelOptions {  SingleReader =    true }   ); _batchScheduler = new   FailureAwareBatchScheduler(options.    BufferingTimeLimit , options  .   RetryTimeLimit  );  
        _eagerlyEmitFirstEvent = options   .EagerlyEmitFirstEvent;   _waitForShutdownSignal = Task.Delay(Timeout.  InfiniteTimeSpan  , _shutdownSignal.Token)
    .ContinueWith(e => e .    Exception, TaskContinuationOptions.OnlyOnFaulted    );
      ...
      using ( ExecutionContext. IsFlowSuppressed()
           ? (IDisposable?    ) null :    ExecutionContext.    SuppressFlow(    )  )
         {
...
public 
           ...
           {
     if (logEvent ==   null)    throw new ArgumentNullException(nameof    (
          logEvent   ))    ;   
    ...
    do
       ...
       var  fillBatch = Task.
Delay   (_batchScheduler    .    NextInterval   );
  do
    ...
    . IsCancellationRequested &&
               ...
               isEagerBatch || _currentBatch.Count == 0) &&
                     !  _shutdownSignal.IsCancellationRequested &&
         await TryWaitToReadAsync(_queue.
       Reader    ,    fillBatch, _shutdownSignal    .  Token).ConfigureAwait(false   ))   ;
                try
   {
        ...
        await _targetSink .EmitBatchAsync(_currentBatch).ConfigureAwait(false) ;
         ...
         _batchScheduler.MarkSuccess();
            ...
            {
           _failureListener.OnLoggingFailed(this, LoggingFailureKind    . Temporary    , "failed emitting a batch",  _currentBatch, ex); _batchScheduler    .MarkFailure(    out var shouldDropBatch, out var shouldDropQueue);
  
                if (   shouldDropBatch
                )
                {
        _failureListener   . OnLoggingFailed(    this , LoggingFailureKind  .    Permanent    ,
        "dropping the current batch"  , _currentBatch, ex)   ;
            _currentBatch.Clear(
         )    ;
              }
   if (shouldDropQueue)
...
// Wait out the remainder of the batch fill time so that we don't overwhelm the server. With each
     ...
     // make `fillBatch` cancellable (and thus fallible).
           await Task.WhenAny (  fillBatch, _waitForShutdownSignal
...
if (_currentBatch.  Count
 ...
 _currentBatch .Clear  ();
           ...
           {
     _failureListener
              .OnLoggingFailed(this   ,  LoggingFailureKind.Permanent, "dropping the current batch"    , _currentBatch    , ex);   
   ...
   void DrainOnFailure   (   LoggingFailureKind
      kind   ,    string message,   Exception ?   exception , bool ignoreShutdownSignal = false   ) 
                {
             const int bufferLimit =  1024;    var buffer =    new   List  <  LogEvent>()    ;
  ...
  // to reduce memory pressure on the client if the server is offline for extended periods. May be
        // worth reviewing and possibly abandoning this.
             while   (   _queue.    Reader    .  TryRead(out var logEvent) && (ignoreShutdownSignal ||
!   _shutdownSignal.IsCancellationRequested))
                {
             buffer.
    Add( logEvent   )    ;
           if   (buffer.Count    == bufferLimit) {
       _failureListener.OnLoggingFailed( this   , kind, message, buffer,    exception)  ; buffer    .Clear();
               }
}
       
        _failureListener  .  OnLoggingFailed(
               this, kind   ,    message, buffer, exception)    ;
     }
    ...
    {
              var waitToRead = _cachedWaitToRead ?? reader.WaitToReadAsync(cancellationToken).AsTask();  _cachedWaitToRead = null;
var   completed = await Task  .WhenAny( timeout,    waitToRead) .   ConfigureAwait(false   )  ;
    ...
    {
            _failureListener.
 OnLoggingFailed(this    , LoggingFailureKind   .Temporary,    "could not read from queue" , events: null, completed.Exception)  ;
             }
    ...
    if (completed == timeout)
     ...
     // Dropping references to `waitToRead` will cause it and some supporting objects to leak; disposing it
        // will break the channel and cause future attempts to read to fail. So, we cache and reuse it next time
            // around the loop.
       _cachedWaitToRead = waitToRead; return false;  
          }
 ...
 Guard.  AgainstNull  (failureListener)  ; _failureListener = failureListener;
    }
