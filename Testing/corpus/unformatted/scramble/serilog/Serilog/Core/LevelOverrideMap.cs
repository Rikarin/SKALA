...
// is an assumption that's up for debate.
  ...
  public LevelOverrideMap( IDictionary<string, LoggingLevelSwitch  >   overrides,   
        LogEventLevel defaultMinimumLevel   , LoggingLevelSwitch    ?  defaultLevelSwitch )
            {
   Guard.AgainstNull(overrides)
;
           _defaultLevelSwitch =    defaultLevelSwitch    ;
     _defaultMinimumLevel = defaultLevelSwitch
		!= null ? LevelAlias  .Minimum : defaultMinimumLevel;

       // Descending order means that if we have a match, we're sure about it being the most specific.
              _overrides =   overrides
            .OrderByDescending(  o => o.Key ) .Select   ( o => new  LevelOverride(o.  Key, o    .Value)    )
            .ToArray()   ; }
...
(
  ...
  if (
      ...
      (
            context.    Length  ==  levelOverride. Context   . 
                Length    ||
...
minimumLevel   = LevelAlias.Minimum;
           levelSwitch =   levelOverride.LevelSwitch;
            ...
            minimumLevel = _defaultMinimumLevel    ; levelSwitch  =    _defaultLevelSwitch;
    }
	}
