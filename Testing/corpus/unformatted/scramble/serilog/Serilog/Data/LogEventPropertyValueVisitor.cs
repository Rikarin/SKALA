...
public abstract class LogEventPropertyValueVisitor<TState, TResult> {
    ...
    protected virtual TResult Visit
        (TState state, LogEventPropertyValue value) {
        Guard.AgainstNull
            (value);

        if (value is ScalarValue sv) return VisitScalarValue(state, sv);
        if (value is
            SequenceValue seqv)
            return VisitSequenceValue(state, seqv);

        if (value is StructureValue strv) return VisitStructureValue(state, strv);

        if (value
            is DictionaryValue dictv)
    ...
    protected abstract TResult VisitStructureValue
        (TState state, StructureValue structure);
    ...
    protected
        abstract TResult VisitDictionaryValue(TState state, DictionaryValue dictionary);

    /// <summary>
    ///     Visit a value of an unsupported type. Always throws <see cref="NotSupportedException" />, when is not
    ///     overridden.
    /// </summary>
    /// <param name="state">Operation state.</param>
    /// <param name="value">The value to visit.</param>
