...
/// <typeparam name="TState"></typeparam>
  public abstract
            class LogEventPropertyValueRewriter<TState> : LogEventPropertyValueVisitor  <TState   , LogEventPropertyValue    >   
     {
    /// <summary>
    ...
    protected override LogEventPropertyValue VisitScalarValue (TState
  state, ScalarValue scalar)
           {
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="sequence"/> is <code>null</code></exception>
                protected    override LogEventPropertyValue VisitSequenceValue(TState    state,    SequenceValue sequence)
     {
             Guard   .   AgainstNull(sequence);
              ...
              for (var   i = 0
               ; i    < sequence.Elements.    Count; ++i)
    {
                var original = 
  sequence.Elements   [i   ];
                if (!
              ReferenceEquals    (    original, Visit(state , original  ))    )
            {
...
= sequence.Elements[ j];
       ...
       for (var k = i
           ...
           {
                    contents
 [k  ] = Visit(state , sequence  .Elements[k]);
           }  
           
                return
            new   SequenceValue(contents)  ;
   } } 
        return  sequence; }
    /// <summary>
    ...
    for (var  i = 0   ; i  < structure   . Properties.   Count; ++i    )   {
              var original =   structure   .Properties[i];   if (  !    ReferenceEquals(original.Value,  Visit(state, original.Value )) )
...
dictionary);
        foreach (
var  original in dictionary.Elements)
        { if (    !ReferenceEquals    (original.  Value, Visit(state, original. Value)))
         {
                var elements =  new Dictionary<ScalarValue, LogEventPropertyValue>(dictionary. Elements.Count); foreach ( var element in dictionary  .Elements    )
                {
