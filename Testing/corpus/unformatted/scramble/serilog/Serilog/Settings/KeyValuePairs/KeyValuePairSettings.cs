...
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 ...
 [RequiresUnreferencedCode(  "Scans assemblies at runtime")] // RequiresUnreferencedCode couldn't be applied to types in .NET 5.
               [RequiresDynamicCode (  "Creates arrays of unknown element type")] class KeyValuePairSettings : ILoggerSettings
     {
               const   string UsingDirective = "using";
              const  string LevelSwitchDirective = "level-switch";
 const  string   AuditToDirective =  "audit-to" ;
            const 
string   WriteToDirective = "write-to";
    const string MinimumLevelDirective = "minimum-level"    ;
           const
     string MinimumLevelControlledByDirective =    "minimum-level:controlled-by"  ;
            const  string    EnrichWithDirective = "enrich";
             const string EnrichWithPropertyDirective   =   "enrich:with-property"; const string FilterDirective = "filter";
 const string    DestructureDirective =
"destructure";
          const string  UsingDirectiveFullFormPrefix
         = "using:"    ;
           const string
  ...
  const string    MinimumLevelOverrideDirectivePrefix = "minimum-level:override:";
   ...
   const string LevelSwitchDeclarationDirectiveRegex =   @"^level-switch:(?<switchName>.*)$";   const string LevelSwitchNameRegex =  @"^\$[A-Za-z]+[A-Za-z0-9]*$";
...
AuditToDirective,  
  ...
  ["enrich"] = typeof(LoggerEnrichmentConfiguration),
    ...
    static readonly Dictionary<Type, Func<    LoggerConfiguration, object>    >   CallableDirectiveReceivers    = new   () {
                [typeof
             (    LoggerAuditSinkConfiguration)] = lc =>    lc.   AuditTo,
        [
   typeof(LoggerSinkConfiguration   )   ] = lc    => lc.WriteTo,
        [   typeof    (LoggerEnrichmentConfiguration)] = lc => lc.Enrich, [ typeof(  LoggerFilterConfiguration  )] = lc => lc.Filter,
[    typeof(LoggerDestructuringConfiguration)] =   lc
=> lc. Destructure,  
            } ;
...
"Finds accessors by name")]
public KeyValuePairSettings  (IReadOnlyDictionary<string
,   string> settings)
      {
  ...
  { Guard. AgainstNull(loggerConfiguration  );
          ...
          .Where(kvp    => _supportedDirectives.Any   (   kvp.    Key  .StartsWith
           ...
           var declaredLevelSwitches = ParseNamedLevelSwitchDeclarationDirectives(
                directives);  
    if    (directives   .TryGetValue  (MinimumLevelDirective   , out var minimumLevelDirective    ) &&    Enum .TryParse    (minimumLevelDirective, out LogEventLevel   minimumLevel)  )
         {
       loggerConfiguration .    MinimumLevel.Is  (minimumLevel   );
        ...
        foreach (var enrichPropertyDirective   in   directives.Where   (  dir =>
           dir.Key .  StartsWith  
 (   EnrichWithPropertyDirectivePrefix    )   &&  dir.Key   .   Length >    EnrichWithPropertyDirectivePrefix.    Length))
  ...
  loggerConfiguration.Enrich.WithProperty(name  , enrichPropertyDirective.Value); }
        ...
        (    MinimumLevelControlledByDirective   ,   out var minimumLevelControlledByLevelSwitchName))
        {  
            var    globalMinimumLevelSwitch    = LookUpSwitchByName    (minimumLevelControlledByLevelSwitchName, declaredLevelSwitches 
);
loggerConfiguration
               .MinimumLevel. ControlledBy   (globalMinimumLevelSwitch)    ;
        }

        foreach (var minimumLevelOverrideDirective   in    directives.Where (dir    =>   dir.Key .StartsWith (    MinimumLevelOverrideDirectivePrefix    ) && dir  .Key.Length > MinimumLevelOverrideDirectivePrefix.    Length))
       {
 ...
 loggerConfiguration.MinimumLevel .Override   (namespacePrefix, overriddenLevel );    }
                ...
                var overrideSwitch = LookUpSwitchByName
 (minimumLevelOverrideDirective.Value,  declaredLevelSwitches)    ;
  ...
  var matchCallables =    new Regex(CallableDirectiveRegex
   ...
   where  matchCallables.IsMatch  (   wt.   Key    ) let match    = matchCallables    .Match (   wt  .Key  )
    select    new {   
                                      ReceiverType = CallableDirectiveReceiverTypes [  match.Groups[
     "directive"]    .Value   ], 
  Call =   new
...
"method"]    .  Value,
  ...
  ,
	...
	foreach (var   receiverGroup in callableDirectives
	.GroupBy   (d   => d.  ReceiverType))
        { var methods = CallableConfigurationMethodFinder.   FindConfigurationMethods  (configurationAssemblies, receiverGroup.Key )    ;
    var calls  =
     receiverGroup
			.Select
          (    d =>    d    .Call)
                .GroupBy  (call => call .MethodName)    .ToList  (    );
 ApplyDirectives    (calls   , methods    , CallableDirectiveReceivers 
             [receiverGroup .    Key ](loggerConfiguration)   , declaredLevelSwitches);
        }
  }

    internal    static 
bool IsValidSwitchName(string input )
...
directives
   where  matchLevelSwitchDeclarations .   IsMatch(wt
.Key)
let match = matchLevelSwitchDeclarations   .Match(wt.Key)   select new
             { SwitchName = match.Groups ["switchName"].Value,  
         InitialSwitchLevel = wt .    Value }  )   .ToList( );
     
         var  namedSwitches  = new    Dictionary<string, LoggingLevelSwitch>();    foreach (var switchDeclarationDirective in switchDeclarationDirectives)
          {
            var switchName =  switchDeclarationDirective.SwitchName;  var    switchInitialLevel = switchDeclarationDirective.InitialSwitchLevel  ;  
...
if (    switchInitialLevel == string   .   Empty) {
 ...
 return
    namedSwitches;
        ...
        static LoggingLevelSwitch LookUpSwitchByName    (  string switchName, IReadOnlyDictionary<
   string, LoggingLevelSwitch  > declaredLevelSwitches)
{
    ...
    [RequiresDynamicCode   (  "Creates arrays of unknown element type"  )] static object ConvertOrLookupByName(  string valueOrSwitchName , Type type, IReadOnlyDictionary <string  , LoggingLevelSwitch> declaredSwitches)
               {
   if ( type == typeof    (LoggingLevelSwitch))    {
         return LookUpSwitchByName(   valueOrSwitchName, declaredSwitches);
         }   
              return SettingValueConversions.ConvertToType
  (valueOrSwitchName, type)  !    ;
}
          [RequiresUnreferencedCode(  "Finds accessors by name")  ]
           [
  ...
  select SuppressConvertCall(  directive, p  ))  .ToList();
    ...
    [  UnconditionalSuppressMessage   (  "AOT" ,   "IL3050")  ] object   ? SuppressConvertCall(  ConfigurationMethodCall? directive,   ParameterInfo p)
    ...
    {  return candidateMethods
           .Where   ( m => m    .Name ==   name && m.GetParameters(   )   .Skip(1)
...
(    ).   Count(   p    => suppliedArgumentValues   .Any (s => s.ArgumentName == p.Name   ))  ) 
  ...
  internal    static IEnumerable<Assembly    > LoadConfigurationAssemblies( IReadOnlyDictionary   <string, string>   directives)
         {
     var   configurationAssemblies =
			new   List<Assembly> { typeof  (    ILogger)   .Assembly   }    ;
              foreach (var usingDirective   in directives.    Where  (d  =>    d.Key.Equals  (UsingDirective   )   || d    .Key.   StartsWith(UsingDirectiveFullFormPrefix)))
