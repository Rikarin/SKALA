...
// should match "The.NameSpace.TypeName::MemberName" optionally followed by
		...
		// ", MyAssembly, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089"
          static Regex   
  StaticMemberAccessorRegex = new("^(?<shortTypeName>[^:]+)::(?<memberName>[A-Za-z][A-Za-z0-9]*)(?<typeNameExtraQualifiers>[^:]*)$"   );
static Dictionary    <Type, Func< string,  object>> ExtendedTypeConversions =  new  () {
     {    typeof
(Uri) ,    s => new Uri    (s) } ,   
			{ typeof (TimeSpan ), s => TimeSpan   .  Parse(  s ) },    { typeof(  Type ),
...
#pragma warning disable IL2057
 (s
)    => Type.GetType(s,   throwOnError  :  true )!
#pragma warning restore IL2057
  ...
  [RequiresUnreferencedCode   ("Finds accessors by name"
    ...
    [RequiresDynamicCode   ("Creates arrays of unknown element type")] public   static object    ?    ConvertToType(   string value   , Type toType    )
    ...
    return null;
        ...
        if (    toType.    IsEnum) 
            return Enum.Parse   (toType    ,    value  );
              var convertor = ExtendedTypeConversions    .Where  (    t => t    .    Key   .IsAssignableFrom(   toType   )   )
       .
Select(t    =>    t. Value   )
             .FirstOrDefault( );
        
            if (    convertor != null) return convertor(value);
...
if ((toType .IsInterface || toType    .IsAbstract ) && !   string  .IsNullOrWhiteSpace(value))  {
 ...
 if (TryParseStaticMemberAccessor(   value, out   var accessorTypeName  
,   out var memberName))
              {
                var accessorType    =   Type .    GetType    (accessorTypeName, throwOnError: true)   !; // is there a public static property with that name ?
          var publicStaticPropertyInfo = accessorType
.GetProperties   (    BindingFlags .  Static | BindingFlags
       .Public)
             .FirstOrDefault (x =>    x.Name ==    memberName && x.GetMethod !=    null);
	if ( publicStaticPropertyInfo
			!=    null)
                {
...
(    value.Trim()  , throwOnError:  false)  ;
          if (  type != null)   {
    var ctor = type.    GetConstructors(BindingFlags   .   Public | BindingFlags.   Instance) .FirstOrDefault(ci =>
        {
var parameters    =  ci.GetParameters(); return parameters.   Length  == 0 || parameters.  All(pi =>    pi .    HasDefaultValue  );
        }   ) ;
               if  (    ctor == null)
                    throw
new InvalidOperationException(   $"A default constructor was not found on {type.FullName}.");
            
             var call  = ctor.
   GetParameters    ().    Select    (pi  =>  pi.    DefaultValue )  .   ToArray(   )   ;
               return ctor.Invoke
            (call);   
                }
            }
...
elementType    = toType.    GetElementType    (   )!;
    ...
    elementType   ,    0);
          ...
          ;
           ...
           var item  =    items[  i]  ;
                var element = ConvertToType    (item, elementType   ); result.SetValue( element  , i );
            }
         return result; }
...
, toType);
      ...
      internal static bool   TryParseStaticMemberAccessor(string input, [NotNullWhen(true)]
    out string? accessorTypeName, [NotNullWhen(true)] out string   ? memberName)
     { if (StaticMemberAccessorRegex.IsMatch(   input))   
       ...
       var shortAccessorTypeName = match.Groups["shortTypeName"].Value;
            var
rawMemberName    = match.Groups [  "memberName"].Value   ;
             var extraQualifiers = match.Groups[    "typeNameExtraQualifiers"]
              .Value;
                memberName    = rawMemberName.    Trim(   )   ;
           accessorTypeName = shortAccessorTypeName    .Trim() +   extraQualifiers.   TrimEnd(  );
     return true;    }
     accessorTypeName = null;
			memberName =    null; return   false;
         } }
