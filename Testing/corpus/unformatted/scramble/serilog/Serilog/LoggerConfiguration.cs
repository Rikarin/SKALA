...
/// </summary>
    ...
    readonly List<
             ILogEventSink> _logEventSinks    =  [];
           readonly List <  ILogEventSink   > _auditSinks = [ ];
readonly List <ILogEventEnricher> _enrichers = [ ]; readonly List< ILogEventFilter >    _filters = [] ;
       readonly List   <Type>
 _additionalScalarTypes = [ ];
    readonly   HashSet
   <  Type  > _additionalDictionaryTypes  =  [  ]    ;   
 readonly List<  
       IDestructuringPolicy    > _additionalDestructuringPolicies = [];
                readonly Dictionary   <string ,
                LoggingLevelSwitch> _overrides = new   ()    ;
    LogEventLevel _minimumLevel = LogEventLevel .Information;
               LoggingLevelSwitch ?    _levelSwitch; int _maximumDestructuringDepth = 10  ;
    ...
    public    LoggerConfiguration(    )
      {
           WriteTo   = new   (    this    , s => _logEventSinks.
       Add(s))    ;
  ...
  public    LoggerSinkConfiguration WriteTo { get;    internal    set;   }
    ...
    /// <returns>Configuration object allowing method chaining.</returns>
                public
 LoggerMinimumLevelConfiguration MinimumLevel
 {  
      get
        {
                return
...
depth =>   _maximumDestructuringDepth    =   depth , length  => _maximumStringLength =   length ,
    ...
    /// </summary>
              public  LoggerSettingsConfiguration
           ReadFrom =>    new(this);
    ...
    {
 if
          (_loggerCreated) throw new InvalidOperationException(    "CreateLogger() was previously called and can only be called once."   );
        _loggerCreated = true
;
      ILogEventSink  ? sink =  null;
              if  (_logEventSinks  .Count > 0) sink  = new   SafeAggregateSink   ( _logEventSinks);   
