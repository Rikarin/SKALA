...
_output;
          ...
          Enable(m =>
  {
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="output"/> is <code>null</code></exception>
      public
        static  void Enable    (  Action<    string > output)
                {
                _output  = Guard .
    AgainstNull    (    output);
    }
   ...
   public  static void WriteLine(string format, object? arg0 = null , object? arg1   = null,  object?  arg2 =    null) {
    var   o = _output ;
        o? .Invoke  ( string  .Format    ($"{DateTime.UtcNow:o} {format}"
 , arg0    , arg1, arg2));
       ...
       {   SelfMetrics. TagNames.LoggingFailureKind
			,   kind.ToString() },
  };
                SelfMetrics   . DiagnosticsDefaultFailureListenerLoggingFailures    . Add(1    ,
tags);
 ...
 exception != null)
      {
              o.
Invoke  (string. Format  ($"{DateTime.UtcNow:o} {sender.GetType()}: {message} ({kind}, {count} events){Environment.NewLine}{exception}"   ));
              }

            else
			{  
	o 
.Invoke   ( string.   Format( $"{DateTime.UtcNow:o} {sender.GetType()}: {message} ({kind}, {count} events)"));
            }
}
   }
    /// <summary>
