...
/// </summary>
 public class JsonValueFormatter : LogEventPropertyValueVisitor<
		TextWriter, bool  >
  {
  readonly string
            ? _typeTagName   ;
	...
	public JsonValueFormatter(    string ?
            typeTagName =    DefaultTypeTagName    )
   {
    ...
    protected override  bool VisitScalarValue(TextWriter state
           , ScalarValue scalar)
    {
                Guard
  .AgainstNull(  scalar);
...
FormatLiteralValue( scalar.Value,  state)  ;
    ...
    protected override bool VisitSequenceValue ( TextWriter    state    , SequenceValue sequence  ) {
        ...
        state.Write('[')   ;   char    ? delim = null;
         for
               ( var    i   =   0  ; i < sequence  .  Elements.Count; i++   )
 {
             if (delim != null) {
           state  .Write   (delim  .Value);
         }
                delim = ',';
            Visit   (state, sequence
       .Elements[i] );
               } state.  Write   (  ']')  ;
          return  false ; }
    /// <summary>
    ...
    protected    override bool    VisitStructureValue   (TextWriter state,
       StructureValue structure )
 { state.    Write('{')  ;
       
      char?  delim  = null  ;
         
             for (var  i =    0;  i   <    structure.Properties.
    Count; i++)
           { if ( delim != null   )
{ state.Write  (    delim.Value);
    }
 ...
 Visit(state    , prop  .Value); }
    ...
    if  ( _typeTagName != null && structure.TypeTag !=
         null)
                {
   state.
   Write(delim   );
           WriteQuotedJsonString(_typeTagName, state );
            state. Write  (':'); WriteQuotedJsonString(structure   . TypeTag  , state   );
             }
                state    .  Write   ('}'    );
		return
false;
...
state .Write(delim.  Value)  ;
    ...
    delim  = ',';
            ...
            if (key is    null )
          {
            ...
            WriteQuotedJsonString(  key.ToString
       (  )! ,  state);
                state.   Write(
':');
            }
...
is    long l   )
         {
     FormatExactNumericValue(l , output); return;
            }
            if ( value is ulong ul)
          {
			FormatExactNumericValue    ( ul,   output)    ;   return    ;
   }
...
value is short s)
 ...
 if (
               value is ushort  us)  
   {
       ...
       {  
               FormatDoubleValue( d   , output   )    ; return;
               }
            ...
            if
              ...
              {
             FormatFloatValue( f, output )   ; return;
          }
          if (  value is bool   b ) {
              FormatBooleanValue(    b, output ); return ;
    ...
    is char )
{
  ...
  if ( value is DateTime dt) {
                FormatDateTimeValue(dt   , output   )  ;  return;
           }
...
. ToString   (CultureInfo.InvariantCulture ), output)    ;
          return;  }
#if FEATURE_SPAN
        Span<char> buffer = stackalloc char[64];
        if (value.TryFormat(buffer, out var written, "R", CultureInfo.InvariantCulture))
...
static void FormatExactNumericValue (int value  , TextWriter output  )   {
    ...
    static void FormatExactNumericValue(uint
    ...
    static void FormatExactNumericValue(  long value , TextWriter   output )    {
        ...
        output.Write(value.ToString(null, CultureInfo.InvariantCulture));
          ...
          static void FormatExactNumericValue( ulong    value, TextWriter   
          output)
	{   
#if FEATURE_SPAN
        Span<char> buffer = stackalloc char[64];
  ...
  static
           void FormatExactNumericValue( decimal value, TextWriter output)
 {
#if FEATURE_SPAN
        Span<char> buffer = stackalloc char[64];
        if (value.TryFormat(buffer, out var written, provider: CultureInfo.InvariantCulture))
    ...
    static    void FormatExactNumericValue(short value, TextWriter   output)
        ...
        static void  FormatExactNumericValue(ushort   value, TextWriter  output)
      {
        ...
        output.Write(value.ToString(null, CultureInfo.InvariantCulture));
          ...
          static void
          FormatDateTimeValue    ( DateTime value,  TextWriter output)
  {
    output  . Write('\"');  
  ...
  static   void FormatDateTimeOffsetValue(   DateTimeOffset   value, TextWriter
		output)  
    {
        output.Write ('\"')    ;
...
#endif
             static void   FormatLiteralObjectValue  
        (object value, TextWriter output    )
    {
...
output.Write( "\\t");
                        break; default:
        output.Write("\\u" ); output .   Write (   ((int)c).   ToString    ("X4")   )  ;
   break;
                }
   }
        }
