...
Display    ;
    ...
    public static void Render(MessageTemplate template, IReadOnlyDictionary <
 ...
 .Where    (   kvp    =>    !( TemplateContainsPropertyName(template
              ,   kvp.Key    ) || 
                                TemplateContainsPropertyName(  outputTemplate , kvp  .Key  ))  ) .Select   (kvp => new   LogEventProperty (kvp .    Key  , kvp.Value)) );
JsonValueFormatter  .Format(sv    , output ); return;
        }
             output.Write("{ ");
    ...
    foreach (var kvp
               in properties)
           {
    if (TemplateContainsPropertyName(    template    ,
  kvp  .Key  ) ||
...
continue  ; }
        ...
        output.Write(delim); delim   =    ", " ;
            output
. Write    (    kvp.Key)    ;
             output.Write(    ": "   ); kvp.  Value.Render (output    , null, formatProvider);
        }

             output.Write(" }");
 }
         static bool  
          TemplateContainsPropertyName (  MessageTemplate template, string propertyName)
    {
			
             if (template    .PositionalProperties != null    )
            {    for (  var   i    = 0    ; i < template.   PositionalProperties.Length; i ++)
       {
              var   token = template   .    PositionalProperties[ i] ; if (token.PropertyName  ==   propertyName) 
                {
        return true    ;
       }
			}
