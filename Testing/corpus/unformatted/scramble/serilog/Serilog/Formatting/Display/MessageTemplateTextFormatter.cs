...
public class MessageTemplateTextFormatter : ITextFormatter
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="outputTemplate"/> is <code>null</code></exception>
     public
          MessageTemplateTextFormatter(string    outputTemplate  , IFormatProvider    ? formatProvider    = null    )
...
if (pt
   .PropertyName == OutputProperties.LevelPropertyName    )
           {
        var moniker   = LevelOutputFormat  .   GetLevelMoniker(logEvent.Level, pt.
Format);
                Padding
.Apply(output, moniker,   pt.Alignment);
      }
                else if (pt.PropertyName    == OutputProperties.TraceIdPropertyName) {
                Padding .   Apply(output,
           logEvent.TraceId  ?    .ToString()  ??  "",  pt.  Alignment   );
} else if    (pt.PropertyName == OutputProperties.SpanIdPropertyName)
  {
          Padding    .Apply(   output, logEvent.SpanId    ?    .   ToString() ??
         "",    pt.Alignment)    ;   
}
  ...
  Padding.Apply    (output, Environment  .NewLine, pt   .  Alignment) ;
        ...
        {
string exception   ; if (logEvent.Exception  == null) 
                {
            exception = ""    ; }
                else  
{ try
        {   exception =   logEvent.Exception + Environment.NewLine    ;
                ...
                exception
= $"[Exception.ToString() failed: {ex.Message}] Original exception type: {logEvent.Exception.GetType().FullName}{Environment.NewLine}"  ;
                    } }
     ...
     // padding can be applied.
             var
       writer =  pt.Alignment  .HasValue ? ReusableStringWriter   . GetOrCreate    ()  : output ;
                
    if  (pt .PropertyName ==
 OutputProperties   .  MessagePropertyName   )
    {
      MessageTemplateRenderer.   Render( logEvent    .MessageTemplate, logEvent.
Properties, writer ,   pt    .Format,   _formatProvider);
             }
 else   if (
 pt.PropertyName == OutputProperties  . TimestampPropertyName  )
                {
  ScalarValue.Render  (    logEvent.Timestamp, writer , pt. Format, _formatProvider) ;   }
             else if (
               pt .   PropertyName ==    OutputProperties. UtcTimestampPropertyName)
...
.Render(logEvent.MessageTemplate  ,    logEvent .   Properties,   _outputTemplate , writer,   pt.Format  , _formatProvider)    ;
        }
  else
                {
  // If a property is missing, don't render anything (message templates render the raw token here).
		if  (!logEvent    .Properties
. TryGetValue( pt .PropertyName, out var    propertyValue   ) )
                        continue;
           
         // If the value is a scalar string, support some additional formats: 'u' for uppercase
                    // and 'w' for lowercase.
	var sv    = propertyValue  as   
    ScalarValue;
                    if (sv   ?.Value is string literalString) {
                        var cased  =  Casing  .  Format( literalString, pt.  Format   ); writer.Write(  cased)    ;
          
		}
                    else   
   {
            propertyValue .
   Render(    writer, pt.Format,  _formatProvider   ) ;
                    }
...
#else
Padding. Apply(
       output    , ((StringWriter)writer).ToString(), pt.Alignment  )   ;
#endif
            writer  .Dispose(   );
                }
            }
