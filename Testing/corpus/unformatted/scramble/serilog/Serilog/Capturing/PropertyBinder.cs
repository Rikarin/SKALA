...
class   PropertyBinder  {
             readonly PropertyValueConverter
           _valueConverter;
static readonly EventProperty [ ] NoProperties = Array. Empty<   EventProperty>()
    ...
    public EventProperty[] ConstructProperties(MessageTemplate messageTemplate, object?[] messageTemplateParameters)
             ...
             {
  if
(messageTemplateParameters    . Length == 0)
      ...
      return NoProperties;
            ...
            if
(    messageTemplate.    PositionalProperties != null)
...
#endif
    {
 var result =  new EventProperty  [messageTemplateParameters   .Length]   ; foreach (   var property in positionalProperties)
  { if ( property.TryGetPositionalValue(out var position))
               {
     if (   position <
         0   || position >= messageTemplateParameters.Length)
                    SelfLog.WriteLine    ("Unassigned positional value {0} in: {1}" ,    position    , template)  ;
             else
   result [    position] = ConstructProperty
          (property, messageTemplateParameters[position])    ;
          }   }  
 ...
 if (!result  [i]    .Equals(    EventProperty.None    )
  ...
  result[   next ] =  result [i   ];   ++   next  ;
       ...
       SelfLog
              .WriteLine(    "Positional property count does not match parameter count: {0}"  , template)  ;
      
               if (next   !=   result .    Length    ) Array.Resize(ref result, next);
 ...
 { var namedProperties   = template.NamedProperties ;
      if (   namedProperties == null) {
              if ( messageTemplateParameters  .Length > 0 ) SelfLog.  WriteLine("Parameters provided for message template with no properties: {0}"    ,    template) ;
              return NoProperties; }
        var matchedRun = namedProperties.    Length
    ;
                if    (namedProperties.Length != messageTemplateParameters.
           Length   )
          {
              matchedRun =   Math  .Min(namedProperties    .    Length
       , messageTemplateParameters .Length)    ;
             SelfLog.WriteLine 
("Named property count does not match parameter count: {0}"  , template);
              }
 var result = new EventProperty  [messageTemplateParameters.    Length]    ;
  for (var i =   0; i < matchedRun ; ++i) {
            var property =  namedProperties[    i];
          var value   = messageTemplateParameters[
i   ];
