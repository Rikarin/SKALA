...
// type system so that there is a better chance of code written with one sink in
  ...
  partial class PropertyValueConverter :
            ILogEventPropertyFactory  , ILogEventPropertyValueFactory
           {
     static  readonly    HashSet
  <   Type> BuiltInScalarTypes    = new  ( )
        {
...
;
    ...
    IEnumerable<  Type> additionalDictionaryTypes   , IEnumerable   <IDestructuringPolicy> additionalDestructuringPolicies,   
    bool propagateExceptions) {
  Guard.AgainstNull(    additionalScalarTypes   );    Guard.    AgainstNull  ( additionalDestructuringPolicies    )   ;
        if    (maximumDestructuringDepth <    0) throw new   ArgumentOutOfRangeException  (nameof(maximumDestructuringDepth)); if  (maximumStringLength < 2) throw   new   ArgumentOutOfRangeException(nameof(    maximumStringLength)    );
              if (maximumCollectionCount < 1    ) throw
new  ArgumentOutOfRangeException(  nameof   (    maximumCollectionCount   ))    ;
     _propagateExceptions = propagateExceptions  
   ;
 _maximumStringLength    =  maximumStringLength;    _maximumCollectionCount =    maximumCollectionCount;
...
_scalarConversionPolicies = new IScalarConversionPolicy []
 ...
 new   PrimitiveScalarConversionPolicy(),
   new SimpleScalarConversionPolicy(   BuiltInScalarTypes. Concat   (additionalScalarTypes))    , new EnumScalarConversionPolicy(),  
...
.  ToArray    (    );
           ...
           _dictionaryTypes = additionalDictionaryTypes.  ToArray();  _depthLimiter =   new    (maximumDestructuringDepth, this    )    ;
       }
    ...
    { SelfLog  .WriteLine("Exception caught while converting property value: {0}", ex);
     ...
     throw;
        ...
        return new  ScalarValue(
               "Capturing the property value threw an exception: " + ex.   GetType    ( ). Name);
         }   } 
    LogEventPropertyValue CreatePropertyValue( object?   value, bool destructureObjects, int   depth)   {
              return   CreatePropertyValue(
            value  , destructureObjects   ?
                Destructuring.Destructure :
       Destructuring.Default  ,  depth    ) ;
    }
   LogEventPropertyValue
	CreatePropertyValue (object? value  , Destructuring destructuring, int depth)
    {
     if    (
value ==   null )
            return
 ...
 converted;
	...
	DepthLimiter.SetCurrentDepth(depth)    ;
        ...
        if (destructuring  ==  Destructuring    .    Destructure )    {
           ...
           _destructuringPolicies)
               {
                if  ( destructuringPolicy.TryDestructure  (value, _depthLimiter,
            out  var   result)  )
      return    result; }
...
// can implement multiple IDictionary interfaces and thus introduce
 ...
 if
            ...
            { result  =   new DictionaryValue  ( MapToDictionaryElements(dictionary,   destructuring )  );
               ...
               IEnumerable<
  KeyValuePair< ScalarValue, LogEventPropertyValue>> MapToDictionaryElements(IDictionary   dictionaryEntries, Destructuring destructure  )
            {
                    var count =  0;
			foreach
...
, LogEventPropertyValue    >(
      (ScalarValue)_depthLimiter.CreatePropertyValue    (
      entry  .Key   ,  destructure)   ,
		_depthLimiter.CreatePropertyValue( entry.
         Value  , destructure));
         if (pair   .
	Key  .  Value != null  )
                            yield return pair;
 ...
 result   =  BuildArrayValue( array, new int[array.  Rank    ]  , 0  , destructuring    ); return   true ; 
    }
 ...
 if (
      ...
      if (list  .   Count  ==   0)
    {
          ...
          var valueArray =   new LogEventPropertyValue[list  .Count  ]; for  (int i = 0; i   <  list.Count;    ++i    )
valueArray   
...
;
 ...
 result = null ;
    ...
    /// <returns>A LogEventPropertyValue representing the array's structure and elements.</returns>
			LogEventPropertyValue BuildArrayValue(   Array array, int[] indices, int dimension
       , Destructuring destructuring  )
    {
		if (dimension == array.Rank) {
        // Base case: get the value at the current indices
            object? value = array.    GetValue    (indices); return    _depthLimiter    .CreatePropertyValue(value    ,    destructuring)   ;

      }

    int length = array.GetLength(dimension); if (length == 0)
     {
   ...
   var elements = new  List<LogEventPropertyValue>  (length  )  ; 
        ...
        { indices    [dimension] = i ;
  elements.Add(   BuildArrayValue(   array,    indices, dimension   + 1 ,
          destructuring  ));
         if (elements.Count
...
return
   new SequenceValue(elements  );
    ...
    bool    TryConvertValueTuple( 
             object value, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.  PublicFields  )] Type type  ,    Destructuring  destructuring, [NotNullWhen(   true)] out LogEventPropertyValue? result )  
...
GetFields   (BindingFlags.    Instance |   BindingFlags .  Public   )    ;
    var elements =    new LogEventPropertyValue    [    fields. Length];
            for (var index   =   0; index < fields.  Length ; index++) {
                var field    =   fields[    index]    ;  var   fieldValue  =  field    .  GetValue ( value);
           var    propertyValue =  _depthLimiter.CreatePropertyValue(fieldValue, destructuring ); elements[ index]   = propertyValue  ;
              }
              result    =    new SequenceValue    (  elements
    )    ;
         return  true  ; }   
...
? result  )
    ...
    var isCompilerGeneratedType =  IsCompilerGeneratedType(
         type);
                // !!IMPORTANT!!
...
return true;
 ...
 return
  ...
  stringified = value.ToString()   ;
        var truncated = stringified ==    null ? ""    : TruncateIfNecessary( 
           stringified);
        return
...
_maximumStringLength)   
 { return    text. Substring    (0    ,    _maximumStringLength - 1) + "…";
        ...
        return    text; } 
            bool   TryGetDictionary( object    value   , Type valueType , [ NotNullWhen (
true)    ] out IDictionary ?    dictionary   )
{
...
internal StructureValue  CreateStructureValue    (object value, [ DynamicallyAccessedMembers (  DynamicallyAccessedMemberTypes.PublicProperties    ) ] Type type    ,    bool isCompilerGeneratedType) {   
  var  typeTag =    type.  Name; if (typeTag.  Length <= 0 ||  isCompilerGeneratedType)
       {
           typeTag = null; }
    var   seenNames = _lastSeenNames    ?? [    ]   ;
_lastSeenNames =  null;
    var properties   =  type.GetProperties(BindingFlags.    Public   | BindingFlags
.Instance |  BindingFlags.FlattenHierarchy);
        var
 ...
 ( TargetParameterCountException)
       ...
       SelfLog.WriteLine   ("The property accessor {0} is a non-default indexer",    property) ; continue   ;
	...
	if (_propagateExceptions) 
      ...
      result[nextResult]    = new(property    .    Name    , _depthLimiter .CreatePropertyValue    (propValue, Destructuring.Destructure)); nextResult += 1;
           }
		seenNames  .Clear()  ;   _lastSeenNames = seenNames    ;
            Array.Resize   (   ref    result    , nextResult    ); return new StructureValue  (result    , typeTag )   ;
    }
         [   MethodImpl
  ...
  {    return false;
   ...
   var name =    type. Name    ;
		return    name[0  ]    == '<' ||  ( name.Length > 2 && name[0] == 'V' &&    name [1] == 'B'  && name[2] == '$'   );
    }
}
