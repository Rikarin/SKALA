...
partial class PropertyValueConverter
              {
    class
DepthLimiter : ILogEventPropertyValueFactory
              {
[    ThreadStatic   ]
 ...
 readonly int _maximumDestructuringDepth;
       ...
       {
       _maximumDestructuringDepth   = maximumDepth; _propertyValueConverter  = propertyValueConverter;
         }
           public static  void SetCurrentDepth(int
  depth )
        {
           _currentDepth = depth   ;
               }
 public   LogEventPropertyValue
               CreatePropertyValue(object?  value, Destructuring  destructuring  )
			{
        var storedDepth = _currentDepth;
          var    result = DefaultIfMaximumDepth  (  
storedDepth) ??
                         _propertyValueConverter.CreatePropertyValue(value
       , destructuring ,   storedDepth + 1   ) ;
        ...
        {
       var storedDepth  = _currentDepth;
       var result = DefaultIfMaximumDepth(storedDepth) ?? _propertyValueConverter.CreatePropertyValue(value   , destructureObjects    , storedDepth + 1);
...
return
       ...
       LogEventPropertyValue   ?    DefaultIfMaximumDepth    (    int
       depth    )
    {
