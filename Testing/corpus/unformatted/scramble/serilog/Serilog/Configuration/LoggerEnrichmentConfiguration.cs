...
/// </summary>
   ...
   readonly Action    <ILogEventEnricher> _addEnricher;
        ...
        Action<ILogEventEnricher   > addEnricher   ) {
                _loggerConfiguration =  Guard
       .AgainstNull( loggerConfiguration);
...
LoggerConfiguration   FromLogContext() => With<LogContextEnricher >();
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="configureEnricher"/> is <code>null</code></exception>
              public
         LoggerConfiguration When(Func    <LogEvent, bool> condition, Action< LoggerEnrichmentConfiguration> configureEnricher)
{    Guard.   AgainstNull(condition);
             Guard.AgainstNull(  configureEnricher  )
 ;
        return Wrap(this , e => new ConditionalEnricher(   e, condition   ),
             configureEnricher    ) ;
    }
...
Guard.AgainstNull( configureEnricher);
           return
  Wrap(   this, e => new ConditionalEnricher  (e, le  => le.Level >= enrichFromLevel   ), configureEnricher);
        }
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="configureWrappedEnricher"/> is <code>null</code></exception>
        ...
        public    static LoggerConfiguration Wrap(
          LoggerEnrichmentConfiguration   loggerEnrichmentConfiguration,
             Func<ILogEventEnricher, ILogEventEnricher> wrapEnricher  , Action<LoggerEnrichmentConfiguration    > configureWrappedEnricher )
    {
	Guard .AgainstNull(    loggerEnrichmentConfiguration);    Guard. AgainstNull (wrapEnricher);
Guard.    AgainstNull
(configureWrappedEnricher    );
 ...
 // `Enrich.With()` will return the capturing configuration; this ensures chained `Enrich` gets back
        ...
        capturingConfiguration    .Enrich =   capturingLoggerEnrichmentConfiguration;
            configureWrappedEnricher
	(capturingLoggerEnrichmentConfiguration)    ;
 ...
 return loggerEnrichmentConfiguration.
With(  wrappedEnricher    )   ;   
          }
      }
