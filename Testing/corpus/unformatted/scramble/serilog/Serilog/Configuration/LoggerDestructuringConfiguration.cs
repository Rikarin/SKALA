...
/// </summary>
    ...
    readonly  Action<Type  >    _addScalar;  readonly Action    <Type> _addDictionaryType;   
            readonly    Action<IDestructuringPolicy > _addPolicy    ;  readonly    Action    <int>   _setMaximumDepth;
  readonly  Action  <int
>   _setMaximumStringLength;
    readonly Action<int
               > _setMaximumCollectionCount;
          
              internal    LoggerDestructuringConfiguration (
	LoggerConfiguration loggerConfiguration ,
    Action<  Type>  addScalar,  Action<Type> addDictionaryType,
        Action   <   IDestructuringPolicy> addPolicy,    Action<   int>    setMaximumDepth   ,
   ...
   Action<int>    setMaximumCollectionCount   )
     {
       _loggerConfiguration = Guard.AgainstNull
             (loggerConfiguration);
 _addScalar = Guard  . AgainstNull(addScalar  ) ; _addDictionaryType =    Guard.AgainstNull(addDictionaryType);
        _addPolicy    = Guard.   AgainstNull(    addPolicy    ); _setMaximumDepth = Guard   .    AgainstNull(setMaximumDepth  )   ;
 _setMaximumStringLength = Guard.AgainstNull(    setMaximumStringLength) ; _setMaximumCollectionCount = Guard   . AgainstNull   (setMaximumCollectionCount)   ;
              }
...
AsScalar    (Type scalarType)
     ...
     _addScalar    (scalarType   )  ; return   _loggerConfiguration ;
         }
    /// <summary>
    /// Treat objects of the specified type as scalar values, i.e., don't break
    /// them down into properties even when destructuring complex types.
 ...
 // ReSharper disable once MemberCanBePrivate.Global
   ...
   Guard. AgainstNull(destructuringPolicies   );
               ...
               foreach (var destructuringPolicy
              in destructuringPolicies )
             {
...
(typeof    (T ));
   ...
   public LoggerConfiguration ByTransforming<TValue>(  Func<TValue, object>  transformation) {
    ...
    /// <exception cref="ArgumentNullException">When <paramref name="transformation"/> is <code>null</code></exception>
              public LoggerConfiguration    ByTransformingWhere  <TValue >(   Func<    Type    , bool  > predicate,
            Func< TValue, object> transformation)
    {
       ...
       transformation); 
           ...
           var policy   = new ProjectedDestructuringPolicy   (predicate, o => transformation(( TValue)o));
            return With(policy   );   }
    /// <summary>
    ...
    /// <exception cref="ArgumentOutOfRangeException">When <paramref name="maximumDestructuringDepth"/> is negative</exception>
     ...
     {
         if    (maximumDestructuringDepth <  0) throw new ArgumentOutOfRangeException(nameof   (
   maximumDestructuringDepth ),   "Maximum destructuring depth must be positive.");
            _setMaximumDepth   (maximumDestructuringDepth    ); return _loggerConfiguration;
               }
    /// <summary>
    ...
    /// <exception cref="ArgumentOutOfRangeException">When <paramref name="maximumStringLength"/> is less than 2</exception>
     ...
     if (maximumStringLength < 2
          ...
          _setMaximumStringLength(maximumStringLength)  ; return    _loggerConfiguration;
             }
    /// <summary>
