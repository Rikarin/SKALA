...
class ChainedCalls {
    ...
    void DoesNotFit(IEnumerable<string> source) {
        var b = source.Where(x => x.Length > 3).OrderBy(x => x).Select(x => x.ToUpperInvariant()).ToList().AsReadOnly()
            .Count();
    }

    void PropertyInTheChain(IEnumerable<string> source) {
        var c = source.Where(x => x.Length > 3).ToList().Count.ToString().Trim().Substring(0, 1).ToUpperInvariant()
            .Trim();
    }
    ...
    void RootedAtAConditionalAccess(IEnumerable<string>? sourceOfStringsHere) {
        var d = sourceOfStringsHere?.Where(x => x.Length > 3).OrderBy(x => x).Select(x => x.ToUpperInvariant()).ToList()
            .Count();
    }
    ...
    void TheSameChainWithoutTheConditional(IEnumerable<string> sourceOfStringsHere) {
        var e = sourceOfStringsHere.Where(x => x.Length > 3).OrderBy(x => x).Select(x => x.ToUpperInvariant()).ToList()
            .Count();
    }
    ...
    int ExpressionBodied(IEnumerable<string>? sourceOfStringsHere) =>
        sourceOfStringsHere?.Where(x => x.Length > 3).OrderByDescending(x => x).Select(x => x.ToUpperInvariant())
            .ToList().Count()
        ?? 0;
    ...
    void TheReceiverIsItselfAChain(IEnumerable<string> sourceOfStringsHere, IEnumerable<string> more) {
        var f = sourceOfStringsHere.Concat(more)?.Where(x => x.Length > 3).OrderBy(x => x).Select(x => x.Trim())
            .ToList().Count();
    }
