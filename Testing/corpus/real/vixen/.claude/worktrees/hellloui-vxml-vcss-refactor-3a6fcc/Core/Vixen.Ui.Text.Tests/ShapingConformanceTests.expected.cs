...
public class ShapingConformanceTests {
    ...
    static string? Check(FontFace font, string text, List<(string Name, double X, double Y)> expected) {
        ...
        if (actual.Count != expected.Count) {
            return
                $"expected {expected.Count} glyphs, got {actual.Count}: [{Describe(actual)}] want [{Describe(expected)}]";
        }
    ...
    static string Describe(IReadOnlyList<(string Name, double X, double Y)> glyphs) =>
        string.Join(
            " ",
            glyphs.Select(glyph => string.Create(
                    CultureInfo.InvariantCulture,
                    $"{glyph.Name}@{glyph.X:0.#},{glyph.Y:0.#}"
                )
            )
        );
    ...
    static IEnumerable<(string Id, string Font, string Text, List<(string Name, double X, double Y)> Expected)>
        Cases() {
        using var stream = Assembly.GetExecutingAssembly()
        ...
        while (reader.ReadLine() is { } line) {
            ...
            foreach (var codePoint in fields[2].Split(' ', StringSplitOptions.RemoveEmptyEntries)) {
                text.Append(
                    char.ConvertFromUtf32(int.Parse(codePoint, NumberStyles.HexNumber, CultureInfo.InvariantCulture))
                );
            }
