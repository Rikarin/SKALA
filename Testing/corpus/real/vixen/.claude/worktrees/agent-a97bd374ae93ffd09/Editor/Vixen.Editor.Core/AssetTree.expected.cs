...
public static class AssetTree {
    ...
    sealed class Builder(string name, string path, AssetId guid) {
        ...
        public AssetTreeNode Build() =>
            new(name, path, true, Guid, [.. Folders.Select(folder => folder.Build()).Concat(Files).Order(Order)]);
    }
    ...
    /// </remarks>
    static readonly IComparer<AssetTreeNode> Order = Comparer<AssetTreeNode>.Create((left, right) => {
