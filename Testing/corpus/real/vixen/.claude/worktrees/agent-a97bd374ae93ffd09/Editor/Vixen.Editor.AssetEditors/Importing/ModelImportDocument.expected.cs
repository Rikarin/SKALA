...
public sealed class ModelImportEdits {
    ...
    [Inspector]
    [Tooltip(
        "Rename a part: Source is what the file calls it, Name is what it is addressed as. Applies on the next import."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Below this a vertex is not in the shape. In the mesh's final units, so it follows Scale. Zero keeps every rounding-noise delta."
    )]
    ...
    [Inspector]
    [Tooltip(
        "What distance-field shadows and occlusion read. The most expensive part of the import; off for a project not lighting this way."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Quality and cost at once — doubling it is eight times the samples. Other axes follow the bounds so cells stay near-cubic."
    )]
    ...
    [Inspector]
    [Tooltip(
        "What makes the sign survive meshes that are not closed. Noisy below sixteen; stops changing above sixty-four."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Room outside the surface for a ray to slow down in. With none, the surface lies on the volume's own face."
    )]
    ...
    [Inspector]
    [Tooltip(
        "What the virtualized path draws: every level of detail at once, plus a fallback mesh. Off for a mesh that is already a hundred triangles."
    )]
    ...
    [Inspector]
    [Tooltip(
        "The unit of culling and of streaming both. A hundred and twenty-eight is about where the per-cluster overhead stops mattering."
    )]
    ...
    [Inspector]
    [Tooltip(
        "At most 256, because a cluster's triangles index its own vertex list with a byte. Where it binds, the cluster is split rather than the mesh refused."
    )]
    ...
    [Inspector]
    [Tooltip(
        "How much a level of detail can actually remove. A group's outer boundary is locked, so a small group has little interior to collapse."
    )]
    ...
    [Inspector]
    [Tooltip(
        "A cut through the finished hierarchy at a fixed budget. What WebGL2 draws and what the physics cook reads."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Replace the file's topology with all-quads. Destructive by definition, and the point of it for a generated mesh."
    )]
    ...
    [Inspector]
    [Tooltip(
        "A budget rather than a guarantee: a patch's quad count is a product of two sides, so a snaky partition overshoots and the report says by how much."
    )]
    ...
    [Inspector]
    [Tooltip(
        "A crease above this becomes a boundary of the layout, so it comes out as a chain of edges rather than something snapped to."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Exact: the seam snaps to zero and the mirror is a sign flip, so vertex k and its mirror correspond bit for bit. The model's own space."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Paths to .vxspline assets. A curve saved beside the mesh survives the mesh being regenerated, which a painted guide does not."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Never, only for meshes that arrived without any, or always. The middle one fixes generated meshes and leaves an artist's atlas alone."
    )]
    ...
    [Inspector]
    [Tooltip(
        "The packer needs it because the margin is counted in texels, and a margin in UV units means a different number of texels on every island."
    )]
    ...
    [Inspector]
    [Tooltip(
        "What stops one island's mip bleeding into the next. Four is enough for a full mip chain at most resolutions."
    )]
    ...
    [Inspector]
    [Tooltip(
        "Set it and every chart gets the same density — no more a face at half the resolution of the boots. Zero packs for efficiency instead."
    )]
