...
public class MeshletValidatorTests {
    ...
    public void AnErrorThatDoesNotIncreaseIsRefused() {
        ...
        var sabotaged = built with {
            Groups = [
                .. built.Groups.Select((candidate, index) => index == 0
    ...
    public void AClusterWhoseParentErrorDisagreesWithItsGroupIsRefused() {
        ...
        var sabotaged = built with {
            Meshlets = [
                .. built.Meshlets.Select((meshlet, index) =>
                    index == child ? meshlet with { ParentError = meshlet.ParentError * 2f } : meshlet
                )
            ]
        };
    ...
    public void ALostTriangleIsRefused() {
        ...
        var sabotaged = built with {
            Meshlets = [
                .. built.Meshlets.Select((meshlet, index) =>
                    index == leaf ? meshlet with { TriangleCount = meshlet.TriangleCount - 1 } : meshlet
                )
            ]
        };
    ...
    public void AClusterPointingPastTheSharedArraysIsRefused() {
        ...
        var sabotaged = built with {
            Meshlets = [
                .. built.Meshlets.Select((meshlet, index) =>
                    index == 0 ? meshlet with { VertexOffset = built.Vertices.Length } : meshlet
                )
            ]
        };
