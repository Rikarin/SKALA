...
static async Task<int> Run(Arguments arguments) {
    ...
    var (nodes, merged) = Scope.Deduplicate(
        NonCSharpNodes.Read(root, links)
            .Concat(
                documented
    ...
    if (arguments.UpdateExemptions) {
        ...
        Console.WriteLine(
            $"{Coverage.RelativePath}: {update.Added} added as `{Coverage.SeedReason}`, "
        ...
        if (update.Added > 0) {
            ...
            Console.WriteLine(
                "Read the diff before committing. A type added here is a type nobody "
    ...
    if (arguments.Release is { } release) {
        ...
        Console.WriteLine();
        Console.WriteLine(
            $"release {release}: {record.Types} types, {record.Members} members, "
    ...
    Console.WriteLine();
    Console.WriteLine(
        $"{documented.Count} of {projects.Count} projects documented "
        + $"(samples, benchmarks and tests are examples rather than surface), "
        + $"{graph.GeneratedDocumentCount} generated documents"
    );

    Console.WriteLine(
        $"{nodes.Count(node => node.UsedByCount > 0)} types are used by another, "
    ...
    Console.WriteLine(
        $"{nodes.Count} types, {nodes.Sum(node => node.Members.Count)} members, "
    ...
    Console.WriteLine();
    Console.WriteLine(
        $"index {written.IndexBytes / 1024.0:F0} kB, pages {written.PageBytes / 1024.0 / 1024.0:F2} MB "
        + $"in {written.Chunks} chunks ({written.SplitChunks} namespaces split) → {arguments.Output}"
    );
    Console.WriteLine(
        $"{pages.Count} guide pages, {documentedBy.Count} symbols with prose, "
    ...
    Console.WriteLine(
        $"coverage: {covered} of {nodes.Count} types have a page, {exemptions.Count} exempt, "
    ...
    if (problems.Count > 0) {
        Console.Error.WriteLine();
        Console.Error.WriteLine(
            $"{problems.Count} problem{(problems.Count == 1 ? string.Empty : "s")} "
            + "in the written half:"
        );
