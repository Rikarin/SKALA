...
public sealed partial class ColorField : Control {
    ...
    protected override void OnDraw(DrawContext context) {
            ...
            for (var i = 0; i < Samples; i++) {
                ...
                for (var j = 0; j < Samples; j++) {
                    ...
                    context.FillRectangle(
                        new Rectangle(
                            bounds.X + (i * slice),
                            bounds.Y + (j * bounds.Height / Samples),
                            slice + 1f,
                            (bounds.Height / Samples) + 1f
                        ),
        ...
        context.StrokeRectangle(
            new Rectangle(centre.X - 6f, centre.Y - 6f, 12f, 12f),
            Color4.White,
            2f,
            BoxStyle.Rounded(CornerRadii.Uniform(6f))
        );
        context.StrokeRectangle(
            new Rectangle(centre.X - 7f, centre.Y - 7f, 14f, 14f),
            Color4.Black,
            1f,
            BoxStyle.Rounded(CornerRadii.Uniform(7f))
        );
    }
...
public sealed partial class ColorSwatch : Control {
    ...
    protected override void OnCreated() {
        ...
        AddHandler<PointerEvent>(static (element, args) => {
