...
public static class PhysicalSky {
    ...
    static float Luminance(Vector3 linear) => (0.2126f * linear.X) + (0.7152f * linear.Y) + (0.0722f * linear.Z);
    ...
    static Vector3 Sky(Vector3 view, Vector3 sun, float turbidity, float sunTheta) {
        ...
        var zenithY = MathF.Max(
            (((4.0453f * turbidity) - 4.9710f) * MathF.Tan(chi)) - (0.2155f * turbidity) + 2.4192f,
            0f
        );
