...
public static class RuleAudit {
    public static string Run(IReadOnlyList<string> paths, bool semanticInLoose) {
        return Report(paths, semanticInLoose);
    }

    static string Report(IReadOnlyList<string> paths, bool semanticInLoose) {
        var loaded = ProjectLoader.Load(
            new LoadRequest {
                RepositoryRoot = Path.GetFullPath(paths.Count > 0 ? paths[0] : "."),
                Mode = LoadMode.Loose,
                Paths = paths
            }
        );
