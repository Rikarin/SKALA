...
switch (args[0]) {
    ...
    case "xmldoc":
        ...
        // ⚠ `xmldoc --oracle` is the other question and the newer one: not "what does the
        // sub-formatter cost against a profile that never moves", but "does it agree with the
        // profile that does". It reads the `.xmldoc.expected.cs` fixtures, which exist because
        // `CSharpFormatDocComments` was measured to be a real cleanup task rather than an absent
        // capability. See OracleProfile.DocComments.
        if (args.Contains("--oracle")) {
            Console.Write(XmlDocOracle.Measure());
            return 0;
        }
...
static int Regenerate(string[] sets, string? only) {
    ...
    written += RegenerateDocComments(runner, editorConfig, version, hash, sets, only);
...
// ⚠ The third profile's fixtures: `constructs/xmldoc/` under `CSharpFormatDocComments`, and nothing
// else. This is the profile that makes the `resharper_xmldoc_*` family measurable at all — every
// other fixture in the corpus was generated by a profile that returns documentation comments exactly
// as written, which is what SK-DIV-0006 mistook for a property of the tool.
//
// ⚠ Batched and flattened like the format-only profile rather than laid out as a tree like the
// cleanup one, and the difference is real rather than an oversight: reformatting a documentation
// comment is a per-file question — it reads the comment's own text and the .editorconfig — with no
// `var`, no using removal and nothing else that needs a compilation. What the cleanup profile's
// comment says about batching does not apply here.
static int RegenerateDocComments(
    OracleRunner runner,
    string editorConfig,
    string version,
    string hash,
    ...
    var profile = OracleProfile.DocComments;
    var header = new OracleHeader(version, hash, profile.Name, OracleFixture.Today);
    var wanted = new HashSet<string>(sets, StringComparer.Ordinal);
    var files = Only([.. Corpus.DocCommented().Where(file => wanted.Contains(file.Set))], only).ToArray();
    if (files.Length == 0) {
        return 0;
    }
    ...
    Console.WriteLine($"  xmldoc: {written.ToString(CultureInfo.InvariantCulture)} written");
    return written;
}
