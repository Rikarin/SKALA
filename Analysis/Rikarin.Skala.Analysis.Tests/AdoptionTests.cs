...
public sealed class AdoptionTests {
    ...
    /// <summary>
    /// ⚠ <b>`formatting: clean` counted findings the formatter refuses to fix, and was therefore
    /// unsatisfiable.</b> Measured on Vixen's <c>Core/Vixen.Water</c> after a full <c>skala
    /// format</c>: <c>format --check</c> reported "0 files would be reformatted" and the <c>ci</c>
    ...
    /// <summary>
    /// ⚠ <b>`skala explain` is documented as taking `&lt;ruleId | optionKey&gt;` and rejected every
    /// option key tried</b> — <c>insert_final_newline</c>,
