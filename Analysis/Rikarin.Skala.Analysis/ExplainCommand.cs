...
public static class ExplainCommand {
    public static CommandResult Run(string ruleId) {
        if (RuleCatalog.Find(ruleId) is not { } rule) {
            var near = RuleCatalog.All
                .Where(candidate => candidate.Id.StartsWith(
                        ruleId[..Math.Min(4, ruleId.Length)],
            ...
            return new CommandResult(
                ...
                $"skala explain: '{ruleId}' is not a Skala rule."
                + (near.Length > 0 ? " Did you mean " + string.Join(", ", near) + "?" : string.Empty)
                + "\n"
            );
        }
