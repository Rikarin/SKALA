...
class Build : NukeBuild {
    ...
    /// </summary>
    /// <remarks>
    ///     ⚠ <c>Testing/corpus</c> and the two fixture trees under <c>Rules.Tests</c> are excluded on
    ///     purpose. Those files are inputs: half of them are deliberately misformatted and the rest are
    ///     vendored from other people's trees, and a formatter that reformats its own test corpus has
    ...
    ///         file walk itself honours, so this target names top-level directories and nothing else and
    ///         a new project is covered the day it is added.
    ///     </para>
    /// </remarks>
    Target Lint =>
        definition => definition
    ...
    Target Unformat =>
        definition => definition
            .DependsOn(Compile)
            .Executes(() => DotNetRun(settings => Harness(settings, "unformat")));
