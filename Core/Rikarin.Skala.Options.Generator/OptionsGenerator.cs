...
public sealed class OptionsGenerator : IIncrementalGenerator {
    ...
    static string EmitRegistry(OptionRegistry model) {
        ...
        foreach (var option in model.Options) {
            ...
            var export = option.Export.Count == 0
                ? "[]"
                : "[" + string.Join(", ", option.Export.Select(static a => OptionRegistryReader.Literal(a))) + "]";
            var expands = option.Expands.Count == 0
                ? "[]"
                : "[" + string.Join(", ", option.Expands.Select(static e => "OptionId." + Naming.Pascal(e))) + "]";
            builder.AppendLine(
                $"        new(OptionId.{option.MemberName}, {OptionRegistryReader.Literal(option.Key)}, {aliases}, {export}, "
    ...
    /// <summary>
    ///     Emits <see cref="Naming.Prefixes" /> so the runtime call sites share one list.
    /// </summary>
    /// <remarks>
    ///     ⚠ Generated rather than hand-written in <c>Rikarin.Skala.Options</c>, because that project
    ///     has no <c>.cs</c> of its own — <c>options.json</c> and the generator are the whole assembly.
    ///     A hand-written copy would be a fifth restatement of the list this exists to unify.
    /// </remarks>
    static string EmitKeyPrefixes() {
        ...
        builder.AppendLine(
            """
            /// <summary>
            /// Every .editorconfig key prefix Skala knows, most specific first.
            /// </summary>
            /// <remarks>
            /// ⚠ The single authority. <c>OptionResolver</c> resolves precedence with it,
            /// <c>ConfigCommands.Family</c> groups the tier report with it and <c>SweepPlan.Strip</c>
            /// matches <c>--family=</c> with it. Each of those held its own copy until the
            /// <c>skala_</c> rename, no two of them agreed, and every disagreement failed silently
            /// into a wrong-but-plausible answer rather than into an error.
            /// <para>
            /// Order is significance order: <c>skala_xmldoc_</c> before <c>skala_</c>, or every xmldoc
            /// key strips to the wrong stem.
            /// </para>
            /// </remarks>
            public static class OptionKeyPrefixes {
            """
        );
        builder.AppendLine("    public static readonly string[] Ordered = [");
        foreach (var prefix in Naming.Prefixes) {
        ...
        builder.AppendLine(
            """
                /// <summary>The key with its prefix removed, or unchanged when it carries none.</summary>
