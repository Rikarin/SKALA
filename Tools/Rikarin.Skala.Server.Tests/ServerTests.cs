...
public sealed class Scratch : IDisposable {
    ...
    public void Dispose() {
        try {
            ...
            // A socket the runtime still holds is not worth failing a test over.
        }
    }
}
...
public sealed class DaemonTests {
    [Fact]
    public async Task Format_OverTheSocket_MatchesTheInProcessAnswer() {
        // ⚠ The property docs/plan/11 calls the daemon's correctness rule: "every command must work
        // identically with SKALA_NO_DAEMON=1. The daemon is only allowed to make things faster."
        // The suite asserts it rather than trusting it, because a cache that answers differently is
        // exactly the bug a daemon exists to introduce.
        using var scratch = new Scratch();
        var path = scratch.Write("A.cs", "class C{void M(){M();}}\n");
        var expected = CSharpFormatter.FormatFile(path).Formatted;
        ...
        Assert.Equal(expected, response.Formatted);
    }
    ...
    public async Task Format_AnsweredTwice_ComesFromTheCacheTheSecondTime() {
        ...
        DaemonClient.Send(scratch.Root, new DaemonRequest { Command = "format", Path = path });
        var status = DaemonClient.Send(scratch.Root, new DaemonRequest { Command = "status" });

        await stopping.CancelAsync();
        await running;

        Assert.Contains("1 hits", status!.Status, StringComparison.Ordinal);
    }

    [Fact]
    public async Task Format_AfterTheFileChanges_IsRecomputed() {
        // ⚠ The daemon never watches the filesystem (docs/plan/11: "It is asked; it does not
        // observe"), so the cache key is the file's content and not its path. Without that, this
        // test is the shape of every stale-daemon bug: edit, format, get yesterday's answer.
        using var scratch = new Scratch();
        var path = scratch.Write("C.cs", "class C{void M(){M();}}\n");
        ...
        var first = DaemonClient.Send(scratch.Root, new DaemonRequest { Command = "format", Path = path });
        File.WriteAllText(path, "class C{void M(){M();N();}}\n");
        var second = DaemonClient.Send(scratch.Root, new DaemonRequest { Command = "format", Path = path });

        await stopping.CancelAsync();
        await running;

        Assert.NotEqual(first!.Formatted, second!.Formatted);
        Assert.Contains("N();", second.Formatted, StringComparison.Ordinal);
    }

    [Fact]
    public async Task AClientOfAnotherVersion_IsRefused() {
        // No negotiation and no compatibility window (docs/plan/11 § "The daemon"): two Skala
        // versions formatting one repository is the failure the version pinning exists to prevent.
        using var scratch = new Scratch();
        await using var daemon = new Daemon(scratch.Root);
        daemon.Listen();
        using var stopping = new CancellationTokenSource();
        ...
        var response = DaemonClient.Send(
            scratch.Root,
            new DaemonRequest { Version = "skala/0", Command = "status" }
        );
        ...
        Assert.Contains("protocol mismatch", response.Error!, StringComparison.Ordinal);
    }

    [Fact]
    public void WithNoDaemonRunning_TheClientReturnsNullRatherThanThrowing() {
        using var scratch = new Scratch();
        Assert.Null(DaemonClient.Send(scratch.Root, new DaemonRequest { Command = "status" }));
        Assert.False(DaemonClient.Probe(scratch.Root));
    }
    ...
    public async Task ASocketFileWithNoDaemonBehindIt_IsReclaimed() {
        ...
        using var scratch = new Scratch();
        var socket = DaemonProtocol.SocketPath(scratch.Root);
