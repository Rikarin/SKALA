...
public sealed class EmptyTypeAnalyzer : DiagnosticAnalyzer {
    ...
    static void Analyze(SyntaxNodeAnalysisContext context) {
        ...
        // ⚠ `Foo;` is never reported and `Foo { }` is. The semicolon form exists precisely to
        // declare a complete type with nothing in it — a unit, a message, a marker — so writing one
        // is a statement. Braces around nothing are an omission.
        //
        // ⚠ This was scoped to records and should never have been: C# 12 allows the semicolon body
        // on every type declaration, and the argument above does not mention records at all. It
        // fired on `public sealed class Clean;` — a fixture in `AnalysisTests` whose whole purpose is
        // to be a file with nothing to report — which is how the inconsistency was found.
