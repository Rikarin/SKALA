...
internal static class AsyncContext {
    ...
    ///     <see cref="NullComparison.IsExpressionTreeType" />. <c>AnonymousFunctionExpressionSyntax</c> is
    ///     the base of the three node kinds the other copy lists, so the walks were already the same set.
    /// </remarks>
    public static bool InsideExpressionTree(SemanticModel model, SyntaxNode node, CancellationToken cancellation) {
        for (var current = node.Parent; current is not null; current = current.Parent) {
            if (current is AnonymousFunctionExpressionSyntax
                && NullComparison.IsExpressionTreeType(model.GetTypeInfo(current, cancellation).ConvertedType)) {
                return true;
            }
        }
