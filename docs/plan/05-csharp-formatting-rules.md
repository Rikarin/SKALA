...
reason recorded rather than Tier A.** M1 found five: `skala_blank_lines_inside_type` and
`skala_blank_lines_inside_namespace` (`remove_blank_lines_near_braces` wins over both by the documented
...
`skala_space_in_singleline_method`, whose shape no longer exists in any output, and
`skala_place_simple_list_pattern_on_single_line`, which `skala_keep_existing_list_patterns_arrangement` outranks
...
[Serializable] class A          // skala_space_after_attributes = true
[A] [B] class C                 // skala_space_between_attribute_sections = true
where T : struct                // skala_space_before_type_parameter_constraint_colon = true
: base(x)                       // space_before_colon_in_ctor_initializer = true
public int X { get; set; }      // skala_space_before_singleline_accessorholder + between_accessors
public void M() { }             // space_within_empty_method_parentheses = false, skala_empty_block_style = together
new[] { 1, 2, 3 }               // skala_space_within_single_line_array_initializer_braces = true
```
...
inside of every parenthesis by `skala_space_within_parentheses`, which was right for the export's values
and wrong about which key produced them. The oracle answers each construct separately —
`skala_space_before_if_parentheses = false` gives `if(n > 0)` and leaves `while (…)` alone,
`skala_space_within_if_parentheses = true` gives `if ( n > 0 )` and touches nothing else — so a rule
...
`skala_space_within_array_rank_brackets` and its empty twin, and `skala_space_around_dot`. Fidelity cannot see
...
⚠ **An accessor body's braces are `skala_space_in_singleline_accessorholder`'s, not
`skala_space_in_singleline_method`'s.** `get { return _n; }` is the one single-line body any input
...
`skala_space_before_pointer_asterik_declaration` writes `int*p`; a function pointer's asterisk hangs from
...
`skala_space_before_array_access_brackets` has no gap to govern and `{ [a] = 1, [b] = 2 }` lost the space
after the comma; and `new (string Name, int Value)[]` opens a tuple *type*, which
`skala_space_before_new_parentheses` has nothing to say about.
...
```csharp
class C {                          // skala_remove_blank_lines_near_braces_in_declarations: no blank after {
    int _a;
    int _b;                        // around_field = 1, but adjacent single-line fields: 0
                                   // (around_single_line_field), so these stay tight
                                   // ← around_invocable = 1
    /// <summary>Docs.</summary>   // skala_stick_comment = true: the blank goes ABOVE the comment
    public void M() { }
...
`csharp_new_line_before_open_brace = none` (K&R), `skala_new_line_before_else/catch/finally = false`,
...
`skala_indent_preprocessor_if = no_indent` and `skala_indent_preprocessor_other = no_indent` (directives at
column 0) but `skala_indent_preprocessor_region = usual_indent` (regions indent with code),
`indent_case_from_switch` via `skala_indent_switch_labels = true`, `csharp_indent_braces = false`,
`resharper_indent_wrapped_function_names = false`, `skala_outdent_binary_ops = false`,
`resharper_outdent_commas = false`, `skala_outdent_dots` — the outdent family is all off, which
...
- `skala_outdent_statement_labels` is implemented: it is one `Indent(Outdent)` scope around a label's two
...
- `skala_outdent_binary_ops` and `skala_outdent_dots` are **implemented** as of T5b, and so is
  `skala_outdent_binary_pattern_ops`. The sentence they used to carry — "each moves the wrapped operator
  ...
  skala_outdent_dots                `.`   12 → 11      (1 + 0: space_after_dot = false)
  ```
...
- `skala_outdent_binary_pattern_ops` was "unverified: no input has yet been found that both wraps a binary
  ...
  about the probes.
- `outdent_commas` is masked under this export and observable with `skala_wrap_before_comma = true`, which
...
The seven `indent_*_pars` and `indent_*_angles` keys divide the same way: `skala_indent_invocation_pars`
and `skala_indent_method_decl_pars` are observable and place a closing delimiter, which is the wrapping
pass's; `skala_indent_primary_constructor_decl_pars` needs
`skala_wrap_before_primary_constructor_declaration_rpar = true` to have a delimiter on its own line at
all; and `skala_indent_pars`, `skala_indent_statement_pars`, `skala_indent_typearg_angles` and `skala_indent_typeparam_angles`
...
`skala_place_simple_embedded_statement_on_same_line = if_owner_is_single_line`,
`skala_place_simple_case_statement_on_same_line = if_owner_is_single_line`,
`skala_place_type_constraints_on_same_line = true`, `skala_place_constructor_initializer_on_same_line = true`,
`place_primary_constructor_initializer_on_same_line = true`,
`skala_wrap_enum_declaration = chop_always` with `skala_max_enum_members_on_line = 1` (one enum member per
line), `skala_blank_lines_after_file_scoped_namespace_directive = 1`.
...
- **`resharper_new_line_before_enumerators` is not in `options.json`.** ⚠ It is, from M3: the
  importer's blind spot is repaired and 37 keys are registered, `skala_prefer_wrap_around_eq`
...
- **`place_*_on_same_line = true` is permissive, not mandatory.** `skala_place_type_constraints_on_same_line`
  and `skala_place_constructor_initializer_on_same_line` at `true` do not *join* a `where` clause or a
...
- ⚠ **`csharp_new_line_between_query_expression_clauses` and `skala_place_linq_into_on_new_line` were
  ...
  the line runs out. `skala_place_linq_into_on_new_line` decides the *continuation's* `into`
  ...
  Tier A, pinned by `constructs/wrapping/linq-query.cs`; so are `skala_align_linq_query` and
  `skala_wrap_before_linq_expression`, which were Tier D behind the missing break point rather than behind
  anything to do with the keys. `BreakPlan.PlanQuery` is the break point.

### `place_*` and `if_owner_is_single_line`

Five keys — `skala_place_expr_method_on_single_line`, `…_property_…`, `…_accessor_…`,
`skala_place_simple_case_statement_on_same_line`, `skala_place_simple_embedded_statement_on_same_line` — all set
...
importer dropped. ⚠ `skala_place_attribute_on_same_line` (the language-agnostic one) *is* in the
registry and is Tier D: the six per-owner keys cover every C# attribute target, so it never gets to
decide. `skala_max_attribute_length_for_same_line` is Tier D for the same kind of reason — a length
...
Read off the fixtures: with `skala_keep_existing_invocation_parens_arrangement = false`, `Foo(\n a)`
...
⚠ `skala_keep_existing_linebreaks` reads like one of the family and is not: it is the
...
|---|---|---|
| `skala_keep_existing_invocation_parens_arrangement` and the rest of the delimited family | the construct's delimiters | ⚠ no — `place_simple_*_on_single_line` is inert while keep is on |
| `skala_keep_existing_expr_member_arrangement` | the gap after a member's `=>` | ⚠ no — same |
| `skala_keep_existing_embedded_arrangement` | the gap before an embedded statement | ✅ **yes** |

`if (depth < 0) throw new ArgumentOutOfRangeException(…);` written on one 168-column line comes back
from the oracle with the `throw` on a line of its own, under `skala_keep_existing_embedded_arrangement =
true`. The key says the author's break is not *removed*; it does not say a break may not be added,
and `skala_place_simple_embedded_statement_on_same_line = if_owner_is_single_line` then does exactly what
...
⚠ **`skala_keep_existing_list_patterns_arrangement` preserves each *individual* item gap**, which a fill
...
The 47 `wrap_*` keys, plus the `max_*_on_line` counters, plus `skala_max_line_length = 120`. This
...
- **The counters are not width tests.** `skala_max_initializer_elements_on_line = 4` chops five elements
  onto five lines at 41 columns wide, and `skala_max_array_initializer_elements_on_line = 10000` leaves the
...
- **A property in a call chain travels with the call it feeds**, not with the call before it —
  ...
  `skala_wrap_after_property_in_chained_method_calls = false` reads like.
- **`skala_keep_existing_switch_expression_arrangement` outranks `chop_always`.** With it on,
...
- **A chain of ternaries is a list rather than a staircase, and it is a different construct from a
  ...
  `:`, sized by `skala_wrap_ternary_expr_style`; a chain of them wraps **after each `:`**, one member per
  line, and neither `skala_wrap_ternary_expr_style` nor `skala_wrap_before_ternary_opsigns` moves any of it —
...
`skala_keep_existing_declaration_block_arrangement` and `skala_keep_existing_embedded_block_arrangement` gate the
rule.
...
|---|---|---|
| Invocation arguments | `skala_wrap_arguments_style` | `chop_if_long` |
| Declaration parameters | `skala_wrap_parameters_style` | `chop_if_long` |
| Primary-ctor parameters | `skala_wrap_primary_constructor_parameters_style` | `chop_if_long` |
| Base list | `skala_wrap_extends_list_style` | `chop_if_long` |
| Chained calls | `skala_wrap_chained_method_calls` | `chop_if_long` |
| Ternary | `skala_wrap_ternary_expr_style` | `chop_if_long` |
| Multiple declarators | `skala_wrap_multiple_declaration_style` | `chop_if_long` |
| Enum members | `wrap_enumeration_style` | `chop_if_long` |
| **Switch expression** | `skala_wrap_switch_expression` | `chop_always` |
| Array initializer | `skala_wrap_array_initializer_style` | `wrap_if_long` (fill) |
| Ctor initializer | `wrap_ctor_initializer_style` | `wrap_if_long` |
| Base clause | `wrap_base_clause_style` | `wrap_if_long` |
| Initializer elements per line | `skala_max_initializer_elements_on_line` | 4 |
...
`skala_wrap_before_binary_opsign = true` (operator starts the new line),
`skala_wrap_after_dot_in_method_calls = false` (the `.` starts the new line),
`skala_wrap_before_first_method_call = false`, `skala_wrap_after_invocation_lpar = true` and
`skala_wrap_before_invocation_rpar = true` (so a chopped call puts `(` at the end of the first line
and `)` on its own), same for declarations, `skala_wrap_before_comma = false`,
`skala_wrap_before_arrow_with_expressions`, `skala_wrap_before_extends_colon`, `skala_wrap_before_ternary_opsigns`,
`prefer_line_break_after_multiline_lparen = true`.

Which yields, at 120 columns:

```csharp
// chop_if_long, skala_wrap_after_invocation_lpar, skala_wrap_before_invocation_rpar, skala_wrap_before_comma = false
...
// chained calls, chop_if_long, wrap_after_dot = false, skala_wrap_before_first_method_call = false
...
`…_parameter`, `…_calls_chain`, `…_expression`, `skala_align_multiline_binary_expressions_chain`,
`skala_align_multiline_switch_expression` all `false`; `align_multiline_type_argument = true` and
...
**`skala_align_multiline_statement_conditions`**. Measured at milestone 3.1, before implementing anything:
...
masked by `skala_align_multiline_statement_conditions` and returns the same file at either value. The gap
was a missing break point at the header's `;`, which `skala_wrap_for_stmt_header_style` governs and which
milestone 3.2 built.
...
as XML, re-wrap text to `skala_xmldoc_max_line_length = 120`, break before
`summary,remarks,example,returns,param,typeparam,value,para`, `skala_xmldoc_max_blank_lines_between_tags = 0`,
`skala_xmldoc_indent_child_elements`/`skala_xmldoc_attribute_indent = single_indent`,
`skala_xmldoc_space_before_self_closing = true`, `space_after_triple_slash = true`.
...
`skala_space_after_triple_slash` stays demoted". That is a fact about which fixtures happened to
