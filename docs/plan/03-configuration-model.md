...
truth for what an option *is*, and one file beside it for what another tool calls it — see "The
export bridge" below:
...
### The export bridge, and why `options.json` names no other tool

⚠ **`options.json` mentioned ReSharper 1 489 times and now mentions it zero times.** The four ways it
did, and what happened to each:

| Field | Count | Outcome |
|---|---:|---|
| `export` | 394 options, 1 429 spellings | **Relocated** to `Core/Rikarin.Skala.Options/export-bridge.json` |
| `docs` | 368 of 402 values | **Deleted**, field and all |
| prose | 70 mentions | **Rewritten**, every measurement kept |
| `construct` | 0 | already clean |

The export spellings could not be deleted, and that is the whole reason the bridge is a file rather
than nothing. `jb cleanupcode` understands `resharper_*` and no other namespace; handed a `skala_*`
key it **ignores the line without a word** and formats at whatever the export already said. So a
deleted spelling makes the key-flip sweep report its option `INERT` on a silent success, and
`CanonicalEditorConfig.Compose` drops the line from the payload `skala config sync` ships. The
mapping is also **not derivable**: `skala_x` is `resharper_x` for some keys and `resharper_csharp_x`
for others, and most keys carry both plus unprefixed forms.

⚠ **The bridge is `export-bridge.json`, not `resharper-bridge.json`, and the reason only appears
when you try the other name.** `options.json` has to tell a reader where the `export` array went, and
a filename carrying the vendor's name puts the word straight back into the one file this exercise is
about. It also must not end in `options.json`: the generator picks the registry out of
`AdditionalTexts` by filename suffix, so `export-options.json` would match both filters and it would
read one file as the other.

`OptionsGenerator` joins the two on the option key and checks the join **both ways** — `SKG006` if
the bridge is missing, `SKG007` if an option has no entry or an entry names no option. Both
directions, because neither failure raises anything at runtime.

The `docs` field went because Skala owns these options now: 368 of its 402 values pointed at
`jetbrains.com/help/resharper/…`, at a page describing the key under the one spelling Skala does not
read. Neither `skala explain <option>` nor the generated page shows anything in its place — the
summary carries the measurement, `oracle` names the fixture that pins it, `inert` says when no input
can move it, and a "see also" that lands the reader on another tool's configuration advice is worse
than no line. ⚠ **The other 34 values were not JetBrains links and are lost with them** — 29
`learn.microsoft.com` code-style pages and 5 `spec.editorconfig.org`. That is the price of removing
the field rather than nulling 368 of its values, and it is recorded rather than glossed; recovering
them means reintroducing `docs` for the ~34 entries where the option really is somebody else's.

What was measured, 2026-09-04, before saying the relocation was safe:

- `grep -ci resharper Core/Rikarin.Skala.Options/options.json`: **1 133 → 0**.
- `Distribution/Rikarin.Skala.Canonical/canonical.editorconfig` regenerated by `./build.sh Canonical`:
  **byte-identical**, same 414 lines, same `sha256:70934c79…`, same 377 assignments over 3 sections.
  This is the check that mattered — `Translate` resolves every export key through the same
  specificity ranks it did when the array lived in `options.json`.
- The oracle at 2025.2.6, driven through `OracleRunner` over the 114 inputs of
  `corpus/constructs/spaces/`: **114/114 byte-identical to the committed `.expected.cs`**.
- Negative control, same run: the same option written as the raw `skala_space_within_empty_braces =
  false` straight into the configuration, bypassing the bridge — **all 114 identical to the
  unconfigured run**, i.e. the oracle silently ignored it. Routed through the bridge instead, 20
  files move and `void M() { }` becomes `void M() {}`. The bridge is what makes the oracle
  configurable, demonstrated rather than asserted.
- The provenance digest is `sha256("root = true\n\n" + editor_config_template)[..16]` =
  `14c031ee7ef4b616`, recomputed and unmoved; neither this change nor anything in it touches
  `editor_config_template`.

⚠ Adding support for an option is therefore: add the JSON entry, **add its export spelling to
`export-bridge.json`**, add the corpus file, implement it, and add it to
`PhaseOneOptions.Implemented` or `ArrangementOptions.Implemented`. **Forgetting any of them fails a
test, and forgetting the bridge entry additionally fails to compile** — `SKG007`, which is the one
step whose omission is otherwise invisible, because the oracle answers a key it cannot read with a
plausible unconfigured result. There is no `[MustHandle]` attribute anywhere in the tree — the
sentence that used to stand here described an exhaustiveness check that was never written. The
generator's real build-failure diagnostics are `SKG001`–`SKG007` and `SK9004`, and they are about
the registry files rather than about implementations.
