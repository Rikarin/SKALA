...
prefix: the exit code for "this file needs formatting" changed from 1 to 2 as a *fix*, and
`dotnet_style_require_accessibility_modifiers` changed type as part of *implementing* it. Both are
`fix:` or `feat:` by any honest reading of the change, and both break a consumer.
...
| Option A→D, or a default or type changes | `options.json` | **major**; D→A is **minor** |
...
| **Compare the formatter's *source* between releases** | Every commit to `Formatting/` would be a minor. Most are not: the last four milestones include refactors that moved no bytes and one-line fixes that moved 7 000. Source diff has no denominator. |
...
| option registry | **major** | 84 changes: 75 tier moves (all `D → A`), 1 type change, 8 other |
...
⚠ **The 84 option changes are all in the safe direction** — 75 keys moved `D → A`, meaning they are
honoured now and were inert before. That is `minor` by the table. The single `major` in that surface
is `dotnet_style_require_accessibility_modifiers` changing type from `string` to an enum: a value a
repository has written down may no longer parse.
