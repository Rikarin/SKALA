...
| `resharper_csharp_blank_lines_around_single_line_local_method` | `0` | sweep `Divergent` on `constructs/blank-lines/resharper_csharp_blank_lines_around_single_line_local_method.cs`; **disagrees at the export's `0`**, 2/3 values agree, baseline already diverged | S — wired already; a fidelity fix in an existing rule |
